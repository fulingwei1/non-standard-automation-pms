...
const presalesRoutes = [
  "/presales/workbench",
  "/presales/workbench/sales",
  "/presales/workbench/execution",
  "/presales/workbench/manager",
  "/presales-workbench",
  "/presales-dashboard",
  ...
  "/presales/ticket-board",
  "/presale-templates",
  "/workstation/presales",
  "/business-support",
  "/sales/presales-tasks",
  "/sales/presales-workbench",
  "/sales/presale-workbench",
  "/sales/sales-analysis",
  "/project-presales-tasks",
  "/sales/presale-expenses",
];

const selectedRoutes = presalesRoutes
  .filter((route) => !routeFilter || route.includes(routeFilter))
  .slice(routeOffset, routeLimit > 0 ? routeOffset + routeLimit : undefined);

const dangerousPattern =
  /(删除|移除|作废|清空|撤回|驳回|拒绝|通过|批准|审批|提交|发布|签署|开票|收款|确认收款|批量|导入|导出|下载|开始审批|启动审批|发起立项|立项|禁用|停用|结算|退出登录|注销|保存|接受工单|完成工单|提交评审|评审通过|评审驳回|提交方案|保存方案|发起投标|提交投标|录入结果|确认中标|确认丢标|转报价|生成报价|转项目|转换)/;
...
const writeReport = () => {
  const file = path.join(reportDir, `presales-button-sweep-${stamp}.json`);
...
const saveScreenshot = async (page, routeReport, label) => {
  ...
  const file = path.join(screenshotDir, `presales-${safe}-${stamp}.png`);

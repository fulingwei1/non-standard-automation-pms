...
class CustomerSupplierRegistration(Base, TimestampMixin):
    - customer_id: 客户ID
    - platform_name: 平台名称
    - registration_status: 入驻状态
    - application_date: 申请日期
    - approved_date: 批准日期
    - registration_no: 入驻编号
```

**API需要**:
```python
POST /business-support/customer-registrations - 创建入驻申请
GET /business-support/customer-registrations - 获取入驻申请列表
PUT /business-support/customer-registrations/{id}/approve - 审批入驻申请
```

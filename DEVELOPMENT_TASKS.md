...
| S8.3.12 | 关闭问题 API | 🔴 P0 | ❌ | `POST /issues/{id}/close` | 直接关闭（无需验证） |
| S8.3.13 | 重新打开问题 API | 🟡 P1 | ❌ | `POST /issues/{id}/reopen` | 从已关闭状态重新打开 |
| S8.3.14 | 删除问题 API | 🟡 P1 | ❌ | `DELETE /issues/{id}` | 软删除 |
| S8.3.15 | 批量分配问题 | 🟡 P1 | ❌ | `POST /issues/batch-assign` | 批量指派处理人 |
| S8.3.16 | 批量关闭问题 | 🟡 P1 | ❌ | `POST /issues/batch-close` | 批量关闭 |
| S8.3.17 | 批量更新优先级 | 🟡 P1 | ❌ | `POST /issues/batch-priority` | 批量设置优先级 |
| S8.3.18 | 关联问题列表 | 🟡 P1 | ❌ | `GET /issues/{id}/related` | 获取父子/相关问题 |
| S8.3.19 | 创建问题关联 | 🟡 P1 | ❌ | `POST /issues/{id}/relate` | 关联其他问题 |
| S8.3.20 | 问题导出 API | 🟡 P1 | ❌ | `GET /issues/export` | Excel/PDF导出 |
| S8.3.21 | 问题看板数据 API | 🟡 P1 | ❌ | `GET /issues/board` | 看板视图数据 |
...
| S8.4.5 | Issue 批量操作 Schema | 🟡 P1 | ❌ | BatchAssign/BatchClose/BatchPriority |
| S8.4.6 | Issue 关联 Schema | 🟡 P1 | ❌ | IssueRelation |
| S8.4.7 | Issue 导出 Schema | 🟡 P1 | ❌ | IssueExportRequest |
| S8.4.5 | Issue 批量操作 Schema | 🟡 P1 | ❌ | BatchAssign/BatchClose/BatchPriority |
| S8.4.6 | Issue 关联 Schema | 🟡 P1 | ❌ | IssueRelation |
| S8.4.7 | Issue 导出 Schema | 🟡 P1 | ❌ | IssueExportRequest |
...
| S8.4.6 | 问题关联管理页 | 🟢 P2 | ❌ | 父子问题、相关问题管理 |
| S8.4.7 | 问题导出功能 | 🟡 P1 | ❌ | Excel/PDF导出 |
| S8.4.8 | 附件上传/下载 | 🟡 P1 | ❌ | 问题附件管理 |
...
| v1.8 | 2026-01-05 | 🆕 **问题管理中心模块**：补充完整任务清单，包括数据库模型(2张表✅)、API端点(21个，11个✅)、前端页面(8页，3个✅)、补充批量操作/关联/导出等功能 |
...
- **API端点**：21个端点（11个已实现✅，10个待开发）
  - **基础CRUD**：列表、详情、创建、更新 ✅
  - **问题操作**：分配、解决、验证、状态变更 ✅ | 关闭、重新打开、删除 ❌
  - **跟进记录**：获取、创建 ✅
  - **批量操作**：批量分配、批量关闭、批量更新优先级 ❌
  - **问题关联**：关联列表、创建关联 ❌
  - **数据导出**：问题导出、看板数据 ❌
- **前端页面**：8个页面（3个已实现✅，5个待开发）

...
> 最后更新：2026-01-06（已更新任务状态）
> 版本：v1.30
> 最新统计：836个API端点，44个端点文件，95个前端页面，整体完成度约96%
> 🎉 **P1/P2扩展功能全面完成**：完成个人任务中心批量操作API（新增批量催办功能）、物料调拨管理API、PMO项目结项管理API、绩效管理API（9个端点全部完成）、报表中心API（9个端点全部完成）、设备管理API（6个端点全部完成）、销售报表API（4个端点全部完成）。数据导入导出API框架已实现。
> 重大进展：✅ **整体完成度提升至92%**（819个API端点），✅ **20个模块100%完成**（生产管理95个端点、销售管理74个端点、缺料管理31个端点、验收管理21个端点、ECN管理27个端点、外协管理22个端点、问题管理34个端点、预警与异常24个端点、个人任务中心29个端点、PMO项目管理部38个端点、售前技术支持30个端点、绩效管理9个端点、报表中心15个端点、数据导入导出10个端点、研发项目管理23个端点、售后/知识库29个端点、工时管理13个端点、设备管理6个端点、销售报表4个端点、物料调拨4个端点），✅ **项目健康度自动计算100%完成**（定时任务已配置，每小时执行），✅ **10个P0预警服务100%完成**（缺料预警、任务延期、生产计划、报工超时、进度汇总、齐套检查、到货延迟、任务到期提醒、外协交期、里程碑风险），⚠️ **项目管理模块85%完成**（32个端点，核心功能全部完成），⚠️ **进度跟踪模块85%完成**（43个端点，核心功能全部完成，待补充P2扩展功能）
...
> **最后更新**: 2026-01-06（最新评估）  
> **代码统计**: 837个API端点，44个端点文件，95个前端页面
> **整体完成度**: 约97%（核心功能99%，扩展功能95%）

| 层级 | 模块 | 完成度 | 说明 |
|------|------|:------:|------|
| 数据库模型 | 核心业务模型 | ✅ 97% | 项目/采购/ECN/验收/外协/预警/生产/PMO/技术规格/问题管理等 |
| 数据库模型 | 缺料管理模型 | ✅ 100% | 🆕 10张表全部实现（含5张扩展表） |
| 数据库模型 | 研发项目模型 | ❌ 0% | 🆕 需新建6张专用表 |
| 数据库模型 | 售后/知识库模型 | ❌ 0% | 🆕 需新建6张专用表 |
| 数据库模型 | 项目复盘模型 | ❌ 0% | 🆕 需新建3张专用表 |
| 数据库迁移 | SQL 迁移脚本 | ⚠️ 85% | 部分新表待生成迁移脚本 |
| Pydantic Schema | API 数据模式 | ⚠️ 85% | 大部分模块已补充 |
| API 端点 | 项目管理 | ✅ 100% | 🆕 已实现51个端点（基础CRUD + 看板 + 统计 + 健康度自动计算 + 里程碑预警 + 成本超支预警 + 模板管理 + 复制克隆 + 归档 + 资源优化 + 关联分析 + 风险矩阵 + 变更影响分析 + 项目概览 + 时间线 + 阶段推进 + 三维状态查询 + 项目导出 + 项目导入 + 批量操作 + 项目仪表盘 + 项目复盘） |
| API 端点 | 缺料管理 | ✅ 100% | 🆕 已实现31个端点（齐套检查/上报/跟踪/替代/调拨/统计） |
| API 端点 | 进度跟踪 | ⚠️ 85% | 🆕 已实现43个端点（WBS模板、任务管理、进度填报、看板、基线对比、偏差分析、关键路径计算、任务阻塞/取消/删除、依赖删除、报告更新/删除/统计、进度日志查询、批量操作、完成率统计） |
| API 端点 | 生产管理 | ✅ 100% | 🆕 已实现95个端点（核心模块，包含工序字典） |
| API 端点 | 销售管理(O2C) | ✅ 100% | 🆕 已实现74个端点（线索到回款，包含收款计划管理、销售报表） |
| API 端点 | 验收管理 | ✅ 100% | 🆕 已实现21个端点（包含签字、报告生成，代码优化，1420行） |
| API 端点 | ECN管理 | ✅ 100% | 🆕 已实现27个端点（包含评估、审批、执行、追溯、统计） |
| API 端点 | 外协管理 | ✅ 100% | 🆕 已实现22个端点（包含供应商、订单、交付、质检、进度、付款、对账单） |
| API 端点 | 预警与异常 | ✅ 100% | 🆕 已实现24个端点（预警规则5个+预警记录6个+异常事件7个+健康度快照2个+统计2个+通知2个） |
| API 端点 | 问题管理 | ✅ 100% | 🆕 已实现34个端点（28个基础+6个模板管理）+ 前端95%完成 |
| API 端点 | 个人任务中心 | ✅ 100% | 🆕 已实现29个端点（包含批量操作10个端点：完成、转办、优先级、进度、催办、删除、开始、暂停、标签、统计） |
| API 端点 | PMO项目管理部 | ✅ 100% | 🆕 已实现38个端点（资源分配、项目结项5个、文档归档、PMO驾驶舱、周报、资源负荷总览、风险预警墙） |
| API 端点 | 售前技术支持 | ✅ 100% | 🆕 已实现30个端点（工单管理、方案管理、模板管理、投标管理、模板使用统计、投标分析报表） |
| API 端点 | 工时管理详细 | ✅ 100% | 🆕 已实现13个端点（包含月度汇总、待审核列表、统计分析） |
| API 端点 | 绩效管理 | ✅ 100% | 🆕 已实现9个端点（个人绩效、团队绩效、部门绩效、项目绩效、绩效对比、趋势分析、排行榜） |
| API 端点 | 报表中心 | ✅ 100% | 🆕 已实现15个端点（报表配置3个、报表生成4个、报表模板2个、研发费用报表6个） |
| API 端点 | 数据导入导出 | ✅ 100% | 🆕 已实现10个端点（导入模板、预览验证、上传导入、导出功能全部完成，Excel处理逻辑已完善） |
| API 端点 | 设备管理 | ✅ 100% | 🆕 已实现6个端点（设备列表、创建、详情、状态更新、保养记录查询/添加） |
| API 端点 | 销售报表 | ✅ 100% | 🆕 已实现4个端点（销售漏斗、赢单/丢单分析、销售业绩统计、客户贡献分析） |
| API 端点 | 物料调拨 | ✅ 100% | 🆕 已实现4个端点（调拨申请列表、创建、审批、执行） |
| API 端点 | 商务支持 | ✅ 85% | 🆕 已实现31个端点（business_support.py 13个 + business_support_orders.py 18个） |
| API 端点 | 研发项目管理 | ✅ 100% | 🆕 已实现23个端点（立项、审批、结项、费用归集、工时汇总、研发费用报表6个） |
| API 端点 | 售后/知识库 | ✅ 100% | 🆕 已实现29个端点（服务工单、现场服务、客户沟通、满意度调查、知识库管理） |
| API 端点 | 通知中心 | ✅ 100% | 🆕 已实现8个端点（全部完成） |
| 前端界面 | Web UI | ✅ 96% | 🆕 已实现95个页面，核心页面已连接API，WBS模板管理和齐套看板页面已完善 |
| 后台服务 | 健康度自动计算 | ✅ 100% | 🆕 已完全实现（服务+API+定时任务） |
| 后台服务 | 健康度快照 | ✅ 100% | 🆕 已完全实现 |
| 后台服务 | 定时任务框架 | ✅ 100% | 🆕 APScheduler已集成 |
| 后台服务 | 其他预警服务 | ✅ 100% | 已实现10个P0预警服务 |
| 移动端 | 企微/钉钉 | ❌ 0% | 未开始 |
| 系统集成 | ERP/PDM | ❌ 0% | 未开始 |
...
| 0.1.7 | 问题编码规则 (ISyymmddxxx) | 🔴 P0 | ✅ | 编码规范文档 |
...
| 0.2.5 | 健康度自动计算规则 | 🟡 P1 | ✅ | 已实现（HealthCalculator服务） |
...
| 1.1.15 | 数据权限服务 | 🔴 P0 | ✅ | `DataScopeService` | 实现数据权限过滤（ALL/DEPT/PROJECT/OWN/CUSTOMER） |
| 1.1.16 | 权限审计服务 | 🔴 P0 | ✅ | `PermissionAuditService` | 记录权限操作审计日志 |
| 1.1.17 | 权限审计查询 API | 🟡 P1 | ✅ | `GET /audits` | 查询权限审计日志 |
| 1.1.18 | 项目数据权限过滤 | 🔴 P0 | ✅ | `GET /projects` | 项目列表自动应用数据权限过滤 |
...
| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| 1.3.1 | 项目列表 API | 🔴 P0 | ✅ | `GET /projects` | 支持多维度筛选 |
| 1.3.2 | 项目详情 API | 🔴 P0 | ✅ | `GET /projects/{id}` | 包含关联数据 |
| 1.3.3 | 创建项目 API | 🔴 P0 | ✅ | `POST /projects` | 已完善字段 |
| 1.3.4 | 更新项目 API | 🔴 P0 | ✅ | `PUT /projects/{id}` | 已完成 |
...
| 1.3.11 | 健康度自动计算 | 🟡 P1 | ✅ | 后台服务 | 规则引擎（已实现） |
...
| 1.3.22 | 阶段状态列表 | 🟡 P1 | ✅ | `GET /project-stages/{stage_id}/statuses` |
...
| 1.3.27 | 完成里程碑 | 🔴 P0 | ✅ | `PUT /milestones/{id}/complete` | 包含自动触发开票 |
| 1.3.28 | 里程碑预警 | 🟡 P1 | ✅ | 后台服务 | 每天8点执行，检查即将到期和已逾期里程碑 |
...
| 1.3.36 | 成本超支预警 | 🟢 P2 | ✅ | 后台服务 | 每天10点执行，检查成本超支超过5%的项目 |
...
| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| 1.3.41 | 项目概览数据 | 🟡 P1 | ✅ | `GET /projects/{id}/summary` | 包含统计数据、成本汇总、最近活动 |
| 1.3.42 | 项目时间线 | 🟡 P1 | ✅ | `GET /projects/{id}/timeline` | 项目生命周期事件时间线 |
| 1.3.43 | 获取项目三维状态 | 🟡 P1 | ✅ | `GET /projects/{id}/status` | 获取当前阶段、状态、健康度 |
| 1.3.44 | 阶段推进（含阶段门校验） | 🟡 P1 | ✅ | `POST /projects/{id}/stage-advance` | 8个阶段门校验规则 |
| 1.3.45 | 项目统计（按客户/月份） | 🟡 P1 | ✅ | `GET /projects/stats?group_by=customer/month` | 支持按客户和按月份统计 |
| 1.3.46 | 项目列表导出 | 🟡 P1 | ✅ | `POST /export/project_list` | Excel导出，支持筛选条件 |
| 1.3.47 | 项目详情导出 | 🟡 P1 | ✅ | `POST /export/project_detail` | Excel导出，包含任务和成本 |
| 1.3.48 | 项目导入模板下载 | 🟡 P1 | ✅ | `GET /templates/PROJECT` | Excel模板下载 |
| 1.3.49 | 项目导入预览 | 🟡 P1 | ✅ | `POST /preview` | 预览导入数据，验证格式 |
| 1.3.50 | 项目批量导入 | 🟡 P1 | ✅ | `POST /upload?template_type=PROJECT` | 从Excel批量导入项目 |

#### 4.3.10 项目批量操作

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| 1.3.51 | 批量更新项目状态 | 🟡 P1 | ✅ | `POST /projects/batch/update-status` | 批量更新项目状态（ST01-ST30） |
| 1.3.52 | 批量更新项目阶段 | 🟡 P1 | ✅ | `POST /projects/batch/update-stage` | 批量更新项目阶段（S1-S9） |
| 1.3.53 | 批量分配项目经理 | 🟡 P1 | ✅ | `POST /projects/batch/assign-pm` | 批量分配项目经理 |
| 1.3.54 | 批量归档项目 | 🟡 P1 | ✅ | `POST /projects/batch/archive` | 批量归档项目 |

#### 4.3.11 项目仪表盘与可视化

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| 1.3.55 | 项目仪表盘 | 🟢 P2 | ✅ | `GET /projects/{id}/dashboard` | 项目数据聚合，包含进度/成本/任务/里程碑/风险/问题等 |
| 1.3.56 | 项目甘特图数据 | 🟢 P2 | ✅ | `GET /projects/{id}/gantt` | 已在进度跟踪模块实现 |

#### 4.3.12 项目复盘管理

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| 1.3.57 | 复盘报告列表 | 🟢 P2 | ✅ | `GET /project-reviews` | 获取项目复盘报告列表，支持分页和筛选 |
| 1.3.58 | 创建复盘报告 | 🟢 P2 | ✅ | `POST /project-reviews` | 基于项目结项记录创建复盘报告 |
| 1.3.59 | 复盘报告详情 | 🟢 P2 | ✅ | `GET /project-reviews/{id}` | 获取复盘报告详情 |
| 1.3.60 | 项目经验教训 | 🟢 P2 | ✅ | `GET /projects/{id}/lessons-learned` | 从问题记录和结项记录中提取经验教训 |
...
| 1.4.7 | 物料替代关系 | 🟢 P2 | ✅ | `GET /materials/{id}/alternatives` | 查询物料的替代关系（可替代的物料和可被替代的物料） |
...
| 1.4.26 | 到货状态更新 | 🔴 P0 | ✅ | `PUT /purchase-order-items/{item_id}/receive` |
...
| 1.4.42 | 更新项目健康度 | 🔴 P0 | ✅ | 后台服务 | 缺料→H3阻塞（定时任务已配置，每小时执行） |
...
> 功能说明：对客户签订的技术规格书和采购订单/BOM进行不一致性匹配，进行风险预警
> 参考设计文档：技术规格与采购订单匹配预警功能规划

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| 1.4.48 | 技术规格要求列表 | 🔴 P0 | ✅ | `GET /technical-spec/requirements` | 分页+筛选 |
| 1.4.49 | 创建技术规格要求 | 🔴 P0 | ✅ | `POST /technical-spec/requirements` | 手动录入或文档提取 |
| 1.4.50 | 更新技术规格要求 | 🔴 P0 | ✅ | `PUT /technical-spec/requirements/{id}` | |
| 1.4.51 | 删除技术规格要求 | 🔴 P0 | ✅ | `DELETE /technical-spec/requirements/{id}` | |
| 1.4.52 | 从文档提取规格要求 | 🟡 P1 | ✅ | `POST /technical-spec/requirements/extract` | 支持Excel/Word/PDF |
| 1.4.53 | 规格匹配检查 | 🔴 P0 | ✅ | `POST /technical-spec/match/check` | 手动触发检查 |
| 1.4.54 | 匹配记录列表 | 🔴 P0 | ✅ | `GET /technical-spec/match/records` | 查看匹配结果 |
| 1.4.55 | 采购订单创建时自动检查 | 🔴 P0 | ✅ | 集成到采购订单API | 自动匹配并预警 |
| 1.4.56 | BOM审批时自动检查 | 🔴 P0 | ✅ | 集成到BOM审批API | 自动匹配并预警 |
| 1.4.57 | 定时任务检查 | 🟡 P1 | ✅ | 后台定时任务 | 每日检查所有项目 |
| 1.4.58 | 规格不匹配预警规则 | 🔴 P0 | ✅ | 预警系统集成 | 自动生成预警记录 |
...
| 1.5.12a | 任务进度日志 | 🟡 P1 | ✅ | `GET /tasks/{id}/logs` | 🆕 查询任务进度变更历史 |
| 1.5.13 | 更新任务进度 | 🔴 P0 | ✅ | `PUT /tasks/{id}/progress` | 百分比进度 |
| 1.5.13a | 批量更新任务进度 | 🟡 P1 | ✅ | `POST /tasks/batch/progress` | 🆕 批量更新多个任务进度 |
| 1.5.14 | 完成任务 | 🔴 P0 | ✅ | `PUT /tasks/{id}/complete` | |
| 1.5.15 | 任务依赖关系 | 🟡 P1 | ✅ | `GET /tasks/{id}/dependencies` | 前置任务 |
| 1.5.16 | 设置任务依赖 | 🟡 P1 | ✅ | `POST /tasks/{id}/dependencies` | |
| 1.5.16a | 删除任务依赖 | 🟡 P1 | ✅ | `DELETE /task-dependencies/{id}` | 🆕 删除依赖关系 |
| 1.5.17 | 任务负责人分配 | 🔴 P0 | ✅ | `PUT /tasks/{id}/assignee` | |
| 1.5.17a | 批量分配任务负责人 | 🟡 P1 | ✅ | `POST /tasks/batch/assignee` | 🆕 批量分配多个任务负责人 |
| 1.5.17b | 阻塞任务 | 🔴 P0 | ✅ | `PUT /tasks/{id}/block` | 🆕 设置任务为阻塞状态 |
| 1.5.17c | 解除阻塞 | 🔴 P0 | ✅ | `PUT /tasks/{id}/unblock` | 🆕 解除任务阻塞 |
| 1.5.17d | 取消任务 | 🟡 P1 | ✅ | `PUT /tasks/{id}/cancel` | 🆕 取消任务 |
| 1.5.17e | 删除任务 | 🟡 P1 | ✅ | `DELETE /tasks/{id}` | 🆕 删除任务（仅管理员） |
...
| 1.5.20a | 更新进度报告 | 🟡 P1 | ✅ | `PUT /progress-reports/{id}` | 🆕 更新报告内容 |
| 1.5.20b | 删除进度报告 | 🟡 P1 | ✅ | `DELETE /progress-reports/{id}` | 🆕 删除报告 |
| 1.5.20c | 进度报告统计 | 🟡 P1 | ✅ | `GET /progress-reports/statistics` | 🆕 报告统计分析 |
...
| 1.5.26a | 任务完成率统计 | 🟡 P1 | ✅ | `GET /reports/task-completion-rate` | 🆕 按部门/人员统计完成率 |
| 1.5.27 | 延期/阻塞预警 | 🔴 P0 | ✅ | 后台服务 | 自动触发预警（任务延期预警+阻塞问题预警） |
...
| 1.6.19 | 从问题创建异常 | 🟡 P1 | ✅ | `POST /exceptions/from-issue` | 问题转异常事件 |

#### 4.6.4 项目健康度快照

| 序号 | 任务 | 优先级 | 状态 | API 路径 |
|:----:|------|:------:|:----:|----------|
| 1.6.19 | 生成健康度快照 | 🟡 P1 | ✅ | 后台定时任务（已实现） |
| 1.6.20 | 健康度趋势查询 | 🟡 P1 | ✅ | `GET /projects/{id}/health-history` |
| 1.6.21 | 预警统计分析 | 🟢 P2 | ✅ | `GET /alerts/statistics` |
| 1.6.22 | 预警统计看板 | 🟢 P2 | ✅ | `GET /alerts/statistics/dashboard` |
...
| 2.4.7 | 负荷超限预警 | 🟡 P1 | ✅ | 后台服务 | >110%预警，每天11点执行 |
...
| 2.7.11 | 岗位职责任务生成 | 🟡 P1 | ✅ | 后台服务 | 每天凌晨4点执行，根据模板自动生成任务 |
| 2.7.12 | 任务提醒服务 | 🟡 P1 | ✅ | 后台服务 | 任务到期提醒（check_task_deadline_reminder） |
...
| 2.8.11 | 延迟预警 | 🔴 P0 | ✅ | 后台服务 | 自动标记延迟（check_delivery_delay） |
...
| 2.9.2 | 查看指定人员绩效 | 🟡 P1 | ✅ | `GET /performance/user/{id}` | 权限控制（通过/my接口实现） |
...
| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| 2.11.1 | 下载导入模板 | 🟡 P1 | ✅ | `GET /import/templates/{type}` | 按类型下载（项目模板已实现，Excel处理逻辑完善） |
| 2.11.2 | 获取所有模板类型 | 🟡 P1 | ✅ | `GET /import/templates` | 项目/任务/人员/工时等 |
| 2.11.3 | 预览导入数据 | 🟡 P1 | ✅ | `POST /import/preview` | 上传预览（Excel解析、数据验证、错误提示） |
| 2.11.4 | 验证导入数据 | 🟡 P1 | ✅ | `POST /import/validate` | 格式校验（必填字段、日期格式、业务规则验证） |
| 2.11.5 | 上传并导入数据 | 🟡 P1 | ✅ | `POST /import/upload` | 执行导入（项目批量导入已实现，支持更新已存在项目） |

#### 5.11.2 数据导出

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| 2.11.6 | 导出项目列表 | 🟡 P1 | ✅ | `POST /import/export/project_list` | Excel |
| 2.11.7 | 导出项目详情 | 🟡 P1 | ✅ | `POST /import/export/project_detail` | 含任务/成本 |
| 2.11.8 | 导出任务列表 | 🟡 P1 | ✅ | `POST /import/export/task_list` | |
| 2.11.9 | 导出工时数据 | 🟡 P1 | ✅ | `POST /import/export/timesheet` | 按日期范围 |
| 2.11.10 | 导出负荷数据 | 🟡 P1 | ✅ | `POST /import/export/workload` | |
...
| 2.5.37 | 合同付款节点 | 🔴 P0 | ✅ | `GET /projects/{id}/payment-plans` | 已实现为项目收款计划 |
| 2.5.38 | 添加付款节点 | 🔴 P0 | ✅ | `POST /projects/{id}/payment-plans` | 已实现为项目收款计划，绑定里程碑 |
...
| 序号 | 任务 | 优先级 | 状态 | API 路径 |
|:----:|------|:------:|:----:|----------|
| 3.1.1 | 售后工单列表 | 🟡 P1 | ✅ | `GET /service-tickets` |
| 3.1.2 | 创建售后工单 | 🟡 P1 | ✅ | `POST /service-tickets` |
| 3.1.3 | 工单详情 | 🟡 P1 | ✅ | `GET /service-tickets/{id}` |
| 3.1.4 | 分派工单 | 🟡 P1 | ✅ | `PUT /service-tickets/{id}/assign` |
| 3.1.5 | 更新工单状态 | 🟡 P1 | ✅ | `PUT /service-tickets/{id}/status` |
| 3.1.6 | 关闭工单 | 🟡 P1 | ✅ | `PUT /service-tickets/{id}/close` |
...
| 3.2.3 | 成本对比分析 | 🟢 P2 | ✅ | `GET /costs/projects/{id}/cost-analysis` |
...
| 3.4.3 | 项目利润分析 | 🟢 P2 | ✅ | `GET /costs/projects/{id}/profit-analysis` |
| 3.4.4 | 成本趋势分析 | 🟢 P2 | ✅ | `GET /costs/cost-trends` |
...
| 3.5.5 | 决策驾驶舱数据 | 🟢 P2 | ✅ | `GET /reports/dashboard/executive` |
...
| S1.1.4 | 缺料预警模型（扩展） | 🔴 P0 | ✅ | `mat_shortage_alert` | 完整版预警模型，已对齐设计文档 |
| S1.1.5 | 缺料上报模型 | 🔴 P0 | ✅ | `shortage_reports` | 车间上报、紧急程度、图片、确认状态 |
| S1.1.6 | 到货跟踪模型 | 🔴 P0 | ✅ | `material_arrivals` | PO 关联、承诺/预计/实际到货、延迟标记 |
| S1.1.7 | 物料替代申请模型 | 🟡 P1 | ✅ | `material_substitutions` | 原料/替代料、审批流程 |
| S1.1.8 | 物料调拨申请模型 | 🟡 P1 | ✅ | `material_transfers` | 跨工单调拨、审批流程 |
...
| 序号 | 任务 | 优先级 | 状态 | 文件名 |
|:----:|------|:------:|:----:|--------|
| S1.2.1 | 生成 SQLite 迁移脚本 | 🔴 P0 | ✅ | `20260106_shortage_management_sqlite.sql` |
| S1.2.2 | 生成 MySQL 迁移脚本 | 🔴 P0 | ✅ | `20260106_shortage_management_mysql.sql` |
...
| S2.2.4 | 确认上报 | 🔴 P0 | ✅ | `PUT /shortage/reports/{id}/confirm` | 仓管确认 |
| S2.2.5 | 处理上报 | 🔴 P0 | ✅ | `PUT /shortage/reports/{id}/handle` | 开始处理 |
| S2.2.6 | 解决上报 | 🔴 P0 | ✅ | `PUT /shortage/reports/{id}/resolve` | 关闭工单 |
| S2.2.7 | 驳回上报 | 🟡 P1 | ✅ | `PUT /shortage/reports/{id}/reject` | 无效上报 |

#### S2.3 到货跟踪 API

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| S2.3.1 | 到货列表 | 🔴 P0 | ✅ | `GET /shortage/arrivals` | 在途物料列表 |
| S2.3.2 | 到货详情 | 🔴 P0 | ✅ | `GET /shortage/arrivals/{id}` | |
| S2.3.3 | 更新到货状态 | 🔴 P0 | ✅ | `PUT /shortage/arrivals/{id}/status` | 已发货/到厂/入库 |
| S2.3.4 | 创建跟催记录 | 🔴 P0 | ✅ | `POST /shortage/arrivals/{id}/follow-up` | 跟催供应商 |
| S2.3.5 | 确认收货 | 🔴 P0 | ✅ | `POST /shortage/arrivals/{id}/receive` | 质检+入库 |
| S2.3.6 | 延迟到货列表 | 🟡 P1 | ✅ | `GET /shortage/arrivals/delayed` | 延迟预警 |

#### S2.4 物料替代/调拨 API

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| S2.4.1 | 替代申请列表 | 🟡 P1 | ✅ | `GET /shortage/substitutions` | 已实现 |
| S2.4.2 | 创建替代申请 | 🟡 P1 | ✅ | `POST /shortage/substitutions` | 已实现 |
| S2.4.3 | 审批替代申请 | 🟡 P1 | ✅ | `PUT /shortage/substitutions/{id}/tech-approve` | 技术+生产审批，已实现 |
| S2.4.4 | 执行替代 | 🟡 P1 | ✅ | `PUT /shortage/substitutions/{id}/execute` | 已实现 |
| S2.4.5 | 调拨申请列表 | 🟡 P1 | ✅ | `GET /shortage/transfers` | 已实现 |
| S2.4.6 | 创建调拨申请 | 🟡 P1 | ✅ | `POST /shortage/transfers` | 已实现 |
| S2.4.7 | 审批调拨申请 | 🟡 P1 | ✅ | `PUT /shortage/transfers/{id}/approve` | 已实现 |
| S2.4.8 | 执行调拨 | 🟡 P1 | ✅ | `PUT /shortage/transfers/{id}/execute` | 已实现 |

#### S2.5 缺料统计分析 API

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| S2.5.1 | 缺料看板数据 | 🔴 P0 | ✅ | `GET /shortage/dashboard` | 齐套率分布+预警汇总，已实现 |
| S2.5.2 | 齐套率统计 | 🟡 P1 | ✅ | `GET /shortage/statistics/kit-rate` | 按项目/车间/时间，已实现 |
| S2.5.3 | 缺料原因分析 | 🟡 P1 | ✅ | `GET /shortage/cause-analysis` | Top N 原因，已实现 |
| S2.5.4 | 供应商交期分析 | 🟡 P1 | ✅ | `GET /shortage/supplier-delivery` | 准时率排名，已实现 |
| S2.5.5 | 获取缺料日报 | 🟡 P1 | ✅ | `GET /shortage/daily-report` | 每日汇总，已实现 |
...
| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| S7.1 | 批量提醒 | 🟡 P1 | ✅ | `POST /task-center/batch/urge` | 发送催办通知（已实现为批量催办） |
| S7.2 | 批量打标签 | 🟡 P1 | ✅ | `POST /task-center/batch/tag` | 添加/移除标签 |
| S7.3 | 批量启动任务 | 🟡 P1 | ✅ | `POST /task-center/batch/start` | 批量开始 |
| S7.4 | 批量暂停任务 | 🟡 P1 | ✅ | `POST /task-center/batch/pause` | 批量暂停 |
| S7.5 | 批量删除 | 🟢 P2 | ✅ | `POST /task-center/batch/delete` | 仅限个人任务 |
| S7.6 | 批量操作统计 | 🟢 P2 | ✅ | `GET /task-center/batch/statistics` | 操作历史统计 |

---

### S8. 问题管理中心模块 - 数据库与 API 补全 🆕

> 参考设计文档：`问题管理中心模块设计.xlsx`
> 当前状态：数据库模型和 API 已实现，但任务清单缺失

#### S8.1 ORM 模型开发任务

| 序号 | 任务 | 优先级 | 状态 | 表名 | 说明 |
|:----:|------|:------:|:----:|------|------|
| S8.1.1 | 问题主表模型 | 🔴 P0 | ✅ | `issues` | 统一问题管理，支持项目/任务/验收问题 |
| S8.1.2 | 问题跟进记录模型 | 🔴 P0 | ✅ | `issue_follow_up_records` | 跟进记录、状态变更追溯 |

#### S8.2 SQL 迁移脚本任务

| 序号 | 任务 | 优先级 | 状态 | 文件名 |
|:----:|------|:------:|:----:|--------|
| S8.2.1 | 生成 SQLite 迁移脚本 | 🔴 P0 | ✅ | `20260105_issue_management_sqlite.sql` |
| S8.2.2 | 生成 MySQL 迁移脚本 | 🔴 P0 | ✅ | `20260105_issue_management_mysql.sql` |
| S8.2.3 | 生成扩展表 SQLite 迁移脚本 | 🟡 P1 | ✅ | `20260106_issue_snapshot_template_sqlite.sql` |
| S8.2.4 | 生成扩展表 MySQL 迁移脚本 | 🟡 P1 | ✅ | `20260106_issue_snapshot_template_mysql.sql` |

#### S8.3 问题管理 API

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| S8.3.1 | 问题列表 API | 🔴 P0 | ✅ | `GET /issues` | 分页+多维度筛选 |
| S8.3.2 | 问题详情 API | 🔴 P0 | ✅ | `GET /issues/{id}` | |
| S8.3.3 | 创建问题 API | 🔴 P0 | ✅ | `POST /issues` | 自动生成问题编号 |
| S8.3.4 | 更新问题 API | 🔴 P0 | ✅ | `PUT /issues/{id}` | |
| S8.3.5 | 分配问题 API | 🔴 P0 | ✅ | `POST /issues/{id}/assign` | 指派处理人 |
| S8.3.6 | 解决问题 API | 🔴 P0 | ✅ | `POST /issues/{id}/resolve` | 填写解决方案 |
| S8.3.7 | 验证问题 API | 🔴 P0 | ✅ | `POST /issues/{id}/verify` | 验证解决结果 |
| S8.3.8 | 变更问题状态 API | 🔴 P0 | ✅ | `POST /issues/{id}/status` | 状态流转 |
| S8.3.9 | 获取跟进记录 | 🔴 P0 | ✅ | `GET /issues/{id}/follow-ups` | |
| S8.3.10 | 创建跟进记录 | 🔴 P0 | ✅ | `POST /issues/{id}/follow-ups` | |
| S8.3.11 | 问题统计 API | 🟡 P1 | ✅ | `GET /issues/statistics/overview` | 多维度统计 |
| S8.3.12 | 删除问题 API | 🟡 P1 | ✅ | `DELETE /issues/{id}` | 软删除（仅管理员） |
| S8.3.13 | 关闭问题 API | 🔴 P0 | ✅ | `POST /issues/{id}/close` | 直接关闭（无需验证） |
| S8.3.14 | 取消问题 API | 🟡 P1 | ✅ | `POST /issues/{id}/cancel` | 取消/撤销问题 |
| S8.3.15 | 关联问题列表 | 🟡 P1 | ✅ | `GET /issues/{id}/related` | 获取关联的父子问题 |
| S8.3.16 | 创建关联问题 | 🟡 P1 | ✅ | `POST /issues/{id}/related` | 创建子问题或关联问题 |
| S8.3.17 | 项目问题列表 | 🟡 P1 | ✅ | `GET /issues/projects/{id}/issues` | 项目下的所有问题 |
| S8.3.18 | 机台问题列表 | 🟡 P1 | ✅ | `GET /issues/machines/{id}/issues` | 机台下的所有问题 |
| S8.3.19 | 任务问题列表 | 🟡 P1 | ✅ | `GET /issues/tasks/{id}/issues` | 任务下的所有问题 |
| S8.3.20 | 验收问题列表 | 🟡 P1 | ✅ | `GET /issues/acceptance-orders/{id}/issues` | 验收单下的所有问题 |
| S8.3.21 | 批量分配问题 | 🟡 P1 | ✅ | `POST /issues/batch-assign` | 批量指派处理人 |
| S8.3.22 | 批量状态变更 | 🟡 P1 | ✅ | `POST /issues/batch-status` | 批量更新状态 |
| S8.3.23 | 批量关闭问题 | 🟡 P1 | ✅ | `POST /issues/batch-close` | 批量关闭 |
| S8.3.24 | 问题导出 API | 🟡 P1 | ✅ | `GET /issues/export` | 导出Excel/CSV |
| S8.3.25 | 问题导入 API | 🟢 P2 | ✅ | `POST /issues/import` | 批量导入问题 |
| S8.3.26 | 问题看板数据 API | 🟡 P1 | ✅ | `GET /issues/board` | 看板视图数据 |
| S8.3.27 | 问题趋势分析 API | 🟢 P2 | ✅ | `GET /issues/statistics/trend` | 时间序列趋势 |
| S8.3.28 | 问题原因分析 API | 🟢 P2 | ✅ | `GET /issues/statistics/cause-analysis` | Top N 原因统计 |
| S8.3.29 | 问题模板列表 API | 🟢 P2 | ✅ | `GET /issues/templates` | 模板列表查询 |
| S8.3.30 | 问题模板详情 API | 🟢 P2 | ✅ | `GET /issues/templates/{id}` | 模板详情 |
| S8.3.31 | 创建问题模板 API | 🟢 P2 | ✅ | `POST /issues/templates` | 创建模板 |
| S8.3.32 | 更新问题模板 API | 🟢 P2 | ✅ | `PUT /issues/templates/{id}` | 更新模板 |
| S8.3.33 | 删除问题模板 API | 🟢 P2 | ✅ | `DELETE /issues/templates/{id}` | 删除模板 |
| S8.3.34 | 从模板创建问题 API | 🟢 P2 | ✅ | `POST /issues/templates/{id}/create-issue` | 从模板快速创建问题 |

#### S8.4 Pydantic Schema 任务

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S8.4.1 | Issue 相关 Schema | 🔴 P0 | ✅ | Create/Update/Response |
| S8.4.2 | IssueFollowUp 相关 Schema | 🔴 P0 | ✅ | Create/Response |
| S8.4.3 | Issue 操作请求 Schema | 🔴 P0 | ✅ | Assign/Resolve/Verify/StatusChange |
| S8.4.4 | Issue 统计 Schema | 🟡 P1 | ✅ | IssueStatistics |
| S8.4.5 | Issue 批量操作 Schema | 🟡 P1 | ✅ | BatchAssign/BatchStatus/BatchClose |
| S8.4.6 | Issue 导出/导入 Schema | 🟡 P1 | ✅ | ExportRequest/ImportRequest |
| S8.4.7 | Issue 看板 Schema | 🟡 P1 | ✅ | BoardResponse |
| S8.4.8 | Issue 关联 Schema | 🟡 P1 | ✅ | RelatedIssueCreate/RelatedIssueResponse |
| S8.4.9 | Issue 模板 Schema | 🟢 P2 | ✅ | IssueTemplateCreate/Update/Response/ListResponse |
| S8.4.10 | Issue 从模板创建 Schema | 🟢 P2 | ✅ | IssueFromTemplateRequest |

#### S8.5 后台服务任务

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S8.5.1 | 问题逾期预警服务 | 🔴 P0 | ✅ | 定时检查逾期问题，发送提醒 |
| S8.5.2 | 阻塞问题预警服务 | 🔴 P0 | ✅ | 阻塞问题自动触发项目健康度更新 |
| S8.5.3 | 问题超时升级服务 | 🟡 P1 | ✅ | 长时间未处理自动升级 |
| S8.5.4 | 问题统计快照服务 | 🟡 P1 | ✅ | 每日生成问题统计快照 |
| S8.5.5 | 问题通知服务 | 🟡 P1 | ✅ | 问题分配/解决/逾期时发送通知 |

#### S8.6 系统集成任务

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S8.6.1 | 问题与预警系统集成 | 🔴 P0 | ✅ | 阻塞问题自动触发预警 |
| S8.6.2 | 问题与项目健康度集成 | 🔴 P0 | ✅ | 阻塞问题影响项目健康度（H3） |
| S8.6.3 | 问题与验收系统集成 | 🟡 P1 | ✅ | 验收问题关联验收单 |
| S8.6.4 | 问题与任务系统集成 | 🟡 P1 | ✅ | 任务问题关联任务，影响任务状态 |
| S8.6.5 | 问题通知中心集成 | 🟡 P1 | ✅ | 问题操作触发通知 |

---

### S10. 技术规格匹配预警模块 - 数据库与 API 补全 🆕

> 参考设计文档：技术规格与采购订单匹配预警功能规划
> 功能说明：对客户签订的技术规格书和采购订单/BOM进行不一致性匹配，进行风险预警
> 当前状态：数据库模型、API、前端页面已实现 ✅

#### S10.1 ORM 模型开发任务

| 序号 | 任务 | 优先级 | 状态 | 表名 | 说明 |
|:----:|------|:------:|:----:|------|------|
| S10.1.1 | 技术规格要求模型 | 🔴 P0 | ✅ | `technical_spec_requirements` | 存储技术规格书中的物料规格要求 |
| S10.1.2 | 规格匹配记录模型 | 🔴 P0 | ✅ | `spec_match_records` | 记录每次匹配检查的结果 |

#### S10.2 SQL 迁移脚本任务

| 序号 | 任务 | 优先级 | 状态 | 文件名 |
|:----:|------|:------:|:----:|--------|
| S10.2.1 | 生成 SQLite 迁移脚本 | 🔴 P0 | ✅ | `20260105_technical_spec_sqlite.sql` |
| S10.2.2 | 生成 MySQL 迁移脚本 | 🔴 P0 | ✅ | `20260105_technical_spec_mysql.sql` |

#### S10.3 Pydantic Schema 任务

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S10.3.1 | TechnicalSpecRequirement Schema | 🔴 P0 | ✅ | Create/Update/Response/ListResponse |
| S10.3.2 | SpecMatchRecord Schema | 🔴 P0 | ✅ | Response/ListResponse |
| S10.3.3 | SpecMatchCheck Schema | 🔴 P0 | ✅ | Request/Response/Result |
| S10.3.4 | SpecExtract Schema | 🟡 P1 | ✅ | Request/Response |

#### S10.4 核心服务任务

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S10.4.1 | 规格要求提取器 | 🔴 P0 | ✅ | 从规格描述中提取关键参数（电压、电流、精度等） |
| S10.4.2 | 智能规格匹配器 | 🔴 P0 | ✅ | 文本相似度+关键参数对比算法 |
| S10.4.3 | 规格匹配检查服务 | 🔴 P0 | ✅ | 采购订单和BOM集成检查服务 |
| S10.4.4 | 文档自动提取服务 | 🟡 P1 | ✅ | 支持Excel/Word/PDF文档解析（已扩展自动提取功能） |

#### S10.5 技术规格管理 API

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| S10.5.1 | 规格要求列表 API | 🔴 P0 | ✅ | `GET /technical-spec/requirements` | 分页+筛选 |
| S10.5.2 | 规格要求详情 API | 🔴 P0 | ✅ | `GET /technical-spec/requirements/{id}` | |
| S10.5.3 | 创建规格要求 API | 🔴 P0 | ✅ | `POST /technical-spec/requirements` | 手动录入 |
| S10.5.4 | 更新规格要求 API | 🔴 P0 | ✅ | `PUT /technical-spec/requirements/{id}` | |
| S10.5.5 | 删除规格要求 API | 🔴 P0 | ✅ | `DELETE /technical-spec/requirements/{id}` | |
| S10.5.6 | 从文档提取规格要求 | 🟡 P1 | ✅ | `POST /technical-spec/requirements/extract` | 文档解析提取 |

#### S10.6 规格匹配检查 API

| 序号 | 任务 | 优先级 | 状态 | API 路径 | 说明 |
|:----:|------|:------:|:----:|----------|------|
| S10.6.1 | 手动触发匹配检查 | 🔴 P0 | ✅ | `POST /technical-spec/match/check` | 检查项目/BOM/采购订单 |
| S10.6.2 | 匹配记录列表 API | 🔴 P0 | ✅ | `GET /technical-spec/match/records` | 查看匹配结果 |
| S10.6.3 | 采购订单创建时自动检查 | 🔴 P0 | ✅ | 集成到采购订单API | 自动匹配并预警 |
| S10.6.4 | BOM审批时自动检查 | 🔴 P0 | ✅ | 集成到BOM审批API | 自动匹配并预警 |
| S10.6.5 | 定时任务检查 | 🟡 P1 | ✅ | 后台定时任务 | 每日检查所有项目 |

#### S10.7 预警集成任务

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S10.7.1 | 规格不匹配预警规则 | 🔴 P0 | ✅ | 自动创建预警规则 |
| S10.7.2 | 预警记录自动生成 | 🔴 P0 | ✅ | 不匹配时自动生成预警 |
| S10.7.3 | 预警与预警中心集成 | 🔴 P0 | ✅ | 集成到预警中心 |

#### S10.8 前端页面任务

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S10.8.1 | 技术规格管理页面 | 🔴 P0 | ✅ | 规格要求CRUD、文档上传 |
| S10.8.2 | 规格匹配检查页面 | 🔴 P0 | ✅ | 匹配结果查看、差异详情 |
| S10.8.3 | 采购订单详情页集成 | 🔴 P0 | ✅ | 显示规格匹配状态和预警 |
| S10.8.4 | BOM管理页集成 | 🔴 P0 | ✅ | 显示规格匹配状态 |
| S10.8.5 | 项目详情页集成 | 🟡 P1 | ✅ | 显示规格匹配概览 |
...
| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S8.1.1 | 齐套检查列表页 | 🔴 P0 | ✅ | `KitCheck.jsx` 已上线，接入 `kitCheckApi` 实时筛选 |
| S8.1.2 | 齐套检查详情页 | 🔴 P0 | ✅ | `KitCheck.jsx` 内提供详情抽屉+开工确认 |
| S8.1.3 | 缺料上报页（移动端） | 🔴 P0 | ✅ | `mobile/MobileScanShortage.jsx`+`MobileShortageReport.jsx` |
| S8.1.4 | 到货跟踪页 | 🟡 P1 | ✅ | `ArrivalTrackingList.jsx`/`ArrivalManagement.jsx` |
| S8.1.5 | 物料替代/调拨页 | 🟡 P1 | ✅ | `SubstitutionDetail.jsx`+`TransferDetail.jsx` |
| S8.1.6 | 缺料统计分析页 | 🟡 P1 | ✅ | `ShortageManagement.jsx` 仪表板统计 |

#### S8.2 售后/知识库页面

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S8.2.1 | 售后工单列表页 | 🟡 P1 | ✅ | `ServiceTicketManagement.jsx` 列表+筛选完成 |
| S8.2.2 | 售后工单详情页 | 🟡 P1 | ✅ | 列表页内详情抽屉/对话框已实现处理记录 |
| S8.2.3 | 知识库首页 | 🟢 P2 | ✅ | `ServiceKnowledgeBase.jsx` 综合检索+分类 |
| S8.2.4 | 知识库详情页 | 🟢 P2 | ✅ | `LessonsLearnedLibrary.jsx`/`ServiceKnowledgeBase` 详情联动 |
...
| S8.3.2 | 通知列表页 | 🔴 P0 | ✅ | `NotificationCenter.jsx` 支持批量已读/清空 |
...
| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S8.4.1 | 问题列表页 | 🔴 P0 | ✅ | 列表+多维度筛选+统计 |
| S8.4.2 | 问题详情页 | 🔴 P0 | ✅ | 详情+跟进记录+操作 |
| S8.4.3 | 创建问题表单 | 🔴 P0 | ✅ | 问题创建+分配 |
| S8.4.4 | 问题看板页 | 🟡 P1 | ✅ | 看板视图（待办/处理中/已解决） |
| S8.4.5 | 问题统计页 | 🟡 P1 | ✅ | 多维度统计图表 |
| S8.4.6 | 批量操作功能 | 🟡 P1 | ✅ | 批量分配/状态变更/关闭 |
| S8.4.7 | 问题导出功能 | 🟡 P1 | ✅ | Excel/CSV导出 |
| S8.4.8 | 问题导入功能 | 🟢 P2 | ✅ | 批量导入 |
| S8.4.9 | 关联问题管理 | 🟡 P1 | ✅ | 父子问题关联视图 |
| S8.4.10 | 问题模板功能 | 🟢 P2 | ✅ | 快速创建常用问题（前端集成完成） |
...
| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| F.M.1 | 工人任务列表 | 🔴 P0 | ✅ | `mobile/MobileWorkerTaskList.jsx` |
| F.M.2 | 扫码开工 | 🔴 P0 | ✅ | `mobile/MobileScanStart.jsx` |
| F.M.3 | 进度上报 | 🔴 P0 | ✅ | `mobile/MobileProgressReport.jsx` |
| F.M.4 | 完工报告 | 🔴 P0 | ✅ | `mobile/MobileCompleteReport.jsx` |
| F.M.5 | 异常上报 | 🔴 P0 | ✅ | `mobile/MobileExceptionReport.jsx` |
| F.M.6 | 领料申请 | 🔴 P0 | ✅ | `mobile/MobileMaterialRequisition.jsx` |
| F.M.7 | 我的工时 | 🟡 P1 | ❌ | 工时统计 |
...
| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| F.5A.1 | PMO 驾驶舱 | 🟡 P1 | ✅ | `PMODashboard.jsx` |
| F.5A.2 | 立项申请列表 | 🟡 P1 | ✅ | `InitiationManagement.jsx` |
| F.5A.3 | 立项申请表单 | 🟡 P1 | ✅ | `InitiationManagement.jsx` 对话框 |
| F.5A.4 | 立项评审页 | 🟡 P1 | ✅ | `InitiationManagement.jsx` 评审流程 |
| F.5A.5 | 项目阶段门页 | 🟡 P1 | ✅ | `ProjectPhaseManagement.jsx` |
| F.5A.6 | 风险管理页 | 🟡 P1 | ✅ | `RiskManagement.jsx`/`RiskWall.jsx` |
| F.5A.7 | 项目会议管理 | 🟢 P2 | ✅ | `MeetingManagement.jsx` |
| F.5A.8 | 项目结项页 | 🟡 P1 | ✅ | `ProjectClosureManagement.jsx` |
| F.5A.9 | 项目状态周报 | 🟡 P1 | ✅ | `WeeklyReport.jsx` |

### 7.5B 售前技术支持页面（第二期）🆕

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| F.5B.1 | 售前工作台 | 🟡 P1 | ✅ | `PresalesWorkstation.jsx` |
| F.5B.2 | 支持工单列表 | 🟡 P1 | ✅ | `PresalesTasks.jsx` |
| F.5B.3 | 工单看板页 | 🟡 P1 | ✅ | `PresalesManagerWorkstation.jsx` 看板视图 |
| F.5B.4 | 支持申请表单 | 🟡 P1 | ✅ | `PresalesIntegration.jsx` 提交流程 |
| F.5B.5 | 工单处理页 | 🟡 P1 | ✅ | `PresalesManagerWorkstation.jsx` 处理面板 |
| F.5B.6 | 方案设计工作台 | 🟡 P1 | ✅ | `PresalesIntegration.jsx`/`OpportunityManagement.jsx` 方案区 |
| F.5B.7 | 方案库页面 | 🟡 P1 | ✅ | `SalesTemplateCenter.jsx` |
| F.5B.8 | 投标管理页 | 🟡 P1 | ✅ | `BiddingCenter.jsx` |
| F.5B.9 | 售前统计报表 | 🟡 P1 | ✅ | `PresalesManagerWorkstation.jsx` 统计卡片 |

### 7.5C 个人任务中心页面（第二期）🆕

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| F.5C.1 | 任务中心首页 | 🔴 P0 | ✅ | `TaskCenter.jsx` 统计卡片+今日任务 |
| F.5C.2 | 任务列表页 | 🔴 P0 | ✅ | `TaskCenter.jsx` 列表/看板双视图 |
| F.5C.3 | 任务卡片组件 | 🔴 P0 | ✅ | `TaskCenter.jsx` 中的 `TaskCard` 组件 |
...
| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| F.5D.1 | 缺料管理看板 | 🔴 P0 | ✅ | `ShortageManagementBoard.jsx` |
| F.5D.2 | 缺料上报列表 | 🔴 P0 | ✅ | `ShortageReportList.jsx` |
| F.5D.3 | 到货跟踪列表 | 🔴 P0 | ✅ | `ArrivalTrackingList.jsx` |
| F.5D.4 | 替代申请列表 | 🟡 P1 | ✅ | `SubstitutionDetail.jsx` 支持审批 |
| F.5D.5 | 调拨申请列表 | 🟡 P1 | ✅ | `TransferDetail.jsx`/`TransferNew.jsx` |
| F.5D.6 | 缺料日报页 | 🟡 P1 | ✅ | `ShortageManagement.jsx` 仪表板含日报 |

### 7.5E 移动端缺料上报（第二期）🆕

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| F.5E.1 | 扫码上报页 | 🔴 P0 | ✅ | `mobile/MobileScanShortage.jsx` 扫码/搜索工单 |
| F.5E.2 | 上报表单页 | 🔴 P0 | ✅ | `mobile/MobileShortageReport.jsx` 支持拍照与提交 |
| F.5E.3 | 我的上报记录 | 🔴 P0 | ✅ | `mobile/MobileMyShortageReports.jsx` 进度列表 |
...
| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| F.6.1 | 线索列表页 | 🔴 P0 | ✅ | `LeadManagement.jsx` |
| F.6.2 | 线索详情页 | 🔴 P0 | ✅ | `LeadDetail.jsx` |
| F.6.3 | 商机列表页 | 🔴 P0 | ✅ | `OpportunityManagement.jsx` |
| F.6.4 | 商机详情页 | 🔴 P0 | ✅ | `OpportunityDetail.jsx` |
| F.6.5 | 商机看板页 | 🔴 P0 | ✅ | `OpportunityBoard.jsx` |
| F.6.6 | 报价列表页 | 🔴 P0 | ✅ | `QuoteManagement.jsx`/`QuotationList.jsx` |
| F.6.7 | 报价创建/编辑 | 🔴 P0 | ✅ | `QuoteCreateEdit.jsx` |
| F.6.8 | 合同列表页 | 🔴 P0 | ✅ | `ContractList.jsx`/`ContractManagement.jsx` |
| F.6.9 | 合同详情页 | 🔴 P0 | ✅ | `ContractDetail.jsx` |
| F.6.10 | 发票列表页 | 🔴 P0 | ✅ | `InvoiceManagement.jsx` |
| F.6.11 | 回款列表页 | 🔴 P0 | ✅ | `PaymentManagement.jsx` |
| F.6.12 | 应收账款页 | 🟡 P1 | ✅ | `ReceivableManagement.jsx` |

### 7.7 决策与分析页面（第三期）

| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| F.7.1 | 决策驾驶舱 | 🟢 P2 | ✅ | `ExecutiveDashboard.jsx` 综合 KPI 看板 |
| F.7.2 | 成本分析页 | 🟢 P2 | ❌ | 图表+对比 |
| F.7.3 | 项目复盘页 | 🟢 P2 | ✅ | `ProjectReviewList.jsx`+`ProjectReviewDetail.jsx` |
| F.7.4 | 知识库页面 | 🟢 P2 | ✅ | `KnowledgeBase.jsx`/`LessonsLearnedLibrary.jsx` |
...
| 序号 | 任务 | 优先级 | 状态 | 说明 |
|:----:|------|:------:|:----:|------|
| S.1 | 健康度自动计算服务 | 🔴 P0 | ✅ | 定时任务（已实现：服务+API+定时任务） |
| S.2 | 缺料预警生成服务 | 🔴 P0 | ✅ | 定时任务（每天7:00执行） |
| S.3 | 里程碑预警服务 | 🔴 P0 | ✅ | 定时任务（每天8:00执行，含风险预警） |
| S.4 | 🆕 任务延期预警服务 | 🔴 P0 | ✅ | 进度跟踪（每小时执行） |
| S.5 | 🆕 生产计划预警服务 | 🔴 P0 | ✅ | 生产管理（每天11:00执行） |
| S.6 | 🆕 报工超时提醒服务 | 🔴 P0 | ✅ | 生产管理（每小时执行） |
| S.7 | 外协交期预警服务 | 🟡 P1 | ✅ | 定时任务（每天15:00执行，已实现P0版本） |
| S.8 | 收款提醒服务 | 🟡 P1 | ✅ | 定时任务（每天9:30执行） |
| S.9 | 🆕 逾期应收预警服务 | 🟡 P1 | ✅ | O2C 模块（每天10:30执行） |
| S.10 | 预警升级服务 | 🟡 P1 | ✅ | 定时任务（每小时整点+10分执行） |
| S.11 | 健康度快照服务 | 🟡 P1 | ✅ | 每日快照（已实现） |
| S.12 | 消息推送服务 | 🟡 P1 | ✅ | 企微/邮件（基础框架已实现，每小时整点+15分执行） |
| S.13 | 🆕 商机阶段超时提醒 | 🟡 P1 | ✅ | O2C 模块（每天15:30执行） |
| S.14 | 🆕 进度汇总计算服务 | 🔴 P0 | ✅ | 自动汇总（每小时执行） |
| S.15 | 🆕 生产日报自动生成 | 🟡 P1 | ✅ | 生产管理（每天5:00执行） |
| S.16 | 🆕 设备保养提醒服务 | 🟢 P2 | ✅ | 生产管理（每天8:30执行） |
| S.17 | 🆕 每日齐套检查服务 | 🔴 P0 | ✅ | 缺料管理（每天6:00执行） |
| S.18 | 🆕 到货延迟检查服务 | 🔴 P0 | ✅ | 缺料管理（每天14:00执行） |
| S.19 | 🆕 缺料日报生成服务 | 🟡 P1 | ✅ | 缺料管理（每天5:15执行） |
| S.20 | 🆕 岗位职责任务生成 | 🟡 P1 | ✅ | 任务中心（每天4:00执行） |
| S.21 | 🆕 任务到期提醒服务 | 🔴 P0 | ✅ | 任务中心（每小时执行） |
| S.22 | 🆕 售前工单超时提醒 | 🟡 P1 | ✅ | 售前支持（每天16:00执行） |
| S.23 | 🆕 负荷超限预警服务 | 🟡 P1 | ✅ | 资源负荷（每天11:00执行） |
| S.24 | 🆕 问题逾期预警服务 | 🔴 P0 | ✅ | 问题管理（每小时执行） |
| S.25 | 🆕 阻塞问题预警服务 | 🔴 P0 | ✅ | 问题管理→项目健康度（每小时执行） |
| S.26 | 🆕 问题超时升级服务 | 🟡 P1 | ✅ | 问题管理（每天1:30执行） |
...
| v1.8 | 2026-01-05 | 🆕 **问题管理中心模块**：补充完整任务清单，包括数据库模型(2张表✅)、API端点(28个，11个✅)、前端页面(10页，3个✅)、后台服务(4个❌)、编码规则(1个✅) |
| v1.9 | 2026-01-05 | 🆕 **技术规格匹配预警模块**：实现技术规格书与采购订单/BOM的智能匹配检查功能，包括数据库模型(2张表✅)、API端点(11个✅)、核心服务(4个✅)、前端页面(2页✅)、预警集成(3个✅)、定时任务(1个✅) |
| v1.10 | 2026-01-05 | 📊 **任务状态更新**：根据最新代码统计更新任务完成状态，636个API端点，39个端点文件，95个前端页面。更新了用户认证、主数据、项目管理、生产管理、销售管理、ECN、外协、验收、工时管理等模块的任务状态 |
| v1.11 | 2026-01-05 | 🎉 **健康度自动计算服务完成**：实现了完整的健康度自动计算服务（HealthCalculator服务+定时任务+API端点），681个API端点，40个端点文件。问题管理模块从55%提升到70%，销售管理模块从75%提升到85%，项目管理模块从30%提升到35%，后台服务从22%提升到30% |
| v1.12 | 2026-01-05 | 📊 **代码优化与完善**：验收管理模块代码优化（从1352行增加到1420行），商务支持订单模块新增18个端点（business_support_orders.py），API端点总数从681个增加到687个，商务支持模块从70%提升到85%，整体完成度从70%提升到71% |
| v1.13 | 2026-01-06 | 🎉 **缺料管理模块100%完成**：完成缺料管理模块全面检查与补全，包括数据库表（10张表✅）、API端点（35个✅）、SQL迁移脚本（SQLite+MySQL✅）。所有API端点已实现：齐套检查(5个)、缺料上报(7个)、到货跟踪(6个)、物料替代/调拨(8个)、缺料统计(5个)。缺料管理模块从60%提升到100% |
| v1.14 | 2026-01-06 | 📊 **多模块任务状态更新**：根据代码检查更新任务状态，生产管理模块从82%提升到100%（95个端点），ECN管理模块从80%提升到100%（27个端点），验收管理模块从92%提升到100%（21个端点），外协管理模块从70%提升到95%（21个端点），预警与异常模块100%完成（24个端点）。整体完成度从73%提升到75% |
| v1.15 | 2026-01-06 | 📊 **多模块任务状态更新**：验收管理模块100%完成（14个API：模板5个、问题5个、签字报告4个）。预警规则管理5个API全部完成，预警记录管理7个API全部完成，异常事件管理7个API全部完成，工序字典管理4个API全部完成，项目健康度快照3个任务全部完成。ECN管理17个API全部完成，外协管理20个API全部完成。累计更新79个P0/P1/P2任务标记为完成。整体完成度从75%提升到76% |
| v1.16 | 2026-01-06 | 🎉 **10个P0预警服务100%完成**：实现了完整的预警服务体系，包括缺料预警生成（每天7:00）、任务延期预警（每小时）、生产计划预警（每天11:00）、报工超时提醒（每小时）、进度汇总计算（每小时）、每日齐套检查（每天6:00）、到货延迟检查（每天14:00）、任务到期提醒（每小时）、外协交期预警（每天15:00）、里程碑风险预警（每天8:10）。所有服务已集成到定时任务调度器，自动创建预警规则和记录。后台服务完成度从0%提升到100%，整体完成度从76%提升到77% |
| v1.17 | 2026-01-06 | 🎉 **前端界面完成度提升至85%**：完成通知中心页面API集成（添加notificationApi），完成生产管理仪表板API集成（移除所有mock数据fallback），完成任务中心页面API集成（添加taskCenterApi），完善验收管理、采购订单、缺料管理等核心页面的API连接。前端完成度从75%提升到85%，整体完成度从77%提升到78% |
| v1.18 | 2026-01-06 | 📊 **前端界面完成度提升至88%**：完善物料跟踪页面API集成（materialApi和purchaseApi），物料管理相关页面已连接真实API。前端完成度从85%提升到88%，整体完成度从78%提升到79% |
| v1.19 | 2026-01-06 | 🎉 **前端界面完成度提升至90%**：完成售前管理页面API集成（PresalesTasks.jsx连接presaleApi.tickets），售前技术任务中心已连接真实API，支持任务列表、筛选、搜索等功能。前端完成度从88%提升到90%，整体完成度从79%提升到80% |
| v1.20 | 2026-01-06 | 📊 **前端界面完成度提升至92%**：完成售前经理工作台API集成（PresalesManagerWorkstation.jsx连接presaleApi），移除所有mock数据，实现方案统计、投标项目统计、待审核方案列表等功能。前端完成度从90%提升到92%，整体完成度从80%提升到81% |
| v1.21 | 2026-01-06 | 📊 **前端界面完成度提升至93%**：优化物料分析页面（MaterialAnalysis.jsx），使用专门的`/projects/{project_id}/material-status` API替代手动计算，提升性能和准确性，添加fallback机制确保兼容性。前端完成度从92%提升到93%，整体完成度从81%提升到82% |
| v1.22 | 2026-01-06 | 📊 **前端界面完成度提升至94%**：完善发票管理页面（InvoiceManagement.jsx）和采购订单详情页面（PurchaseOrderDetail.jsx），移除所有mock数据fallback，完善错误处理和加载状态。前端完成度从93%提升到94%，整体完成度从82%提升到83% |
| v1.23 | 2026-01-06 | 📊 **前端界面完成度提升至95%**：完善售后/知识库相关页面，包括服务工单管理（ServiceTicketManagement.jsx）、知识库管理（ServiceKnowledgeBase.jsx）和客户满意度调查（CustomerSatisfaction.jsx），连接serviceApi，移除所有mock数据，实现完整的数据转换和状态映射。前端完成度从94%提升到95%，整体完成度从83%提升到84% |
| v1.24 | 2026-01-06 | 📊 **前端界面完成度提升至96%**：完善WBS模板管理页面（WBSTemplateManagement.jsx）和齐套看板页面（KitRateBoard.jsx），连接progressApi和purchaseApi，移除所有mock数据注释，实现完整的API集成。前端完成度从95%提升到96%，整体完成度从84%提升到85% |
| v1.25 | 2026-01-06 | 🎉 **进度跟踪模块大幅完善**：完成进度跟踪模块核心功能补全，包括创建独立的ProgressReport模型和数据库表、完善进度报告API（创建/查询/更新/删除）、新增任务依赖删除、任务阻塞/取消/删除功能、进度报告统计功能。新增9个API端点，进度跟踪模块从40%提升到75%（38个端点）。创建数据库迁移脚本（SQLite+MySQL） |
| v1.26 | 2026-01-06 | 🎉 **P1/P2扩展功能全面完成**：完成个人任务中心批量操作API（新增批量催办功能）、物料调拨管理API、PMO项目结项管理API、绩效管理API（9个端点全部完成）、报表中心API（9个端点全部完成）、设备管理API（6个端点全部完成）、销售报表API（4个端点全部完成）。数据导入导出API框架已实现。整体完成度从88%提升到92% |
| v1.27 | 2026-01-06 | 🎉 **P2扩展功能全面实现**：完成缺料原因分析API、外协对账单API、设备档案API（服务历史记录）、项目成本分析API（成本对比分析、项目利润分析2个端点）、BI报表API（交付准时率、项目健康度分布、人员利用率、供应商绩效、决策驾驶舱5个端点）。新增8个P2 API端点，整体完成度从92%提升到94% |
| v1.28 | 2026-01-06 | 🎉 **项目复盘和知识库P2功能完成**：完成项目复盘API（复盘报告列表、创建复盘报告2个端点）、项目经验教训API（问题总结1个端点）、知识库特定API（问题库列表、方案库列表、搜索知识库、添加知识条目4个端点）。新增7个P2 API端点，整体完成度从94%提升到95% |
| v1.29 | 2026-01-06 | 🎉 **项目成本管理P1功能完成**：完成项目成本汇总API（成本汇总统计，包含预算对比、合同对比、按类型/分类/机台统计）、成本明细列表API（支持分页和多条件筛选）。新增2个P1 API端点，整体完成度从95%提升到96% |
| v1.30 | 2026-01-06 | 🎉 **数据导入导出功能完善**：完善数据导入导出Excel处理逻辑，包括项目导入模板下载（支持样式和格式设置）、数据预览（Excel解析、数据验证、错误提示）、数据验证（必填字段、日期格式、业务规则验证）、批量导入（项目批量导入已实现，支持更新已存在项目）、项目列表导出、项目详情导出（含任务和成本）、任务列表导出、工时数据导出、负荷数据导出。所有Excel处理功能已实现，数据导入导出模块从90%提升到100%，整体完成度从96%提升到97% |
| v1.31 | 2026-01-06 | 🎉 **批量操作功能完善**：确认并更新批量操作API状态，包括批量提醒（批量催办）、批量打标签、批量启动任务、批量暂停任务、批量删除任务、批量操作统计。所有批量操作功能已实现，个人任务中心模块100%完成 |
...
### 问题管理中心模块
- **核心定位**：统一的问题管理平台，支持项目问题、任务问题、验收问题等全场景
- **关键特性**：
...
- **API端点**：34个端点（全部已实现✅）
  - **基础CRUD**：列表、详情、创建、更新 ✅
  - **问题操作**：分配、解决、验证、状态变更、关闭、取消 ✅
  - **跟进管理**：获取、创建跟进记录 ✅
  - **关联管理**：关联问题、项目/机台/任务问题列表 ✅
  - **批量操作**：批量分配、状态变更、关闭 ✅
  - **模板管理**：问题模板管理（6个端点）✅
  - **统计分析**：统计 ✅
- **前端页面**：10个页面（3个已实现✅，7个待开发）
  - 列表页、详情页、创建表单 ✅
  - 看板页、统计页、批量操作、导出导入、关联管理、模板功能 ❌
- **后台服务**：4个定时任务（全部待开发❌）
  - 问题逾期预警、阻塞问题预警、超时升级、统计快照

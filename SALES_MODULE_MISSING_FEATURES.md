...
**当前实现**:
- ✅ `/sales/quote-templates` 提供列表/分页、创建、更新、版本创建与版本发布接口
- ✅ `/sales/quote-templates/{id}/apply` 支持结合 CPQ 规则集返回默认价格预测
- ⚠️ 模板共享与审批规则仍需完善（仅支持 ALL/TEAM/PERSONAL 可见范围，缺少模板审批流程）
...
**当前实现**:
- ✅ `/sales/contract-templates` 支持模板/版本创建、更新与版本发布
- ✅ `/sales/contract-templates/{id}/apply` 返回最新条款/审批流结构，便于项目/合同同步
- ❌ 条款库与条款引用规则仍未细化
...
**当前实现**:
- ✅ `/customers/{id}/360` 汇总项目/商机/报价/合同/发票/收款节点与沟通记录
- ⚠️ 仍需增加满意度、服务记录及可配置指标
...
**当前实现**:
- ✅ 新增 `cpq_rule_sets` 表及 `/sales/cpq/rule-sets` 管理入口，可配置配置项/定价矩阵/审批阈值
- ✅ `/sales/cpq/price-preview` + 模板应用接口输出预测价格、调价轨迹与审批提示
- ❌ UI 配置器、复杂规则编排与审批联动仍缺失

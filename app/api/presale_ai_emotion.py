...
async def get_emotion_analysis(ticket_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        analysis = (
            ...
            .filter(PresaleAIEmotionAnalysis.presale_ticket_id == ticket_id)
...
@router.get(
    "/follow-up-reminders", response_model=FollowUpReminderListResponse, summary="获取跟进提醒列表"
)
async def get_follow_up_reminders(
    ...
    try:
        service = AIEmotionService(db)
        reminders = service.get_follow_up_reminders(status=status, priority=priority, limit=limit)
...
async def batch_analyze_customers(request: BatchAnalysisRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        result = await service.batch_analyze_customers(
            customer_ids=request.customer_ids, analysis_type=request.analysis_type
        )
        return result
    except Exception as e:

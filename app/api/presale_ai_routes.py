...
async def generate_solution(
    ...
    try:
        result = service.generate_solution(request=request, user_id=current_user.id)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"方案生成失败: {str(e)}"
        )
...
async def generate_architecture(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"架构图生成失败: {str(e)}"
        )
...
async def generate_bom(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"BOM生成失败: {str(e)}"
        )
...
async def get_solution(
    solution_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """
    获取方案详情
    """
...
async def update_solution(
    ...
    try:
        updated_solution = service.update_solution(
            solution_id=solution_id, update_data=request.model_dump(exclude_unset=True)
        )
...
async def export_solution_pdf(
    ...
    try:
        ...
        return {"success": True, "pdf_path": pdf_path, "message": "PDF导出成功"}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"PDF导出失败: {str(e)}"
        )
...
async def get_template_library(
    ...
    templates = service.get_template_library(
        industry=industry, equipment_type=equipment_type, is_active=is_active
    )
...
async def create_template(
    ...
    try:
        template = template_service.create_template(
            data=request.model_dump(), user_id=current_user.id
        )

        return SolutionTemplateResponse.model_validate(template)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"模板创建失败: {str(e)}"
        )
...
async def update_template(
    ...
    try:
        template = template_service.update_template(
            template_id=template_id, data=request.model_dump(exclude_unset=True)
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"模板更新失败: {str(e)}"
        )

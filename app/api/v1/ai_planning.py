...
from app.models.ai_planning import AIProjectPlanTemplate, AIResourceAllocation, AIWbsSuggestion
...
async def list_plan_templates(
    ...
    支持按项目类型、行业、复杂度筛选
    """
    query = db.query(AIProjectPlanTemplate).filter(AIProjectPlanTemplate.is_active == True)
...
async def get_wbs_suggestions(
    ...
    query = db.query(AIWbsSuggestion).filter(
        AIWbsSuggestion.project_id == project_id, AIWbsSuggestion.is_active == True
    )
...
async def accept_wbs_suggestion(
    wbs_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """采纳WBS建议"""
...
async def get_resource_allocations(
    ...
    query = db.query(AIResourceAllocation).filter(
        AIResourceAllocation.project_id == project_id, AIResourceAllocation.is_active == True
    )
...
async def get_project_schedule(
    ...
    result = optimizer.optimize_schedule(project_id=project_id, start_date=None, constraints=None)
...
async def get_ai_accuracy_statistics(
    ...
    query_wbs = db.query(AIWbsSuggestion).filter(AIWbsSuggestion.is_accepted.isnot(None))
    ...
    query_ra = db.query(AIResourceAllocation).filter(AIResourceAllocation.is_accepted.isnot(None))

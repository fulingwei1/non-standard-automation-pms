...
@router.get(
    "/acceptance-issues/{issue_id}/follow-ups",
    response_model=List[IssueFollowUpResponse],
...
def read_issue_follow_ups(
    ...
    follow_ups = (
        db.query(IssueFollowUp)
        .filter(IssueFollowUp.issue_id == issue_id)
        .order_by(IssueFollowUp.created_at)
        .all()
    )

...
def get_locked_accounts(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="获取锁定账户列表失败"
        )
...
def unlock_account(
    ...
    try:
        success = AccountLockoutService.unlock_account(
            username=unlock_data.username, admin_user=current_user.username, db=db
        )

        if success:
            return ResponseModel(code=200, message=f"账户 {unlock_data.username} 已成功解锁")
        else:
            raise HTTPException(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"解锁账户失败: {str(e)}"
        )
...
def get_login_history(
    ...
    try:
        history = AccountLockoutService.get_login_history(
            username=query.username, limit=query.limit, db=db
        )
        return ResponseModel(
            code=200, message="获取成功", data={"history": history, "total": len(history)}
        )
    except Exception as e:
        logger.error(f"获取登录历史失败: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取登录历史失败: {str(e)}"
        )
...
def get_ip_blacklist(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取IP黑名单失败: {str(e)}"
        )
...
def remove_ip_blacklist(
    ...
    try:
        success = AccountLockoutService.remove_ip_from_blacklist(
            ip=remove_data.ip, admin_user=current_user.username
        )

        if success:
            return ResponseModel(code=200, message=f"IP {remove_data.ip} 已从黑名单移除")
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="移除失败，Redis不可用"
            )
    except HTTPException:
        raise
    except Exception as e:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"移除IP黑名单失败: {str(e)}"
        )


@router.get(
    "/lockout-status/{username}", response_model=ResponseModel, status_code=status.HTTP_200_OK
)
def check_lockout_status(
    ...
    try:
        ...
        return ResponseModel(code=200, message="查询成功", data=status_info)
    except Exception as e:
        logger.error(f"查询锁定状态失败: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"查询锁定状态失败: {str(e)}"
        )

...
def get_categories(
    ...
    for cat in categories:
        product_count = (
            db.query(func.count(AdvantageProduct.id))
            .filter(AdvantageProduct.category_id == cat.id, AdvantageProduct.is_active)
            .scalar()
        )
...
@router.post(
    "/categories",
    response_model=AdvantageProductCategoryResponse,
...
def create_category(
    ...
    existing = (
        ...
        .filter(AdvantageProductCategory.code == category_in.code)
        .first()
    )
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"类别编码 '{category_in.code}' 已存在"
        )
    ...
    return AdvantageProductCategoryResponse(**{**category.__dict__, "product_count": 0})
...
def update_category(
    ...
    category = (
        db.query(AdvantageProductCategory)
        .filter(AdvantageProductCategory.id == category_id)
        .first()
    )
    if not category:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="类别不存在")
    ...
    product_count = (
        db.query(func.count(AdvantageProduct.id))
        .filter(AdvantageProduct.category_id == category.id, AdvantageProduct.is_active)
        .scalar()
    )

    return AdvantageProductCategoryResponse(**{**category.__dict__, "product_count": product_count})

...
def _call_llm(messages: List[Dict[str, str]], timeout: int = 60) -> str:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_504_GATEWAY_TIMEOUT, detail="AI 请求超时，请稍后重试"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"AI 调用失败：{str(e)}"
        )
...
请按照系统 prompt 中的 JSON 格式返回分析结果。"""
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"AI 响应解析失败：{str(e)}"
        )
...
@router.post(
    "/annual-plan", summary="AI 年度经营计划", description="基于战略和 CSF 生成年度重点工作"
)
...
@router.post("/dept-objectives", summary="AI 部门工作分解", description="基于战略生成部门 OKR 目标")
...
async def ai_apply(
    ...
    if type not in ["csf", "kpi", "annual_work", "dept_objective"]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"不支持的类型：{type}")
        ...
        if type == "csf":
            ...
            for work in works:
                ...
                result["details"].append(
                    {"type": "annual_work", "id": work_id, "name": work.get("name")}
                )
            ...
            if dept_id:
                for obj in objectives:
                    ...
                    result["created_count"] += 1
                    result["details"].append(
                        {"type": "dept_objective", "id": obj_id, "objective": obj.get("objective")}
                    )
...
def _insert_annual_work(cursor, work: Dict[str, Any], strategy_id: int) -> int:
    ...
    if not csf_id and work.get("csf_code"):
        cursor.execute(
            "SELECT id FROM csfs WHERE code = ? AND strategy_id = ?",

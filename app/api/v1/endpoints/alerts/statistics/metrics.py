...
@router.get(
    "/alerts/statistics/response-metrics", response_model=dict, status_code=status.HTTP_200_OK
)
def get_response_metrics(
    ...
    acknowledged_alerts = query.filter(
        AlertRecord.acknowledged_at.isnot(None), AlertRecord.triggered_at.isnot(None)
    ).all()

    # 查询已处理的预警（用于计算解决时间）
    resolved_alerts = query.filter(
        AlertRecord.acknowledged_at.isnot(None), AlertRecord.handle_end_at.isnot(None)
    ).all()
    ...
    # 平均响应时间和解决时间
    avg_response_time = (
        sum(rt["hours"] for rt in response_times) / len(response_times) if response_times else 0
    )
    avg_resolve_time = (
        sum(rt["hours"] for rt in resolve_times) / len(resolve_times) if resolve_times else 0
    )
...
@router.get(
    "/alerts/statistics/efficiency-metrics", response_model=dict, status_code=status.HTTP_200_OK
)

...
@router.get(
    "/alerts/subscriptions", response_model=PaginatedResponse, status_code=status.HTTP_200_OK
)
def read_alert_subscriptions(
    ...
    # 分页
    subscriptions = apply_pagination(
        query.order_by(AlertSubscription.created_at.desc()), pagination.offset, pagination.limit
    ).all()
...
def read_alert_subscription(
    ...
    subscription = (
        ...
        .filter(
            AlertSubscription.id == subscription_id, AlertSubscription.user_id == current_user.id
        )
        .first()
    )
    ...
    *,
    ...
    if subscription_in.alert_type is not None:
        ...
        if subscription_in.alert_type and subscription_in.alert_type not in valid_types:
            raise HTTPException(
                status_code=400, detail=f"无效的预警类型: {subscription_in.alert_type}"
            )
    ...
    if subscription_in.min_level is not None:
        ...
        if subscription_in.min_level not in valid_levels:
            raise HTTPException(
                status_code=400, detail=f"无效的预警级别: {subscription_in.min_level}"
            )
    ...
    if subscription_in.alert_type is not None or subscription_in.project_id is not None:
        new_type = (
            subscription_in.alert_type
            if subscription_in.alert_type is not None
            else subscription.alert_type
        )
        new_project_id = (
            subscription_in.project_id
            if subscription_in.project_id is not None
            else subscription.project_id
        )

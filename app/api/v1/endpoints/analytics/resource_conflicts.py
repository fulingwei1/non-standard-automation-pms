...
def get_project_conflicts(
    ...
    # 获取项目的所有资源计划ID
    plan_ids = (
        db.query(ProjectStageResourcePlan.id)
        .filter(ProjectStageResourcePlan.project_id == project_id)
        .subquery()
    )
...
def get_employee_conflicts(
    ...
    特定员工的冲突
    """
    query = db.query(ResourceConflict).filter(ResourceConflict.employee_id == employee_id)
...
def get_conflicts_summary(
    ...
    unresolved = (
        ...
        .filter(ResourceConflict.is_resolved == 0)
        .group_by(ResourceConflict.severity)
        .all()
    )
    ...
    # 受影响员工数
    affected_employees = (
        db.query(func.count(func.distinct(ResourceConflict.employee_id)))
        .filter(ResourceConflict.is_resolved == 0)
        .scalar()
    )
    ...
    # 受影响项目数
    affected_plans = (
        db.query(ResourceConflict.plan_a_id, ResourceConflict.plan_b_id)
        .filter(ResourceConflict.is_resolved == 0)
        .all()
    )
    ...
    affected_projects = (
        db.query(func.count(func.distinct(ProjectStageResourcePlan.project_id)))
        .filter(ProjectStageResourcePlan.id.in_(plan_ids))
        .scalar()
        if plan_ids
        else 0
    )
...
def _format_conflict(
    db: Session, conflict: ResourceConflict, current_project_id: int = None
) -> dict:
    """格式化冲突记录"""
    ...
    # 获取项目信息
    project_a = (
        db.query(Project).filter(Project.id == plan_a.project_id).first() if plan_a else None
    )
    project_b = (
        db.query(Project).filter(Project.id == plan_b.project_id).first() if plan_b else None
    )
    ...
    return {
        ...
        "this_project": {
            ...
            "allocation_pct": (
                float(this_plan.allocation_pct) if this_plan and this_plan.allocation_pct else 100
            ),
        ...
        "conflict_with": {
            ...
            "allocation_pct": (
                float(other_plan.allocation_pct)
                if other_plan and other_plan.allocation_pct
                else 100
            ),
            "period": (
                f"{other_plan.planned_start} ~ {other_plan.planned_end}" if other_plan else None
            ),
        },
...
def _check_time_overlap(
    ...
    """检查两个计划的时间重叠"""
    if not all(
        [plan_a.planned_start, plan_a.planned_end, plan_b.planned_start, plan_b.planned_end]
    ):
        return None

...
def get_skill_list(
    ...
    for tag in tags:
        count = (
            db.query(func.count(HrEmployeeTagEvaluation.id))
            .filter(
                HrEmployeeTagEvaluation.tag_id == tag.id,
            )
            .scalar()
...
def search_available_employees(
    ...
    qualified_employees = (
        ...
        .group_by(HrEmployeeTagEvaluation.employee_id)
        .having(func.count(HrEmployeeTagEvaluation.id) >= len(tag_ids))  # 具备所有技能
        .all()
    )
            ...
            critical_gaps.append(
                {
                    ...
                    "severity": (
                        "CRITICAL" if qualified == 0 else ("HIGH" if gap >= 2 else "MEDIUM")
                    ),
                    "current_headcount": qualified,
                    "required_headcount": min_required,
                    "gap": gap,
                    "recommendation": (
                        "紧急招聘或外部顾问" if qualified == 0 else "建议安排培训或招聘"
                    ),
                }
            )
        ...
        data={
            ...
            "summary": {
                ...
                "training_candidates": sum(
                    len(t["potential_trainees"]) for t in training_opportunities
                ),
            },
        }
    )

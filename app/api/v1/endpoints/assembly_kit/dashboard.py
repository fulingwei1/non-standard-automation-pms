...
router = APIRouter(prefix="/assembly-kit/dashboard", tags=["dashboard"])
    ...
    for stage in stages:
        ...
        stage_stats.append(
            AssemblyDashboardStageStats(
                ...
                avg_kit_rate=(
                    round(total_rate / total_projects, 2) if total_projects > 0 else Decimal(0)
                ),
            )
        )
    return stage_stats
...
def _build_recent_analyses_list(db, recent_analyses, Project, BomHeader, Machine):
    ...
    for r in recent_analyses[:10]:
        ...
        machine = (
            db.query(Machine).filter(Machine.id == r.machine_id).first() if r.machine_id else None
        )
...
class AssemblyKitDashboardEndpoint(BaseDashboardEndpoint):
    ...
    def __init__(self):
        ...
        self.router = APIRouter(prefix="/assembly-kit/dashboard", tags=["dashboard"])
    ...
    def get_dashboard_data(
        self, db: Session, current_user: User, project_ids: Optional[str] = None
    ) -> Dict[str, Any]:
        """获取装配齐套看板数据"""
        # 获取最近的齐套分析记录(每个项目取最新一条)
        subquery = (
            db.query(MaterialReadiness.project_id, func.max(MaterialReadiness.id).label("max_id"))
        ...
        # 预警汇总
        alert_summary = {
            level: db.query(ShortageDetail)
            .filter(ShortageDetail.alert_level == level, ShortageDetail.shortage_qty > 0)
            .count()
        ...
        for s in pending_suggestions:
            ...
            machine = (
                db.query(Machine).filter(Machine.id == s.machine_id).first()
                if s.machine_id
                else None
            )
    ...
    def _get_dashboard_handler(
        ...
        try:
            ...
            return ResponseModel(code=200, message="success", data=dashboard_response)

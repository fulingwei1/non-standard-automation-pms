...
async def execute_kit_analysis(
    ...
    # 计算整体齐套率
    overall_kit_rate = (
        Decimal(overall_stats["fulfilled"] / overall_stats["total"] * 100)
        if overall_stats["total"] > 0
        else Decimal(100)
    )
    blocking_kit_rate = (
        Decimal(overall_stats["blocking_fulfilled"] / overall_stats["blocking_total"] * 100)
        if overall_stats["blocking_total"] > 0
        else Decimal(100)
    )
    ...
    readiness = MaterialReadiness(
        ...
        planned_start_date=getattr(request, "planned_start_date", None)
        ...
        stage_kit_rates=json.dumps(
            {
                s.stage_code: {
                    "kit_rate": float(s.kit_rate),
                    "blocking_rate": float(s.blocking_rate),
...
async def get_analysis_detail(readiness_id: int, db: Session = Depends(deps.get_db)):
    """获取齐套分析详情"""
    ...
    # 获取缺料明细
    shortage_details = (
        db.query(ShortageDetail).filter(ShortageDetail.readiness_id == readiness_id).all()
    )
    ...
    try:
        ...
        optimization_suggestions = AssemblyKitOptimizer.generate_optimization_suggestions(
            db, readiness
        )
    except Exception:

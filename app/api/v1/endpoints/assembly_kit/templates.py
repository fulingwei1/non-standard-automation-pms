...
router = APIRouter(prefix="/assembly-kit/templates", tags=["templates"])
...
async def get_assembly_templates(
    ...
    templates = query.order_by(
        AssemblyTemplate.is_default.desc(), AssemblyTemplate.created_at.desc()
    ).all()
...
async def create_assembly_template(
    ...
    existing = (
        ...
        .filter(AssemblyTemplate.template_code == template_data.template_code)
        .first()
    )
    if existing:
    ...
    return ResponseModel(
        code=200, message="创建成功", data=AssemblyTemplateResponse.model_validate(template)
    )
...
async def update_assembly_template(
    ...
    return ResponseModel(
        code=200, message="更新成功", data=AssemblyTemplateResponse.model_validate(template)
    )

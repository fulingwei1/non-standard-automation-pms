...
from app.core.auth import get_current_user, is_superuser
...
class BackupRequest(BaseModel):
    ...
    backup_type: str = Field(default="full", description="备份类型: full/database/files")
...
async def list_backups(
    backup_type: str = "database", current_user: User = Depends(get_current_user)
):
    """
    列出所有备份文件
...
async def verify_backup(backup_file: str, current_user: User = Depends(get_current_user)):
    """
    验证备份文件完整性
...
async def check_backup_health(current_user: User = Depends(get_current_user)):
    """
    检查备份系统健康状态

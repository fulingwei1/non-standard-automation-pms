...
router = APIRouter(prefix="/bonus/statistics", tags=["statistics"])
...
@router.get(
    "/statistics",
    response_model=ResponseModel[BonusStatisticsResponse],
...
def get_bonus_statistics(
    *,
    ...
    total_calculated = calc_query.with_entities(
    ...
    total_distributed = dist_query.filter(BonusDistribution.status == "PAID").with_entities(
        func.sum(BonusDistribution.distributed_amount)
    ).scalar() or Decimal("0")

    total_pending = dist_query.filter(BonusDistribution.status == "PENDING").with_entities(
        func.sum(BonusDistribution.distributed_amount)
    ).scalar() or Decimal("0")
    ...
    by_type = {}
    type_query = (
        calc_query.join(BonusRule)
        .with_entities(BonusRule.bonus_type, func.sum(BonusCalculation.calculated_amount))
        .group_by(BonusRule.bonus_type)
        .all()
    )
    ...
    by_department = {}
    dept_query = (
        calc_query.join(User, BonusCalculation.user_id == User.id)
        .with_entities(User.department, func.sum(BonusCalculation.calculated_amount))

...
async def get_change_impact(
    ...
    # 获取最新的分析结果
    analysis = (
        db.query(ChangeImpactAnalysis)
        .filter(ChangeImpactAnalysis.change_request_id == change_id)
...
@router.post(
    "/changes/{change_id}/suggestions", response_model=List[ChangeResponseSuggestionResponse]
)
async def generate_suggestions(
    ...
    try:
        ...
        suggestions = await service.generate_suggestions(
            change_id, analysis.id, current_user.id, request.max_suggestions
        )
        return suggestions
    except Exception as e:
...
async def list_suggestions(
    ...
    """获取方案列表"""
    suggestions = (
        db.query(ChangeResponseSuggestion)
        .filter(ChangeResponseSuggestion.change_request_id == change_id)
        .order_by(
...
@router.get(
    "/changes/{change_id}/suggestions/{suggestion_id}",
    response_model=ChangeResponseSuggestionResponse,
)
...
async def get_impact_stats(
    ...
    # 按风险级别统计
    risk_stats = (
        db.query(ChangeImpactAnalysis.overall_risk_level, func.count(ChangeImpactAnalysis.id))
        .group_by(ChangeImpactAnalysis.overall_risk_level)
        .all()
    )
    ...
    chain_reaction_count = (
        db.query(func.count(ChangeImpactAnalysis.id))
        .filter(ChangeImpactAnalysis.chain_reaction_detected == True)
        .scalar()
    )

    critical_path_count = (
        db.query(func.count(ChangeImpactAnalysis.id))
        .filter(ChangeImpactAnalysis.schedule_critical_path_affected == True)
        .scalar()
    )

    budget_exceeded_count = (
        db.query(func.count(ChangeImpactAnalysis.id))
        .filter(ChangeImpactAnalysis.cost_budget_exceeded == True)
        .scalar()
    )
    ...
    return {
        ...
        "chain_reaction_rate": float(
            chain_reaction_count / total_analyses * 100 if total_analyses > 0 else 0
        ),
        "critical_path_impact_rate": float(
            critical_path_count / total_analyses * 100 if total_analyses > 0 else 0
        ),
        "budget_exceeded_rate": float(
            budget_exceeded_count / total_analyses * 100 if total_analyses > 0 else 0
        ),
    }
...
async def get_effectiveness(
    ...
    selected = (
        db.query(func.count(ChangeResponseSuggestion.id))
        .filter(ChangeResponseSuggestion.status == "SELECTED")
        .scalar()
    )
    implemented = (
        db.query(func.count(ChangeResponseSuggestion.id))
        .filter(ChangeResponseSuggestion.implementation_status == "COMPLETED")
        .scalar()
    )

    avg_feasibility = db.query(func.avg(ChangeResponseSuggestion.feasibility_score)).scalar() or 0
    avg_effectiveness = (
        db.query(func.avg(ChangeResponseSuggestion.effectiveness_score)).scalar() or 0
    )
    avg_ai_score = (
        db.query(func.avg(ChangeResponseSuggestion.ai_recommendation_score)).scalar() or 0
    )

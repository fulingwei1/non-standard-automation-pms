...
from app.common.date_range import get_last_month_range, get_month_range, get_week_range
...
def get_sales_stats(db: Session, current_user: User) -> dict:
    ...
    # 活跃线索统计
    active_leads = (
        db.query(func.count(Lead.id)).filter(Lead.status.notin_(["CONVERTED", "LOST"])).scalar()
        or 0
    )

    last_month_leads = (
        db.query(func.count(Lead.id))
        .filter(Lead.status.notin_(["CONVERTED", "LOST"]), Lead.created_at < month_start)
        .scalar()
        or 0
    )
    ...
    return create_stats_response(
        [
            ...
            create_stat_card(
                "revenue", "本月签约", format_currency(monthly_revenue), trend=revenue_trend
            ),
        ]
    )
...
def get_engineer_stats(db: Session, current_user: User) -> dict:
    ...
    # 待处理ECN（用户提交的待审核ECN）
    pending_ecn = (
        db.query(func.count(Ecn.id))
        .filter(Ecn.applicant_id == user_id, Ecn.status.in_(["DRAFT", "PENDING_REVIEW"]))
        .scalar()
        or 0
    )
    ...
    pending_evaluations = (
        db.query(func.count(EcnEvaluation.id))
        .filter(EcnEvaluation.evaluator_id == user_id, EcnEvaluation.status == "PENDING")
        .scalar()
        or 0
    )
    ...
    return create_stats_response(
        [
            ...
            create_stat_card(
                "hours-logged", "本周工时", format_hours(weekly_hours), trend=round(hours_trend, 1)
            ),
...
def get_procurement_stats(db: Session, current_user: User) -> dict:
    ...
    pending_orders = (
        db.query(func.count(PurchaseOrder.id))
        .filter(PurchaseOrder.status.in_(pending_statuses))
        .scalar()
        or 0
    )
    ...
    overdue_orders = (
        db.query(func.count(PurchaseOrder.id))
        .filter(
            PurchaseOrder.status.in_(not_completed_statuses), PurchaseOrder.required_date < today
        )
        .scalar()
        or 0
    )
...
def get_production_stats(db: Session, current_user: User) -> dict:
    ...
    total_orders = (
        db.query(func.count(WorkOrder.id)).filter(WorkOrder.status.in_(active_statuses)).scalar()
        or 0
    )
    ...
    in_progress = (
        ...
        .filter(WorkOrder.status.in_(["STARTED", "PAUSED"]))
        .scalar()
        or 0
    )
    ...
    # 良品率计算（合格数量 / 完成数量）
    yield_data = (
        db.query(func.sum(WorkOrder.qualified_qty), func.sum(WorkOrder.completed_qty))
        .filter(WorkOrder.status.in_(["COMPLETED", "APPROVED"]), WorkOrder.completed_qty > 0)
        .first()
    )
...
def get_dashboard_stats(
    ...
    if stats_type not in STATS_FUNCTIONS:
        return ResponseModel(code=400, message=f"不支持的统计类型: {stats_type}", data=None)
    ...
    try:
        ...
        return ResponseModel(code=500, message=f"获取统计数据失败: {str(e)}", data=None)

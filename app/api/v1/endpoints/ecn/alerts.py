...
def _pending_approval_tasks(
    ...
    *,
    ...
    if approval_level is None:
        return tasks
    return [task for task in tasks if _approval_task_level(task) == approval_level]
...
def _find_users_for_alert(db: Session, alert_type: str, alert_data: dict, ecn: Ecn) -> List[int]:
    ...
    if alert_type == "EVALUATION_OVERDUE":
        ...
        task_id = alert_data.get("approval_task_id") or alert_data.get("task_id")
        if task_id:
            task = (
                db.query(ApprovalTask)
                .join(ApprovalInstance, ApprovalTask.instance_id == ApprovalInstance.id)
            ...
            if approval_level:
                for task in _pending_approval_tasks(
                    db,
                    ecn_id=ecn.id,
...
def get_ecn_overdue_alerts(
    ...
    pending_evals = (
        db.query(EcnEvaluation)
        .filter(EcnEvaluation.status == "PENDING", EcnEvaluation.created_at < eval_timeout)
        .all()
    )
    ...
    overdue_tasks = (
        db.query(EcnTask)
        .filter(EcnTask.status.in_(["PENDING", "IN_PROGRESS"]), EcnTask.planned_end < now.date())
        .all()
    )
...
@router.post(
    "/ecns/batch-process-overdue-alerts",
    ...
    *,
    ...
    for alert_data in alerts:
        try:
            ...
            if not alert_type or not ecn_id:
                results.append({"alert": alert_data, "status": "failed", "message": "缺少必要字段"})
                fail_count += 1
                continue
            ...
            ecn = db.query(Ecn).filter(Ecn.id == ecn_id).first()
            if not ecn:
                results.append({"alert": alert_data, "status": "failed", "message": "ECN不存在"})
                fail_count += 1
                continue
            ...
            if alert_type == "EVALUATION_OVERDUE":
                ...
                if eval_dept:
                    ...
                    if evaluation and evaluation.evaluator_id:
                        ...
                        dept = (
                            db.query(Department).filter(Department.dept_name == eval_dept).first()
                        )
                        if dept and dept.manager_id:
                            user_ids.append(dept.manager_id)
                        else:
                ...
                # 审批超时：找到审批人员
                user_ids.extend(_find_users_for_alert(db, alert_type, alert_data, ecn))
                ...
                if task_id:
                    task = (
                        db.query(EcnTask)
                        .filter(EcnTask.id == task_id, EcnTask.ecn_id == ecn_id)
                        .first()
                    )
            ...
            if user_ids:
                ...
                results.append(
                    {"alert": alert_data, "status": "failed", "message": "未找到相关人员"}
                )
                fail_count += 1

        except Exception as e:
            results.append({"alert": alert_data, "status": "failed", "message": str(e)})
            fail_count += 1
    ...
    return ResponseModel(
        ...
        data={"success_count": success_count, "fail_count": fail_count, "results": results},
    )

...
@router.post(
    "/ecns/{ecn_id}/sync-to-bom", response_model=ResponseModel, status_code=status.HTTP_200_OK
)
def sync_ecn_to_bom(
    *,
    ...
    try:
        ...
        return ResponseModel(
            code=200, message=f"已同步{result['updated_count']}个物料变更到BOM", data=result
        )
...
@router.post(
    "/ecns/{ecn_id}/sync-to-project", response_model=ResponseModel, status_code=status.HTTP_200_OK
)
def sync_ecn_to_project(
    *,
    ...
    try:
        ...
        return ResponseModel(code=200, message="ECN变更已同步到项目", data=result)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@router.post(
    "/ecns/{ecn_id}/sync-to-purchase", response_model=ResponseModel, status_code=status.HTTP_200_OK
)
def sync_ecn_to_purchase(
    *,
    ...
    try:
        ...
        return ResponseModel(
            code=200, message=f"已同步{result['updated_count']}个采购订单变更", data=result
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@router.post(
    "/ecns/batch-sync-to-bom", response_model=ResponseModel, status_code=status.HTTP_200_OK
)
...
@router.post(
    "/ecns/batch-sync-to-project", response_model=ResponseModel, status_code=status.HTTP_200_OK
)
...
@router.post(
    "/ecns/batch-sync-to-purchase", response_model=ResponseModel, status_code=status.HTTP_200_OK
)
...
@router.post(
    "/ecns/{ecn_id}/batch-create-tasks",
...
def batch_create_ecn_tasks(
    *,
    ...
    try:
        ...
        return ResponseModel(
            code=200, message=f"批量创建任务成功：共{result['created_count']}个", data=result
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

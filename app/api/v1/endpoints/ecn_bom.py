...
def create_ecn(
    ...
    try:
        ...
        return {
            ...
            "affected_projects": safe_json_loads(row["affected_projects"], []),
            "status": row["status"],
...
def list_ecn(
    ...
    try:
        ...
        items = []
        for row in rows:
            affected_projects_list = safe_json_loads(row["affected_projects"], [])
            items.append({
                "id": row["id"],
                "ecn_no": row["ecn_no"],
                "title": row["title"],
                "description": row["description"],
                "change_type": row["change_type"],
                "affected_projects": affected_projects_list,
                "affected_projects_count": len(affected_projects_list),
                "status": row["status"],
                "created_by": row["created_by"],
                "priority": row["priority"],
                "created_at": row["created_at"],
...
def apply_ecn_to_bom(ecn_id: int):
    ...
    try:
        ...
        affected_projects = safe_json_loads(ecn_row["affected_projects"], [])

...
def init_tables():
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_ecn_records_change_type ON ecn_records(change_type)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_ecn_bom_changes_ecn_id ON ecn_bom_changes(ecn_id)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_ecn_bom_changes_project_id ON ecn_bom_changes(project_id)"
    )
...
def generate_ecn_no(db_conn) -> str:
    ...
    cursor.execute(
        "SELECT ecn_no FROM ecn_records WHERE ecn_no LIKE ? ORDER BY ecn_no DESC LIMIT 1",
        (f"{prefix}%",),
    )
    row = cursor.fetchone()
...
def create_ecn(
    ...
    if change_type not in valid_change_types:
        raise HTTPException(
            status_code=400, detail=f"无效的变更类型，必须是：{', '.join(valid_change_types)}"
        )
    ...
    if priority not in valid_priorities:
        raise HTTPException(
            status_code=400, detail=f"无效的优先级，必须是：{', '.join(valid_priorities)}"
        )
    ...
    try:
        ...
        """,
            (
                ...
                description,
                change_type,
                json.dumps(affected_projects_list),
                created_by,
                priority,
            ),
        )
...
def list_ecn(
    status: Optional[str] = Query(
        None, description="状态筛选：draft/reviewing/approved/implemented/closed"
    ),
    change_type: Optional[str] = Query(
        None, description="变更类型筛选：材料替换/设计变更/工艺优化"
    ),
            ...
            bom_changes.append(
                {
                    ...
                    "old_value": (
                        json.loads(change_row["old_value"]) if change_row["old_value"] else None
                    ),
                    "new_value": (
                        json.loads(change_row["new_value"]) if change_row["new_value"] else None
                    ),
                    "cost_impact": (
                        float(change_row["cost_impact"]) if change_row["cost_impact"] else 0
                    ),
...
def get_ecn_impact(ecn_id: int):
    ...
    try:
        ...
        # 预估成本影响（根据变更类型和优先级估算）
        priority_multiplier = {"low": 1.0, "medium": 1.5, "high": 2.0, "urgent": 3.0}.get(
            ecn_row["priority"], 1.0
        )

        change_type_base_cost = {"材料替换": 5000, "设计变更": 10000, "工艺优化": 3000}.get(
            ecn_row["change_type"], 5000
        )

        estimated_cost = change_type_base_cost * affected_projects_count * priority_multiplier

        # 影响分析详情
        impact_details = {"projects": [], "bom_items": [], "cost_breakdown": {}}
        ...
        for project_id in affected_projects:
            ...
            impact_details["projects"].append(
                {"project_id": project_id, "bom_changes_count": proj_row["count"] or 0}
            )
        ...
        impact_details["cost_breakdown"] = {
            "material_replacement": (
                estimated_cost * 0.5 if ecn_row["change_type"] == "材料替换" else 0
            ),
            "design_change": estimated_cost * 0.7 if ecn_row["change_type"] == "设计变更" else 0,
            "process_optimization": (
                estimated_cost * 0.3 if ecn_row["change_type"] == "工艺优化" else 0
            ),

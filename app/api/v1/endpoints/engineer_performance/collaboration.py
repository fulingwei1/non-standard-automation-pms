...
async def get_collaboration_matrix(
    ...
    return ResponseModel(code=200, message="success", data=matrix)
...
async def get_ratings_received(
    ...
    ratings, total = service.get_ratings_received(
        user_id=user_id, period_id=period_id, limit=pagination.limit, offset=pagination.offset
    )
...
async def get_pending_ratings(
    ...
    return ResponseModel(code=200, message="success", data=pending)
...
async def auto_select_collaborators(
    ...
    try:
        collaborators = service.auto_select_collaborators(engineer_id, period_id, target_count)
        invitations = service.create_rating_invitations(engineer_id, period_id, collaborators)
...
async def submit_rating(
    ...
    try:
        ...
        return ResponseModel(
            code=200,
            message="评价提交成功",
            data={"rating_id": rating.id, "total_score": float(rating.total_score or 0)},
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_rating_statistics(
    ...
    try:
        ...
        return ResponseModel(code=200, message="获取评价统计成功", data=stats)
...
async def get_collaboration_trend(
    ...
    try:
        ...
        return ResponseModel(code=200, message="获取协作趋势成功", data=trend)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def analyze_rating_quality(
    ...
    try:
        ...
        return ResponseModel(code=200, message="评价质量分析成功", data=analysis)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def auto_complete_missing_ratings(
    ...
    try:
        count = service.auto_complete_missing_ratings(period_id, Decimal(str(default_score)))
        return ResponseModel(
            code=200, message=f"自动完成{count}个缺失评价", data={"completed_count": count}
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

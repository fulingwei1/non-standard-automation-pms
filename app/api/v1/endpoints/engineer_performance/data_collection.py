...
async def get_data_collection(
    ...
    return ResponseModel(code=200, message="数据采集成功", data=data)
...
async def extract_self_evaluation(
    ...
    result = collector.extract_self_evaluation_from_work_logs(engineer_id, start_date, end_date)

    return ResponseModel(code=200, message="自我评价提取成功", data=result)
...
async def trigger_collect_all(
    ...
    data = collector.collect_all_data(engineer_id, period.start_date, period.end_date)

    return ResponseModel(code=200, message="数据采集完成", data=data)
...
async def generate_collection_report(
    ...
    report = collector.generate_collection_report(engineer_id, start_date, end_date)

    return ResponseModel(code=200, message="数据采集报告生成成功", data=report)

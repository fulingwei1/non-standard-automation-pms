...
async def check_data_completeness(
    ...
    return ResponseModel(code=200, message="数据完整性检查完成", data=report)
...
async def generate_data_quality_report(
    ...
    return ResponseModel(code=200, message="数据质量报告生成成功", data=report)
...
async def get_missing_data_reminders(
    ...
    return ResponseModel(code=200, message="获取数据缺失提醒成功", data=reminders)
...
async def get_auto_fix_suggestions(
    ...
    return ResponseModel(code=200, message="获取自动修复建议成功", data=suggestions)
...
async def auto_fix_data_issues(
    ...
    try:
        ...
        return ResponseModel(code=200, message="自动修复完成", data=result)
...
async def send_data_missing_reminders(
    ...
    try:
        result = service.send_data_missing_reminders(
            period_id=period_id, reminder_types=reminder_types
        )
        return ResponseModel(code=200, message=f"已发送{result['sent_count']}个提醒", data=result)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def export_data_quality_report(
    ...
    try:
        report = service.export_data_quality_report(
            period_id=period_id, department_id=department_id, format=format
        )
        return ResponseModel(code=200, message="报告导出成功", data=report)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

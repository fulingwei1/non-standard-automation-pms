...
async def generate_work_logs(
    ...
    if request.start_date and request.end_date:
        ...
        # 默认生成昨日工作日志
        stats = generator.generate_yesterday_work_logs(auto_submit=request.auto_submit)

    return ResponseModel(code=200, message="工作日志生成完成", data=stats)
...
async def sync_design_reviews(
    ...
    return ResponseModel(code=200, message="设计评审同步完成", data=stats)
...
async def sync_debug_issues(
    ...
    return ResponseModel(code=200, message="调试问题同步完成", data=stats)
...
async def identify_knowledge(
    ...
    return ResponseModel(
        code=200,
        message="知识贡献识别完成",
        data={"ticket_stats": ticket_stats, "kb_stats": kb_stats},
    )
...
async def sync_all_performance_data(
    ...
    if request.start_date and request.end_date:
        ...
        work_log_stats = generator.generate_yesterday_work_logs(auto_submit=request.auto_submit)
    ...
    return ResponseModel(code=200, message="所有绩效数据同步完成", data=results)
...
async def get_sync_status(
    ...
    status = {
        ...
        "design_review": {"description": "设计评审自动同步", "last_sync": None, "recent_count": 0},
        "debug_issue": {"description": "调试问题自动同步", "last_sync": None, "recent_count": 0},
        "knowledge": {"description": "知识贡献自动识别", "last_sync": None, "recent_count": 0},
    }

    # 这里可以添加实际的统计逻辑
    # 例如从数据库查询最近同步的记录数量

    return ResponseModel(code=200, message="获取同步状态成功", data=status)

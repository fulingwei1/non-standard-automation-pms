...
async def adjust_performance(
    ...
    try:
        result = service.adjust_performance(
            ...
            new_total_score=(
                Decimal(str(request.new_total_score)) if request.new_total_score else None
            ),
...
async def get_adjustment_history(
    result_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    ...
    return ResponseModel(code=200, message="获取调整历史成功", data=history)
...
async def get_evaluation_tasks(
    ...
    tasks = service.get_manager_evaluation_tasks(manager_id=current_user.id, period_id=period_id)
...
async def submit_evaluation(
    ...
    try:
        ...
        return ResponseModel(code=200, message="评价提交成功", data=result)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_engineers_for_evaluation(
    ...
    try:
        engineers = service.get_engineers_for_evaluation(
            manager_id=current_user.id, period_id=period_id
        )

        return ResponseModel(code=200, message="获取工程师列表成功", data=engineers)

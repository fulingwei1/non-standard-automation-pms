...
async def get_engineer_trend(
    ...
    return ResponseModel(code=200, message="获取趋势数据成功", data=trend_data)
...
async def identify_ability_changes(
    ...
    return ResponseModel(code=200, message="识别能力变化成功", data=changes)
...
async def get_department_trend(
    ...
    return ResponseModel(code=200, message="获取部门趋势数据成功", data=trend_data)
...
async def compare_departments(
    ...
    try:
        comparison = service.compare_departments(
            department_ids=request.department_ids, period_id=request.period_id
        )
        return ResponseModel(code=200, message="部门对比分析成功", data=comparison)

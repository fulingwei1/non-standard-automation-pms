...
def get_my_projects(
    ...
    project_members = (
        ...
        .filter(and_(ProjectMember.user_id == current_user.id, ProjectMember.is_active))
        .all()
    )
    ...
    for project in projects:
        ...
        my_allocation = next(
            (pm.allocation_pct for pm in project_members if pm.project_id == project.id), 100
        )
        ...
        my_tasks = (
            ...
            .filter(
                and_(
                    TaskUnified.project_id == project.id, TaskUnified.assignee_id == current_user.id
                )
            )
            .all()
        )
        ...
        task_stats = schemas.TaskStatsResponse(
            ...
            pending_approval_tasks=len(
                [t for t in my_tasks if t.approval_status == "PENDING_APPROVAL"]
            ),
        )

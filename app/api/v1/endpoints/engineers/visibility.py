...
def _collect_department_stats(db: Session, all_tasks, User):
    ...
    dept_stats = defaultdict(
        lambda: {
            ...
            "delayed_tasks": 0,
            "members": defaultdict(
                lambda: {"name": "", "total_tasks": 0, "completed": 0, "in_progress": 0}
            ),
        }
    )
    ...
    for idx, (dept_name, stats) in enumerate(dept_stats.items(), 1):
        ...
        members = [
            schemas.MemberProgressSummary(
                ...
                progress_pct=(
                    (m["completed"] / m["total_tasks"] * 100) if m["total_tasks"] > 0 else 0
                ),
            )
        ...
        department_progress.append(
            schemas.DepartmentProgressSummary(
                department_id=idx,
...
def _build_active_delays(db: Session, delayed_tasks, User, schemas):
    ...
    for task in delayed_tasks:
        ...
        active_delays.append(
            schemas.ActiveDelayInfo(
                task_id=task.id,
...
@router.get(
    ...
    response_model=schemas.ProjectProgressVisibilityResponse,
)
def get_project_progress_visibility(
    ...
    stage_progress = {
        stage.stage_code: schemas.StageProgressSummary(
            progress=float(stage.progress_pct) if stage.progress_pct else 0.0, status=stage.status
        )
        for stage in stages
    }
    ...
    # 活跃延期列表
    delayed_tasks = [
        t for t in all_tasks if t.is_delayed and t.status not in ["COMPLETED", "CANCELLED"]
    ]

...
def export_issues(
        ...
        data.append(
            {
                ...
                "提出时间": (
                    issue.report_date.strftime("%Y-%m-%d %H:%M:%S") if issue.report_date else ""
                ),
    ...
    return StreamingResponse(
        ...
        headers={
            ...
            "Content-Disposition": (
                "attachment; " 'filename="issues.xlsx"; ' f"filename*=UTF-8''{encoded_filename}"
            )
        },
    )
        ...
        for index, row in df.iterrows():
            try:
                ...
                issue = Issue(
                    ...
                    due_date=(
                        pd.to_datetime(row["要求完成日期"]).date()
                        if pd.notna(row.get("要求完成日期"))
                        else None
                    ),
                    status=str(row.get("状态", "OPEN")).strip(),
                    is_blocking=str(row.get("是否阻塞", "否")).strip().lower()
                    in ["是", "true", "1"],
                )

...
def _build_issue_list_response(
    issues: list, total: int, page: int, page_size: int
) -> IssueListResponse:
    """构建问题列表响应"""
    items = []
    for issue in issues:
...
def get_project_issues(
    ...
    query = (
        ...
        .filter(Issue.project_id == project_id, Issue.status != "DELETED")
    )
...
def get_machine_issues(
    ...
    query = (
        db.query(Issue)
        .options(joinedload(Issue.service_ticket))
        .filter(Issue.machine_id == machine_id, Issue.status != "DELETED")
    )
    query = DataScopeService.filter_issues_by_scope(db, query, current_user)
...
def get_task_issues(
    ...
    query = (
        db.query(Issue)
        .options(joinedload(Issue.service_ticket))
        .filter(Issue.task_id == task_id, Issue.status != "DELETED")
    )
    query = DataScopeService.filter_issues_by_scope(db, query, current_user)
...
def get_acceptance_order_issues(
    ...
    query = db.query(Issue).filter(Issue.acceptance_order_id == order_id, Issue.status != "DELETED")
    query = DataScopeService.filter_issues_by_scope(db, query, current_user)

...
class KitRateDashboardEndpoint(BaseDashboardEndpoint):
    ...
    def __init__(self):
        ...
        self.router.add_api_route(
            "/kit-rate/dashboard", dashboard_endpoint, methods=["GET"], summary="获取齐套看板数据"
        )
        ...
        self.router.add_api_route(
            "/kit-rate/trend", trend_endpoint, methods=["GET"], summary="获取齐套率趋势分析"
        )
        ...
        async def snapshots_endpoint(
            ...
            snapshot_type: Optional[str] = Query(
                None, description="快照类型: DAILY/STAGE_CHANGE/MANUAL"
            ),
            current_user: User = Depends(user_dependency),
        ):
    ...
    def _get_trend_handler(
        ...
        if group_by not in ["day", "month"]:
            raise HTTPException(status_code=400, detail="group_by 必须是 day 或 month")

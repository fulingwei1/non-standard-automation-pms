...
def list_lessons(
    *,
    ...
    category: Optional[str] = Query(
        None, description="分类：technical/management/quality/cost/schedule/customer"
    ),
    ...
    total = (
        db.execute(count_sql, {k: v for k, v in params.items() if k not in ["offset", "limit"]})
        .fetchone()
        .total
    )
...
def get_stats(
    *,
    ...
    by_category = [
        {"category": r.category, "count": r.count} for r in db.execute(category_sql).fetchall()
    ]
    ...
    by_type = [
        {"lesson_type": r.lesson_type, "count": r.count} for r in db.execute(type_sql).fetchall()
    ]

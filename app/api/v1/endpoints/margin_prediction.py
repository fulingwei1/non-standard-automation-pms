...
def predict_margin(
    *,
    ...
    data_quality = "优秀" if input_completeness >= 0.75 else "良好" if input_completeness >= 0.5 else "一般" if input_completeness >= 0.25 else "不足"
    
    # ========== 8. 成本结构 ==========
    cost_structure = [
        {"cost_type": "BOM 物料成本", "amount": round(bom_cost, 0), "percentage": round(bom_cost / risk_adjusted_cost * 100, 1) if risk_adjusted_cost > 0 else 0},
        {"cost_type": "设计变更物料", "amount": round(design_change_cost, 0), "percentage": round(design_change_cost / risk_adjusted_cost * 100, 1) if risk_adjusted_cost > 0 else 0},
        {"cost_type": "研发工时", "amount": round(rd_labor_cost, 0), "percentage": round(rd_labor_cost / risk_adjusted_cost * 100, 1) if risk_adjusted_cost > 0 else 0, "detail": f"{rd_hours}小时 × ¥{rd_hourly_rate:.0f}/小时"},
        {"cost_type": "生产人工", "amount": round(production_labor_cost, 0), "percentage": round(production_labor_cost / risk_adjusted_cost * 100, 1) if risk_adjusted_cost > 0 else 0},
        {"cost_type": "出差费用", "amount": round(travel_cost, 0), "percentage": round(travel_cost / risk_adjusted_cost * 100, 1) if risk_adjusted_cost > 0 else 0},
        {"cost_type": "制造费用", "amount": round(overhead_cost, 0), "percentage": round(overhead_cost / risk_adjusted_cost * 100, 1) if risk_adjusted_cost > 0 else 0},
        {"cost_type": "风险准备金", "amount": round(risk_reserve, 0), "percentage": round(risk_reserve / risk_adjusted_cost * 100, 1) if risk_adjusted_cost > 0 else 0},
    ]
    ...
    if input_completeness < 0.5:
        recommendations.append(f"📊 当前输入完整度{int(input_completeness*100)}%, 建议补充：{['物料成本','设计变更费用','出差费用','研发工时'][int((1-input_completeness)*4)]}")
    ...
    if design_change_cost / bom_cost > 0.08:
        recommendations.append(f"⚠️ 设计变更费用占比过高 ({design_change_cost/bom_cost*100:.1f}%), 建议加强前期需求评审，减少后期变更")
    
    if rd_labor_cost / contract_amount > 0.25:
        recommendations.append(f"💰 研发成本占比偏高 ({rd_labor_cost/contract_amount*100:.1f}%), 建议评估模块化设计或复用已有方案")
    
    if travel_cost / contract_amount > 0.05:
        recommendations.append(f"✈️ 出差费用占比过高 ({travel_cost/contract_amount*100:.1f}%), 建议优化现场调试流程或远程支持")
    
    if predicted_margin < 20:
        recommendations.append("⚠️ 预测毛利率偏低，建议：1) 优化设计方案降低成本 2) 与供应商协商价格 3) 提高报价")
    
    if industry_coef["risk_factor"] > 1.2:
        recommendations.append(f"⚡ {industry or '该'}行业风险系数较高 ({industry_coef['risk_factor']}), 建议预留充足风险准备金")
...
def get_project_bom_costs(
    *,
    ...
    bom_headers = db.query(BomHeader).filter(
        ...
        BomHeader.status == 'RELEASED'  # 只统计已发布的 BOM
    ).all()
    ...
    # 查询最新采购价格
    purchase_prices = {}
    purchase_orders = db.query(PurchaseOrderItem, PurchaseOrder.order_date)\
        .join(PurchaseOrder)\

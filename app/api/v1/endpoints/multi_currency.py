...
def get_project_currency_summary(
    ...
    # 获取项目基本信息（兼容旧库 projects 表没有 currency 列）
    try:
        cursor.execute("""
        ...
        if "no such column: currency" not in str(exc):
            conn.close()
            raise
    ...
    original_currency = project["original_currency"] if "original_currency" in project.keys() else "CNY"

...
def get_permission(
    ...
    if not permission:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="权限不存在或无访问权限")
...
def create_permission(
    ...
    if service.check_permission_code_exists(perm_code, current_user.tenant_id):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"权限编码 {perm_code} 已存在"
        )
...
def update_permission(
    ...
    if permission.is_system:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="系统预置权限不允许修改"
        )
...
def delete_permission(
    ...
    if not permission or permission.tenant_id != current_user.tenant_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="权限不存在或无删除权限")

    if permission.is_system:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="系统预置权限不允许删除"
        )
...
def get_role_permissions(
    ...
    if not role:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="角色不存在或无访问权限")
...
def get_user_permissions(
    ...
    if not current_user.is_superuser and user.tenant_id != current_user.tenant_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="无权查询其他租户的用户权限"
        )
...
def check_user_permission(
    ...
    # 检查权限
    has_permission = service.check_user_permission(user_id, permission_code, user, user.tenant_id)

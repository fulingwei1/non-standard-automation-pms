...
async def predict_win_rate(
    ...
    salesperson_stats = (
        db.query(
            ...
            func.sum(func.case((Project.outcome == LeadOutcomeEnum.WON.value, 1), else_=0)).label(
                "won"
            ),
        )
        .filter(
    ...
    similar_count = (
        ...
        .filter(
            Project.evaluation_score.between(
                request.dimension_scores.total_score - 10, request.dimension_scores.total_score + 10
            ),
            Project.outcome.in_([LeadOutcomeEnum.WON.value, LeadOutcomeEnum.LOST.value]),
        )
        .scalar()

    ...
    if dimension == "workshop":
        ...
        items = [
            ...
            for idx, row in enumerate(
                sorted(results, key=lambda x: float(x.avg_oee) if x.avg_oee else 0, reverse=True)
            )
        ]
        ...
        items = [
            ...
            for idx, row in enumerate(
                sorted(
                    results,
                    key=lambda x: float(x.avg_efficiency) if x.avg_efficiency else 0,
                    reverse=True,
                )
            )
        ]
        ...
        if compare_periods:
            ...
            items = [
                {
                    ...
                    "avg_quality": (
                        float(current_data.avg_quality) if current_data.avg_quality else 0
                    ),
                },
                {
                    "period": "上一期",
            ...
            output_change = (
                (
                    (current_data.total_output - prev_data.total_output)
                    / prev_data.total_output
                    * 100
                )
                if prev_data.total_output > 0
                else 0
            )

...
def create_workstation(
    *,
    ...
    existing = (
        ...
        .filter(Workstation.workstation_code == workstation_in.workstation_code)
        .first()
    )
    if existing:
        raise HTTPException(status_code=400, detail="工位编码已存在")

    workstation = Workstation(workshop_id=workshop_id, status="IDLE", **workstation_in.model_dump())
...
def get_workstation_status(
    *,
    ...
    if workstation.current_work_order_id:
        work_order = (
            db.query(WorkOrder).filter(WorkOrder.id == workstation.current_work_order_id).first()
        )

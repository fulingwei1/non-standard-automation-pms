...
async def extract_lessons(
    ...
    lessons_data = extractor.extract_lessons(
        review_id=request.review_id, min_confidence=request.min_confidence
    )
...
@router.get("/", response_model=List[ProjectLessonResponse], include_in_schema=False)
async def list_lessons(
    ...
    lessons = (
        query.order_by(ProjectLesson.priority.desc(), ProjectLesson.ai_confidence.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
async def categorize_lessons(
    ...
    return {"review_id": review_id, "categorized": categorized, "total_count": len(lessons)}

...
async def get_review_stats(
    ...
    ai_generated_count = (
        db.query(func.count(ProjectReview.id)).filter(ProjectReview.ai_generated == True).scalar()
    )

    avg_quality_score = (
        db.query(func.avg(ProjectReview.quality_score))
        .filter(ProjectReview.quality_score.isnot(None))
        .scalar()
    )

    by_type = (
        db.query(ProjectReview.review_type, func.count(ProjectReview.id))
        .group_by(ProjectReview.review_type)
        .all()
    )

    by_status = (
        db.query(ProjectReview.status, func.count(ProjectReview.id))
        .group_by(ProjectReview.status)
        .all()
    )

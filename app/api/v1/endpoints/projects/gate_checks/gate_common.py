...
def check_gate_detailed(db: Session, project: Project, target_stage: str) -> Dict[str, Any]:
    ...
    if target_stage == "S2":
        conditions = [
            GateCheckCondition(
                ...
                status=(
                    ...
                    and project.customer_name
                    and project.customer_contact
                    and project.customer_phone
                    else "FAILED"
                ),
                message=(
                    "客户信息已完整"
                    if project.customer_id and project.customer_name
                    else "请填写客户信息"
                ),
        ...
        conditions = [
            GateCheckCondition(
                ...
                message=(
                    f"已确认 {spec_docs_count} 个规格书文档"
                    if spec_docs_count > 0
                    else "请上传并确认需求规格书"
                ),
            ...
            GateCheckCondition(
                ...
                status=(
                    ...
                    and (
                        "验收标准" in project.requirements
                        or "acceptance" in project.requirements.lower()
                    )
                    else "FAILED"
                ),
                message=(
    ...
    if not gate_passed:
        ...
        if missing_items:
            suggestions.append(
                f"缺失项：{', '.join(missing_items[:3])}{'...' if len(missing_items) > 3 else ''}"
            )

...
def check_gate_s6_to_s7(db: Session, project: Project) -> Tuple[bool, List[str]]:
    ...
    for machine in machines:
        if machine.progress_pct < 100:
            missing.append(
                f"机台 {machine.machine_code} 装配未完成（进度：{machine.progress_pct}%，需达到100%）"
            )

        if machine.status not in ["ASSEMBLED", "READY", "COMPLETED"]:
            missing.append(
                f"机台 {machine.machine_code} 状态未达到装配完成（当前状态：{machine.status}）"
            )
    ...
    unresolved_review_issues = (
        ...
        .join(TechnicalReview, ReviewIssue.review_id == TechnicalReview.id)
        .filter(

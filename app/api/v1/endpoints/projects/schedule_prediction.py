...
def get_project_alerts(
    ...
    try:
        ...
        total = db.query(ScheduleAlert).filter(ScheduleAlert.project_id == project_id).count()

        unread_count = (
            db.query(ScheduleAlert)
            .filter(ScheduleAlert.project_id == project_id, ScheduleAlert.is_read == False)
            .count()
        )
...
def mark_alert_as_read(
    ...
    alert = (
        db.query(ScheduleAlert)
        .filter(ScheduleAlert.id == alert_id, ScheduleAlert.project_id == project_id)
        .first()
    )
...
def get_catch_up_solutions(
    ...
    query = db.query(CatchUpSolution).filter(CatchUpSolution.project_id == project_id)

    if status:
        query = query.filter(CatchUpSolution.status == status)

    solutions = query.order_by(
        CatchUpSolution.is_recommended.desc(), CatchUpSolution.created_at.desc()
    ).all()
    ...
    return success_response(data=result, message="获取方案列表成功")
...
def approve_solution(
    ...
    solution = (
        db.query(CatchUpSolution)
        .filter(CatchUpSolution.id == solution_id, CatchUpSolution.project_id == project_id)
        .first()
    )
    ...
    return success_response(message=f"方案已{'批准' if request.approved else '拒绝'}")
...
def generate_schedule_report(
    ...
    try:
        ...
        # 获取最近的预测记录
        latest_prediction = (
            db.query(ProjectSchedulePrediction)
            .filter(ProjectSchedulePrediction.project_id == project_id)
            .order_by(desc(ProjectSchedulePrediction.prediction_date))
            .first()
        )
        ...
        report_summary = {
            ...
            "confidence": (
                float(latest_prediction.confidence) if latest_prediction.confidence else 0
            ),
        }
...
def get_risk_overview(
    ...
    try:
        ...
        return success_response(data=overview, message="获取风险概览成功")
...
def get_prediction_history(
    ...
    result = [
        {
            ...
            "predicted_completion_date": (
                str(pred.predicted_completion_date) if pred.predicted_completion_date else None
            ),
    ...
    return success_response(data=result, message="获取历史预测成功")

...
scope_filter = create_scope_filter(
    model=Project, scope_service=DataScopeService, filter_method="filter_projects_by_scope"
)


@router.post(
    "/batch/update-status", response_model=BatchOperationResponse, status_code=status.HTTP_200_OK
)
def batch_update_project_status(
    *,
    ...
    if batch_request.new_status not in valid_statuses:
        raise HTTPException(status_code=400, detail="无效的状态编码。有效值：ST01-ST30")

    executor = BatchOperationExecutor(
        model=Project, db=db, current_user=current_user, scope_filter_func=scope_filter
    )
    ...
    def log_operation(project: Project, op_type: str):
        ...
        status_log = ProjectStatusLog(
            ...
            change_reason=batch_request.reason
            or f"批量状态变更：{old_status} → {batch_request.new_status}",
...
@router.post(
    "/batch/update-stage", response_model=BatchOperationResponse, status_code=status.HTTP_200_OK
)
def batch_update_project_stage(
    *,
    ...
    if batch_request.new_stage not in valid_stages:
        raise HTTPException(
            status_code=400, detail=f"无效的阶段编码。有效值：{', '.join(valid_stages)}"
        )
    ...
    def log_operation(project: Project, op_type: str):
        ...
        status_log = ProjectStatusLog(
            ...
            change_reason=batch_request.reason
            or f"批量阶段变更：{old_stage} → {batch_request.new_stage}",
            changed_by=current_user.id,
            changed_at=datetime.now(),
        )
        db.add(status_log)
...
@router.post(
    "/batch/assign-pm", response_model=BatchOperationResponse, status_code=status.HTTP_200_OK
)

...
def get_template_versions(
    *,
    ...
    total = query.count()
    versions = apply_pagination(
        query.order_by(desc(ProjectTemplateVersion.version_no)), pagination.offset, pagination.limit
    ).all()
...
def create_template_version(
    *,
    ...
    latest_version = (
        ...
        .filter(ProjectTemplateVersion.template_id == template_id)
        .order_by(desc(ProjectTemplateVersion.version_no))
        .first()
    )
    ...
    version = ProjectTemplateVersion(
        ...
        release_notes=getattr(version_in, "description", "")
        or getattr(version_in, "release_notes", ""),
        template_config=getattr(version_in, "config", None)
        or getattr(version_in, "template_config", None),
        status="draft",
...
def publish_template_version(
    *,
    ...
    if version.status == "published":
        return ResponseModel(code=200, message="版本已发布", data={"version_id": version_id})
...
def compare_template_versions(
    *,
    ...
    return ResponseModel(code=200, message="版本比较完成", data=differences)


@router.post(
    "/templates/{template_id}/versions/{version_id}/rollback",
    response_model=ResponseModel,
    status_code=status.HTTP_200_OK,
)

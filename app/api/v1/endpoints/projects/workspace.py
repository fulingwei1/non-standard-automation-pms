...
def get_project_workspace(
    ...
    from app.services.project_workspace_service import (
        build_bonus_info,
        build_document_info,
        build_project_downstream_context,
    ...
    downstream_context = build_project_downstream_context(db, project)
...
@router.get("/projects/{project_id}/downstream-context", response_model=dict)
def get_project_downstream_context(
    project_id: int,
    db: Session = Depends(deps.get_db),
    current_user: User = Depends(security.get_current_active_user),
) -> Any:
    """
    获取项目进入工程/BOM/采购齐套后的后续模块上下文。

    供项目经理在项目工作台内查看工程评审、ECN、BOM、齐套率、
    关键缺料和下一步动作。
    """
    from app.services.project_workspace_service import build_project_downstream_context
    from app.utils.permission_helpers import check_project_access_or_raise

    check_project_access_or_raise(db, current_user, project_id)
    project = get_or_404(db, Project, project_id, "项目不存在")

    return build_project_downstream_context(db, project)

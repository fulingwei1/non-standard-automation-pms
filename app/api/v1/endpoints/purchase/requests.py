...
def create_purchase_request(
    ...
    db.commit()
    db.refresh(request)
    return serialize_purchase_request(request, include_items=True)
...
def submit_purchase_request(
    ...
    db.commit()
    return ResponseModel(code=200, message="采购申请提交成功", data=None)
...
def approve_purchase_request(
    ...
    db.commit()
    return ResponseModel(code=200, message="采购申请审批完成", data=None)
...
def delete_purchase_request(
    ...
    db.delete(request)
    db.commit()
    return ResponseModel(code=200, message="采购申请已删除", data=None)


@router.post("/requests/{request_id}/generate-orders", response_model=ResponseModel[str])
def generate_orders_from_request(
    ...
    try:
        ...
        if success:
            return ResponseModel(code=200, message="从采购申请生成订单成功", data=None)
        else:
            return ResponseModel(code=404, message="采购申请不存在", data=None)
    except Exception as e:
        logger.error(f"从采购申请生成订单失败: {str(e)}")
        return ResponseModel(code=500, message=f"从采购申请生成订单失败: {str(e)}", data=None)

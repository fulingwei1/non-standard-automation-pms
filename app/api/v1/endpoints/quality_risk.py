...
async def analyze_work_logs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"分析失败: {str(e)}"
        )


@router.get(
    "/detections", response_model=List[QualityRiskDetectionResponse], summary="查询质量风险检测记录"
)
...
async def get_detection(
    detection_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """获取质量风险检测详情"""
    service = QualityRiskManagementService(db)
...
async def generate_test_recommendation(
    ...
    recommendation = service.generate_test_recommendation(
        detection_id=detection_id, current_user_id=current_user.id
    )

    if not recommendation:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="质量风险检测记录不存在")
...
async def list_recommendations(
    ...
    recommendations = service.list_recommendations(
        project_id=project_id, priority_level=priority_level, status=status, skip=skip, limit=limit
    )
...
async def update_recommendation(
    ...
    recommendation = service.update_recommendation(
        recommendation_id=recommendation_id, update_data=update_data.model_dump(exclude_unset=True)
    )

    if not recommendation:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="测试推荐不存在")
...
async def generate_quality_report(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"报告生成失败: {str(e)}"
        )
...
async def get_statistics_summary(
    ...
    """获取质量风险统计摘要"""
    service = QualityRiskManagementService(db)
    summary = service.get_statistics_summary(project_id=project_id, days=days)
    return summary

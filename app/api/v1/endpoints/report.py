...
from app.models.report import ArchiveStatusEnum, OutputFormatEnum
...
def get_template(
    *,
    ...
    if not result:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="报表模板不存在")
...
def update_template(
    *,
    ...
    return ResponseModel(code=0, message="报表模板更新成功", data={"id": template.id})
...
def delete_template(
    *,
    ...
    return ResponseModel(code=0, message="报表模板删除成功")
...
def toggle_template(
    *,
    ...
    return ResponseModel(
        code=0, message=f"报表模板已{'启用' if enabled else '禁用'}", data={"enabled": enabled}
    )
    ...
    *,
    ...
    try:
        ...
        data = service.generate_report_data(
            template_id=template_id, period=period, generated_by=f"USER_{current_user.id}"
        )

        # 导出为文件
        template = data["template"]
        if template.output_format == OutputFormatEnum.EXCEL.value:
            file_path = ReportExcelService.export_to_excel(data=data, template_name=template.name)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"报表生成失败: {str(e)}"
        )
...
def preview_report(
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"报表预览失败: {str(e)}"
        )
...
def export_report(
    *,
    ...
    return generate_report(db=db, current_user=current_user, template_id=template_id, period=period)
...
def get_archive(
    *,
    ...
    if not result:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="报表归档不存在")
...
def download_archive(
    *,
    ...
    if archive.status != ArchiveStatusEnum.SUCCESS.value:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="报表生成失败，无法下载"
        )

    file_path = Path(archive.file_path)
    if not file_path.exists():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="报表文件不存在")
...
def batch_download_archives(
    *,
    ...
    return ResponseModel(code=0, message="批量下载准备完成", data={"files": files})
...
@router.post(
    "/templates/{template_id}/recipients",
...
def add_recipient(
    *,
    ...
    return ResponseModel(code=0, message="收件人添加成功", data={"id": recipient.id})
...
def delete_recipient(
    *,
    ...
    return ResponseModel(code=0, message="收件人删除成功")

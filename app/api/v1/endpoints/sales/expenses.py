...
def expense_lost_projects(
    ...
    for expense_data in result["expenses"]:
        ...
        expense = PresaleExpense(
            ...
            or (project.project_code if project else None),
            project_name=expense_data.get("project_name")
            or (project.project_name if project else None),
...
def get_lost_project_expenses(
    ...
    if project_id:
        expenses = db.query(PresaleExpense).filter(PresaleExpense.project_id == project_id).all()
    ...
    paginated_expenses = result["expenses"][

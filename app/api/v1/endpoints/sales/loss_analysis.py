...
def get_loss_by_stage(
    stage: str = Query(
        ..., description="投入阶段：requirement_only/design/detailed_design/quotation"
    ),
    start_date: Optional[date] = Query(None, description="开始日期"),
    end_date: Optional[date] = Query(None, description="结束日期"),
    db: Session = Depends(deps.get_db),
    ...
    """
    service = LossDeepAnalysisService(db)
    result = service.analyze_by_stage(stage=stage, start_date=start_date, end_date=end_date)
...
def get_loss_patterns(
    ...
    """
    service = LossDeepAnalysisService(db)
    result = service.analyze_lost_projects(start_date=start_date, end_date=end_date)

    return ResponseModel(code=200, message="分析成功", data=result.get("pattern_analysis", {}))
...
def get_loss_by_person(
    ...
    return ResponseModel(
        code=200,
        message="分析成功",
        data={"by_person": by_person, "summary": investment_analysis.get("summary", {})},
    )

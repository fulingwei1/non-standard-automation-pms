...
@router.get("/quotes/items", response_model=Response[List[QuoteResponse]])
def get_quote_items(
    ...
    try:
        ...
        return Response.error(message=f"获取报价items失败: {str(e)}")
...
def create_quote_items(
    ...
    try:
        # TODO: 实现items创建逻辑
        return Response.success(message="报价items创建成功")
    except Exception as e:
        return Response.error(message=f"创建报价items失败: {str(e)}")

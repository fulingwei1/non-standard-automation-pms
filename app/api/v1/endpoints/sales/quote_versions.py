...
@router.get("/quotes/versions", response_model=ResponseModel[List[QuoteResponse]])
def get_quote_versions(
    ...
    try:
        ...
        return ResponseModel(code=500, message=f"获取报价versions失败: {str(e)}")
...
def create_quote_versions(
    ...
    try:
        # TODO: 实现versions创建逻辑
        return ResponseModel(code=200, message="报价versions创建成功")
    except Exception as e:
        return ResponseModel(code=500, message=f"创建报价versions失败: {str(e)}")

...
def get_competitor_price_comparison(
    ...
    return {
        ...
        "price_position": (
            "偏高"
            if our_price and our_price > avg_price * 1.1
            else "偏低" if our_price and our_price < avg_price * 0.9 else "适中"
        ),
        "competitors": competitors,
        "analysis": {
            "price_gap_with_lowest": (
                our_price - min(c["price"] for c in competitors) if our_price else None
            ),
            "price_gap_with_highest": (
                max(c["price"] for c in competitors) - our_price if our_price else None
            ),

...
def get_scheduler_metrics_prometheus(
    ...
    try:
        ...
        for job_id, stats in job_snapshot.items():
            ...
            labels = (
                f'job_id="{job_id}",job_name="{job_name}",owner="{owner}",category="{category}"'
            )
        ...
        for job_id, stats in statistics.items():
            ...
            if stats.get("avg_duration_ms") is not None:
                lines.append(
                    f'scheduler_job_duration_avg_ms{{{labels}}} {stats["avg_duration_ms"]:.2f}'
                )
            if stats.get("p50_duration_ms") is not None:
                lines.append(
                    f'scheduler_job_duration_p50_ms{{{labels}}} {stats["p50_duration_ms"]:.2f}'
                )
            if stats.get("p95_duration_ms") is not None:
                lines.append(
                    f'scheduler_job_duration_p95_ms{{{labels}}} {stats["p95_duration_ms"]:.2f}'
                )
            if stats.get("p99_duration_ms") is not None:
                lines.append(
                    f'scheduler_job_duration_p99_ms{{{labels}}} {stats["p99_duration_ms"]:.2f}'
                )
            if stats.get("min_duration_ms") is not None:
                lines.append(
                    f'scheduler_job_duration_min_ms{{{labels}}} {stats["min_duration_ms"]:.2f}'
                )
            if stats.get("max_duration_ms") is not None:
                lines.append(
                    f'scheduler_job_duration_max_ms{{{labels}}} {stats["max_duration_ms"]:.2f}'
                )
        ...
        for channel, stats in notification_snapshot.items():
            ...
            lines.append(
                f'scheduler_notification_success_total{{{labels}}} {stats.get("success_count", 0)}'
            )
            lines.append(
                f'scheduler_notification_failure_total{{{labels}}} {stats.get("failure_count", 0)}'
            )
        ...
        prometheus_text += (
            "# HELP scheduler_job_last_duration_ms Duration of the last job run in milliseconds\n"
        )
        prometheus_text += "# TYPE scheduler_job_last_duration_ms gauge\n"
        prometheus_text += (
            "# HELP scheduler_job_last_run_timestamp Unix timestamp of the last job run\n"
        )
        prometheus_text += "# TYPE scheduler_job_last_run_timestamp gauge\n"
        prometheus_text += (
            "# HELP scheduler_job_duration_avg_ms Average duration of job runs in milliseconds\n"
        )
        prometheus_text += "# TYPE scheduler_job_duration_avg_ms gauge\n"
        prometheus_text += (
            "# HELP scheduler_job_duration_p50_ms P50 duration of job runs in milliseconds\n"
        )
        prometheus_text += "# TYPE scheduler_job_duration_p50_ms gauge\n"
        prometheus_text += (
            "# HELP scheduler_job_duration_p95_ms P95 duration of job runs in milliseconds\n"
        )
        prometheus_text += "# TYPE scheduler_job_duration_p95_ms gauge\n"
        prometheus_text += (
            "# HELP scheduler_job_duration_p99_ms P99 duration of job runs in milliseconds\n"
        )
        ...
        prometheus_text += (
            "# HELP scheduler_notification_failure_total Failed alert notifications per channel\n"
        )
        prometheus_text += "# TYPE scheduler_notification_failure_total counter\n"
        prometheus_text += "\n".join(lines)

        return Response(
            content=prometheus_text, media_type="text/plain; version=0.0.4; charset=utf-8"
        )
    except Exception as e:

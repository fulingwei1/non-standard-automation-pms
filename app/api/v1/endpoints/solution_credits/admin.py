...
async def admin_get_configs(
    db: Session = Depends(deps.get_db), current_user=Depends(deps.get_current_active_superuser)
):
    """
    【管理员】获取积分系统配置
    """
...
async def admin_update_config(
    ...
    success, message = service.update_config(
        key=request.key, value=request.value, description=request.description
    )

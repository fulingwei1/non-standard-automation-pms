...
class StaffMatchingDashboardEndpoint(BaseDashboardEndpoint):
    ...
    def _register_custom_routes(self):
        ...
        self.router.add_api_route(
            "/", dashboard_endpoint, methods=["GET"], summary="获取人员匹配仪表板"
        )
    ...
    def get_dashboard_data(self, db: Session, current_user: User) -> Dict[str, Any]:
        ...
        # 需求统计
        open_needs = (
            db.query(func.count(MesProjectStaffingNeed.id))
        ...
        # 按优先级统计
        priority_counts = (
            db.query(MesProjectStaffingNeed.priority, func.count(MesProjectStaffingNeed.id))
        ...
        # 匹配统计
        total_requests = (
            db.query(func.count(func.distinct(HrAIMatchingLog.request_id))).scalar() or 0
        )
        total_matched = db.query(func.count(HrAIMatchingLog.id)).scalar() or 0
        accepted = (
            db.query(func.count(HrAIMatchingLog.id)).filter(HrAIMatchingLog.is_accepted).scalar()
            or 0
        )
        rejected = (
            db.query(func.count(HrAIMatchingLog.id))
        ...
        avg_score = (
            db.query(func.avg(HrAIMatchingLog.total_score))
        ...
        # 最近匹配
        recent_logs = (
            db.query(HrAIMatchingLog).order_by(HrAIMatchingLog.matching_time.desc()).limit(10).all()
        )
            ...
            recent_matches.append(
                self.create_list_item(
                    ...
                    status=(
                        "accepted"
                        if log.is_accepted
                        else ("rejected" if log.is_accepted is False else "pending")
                    ),
                    extra={
        ...
        total_headcount_needed = (
            db.query(func.sum(MesProjectStaffingNeed.headcount))

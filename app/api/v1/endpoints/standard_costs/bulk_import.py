...
async def import_standard_costs(
    *,
    ...
    if file_ext not in [".xlsx", ".xls", ".csv"]:
        raise HTTPException(
            status_code=400, detail="不支持的文件格式，请上传Excel(.xlsx/.xls)或CSV文件"
        )
    ...
    try:
        ...
        if missing_fields:
            raise HTTPException(
                status_code=400, detail=f"缺少必填字段：{', '.join(missing_fields)}"
            )
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if not cost_code or cost_code == "nan":
                    errors.append(
                        {"row": row_num, "field": "cost_code", "message": "成本项编码不能为空"}
                    )
                    error_count += 1
                    continue
                ...
                cost = StandardCost(
                    ...
                    specification=(
                        str(row.get("specification", "")).strip()
                        if pd.notna(row.get("specification"))
                        else None
                    ),
                    ...
                    source_description=(
                        str(row.get("source_description", "")).strip()
                        if pd.notna(row.get("source_description"))
                        else None
                    ),
        ...
        return StandardCostImportResult(
            success_count=success_count, error_count=error_count, errors=errors, warnings=warnings
        )

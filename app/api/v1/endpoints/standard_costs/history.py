...
def get_standard_cost_history(
    ...
    history_records = apply_pagination(
        query.order_by(desc(StandardCostHistory.change_date)), pagination.offset, pagination.limit
    ).all()
...
def get_cost_history_by_id(
    *,
    ...
    history_records = (
        db.query(StandardCostHistory)
        .filter(StandardCostHistory.standard_cost_id == cost_id)
        .order_by(desc(StandardCostHistory.change_date))
        .all()
    )

...
def _generate_budget_no(db: Session) -> str:
    ...
    last_budget = (
        ...
        .filter(ProjectBudget.budget_no.like(f"{prefix}%"))
        .order_by(ProjectBudget.budget_no.desc())
        .first()
    )
...
@router.post(
    "/projects/{project_id}/costs/apply-standard", response_model=ApplyStandardCostResponse
)
    ...
    *,
    ...
    for idx, item in enumerate(request.cost_items, start=1):
        ...
        standard_cost = (
            ...
            .filter(StandardCost.cost_code == cost_code, StandardCost.is_active == True)
            .first()
        )

        if not standard_cost:
            raise HTTPException(status_code=404, detail=f"标准成本 {cost_code} 不存在或已停用")
    ...
    for item_data in budget_items_data:
        item = ProjectBudgetItem(budget_id=budget.id, **item_data)
        db.add(item)
...
@router.get(
    "/projects/{project_id}/costs/compare-standard", response_model=ProjectCostComparisonResponse
)
def compare_project_cost_with_standard(
    *,
    ...
    budget = (
        db.query(ProjectBudget)
        .filter(ProjectBudget.project_id == project_id, ProjectBudget.status == "APPROVED")
    ...
    for item in budget.items:
        ...
        if cost_code:
            ...
            if standard_cost:
                ...
                variance_rate = (
                    (variance / standard_total * 100) if variance and standard_total else None
                )
    ...
    total_variance_rate = (
        (total_variance / total_standard * 100) if total_standard else Decimal("0")
    )

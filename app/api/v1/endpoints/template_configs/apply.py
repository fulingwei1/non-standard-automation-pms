...
@router.post(
    "/{config_id}/create-project", response_model=ResponseModel, status_code=status.HTTP_201_CREATED
)
def create_project_from_config(
    *,
    ...
    for stage_cfg in sorted(stages_config, key=lambda s: s.get("sequence", 0)):
        ...
        estimated_days = stage_cfg.get("custom_estimated_days") or stage_cfg.get(
            "estimated_days", 7
        )
        ...
        for node_cfg in sorted(nodes, key=lambda n: n.get("sequence", 0)):
            ...
            task = WbsTemplateTask(
                ...
                owner_role_code=node_cfg.get("custom_owner_role_code")
                or node_def.get("owner_role_code", "PM"),

...
@router.get(
    "/department-comparison", response_model=DepartmentComparisonResponse, summary="部门工时对比"
)
@require_permission("timesheet:read")
...
@router.get(
    "/project-distribution", response_model=ProjectDistributionResponse, summary="项目工时分布"
)
@require_permission("timesheet:read")
...
@router.get(
    "/forecast/completion", response_model=CompletionForecastResponse, summary="预测完工时间"
)
@require_permission("timesheet:read")
def forecast_completion_time(
    ...
    """
    service = TimesheetForecastService(db)
    return service.forecast_completion(project_id=project_id, forecast_method=forecast_method)


@router.get(
    "/forecast/workload-alert", response_model=List[WorkloadAlertResponse], summary="人员负荷预警"
)
@require_permission("timesheet:read")

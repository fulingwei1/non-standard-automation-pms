...
async def complete_2fa_login(
    request_data: TwoFactorLoginRequest, req: Request, db: Session = Depends(get_db)
):
    """
    ...
    try:
        payload = jwt.decode(
            request_data.temp_token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM]
        )
    ...
    # 2. 查询用户
    result = db.execute(
        text("SELECT * FROM users WHERE id = :user_id LIMIT 1"), {"user_id": user_id}
    ).fetchone()
    ...
    if not user_dict.get("two_factor_enabled"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="用户未启用2FA")
    ...
    success, message = service.verify_2fa_code(db, user, request_data.code, client_ip)
...
async def setup_two_factor(
    ...
    if current_user.two_factor_enabled:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="2FA已启用，请先禁用后再重新设置"
        )
...
async def enable_two_factor(
    ...
    """
    if current_user.two_factor_enabled:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="2FA已启用")
    ...
    return TwoFactorEnableResponse(success=True, message=message, backup_codes=backup_codes)
...
async def verify_two_factor(
    ...
    if not current_user.two_factor_enabled:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="未启用2FA")
    ...
    # 验证2FA码
    service = get_two_factor_service()
    success, message = service.verify_2fa_code(db, current_user, request.code, client_ip)

    if not success:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=message)

    return TwoFactorVerifyResponse(success=True, message=message)
...
async def disable_two_factor(
    ...
    # 禁用2FA
    service = get_two_factor_service()
    success, message = service.disable_2fa_for_user(db, current_user, request.password)

    if not success:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=message)

    return TwoFactorDisableResponse(success=True, message=message)
...
async def regenerate_backup_codes(
    ...
    return BackupCodesRegenerateResponse(success=True, message=message, backup_codes=backup_codes)

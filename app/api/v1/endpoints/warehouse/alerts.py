...
def get_alert_summary(db: Session = Depends(get_db)):
    ...
    over = (
        db.query(func.count(Inventory.id))
        .filter(Inventory.quantity > Inventory.max_stock, Inventory.max_stock > 0)
        .scalar()
        or 0
    )
...
def list_alerts(
    ...
    if alert_type != "OVERSTOCK":
        q = db.query(Inventory).filter(
            Inventory.available_quantity <= Inventory.min_stock, Inventory.min_stock > 0
        )
        if keyword:
            q = q.filter(
    ...
    if alert_type != "LOW":
        q = db.query(Inventory).filter(
            Inventory.quantity > Inventory.max_stock, Inventory.max_stock > 0
        )
        if keyword:
            q = q.filter(
                (Inventory.material_code.contains(keyword))

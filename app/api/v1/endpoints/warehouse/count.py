...
def _generate_count_no(db: Session) -> str:
    ...
    pattern = f"SC{today}%"
    last = db.query(StockCountOrder).filter(StockCountOrder.count_no.like(pattern)).order_by(desc(StockCountOrder.count_no)).first()
    seq = int(last.count_no[-4:]) + 1 if last else 1
    return f"SC{today}{seq:04d}"


@router.get("/stock-count", response_model=dict)
def list_count_orders(
    ...
    return {
        "items": [CountOrderOut.model_validate(i).model_dump() for i in items],
        "total": total,
        "page": page,
        "page_size": page_size,
    }
...
def get_count_order(order_id: int, db: Session = Depends(get_db)):
    order = db.query(StockCountOrder).filter(StockCountOrder.id == order_id).first()
    if not order:
...
def create_count_order(data: CountOrderCreate, db: Session = Depends(get_db)):
    ...
    for item_data in data.items:
        item = StockCountItem(order_id=order.id, **item_data.model_dump())
        db.add(item)
...
def update_count_item(
    ...
    item = db.query(StockCountItem).filter(
        ...
        StockCountItem.order_id == order_id,
    ).first()
    if not item:
        ...
        for item in order.items:
            if item.actual_quantity is not None:
                if item.diff_quantity == 0:
                    ...
                    diff += 1
        order.matched_items = matched
        order.diff_items = diff
    db.commit()

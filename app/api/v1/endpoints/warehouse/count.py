...
def _gen_count_no(db):
    ...
    last = (
        db.query(StockCountOrder)
        .filter(StockCountOrder.count_no.like(f"SC{today}%"))
        .order_by(desc(StockCountOrder.count_no))
        .first()
    )
...
def list_count_orders(
    ...
    total = q.count()
    items = (
        q.order_by(desc(StockCountOrder.created_at))
...
def create_count_order(
    ...
    db.flush()
    for d in data.items:
        db.add(StockCountItem(order_id=o.id, **d.model_dump()))
    db.commit()
    db.refresh(o)
    return o
...
def update_count_item(
    ...
    item = (
        ...
        .filter(StockCountItem.id == item_id, StockCountItem.order_id == order_id)
        .first()
    )
    if not item:

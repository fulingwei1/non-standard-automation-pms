...
def list_locations(
    ...
    if is_active is not None:
        q = q.filter(WarehouseLocation.is_active == is_active)
    if keyword:
        q = q.filter(
            (WarehouseLocation.location_code.contains(keyword)) |
            (WarehouseLocation.location_name.contains(keyword))
        )
    total = q.count()
    rows = q.order_by(WarehouseLocation.location_code).offset((page - 1) * page_size).limit(page_size).all()

    # Attach warehouse_name
    wh_ids = {r.warehouse_id for r in rows}
    wh_map = {}
    if wh_ids:
        whs = db.query(Warehouse).filter(Warehouse.id.in_(wh_ids)).all()
        wh_map = {w.id: w.warehouse_name for w in whs}

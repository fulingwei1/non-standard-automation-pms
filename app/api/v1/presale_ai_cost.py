...
async def estimate_cost(
    ...
    **返回**:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"估算失败: {str(e)}"
        )


@router.get(
    "/cost-estimation/{estimation_id}",
...
async def get_cost_estimation(
    ...
    estimation = (
        ...
        .filter(PresaleAICostEstimation.id == estimation_id)
        .first()
    )
    ...
    return CostEstimationResponse(
        ...
        pricing_recommendations=(
            PricingRecommendation(**estimation.pricing_recommendations)
            if estimation.pricing_recommendations
            else None
        ),
...
async def optimize_cost(
    ...
    **返回**:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"优化失败: {str(e)}"
        )
...
async def pricing_recommendation(
    ...
    **返回**:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"定价推荐失败: {str(e)}"
        )
...
async def get_cost_breakdown(
    ...
    estimation = (
        db.query(PresaleAICostEstimation)
        .filter(PresaleAICostEstimation.presale_ticket_id == ticket_id)
        .order_by(PresaleAICostEstimation.created_at.desc())
        .first()
    )

    if not estimation:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="未找到该工单的成本估算记录"
        )
...
async def cost_comparison(
    ...
    **返回**:
    ...
    if len(input_data.estimation_ids) < 2 or len(input_data.estimation_ids) > 5:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="对比方案数量应在2-5个之间"
        )

    estimations = (
        db.query(PresaleAICostEstimation)
        .filter(PresaleAICostEstimation.id.in_(input_data.estimation_ids))
        .all()
    )
    ...
    return CostComparisonResponse(
        items=items, comparison_summary=comparison_summary, recommendations=recommendations
    )

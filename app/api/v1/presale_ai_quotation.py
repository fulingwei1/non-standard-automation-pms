...
async def generate_quotation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"生成报价单失败: {str(e)}"
        )


@router.post(
    "/generate-three-tier-quotations",
    response_model=ThreeTierQuotationResponse,
    summary="生成三档报价方案",
)
async def generate_three_tier_quotations(
    ...
    try:
        ...
        if request.budget_range:
            avg_budget = (
                request.budget_range.get("min", 0) + request.budget_range.get("max", 999999)
            ) / 2
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"生成三档方案失败: {str(e)}"
        )


@router.get("/quotation/{quotation_id}", response_model=QuotationResponse, summary="获取报价单")
async def get_quotation(
    quotation_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """
    获取指定报价单详情
    """
    ...
    if not quotation:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"报价单 {quotation_id} 不存在"
        )
...
async def export_quotation_pdf(
    ...
    try:
        ...
        return {"quotation_id": quotation_id, "pdf_url": pdf_path, "message": "PDF生成成功"}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"导出PDF失败: {str(e)}"
        )
...
async def send_quotation_email(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"发送邮件失败: {str(e)}"
        )


@router.get(
    "/quotation-history/{ticket_id}",
    response_model=QuotationHistoryResponse,
    summary="获取报价单版本历史",
)
async def get_quotation_history(
    ...
    try:
        ...
        if not quotations:
            return {"quotation_id": None, "current_version": 0, "versions": [], "total_versions": 0}
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取历史失败: {str(e)}"
        )
...
async def approve_quotation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"审批失败: {str(e)}"
        )
...
async def _send_email_task(
    to_email: str, cc_emails: List[str], subject: str, message: str, pdf_path: str = None
):
    """
    异步发送邮件任务

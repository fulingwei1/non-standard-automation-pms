...
async def predict_win_rate(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"预测赢率失败: {str(e)}"
        )
...
async def get_win_rate_prediction(
    ...
    try:
        ...
        if not prediction:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"未找到预测记录: {prediction_id}"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取预测结果失败: {str(e)}"
        )
...
async def get_influencing_factors(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取影响因素失败: {str(e)}"
        )
...
async def get_competitor_analysis(
    ...
    try:
        ...
        if not analysis:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"未找到工单 {ticket_id} 的竞品分析"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取竞品分析失败: {str(e)}"
        )
...
async def get_improvement_suggestions(
    ...
    try:
        ...
        if not suggestions:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"未找到工单 {ticket_id} 的改进建议"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取改进建议失败: {str(e)}"
        )
...
async def update_actual_result(
    ...
    try:
        ...
        return {
            ...
            "prediction_error": (
                float(history.prediction_error) if history.prediction_error else None
            ),
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"更新实际结果失败: {str(e)}"
        )

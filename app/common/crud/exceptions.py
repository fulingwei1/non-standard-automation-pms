...
def raise_not_found(resource_name: str, resource_id: int):
    """抛出404异常"""
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND, detail=f"{resource_name} (ID: {resource_id}) 不存在"
    )
...
def raise_already_exists(resource_name: str, field: str, value: str):
    """抛出409冲突异常"""
    raise HTTPException(
        status_code=status.HTTP_409_CONFLICT, detail=f"{resource_name} 的 {field}={value} 已存在"
    )

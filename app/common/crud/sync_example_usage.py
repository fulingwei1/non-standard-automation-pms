...
# ========== 示例0: 推荐的BaseCRUDService + QueryParams ==========


class MilestoneCrudService(
    BaseCRUDService[
        ProjectMilestone, MilestoneCreate, MilestoneUpdate, MilestoneResponse
    ]
):
    """推荐的新版Service，用最少代码换取统一能力"""
...
class MilestoneService(
    ...
    def __init__(self, db: Session):
        super().__init__(model=ProjectMilestone, db=db, resource_name="里程碑")
...
class MilestoneServiceAdvanced(
    ...
    def create_for_project(
        self, project_id: int, milestone_in: MilestoneCreate
    ) -> MilestoneResponse:
        """为项目创建里程碑（自定义方法）"""
        # 确保使用项目ID
        milestone_data = milestone_in.model_dump()

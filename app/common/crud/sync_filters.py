...
class SyncQueryBuilder:
    ...
    def build_list_query(
        ...
        *,
        ...
        # 应用筛选条件
        if filters:
            filter_conditions = SyncQueryBuilder._build_filter_conditions(model, filters)
    ...
    @staticmethod
    def _build_filter_conditions(model: Type[ModelType], filters: Dict[str, Any]) -> List:
        """构建筛选条件"""
        conditions = []
    ...
    @staticmethod
    def _build_keyword_conditions(model: Type[ModelType], keyword: str, fields: List[str]) -> List:
        """构建关键词搜索条件"""
        return build_keyword_conditions(model, keyword, fields)

    @staticmethod
    def execute_list_query(query: Query, count_query: Query) -> tuple[List[ModelType], int]:
        """
        执行列表查询（同步版本）

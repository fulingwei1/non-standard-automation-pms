...
class SyncBaseService(Generic[ModelType, CreateSchemaType, UpdateSchemaType, ResponseSchemaType]):
    ...
    def __init__(self, model: Type[ModelType], db: Session, resource_name: str = None):
        """
        Args:
            model: SQLAlchemy模型类
            db: 数据库会话（同步Session）
    ...
    def get(self, id: int, *, load_relationships: Optional[List[str]] = None) -> ResponseSchemaType:
        """
        获取单个对象
    ...
    def get_by_field(
        self, field_name: str, value: Any, *, load_relationships: Optional[List[str]] = None
        ...
        Returns:
            响应对象或None
        """
        obj = self.repository.get_by_field(field_name, value, load_relationships=load_relationships)
        if not obj:
            return None
    ...
    def update(
        self, id: int, obj_in: UpdateSchemaType, *, check_unique: Optional[Dict[str, Any]] = None
    ) -> ResponseSchemaType:
        """
        更新对象
    ...
    def delete(self, id: int, *, soft_delete: bool = False) -> bool:
        """
        删除对象

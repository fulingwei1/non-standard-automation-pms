...
class BaseStatisticsService:
    ...
    async def count_by_status(
        self, status_field: str = "status", filters: Optional[Dict[str, Any]] = None
    ) -> Dict[str, int]:
        """
        按状态统计
        ...
        Returns:
            {状态值: 数量}
        """
        query = select(getattr(self.model, status_field), func.count(self.model.id))
    ...
    async def count_by_field(
        ...
        query = select(model_field, func.count(self.model.id))
    ...
    async def count_total(self, filters: Optional[Dict[str, Any]] = None) -> int:
        ...
        return await QueryBuilder.execute_list_query(
            *QueryBuilder.build_list_query(model=self.model, db=self.db, filters=filters), self.db
        )[1]
    ...
    async def count_by_date_range(
        ...
        query = select(date_format.label("date"), func.count(self.model.id).label("count")).where(
            and_(model_field >= start_date, model_field <= end_date)
        )
    ...
    async def get_trend(
        ...
        if value_field:
            ...
            query = select(
                date_format.label("date"), func.sum(model_value_field).label("value")
            ).where(
                and_(
            ...
            return [{"date": str(row[0]), "value": float(row[1] or 0)} for row in result.all()]
        else:
            # 统计数量
            return await self.count_by_date_range(date_field, start_date, end_date, period, filters)
    ...
    async def get_distribution(
        ...
        return {
            ...
            "total": total,
            "percentages": {
                k: round(v / total * 100, 2) if total > 0 else 0 for k, v in counts.items()
            },
        }

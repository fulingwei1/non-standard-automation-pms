...
def _memory_blacklist_jti_key(jti: str) -> str:
    """内存黑名单中用于表示JTI撤销状态的键格式。"""
    return f"jti:{jti}"
...
def revoke_token_jti(jti: Optional[str], ttl_seconds: Optional[int] = None) -> bool:
    ...
    ttl = max(int(ttl_seconds or settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60), 60)
    ...
    if redis_client:
        try:
            ...
            logger.debug(f"JTI已加入Redis黑名单: {jti[:8]}...")
            return True
        except Exception as e:
            logger.warning(f"写入Redis黑名单失败，降级到内存: {e}")
...
def revoke_token(token: Optional[str]) -> None:
    ...
    try:
        ...
        # 兼容误传JTI的调用（例如直接传 access_token_jti）
        if isinstance(token, str) and "." not in token:
            revoke_token_jti(token)
        with _token_blacklist_lock:
            _token_blacklist.add(token)
...
def is_token_revoked(token: Optional[str]) -> bool:
    ...
    try:
        ...
        if redis_client:
            try:
                if redis_client.exists(f"jwt:blacklist:{jti}"):
                    return True
            except Exception as e:
        ...
        with _token_blacklist_lock:
            return (
                ...
                or _memory_blacklist_jti_key(jti) in _token_blacklist
            )
    except JWTError:
        # token可能本身就是jti（例如内部调用误传jti）
        redis_client = get_redis_client()
        if redis_client:
            try:
                if redis_client.exists(f"jwt:blacklist:{token}"):
                    return True
            except Exception:
                pass
        ...
        with _token_blacklist_lock:
            ...
            return (
                ...
                or _memory_blacklist_jti_key(token_str) in _token_blacklist
            )


def _ensure_access_session_active(db: Session, token_jti: Optional[str]) -> None:
    """
    校验access token对应会话是否仍处于活跃状态。

    说明：
    - 找到会话且已失效 -> 拒绝访问（登出/强制下线后立即生效）
    - 找不到会话 -> 兼容历史token，允许继续（避免会话写入异常导致全量登录失效）
    """
    if not token_jti:
        return
    ...
    try:
        ...
        session = SessionService.get_session_by_jti(db, token_jti, token_type="access")
        ...
        logger.warning(f"会话状态校验失败，降级放行: {e}")
...
async def verify_token_and_get_user(token: str, db: Session) -> User:
    ...
    try:
        ...
        _ensure_access_session_active(db, token_jti)
    except JWTError as e:
...
def require_permission(permission_code: str):
    ...
    def _enforce_permission(current_user: "User", db: Optional[Session] = None) -> None:
        """统一权限校验，失败时抛出 403。"""
    ...
    # Return a dependency function that performs real permission checks.
    def permission_dependency(
        current_user: "User" = _Depends(get_current_active_user),
        db: Session = _Depends(get_db),
    ) -> "User":
        _enforce_permission(current_user, db)
        return current_user

    # Support decorator use: @require_permission("x")
    def decorator(func):
        ...
        def _resolve_context(args, kwargs):
            ...
            # Best-effort fallback: infer user-like object from bound args.
            ...
            if current_user is None:
                logger.error(
                    "Permission decorator misconfigured: function %s missing current_user parameter",
                    func.__name__,
                )
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="权限校验配置错误：缺少 current_user 参数",
                )
        ...
        if inspect.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                current_user, db = _resolve_context(args, kwargs)
                _enforce_permission(current_user, db)

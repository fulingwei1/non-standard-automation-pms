...
class CSRFMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if "CSRF" in str(e) or "csrf" in str(e):
                ...
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="CSRF验证失败"
                )
            raise
    ...
    def _validate_api_request(self, request: Request) -> None:
        ...
        if not auth_header.startswith("Bearer "):
            ...
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="需要JWT认证")
        ...
        if not origin and not referer:
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="CSRF验证失败：缺少Origin或Referer头"
            )
        ...
        if origin:
            ...
            if referer_origin and not self._is_origin_allowed(referer_origin):
                ...
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail=f"CSRF验证失败：不允许的来源"
                )

        logger.debug(
            f"CSRF验证通过: {request.method} {request.url.path} " f"from {origin or referer_origin}"
        )
...
__all__ = ["CSRFMiddleware", "generate_csrf_token", "verify_csrf_token"]

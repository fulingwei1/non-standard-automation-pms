...
def validate_tenant_match(user: Any, *tenant_ids: Optional[int]) -> bool:
    """
    验证多个资源是否都属于用户可访问的租户
    ...
    if non_null_tenants and len(set(non_null_tenants)) > 1:
        logger.warning(f"Multiple different tenant IDs in one operation: {non_null_tenants}")
        return False
...
def ensure_tenant_consistency(
    user: Any, resource_data: dict, tenant_field: str = "tenant_id"
) -> dict:
    """
    确保创建的资源使用正确的租户ID
    ...
    if is_superuser and user_tenant_id is None:
        ...
        logger.debug(f"Superuser creating resource with tenant_id={requested_tenant_id}")
...
def check_bulk_access(user: Any, resources: List[Any], tenant_field: str = "tenant_id") -> bool:
    """

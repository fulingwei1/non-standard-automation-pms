...
def create_limiter(
    ...
    if storage_uri:
        try:
            ...
            logger.warning(f"无法连接到Redis存储 {storage_uri}，降级到内存存储: {e}")
    ...
    logger.warning(
        "速率限制器使用内存存储，在分布式部署中可能不准确。" "建议配置Redis存储以获得准确的限流。"
    )
    return limiter
...
def get_rate_limit_status(request: Request) -> dict:
    ...
    try:
        ...
        return {
            "limit": (
                request.state.view_rate_limit if hasattr(request.state, "view_rate_limit") else None
            ),
            "remaining": (
                request.state.view_rate_limit_remaining
                if hasattr(request.state, "view_rate_limit_remaining")
                else None
            ),
            "reset": (
                request.state.view_rate_limit_reset
                if hasattr(request.state, "view_rate_limit_reset")
                else None
            ),
        }
    except Exception as e:

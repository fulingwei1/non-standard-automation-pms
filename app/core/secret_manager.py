...
class SecretKeyManager:
    ...
    def verify_token_with_fallback(
        self, token: str, algorithms: List[str] = None, options: Dict[str, Any] = None
        ...
        # 1. 先用当前密钥验证
        try:
            payload = jwt.decode(token, self.current_key, algorithms=algorithms, options=options)
        ...
        for idx, old_key in enumerate(self.old_keys):
            try:
                payload = jwt.decode(token, old_key, algorithms=algorithms, options=options)
                logger.warning(
                    f"使用旧密钥#{idx+1}验证Token成功。" "建议用户重新登录以获取新Token。"
                )
                # 在payload中标记使用了旧密钥

    ...
    def _ensure_can_complete(self):
        ...
        try:
            ...
            can_complete, missing_items = (
                integration_service.check_milestone_completion_requirements(self.model)
            )
    ...
    def _auto_trigger_invoice(self):
        ...
        try:
            ...
            for plan in payment_plans:
                ...
                if plan.contract_id:
                    contract = (
                        self.db.query(Contract).filter(Contract.id == plan.contract_id).first()
                    )
                ...
                logging.info(f"里程碑 {self.model.milestone_code} 自动触发开票: {invoice_code}")

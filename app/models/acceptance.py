...
class TemplateCategory(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_category_template", "template_id"),)
...
class TemplateCheckItem(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_check_item_category", "category_id"),)
...
class AcceptanceOrder(Base, TimestampMixin):
    ...
    # 正式完成标记
    is_officially_completed = Column(
        Boolean, default=False, comment="是否正式完成（已上传客户签署文件）"
    )
...
class AcceptanceOrderItem(Base, TimestampMixin):
    ...
    category_id = Column(Integer, ForeignKey("template_categories.id"), comment="模板分类ID")
    template_item_id = Column(
        Integer, ForeignKey("template_check_items.id"), comment="模板检查项ID"
    )
...
class IssueFollowUp(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_follow_up_issue", "issue_id"),)
...
class AcceptanceSignature(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_signature_order", "order_id"),)
...
class AcceptanceReport(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_report_order", "order_id"),)

...
class APIKey(Base):
    ...
    key_hash = Column(
        String(64), unique=True, nullable=False, index=True, comment="API Key哈希值（SHA256）"
    )
    ...
    tenant_id = Column(
        Integer, ForeignKey("tenants.id", ondelete="CASCADE"), nullable=True, comment="所属租户"
    )

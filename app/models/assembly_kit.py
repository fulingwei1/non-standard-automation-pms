...
class MaterialReadiness(Base, TimestampMixin):
    ...
    confirmer = relationship("User", foreign_keys=[confirmed_by])
    shortage_details = relationship(
        "ShortageDetail", back_populates="readiness", cascade="all, delete-orphan", lazy="dynamic"
    )
...
class KitRateSnapshot(Base):
    ...
    project_id = Column(Integer, ForeignKey("projects.id"), nullable=False, comment="项目ID")
    machine_id = Column(
        Integer, ForeignKey("machines.id"), comment="机台ID（可选，不填则为项目级快照）"
    )
    ...
    snapshot_type = Column(
        String(20), nullable=False, default="DAILY", comment="快照类型：DAILY/STAGE_CHANGE/MANUAL"
    )

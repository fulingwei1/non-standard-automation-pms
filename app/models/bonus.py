...
class BonusCalculation(Base, TimestampMixin):
    ...
    # 关联对象
    period_id = Column(
        Integer, ForeignKey("performance_period.id"), comment="考核周期ID（绩效奖金）"
    )
    project_id = Column(Integer, ForeignKey("projects.id"), comment="项目ID（项目奖金）")
    milestone_id = Column(
        Integer, ForeignKey("project_milestones.id"), comment="里程碑ID（里程碑奖金）"
    )
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, comment="受益人ID")

    # 计算依据
    performance_result_id = Column(
        Integer, ForeignKey("performance_result.id"), comment="绩效结果ID"
    )
    project_contribution_id = Column(
        Integer, ForeignKey("project_contribution.id"), comment="项目贡献ID"
    )

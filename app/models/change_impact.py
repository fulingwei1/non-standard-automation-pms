...
class ChangeImpactAnalysis(Base, TimestampMixin):
    ...
    analysis_status = Column(
        String(20), default="PENDING", comment="状态: PENDING/ANALYZING/COMPLETED/FAILED"
    )
    ...
    # === 进度影响分析 ===
    schedule_impact_level = Column(
        String(20), comment="进度影响级别: NONE/LOW/MEDIUM/HIGH/CRITICAL"
    )
    ...
    # === 资源影响分析 ===
    resource_impact_level = Column(
        String(20), comment="资源影响级别: NONE/LOW/MEDIUM/HIGH/CRITICAL"
    )
...
class ChangeResponseSuggestion(Base, TimestampMixin):
    ...
    suggestion_type = Column(
        String(50), comment="方案类型: APPROVE/REJECT/MODIFY/MITIGATE/POSTPONE"
    )
    ...
    status = Column(
        String(20),
        default="PROPOSED",
        comment="状态: PROPOSED/REVIEWED/SELECTED/REJECTED/IMPLEMENTED",
    )
    ...
    implementation_status = Column(
        String(20), comment="实施状态: NOT_STARTED/IN_PROGRESS/COMPLETED/CANCELLED"
    )
    ...
    change_request = relationship(
        "ChangeRequest", back_populates="response_suggestions", lazy="joined"
    )
    impact_analysis = relationship(
        "ChangeImpactAnalysis", back_populates="response_suggestions", lazy="joined"
    )
    creator = relationship("User", foreign_keys=[created_by], lazy="joined")

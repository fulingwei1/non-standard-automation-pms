...
class ChangeRequest(Base, TimestampMixin):
    ...
    change_type = Column(Enum(ChangeTypeEnum), nullable=False, comment="变更类型")
    change_source = Column(Enum(ChangeSourceEnum), nullable=False, comment="变更来源")
    ...
    cost_impact_level = Column(Enum(ImpactLevelEnum), comment="成本影响程度")
    time_impact = Column(Integer, comment="时间影响（天）")
    time_impact_level = Column(Enum(ImpactLevelEnum), comment="时间影响程度")
    scope_impact = Column(Text, comment="范围影响描述")
    scope_impact_level = Column(Enum(ImpactLevelEnum), comment="范围影响程度")
    ...
    approval_decision = Column(
        Enum(ApprovalDecisionEnum), default=ApprovalDecisionEnum.PENDING, comment="审批决策"
    )
    ...
    approval_records = relationship(
        "ChangeApprovalRecord", back_populates="change_request", cascade="all, delete-orphan"
    )
    notifications = relationship(
        "ChangeNotification", back_populates="change_request", cascade="all, delete-orphan"
    )
    impact_analyses = relationship(
        "ChangeImpactAnalysis", back_populates="change_request", cascade="all, delete-orphan"
    )
    response_suggestions = relationship(
        "ChangeResponseSuggestion", back_populates="change_request", cascade="all, delete-orphan"
    )
...
class ChangeApprovalRecord(Base, TimestampMixin):
    ...
    decision = Column(Enum(ApprovalDecisionEnum), nullable=False, comment="审批决策")

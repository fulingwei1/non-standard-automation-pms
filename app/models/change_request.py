...
class ChangeRequest(Base, TimestampMixin):
    ...
    change_type = Column(
        Enum(ChangeTypeEnum, values_callable=lambda enum_cls: [item.value for item in enum_cls]),
        nullable=False,
        comment='变更类型'
    )
    change_source = Column(
        Enum(ChangeSourceEnum, values_callable=lambda enum_cls: [item.value for item in enum_cls]),
        nullable=False,
        comment='变更来源'
    )
    ...
    cost_impact_level = Column(
        Enum(ImpactLevelEnum, values_callable=lambda enum_cls: [item.value for item in enum_cls]),
        comment='成本影响程度'
    )
    ...
    # 工作流状态
    status = Column(
        Enum(ChangeStatusEnum, values_callable=lambda enum_cls: [item.value for item in enum_cls]),

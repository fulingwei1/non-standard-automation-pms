...
class EarnedValueData(Base, TimestampMixin):
    ...
    period_date = Column(Date, nullable=False, comment="周期截止日期（周末日期/月末日期/季末日期）")
    period_label = Column(String(50), comment="周期标签（如：2026-W07, 2026-02, 2026-Q1）")
    ...
    budget_at_completion = Column(
        Numeric(18, 4), nullable=False, comment="BAC - 完工预算（Budget at Completion）"
    )

    # 货币
    currency = Column(String(10), default="CNY", nullable=False, comment="币种（CNY/USD/EUR等）")

    # 计算结果缓存（提升查询性能，避免重复计算）
    # 偏差指标
    schedule_variance = Column(
        Numeric(18, 4), comment="SV - 进度偏差（Schedule Variance = EV - PV）"
    )
    cost_variance = Column(Numeric(18, 4), comment="CV - 成本偏差（Cost Variance = EV - AC）")

    # 绩效指数
    schedule_performance_index = Column(
        Numeric(10, 6), comment="SPI - 进度绩效指数（Schedule Performance Index = EV / PV）"
    )
    cost_performance_index = Column(
        Numeric(10, 6), comment="CPI - 成本绩效指数（Cost Performance Index = EV / AC）"
    )

    # 预测指标
    estimate_at_completion = Column(
        Numeric(18, 4), comment="EAC - 完工估算（Estimate at Completion）"
    )
    estimate_to_complete = Column(
        Numeric(18, 4), comment="ETC - 完工尚需估算（Estimate to Complete = EAC - AC）"
    )
    variance_at_completion = Column(
        Numeric(18, 4), comment="VAC - 完工偏差（Variance at Completion = BAC - EAC）"
    )
    to_complete_performance_index = Column(
        Numeric(10, 6), comment="TCPI - 完工尚需绩效指数（To-Complete Performance Index）"
    )

    # 完成百分比
    planned_percent_complete = Column(Numeric(5, 2), comment="计划完成百分比（PV / BAC * 100）")
    actual_percent_complete = Column(Numeric(5, 2), comment="实际完成百分比（EV / BAC * 100）")
    ...
    is_baseline = Column(Boolean, default=False, comment="是否基准数据（项目启动时的基准）")
    is_forecast = Column(Boolean, default=False, comment="是否预测数据（未来的预测值）")
    is_verified = Column(Boolean, default=False, comment="是否已核实（PMO或财务核实）")
    ...
    __table_args__ = (
        ...
        UniqueConstraint("project_id", "period_type", "period_date", name="uq_evm_project_period"),
        # 索引优化查询性能
...
class EarnedValueSnapshot(Base, TimestampMixin):
    ...
    snapshot_code = Column(
        String(100), unique=True, nullable=False, comment="快照编码（如：PRJ001-2026-02-EVM）"
    )
    snapshot_name = Column(String(200), comment="快照名称")
    snapshot_date = Column(Date, nullable=False, comment="快照日期")
    snapshot_type = Column(
        String(20), default="MONTHLY", comment="快照类型：WEEKLY/MONTHLY/QUARTERLY/MILESTONE"
    )
    ...
    # 关联EVM数据
    evm_data_id = Column(
        Integer, ForeignKey("earned_value_data.id", ondelete="SET NULL"), comment="关联的EVM数据ID"
    )

    # 快照数据（JSON格式存储完整的EVM分析结果）
    snapshot_data = Column(Text, comment="快照数据（JSON格式，包含所有EVM指标和分析结果）")

    # 分析结论
    performance_status = Column(String(20), comment="绩效状态：EXCELLENT/GOOD/WARNING/CRITICAL")
    trend_direction = Column(String(20), comment="趋势方向：IMPROVING/STABLE/DECLINING")

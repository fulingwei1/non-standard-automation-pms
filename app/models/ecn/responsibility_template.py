...
class EcnResponsibility(Base, TimestampMixin):
    ...
    responsibility_type = Column(
        String(20), default="PRIMARY", comment="责任类型：PRIMARY/SECONDARY/SUPPORT"
    )
...
class EcnSolutionTemplate(Base, TimestampMixin):
    ...
    # 关系
    source_ecn = relationship(
        "Ecn", foreign_keys=[source_ecn_id], back_populates="solution_template"
    )

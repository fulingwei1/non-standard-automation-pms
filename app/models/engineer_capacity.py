...
class EngineerCapacity(Base, TimestampMixin):
    ...
    # 承载力评估
    avg_concurrent_projects = Column(
        Float, default=1.0, comment="平均同时负责项目数（从历史数据计算）"
    )
    ...
    # AI 能力评估
    ai_skill_level = Column(
        String(20), default="NONE", comment="AI 技能等级 NONE/BASIC/INTERMEDIATE/ADVANCED/EXPERT"
    )
    ai_tools = Column(Text, comment='常用 AI 工具 JSON ["Copilot", "Cursor", "Claude", "ChatGPT"]')
    ai_usage_frequency = Column(
        String(20), default="NEVER", comment="使用频率 NEVER/RARELY/SOMETIMES/OFTEN/DAILY"
    )
    ai_efficiency_boost = Column(
        Float, default=1.0, comment="AI 带来的效率提升系数 (1.0=无提升，2.0=效率翻倍)"
    )
    ...
    # 核心能力评估
    multi_project_capacity = Column(
        Integer, default=1, comment="多项目并行能力 (同时负责项目数上限)"
    )
    multi_project_efficiency = Column(
        Float, default=1.0, comment="多项目效率系数 (>1 表示多任务处理效率高)"
    )
    ...
    workload_status = Column(
        String(20), default="NORMAL", comment="工作负载状态 OVERLOAD/NORMAL/IDLE"
    )
...
class EngineerTaskAssignment(Base, TimestampMixin):
    ...
    # 状态
    status = Column(
        String(20), default="PENDING", comment="状态 PENDING/IN_PROGRESS/COMPLETED/CANCELLED"
    )
...
class WorkloadWarning(Base, TimestampMixin):
    ...
    # 预警类型
    warning_type = Column(
        String(50), nullable=False, comment="预警类型 OVERLOAD/CONFLICT/DELAY/QUALITY"
    )
    warning_level = Column(
        String(20), default="MEDIUM", comment="预警级别 LOW/MEDIUM/HIGH/CRITICAL"
    )
    ...
    # 状态
    status = Column(
        String(20), default="ACTIVE", comment="状态 ACTIVE/ACKNOWLEDGED/RESOLVED/IGNORED"
    )

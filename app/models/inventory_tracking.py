...
class MaterialTransaction(Base, TimestampMixin):
    ...
    related_order_type = Column(
        String(30),
        comment="关联单据类型: PURCHASE_ORDER-采购单, WORK_ORDER-工单, REQUISITION-领料单, COUNT_TASK-盘点任务",
    )
    ...
    cost_method = Column(
        ...
        comment="成本核算方法: FIFO-先进先出, LIFO-后进先出, WEIGHTED_AVG-加权平均",
    )
...
class MaterialStock(Base, TimestampMixin):
    ...
    available_quantity = Column(
        Numeric(14, 4), nullable=False, default=0, comment="可用数量(总数-预留)"
    )
    ...
    status = Column(
        String(20),
        default="NORMAL",
        comment="状态: NORMAL-正常, LOW-低库存, LOCKED-锁定, EXPIRED-过期",
    )
...
class MaterialReservation(Base, TimestampMixin):
    ...
    status = Column(
        String(20),
        default="ACTIVE",
        comment="状态: ACTIVE-生效中, PARTIAL_USED-部分使用, USED-已使用, CANCELLED-已取消, EXPIRED-已过期",
    )
...
class StockAdjustment(Base, TimestampMixin):
    ...
    status = Column(
        String(20),
        default="PENDING",
        comment="状态: PENDING-待审批, APPROVED-已批准, REJECTED-已拒绝",
    )
...
class StockCountTask(Base, TimestampMixin):
    ...
    # 盘点范围
    count_type = Column(
        String(20), default="FULL", comment="盘点类型: FULL-全盘, PARTIAL-抽盘, CYCLE-循环盘点"
    )
    ...
    status = Column(
        String(20),
        default="PENDING",
        comment="状态: PENDING-待盘点, IN_PROGRESS-盘点中, COMPLETED-已完成, CANCELLED-已取消",
    )
...
class StockCountDetail(Base, TimestampMixin):
    ...
    # 盘点任务
    task_id = Column(
        Integer, ForeignKey("stock_count_task.id"), nullable=False, comment="盘点任务ID"
    )
    ...
    status = Column(
        String(20),
        default="PENDING",
        comment="状态: PENDING-待盘点, COUNTED-已盘点, RECOUNTED-已复盘, CONFIRMED-已确认",
    )

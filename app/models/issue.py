...
class Issue(Base, TimestampMixin):
    ...
    acceptance_order_id = Column(
        Integer, ForeignKey("acceptance_orders.id"), comment="关联验收单ID"
    )
    ...
    root_cause = Column(
        String(20),
        comment="问题原因：DESIGN_ERROR/MATERIAL_DEFECT/PROCESS_ERROR/EXTERNAL_FACTOR/OTHER",
    )
    ...
    service_ticket = relationship(
        "ServiceTicket", foreign_keys=[service_ticket_id], back_populates="related_issues"
    )
    follow_ups = relationship(
        "IssueFollowUpRecord", back_populates="issue", lazy="dynamic", cascade="all, delete-orphan"
    )

    __table_args__ = ({"comment": "问题表 - 统一的问题管理中心"},)
...
class SolutionTemplate(Base, TimestampMixin):
    ...
    source_issue_id = Column(
        Integer, ForeignKey("issues.id"), comment="来源问题ID（从哪个问题提取的模板）"
    )
...
class IssueFollowUpRecord(Base, TimestampMixin):
    ...
    follow_up_type = Column(
        String(20),
        nullable=False,
        comment="跟进类型：COMMENT/STATUS_CHANGE/ASSIGNMENT/SOLUTION/VERIFICATION",
    )
    ...
    __table_args__ = ({"comment": "问题跟进记录表"},)

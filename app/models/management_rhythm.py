...
class ManagementRhythmConfig(Base, TimestampMixin):
    ...
    # 节律信息
    rhythm_level = Column(
        String(20), nullable=False, comment="节律层级:STRATEGIC/OPERATIONAL/OPERATION/TASK"
    )
    cycle_type = Column(
        String(20), nullable=False, comment="周期类型:QUARTERLY/MONTHLY/WEEKLY/DAILY"
    )
...
class StrategicMeeting(Base, TimestampMixin):
    ...
    rhythm_config_id = Column(
        Integer, ForeignKey("management_rhythm_config.id"), comment="节律配置ID"
    )

    # 会议信息
    rhythm_level = Column(
        String(20), nullable=False, comment="会议层级:STRATEGIC/OPERATIONAL/OPERATION/TASK"
    )
    cycle_type = Column(
        String(20), nullable=False, comment="周期类型:QUARTERLY/MONTHLY/WEEKLY/DAILY"
    )
    ...
    # 状态
    status = Column(
        String(20), default="SCHEDULED", comment="状态:SCHEDULED/ONGOING/COMPLETED/CANCELLED"
    )
...
class MeetingActionItem(Base, TimestampMixin):
    ...
    # 状态
    status = Column(
        String(20), default="PENDING", comment="状态:PENDING/IN_PROGRESS/COMPLETED/OVERDUE"
    )
...
class ReportMetricDefinition(Base, TimestampMixin):
    ...
    calculation_type = Column(
        String(20), nullable=False, comment="计算类型:COUNT/SUM/AVG/MAX/MIN/RATIO/CUSTOM"
    )
    ...
    format_type = Column(
        String(20), default="NUMBER", comment="格式类型:NUMBER/PERCENTAGE/CURRENCY"
    )

...
class OutsourcingOrderItem(Base, TimestampMixin):
    ...
    delivery_items = relationship(
        "OutsourcingDeliveryItem", back_populates="order_item", lazy="dynamic"
    )
...
class OutsourcingDeliveryItem(Base, TimestampMixin):
    ...
    order_item_id = Column(
        Integer, ForeignKey("outsourcing_order_items.id"), nullable=False, comment="订单明细ID"
    )
    ...
    inspections = relationship(
        "OutsourcingInspection", back_populates="delivery_item", lazy="dynamic"
    )

    __table_args__ = (Index("idx_os_deli_item_delivery", "delivery_id"),)
...
class OutsourcingInspection(Base, TimestampMixin):
    ...
    delivery_item_id = Column(
        Integer, ForeignKey("outsourcing_delivery_items.id"), nullable=False, comment="交付明细ID"
    )
...
class OutsourcingEvaluation(Base, TimestampMixin):
    ...
    # 关系
    vendor = relationship(
        "Vendor", foreign_keys=[vendor_id], back_populates="outsourcing_evaluations"
    )

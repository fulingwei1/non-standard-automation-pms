...
class PerformanceEvaluation(Base, TimestampMixin):
    ...
    # 评价人
    evaluator_id = Column(Integer, ForeignKey('user.id'), nullable=False, comment='评价人ID')
...
class PerformanceAppeal(Base, TimestampMixin):
    ...
    # 申诉人
    appellant_id = Column(Integer, ForeignKey('user.id'), nullable=False, comment='申诉人ID')
    ...
    # 处理结果
    handler_id = Column(Integer, ForeignKey('user.id'), comment='处理人ID')

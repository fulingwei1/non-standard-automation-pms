...
class PipelineBreakRecord(Base, TimestampMixin):
    ...
    break_stage = Column(
        String(20),
        nullable=False,
        comment="断链环节：LEAD_TO_OPP/OPP_TO_QUOTE/QUOTE_TO_CONTRACT/CONTRACT_TO_PROJECT/PROJECT_TO_INVOICE/INVOICE_TO_PAYMENT",
    )
...
class PipelineHealthSnapshot(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        # SQLite index names must be unique across the whole database.
        Index("idx_pipeline_snapshot_date", "snapshot_date"),
        {"comment": "健康度快照表"},
    )
...
class AccountabilityRecord(Base, TimestampMixin):
    ...
    issue_type = Column(
        String(50), nullable=False, comment="问题类型：DELAY/COST_OVERRUN/INFORMATION_GAP/BREAK"
    )
    responsible_person_id = Column(
        Integer, ForeignKey("users.id"), nullable=False, comment="责任人ID"
    )
    responsible_department = Column(String(50), comment="责任部门")

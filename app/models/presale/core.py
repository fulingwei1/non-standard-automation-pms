...
class PresaleTicketDeliverable(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_deliverable_ticket", "ticket_id"), {"comment": "工单交付物表"})
...
class PresaleTicketProgress(Base):
    ...
    __table_args__ = (Index("idx_progress_ticket", "ticket_id"), {"comment": "工单进度记录表"})
...
class PresaleSolution(Base, TimestampMixin):
    ...
    # 技术参数模板关联（2026-03-12 新增）
    template_id = Column(
        Integer,
        ForeignKey("technical_parameter_templates.id"),
        comment="关联技术参数模板ID"
    )
    template_parameters = Column(JSON, comment="使用的技术参数（JSON格式）")
    ...
    __table_args__ = (
        ...
        Index("idx_solution_project", "project_id"),
...
class PresaleSolutionCost(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_cost_solution", "solution_id"), {"comment": "方案成本明细表"})
...
class PresaleCustomerTechProfile(Base, TimestampMixin):
    ...
    __table_args__ = {"comment": "客户技术档案表"}

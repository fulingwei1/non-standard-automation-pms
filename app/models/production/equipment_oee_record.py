...
class EquipmentOEERecord(Base, TimestampMixin):
    ...
    operating_time = Column(
        Integer, nullable=False, comment="运行时间(分钟) = 计划生产时间 - 停机时间"
    )
    ...
    # OEE核心指标 (百分比 0-100)
    availability = Column(
        Numeric(5, 2), nullable=False, comment="可用率(%) = 运行时间/计划生产时间"
    )
    performance = Column(
        Numeric(5, 2), nullable=False, comment="性能率(%) = (理想周期×实际产量)/运行时间"
    )

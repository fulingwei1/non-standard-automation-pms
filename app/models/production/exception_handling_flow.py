...
class ExceptionHandlingFlow(Base, TimestampMixin):
    ...
    # 流程状态
    status = Column(
        Enum(FlowStatus), nullable=False, default=FlowStatus.PENDING, comment="流程状态"
    )

    # 升级信息
    escalation_level = Column(
        Enum(EscalationLevel), nullable=False, default=EscalationLevel.NONE, comment="升级级别"
    )
    ...
    escalated_to_id = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="升级至处理人ID"
    )
    ...
    escalated_to = relationship(
        "User", foreign_keys=[escalated_to_id], backref="escalated_exceptions"
    )

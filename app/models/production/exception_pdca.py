...
class ExceptionPDCA(Base, TimestampMixin):
    ...
    # 关联异常
    exception_id = Column(
        Integer, ForeignKey("production_exception.id"), nullable=False, comment="异常ID"
    )
    ...
    current_stage = Column(
        Enum(PDCAStage), nullable=False, default=PDCAStage.PLAN, comment="当前阶段"
    )
    ...
    plan_owner_id = Column(Integer, ForeignKey("users.id"), nullable=True, comment="计划负责人ID")
    ...
    do_owner_id = Column(Integer, ForeignKey("users.id"), nullable=True, comment="执行负责人ID")
    do_completed_at = Column(DateTime, nullable=True, comment="Do阶段完成时间")

    # Check阶段
    check_result = Column(Text, nullable=True, comment="检查结果")
    check_effectiveness = Column(
        String(20), nullable=True, comment="有效性：EFFECTIVE/PARTIAL/INEFFECTIVE"
    )
    check_data = Column(Text, nullable=True, comment="数据分析（JSON格式）")
    check_gap = Column(Text, nullable=True, comment="差距分析")
    check_owner_id = Column(Integer, ForeignKey("users.id"), nullable=True, comment="检查负责人ID")
    ...
    act_owner_id = Column(Integer, ForeignKey("users.id"), nullable=True, comment="改进负责人ID")

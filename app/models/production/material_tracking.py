...
class MaterialBatch(Base, TimestampMixin):
    ...
    quality_inspector_id = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="质检员ID"
    )
    ...
    status = Column(
        String(20),
        default="ACTIVE",
        comment="状态: ACTIVE-使用中, DEPLETED-已耗尽, EXPIRED-已过期, LOCKED-已锁定",
    )
...
class MaterialConsumption(Base, TimestampMixin):
    ...
    requisition_id = Column(
        Integer, ForeignKey("material_requisition.id"), nullable=True, comment="领料单ID"
    )
    ...
    consumption_type = Column(
        String(20),
        default="PRODUCTION",
        comment="消耗类型: PRODUCTION-生产, TESTING-测试, WASTE-报废, REWORK-返工, OTHER-其他",
    )
...
class MaterialAlert(Base, TimestampMixin):
    ...
    alert_type = Column(
        String(30),
        nullable=False,
        comment="预警类型: SHORTAGE-缺料, LOW_STOCK-低库存, EXPIRED-过期, SLOW_MOVING-呆滞, HIGH_WASTE-高浪费",
    )
    alert_level = Column(
        String(20),
        default="WARNING",
        comment="预警级别: INFO-提示, WARNING-警告, CRITICAL-严重, URGENT-紧急",
    )
    ...
    status = Column(
        String(20),
        default="ACTIVE",
        comment="状态: ACTIVE-活动, RESOLVED-已解决, IGNORED-已忽略, CLOSED-已关闭",
    )
...
class MaterialAlertRule(Base, TimestampMixin):
    ...
    # 物料范围
    material_id = Column(
        Integer, ForeignKey("materials.id"), nullable=True, comment="物料ID (NULL表示全局规则)"
    )
    category_id = Column(
        Integer, ForeignKey("material_categories.id"), nullable=True, comment="物料分类ID"
    )
    ...
    threshold_type = Column(
        String(20),
        default="PERCENTAGE",
        comment="阈值类型: PERCENTAGE-百分比, FIXED-固定值, DAYS-天数",
    )

...
class ProductionSchedule(Base, TimestampMixin):
    ...
    conflicts = relationship(
        ...
        foreign_keys="ProductionResourceConflict.schedule_id",
    )
...
class ProductionResourceConflict(Base, TimestampMixin):
    ...
    # 排程信息
    schedule_id = Column(
        Integer, ForeignKey("production_schedule.id"), nullable=False, comment="排程ID"
    )
    conflicting_schedule_id = Column(
        Integer, ForeignKey("production_schedule.id"), nullable=True, comment="冲突的排程ID"
    )
    ...
    conflict_type = Column(
        String(50),
        nullable=False,
        comment="冲突类型: EQUIPMENT/WORKER/TIME_OVERLAP/SKILL_MISMATCH/CAPACITY_EXCEEDED",
    )
    ...
    severity = Column(
        String(20), nullable=False, default="MEDIUM", comment="严重程度: LOW/MEDIUM/HIGH/CRITICAL"
    )
    ...
    status = Column(
        ...
        nullable=False,
        default="UNRESOLVED",
        comment="状态: UNRESOLVED/RESOLVED/IGNORED",
    )
    ...
    schedule = relationship(
        "ProductionSchedule", back_populates="conflicts", foreign_keys=[schedule_id]
    )
    conflicting_schedule = relationship(
        "ProductionSchedule", foreign_keys=[conflicting_schedule_id]
    )
...
class ScheduleAdjustmentLog(Base, TimestampMixin):
    ...
    adjustment_type = Column(
        String(50),
        nullable=False,
        comment="调整类型: TIME_CHANGE/RESOURCE_CHANGE/PRIORITY_CHANGE/URGENT_INSERT/CANCEL/RESTORE",
    )
    trigger_source = Column(
        String(50),
        nullable=False,
        comment="触发源: MANUAL/AUTO_OPTIMIZE/CONFLICT_RESOLUTION/URGENT_ORDER",
    )
    ...
    approval_status = Column(
        String(20), nullable=True, comment="审批状态: PENDING/APPROVED/REJECTED"
    )
    ...
    extra_metadata = Column(
        JSON, nullable=True, comment="元数据"

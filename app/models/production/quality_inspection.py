...
class QualityInspection(Base, TimestampMixin):
    ...
    inspection_type = Column(
        ...
        nullable=False,
        default="PROCESS",
        comment="检验类型: IQC(来料)/IPQC(过程)/FQC(成品)/OQC(出货)",
    )
    ...
    inspection_result = Column(
        String(20), nullable=False, default="PENDING", comment="检验结果: PASS/FAIL/PENDING"
    )
    ...
    # 处理信息
    handling_result = Column(
        String(20), nullable=True, comment="处理结果: REWORK(返工)/SCRAP(报废)/CONCESSION(让步接收)"
    )
    rework_order_id = Column(
        Integer, ForeignKey("rework_order.id"), nullable=True, comment="返工单ID"
    )
    ...
    rework_order = relationship(
        "ReworkOrder", foreign_keys=[rework_order_id], back_populates="inspections"
    )
...
class DefectAnalysis(Base, TimestampMixin):
    ...
    # 关联信息
    related_equipment_id = Column(
        Integer, ForeignKey("equipment.id"), nullable=True, comment="关联设备ID"
    )
    related_worker_id = Column(
        Integer, ForeignKey("worker.id"), nullable=True, comment="关联工人ID"
    )
    related_material_id = Column(
        Integer, ForeignKey("materials.id"), nullable=True, comment="关联物料ID"
    )
    ...
    responsible_person_id = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="责任人ID"
    )
    ...
    verification_result = Column(
        String(20), nullable=True, comment="验证结果: EFFECTIVE/INEFFECTIVE"
    )
...
class QualityAlertRule(Base, TimestampMixin):
    ...
    # 规则配置
    alert_type = Column(
        String(20), nullable=False, comment="预警类型: DEFECT_RATE/SPC_UCL/SPC_LCL/TREND"
    )
    target_material_id = Column(
        Integer, ForeignKey("materials.id"), nullable=True, comment="目标物料ID(空则全局)"
    )
    target_process_id = Column(
        Integer, ForeignKey("process_dict.id"), nullable=True, comment="目标工序ID"
    )

    # 阈值配置
    threshold_value = Column(Numeric(12, 4), nullable=False, comment="阈值")
    threshold_operator = Column(
        String(10), nullable=False, default="GT", comment="比较运算符: GT/GTE/LT/LTE/EQ"
    )
    ...
    # 预警配置
    alert_level = Column(
        String(20), nullable=False, default="WARNING", comment="预警级别: INFO/WARNING/CRITICAL"
    )
...
class ReworkOrder(Base, TimestampMixin):
    ...
    quality_inspection_id = Column(
        Integer, ForeignKey("quality_inspection.id"), nullable=True, comment="质检记录ID"
    )
    ...
    inspections = relationship(
        "QualityInspection",
        foreign_keys="QualityInspection.rework_order_id",
        back_populates="rework_order",
    )

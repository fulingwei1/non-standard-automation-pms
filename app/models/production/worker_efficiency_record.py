...
class WorkerEfficiencyRecord(Base, TimestampMixin):
    ...
    efficiency = Column(
        Numeric(6, 2), nullable=False, comment="工作效率(%) = 标准工时/实际工时×100"
    )
    quality_rate = Column(Numeric(5, 2), nullable=False, comment="合格率(%) = 合格数/完成数×100")
    utilization_rate = Column(
        Numeric(5, 2), nullable=False, comment="利用率(%) = (实际工时-空闲)/实际工时×100"
    )

    # 综合效率 = 工作效率 × 合格率 × 利用率
    overall_efficiency = Column(
        Numeric(6, 2), nullable=False, comment="综合效率(%) = 效率×质量×利用率"
    )

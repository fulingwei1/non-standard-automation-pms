...
class ProjectStageResourcePlan(Base, TimestampMixin):
    ...
    staffing_need_id = Column(
        Integer, ForeignKey("mes_project_staffing_need.id"), comment="关联人员需求ID"
    )
...
class ResourceConflict(Base, TimestampMixin):
    ...
    # 冲突的两个资源计划
    plan_a_id = Column(
        Integer, ForeignKey("project_stage_resource_plan.id"), nullable=False, comment="计划A ID"
    )
    plan_b_id = Column(
        Integer, ForeignKey("project_stage_resource_plan.id"), nullable=False, comment="计划B ID"
    )

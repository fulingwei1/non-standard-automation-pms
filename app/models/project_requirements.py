...
class ProjectRequirement(Base, TimestampMixin):
    ...
    requirement_type = Column(
        String(50), nullable=False, comment="需求类型 PRODUCTION/SERVICE/DESIGN/DEBUG"
    )
...
class EngineerRecommendation(Base, TimestampMixin):
    ...
    requirement_id = Column(
        Integer, ForeignKey("project_requirements.id"), nullable=False, comment="需求 ID"
    )

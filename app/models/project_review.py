...
class ProjectReview(Base, TimestampMixin):
    ...
    review_type = Column(
        String(20),
        default="POST_MORTEM",
        comment="复盘类型：POST_MORTEM/MID_TERM/QUARTERLY（结项复盘/中期复盘/季度复盘）",
    )
    ...
    best_practices_list = relationship(
        "ProjectBestPractice", back_populates="review", cascade="all, delete-orphan"
    )
...
class ProjectLesson(Base, TimestampMixin):
    ...
    # 经验教训信息
    lesson_type = Column(
        String(20), nullable=False, comment="类型：SUCCESS/FAILURE（成功经验/失败教训）"
    )
...
class ProjectBestPractice(Base, TimestampMixin):
    ...
    validation_status = Column(
        String(20), default="PENDING", comment="验证状态：PENDING/VALIDATED/REJECTED"
    )

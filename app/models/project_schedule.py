...
class ProjectSchedulePlan(Base, TimestampMixin):
    ...
    # 状态
    status = Column(
        String(20), default="DRAFT", comment="状态 DRAFT/PENDING/APPROVED/ACTIVE/COMPLETED"
    )
    ...
    tasks_rel = relationship(
        "ScheduleTask", back_populates="schedule_plan", cascade="all, delete-orphan"
    )

    __table_args__ = {"comment": "项目进度计划表"}
...
class ScheduleTask(Base, TimestampMixin):
    ...
    # 计划关联
    schedule_plan_id = Column(
        Integer, ForeignKey("project_schedule_plans.id"), nullable=False, comment="计划 ID"
    )
    ...
    __table_args__ = {"comment": "进度计划任务表"}

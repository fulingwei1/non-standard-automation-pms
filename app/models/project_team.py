...
class ProjectTeamPlan(Base, TimestampMixin):
    ...
    # 审核状态
    status = Column(
        String(20), default="DRAFT", comment="状态 DRAFT/PENDING/APPROVED/REJECTED/ARCHIVED"
    )
    ...
    members = relationship(
        "ProjectTeamMember", back_populates="team_plan", cascade="all, delete-orphan"
    )

    __table_args__ = {"comment": "项目组团队方案表"}
...
class ProjectTeamMember(Base, TimestampMixin):
    ...
    # 团队方案关联
    team_plan_id = Column(
        Integer, ForeignKey("project_team_plans.id"), nullable=False, comment="团队方案 ID"
    )
    ...
    # 角色与职责
    role = Column(
        String(50), nullable=False, comment="角色 PM/TECH_LEAD/MECH_ENG/ELEC_ENG/SERVICE_ENG"
    )
    ...
    # 状态
    status = Column(
        String(20), default="PLANNED", comment="状态 PLANNED/CONFIRMED/IN_PROGRESS/COMPLETED"
    )
...
class ProjectTeamApproval(Base, TimestampMixin):
    ...
    __table_args__ = {"comment": "项目组方案审批记录表"}

...
class PurchaseSuggestion(Base, TimestampMixin):
    ...
    # 建议来源
    source_type = Column(
        String(30), nullable=False, comment="来源类型：SHORTAGE/SAFETY_STOCK/FORECAST/MANUAL"
    )
    ...
    # 状态
    status = Column(
        String(20), default="PENDING", comment="状态：PENDING/APPROVED/REJECTED/ORDERED"
    )
    ...
    purchase_order_id = Column(
        Integer, ForeignKey("purchase_orders.id"), comment="生成的采购订单ID"
    )
...
class SupplierPerformance(Base, TimestampMixin):
    ...
    avg_price_vs_market = Column(
        Numeric(6, 2), default=0, comment="平均价格相对市场价(%)，负数表示低于市场价"
    )
...
class PurchaseOrderTracking(Base, TimestampMixin):
    ...
    event_type = Column(
        String(30),
        nullable=False,
        comment="事件类型：CREATED/SUBMITTED/APPROVED/CONFIRMED/SHIPPED/RECEIVED/COMPLETED",
    )

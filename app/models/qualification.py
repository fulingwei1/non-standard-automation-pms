...
class QualificationLevel(Base, TimestampMixin):
    ...
    level_code = Column(
        ...
        unique=True,
        nullable=False,
        comment="等级编码 (ASSISTANT/JUNIOR/MIDDLE/SENIOR/EXPERT)",
    )
    level_name = Column(
        String(50), nullable=False, comment="等级名称 (助理级/初级/中级/高级/专家级)"
    )
...
class PositionCompetencyModel(Base, TimestampMixin):
    ...
    level_id = Column(
        Integer, ForeignKey("qualification_level.id"), nullable=False, comment="等级ID"
    )
...
class EmployeeQualification(Base, TimestampMixin):
    ...
    current_level_id = Column(
        Integer, ForeignKey("qualification_level.id"), nullable=False, comment="当前等级ID"
    )

...
class QualityRiskDetection(Base, TimestampMixin):
    ...
    test_recommendations = relationship(
        "QualityTestRecommendation", back_populates="detection", cascade="all, delete-orphan"
    )
...
class QualityTestRecommendation(Base, TimestampMixin):
    ...
    project_id = Column(Integer, ForeignKey("projects.id"), nullable=False, comment="项目ID")
    detection_id = Column(
        Integer, ForeignKey("quality_risk_detection.id"), comment="关联的风险检测ID"
    )
    ...
    # 测试计划生成
    test_plan_generated = Column(
        Boolean, nullable=False, default=False, comment="是否已生成测试计划"
    )

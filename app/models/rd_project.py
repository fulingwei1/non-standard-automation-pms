...
class RdProjectCategory(Base, TimestampMixin):
    ...
    category_type = Column(
        String(20), nullable=False, comment="分类类型：SELF/ENTRUST/COOPERATION（自主/委托/合作）"
    )
...
class RdProject(Base, TimestampMixin):
    ...
    # 分类信息
    category_id = Column(
        Integer, ForeignKey("rd_project_category.id"), nullable=True, comment="项目分类ID"
    )
    ...
    project_manager_id = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="项目负责人ID"
    )
    ...
    linked_project_id = Column(
        Integer, ForeignKey("projects.id"), nullable=True, comment="关联的非标项目ID"
    )
    ...
    status = Column(
        String(20),
        default="DRAFT",
        comment="状态：DRAFT/PENDING/APPROVED/IN_PROGRESS/COMPLETED/CANCELLED",
    )
    ...
    documents = relationship("ProjectDocument", back_populates="rd_project", lazy="dynamic")
...
class RdCostType(Base, TimestampMixin):
    ...
    category = Column(
        String(20),
        nullable=False,
        comment="费用大类：LABOR/MATERIAL/DEPRECIATION/OTHER（人工/材料/折旧/其他）",
    )
...
class RdCost(Base, TimestampMixin):
    ...
    # 关联信息
    rd_project_id = Column(
        Integer, ForeignKey("rd_project.id"), nullable=False, comment="研发项目ID"
    )
    cost_type_id = Column(
        Integer, ForeignKey("rd_cost_type.id"), nullable=False, comment="费用类型ID"
    )
    ...
    # 材料费用相关（如果费用类型是材料）
    material_id = Column(
        Integer, ForeignKey("materials.id"), nullable=True, comment="物料ID（材料费用）"
    )
    ...
    # 折旧费用相关（如果费用类型是折旧）
    equipment_id = Column(
        Integer, ForeignKey("equipment.id"), nullable=True, comment="设备ID（折旧费用）"
    )
    depreciation_period = Column(String(20), nullable=True, comment="折旧期间（折旧费用）")

    # 来源信息
    source_type = Column(
        String(20), comment="来源类型：MANUAL/CALCULATED/ALLOCATED（手工录入/自动计算/分摊）"
    )
    ...
    allocation_rule_id = Column(
        Integer, ForeignKey("rd_cost_allocation_rule.id"), nullable=True, comment="分摊规则ID"
    )
...
class RdCostAllocationRule(Base, TimestampMixin):
    ...
    allocation_basis = Column(
        String(20), nullable=False, comment="分摊依据：HOURS/REVENUE/HEADCOUNT（工时/收入/人数）"
    )
...
class RdReportRecord(Base, TimestampMixin):
    ...
    report_type = Column(
        String(50),
        nullable=False,
        comment="报表类型：AUXILIARY_LEDGER/DEDUCTION_DETAIL/HIGH_TECH等",
    )

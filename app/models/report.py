...
class TimesheetReportTemplate(Base, TimestampMixin):
    ...
    archives = relationship(
        "ReportArchive", back_populates="template", cascade="all, delete-orphan"
    )
    recipients = relationship(
        "ReportRecipient", back_populates="template", cascade="all, delete-orphan"
    )
...
class ReportArchive(Base, TimestampMixin):
    ...
    # 关联模板
    template_id = Column(
        Integer, ForeignKey("report_template.id"), nullable=False, comment="模板ID"
    )
    report_type = Column(String(50), nullable=False, comment="报表类型")

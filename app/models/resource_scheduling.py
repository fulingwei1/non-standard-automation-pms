...
class ResourceConflictDetection(Base, TimestampMixin):
    ...
    conflict_type = Column(
        String(20), nullable=False, default="PERSON", comment="冲突类型: PERSON/DEVICE/WORKLOAD"
    )
    ...
    # 处理状态
    status = Column(
        String(20), default="DETECTED", comment="状态: DETECTED/ANALYZING/RESOLVED/IGNORED"
    )
    ...
    suggestions = relationship(
        "ResourceSchedulingSuggestion", back_populates="conflict", cascade="all, delete-orphan"
    )
    logs = relationship(
        "ResourceSchedulingLog", back_populates="conflict", cascade="all, delete-orphan"
    )
...
class ResourceSchedulingSuggestion(Base, TimestampMixin):
    ...
    # 关联冲突
    conflict_id = Column(
        Integer, ForeignKey("resource_conflict_detection.id"), nullable=False, comment="冲突ID"
    )
    ...
    solution_type = Column(
        String(30),
        nullable=False,
        comment="方案类型: RESCHEDULE/REALLOCATE/HIRE/OVERTIME/PRIORITIZE",
    )
    ...
    # 采纳情况
    status = Column(
        String(20), default="PENDING", comment="状态: PENDING/ACCEPTED/REJECTED/IMPLEMENTED"
    )
    ...
    implementer = relationship(
        "User", foreign_keys=[implemented_by], backref="implemented_suggestions"
    )
    creator = relationship("User", foreign_keys=[created_by], backref="created_suggestions")
    logs = relationship(
        "ResourceSchedulingLog", back_populates="suggestion", cascade="all, delete-orphan"
    )
...
class ResourceDemandForecast(Base, TimestampMixin):
    ...
    forecast_period = Column(
        String(20), nullable=False, comment="预测周期: 1MONTH/3MONTH/6MONTH/1YEAR"
    )
...
class ResourceUtilizationAnalysis(Base, TimestampMixin):
    ...
    analysis_period = Column(
        String(20), nullable=False, comment="分析周期: DAILY/WEEKLY/MONTHLY/QUARTERLY"
    )
    ...
    # 状态分类
    utilization_status = Column(
        String(20), comment="利用状态: UNDERUTILIZED/NORMAL/OVERUTILIZED/CRITICAL"
    )
    ...
    creator = relationship(
        "User", foreign_keys=[created_by], backref="created_utilization_analyses"
    )
...
class ResourceSchedulingLog(Base):
    ...
    suggestion_id = Column(
        Integer, ForeignKey("resource_scheduling_suggestions.id"), comment="方案ID"
    )
    ...
    action_type = Column(
        String(30),
        nullable=False,
        comment="操作类型: DETECT/ANALYZE/SUGGEST/REVIEW/IMPLEMENT/RESOLVE",
    )

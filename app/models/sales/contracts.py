...
class ContractTemplate(Base, TimestampMixin):
    ...
    current_version_id = Column(
        Integer, ForeignKey("contract_template_versions.id"), comment="当前版本ID"
    )
...
class ContractTemplateVersion(Base, TimestampMixin):
    ...
    template = relationship(
        "ContractTemplate", back_populates="versions", foreign_keys=[template_id]
    )
    creator = relationship(
        "User", foreign_keys=[created_by], backref="contract_template_versions_created"
    )
    publisher = relationship(
        "User", foreign_keys=[published_by], backref="contract_template_versions_published"
    )
...
class Contract(Base, TimestampMixin):
    ...
    quote_version = relationship(
        "QuoteVersion", back_populates="contracts", foreign_keys=[quote_id]
    )
...
class ContractAmendment(Base, TimestampMixin):
    ...
    amendment_type = Column(
        String(20), nullable=False, comment="变更类型：AMOUNT/DELIVERY_DATE/TERMS/OTHER"
    )
...
class ContractTerm(Base, TimestampMixin):
    ...
    term_type = Column(
        String(20),
        nullable=False,
        comment="条款类型: subject/price/delivery/payment/warranty/breach",
    )
    term_content = Column(Text, nullable=False, comment="条款内容")

    contract = relationship("Contract", back_populates="terms")

    __table_args__ = (Index("idx_contract_term_contract", "contract_id"),)
...
class ContractAttachment(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_contract_attachment_contract", "contract_id"),)

...
class PresaleAIWinRate(Base, TimestampMixin):
    ...
    influencing_factors = Column(
        JSON, comment="影响因素列表: [{factor, impact, type: positive/negative}]"
    )
    competitor_analysis = Column(
        JSON,
        comment="竞品分析: {competitors: [], our_advantages: [], competitor_advantages: [], strategy: []}",
    )
    improvement_suggestions = Column(
        JSON, comment="改进建议: {short_term: [], mid_term: [], milestones: []}"
    )
...
class PresaleWinRateHistory(Base, TimestampMixin):
    ...
    prediction_id = Column(
        Integer, ForeignKey("presale_ai_win_rate.id"), comment="关联的预测记录ID"
    )

    # 实际结果
    actual_result = Column(
        SQLEnum(WinRateResultEnum),
        default=WinRateResultEnum.PENDING,
        comment="实际结果: won/lost/pending",
    )

...
class SalesTargetV2(Base, TimestampMixin):
    ...
    target_period = Column(
        Enum(TargetPeriodEnumV2), nullable=False, comment="目标期间(year/quarter/month)"
    )
    ...
    # 目标主体
    target_type = Column(
        Enum(TargetTypeEnumV2), nullable=False, comment="目标类型(company/team/personal)"
    )
    team_id = Column(
        Integer, ForeignKey("sales_teams.id", ondelete="CASCADE"), nullable=True, comment="团队ID"
    )
    user_id = Column(
        Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=True, comment="用户ID"
    )

    # 目标指标
    sales_target = Column(Numeric(15, 2), default=0, nullable=False, comment="销售额目标")
    payment_target = Column(Numeric(15, 2), default=0, nullable=False, comment="回款目标")
    new_customer_target = Column(Integer, default=0, nullable=False, comment="新客户数目标")
    lead_target = Column(Integer, default=0, nullable=False, comment="线索数目标")
    opportunity_target = Column(Integer, default=0, nullable=False, comment="商机数目标")
    deal_target = Column(Integer, default=0, nullable=False, comment="签单数目标")

    # 实际完成值
    actual_sales = Column(Numeric(15, 2), default=0, nullable=False, comment="实际销售额")
    actual_payment = Column(Numeric(15, 2), default=0, nullable=False, comment="实际回款")
    actual_new_customers = Column(Integer, default=0, nullable=False, comment="实际新客户数")
    actual_leads = Column(Integer, default=0, nullable=False, comment="实际线索数")
    actual_opportunities = Column(Integer, default=0, nullable=False, comment="实际商机数")
    actual_deals = Column(Integer, default=0, nullable=False, comment="实际签单数")
    ...
    parent_target = relationship(
        "SalesTargetV2", remote_side=[id], backref="sub_targets", foreign_keys=[parent_target_id]
    )
    ...
    def __repr__(self):
        return (
            f"<SalesTargetV2(id={self.id}, type={self.target_type}, period={self.target_period})>"
        )
...
class TargetBreakdownLog(Base, TimestampMixin):
    ...
    breakdown_type = Column(
        String(20), nullable=False, comment="分解类型：AUTO(自动分解)/MANUAL(手动分解)"
    )
    breakdown_method = Column(
        String(50), comment="分解方法：EQUAL(平均分配)/RATIO(按比例)/CUSTOM(自定义)"
    )
    breakdown_details = Column(Text, comment="分解详情(JSON格式)")

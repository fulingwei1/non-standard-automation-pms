...
class ServiceTicket(Base, TimestampMixin):
    ...
    related_projects = relationship(
        "ServiceTicketProject", back_populates="ticket", cascade="all, delete-orphan"
    )

    # 抄送人员关系
    cc_users = relationship(
        "ServiceTicketCcUser", back_populates="ticket", cascade="all, delete-orphan"
    )

    # 关联问题关系
    related_issues = relationship(
        "Issue", foreign_keys="Issue.service_ticket_id", back_populates="service_ticket"
    )

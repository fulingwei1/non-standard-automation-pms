...
class AlertHandleLog(Base, TimestampMixin):
    ...
    # 操作信息
    action_type = Column(
        String(20), nullable=False, comment="操作类型：create/handle/update/escalate/resolve/close"
    )
...
class ShortageDailyReport(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_daily_report_date", "report_date"), {"comment": "缺料统计日报表"})

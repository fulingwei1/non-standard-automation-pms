...
class MaterialArrival(Base, TimestampMixin):
    ...
    purchase_order_id = Column(
        Integer, ForeignKey("purchase_orders.id"), nullable=True, comment="关联采购订单ID"
    )
    purchase_order_item_id = Column(
        Integer, ForeignKey("purchase_order_items.id"), nullable=True, comment="关联采购订单明细ID"
    )
    ...
    # 状态
    status = Column(
        String(20), default="PENDING", comment="状态：PENDING/IN_TRANSIT/DELAYED/RECEIVED/CANCELLED"
    )
...
class ArrivalFollowUp(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_follow_up_arrival", "arrival_id"), {"comment": "到货跟催记录表"})

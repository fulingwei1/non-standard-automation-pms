...
class MaterialSubstitution(Base, TimestampMixin):
    ...
    status = Column(
        String(20),
        default="DRAFT",
        comment="状态：DRAFT/TECH_PENDING/PROD_PENDING/APPROVED/REJECTED/EXECUTED",
    )
    tech_approver_id = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="技术审批人ID"
    )
    tech_approved_at = Column(DateTime, nullable=True, comment="技术审批时间")
    tech_approval_note = Column(Text, comment="技术审批意见")
    prod_approver_id = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="生产审批人ID"
    )
...
class MaterialTransfer(Base, TimestampMixin):
    ...
    from_project_id = Column(
        Integer, ForeignKey("projects.id"), nullable=True, comment="调出项目ID"
    )
    ...
    status = Column(
        String(20),
        default="DRAFT",
        comment="状态：DRAFT/PENDING/APPROVED/REJECTED/EXECUTED/CANCELLED",
    )

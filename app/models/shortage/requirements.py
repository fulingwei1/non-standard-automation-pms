...
class WorkOrderBom(Base, TimestampMixin):
    ...
    material_type = Column(
        String(20), default="purchase", comment="物料类型：purchase/make/outsource"
    )
...
class MaterialRequirement(Base, TimestampMixin):
    ...
    # 状态
    status = Column(
        String(20), default="pending", comment="状态：pending/partial/fulfilled/cancelled"
    )
    fulfill_method = Column(
        String(20), nullable=True, comment="满足方式：stock/purchase/substitute/transfer"
    )
...
class KitCheck(Base, TimestampMixin):
    ...
    kit_status = Column(
        String(20), default="shortage", comment="齐套状态：complete/partial/shortage"
    )

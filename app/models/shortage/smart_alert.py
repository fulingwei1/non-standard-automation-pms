...
class ShortageAlert(Base, TimestampMixin):
    ...
    # 预警级别与时间
    alert_level = Column(
        String(20), nullable=False, comment="预警级别: INFO/WARNING/CRITICAL/URGENT"
    )
    ...
    # 预测信息
    demand_forecast_id = Column(
        Integer, ForeignKey("material_demand_forecasts.id"), comment="需求预测ID"
    )
    ...
    # 状态与处理
    status = Column(
        String(20), default="PENDING", comment="状态: PENDING/PROCESSING/RESOLVED/CLOSED/CANCELLED"
    )
    auto_handled = Column(Boolean, default=False, comment="是否自动处理")
    handling_plan_id = Column(
        Integer, ForeignKey("shortage_handling_plans.id"), comment="处理方案ID"
    )
    ...
    resolution_type = Column(
        String(50), comment="解决方式: PURCHASE/SUBSTITUTE/TRANSFER/RESCHEDULE"
    )
    ...
    handling_plan = relationship(
        "ShortageHandlingPlan", foreign_keys=[handling_plan_id], back_populates="alerts"
    )
...
class ShortageHandlingPlan(Base, TimestampMixin):
    ...
    # 关联预警
    alert_id = Column(
        Integer, ForeignKey("shortage_alerts_enhanced.id"), nullable=False, comment="预警ID"
    )
    ...
    solution_type = Column(
        String(50),
        nullable=False,
        comment="方案类型: URGENT_PURCHASE/SUBSTITUTE/TRANSFER/PARTIAL_DELIVERY/RESCHEDULE",
    )
    ...
    score_weights = Column(
        JSON, comment='评分权重 {"feasibility": 0.3, "cost": 0.3, "time": 0.3, "risk": 0.1}'
    )
    ...
    status = Column(
        String(20),
        default="PENDING",
        comment="状态: PENDING/APPROVED/REJECTED/EXECUTING/COMPLETED/FAILED",
    )
    ...
    alerts = relationship(
        "ShortageAlert",
        foreign_keys="[ShortageAlert.handling_plan_id]",
        back_populates="handling_plan",
    )
...
class MaterialDemandForecast(Base, TimestampMixin):
    ...
    algorithm = Column(
        String(50),
        nullable=False,
        comment="预测算法: MOVING_AVERAGE/EXP_SMOOTHING/LINEAR_REGRESSION/ARIMA",
    )
    ...
    # 影响因素
    influencing_factors = Column(
        JSON, comment='影响因素 {"project_count": 5, "season": "high", ...}'
    )

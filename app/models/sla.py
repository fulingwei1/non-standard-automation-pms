...
class SLAPolicy(Base, TimestampMixin):
    ...
    # 预警设置
    warning_threshold_percent = Column(
        Numeric(5, 2), default=80, comment="预警阈值（百分比，达到此比例时预警）"
    )
...
class SLAMonitor(Base, TimestampMixin):
    ...
    response_status = Column(
        String(20), default="ON_TIME", nullable=False, comment="响应状态：ON_TIME/OVERDUE/WARNING"
    )
    resolve_status = Column(
        String(20), default="ON_TIME", nullable=False, comment="解决状态：ON_TIME/OVERDUE/WARNING"
    )

    # 时间差（小时）
    response_time_diff_hours = Column(
        Numeric(10, 2), comment="响应时间差（小时，正数表示超时，负数表示提前）"
    )
    resolve_time_diff_hours = Column(
        Numeric(10, 2), comment="解决时间差（小时，正数表示超时，负数表示提前）"
    )

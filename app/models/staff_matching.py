...
class HrTagDict(Base, TimestampMixin):
    ...
    tag_type = Column(
        String(20), nullable=False, comment="标签类型 (SKILL/DOMAIN/ATTITUDE/CHARACTER/SPECIAL)"
    )
...
class HrEmployeeProfile(Base, TimestampMixin):
    ...
    def __repr__(self):
        return (
            f"<HrEmployeeProfile employee={self.employee_id} workload={self.current_workload_pct}%>"
        )
...
class HrAIMatchingLog(Base):
    ...
    staffing_need_id = Column(
        Integer, ForeignKey("mes_project_staffing_need.id"), nullable=False, comment="人员需求ID"
    )
    candidate_employee_id = Column(
        Integer, ForeignKey("employees.id"), nullable=False, comment="候选员工ID"
    )
    ...
    recommendation_type = Column(
        String(20), comment="推荐类型 (STRONG/RECOMMENDED/ACCEPTABLE/WEAK)"
    )

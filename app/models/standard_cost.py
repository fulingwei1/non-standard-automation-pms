...
class StandardCost(Base, TimestampMixin):
    ...
    # 成本分类
    cost_category = Column(String(50), nullable=False, comment="成本类别：MATERIAL/LABOR/OVERHEAD")
    ...
    # 元数据
    parent_id = Column(
        Integer, ForeignKey("standard_costs.id"), comment="父成本项ID（用于版本追踪）"
    )
    ...
    history_records = relationship(
        "StandardCostHistory", back_populates="standard_cost", cascade="all, delete-orphan"
    )
...
class StandardCostHistory(Base, TimestampMixin):
    ...
    # 变更信息
    change_type = Column(
        String(20), nullable=False, comment="变更类型：CREATE/UPDATE/ACTIVATE/DEACTIVATE"
    )

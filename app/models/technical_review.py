...
class TechnicalReview(Base, TimestampMixin):
    ...
    equipment_id = Column(
        Integer, ForeignKey("machines.id"), nullable=True, comment="关联设备ID（多设备项目）"
    )
    ...
    status = Column(
        ...
        nullable=False,
        default="DRAFT",
        comment="状态：draft/pending/in_progress/completed/cancelled",
    )
    ...
    meeting_type = Column(
        String(20), nullable=False, default="ONSITE", comment="会议形式：onsite/online/hybrid"
    )
    ...
    # 评审结论
    conclusion = Column(
        String(30), nullable=True, comment="评审结论：pass/pass_with_condition/reject/abort"
    )
    ...
    participants = relationship(
        "ReviewParticipant", back_populates="review", lazy="dynamic", cascade="all, delete-orphan"
    )
    materials = relationship(
        "ReviewMaterial", back_populates="review", lazy="dynamic", cascade="all, delete-orphan"
    )
    checklist_records = relationship(
    ...
    issues = relationship(
        "ReviewIssue", back_populates="review", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class ReviewParticipant(Base):
    ...
    role = Column(
        String(20), nullable=False, comment="角色：host/expert/presenter/recorder/observer"
    )
    is_required = Column(Boolean, default=True, nullable=False, comment="是否必须参与")

    # 出席信息
    attendance = Column(
        String(20), nullable=True, comment="出席状态：pending/confirmed/absent/delegated"
    )
    delegate_id = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="代理人ID（请假时）"
    )
...
class ReviewMaterial(Base, TimestampMixin):
    ...
    material_type = Column(
        String(20), nullable=False, comment="材料类型：drawing/bom/report/document/other"
    )
...
class ReviewChecklistRecord(Base):
    ...
    issue_id = Column(
        Integer,
        ForeignKey("review_issues.id"),
        nullable=True,
        comment="关联问题ID（自动创建的问题）",
    )
...
class ReviewIssue(Base, TimestampMixin):
    ...
    issue_no = Column(
        String(50), unique=True, nullable=False, comment="问题编号：RV-ISSUE-202501-0001"
    )
    ...
    status = Column(
        ...
        nullable=False,
        default="OPEN",
        comment="状态：open/processing/resolved/verified/closed",
    )

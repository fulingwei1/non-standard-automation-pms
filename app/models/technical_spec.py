...
class TechnicalSpecRequirement(Base, TimestampMixin):
    ...
    requirement_level = Column(
        String(20), default="REQUIRED", comment="要求级别：REQUIRED/OPTIONAL/STRICT"
    )
    ...
    match_records = relationship(
        "SpecMatchRecord", back_populates="spec_requirement", lazy="dynamic"
    )
...
class SpecMatchRecord(Base, TimestampMixin):
    ...
    # 匹配结果
    match_status = Column(
        String(20), nullable=False, comment="匹配状态：MATCHED/MISMATCHED/UNKNOWN"
    )

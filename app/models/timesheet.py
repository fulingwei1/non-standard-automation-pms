...
class Timesheet(Base, TimestampMixin):
    ...
    # 审核信息
    approver_id = Column(Integer, ForeignKey('user.id'), comment='审核人ID')
...
class OvertimeApplication(Base, TimestampMixin):
    ...
    # 审批
    approver_id = Column(Integer, ForeignKey('user.id'), comment='审批人ID')
...
class TimesheetApprovalLog(Base):
    ...
    # 审批人
    approver_id = Column(Integer, ForeignKey('user.id'), nullable=False, comment='审批人ID')
    approver_name = Column(String(50), comment='审批人')

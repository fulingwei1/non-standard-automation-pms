...
class Timesheet(Base, TimestampMixin):
    ...
    # 项目任务关联
    project_id = Column(
        Integer, ForeignKey("projects.id"), nullable=True, comment="项目ID（非标项目）"
    )
    project_code = Column(String(50), comment="项目编号")
    project_name = Column(String(200), comment="项目名称")
    rd_project_id = Column(
        Integer,
        ForeignKey("rd_project.id"),
        nullable=True,
        comment="研发项目ID（可选，如果填写则直接关联研发项目）",
    )
...
class TimesheetSummary(Base, TimestampMixin):
    ...
    # 汇总维度
    summary_type = Column(
        String(20), nullable=False, comment="汇总类型:USER_MONTH/PROJECT_MONTH/DEPT_MONTH"
    )
...
class TimesheetRule(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_rule_code", "rule_code"), {"comment": "工时填报规则表"})

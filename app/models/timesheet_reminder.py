...
class TimesheetReminderConfig(Base, TimestampMixin):
    ...
    reminder_type = Column(SQLEnum(ReminderTypeEnum), nullable=False, comment="提醒类型")
    ...
    # 通知配置
    notification_channels = Column(JSON, default=["SYSTEM"], comment="通知渠道列表")
    notification_template = Column(Text, comment="通知模板")

    # 提醒频率控制
    remind_frequency = Column(
        String(20), default="ONCE_DAILY", comment="提醒频率: ONCE_DAILY/TWICE_DAILY/HOURLY"
    )
...
class TimesheetReminderRecord(Base, TimestampMixin):
    ...
    # 关联规则
    config_id = Column(Integer, ForeignKey("timesheet_reminder_config.id"), comment="关联配置ID")
    ...
    # 关联数据
    source_type = Column(String(50), comment="来源类型: timesheet/timesheet_batch/overtime")
    ...
    # 状态
    status = Column(
        SQLEnum(ReminderStatusEnum), default=ReminderStatusEnum.PENDING, comment="提醒状态"
    )
...
class TimesheetAnomalyRecord(Base, TimestampMixin):
    ...
    # 关联工时记录
    timesheet_id = Column(Integer, ForeignKey("timesheet.id"), nullable=False, comment="工时记录ID")
    ...
    # 异常类型
    anomaly_type = Column(SQLEnum(AnomalyTypeEnum), nullable=False, comment="异常类型")
    ...
    # 严重程度
    severity = Column(
        String(20), default="WARNING", comment="严重程度: INFO/WARNING/ERROR/CRITICAL"
    )

    # 检测时间
    detected_at = Column(DateTime, default=datetime.now, nullable=False, comment="检测时间")
    ...
    # 关联提醒
    reminder_id = Column(
        Integer, ForeignKey("timesheet_reminder_record.id"), comment="关联提醒记录ID"
    )

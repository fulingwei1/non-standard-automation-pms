...
class WorkLog(Base, TimestampMixin):
    ...
    # 工时记录关联
    timesheet_id = Column(
        Integer, ForeignKey("timesheet.id"), nullable=True, comment="关联的工时记录ID"
    )

    # 关系
    user = relationship("User", foreign_keys=[user_id])
    mentions = relationship(
        "WorkLogMention", back_populates="work_log", cascade="all, delete-orphan"
    )
...
class WorkLogConfig(Base, TimestampMixin):
    ...
    # 适用范围
    user_id = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="用户ID（NULL表示全员）"
    )
    department_id = Column(
        Integer, ForeignKey("departments.id"), nullable=True, comment="部门ID（可选）"
    )

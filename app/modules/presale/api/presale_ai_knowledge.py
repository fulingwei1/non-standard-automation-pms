...
from app.modules.presale.services.presale_ai_knowledge_service import PresaleAIKnowledgeService
...
def search_similar_cases(
    ...
    try:
        ...
        return SemanticSearchResponse(
            cases=[
                KnowledgeCaseResponse(
                    **case.to_dict(), similarity_score=getattr(case, "similarity_score", None)
                )
                for case in cases
            ],
            total=total,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"语义搜索失败: {str(e)}"
        )
...
def get_case(
    ...
    if not case:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"案例不存在: {case_id}")
...
def recommend_best_practices(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"最佳实践推荐失败: {str(e)}"
        )
...
def extract_case_knowledge(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"知识提取失败: {str(e)}"
        )
...
def ask_question(
    ...
    try:
        ...
        return AIQAResponse(
            ...
            matched_cases=[
                KnowledgeCaseResponse(**case.to_dict()) for case in result["matched_cases"]
            ],
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"智能问答失败: {str(e)}"
        )
...
def search_knowledge_base(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"知识库搜索失败: {str(e)}"
        )
...
def add_case(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"添加案例失败: {str(e)}"
        )
...
def get_all_tags(
    ...
    try:
        ...
        return TagsResponse(tags=result["tags"], tag_counts=result["tag_counts"])
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取标签失败: {str(e)}"
        )
...
def submit_qa_feedback(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"问答记录不存在: {feedback.qa_id}"
        )

...
class AIEmotionService:
    ...
    async def analyze_emotion(
        ...
        # 更新情绪趋势
        await self._update_emotion_trend(
            presale_ticket_id, customer_id, sentiment, purchase_intent_score
        )
    ...
    async def recommend_follow_up(
        ...
        if latest_emotion_analysis_id:
            emotion_analysis = (
                self.db.query(PresaleAIEmotionAnalysis)
                .filter(PresaleAIEmotionAnalysis.id == latest_emotion_analysis_id)
                .first()
            )
        else:
            emotion_analysis = (
                self.db.query(PresaleAIEmotionAnalysis)
                .filter(PresaleAIEmotionAnalysis.presale_ticket_id == presale_ticket_id)
                .order_by(desc(PresaleAIEmotionAnalysis.created_at))
                .first()
            )
        ...
        if emotion_analysis:
            follow_up_recommendation = await self._call_openai_for_follow_up(
                {
                    "sentiment": (
                        emotion_analysis.sentiment.value
                        if emotion_analysis.sentiment
                        else "neutral"
                    ),
                    "purchase_intent_score": (
                        float(emotion_analysis.purchase_intent_score)
                        if emotion_analysis.purchase_intent_score
                        else 50.0
                    ),
                    "churn_risk": (
                        emotion_analysis.churn_risk.value
                        if emotion_analysis.churn_risk
                        else "medium"
                    ),
    ...
    def get_emotion_trend(self, presale_ticket_id: int) -> Optional[PresaleEmotionTrend]:
        ...
        """
        return (
            self.db.query(PresaleEmotionTrend)
            .filter(PresaleEmotionTrend.presale_ticket_id == presale_ticket_id)
            .first()
        )
    ...
    def dismiss_reminder(self, reminder_id: int) -> bool:
        ...
        """
        reminder = (
            self.db.query(PresaleFollowUpReminder)
            .filter(PresaleFollowUpReminder.id == reminder_id)
            .first()
        )
    ...
    async def batch_analyze_customers(
        self, customer_ids: List[int], analysis_type: str = "full"
    ) -> Dict[str, Any]:
        """
        批量分析客户
        """
        ...
        for customer_id in customer_ids:
            try:
                ...
                latest_analysis = (
                    self.db.query(PresaleAIEmotionAnalysis)
                    .filter(PresaleAIEmotionAnalysis.customer_id == customer_id)
                    .order_by(desc(PresaleAIEmotionAnalysis.created_at))
                    .first()
                )
                ...
                if latest_analysis:
                    sentiment = (
                        latest_analysis.sentiment.value if latest_analysis.sentiment else None
                    )
                    purchase_intent_score = (
                        float(latest_analysis.purchase_intent_score)
                        if latest_analysis.purchase_intent_score
                        else None
                    )
                    churn_risk = (
                        latest_analysis.churn_risk.value if latest_analysis.churn_risk else None
                    )
                else:
                    sentiment = None
                ...
                needs_attention = self._needs_attention(
                    sentiment, purchase_intent_score, churn_risk
                )

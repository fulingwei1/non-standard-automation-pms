...
class QuotationPDFService:
    ...
    def generate_pdf(
        self, quotation: PresaleAIQuotation, company_info: Optional[dict] = None
    ) -> str:
        """
        生成PDF报价单
        Args:
        ...
        if not REPORTLAB_AVAILABLE:
            raise RuntimeError(
                "ReportLab is not installed. Please install it: pip install reportlab"
            )
        ...
        info_data = [
            ...
            [
                "报价类型:",
                self._get_type_display(quotation.quotation_type),
                "有效期:",
                f"{quotation.validity_days}天",
            ],
        ...
        items_table = Table(
            items_data, colWidths=[1.5 * cm, 7 * cm, 2 * cm, 2 * cm, 3 * cm, 3 * cm]
        )
    ...
    def generate_comparison_pdf(
        self, quotations: list[PresaleAIQuotation], company_info: Optional[dict] = None
    ) -> str:
        """
        生成三档方案对比PDF
        Args:
    ...
    def _get_type_display(self, quotation_type) -> str:
        """获取报价类型显示名称"""
        type_map = {"basic": "基础版", "standard": "标准版", "premium": "高级版"}
        return type_map.get(
            quotation_type.value if hasattr(quotation_type, "value") else quotation_type, "未知"
        )

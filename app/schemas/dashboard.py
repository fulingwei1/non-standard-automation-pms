...
class TopProjectsSchema(BaseModel):
    ...
    top_profit_margin_projects: List[ProjectCostItem] = Field(
        ..., description="利润率最高的10个项目"
    )
    bottom_profit_margin_projects: List[ProjectCostItem] = Field(
        ..., description="利润率最低的10个项目"
    )
...
class ExportDataSchema(BaseModel):
    ...
    data_type: str = Field(
        ..., description="数据类型: cost_overview/top_projects/cost_alerts/project_dashboard"
    )

...
class CustomerCreate(BaseModel):
    ...
    customer_code: Optional[str] = Field(
        default=None, max_length=50, description="客户编码（留空自动生成）"
    )
    ...
    customer_type: Optional[str] = Field(
        default="enterprise", description="客户类型：enterprise/individual"
    )
    ...
    status: Optional[str] = Field(
        default="potential", description="客户状态：potential/prospect/customer/lost"
    )

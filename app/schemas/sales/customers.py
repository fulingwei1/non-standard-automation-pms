...
class CustomerResponse(TimestampSchema):
    ...
    customer_type: Optional[str] = Field(default="enterprise", description="客户类型")
    ...
    annual_revenue: Optional[Decimal] = Field(default=Decimal("0"), description="年成交额")
    cooperation_years: Optional[int] = Field(default=0, description="合作年限")
    ...
    @classmethod
    def _normalize_customer_type(cls, v):
        return "enterprise" if v is None else v

    @field_validator("customer_level", mode="before")
    @classmethod
    def _normalize_customer_level(cls, v):
        return "C" if v is None else v

    @field_validator("credit_limit", "annual_revenue", mode="before")
    @classmethod
    def _normalize_decimal_fields(cls, v):
        return Decimal("0") if v is None else v

    @field_validator("account_period", "cooperation_years", mode="before")
    @classmethod
    def _normalize_int_fields(cls, v, info):
        if v is None:
            return 30 if info.field_name == "account_period" else 0
        return v

    @field_validator("status", mode="before")
    @classmethod
    def _normalize_status(cls, v):
        return "potential" if v is None else v

...
class ShortageAlertResponse(BaseModel):
    ...
    shortage_qty: Optional[Decimal] = Decimal("0")
    in_transit_qty: Optional[Decimal] = Decimal("0")
    ...
    days_to_shortage: Optional[int] = 0
    impact_projects: Optional[List[Dict]]
    estimated_delay_days: Optional[int] = 0
    estimated_cost_impact: Optional[Decimal] = Decimal("0")
    is_critical_path: Optional[bool] = False
    risk_score: Optional[Decimal] = Decimal("0")
    status: Optional[str] = "PENDING"
    auto_handled: Optional[bool] = False
    ...
    @field_validator("days_to_shortage", "estimated_delay_days", mode="before")
    @classmethod
    def _normalize_int_fields(cls, v):
        return 0 if v is None else v

    @field_validator("is_critical_path", "auto_handled", mode="before")
    @classmethod
    def _normalize_bool_fields(cls, v):
        return False if v is None else v

    @field_validator("alert_level", mode="before")
    @classmethod
    def _normalize_alert_level(cls, v):

...
class AccountLockoutService:
    ...
    def check_lockout(username: str, db: Session = None) -> Dict:
        ...
        db_locked_until: Optional[datetime] = None
        ...
        # Redis不可用时，使用数据库降级锁定逻辑
        if not redis and db:
            db_locked_until = AccountLockoutService._get_db_locked_until(username, db)
            if db_locked_until and db_locked_until > datetime.now():
                return {
                    "locked": True,
                    "locked_until": db_locked_until.isoformat(),
                    "remaining_attempts": 0,
                    "requires_captcha": False,
                    "message": f"账户已锁定，请在{db_locked_until.strftime('%H:%M')}后重试",
                }
    ...
    def _get_attempt_count(username: str, redis, db: Session = None) -> int:
        ...
        if db:
            try:
                count, _ = AccountLockoutService._get_attempt_stats_from_db(username, db)
                return count
            except Exception as e:
    ...
    @staticmethod
    def _get_attempt_stats_from_db(username: str, db: Session) -> Tuple[int, Optional[datetime]]:
        """从数据库获取时间窗口内失败次数和最近失败时间。"""
        cutoff_time = datetime.now() - timedelta(minutes=AccountLockoutService.ATTEMPT_WINDOW_MINUTES)
        base_query = db.query(LoginAttempt).filter(
        ...
        try:
            latest_attempt = base_query.order_by(LoginAttempt.created_at.desc()).first()
            latest_created_at = getattr(latest_attempt, "created_at", None)
            if latest_created_at and not isinstance(latest_created_at, datetime):
    ...
    @staticmethod
    def _get_db_locked_until(username: str, db: Session) -> Optional[datetime]:
        """
        基于数据库失败记录计算锁定截止时间。

        规则：在统计窗口内失败次数达到阈值时，从最近一次失败时间起锁定指定时长。
        """
        attempts, latest_failed_at = AccountLockoutService._get_attempt_stats_from_db(username, db)
        if attempts < AccountLockoutService.LOCKOUT_THRESHOLD or not latest_failed_at:
            return None
        return latest_failed_at + timedelta(minutes=AccountLockoutService.LOCKOUT_DURATION_MINUTES)

    @staticmethod
    def _get_ip_failed_count_from_db(ip: str, db: Session) -> int:
        """统计IP在时间窗口内的失败次数。"""
        cutoff_time = datetime.now() - timedelta(minutes=AccountLockoutService.ATTEMPT_WINDOW_MINUTES)
        return (
    ...
    def record_failed_login(
        ...
        if not redis and db:
            try:
                existing_attempts, _ = AccountLockoutService._get_attempt_stats_from_db(username, db)
                ...
                if attempts >= AccountLockoutService.LOCKOUT_THRESHOLD:
                    ...
                    logger.warning(f"账户已锁定(DB降级): {username}, IP: {ip}, 失败次数: {attempts}")

                ip_attempts = AccountLockoutService._get_ip_failed_count_from_db(ip, db) + 1
                if ip_attempts >= AccountLockoutService.IP_BLACKLIST_THRESHOLD:
                    ip_blacklisted = True
                    logger.critical(f"IP疑似暴力破解(DB降级): {ip}, 失败次数: {ip_attempts}")
            except Exception as e:
                logger.error(f"数据库降级锁定计算失败: {e}")
    ...
    @staticmethod
    def is_ip_blacklisted(ip: str, db: Session = None) -> bool:
        """检查IP是否在黑名单中"""
        redis = get_redis_client()
        if redis:
            try:
        ...
        # Redis不可用时，使用数据库窗口统计降级判断
        if db:
            try:
                ip_attempts = AccountLockoutService._get_ip_failed_count_from_db(ip, db)
                return ip_attempts >= AccountLockoutService.IP_BLACKLIST_THRESHOLD
            except Exception as e:
                logger.error(f"数据库检查IP黑名单失败: {e}")

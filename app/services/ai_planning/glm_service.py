...
class GLMService:
    ...
    def generate_project_plan(
        ...
        prompt = self._build_plan_generation_prompt(
            project_name, project_type, requirements, industry, complexity, reference_projects
        )
    ...
    def decompose_wbs(
        ...
        prompt = self._build_wbs_decomposition_prompt(
            task_name, task_description, task_type, estimated_duration, reference_tasks
        )
    ...
    def recommend_resources(
        self, task_info: Dict, available_users: List[Dict], constraints: Optional[Dict] = None
    ) -> Optional[List[Dict]]:
        """
        推荐资源分配
        ...
        Returns:
            资源分配建议列表
        """
        prompt = self._build_resource_recommendation_prompt(task_info, available_users, constraints)
...
"""
        if reference_tasks:
            ...
            for i, task in enumerate(reference_tasks[:3], 1):
                prompt += (
                    f"{i}. {task.get('name', '未知')} (工时:{task.get('effort_hours', '?')}小时)\n"
                )
...
"""
    ...
    def _build_resource_recommendation_prompt(
        self, task_info: Dict, available_users: List[Dict], constraints: Optional[Dict]
    ) -> str:
        """构建资源推荐提示词"""

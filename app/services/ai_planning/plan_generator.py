...
class AIProjectPlanGenerator:
    ...
    async def generate_plan(
        ...
        try:
            # 1. 查找参考项目
            reference_projects = self._find_reference_projects(project_type, industry, complexity)

            # 2. 检查是否有现成的模板
            if use_template:
                existing_template = self._find_existing_template(project_type, industry, complexity)
            ...
            if not plan_data:
                ...
                plan_data = self._generate_fallback_plan(
                    project_name, project_type, requirements, reference_projects
                )

            # 4. 创建模板对象
            template = self._create_template_from_data(
                plan_data, project_type, industry, complexity, reference_projects
            )
    ...
    def _find_reference_projects(
        self, project_type: str, industry: Optional[str], complexity: Optional[str], limit: int = 5
    ) -> List[Project]:
        """查找参考项目"""
    ...
    def _find_existing_template(
        self, project_type: str, industry: Optional[str], complexity: Optional[str]
    ) -> Optional[AIProjectPlanTemplate]:
        """查找现有模板"""
    ...
    def _generate_fallback_plan(
        ...
        if reference_projects:
            avg_duration = (
                ...
                / len(reference_projects)
                if reference_projects
                else 90
            )

            avg_cost = (
                sum(float(p.actual_cost) for p in reference_projects if p.actual_cost)
                / len(reference_projects)
                if reference_projects
                else 100000
            )
        ...
        phases = [
            {
                "name": "需求分析",
                "duration_days": int(avg_duration * 0.15),
                "deliverables": ["需求规格说明书"],
            },
            {
                "name": "设计阶段",
                "duration_days": int(avg_duration * 0.20),
                "deliverables": ["设计方案", "技术架构"],
            },
            {
                "name": "开发阶段",
                "duration_days": int(avg_duration * 0.40),
            ...
            {
                "name": "部署上线",
                "duration_days": int(avg_duration * 0.10),
                "deliverables": ["部署文档", "培训材料"],
            },
        ]
        ...
        milestones = [
            {"name": "需求评审", "phase": "需求分析", "estimated_day": phases[0]["duration_days"]},
            {
                "name": "设计评审",
                "phase": "设计阶段",
                "estimated_day": sum(p["duration_days"] for p in phases[:2]),
            },
            {
                "name": "开发完成",
                "phase": "开发阶段",
                "estimated_day": sum(p["duration_days"] for p in phases[:3]),
            },
            {
                "name": "测试通过",
                "phase": "测试阶段",
                "estimated_day": sum(p["duration_days"] for p in phases[:4]),
            },

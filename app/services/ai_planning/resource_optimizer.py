...
from app.models import TaskUnified, User
...
class AIResourceOptimizer:
    ...
    async def allocate_resources(
        ...
        allocations = []
        for user in users:
            allocation = await self._analyze_user_match(user, wbs, constraints)
            if allocation:
    ...
    def _get_available_users(
        self, user_ids: Optional[List[int]], wbs: AIWbsSuggestion
    ) -> List[User]:
        """获取可用用户列表"""
    ...
    async def _analyze_user_match(
        self, user: User, wbs: AIWbsSuggestion, constraints: Optional[Dict]
        ...
        # 5. 综合匹配度（加权平均）
        overall_match = (
            skill_match * 0.4 + experience_match * 0.2 + availability * 0.2 + performance * 0.2
        )
        ...
        allocation = AIResourceAllocation(
            ...
            # AI生成信息
            ai_model_version=(
                self.glm_service.model if self.glm_service.is_available() else "RULE_ENGINE"
            ),
            ...
            strengths=json.dumps(
                self._analyze_strengths(user, wbs, skill_match, performance), ensure_ascii=False
            ),
            weaknesses=json.dumps(
                self._analyze_weaknesses(user, wbs, skill_match, availability), ensure_ascii=False
            ),
            # 状态
            status="SUGGESTED",
            is_active=True,
    ...
    def _calculate_performance_score(self, user: User, wbs: AIWbsSuggestion) -> float:
        ...
        completed_tasks = (
            self.db.query(TaskUnified)
            .filter(TaskUnified.assignee_id == user.id, TaskUnified.status == "COMPLETED")
            .limit(20)
            .all()
        )
    ...
    def _analyze_strengths(
        self, user: User, wbs: AIWbsSuggestion, skill_match: float, performance: float
    ) -> List[Dict]:
        """分析优势"""
        ...
        if skill_match >= 80:
            strengths.append(
                {"category": "技能", "description": "技能高度匹配任务要求", "score": skill_match}
            )

        if performance >= 80:
            strengths.append(
                {"category": "绩效", "description": "历史任务完成质量高", "score": performance}
            )

        return strengths

    def _analyze_weaknesses(
        self, user: User, wbs: AIWbsSuggestion, skill_match: float, availability: float
    ) -> List[Dict]:
        """分析劣势"""
        ...
        if skill_match < 60:
            weaknesses.append(
                {"category": "技能", "description": "技能匹配度偏低", "impact": "HIGH"}
            )

        if availability < 40:
            weaknesses.append(
                {"category": "可用性", "description": "当前工作负载过高", "impact": "HIGH"}
            )

        return weaknesses

    def _optimize_allocations(
        self, allocations: List[AIResourceAllocation], wbs: AIWbsSuggestion
    ) -> List[AIResourceAllocation]:
        """优化资源分配（避免冲突）"""

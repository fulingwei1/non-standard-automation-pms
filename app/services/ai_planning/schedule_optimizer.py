...
class AIScheduleOptimizer:
    ...
    def optimize_schedule(
        ...
        # 获取所有WBS任务
        wbs_tasks = (
            self.db.query(AIWbsSuggestion)
            .filter(AIWbsSuggestion.project_id == project_id, AIWbsSuggestion.is_active == True)
            .order_by(AIWbsSuggestion.wbs_code)
            .all()
        )
        ...
        # 2. 生成甘特图数据
        gantt_data = self._generate_gantt_data(wbs_tasks, cpm_result, start_date)
    ...
    def _calculate_cpm(self, tasks: List[AIWbsSuggestion], start_date: date) -> Dict[str, Any]:
        """
        关键路径法(CPM)计算
    ...
    def _get_predecessors(
        self, task: AIWbsSuggestion, task_dict: Dict[int, AIWbsSuggestion]
    ...
    def _generate_gantt_data(
        self, tasks: List[AIWbsSuggestion], cpm_result: Dict, start_date: date
    ) -> List[Dict]:
        """生成甘特图数据"""
    ...
    def _identify_critical_path(self, tasks: List[AIWbsSuggestion], cpm_result: Dict) -> List[Dict]:
        """识别关键路径"""
    ...
    def _analyze_resource_load(self, project_id: int, cpm_result: Dict) -> Dict[int, Dict]:
        """分析资源负载"""

        # 获取所有资源分配
        allocations = (
            self.db.query(AIResourceAllocation)
            .filter(
    ...
    def _detect_conflicts(
        self, tasks: List[AIWbsSuggestion], cpm_result: Dict, resource_load: Dict
    ) -> List[Dict]:
        """检测冲突"""
    ...
    def _generate_recommendations(
        ...
        # 2. 资源平衡建议
        avg_load = (
            sum(r["total_hours"] for r in resource_load.values()) / len(resource_load)
            if resource_load
            else 0
        )
            ...
            recommendations.append(
                {
                    ...
                    "actions": ["重新分配任务，平衡工作负载", "考虑增加人手支援高负载成员"],
                }
            )
    ...
    def _calculate_resource_utilization(self, resource_load: Dict) -> float:
        ...
        utilization = (
            (total_hours / (resource_count * standard_hours)) * 100 if resource_count > 0 else 0
        )

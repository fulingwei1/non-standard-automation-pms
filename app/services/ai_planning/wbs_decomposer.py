...
class AIWbsDecomposer:
    ...
    async def decompose_project(
        self, project_id: int, template_id: Optional[int] = None, max_level: int = 3
        ...
        for task in level_1_tasks:
            ...
            if max_level > 1:
                subtasks = await self._decompose_task(task, project, level=2, max_level=max_level)
    ...
    async def _decompose_task(
        self, parent_task: AIWbsSuggestion, project: Project, level: int, max_level: int
    ) -> List[AIWbsSuggestion]:
        """递归分解任务"""
        ...
        reference_tasks = self._find_reference_tasks(
            parent_task.task_name, parent_task.task_type, project.project_type
        )
        ...
        for i, subtask_data in enumerate(subtasks_data, 1):
            suggestion = self._create_suggestion_from_data(
                subtask_data, project=project, parent_task=parent_task, level=level, sequence=i
            )
    ...
    def _find_reference_tasks(
        self, task_name: str, task_type: str, project_type: str, limit: int = 5
    ) -> List[TaskUnified]:
        """查找参考任务"""

        # 简单的关键词匹配
        query = self.db.query(TaskUnified).filter(
            TaskUnified.task_type == task_type, TaskUnified.status == "COMPLETED"
        )
    ...
    def _create_suggestion_from_data(
        self, data: Dict, project: Project, parent_task: AIWbsSuggestion, level: int, sequence: int
    ) -> AIWbsSuggestion:
        """从数据创建WBS建议对象"""
    ...
    def _calculate_task_duration(
        self, task: AIWbsSuggestion, all_tasks: List[AIWbsSuggestion]
    ) -> float:
        """递归计算任务总工期"""

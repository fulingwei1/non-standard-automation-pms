...
class MilestoneAlertService:
    ...
    def check_milestone_alerts(self):
        ...
        try:
            ...
            alert_count += self._process_upcoming_milestones(
                upcoming_milestones, warning_rule, today, alert_count
            )
    ...
    def _get_or_create_warning_rule(self) -> AlertRule:
        ...
        warning_rule = (
            ...
            .filter(and_(AlertRule.rule_code == "MILESTONE_WARNING", AlertRule.is_enabled))
            .first()
        )
    ...
    def _get_or_create_critical_rule(self) -> AlertRule:
        ...
        critical_rule = (
            self.db.query(AlertRule)
            .filter(and_(AlertRule.rule_code == "MILESTONE_OVERDUE", AlertRule.is_enabled))
            .first()
        )
    ...
    def _create_milestone_alert(
        ...
        if is_overdue:
            ...
            alert_content = (
                f"里程碑 {milestone.milestone_code} 已逾期 {days_diff} 天"
            ...
            alert_content = (
                f"里程碑 {milestone.milestone_code} 将在 {days_diff} 天后到期"
                f"（计划日期：{milestone.planned_date}）"
            )
            alert_level = AlertLevelEnum.WARNING.value

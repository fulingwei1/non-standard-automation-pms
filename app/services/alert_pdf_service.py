...
def build_alert_list_tables(db: Session, alerts: List[AlertRecord], page_size: int = 20) -> List:
    """
    构建预警列表表格（分页）
    ...
    try:
        ...
        for page_idx in range(0, len(alerts), page_size):
            ...
            # 表头
            table_data = [["预警编号", "级别", "标题", "项目", "触发时间", "状态", "处理人"]]
            ...
            for alert in page_alerts:
                ...
                table_data.append(
                    [
                        ...
                        alert.alert_level,
                        (
                            alert.alert_title[:30] + "..."
                            if len(alert.alert_title) > 30
                            else alert.alert_title
                        ),
            ...
            # 创建表格
            alert_table = Table(
                table_data, colWidths=[3 * cm, 2 * cm, 5 * cm, 3 * cm, 3 * cm, 2 * cm, 2 * cm]
            )
...
def build_pdf_content(
    db: Session, alerts: List[AlertRecord], title_style, heading_style, normal_style
) -> List:
    """
    构建PDF内容

...
class AssemblyKitOptimizer:
    ...
    def optimize_estimated_ready_date(
        ...
        for shortage in blocking_shortages:
            # 1. 检查是否有在途采购订单可以提前
            optimized_date = cls._optimize_by_purchase_order(db, shortage)
            if optimized_date:
                optimized_dates.append(optimized_date)

            # 2. 检查是否有替代物料
            substitute_date = cls._optimize_by_substitute(db, shortage)
            if substitute_date:
    ...
    @classmethod
    def _optimize_by_purchase_order(cls, db: Session, shortage: ShortageDetail) -> Optional[date]:
        """通过采购订单优化（检查是否可以加急）"""
        ...
        po_items = (
            ...
            .order_by(PurchaseOrder.promised_date.asc())
            .all()
        )
    ...
    @classmethod
    def _optimize_by_substitute(cls, db: Session, shortage: ShortageDetail) -> Optional[date]:
        """通过替代物料优化"""
        if not shortage.material_id:
            return None

        # 检查BOM项是否有替代物料配置
        bom_item = db.query(BomItem).filter(BomItem.id == shortage.bom_item_id).first()
        ...
        attr = (
            ...
            .filter(
                BomItemAssemblyAttrs.bom_item_id == bom_item.id, BomItemAssemblyAttrs.has_substitute
            )
            .first()
        )
        ...
        try:
            ...
            for sub_material_id in substitute_ids:
                material = db.query(Material).filter(Material.id == sub_material_id).first()
    ...
    @classmethod
    def _suggest_expedite_purchase(cls, db: Session, shortage: ShortageDetail) -> Optional[Dict]:
        """建议加急采购"""
        if not shortage.material_id:
            return None
        ...
        if po_items:
            ...
            return {
                ...
                "required_date": (
                    shortage.required_date.isoformat() if shortage.required_date else None
                ),
    ...
    @classmethod
    def _suggest_substitute(cls, db: Session, shortage: ShortageDetail) -> Optional[Dict]:
        """建议使用替代物料"""
        bom_item = db.query(BomItem).filter(BomItem.id == shortage.bom_item_id).first()
    ...
    @classmethod
    def _suggest_priority_adjustment(cls, db: Session, shortage: ShortageDetail) -> Optional[Dict]:
        """建议调整采购优先级"""
        if not shortage.material_id or not shortage.purchase_order_id:
            return None

        po = db.query(PurchaseOrder).filter(PurchaseOrder.id == shortage.purchase_order_id).first()

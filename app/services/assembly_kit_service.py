...
class AssemblyKitService:
    ...
    def auto_assign_materials_to_stages(self, bom_id: int) -> Dict[str, Any]:
        ...
        for item in bom_items:
            ...
            existing = (
                ...
                .filter(BomItemAssemblyAttrs.bom_item_id == item.id)
                .first()
            )
    ...
    def calculate_stage_kit_rate(
        ...
        bom_query = self.db.query(BomHeader).filter(
            BomHeader.project_id == project_id, BomHeader.status == "RELEASED"
        )
        ...
        assembly_attrs = (
            self.db.query(BomItemAssemblyAttrs)
            .filter(BomItemAssemblyAttrs.bom_id.in_(bom_ids))
            .all()
        )
        ...
        for stage, data in stages_data.items():
            ...
            blocking_kit_rate = (
                ...
                if data["blocking_total"] > 0
                else 100  # 无阻塞物料时视为 100%
            )
    ...
    def save_readiness_analysis(
        ...
        blocking_fulfilled = sum(
            s["blocking_fulfilled"] for s in kit_rate_result["stages"].values()
        )

        # 整体齐套率
        overall_kit_rate = fulfilled_items / total_items * 100 if total_items > 0 else 0
        blocking_kit_rate = blocking_fulfilled / blocking_total * 100 if blocking_total > 0 else 100
    ...
    def get_material_lead_time(self, material_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "estimated_arrival": (
                (last_date + timedelta(days=avg_lead_time)).isoformat() if last_date else None
            ),
        }
    ...
    def calculate_time_based_kit_rate(
        ...
        result = {
            ...
            "stages": {},
            "summary": {
                "total_shortage_items": sum(
                    len(s["shortage_items"]) for s in material_analysis.values()
                ),
                "total_urgent_items": sum(
                    len(s["urgent_items"]) for s in material_analysis.values()
                ),
...
def calculate_stage_kit_rates(
    ...
    List[Dict[str, Any]], bool, Optional[str], Optional[str], Dict[str, int], List[Dict[str, Any]]
]:
    """汇总各阶段齐套率并返回开工判断。"""

...
class AssemblyKitServiceEnhanced:
    ...
    def get_estimated_delivery_from_history(
        self, material_id: int, order_date: Optional[date] = None
    ) -> Optional[date]:
        """
        根据历史数据推算预计到货日期
    ...
    def calculate_enhanced_time_based_kit_rate(
        ...
        # 获取基础齐套率数据
        kit_rate_result = self.calculate_time_based_kit_rate(
            project_id, machine_id, planned_start_date
        )
            ...
            for item in stage_data["shortage_items"]:
                ...
                if spring_festival_info["is_spring_festival_period"]:
                    ...
                    if estimated_arrival:
                        # 如果预计到货在春节期间，顺延
                        if estimated_arrival >= date.fromisoformat(
                            spring_festival_info["pre_festival_start"]
                        ) and estimated_arrival <= date.fromisoformat(
                            spring_festival_info["post_festival_end"]
                        ):

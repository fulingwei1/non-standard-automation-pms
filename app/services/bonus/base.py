...
class BonusCalculatorBase:
    ...
    def get_active_rules(self, bonus_type: Optional[str] = None) -> List[BonusRule]:
        """
        获取启用的奖金规则
        ...
        today = date.today()
        query = query.filter(
            (BonusRule.effective_start_date.is_(None)) | (BonusRule.effective_start_date <= today)
        )
        query = query.filter(
            (BonusRule.effective_end_date.is_(None)) | (BonusRule.effective_end_date >= today)
        )

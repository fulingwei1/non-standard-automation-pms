...
def validate_sheet_for_distribution(sheet: BonusAllocationSheet) -> Tuple[bool, Optional[str]]:
    """
    验证明细表是否可以发放
...
def create_calculation_from_team_allocation(
    ...
    """
    allocation = (
        db.query(TeamBonusAllocation).filter(TeamBonusAllocation.id == team_allocation_id).first()
    )
...
def check_distribution_exists(db: Session, calculation_id: int, user_id: int) -> bool:
    """
    检查是否已发放

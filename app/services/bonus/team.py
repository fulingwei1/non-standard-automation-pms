...
class TeamBonusCalculator(BonusCalculatorBase):
    ...
    def calculate(
        ...
        total_contribution = sum(
            float(c.hours_percentage) if c.hours_percentage else 0 for c in contributions
        )

        if total_contribution > 0:
            for contrib in contributions:
                contribution_score = (
                    float(contrib.hours_percentage) if contrib.hours_percentage else 0
                )
            ...
            # 如果没有贡献记录，平均分配
            members = (
                self.db.query(ProjectMember).filter(ProjectMember.project_id == project.id).all()
            )
            if members:

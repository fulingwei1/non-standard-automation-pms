...
class BonusCalculator:
    ...
    def trigger_acceptance_bonus_calculation(
        ...
        try:
            ...
            if contract:
                sales_rules = self.get_active_rules(bonus_type='SALES_BASED')
        ...
        # 2. 计算售前技术奖金总额（团队奖金）
        try:
            presale_rules = self.get_active_rules(bonus_type='PRESALE_BASED')
            if presale_rules:
                # 查找项目关联的售前工单
                presale_tickets = self.db.query(PresaleSupportTicket).filter(
        ...
        # 3. 计算项目奖金总额（团队奖金）
        try:
            project_rules = self.get_active_rules(bonus_type='PROJECT_BASED')
            if project_rules:
                # 计算总奖金（按项目金额的百分比）
                for rule in project_rules:
                    ...
                    if total_bonus > 0:
                        ...
                        # 获取项目贡献记录（如果有）
                        contributions = self.db.query(ProjectContribution).filter(
                            ProjectContribution.project_id == project.id
                        ).all()

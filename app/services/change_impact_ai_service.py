...
class ChangeImpactAIService:
    ...
    async def analyze_change_impact(
        ...
        # 获取项目信息
        project = self.db.query(Project).filter(Project.id == change.project_id).first()
        ...
        try:
            ...
            # 1. 分析进度影响
            schedule_impact = await self._analyze_schedule_impact(change, project, context_data)

            # 2. 分析成本影响
            cost_impact = await self._analyze_cost_impact(change, project, context_data)

            # 3. 分析质量影响
            quality_impact = await self._analyze_quality_impact(change, project, context_data)

            # 4. 分析资源影响
            resource_impact = await self._analyze_resource_impact(change, project, context_data)

            # 5. 识别连锁反应
            chain_reaction = self._identify_chain_reactions(change, project, context_data)
            ...
            analysis.schedule_critical_path_affected = schedule_impact.get(
            ...
            analysis.cost_contingency_required = Decimal(
                str(cost_impact.get("contingency_required", 0))
            )
            ...
            analysis.resource_reallocation_needed = resource_impact.get(
            ...
            self.db.commit()
            logger.info(
                f"变更影响分析完成: change_id={change_request_id}, duration={analysis.analysis_duration_ms}ms"
            )
    ...
    def _gather_analysis_context(self, change: ChangeRequest, project: Project) -> Dict[str, Any]:
        """收集分析上下文数据"""
        # 获取项目任务
        ...
        dependencies = (
            self.db.query(TaskDependency).filter(TaskDependency.task_id.in_(task_ids)).all()
            if task_ids
            else []
        )
        ...
        # 获取里程碑
        milestones = (
            self.db.query(ProjectMilestone).filter(ProjectMilestone.project_id == project.id).all()
        )
...
"""
        ...
        try:
            response = await call_glm_api(prompt=prompt, temperature=0.3, max_tokens=1500)
    ...
    async def _analyze_quality_impact(
        ...
        return {
            ...
            "testing_impact": (
                "需要增加测试时间和测试用例" if mitigation_required else "测试影响较小"
            ),
            "acceptance_impact": (
                "可能影响验收标准" if level in ["HIGH", "CRITICAL"] else "验收影响可控"
            ),
    ...
    def _calculate_overall_risk(
        ...
        """计算综合风险"""
        # 影响级别权重
        level_scores = {"NONE": 0, "LOW": 25, "MEDIUM": 50, "HIGH": 75, "CRITICAL": 100}
    ...
    def _parse_ai_response(self, response: str, default: Dict[str, Any]) -> Dict[str, Any]:
        """解析AI响应"""
        try:
            # 尝试提取JSON
            import re

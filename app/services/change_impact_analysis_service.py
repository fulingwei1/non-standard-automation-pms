...
def analyze_schedule_impact(
    db: Session, change: PmoChangeRequest, project_id: int
...
def analyze_cost_impact(change: PmoChangeRequest, project: Project) -> Optional[Dict[str, Any]]:
    """
    分析成本影响
...
def build_impact_analysis(
    db: Session, change: PmoChangeRequest, project: Project, project_id: int
) -> Dict[str, Any]:
    """
    构建变更影响分析
...
def calculate_change_statistics(
    changes: List[PmoChangeRequest], impact_analysis: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
    计算变更统计信息
    ...
    total_cost_impact = sum(
        abs(float(change.cost_impact or 0)) for change in changes if change.cost_impact
    )

    affected_projects_count = len(
        set(
            proj["project_id"]
            for impact in impact_analysis
            for proj in impact.get("impacts", {})

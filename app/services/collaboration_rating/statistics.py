...
class RatingStatistics:
    ...
    def get_average_collaboration_score(self, engineer_id: int, period_id: int) -> Decimal:
        """
        获取平均协作得分
    ...
    def get_rating_statistics(self, period_id: int) -> Dict[str, Any]:
        ...
        for rating in completed:
            ...
            if ratee_job_type not in job_type_stats:
                job_type_stats[ratee_job_type] = {"count": 0, "total_score": 0.0, "avg_score": 0.0}
        ...
        for job_type in job_type_stats:
            ...
            stats["avg_score"] = (
                round(stats["total_score"] / stats["count"], 2) if stats["count"] > 0 else 0.0
            )
    ...
    def get_collaboration_trend(self, engineer_id: int, periods: int = 6) -> Dict[str, Any]:
        """
        获取跨部门协作趋势
    ...
    def analyze_rating_quality(self, period_id: int) -> Dict[str, Any]:
        """
        分析评价质量
        ...
        if not ratings:
            return {"total_ratings": 0, "quality_analysis": {}, "recommendations": []}

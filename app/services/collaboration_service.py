...
class CollaborationService:
    ...
    def create_rating(self, data: CollaborationRatingCreate, rater_id: int) -> CollaborationRating:
        """创建跨部门评价"""
        # 获取评价人档案
        rater_profile = (
            self.db.query(EngineerProfile).filter(EngineerProfile.user_id == rater_id).first()
        )

        # 获取被评价人档案
        ratee_profile = (
            self.db.query(EngineerProfile).filter(EngineerProfile.user_id == data.ratee_id).first()
        )
    ...
    def get_rating(self, rating_id: int) -> Optional[CollaborationRating]:
        """获取单个评价"""
        return (
            self.db.query(CollaborationRating).filter(CollaborationRating.id == rating_id).first()
        )

    def get_ratings_received(
        self, user_id: int, period_id: Optional[int] = None, limit: int = 20, offset: int = 0
    ) -> Tuple[List[CollaborationRating], int]:
        """获取收到的评价"""
        query = self.db.query(CollaborationRating).filter(CollaborationRating.ratee_id == user_id)
        ...
        total = query.count()
        items = (
            query.order_by(desc(CollaborationRating.created_at)).offset(offset).limit(limit).all()
        )
    ...
    def get_ratings_given(
        ...
        """获取给出的评价"""
        query = self.db.query(CollaborationRating).filter(CollaborationRating.rater_id == user_id)
    ...
    def get_collaboration_matrix(self, period_id: int) -> Dict[str, Any]:
        ...
        ratings = (
            self.db.query(CollaborationRating)
            .filter(CollaborationRating.period_id == period_id)
            .all()
        )
    ...
    def get_pending_ratings(self, user_id: int, period_id: int) -> List[Dict[str, Any]]:
        """获取待评价的工程师列表"""
        # 获取评价人档案
        rater_profile = (
            self.db.query(EngineerProfile).filter(EngineerProfile.user_id == user_id).first()
        )
        ...
        # 获取可评价的岗位类型
        valid_ratee_types = [
            pair[1] for pair in self.VALID_RATING_PAIRS if pair[0] == rater_profile.job_type
        ]

        # 获取已评价的用户
        rated_users = (
            self.db.query(CollaborationRating.ratee_id)
            .filter(
                CollaborationRating.period_id == period_id, CollaborationRating.rater_id == user_id
            )
            .all()
        )
        ...
        # 获取待评价的工程师
        pending = (
            self.db.query(EngineerProfile, User)
    ...
    def get_collaboration_stats(
        self, user_id: int, period_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """获取协作统计"""
        query = self.db.query(CollaborationRating).filter(CollaborationRating.ratee_id == user_id)
        ...
        if not ratings:
            return {"total_ratings": 0, "avg_score": 0, "by_dimension": {}}
        ...
        return {
            ...
            "avg_score": round(sum(float(r.total_score or 0) for r in ratings) / len(ratings), 2),
            "by_dimension": {
                "communication": (
                    round(sum(comm_scores) / len(comm_scores), 2) if comm_scores else 0
                ),
    ...
    def _group_by_rater_type(self, ratings: List[CollaborationRating]) -> Dict[str, Dict[str, Any]]:
        """按评价人岗位类型分组统计"""
        grouped = {}
        for rating in ratings:
            rater_type = rating.rater_job_type
        ...
        for rater_type in grouped:
            ...
            grouped[rater_type]["avg_score"] = (
                round(grouped[rater_type]["total_score"] / count, 2) if count > 0 else 0
            )

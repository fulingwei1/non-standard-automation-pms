...
class CostDashboardService:
    ...
    def get_cost_overview(self) -> Dict[str, Any]:
        ...
        total_projects = (
            ...
            .filter(Project.stage.notin_(["S0", "S9"]))  # 排除未启动和已关闭
            .count()
        )
        ...
        month_cost_financial = (
            self.db.query(func.sum(FinancialProjectCost.amount).label("total"))
            .filter(
                and_(
        ...
        month_actual_cost = float(month_cost_project.total or 0) + float(
            month_cost_financial.total or 0
        )
    ...
    def get_top_projects(self, limit: int = 10) -> Dict[str, Any]:
        ...
        projects = (
            self.db.query(Project)
            .filter(and_(Project.stage.notin_(["S0", "S9"]), Project.budget_amount > 0))
            .all()
        )
        ...
        top_overrun_projects = sorted(
            ...
            key=lambda x: x["cost_variance_pct"],
            reverse=True,
        )[:limit]
    ...
    def get_project_cost_dashboard(self, project_id: int) -> Dict[str, Any]:
        ...
        cost_breakdown = [
            {
                ...
                "percentage": round(
                    (amount / total_breakdown * 100) if total_breakdown > 0 else 0, 2
                ),
            }
        ...
        for i in range(11, -1, -1):
            ...
            month_actual = float(month_cost_project.total or 0) + float(
                month_cost_financial.total or 0
            )
        ...
        received_amount_data = (
            self.db.query(func.sum(ProjectPaymentPlan.actual_amount).label("total"))
            .filter(
                and_(
        ...
        # 已开票
        invoiced_amount_data = (
            self.db.query(func.sum(ProjectPaymentPlan.invoice_amount).label("total"))

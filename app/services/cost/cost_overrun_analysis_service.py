...
class CostOverrunAnalysisService:
    ...
    def analyze_reasons(
        ...
        reason_stats = defaultdict(
            lambda: {"count": 0, "total_overrun": Decimal("0"), "projects": []}
        )
        ...
        for project in projects:
            ...
            if overrun_analysis["has_overrun"]:
                ...
                for reason in overrun_analysis.get("reasons", []):
                    ...
                    reason_stats[reason]["total_overrun"] += Decimal(
                        str(overrun_analysis["overrun_amount"])
                    )
        ...
        reason_list = [
            {
                ...
                "average_overrun": (
                    float(stats["total_overrun"] / stats["count"]) if stats["count"] > 0 else 0
                ),
    ...
    def analyze_accountability(
        ...
        person_stats = defaultdict(
            lambda: {"overrun_count": 0, "total_overrun": Decimal("0"), "reasons": defaultdict(int)}
        )

        # 按部门归责
        defaultdict(lambda: {"overrun_count": 0, "total_overrun": Decimal("0"), "people": []})
        ...
        for project_data in projects:
            ...
            if project.opportunity_id:
                ...
                if opp and opp.owner_id:
                    ...
                    person_stats[opp.owner_id]["total_overrun"] += Decimal(
                        str(project_data["overrun_amount"])
                    )
            ...
            if project.salesperson_id:
                ...
                person_stats[project.salesperson_id]["total_overrun"] += Decimal(
                    str(project_data["overrun_amount"])
                )
            ...
            if project.pm_id:
                ...
                person_stats[project.pm_id]["total_overrun"] += Decimal(
                    str(project_data["overrun_amount"])
                )
            ...
            person_list.append(
                {
                    ...
                    "person_name": (
                        person.real_name
                        if person and person.real_name
                        else (person.username if person else f"User_{person_id}")
                    ),
    ...
    def analyze_impact(
        ...
        return {
            ...
            "summary": {
                ...
                "overrun_ratio": (
                    float(total_overrun / total_contract_amount * 100)
                    if total_contract_amount > 0
                    else 0
                ),
    ...
    def _calculate_outsourcing_cost(self, project_id: int) -> Decimal:
        ...
        outsourcing_orders = (
            self.db.query(OutsourcingOrder).filter(OutsourcingOrder.project_id == project_id).all()
        )

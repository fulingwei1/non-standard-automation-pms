...
class GLM5CostPredictor:
    ...
    def predict_eac(
        self, project_data: Dict, evm_history: List[Dict], additional_context: Optional[Dict] = None
    ) -> Dict:
        """
        使用GLM-5预测完工估算（EAC）
        ...
        """
        # 构建AI提示词
        prompt = self._build_eac_prediction_prompt(project_data, evm_history, additional_context)
    ...
    def analyze_cost_risks(
        self, project_data: Dict, evm_history: List[Dict], current_evm: Dict
    ) -> Dict:
        """
        分析成本超支风险
        ...
        Returns:
            风险分析结果
        """
        prompt = self._build_risk_analysis_prompt(project_data, evm_history, current_evm)
    ...
    def generate_optimization_suggestions(
        self, project_data: Dict, prediction_result: Dict, risk_analysis: Dict
    ) -> List[Dict]:
        """
        生成成本优化建议
        ...
        Returns:
            优化建议列表
        """
        prompt = self._build_optimization_prompt(project_data, prediction_result, risk_analysis)
    ...
    def _build_eac_prediction_prompt(
        self, project_data: Dict, evm_history: List[Dict], additional_context: Optional[Dict]
    ) -> str:
        """构建EAC预测提示词"""
...
"""
    ...
    def _call_glm5_api(self, prompt: str, temperature: float = 0.5, max_tokens: int = 4000) -> str:
        """
        调用GLM-5 API
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "你是一位资深的项目成本管理和风险分析专家。"},
    ...
    def _parse_eac_prediction(self, ai_response: str, project_data: Dict) -> Dict:
        """解析EAC预测响应"""
        try:
            # 尝试提取JSON内容
            ...
            return {
                ...
                "eac_lower_bound": float(
                    result.get("eac_lower_bound", predicted_eac * Decimal("0.95"))
                ),
                "eac_upper_bound": float(
                    result.get("eac_upper_bound", predicted_eac * Decimal("1.05"))
                ),
...
class CostPredictionService:
    ...
    def create_prediction(
        ...
        # 获取最新的EVM数据
        latest_evm = (
            self.db.query(EarnedValueData)
            .filter(EarnedValueData.project_id == project_id)
        ...
        if use_ai and self.ai_predictor:
            prediction_result = self.ai_predictor.predict_eac(project_data, evm_history_data)
        ...
        prediction = CostPrediction(
            ...
            expected_overrun_amount=Decimal(str(prediction_result["predicted_eac"]))
            - latest_evm.budget_at_completion,
            overrun_percentage=(
                (
                    (
                        Decimal(str(prediction_result["predicted_eac"]))
                        - latest_evm.budget_at_completion
                    )
                    / latest_evm.budget_at_completion
                    * Decimal("100")
                )
                if latest_evm.budget_at_completion > 0
                else Decimal("0")
            ),
    ...
    def _traditional_eac_prediction(self, latest_evm: EarnedValueData) -> Dict:
        """传统EAC预测方法"""
        cpi = (
            latest_evm.cost_performance_index
            if latest_evm.cost_performance_index is not None
            else Decimal("1")
        )
        bac = latest_evm.budget_at_completion
    ...
    def _traditional_risk_analysis(
        self, latest_evm: EarnedValueData, evm_history: List[EarnedValueData]
    ) -> Dict:
        """传统风险分析方法"""
        cpi = float(latest_evm.cost_performance_index or 1)

...
class CostAnalysisService:
    ...
    def predict_project_cost(
        self, project_id: int, based_on_history: bool = True
    ) -> Dict[str, Any]:
        """
        预测项目成本（基于历史工时数据）
        ...
        tasks = (
            self.db.query(Task)
            .filter(Task.project_id == project_id, Task.status.in_(["PENDING", "IN_PROGRESS"]))
            .all()
        )
    ...
    def check_cost_overrun_alerts(self, project_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        检查成本超支预警
    ...
    def compare_project_costs(self, project_ids: List[int]) -> Dict[str, Any]:
        """
        对比不同项目的工时成本
        ...
        # 计算平均值
        avg_cost = (
            sum(d["total_cost"] for d in comparison_data) / len(comparison_data)
            if comparison_data
            else 0
        )
        avg_hours = (
            sum(d["total_hours"] for d in comparison_data) / len(comparison_data)
            if comparison_data
            else 0
        )
    ...
    def analyze_cost_trend(self, project_id: int, months: int = 6) -> Dict[str, Any]:
        """
        分析项目成本趋势
        ...
        for ts in timesheets:
            ...
            if month_key in monthly_costs:
                ...
                monthly_costs[month_key]["personnel_count"] = len(
                    set(t.user_id for t in timesheets if t.work_date.strftime("%Y-%m") == month_key)
                )

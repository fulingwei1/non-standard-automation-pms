...
def check_user_role_permission(
    config: Optional[CultureWallConfig], user_role_codes: List[str]
) -> bool:
    """
    检查用户角色权限
    ...
    # 检查用户的任何一个角色是否在可见角色列表中
    return any(role in config.visible_roles for role in user_role_codes)
...
def format_content(
    content: CultureWallContent, read_records: Dict[int, bool]
...
def get_personal_goals(
    db: Session, user_id: int, today: date, content_types_config: Dict[str, Any]
) -> List[PersonalGoal]:
    """
    获取个人目标
    ...
    monthly_goals = (
        ...
        .order_by(desc(PersonalGoal.created_at))
        .all()
    )
...
def get_notifications(
    db: Session, user_id: int, content_types_config: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
    获取系统通知

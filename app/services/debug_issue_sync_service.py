...
class DebugIssueSyncService:
    ...
    def sync_mechanical_debug_issue(
        self, issue_id: int, force_update: bool = False
        ...
        existing_debug = (
            ...
            .filter(MechanicalDebugIssue.issue_code == issue.issue_no)
            .first()
        )
    ...
    def sync_test_bug_record(
        ...
        # 检查是否已同步（通过问题编号关联）
        existing_bug = (
            self.db.query(TestBugRecord).filter(TestBugRecord.bug_code == issue.issue_no).first()
        )
    ...
    def sync_issue(self, issue_id: int, force_update: bool = False) -> Dict[str, Any]:
        """
        同步问题（自动判断类型）
    ...
    def sync_all_project_issues(
        ...
        query = self.db.query(Issue).filter(
            Issue.category == "PROJECT", Issue.issue_type.in_(["DEFECT", "BUG"])
        )
        ...
        for issue in issues:
            try:
                ...
                stats["error_count"] += 1
                stats["errors"].append(
                    {"issue_id": issue.id, "issue_no": issue.issue_no, "error": str(e)}
                )

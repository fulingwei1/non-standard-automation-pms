...
class DelayRootCauseService:
            ...
            reason_list.append(
                {
                    ...
                    "average_delay_days": (
                        round(stats["total_delay_days"] / stats["count"], 1)
                        if stats["count"] > 0
                        else 0
                    ),
    ...
    def analyze_impact(
        ...
        for project in projects:
            ...
            if self._is_project_delayed(project):
                ...
                if project.contract_amount:
                    daily_cost = Decimal(str(project.contract_amount)) * Decimal(
                        "0.001"
                    )  # 假设每天0.1%成本
                ...
                if project.contract_amount:
                    revenue_impact += (
                        Decimal(str(project.contract_amount))
                        * Decimal("0.0005")
                        * Decimal(str(delay_days))
                    )
                ...
                affected_projects.append(
                    {
                        ...
                        "delay_days": delay_days,
                        "cost_impact": (
                            float(daily_cost * Decimal(str(delay_days)))
                            if project.contract_amount
                            else 0
                        ),
                    }
        ...
        return {
            ...
            "cost_impact": {"total": float(cost_impact), "description": "延期导致的额外成本"},
            "revenue_impact": {
    ...
    def analyze_trends(self, months: int = 12) -> Dict[str, Any]:
        ...
        monthly_stats = defaultdict(
            lambda: {"total_tasks": 0, "delayed_tasks": 0, "delay_rate": 0.0, "total_delay_days": 0}
        )

        # 查询任务
        tasks = (
            self.db.query(Task)
            .filter(Task.plan_start >= start_date, Task.plan_start <= end_date)
            .all()
        )
        ...
        for month in sorted(monthly_stats.keys()):
            ...
            stats["delay_rate"] = (
                round(stats["delayed_tasks"] / stats["total_tasks"] * 100, 2)
                if stats["total_tasks"] > 0
                else 0
            )
            trend_data.append({"month": month, **stats})
        ...
        return {
            ...
            "summary": {
                "average_delay_rate": (
                    round(sum(d["delay_rate"] for d in trend_data) / len(trend_data), 2)
                    if trend_data
                    else 0
                ),

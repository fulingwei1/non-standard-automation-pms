...
class DeliveryValidationService:
    ...
    def get_max_material_lead_time(
        ...
        for item in items:
            ...
            lead_time, remark = DeliveryValidationService.get_material_lead_time(
                db, material_code=item.material_code, material_type=item.item_type
            )
        ...
        if critical_items:
            ...
            max_days = (
                max([m["lead_time_days"] for m in material_lead_times])
                if material_lead_times
                else 0
            )
    ...
    def estimate_project_cycle(
        ...
        for stage, ratio in stage_ratios.items():
            ...
            stage_details.append(
                {"stage": stage, "stage_name": stage_name, "days": stage_days, "ratio": ratio}
            )
    ...
    def validate_delivery_date(
        db: Session, quote: Quote, version: QuoteVersion, items: List[QuoteItem]
    ) -> Dict[str, Any]:
        """
        校验报价交期的合理性
        ...
        if not version.lead_time_days or version.lead_time_days <= 0:
            errors.append(
                {"check": "交期填写", "status": "ERROR", "message": "请填写项目交期（天）"}
            )
        else:
            checks.append(
                {
        ...
        if version.lead_time_days:
            diff_ratio = (
                (version.lead_time_days - estimated_days) / estimated_days
                if estimated_days > 0
                else 0
            )
    ...
    def _get_suggestions(
        quoted_days: Optional[int], material_days: int, estimated_days: int
    ) -> List[str]:
        """生成优化建议"""
        suggestions = []
        ...
        if quoted_days:
            if quoted_days < material_days:
                suggestions.append(
                    f"建议将交期调整为至少{material_days + 7}天，以覆盖最长物料交期并预留缓冲"
                )
            ...
            suggestions.append(
                f"建议交期: {max(material_days, estimated_days)}天（基于物料和项目周期）"
            )

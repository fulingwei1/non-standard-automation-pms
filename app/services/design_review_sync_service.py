...
class DesignReviewSyncService:
    ...
    def sync_from_technical_review(
        self, technical_review_id: int, force_update: bool = False
        ...
        # 获取技术评审
        tech_review = (
            self.db.query(TechnicalReview).filter(TechnicalReview.id == technical_review_id).first()
        )
        ...
        existing_review = (
            ...
            .filter(
                ...
                DesignReview.review_date
                == (
                    tech_review.actual_date.date()
                    if tech_review.actual_date
                    else tech_review.scheduled_date.date()
                ),
            )
            .first()
        )
        ...
        # 获取项目信息
        project = self.db.query(Project).filter(Project.id == tech_review.project_id).first()
    ...
    def sync_all_completed_reviews(
        ...
        if start_date:
            query = query.filter(
                TechnicalReview.actual_date >= datetime.combine(start_date, datetime.min.time())
            )
        if end_date:
            query = query.filter(
                TechnicalReview.actual_date <= datetime.combine(end_date, datetime.max.time())
            )
        ...
        for review in reviews:
            try:
                ...
                stats["error_count"] += 1
                stats["errors"].append(
                    {"review_id": review.id, "review_no": review.review_no, "error": str(e)}
                )

...
def add_document_header(
    doc: Document, report_title: str, period_info: str, rhythm_level: Optional[str] = None
) -> None:
    """
    添加文档标题和报告信息
    """
...
def add_summary_section(doc: Document, summary: Dict[str, Any]) -> None:
    """
    添加执行摘要部分
    """
    if not DOCX_AVAILABLE:
...
def add_comparison_section(doc: Document, comparison_data: Dict[str, Any]) -> None:
    """
    添加与上月对比部分（月度报告独有）
    """
    if not DOCX_AVAILABLE:
    ...
    for idx, (label, comp_data) in enumerate(comparisons, 1):
        ...
        if isinstance(comp_data, dict):
            change_value = (
                comp_data.get("change", 0)
                if label != "完成率"
                else comp_data.get("change_value", 0)
            )
            if change_value > 0:
...
def add_level_statistics_section(
    doc: Document, by_level: Dict[str, Dict[str, Any]], format_rhythm_level_func
) -> None:
    """
    添加按层级统计部分
    """
...
def add_action_items_section(doc: Document, action_summary: Dict[str, Any]) -> None:
    """
    添加行动项统计部分
    """
    if not DOCX_AVAILABLE:
...
def add_key_decisions_section(
    doc: Document, key_decisions: list, period_type: str = "本月"
) -> None:
    """
    添加关键决策部分
    """
    ...
    if key_decisions:
        for idx, decision in enumerate(key_decisions[:20], 1):  # 最多显示20条
            ...
            decision_para = doc.add_paragraph(f"{idx}. {decision_text}", style="List Number")
...
def add_strategic_structures_section(doc: Document, strategic_structures: list) -> None:
    """
    添加战略结构部分（年度报告特有）
    """
    if not DOCX_AVAILABLE:
...
def add_meetings_list_section(
    ...
    for idx, meeting in enumerate(meetings[:50], 1):  # 最多显示50条
        ...
        row_cells[1].text = (
            meeting.get("meeting_date", "")[:10] if meeting.get("meeting_date") else ""
        )

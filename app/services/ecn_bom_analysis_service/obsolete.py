...
def check_obsolete_material_risk(service: "EcnBomAnalysisService", ecn_id: int) -> Dict[str, Any]:
    """
    检查呆滞料风险
    ...
    if not affected_materials:
        return {"ecn_id": ecn_id, "has_obsolete_risk": False, "message": "没有删除或替换的物料"}
    ...
    for affected_mat in affected_materials:
        ...
        material = (
            service.db.query(Material).filter(Material.id == affected_mat.material_id).first()
        )
        ...
        # 检查是否有在途采购订单
        po_items = (
            service.db.query(PurchaseOrderItem)
            .join(PurchaseOrder)
            .filter(
        ...
        in_transit_qty = sum(
            Decimal(item.quantity or 0) - Decimal(item.received_qty or 0) for item in po_items
        )
...
def calculate_obsolete_risk_level(obsolete_qty: Decimal, obsolete_cost: Decimal) -> str:
    """计算呆滞料风险级别"""
    # 风险级别判定规则

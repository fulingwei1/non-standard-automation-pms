...
def get_impact_description(affected_mat: EcnAffectedMaterial) -> str:
    """获取影响描述"""
    change_type_map = {"ADD": "新增", "DELETE": "删除", "UPDATE": "修改", "REPLACE": "替换"}
    ...
    elif getattr(affected_mat, "old_specification", None) and getattr(
        affected_mat, "new_specification", None
    ):
        desc += "（规格变更）"
...
def save_bom_impact(
    ...
    # 检查是否已存在
    existing = (
        service.db.query(EcnBomImpact)
        .filter(EcnBomImpact.ecn_id == ecn_id, EcnBomImpact.bom_version_id == bom_version_id)
        .first()
    )

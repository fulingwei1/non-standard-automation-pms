...
class DimensionConfigService:
    ...
    def _get_department_config(
        self, job_type: str, job_level: Optional[str], effective_date: date, department_id: int
    ) -> Optional[EngineerDimensionConfig]:
        """获取部门级别配置"""
        ...
        if job_level:
            dept_config = (
                dept_query.filter(EngineerDimensionConfig.job_level == job_level)
                .order_by(desc(EngineerDimensionConfig.effective_date))
                .first()
            )
            if dept_config:
                return dept_config
        ...
        # 部门级别通用配置
        return (
            dept_query.filter(EngineerDimensionConfig.job_level.is_(None))
            .order_by(desc(EngineerDimensionConfig.effective_date))
            .first()
        )

    def _get_global_config(
        self, job_type: str, job_level: Optional[str], effective_date: date
    ) -> Optional[EngineerDimensionConfig]:
        """获取全局配置"""
        ...
        if job_level:
            # 先尝试匹配精确级别
            config = (
                query.filter(EngineerDimensionConfig.job_level == job_level)
                .order_by(desc(EngineerDimensionConfig.effective_date))
                .first()
            )
        ...
        return (
            query.filter(EngineerDimensionConfig.job_level.is_(None))
            .order_by(desc(EngineerDimensionConfig.effective_date))
            .first()
        )
    ...
    def create_config(
        ...
        # 验证权重总和为100
        total_weight = (
            data.technical_weight
            + data.execution_weight
            + data.cost_quality_weight
            + data.knowledge_weight
        ...
        config = EngineerDimensionConfig(
            ...
            approval_status=(
                "APPROVED" if not require_approval or department_id is None else "PENDING"
            ),
        )
        save_obj(self.db, config)
        return config

    def _validate_department_manager_permission(self, department_id: int, operator_id: int) -> None:
        """验证部门经理权限"""
    ...
    def get_department_configs(self, manager_id: int) -> Dict[str, Any]:
        ...
        if not manager or not manager.employee_id:
            return {"is_manager": False, "department_id": None, "configs": []}
        ...
        # 获取部门内所有工程师的岗位类型
        employees = (
            self.db.query(Employee).filter(Employee.department == dept.id, Employee.is_active).all()
        )
        ...
        # 获取部门级别配置和全局配置
        dept_configs = self.list_configs(department_id=dept.id, include_global=False)
        global_configs = self.list_configs(include_global=True)

        configs = self._build_config_list(job_type_distribution, dept_configs, global_configs)
    ...
    def _analyze_job_type_distribution(self, profiles: List) -> Dict[str, Dict]:
        """分析岗位类型分布"""
        distribution = {}
        for profile in profiles:
            job_type = profile.job_type
            if job_type not in distribution:
                distribution[job_type] = {"count": 0, "levels": {}}
    ...
    def _build_config_list(
        self, job_type_distribution: Dict, dept_configs: List, global_configs: List
    ) -> List[Dict]:
        """构建配置列表"""
        job_type_names = {
            "mechanical": "机械工程师",
        ...
        for job_type, dist in job_type_distribution.items():
            dept_config = next((c for c in dept_configs if c.job_type == job_type), None)
            global_config = next(
                (c for c in global_configs if c.job_type == job_type and c.is_global), None
            )
    ...
    def approve_config(
        ...
        """
        config = (
            self.db.query(EngineerDimensionConfig)
            .filter(EngineerDimensionConfig.id == config_id)
            .first()
        )
    ...
    def _validate_admin_permission(self, approver_id: int) -> None:
        ...
        if not is_admin:
            # 检查是否有管理员角色
            admin_roles = self.db.query(UserRole).filter(UserRole.user_id == approver_id).all()
    ...
    def get_pending_approvals(self) -> List[EngineerDimensionConfig]:
        ...
        return (
            ...
            .order_by(desc(EngineerDimensionConfig.created_at))
            .all()
        )

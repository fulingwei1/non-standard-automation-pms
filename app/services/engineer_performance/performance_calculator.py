...
class PerformanceCalculator:
    ...
    def _calculate_electrical_score(
        ...
        # PLC模块贡献
        modules = (
            self.db.query(PlcModuleLibrary)
            .filter(
    ...
    def _calculate_solution_score(
        ...
        if not solutions:
            ...
            if total_won > 0:
                ...
                weighted_won = (
                    high_value_won * Decimal("1.2")
                    + low_value_won * Decimal("0.8")
                    + normal_value_won
                )
            ...
            win_rate_score = min(
                Decimal(str(weighted_win_rate)) / 40 * 100, 120
            )  # 目标40%，使用加权值
                ...
                if tickets:
                    ...
                    for ticket in tickets:
                        ...
                        if score >= 4.5:
                            # 检查对应的方案是否中标
                            solution = next(
                                (s for s in solutions if s.ticket_id == ticket.id), None
                            )
            ...
            if high_quality_unwon > 0:
                compensation = (
                    Decimal(str(high_quality_unwon)) * Decimal("0.5") * Decimal("10")
                )  # 每个补偿5分
        ...
        # 3. 项目执行维度（20%权重）
        # 方案交付及时率
        on_time_solutions = sum(1 for s in solutions if s.ticket_id and s.created_at)
        ...
        # 方案模板贡献
        templates = (
            self.db.query(PresaleSolutionTemplate)
            .filter(
    ...
    def _get_collaboration_avg(self, engineer_id: int, period_id: int) -> Decimal:
        ...
        total = sum(
            (r.communication_score or 0)
            + (r.response_score or 0)
            + (r.delivery_score or 0)
            + (r.interface_score or 0)
            for r in ratings
        )

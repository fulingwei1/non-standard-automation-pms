...
class EngineerSchedulingService:
    ...
    def _get_engineer_capacity(self, engineer_id: int) -> Optional[EngineerCapacity]:
        """安全获取工程师能力模型，兼容缺表场景。"""
        try:
            return self.db.query(EngineerCapacity)\
    ...
    def extract_engineer_capacity(self, engineer_id: int) -> Dict[str, Any]:
        ...
        # 1. 查询历史任务分配
        tasks = self._query_task_assignments(
    ...
    def save_or_update_capacity(self, engineer_id: int) -> EngineerCapacity:
        ...
        # 查询现有记录
        capacity = self._get_engineer_capacity(engineer_id)
    ...
    def analyze_engineer_workload(
        ...
        unique_projects = len(set(t.project_id for t in tasks if t.project_id is not None))
    ...
    def generate_workload_warnings(
        ...
        if project_id:
            # 查询项目相关的所有任务分配
            project_tasks = self._query_task_assignments(

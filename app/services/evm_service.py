...
class EVMCalculator:
    ...
    def calculate_estimate_at_completion(
        cls, bac: Decimal, ev: Decimal, ac: Decimal, cpi: Optional[Decimal] = None
    ) -> Decimal:
        """
    ...
    def calculate_to_complete_performance_index(
        cls, bac: Decimal, ev: Decimal, ac: Decimal, eac: Optional[Decimal] = None
    ) -> Optional[Decimal]:
        """
    ...
    @classmethod
    def calculate_percent_complete(cls, value: Decimal, bac: Decimal) -> Optional[Decimal]:
        """
        计算完成百分比
    ...
    def calculate_all_metrics(
        cls, pv: Decimal, ev: Decimal, ac: Decimal, bac: Decimal
...
class EVMService:
    ...
    def get_latest_evm_data(
        self, project_id: int, period_type: Optional[str] = None
    ) -> Optional[EarnedValueData]:
        """获取项目最新的EVM数据"""
        query = self.db.query(EarnedValueData).filter(EarnedValueData.project_id == project_id)
    ...
    def get_evm_trend(
        self, project_id: int, period_type: str = "MONTH", limit: Optional[int] = None

...
class InformationGapAnalysisService:
    ...
    def analyze_missing(
        self, entity_type: str, entity_id: Optional[int] = None  # LEAD/OPPORTUNITY/QUOTE
    ) -> Dict[str, Any]:
        """信息缺失分析"""
        missing_fields = []
    ...
    def analyze_impact(
        ...
        high_quality_conversion = (
            sum(1 for l in high_quality_leads if l.status == "CONVERTED")
            / len(high_quality_leads)
            * 100
            if high_quality_leads
            else 0
        )
        low_quality_conversion = (
            sum(1 for l in low_quality_leads if l.status == "CONVERTED")
            / len(low_quality_leads)
            * 100
    ...
    def get_quality_score(self, entity_type: str, entity_id: int) -> Dict[str, Any]:
        """获取信息质量评分"""
    ...
    def _analyze_batch_missing(self, entity_type: str, entities: List) -> Dict[str, Any]:
        """批量分析信息缺失"""
        total = len(entities)
        quality_distribution = {"high": 0, "medium": 0, "low": 0}  # >=80  # 60-79  # <60
        ...
        return {
            ...
            "common_missing_fields": [
                ...
                for field, count in sorted(
                    common_missing.items(), key=lambda x: x[1], reverse=True
                )[:10]
            ],
        }

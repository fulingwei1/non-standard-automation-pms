...
class InventoryAnalysisService:
    ...
    def get_turnover_rate_data(
        ...
        consumption_query = (
            db.query(
                func.sum(GoodsReceiptItem.qualified_qty * GoodsReceiptItem.inspect_qty / 100).label(
                    "qty"
                )
            )
        ...
        # 使用平均价格估算消耗金额
        avg_price = (
            total_inventory_value / sum(float(m.current_stock or 0) for m in materials)
            if materials
            else 0
        )
        ...
        # 计算周转率
        turnover_rate = (
            (total_consumption / total_inventory_value * 100) if total_inventory_value > 0 else 0
        )
            ...
            category_list.append(
                {
                    ...
                    "value_percentage": round(
                        (
                            (data["value"] / total_inventory_value * 100)
    ...
    def get_stale_materials_data(
        db: Session, threshold_days: int = 90, category_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        获取呆滞物料数据
        ...
        query = (
            ...
            .outerjoin(MaterialCategory, Material.category_id == MaterialCategory.id)
            .filter(Material.current_stock > 0, Material.is_active)
        )
        ...
        age_dist_list = [
            {"age_range": k, "value": round(v, 2)} for k, v in age_distribution.items()
        ]
    ...
    def get_safety_stock_compliance_data(
        ...
        stats["compliant_rate"] = round(
            (
                (stats["compliant"] / materials_with_safety_stock * 100)
                if materials_with_safety_stock > 0
                else 0
            ),
            2,
        )
    ...
    @staticmethod
    def get_abc_analysis_data(db: Session, start_date: date, end_date: date) -> Dict[str, Any]:
        """
        获取物料ABC分类数据
        ...
        query = (
            ...
            .outerjoin(Material, Material.material_code == PurchaseOrderItem.material_code)
            .outerjoin(MaterialCategory, Material.category_id == MaterialCategory.id)
            .filter(
        ...
        if not results:
            return {
                "abc_materials": [],
                "abc_summary": {
                    "A": {"count": 0, "amount_percent": 0, "count_percent": 0},
        ...
        for cls in ["A", "B", "C"]:
            if total_count > 0:
                abc_summary[cls]["count_percent"] = round(
                    abc_summary[cls]["count"] / total_count * 100, 2
                )
    ...
    @staticmethod
    def get_cost_occupancy_data(db: Session, category_id: Optional[int] = None) -> Dict[str, Any]:
        """
        获取库存成本占用数据
        """
        # 按分类统计库存价值
        query = (
            ...
            .filter(Material.is_active)
            .group_by(MaterialCategory.id, MaterialCategory.category_name)
        )
        ...
        top_materials_query = (
            ...
            .outerjoin(MaterialCategory, Material.category_id == MaterialCategory.id)
            .filter(Material.is_active, Material.current_stock > 0)
            .order_by((Material.current_stock * Material.standard_price).desc())
            .limit(50)
        )

...
def check_existing_snapshot(db: Session, snapshot_date: date) -> bool:
    """检查指定日期是否已存在快照"""
    existing = (
        db.query(IssueStatisticsSnapshot)
        .filter(IssueStatisticsSnapshot.snapshot_date == snapshot_date)
        .first()
    )
...
def count_blocking_and_overdue_issues(db: Session, today: date) -> Dict[str, int]:
    """统计阻塞和逾期问题数量"""
    blocking_count = (
        db.query(Issue).filter(Issue.is_blocking, Issue.status.in_(["OPEN", "PROCESSING"])).count()
    )
...
def count_today_issues(db: Session, today: date) -> Dict[str, int]:
    ...
    new_today = (
        db.query(Issue).filter(Issue.created_at >= today_start, Issue.status != "DELETED").count()
    )

    resolved_today = (
        db.query(Issue)
        .filter(Issue.resolved_at >= today_start, Issue.status.in_(["RESOLVED", "CLOSED"]))
        .count()
    )

    closed_today = (
        db.query(Issue).filter(Issue.status == "CLOSED", Issue.updated_at >= today_start).count()
    )

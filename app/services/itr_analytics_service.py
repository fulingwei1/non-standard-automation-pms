...
def analyze_resolution_time(
    ...
    ticket_query = db.query(ServiceTicket).filter(
        ServiceTicket.status == "CLOSED", ServiceTicket.resolved_time.isnot(None)
    )
...
def analyze_satisfaction_trend(
    ...
    query = db.query(CustomerSatisfaction).filter(
        CustomerSatisfaction.status == "COMPLETED", CustomerSatisfaction.overall_score.isnot(None)
    )
        ...
        bottlenecks.append(
            {
                ...
                "severity": (
                    "HIGH" if avg_pending_time > 24 else "MEDIUM" if avg_pending_time > 8 else "LOW"
                ),
            }
        )
        ...
        bottlenecks.append(
            {
                ...
                "severity": (
                    "HIGH"
                    if avg_resolve_time > 72
                    else "MEDIUM" if avg_resolve_time > 24 else "LOW"
                ),
            }
        )
        ...
        bottlenecks.append(
            {
                ...
                "severity": (
                    "HIGH" if avg_close_time > 48 else "MEDIUM" if avg_close_time > 24 else "LOW"
                ),
            }
        )
...
def analyze_sla_performance(
    ...
    for stats in by_policy.values():
        stats["response_rate"] = (
            (stats["response_on_time"] / stats["total"] * 100) if stats["total"] > 0 else 0
        )
        stats["resolve_rate"] = (
            (stats["resolve_on_time"] / stats["total"] * 100) if stats["total"] > 0 else 0
        )

...
class KnowledgeContributionService:
    ...
    def create_contribution(
        self, data: KnowledgeContributionCreate, contributor_id: int
    ) -> KnowledgeContribution:
        """创建知识贡献"""
    ...
    def get_contribution(self, contribution_id: int) -> Optional[KnowledgeContribution]:
        ...
        return (
            ...
            .filter(KnowledgeContribution.id == contribution_id)
            .first()
        )

    def update_contribution(
        self, contribution_id: int, data: KnowledgeContributionUpdate, user_id: int
    ...
    def approve_contribution(
        self, contribution_id: int, approver_id: int, approved: bool = True
    ) -> KnowledgeContribution:
        """审核知识贡献"""
        contribution = self.get_contribution(contribution_id)
        if not contribution:
    ...
    def list_contributions(
        ...
        total = query.count()
        items = (
            query.order_by(desc(KnowledgeContribution.created_at)).offset(offset).limit(limit).all()
        )
    ...
    def record_reuse(self, data: KnowledgeReuseCreate, user_id: int) -> KnowledgeReuseLog:
        """记录知识复用"""
        ...
        if data.rating:
            ...
            contribution.rating_score = Decimal(
                str(round((float(total_rating) + data.rating) / contribution.rating_count, 2))
            )
    ...
    def get_reuse_logs(
        self, contribution_id: int, limit: int = 20, offset: int = 0
        ...
        total = query.count()
        items = query.order_by(desc(KnowledgeReuseLog.created_at)).offset(offset).limit(limit).all()
    ...
    def get_contribution_ranking(
        ...
        ).filter(KnowledgeContribution.status == "approved")
        ...
        results = (
            query.group_by(KnowledgeContribution.contributor_id)
            .order_by(desc("count"), desc("total_reuse"))
            .limit(limit)
            .all()
        )
    ...
    def get_contributor_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        if not contributions:
            return {"total_contributions": 0, "total_reuse": 0, "avg_rating": 0, "by_type": {}}
    ...
    def list_code_modules(
        ...
        """获取代码模块列表"""
        query = self.db.query(CodeModule).filter(CodeModule.status == "active")
        ...
        total = query.count()
        items = (
            query.order_by(desc(CodeModule.reuse_count), desc(CodeModule.created_at))
            .offset(offset)
            .limit(limit)
            .all()
        )
    ...
    def list_plc_modules(
        ...
        """获取PLC模块列表"""
        query = self.db.query(PlcModuleLibrary).filter(PlcModuleLibrary.status == "active")
        ...
        total = query.count()
        items = (
            query.order_by(desc(PlcModuleLibrary.reuse_count), desc(PlcModuleLibrary.created_at))
            .offset(offset)
            .limit(limit)
            .all()
        )

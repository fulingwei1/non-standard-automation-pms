...
class KnowledgeContributionService:
    ...
    def get_contribution_ranking(
        ...
        for i, (contributor_id, count, total_reuse) in enumerate(results, 1):
            ...
            recent_job_type = self.db.query(KnowledgeContribution.job_type).filter(
                KnowledgeContribution.contributor_id == contributor_id,
                KnowledgeContribution.status == 'approved',
                KnowledgeContribution.job_type.isnot(None),
            ).order_by(desc(KnowledgeContribution.created_at)).first()
            ranking.append({
                'rank': i,
                'contributor_id': contributor_id,
                'contributor_name': (user.real_name or user.username) if user else 'Unknown',
                'job_type': recent_job_type[0] if recent_job_type else None,

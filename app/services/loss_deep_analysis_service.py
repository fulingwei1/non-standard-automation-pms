...
class LossDeepAnalysisService:
    ...
    def _analyze_investment_stage(self, projects: List[Project]) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "detailed_design_percentage": (
                    round(stage_stats["detailed_design"] / len(projects) * 100, 2)
                    if projects
                    else 0
                ),
            },
        ...
        return {
            "summary": {
                ...
                "average_hours_per_project": (
                    round(total_hours / len(projects), 1) if projects else 0
                ),
    ...
    def _identify_patterns(self, projects: List[Project]) -> Dict[str, Any]:
        ...
        detailed_design_projects = [
            p for p in projects if self._determine_investment_stage(p) == "detailed_design"
        ]
        ...
        if detailed_design_projects:
            ...
            patterns["detailed_design_loss_patterns"] = [
                ...
                for reason, count in sorted(
                    detailed_reasons.items(), key=lambda x: x[1], reverse=True
                )
            ]

        # 分析销售人员模式
        salesperson_stats = defaultdict(
            lambda: {"total": 0, "lost": 0, "hours": 0.0, "cost": Decimal("0")}
        )
        for project in projects:
            if project.salesperson_id:
    ...
    def analyze_by_stage(
        self, stage: str, start_date: Optional[date] = None, end_date: Optional[date] = None
    ) -> Dict[str, Any]:
        """按投入阶段分析未中标情况"""
        projects = (
        ...
        # 筛选指定阶段的项目
        stage_projects = [p for p in projects if self._determine_investment_stage(p) == stage]

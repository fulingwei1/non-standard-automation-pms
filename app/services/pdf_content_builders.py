...
def build_basic_info_section(
    ...
    # 验收类型中文
    type_map = {"FAT": "出厂验收", "SAT": "现场验收", "FINAL": "终验收"}
    ...
    info_data = [
        ...
        [
            "验收日期",
            order.actual_end_date.strftime("%Y年%m月%d日") if order.actual_end_date else "N/A",
        ],
...
def build_statistics_section(
    order: AcceptanceOrder, db: Session, styles: Dict[str, Any]
) -> List[Any]:
    """
    构建验收项目统计部分
    ...
    items = (
        ...
        .filter(AcceptanceOrderItem.order_id == order.id)
    ...
    stats_table = Table(
        stats_data, colWidths=[3 * cm, 1.5 * cm, 1.5 * cm, 1.5 * cm, 1.5 * cm, 1.5 * cm, 2 * cm]
    )
...
def build_conclusion_section(order: AcceptanceOrder, styles: Dict[str, Any]) -> List[Any]:
    """
    构建验收结论部分
    ...
    result_map = {"PASSED": "通过", "FAILED": "不通过", "CONDITIONAL": "有条件通过"}
...
def build_issues_section(order: AcceptanceOrder, db: Session, styles: Dict[str, Any]) -> List[Any]:
    """
    构建验收问题部分
    ...
    issues = db.query(AcceptanceIssue).filter(AcceptanceIssue.order_id == order.id).all()
    ...
    if issues:
        ...
        if len(issues) > 0:
            ...
            for issue in issues[:10]:  # 最多显示10个问题
                severity_map = {"CRITICAL": "严重", "MAJOR": "一般", "MINOR": "轻微"}
                status_map = {
                    "OPEN": "待处理",
                    "PROCESSING": "处理中",
                    "RESOLVED": "已解决",
...
def build_signatures_section(
    ...
    signatures = (
        db.query(AcceptanceSignature).filter(AcceptanceSignature.order_id == order.id).all()
    )
...
def build_footer_section(current_user: Any, styles: Dict[str, Any]) -> List[Any]:
    """
    构建页脚部分

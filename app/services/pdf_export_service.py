...
try:
    ...
    from reportlab.platypus import Image  # noqa: F401
    from reportlab.platypus import PageBreak  # noqa: F401
...
class PDFExportService:
    ...
    def export_quote_to_pdf(
        ...
        info_data = [
            ...
            [
                "报价日期",
                (
                    quote_data.get("created_at", "").strftime("%Y-%m-%d")
            ...
            [
                "有效期至",
                (
                    quote_data.get("valid_until", "").strftime("%Y-%m-%d")
        ...
        item_table_data.append(["", "", "", "", "", "合计：", f"¥{total_price:,.2f}", ""])

        item_table = Table(
            item_table_data,
            colWidths=[0.8 * cm, 4 * cm, 3 * cm, 1 * cm, 1 * cm, 2 * cm, 2 * cm, 2.2 * cm],
        )
        item_table.setStyle(
            TableStyle(
                [
    ...
    def export_contract_to_pdf(
        ...
        info_data = [
            ...
            [
                "签订日期",
                (
                    contract_data.get("signed_date", "").strftime("%Y-%m-%d")
                    if contract_data.get("signed_date")
                    else ""
                ),
            ],
            [
                "交期",
                (
                    contract_data.get("delivery_deadline", "").strftime("%Y-%m-%d")
                    if contract_data.get("delivery_deadline")
                    else ""
                ),
            ],
        ...
        if deliverables:
            ...
            deliverable_table = Table(
                deliverable_table_data, colWidths=[0.8 * cm, 6 * cm, 2 * cm, 2 * cm, 5.2 * cm]
            )
    ...
    def export_invoice_to_pdf(
        self, invoice_data: Dict[str, Any], company_info: Optional[Dict[str, Any]] = None
    ) -> io.BytesIO:
        """
        导出发票为 PDF
        ...
        info_data = [
            ...
            [
                "开票日期",
                (
                    invoice_data.get("issue_date", "").strftime("%Y-%m-%d")
            ...
            [
                "到期日期",
                (
                    invoice_data.get("due_date", "").strftime("%Y-%m-%d")
...
def create_pdf_response(
    pdf_data: io.BytesIO, filename: str, media_type: str = "application/pdf"
) -> Any:
    """
    创建 PDF 下载响应

...
class EcnCollector(PerformanceDataCollectorBase):
    ...
    def collect_ecn_responsibility_data(
        ...
        try:
            ...
            project_ids = [
                pm.project_id
                for pm in self.db.query(ProjectMember.project_id)
                .filter(ProjectMember.user_id == engineer_id)
                .all()
            ]

            if not project_ids:
                return {"total_ecn": 0, "responsible_ecn": 0, "ecn_responsibility_rate": 0.0}

            # 统计总ECN数
            total_ecn = (
                self.db.query(Ecn)
                .filter(
                    Ecn.project_id.in_(project_ids), Ecn.created_at.between(start_date, end_date)
                )
                .count()
            )

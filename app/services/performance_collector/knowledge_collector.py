...
class KnowledgeCollector(PerformanceDataCollectorBase):
    ...
    def collect_knowledge_contribution_data(
        ...
        try:
            ...
            # 统计被引用次数
            total_reuse_count = sum(c.reuse_count or 0 for c in contributions)
            ...
            return {
                ...
                "document_count": sum(
                    1 for c in contributions if c.contribution_type == "document"
                ),
                "template_count": sum(
                    1 for c in contributions if c.contribution_type == "template"
                ),

...
class PerformanceIntegrationService:
    ...
    def calculate_integrated_score(
        ...
        """
        # 1. 获取基础绩效得分
        base_score = PerformanceIntegrationService._get_base_performance_score(db, user_id, period)

        if base_score is None:
            return None

        # 2. 获取任职资格得分
        qualification_data = PerformanceIntegrationService._get_qualification_score(db, user_id)
    ...
    @staticmethod
    def _get_base_performance_score(db: Session, user_id: int, period: str) -> Optional[Decimal]:
        """获取基础绩效得分"""
        # 获取工作总结
        summary = (
            db.query(MonthlyWorkSummary)
            .filter(MonthlyWorkSummary.employee_id == user_id, MonthlyWorkSummary.period == period)
            .first()
        )
        ...
        # 获取评价记录
        evaluations = (
            db.query(PerformanceEvaluationRecord)
            .filter(
    ...
    @staticmethod
    def _get_qualification_score(db: Session, user_id: int) -> Optional[Dict[str, Any]]:
        """获取任职资格得分"""
                ...
                total_score = (
                    sum(
                        (
                            ...
                            else float(v) if isinstance(v, (int, float)) else 0
                        )
        ...
        return {
            ...
            "certified_date": (
                qualification.certified_date.isoformat() if qualification.certified_date else None
            ),
        }
    ...
    def update_qualification_in_evaluation(
        db: Session, evaluation_id: int, qualification_data: Dict[str, Any]
    ) -> PerformanceEvaluationRecord:
        """在评价中更新任职资格信息"""
        evaluation = (
            db.query(PerformanceEvaluationRecord)
            .filter(PerformanceEvaluationRecord.id == evaluation_id)
            .first()
        )
    ...
    def get_integrated_performance_for_period(
        db: Session, user_id: int, period_id: Optional[int] = None
    ) -> Optional[Dict[str, Any]]:
        """获取指定周期的融合绩效"""
        if period_id:
            ...
            period = (
                ...
                .order_by(PerformancePeriod.end_date.desc())
                .first()
            )
            if not period:
                return None
        ...
        return PerformanceIntegrationService.calculate_integrated_score(db, user_id, period_str)

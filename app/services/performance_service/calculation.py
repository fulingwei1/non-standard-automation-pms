...
def calculate_final_score(db: Session, summary_id: int, period: str) -> Optional[Dict[str, Any]]:
    ...
    weight_config = (
        ...
        .filter(EvaluationWeightConfig.effective_date <= period_date)
        .order_by(EvaluationWeightConfig.effective_date.desc())
        .first()
    )
    ...
    # 3. 分离部门和项目评价
    dept_evaluations = [
        e for e in evaluations if e.evaluator_type == EvaluatorTypeEnum.DEPT_MANAGER
    ]
    project_evaluations = [
        e for e in evaluations if e.evaluator_type == EvaluatorTypeEnum.PROJECT_MANAGER
    ]
    ...
    if project_evaluations:
        ...
        if total_weight > 0:
            ...
            # 如果没有设置项目权重，使用简单平均
            project_score = float(sum([e.score for e in project_evaluations])) / len(
                project_evaluations
            )

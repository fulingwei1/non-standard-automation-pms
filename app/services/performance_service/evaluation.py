...
def create_evaluation_tasks(
    ...
    if employee.employee_id:
        ...
        if emp_obj and emp_obj.department:
            dept = (
                db.query(Department)
                .filter(Department.dept_name == emp_obj.department, Department.is_active)
                .first()
            )
            ...
            if dept and dept.manager_id:
                ...
                if manager_emp:
                    manager_user = (
                        db.query(User)
                        .filter(User.employee_id == manager_emp.id, User.is_active)
                        .first()
                    )
                    ...
                    if manager_user:
                        ...
                        existing = (
                            ...
                            .filter(
                                ...
                                PerformanceEvaluationRecord.evaluator_type
                                == EvaluatorTypeEnum.DEPT_MANAGER,
                            )
                            .first()
                        )
    ...
    for member in project_members:
        project = (
            db.query(Project).filter(Project.id == member.project_id, Project.is_active).first()
        )

...
def get_user_manager_roles(db: Session, user: User) -> Dict[str, Any]:
    ...
    if user.employee_id:
        dept = (
            db.query(Department)
            .filter(Department.manager_id == user.employee_id, Department.is_active)
            .first()
        )
...
def get_manageable_employees(db: Session, user: User, period: Optional[str] = None) -> List[int]:
    """
    获取经理可以评价的员工ID列表
    ...
    if roles["is_dept_manager"] and roles["managed_dept_id"]:
        ...
        if dept:
            ...
            employees = (
                db.query(Employee)
                .filter(
                    Employee.department == dept.dept_name, Employee.is_active  # 使用字符串字段匹配
                )
                .all()
            )
            ...
            for emp in employees:
                user_obj = db.query(User).filter(User.employee_id == emp.id, User.is_active).first()
                if user_obj:
    ...
    if roles["is_project_manager"] and roles["managed_project_ids"]:
        query = db.query(ProjectMember).filter(
            ProjectMember.project_id.in_(roles["managed_project_ids"]), ProjectMember.is_active
        )
        ...
        if period:
            ...
            query = query.filter(
                or_(ProjectMember.start_date.is_(None), ProjectMember.start_date <= period_end),
                or_(ProjectMember.end_date.is_(None), ProjectMember.end_date >= period_start),
            )

...
class PerformanceStatsService:
    ...
    def get_user_performance_stats(
        ...
        query = self.db.query(Timesheet).filter(
            Timesheet.user_id == user_id, Timesheet.status == "APPROVED"
        )
        ...
        for project_id, contribution in project_contributions.items():
            ...
            if project_total > 0:
                contribution["contribution_rate"] = (
                    contribution["total_hours"] / float(project_total)
                ) * 100
    ...
    def analyze_skill_expertise(
        ...
        for skill, hours in skill_hours.items():
            if total_hours > 0:
                ...
                skill_distribution[skill] = {"hours": 0, "percentage": 0}

        # 找出主要技能（占比最高的）
        primary_skill = (
            max(skill_distribution.items(), key=lambda x: x[1]["percentage"])

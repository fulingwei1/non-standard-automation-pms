...
class PerformanceTrendService:
    ...
    def get_engineer_trend(self, engineer_id: int, periods: int = 6) -> Dict[str, Any]:
        """
        获取工程师历史趋势
        ...
        for result in results:
            ...
            if result.indicator_scores:
                ...
                trend_data["dimension_trends"]["technical"].append(
                    float(scores.get("technical_score", 75))
                )
                trend_data["dimension_trends"]["execution"].append(
                    float(scores.get("execution_score", 75))
                )
                trend_data["dimension_trends"]["cost_quality"].append(
                    float(scores.get("cost_quality_score", 75))
                )
                trend_data["dimension_trends"]["knowledge"].append(
                    float(scores.get("knowledge_score", 75))
                )
                trend_data["dimension_trends"]["collaboration"].append(
                    float(scores.get("collaboration_score", 75))
                )
            else:
                # 使用默认值
                    ...
                    changes.append(
                        {
                            ...
                            "earlier_avg": round(earlier_avg, 2),
                            "change_percentage": (
                                round(change / earlier_avg * 100, 1) if earlier_avg > 0 else 0
                            ),
                        }
                    )
    ...
    def get_department_trend(self, department_id: int, periods: int = 6) -> Dict[str, Any]:
        """
        获取部门整体趋势
        ...
        if not user_ids:
            return {"department_id": department_id, "has_data": False}
        ...
        return {
            ...
            "trend": (
                self._calculate_trend([s["avg_score"] for s in period_stats])
                if period_stats
                else "stable"
            ),
    ...
    def compare_departments(self, department_ids: List[int], period_id: int) -> Dict[str, Any]:
        """
        对比多个部门的趋势

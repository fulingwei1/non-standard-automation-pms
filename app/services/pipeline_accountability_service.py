...
class PipelineAccountabilityService:
    ...
    def analyze_cost_impact(
        ...
        return {
            ...
            "by_stage": {stage: float(cost) for stage, cost in cost_by_stage.items()},
            "by_person": {
                person_id: float(cost) for person_id, cost in list(cost_by_person.items())[:20]
            },
        }

    def _calculate_break_cost_impact(self, stage: str, record: Dict[str, Any]) -> Decimal:
        """计算断链造成的成本影响"""
        cost = Decimal("0")
                            ...
                            if opp.id:
                                ...
                                for project in projects:
                                    ...
                                    for ts in timesheets:
                                        user = (
                                            self.db.query(User)
                                            .filter(User.id == ts.user_id)
                                            .first()
                                        )
                                        if user:
                                            hourly_rate = (
                                                self.hourly_rate_service.get_user_hourly_rate(

...
class PipelineBreakAnalysisService:
    ...
    def analyze_pipeline_breaks(
        ...
        # 识别最容易断链的环节
        sorted_stages = sorted(break_rates.items(), key=lambda x: x[1]["break_rate"], reverse=True)
    ...
    def _detect_lead_to_opp_breaks(self, start_date: date, end_date: date) -> Dict[str, Any]:
        """检测线索→商机断链"""
    ...
    def _detect_opp_to_quote_breaks(self, start_date: date, end_date: date) -> Dict[str, Any]:
        """检测商机→报价断链"""
    ...
    def _detect_quote_to_contract_breaks(self, start_date: date, end_date: date) -> Dict[str, Any]:
        """检测报价→合同断链"""
        ...
        for quote in quotes:
            # 检查是否已签订合同
            has_contract = (
                len(quote.opportunity.contracts) > 0
                if quote.opportunity and quote.opportunity.contracts
                else False
            )
                    ...
                    break_records.append(
                        {
                            ...
                            "responsible_person_id": (
                                quote.opportunity.owner_id if quote.opportunity else None
                            ),
                            "responsible_person_name": (
                                quote.opportunity.owner.real_name
                                if quote.opportunity and quote.opportunity.owner
                                else None
                            ),
                        }
                    )
                    ...
                    break_records.append(
                        {
                            ...
                            "days_since_break": days_since,
                            "responsible_person_id": (
                                contract.opportunity.owner_id if contract.opportunity else None
                            ),
                            "responsible_person_name": (
                                contract.opportunity.owner.real_name
                                if contract.opportunity and contract.opportunity.owner
                                else None
                            ),
                        }
                    )
    ...
    def _detect_project_to_invoice_breaks(self, start_date: date, end_date: date) -> Dict[str, Any]:
        """检测项目→发票断链（里程碑验收后）"""
        ...
        for milestone in milestones:
            ...
            if project.contract:
                has_invoice = (
                    len(project.contract.invoices) > 0 if project.contract.invoices else False
                )
                    ...
                    break_records.append(
                        {
                            ...
                            "responsible_person_name": (
                                project.manager.real_name if project.manager else None
                            ),
    ...
    def _detect_invoice_to_payment_breaks(self, start_date: date, end_date: date) -> Dict[str, Any]:
        """检测发票→回款断链"""
                    ...
                    break_records.append(
                        {
                            ...
                            "responsible_person_id": (
                                invoice.contract.opportunity.owner_id
                                if invoice.contract and invoice.contract.opportunity
                                else None
                            ),
                            "responsible_person_name": (
                                invoice.contract.opportunity.owner.real_name
                                if invoice.contract
                                and invoice.contract.opportunity
                                and invoice.contract.opportunity.owner
                                else None
                            ),
                        }
                    )
    ...
    def get_break_reasons(
        ...
        # 这里返回示例数据
        return {"break_stage": break_stage, "reasons": common_reasons, "top_reasons": []}
    ...
    def get_break_warnings(self, days_ahead: int = 7) -> List[Dict[str, Any]]:
        """获取即将断链的预警列表"""
        warnings = []
                        ...
                        warnings.append(
                            {
                                ...
                                "responsible_person_id": lead.owner_id,
                                "responsible_person_name": (
                                    lead.owner.real_name if lead.owner else None
                                ),
                            }
                        )

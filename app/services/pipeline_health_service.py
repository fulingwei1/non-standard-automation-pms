...
class PipelineHealthService:
    ...
    HEALTH_THRESHOLDS = {
        ...
        "CONTRACT": {"H1": 0, "H2": 1, "H3": 1},  # 正常执行  # 有风险  # 执行受阻
        "PAYMENT": {"H1": 0, "H2": 7, "H3": 30},  # 正常回款  # 延迟7天：有风险  # 逾期30天：严重
    }
    ...
    def calculate_lead_health(self, lead_id: int) -> Dict[str, Any]:
        ...
        # 计算距离上次跟进的天数
        last_follow_up = (
            lead.next_action_at.date()
            if lead.next_action_at
            else lead.created_at.date() if lead.created_at else date.today()
        )
        days_since_follow_up = (date.today() - last_follow_up).days

        # 检查是否有跟进记录
        if lead.follow_ups:
            latest_follow_up = max(
                lead.follow_ups, key=lambda f: f.created_at if f.created_at else datetime.min
            )
    ...
    def calculate_opportunity_health(self, opp_id: int) -> Dict[str, Any]:
        ...
        # 计算距离上次进展的天数
        last_progress = (
            opportunity.updated_at.date()
            if opportunity.updated_at
            else opportunity.created_at.date() if opportunity.created_at else date.today()
        )
    ...
    def calculate_pipeline_health(
        ...
        # 计算整体健康度（取最差的）
        if pipeline_health:
            min_health_score = min(h.get("health_score", 100) for h in pipeline_health.values())
            overall_health = "H1"

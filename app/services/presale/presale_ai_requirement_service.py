...
class AIRequirementAnalyzer:
    ...
    async def generate_clarification_questions(
        self, raw_requirement: str, structured_data: Optional[Dict[str, Any]] = None
...
请生成5-10个澄清问题。"""
    ...
    async def perform_feasibility_analysis(
        self, raw_requirement: str, structured_data: Dict[str, Any]
    ) -> FeasibilityAnalysis:
        """
        执行技术可行性分析
...
请分析上述需求。"""
    ...
    def _calculate_confidence_score(
        self, raw_requirement: str, parsed_result: Dict[str, Any]
    ) -> float:
        """计算需求理解置信度"""
    ...
    def _fallback_generate_questions(self, raw_requirement: str) -> List[ClarificationQuestion]:
        ...
        default_questions = [
            ...
            {
                "category": "功能需求",
                "question": "请详细说明自动化系统需要实现的具体功能",
                "importance": "high",
            },
            {
                "category": "约束条件",
                "question": "项目是否有特殊的空间、环境或安全约束？",
                "importance": "high",
            },
            {
                "category": "验收标准",
                "question": "请明确项目的验收标准和成功指标",
                "importance": "critical",
            },
            {
                "category": "资源预算",
    ...
    def _fallback_feasibility_analysis(
...
class PresaleAIRequirementService:
    ...
    def get_analysis(self, analysis_id: int) -> Optional[PresaleAIRequirementAnalysis]:
        """获取分析结果"""
        return (
            self.db.query(PresaleAIRequirementAnalysis)
            .filter(PresaleAIRequirementAnalysis.id == analysis_id)
            .first()
        )

    async def refine_requirement(
        self, request: RequirementRefinementRequest, user_id: int
    ) -> PresaleAIRequirementAnalysis:
        """精炼需求"""
        ...
        # 重新分析
        refined_result = await self.analyzer.analyze_requirement(enhanced_requirement, "deep")
    ...
    def get_clarification_questions(
        ...
        analysis = (
            self.db.query(PresaleAIRequirementAnalysis)
            .filter(PresaleAIRequirementAnalysis.presale_ticket_id == ticket_id)
            .order_by(desc(PresaleAIRequirementAnalysis.created_at))
            .first()
        )
        ...
        questions = [ClarificationQuestion(**q) for q in analysis.clarification_questions]

        return questions, analysis.id

    def update_structured_requirement(
        self, request: RequirementUpdateRequest, user_id: int
    ) -> PresaleAIRequirementAnalysis:
        """更新结构化需求"""
    ...
    def get_requirement_confidence(self, ticket_id: int) -> Dict[str, Any]:
        """获取置信度评分"""

...
class PresaleAIService:
    ...
    def match_templates(
        self, request: TemplateMatchRequest, user_id: int
    ...
    def generate_solution(self, request: SolutionGenerationRequest, user_id: int) -> Dict[str, Any]:
        """
        AI智能方案生成
        1. 分析需求
        2. 匹配模板
        ...
        if request.template_id:
            template = (
                self.db.query(PresaleSolutionTemplate).filter_by(id=request.template_id).first()
            )
        ...
        if request.generate_bom and "equipment_list" in solution_content:
            bom_response = self.generate_bom(
                equipment_list=solution_content["equipment_list"], include_cost=True
            )
...
"""
    ...
    def _calculate_confidence(
        self, solution: Dict[str, Any], template: Optional[PresaleSolutionTemplate]
    ) -> float:
        """计算方案置信度"""
        score = 0.5  # 基础分
...
"""
    ...
    def generate_bom(
        ...
        for equipment in equipment_list:
            ...
            bom_item = self._generate_bom_item(
                equipment, include_cost=include_cost, include_suppliers=include_suppliers
            )
    ...
    def update_solution(self, solution_id: int, update_data: Dict[str, Any]) -> PresaleAISolution:
        """更新方案"""
    ...
    def review_solution(
        self, solution_id: int, reviewer_id: int, status: str, comments: Optional[str] = None
    ) -> PresaleAISolution:
        """审核方案"""
        solution = self.get_solution(solution_id)
        if not solution:

...
class DeliveryPerformanceAnalyzer:
    ...
    def get_delivery_performance_data(
        ...
        query = (
            ...
            .join(Vendor, GoodsReceipt.supplier_id == Vendor.id)
            .filter(
        ...
        for sid, stats in supplier_stats.items():
            ...
            # 计算该供应商平均延期天数
            supplier_delays = [
                d["delay_days"] for d in delayed_orders if d.get("supplier_id") == sid
            ]
        ...
        return {
            ...
            "summary": {
                ...
                "avg_on_time_rate": (
                    round(
                        sum(p["on_time_rate"] for p in performance_list) / len(performance_list), 2
                    )
                    if performance_list
                    else 0
                ),

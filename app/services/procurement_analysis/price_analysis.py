...
class PriceAnalyzer:
    ...
    def get_price_fluctuation_data(
        ...
        query = (
            ...
            .join(Vendor, PurchaseOrder.supplier_id == Vendor.id)
        ...
        results = (
            query.order_by(PurchaseOrderItem.material_code, PurchaseOrder.order_date.desc())
            .limit(1000)
            .all()
        )
        ...
        for row in results:
            ...
            if code not in material_prices:
                material_prices[code] = {
                    ...
                    "standard_price": (
                        float(row.standard_price or 0) if row.standard_price else None
                    ),
                    "price_history": [],
                    "suppliers": set(),
                    "min_price": None,
        ...
        return {
            ...
            "summary": {
                ...
                "high_volatility_count": sum(
                    1 for m in material_prices.values() if m["price_volatility"] > 20
                ),
                "avg_volatility": (
                    round(
                        sum(m["price_volatility"] for m in material_prices.values())
                        / len(material_prices),
                        2,
                    )
                    if material_prices

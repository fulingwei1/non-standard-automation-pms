...
class QualityAnalyzer:
    ...
    def get_quality_rate_data(
        ...
        query = (
            ...
            .join(PurchaseOrder, GoodsReceipt.order_id == PurchaseOrder.id)
        ...
        return {
            ...
            "summary": {
                ...
                "avg_pass_rate": (
                    round(sum(q["overall_pass_rate"] for q in quality_list) / len(quality_list), 2)
                    if quality_list
                    else 0
                ),
                "high_quality_suppliers": sum(
                    1 for q in quality_list if q["overall_pass_rate"] >= 98
                ),
                "low_quality_suppliers": sum(
                    1 for q in quality_list if q["overall_pass_rate"] < 90
                ),
            },
        }

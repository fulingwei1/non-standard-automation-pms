...
class RequestEfficiencyAnalyzer:
    ...
    def get_request_efficiency_data(
        ...
        query = (
            ...
            .outerjoin(PurchaseOrder, PurchaseRequest.id == PurchaseOrder.source_request_id)
            .filter(
                PurchaseRequest.requested_at >= start_date, PurchaseRequest.requested_at <= end_date
            )
                ...
                efficiency_data.append(
                    {
                        ...
                        "order_created_at": (
                            order_created_at.isoformat() if order_created_at else None
                        ),
        ...
        avg_processing_hours = (
            total_processing_hours / processed_count if processed_count > 0 else 0
        )
        processed_within_24h = sum(
            1 for e in efficiency_data if not e.get("is_pending") and e["processing_hours"] <= 24
        )
        processed_within_48h = sum(
            1 for e in efficiency_data if not e.get("is_pending") and e["processing_hours"] <= 48
        )
        ...
        return {
            ...
            "summary": {
                ...
                "within_24h_rate": (
                    round(processed_within_24h / processed_count * 100, 2)

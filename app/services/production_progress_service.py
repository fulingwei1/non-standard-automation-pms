...
class ProductionProgressService:
    ...
    def get_progress_deviations(
        ...
        for wo in work_orders:
            ...
            delay_days = None
            current_progress = wo.progress or 0
            if wo.plan_end_date and current_progress > 0:
                # 基于当前进度速率估算
                if wo.actual_start_time:
                    elapsed_days = (datetime.now() - wo.actual_start_time).days
                    if elapsed_days > 0 and current_progress > 0:
                        daily_rate = current_progress / elapsed_days
                        remaining_days = int((100 - current_progress) / daily_rate)

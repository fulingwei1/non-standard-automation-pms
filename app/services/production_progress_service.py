...
class ProductionProgressService:
    ...
    def calculate_progress_deviation(
        self, work_order_id: int, actual_progress: int, actual_date: Optional[date] = None
    ) -> Tuple[int, int, bool]:
        """
        计算进度偏差 (实际 vs 计划)
    ...
    def identify_bottlenecks(
        self, workshop_id: Optional[int] = None, min_level: int = 1
    ) -> List[Dict]:
        """
        识别瓶颈工位（基于产能利用率）
        ...
        query = (
            ...
            .join(Workstation, WorkstationStatus.workstation_id == Workstation.id)
            .filter(
        ...
        for ws_status, workstation in results:
            ...
            if level >= min_level:
                ...
                pending_wo_count = (
                    self.db.query(func.count(WorkOrder.id))
                    .filter(
                        WorkOrder.workstation_id == workstation.id, WorkOrder.status == "PENDING"
                    )
                    .scalar()
                    or 0
                )
        ...
        # 按瓶颈等级和产能利用率排序
        bottlenecks.sort(
            key=lambda x: (x["bottleneck_level"], x["capacity_utilization"]), reverse=True
        )

        return bottlenecks

    def _calculate_bottleneck_level(
        self, ws_status: WorkstationStatus, workstation_id: int
    ) -> Tuple[int, str]:
        """计算瓶颈等级和原因"""
        ...
        pending_count = (
            self.db.query(func.count(WorkOrder.id))
            .filter(WorkOrder.workstation_id == workstation_id, WorkOrder.status == "PENDING")
            .scalar()
            or 0
        )
    ...
    def evaluate_alert_rules(self, work_order_id: int, user_id: int) -> List[ProgressAlertCreate]:
        ...
        plan_progress, deviation, is_delayed = self.calculate_progress_deviation(
            work_order_id, work_order.progress
        )
        ...
        if work_order.workstation_id:
            ws_status = (
                self.db.query(WorkstationStatus)
                .filter(WorkstationStatus.workstation_id == work_order.workstation_id)
                .first()
            )
    ...
    def create_progress_log(
        self, log_data: ProductionProgressLogCreate, user_id: int
    ) -> ProductionProgressLog:
        """创建进度日志"""
        work_order = self.db.query(WorkOrder).filter(WorkOrder.id == log_data.work_order_id).first()
        ...
        # 获取上一次进度
        last_log = (
            self.db.query(ProductionProgressLog)
            .filter(ProductionProgressLog.work_order_id == log_data.work_order_id)
            .order_by(desc(ProductionProgressLog.logged_at))
            .first()
        )
        ...
        # 计算进度偏差
        plan_progress, deviation, is_delayed = self.calculate_progress_deviation(
            log_data.work_order_id, log_data.current_progress
        )
        ...
        if log_data.work_hours:
            work_order.actual_hours = (
                work_order.actual_hours or Decimal("0")
            ) + log_data.work_hours
        ...
        for alert_data in alerts:
            ...
            if not existing_alert:
                alert = ProgressAlert(**alert_data.model_dump(), triggered_at=datetime.now())
                self.db.add(alert)
    ...
    def _update_workstation_status(self, workstation_id: int, work_order_id: int):
        ...
        ws_status = (
            self.db.query(WorkstationStatus)
            .filter(WorkstationStatus.workstation_id == workstation_id)
            .first()
        )
        ...
        # 计算今日产能利用率
        today = date.today()
        today_logs = self.db.query(func.sum(ProductionProgressLog.work_hours)).filter(
        ...
        ws_status.capacity_utilization = min(
            (
                ...
                if ws_status.planned_hours_today > 0
                else Decimal("0")
            ),
            Decimal("100"),
        )
    ...
    def get_realtime_overview(self, workshop_id: Optional[int] = None) -> RealtimeProgressOverview:
        ...
        completed_today = query.filter(
            WorkOrder.status == "COMPLETED", func.date(WorkOrder.actual_end_time) == today
        ).count()

        # 统计延期工单
        delayed_query = (
            self.db.query(ProductionProgressLog.work_order_id)
            .filter(ProductionProgressLog.is_delayed == 1)
            .distinct()
        )
        if workshop_id:
            delayed_query = delayed_query.join(WorkOrder).filter(
        ...
        # 计算整体产能利用率
        avg_utilization = self.db.query(func.avg(WorkstationStatus.capacity_utilization)).join(
            Workstation
        )
        if workshop_id:
        ...
        # 计算平均效率
        avg_efficiency = (
            self.db.query(func.avg(WorkstationStatus.efficiency_rate))
            .join(Workstation)
            .filter(WorkstationStatus.efficiency_rate.isnot(None))
        )
        if workshop_id:
    ...
    def get_work_order_timeline(self, work_order_id: int) -> Optional[WorkOrderProgressTimeline]:
        ...
        logs = (
            self.db.query(ProductionProgressLog)
            .filter(ProductionProgressLog.work_order_id == work_order_id)
            .order_by(ProductionProgressLog.logged_at)
            .all()
        )
        ...
        alerts = (
            self.db.query(ProgressAlert)
            .filter(ProgressAlert.work_order_id == work_order_id)
            .order_by(desc(ProgressAlert.triggered_at))
            .all()
        )
    ...
    def get_progress_deviations(
        self, workshop_id: Optional[int] = None, min_deviation: int = 10, only_delayed: bool = True
    ) -> List[ProgressDeviation]:
        """获取进度偏差列表"""
        query = self.db.query(WorkOrder).filter(WorkOrder.status.in_(["IN_PROGRESS", "PENDING"]))

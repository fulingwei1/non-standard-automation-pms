...
class ProjectContributionService:
    ...
    def calculate_member_contribution(
        self, project_id: int, user_id: int, period: str  # YYYY-MM
    ) -> ProjectMemberContribution:
        """
        计算项目成员贡献度
        ...
        # 检查是否已存在
        contribution = (
            self.db.query(ProjectMemberContribution)
            .filter(
        ...
        if not contribution:
            contribution = ProjectMemberContribution(
                project_id=project_id, user_id=user_id, period=period
            )
        ...
        contribution.task_hours = sum(float(t.estimated_hours or 0) for t in tasks)
        contribution.actual_hours = sum(float(t.actual_hours or 0) for t in tasks)
        ...
        contribution.issue_resolved = len(
            [i for i in issues if i.status in ["RESOLVED", "CLOSED", "VERIFIED"]]
        )
    ...
    def _calculate_contribution_score(self, contribution: ProjectMemberContribution) -> Decimal:
        ...
        if contribution.actual_hours > 0:
            score += (
                Decimal(str(contribution.actual_hours)) * Decimal("0.2") / Decimal("10")
            )  # 归一化
        ...
        if contribution.bonus_amount > 0:
            score += (
                (contribution.bonus_amount / Decimal("100000")) * Decimal("10") * Decimal("0.1")
            )

        return score

    def get_project_contributions(
        self, project_id: int, period: Optional[str] = None
        ...
        return query.order_by(ProjectMemberContribution.contribution_score.desc()).all()

    def get_user_project_contributions(
        self, user_id: int, start_period: Optional[str] = None, end_period: Optional[str] = None
    ) -> List[ProjectMemberContribution]:
        """
        获取用户的项目贡献汇总
    ...
    def rate_member_contribution(
        self, project_id: int, user_id: int, period: str, pm_rating: int, rater_id: int
    ) -> ProjectMemberContribution:
        """
        项目经理评分
        ...
        contribution.contribution_score = (base_score * Decimal("0.7")) + (
            pm_score * Decimal("0.3")
        )
    ...
    def generate_contribution_report(
        ...
        top_contributors = sorted(
            contributions, key=lambda c: float(c.contribution_score or 0), reverse=True
        )[:10]

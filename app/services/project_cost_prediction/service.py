...
class ProjectCostPredictionService:
    ...
    def _get_suggestions_summary(self, project_id: int) -> Dict:
        """获取优化建议摘要"""
        pending = (
            self.db.query(CostOptimizationSuggestion)
            .filter(
        ...
        return {"pending": pending, "approved": approved, "in_progress": in_progress}

    def _calculate_health_score(
        self, prediction: CostPrediction, suggestions_summary: Dict
    ) -> float:
        """计算健康评分"""
        health_score = 100.0

...
def add_tasks_sheet(wb, db: Session, project_id: int, styles: Dict[str, Any]) -> None:
    ...
    tasks = (
        db.query(Task).filter(Task.project_id == project_id).order_by(desc(Task.created_at)).all()
    )
    ...
    for row_idx, task in enumerate(tasks, 2):
        ...
        ws.cell(
            row=row_idx,
            column=7,
            value=task.planned_start_date.strftime("%Y-%m-%d") if task.planned_start_date else "",
        )
        ws.cell(
            row=row_idx,
            column=8,
            value=task.planned_end_date.strftime("%Y-%m-%d") if task.planned_end_date else "",
        )
        ws.cell(
            row=row_idx,
            column=9,
            value=task.actual_start_date.strftime("%Y-%m-%d") if task.actual_start_date else "",
        )
        ws.cell(
            row=row_idx,
            column=10,
            value=task.actual_end_date.strftime("%Y-%m-%d") if task.actual_end_date else "",
        )
        ws.cell(row=row_idx, column=11, value=task.assignee_name or "")
        ws.cell(
            row=row_idx,
            column=12,
            value=task.created_at.strftime("%Y-%m-%d %H:%M:%S") if task.created_at else "",
        )
...
def add_costs_sheet(wb, db: Session, project_id: int, styles: Dict[str, Any]) -> None:
    ...
    costs = (
        ...
        .order_by(desc(ProjectCost.cost_date))
        .all()
    )
    ...
    for row_idx, cost in enumerate(costs, 2):
        ...
        ws.cell(
            row=row_idx,
            column=1,
            value=cost.cost_date.strftime("%Y-%m-%d") if cost.cost_date else "",
        )
        ...
        ws.cell(
            row=row_idx,
            column=7,
            value=cost.created_at.strftime("%Y-%m-%d %H:%M:%S") if cost.created_at else "",
        )
...
def create_project_detail_excel(
    db: Session, project, include_tasks: bool, include_costs: bool
) -> BytesIO:
    """
    创建项目详情Excel文件

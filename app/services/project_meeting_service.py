...
class ProjectMeetingService:
    ...
    def get_project_meetings(
        ...
        """
        # 查询主项目关联的会议
        query1 = self.db.query(StrategicMeeting).filter(StrategicMeeting.project_id == project_id)
    ...
    def link_meeting_to_project(
        self, meeting_id: int, project_id: int, is_primary: bool = False
    ) -> bool:
        """
        将会议关联到项目
        ...
        Returns:
            是否成功
        """
        meeting = self.db.query(StrategicMeeting).filter(StrategicMeeting.id == meeting_id).first()
    ...
    def unlink_meeting_from_project(self, meeting_id: int, project_id: int) -> bool:
        """
        取消会议与项目的关联
    ...
    def get_project_meeting_statistics(self, project_id: int) -> Dict[str, Any]:
        """
        获取项目会议统计信息
        ...
        for meeting in meetings:
            ...
            completed_action_items += len([ai for ai in action_items if ai.status == "COMPLETED"])

...
class ProjectMembersService:
    ...
    def get_member_by_id(
        self, project_id: int, member_id: int, enrich: bool = True
    ) -> ProjectMember:
        """获取项目成员详情"""
        member = (
    ...
    def list_members(
        ...
        # 构建查询
        query = self.db.query(ProjectMember).filter(ProjectMember.project_id == project_id)
    ...
    def update_member(
        self, project_id: int, member_id: int, update_data: Dict[str, Any], enrich: bool = True
    ) -> ProjectMember:
        """更新项目成员信息"""
        member = (
            self.db.query(ProjectMember)
    ...
    def batch_add_members(
        ...
        for user_id in user_ids:
            ...
            # 检查冲突
            conflict_info = self.check_member_conflicts(user_id, start_date, end_date, project_id)
    ...
    def get_dept_users_for_project(self, project_id: int, dept_id: int) -> Dict[str, Any]:
        """获取部门用户列表（用于批量添加成员）"""
        ...
        # 获取关联的用户
        users = self.db.query(User).filter(User.employee_id.in_(employee_ids), User.is_active).all()
        ...
        return {"dept_id": dept_id, "dept_name": dept.dept_name, "users": available_users}

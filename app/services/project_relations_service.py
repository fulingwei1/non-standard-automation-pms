...
def get_material_transfer_relations(
    db: Session, project_id: int, relation_type: Optional[str]
) -> List[Dict[str, Any]]:
    """
    获取物料转移关联关系
...
def get_shared_resource_relations(
    ...
    try:
        ...
        shared_resource_projects = (
            db.query(
                PmoResourceAllocation.project_id,
                func.count(PmoResourceAllocation.id).label("shared_count"),
            )
            .filter(
...
def get_shared_customer_relations(
    db: Session, project: Project, project_id: int, relation_type: Optional[str]
) -> List[Dict[str, Any]]:
    """
    获取共享客户关联关系
    ...
    customer_projects = (
        db.query(Project)
        .filter(
            Project.customer_id == project.customer_id, Project.id != project_id, Project.is_active
        )
        .all()
    )
...
def calculate_relation_statistics(relations: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    计算关联关系统计信息
...
def discover_same_customer_relations(
    db: Session, project: Project, project_id: int
) -> List[Dict[str, Any]]:
    """
    发现相同客户的项目关联
...
def discover_same_pm_relations(
    ...
    pm_projects = (
        db.query(Project)
        .filter(Project.pm_id == project.pm_id, Project.id != project_id, Project.is_active)
        .all()
    )
...
def discover_material_transfer_relations(db: Session, project_id: int) -> List[Dict[str, Any]]:
    """
    发现物料转移的项目关联
    ...
    for transfer in material_transfers:
        related_project_id = (
            ...
            if transfer.from_project_id == project_id
...
def discover_shared_resource_relations(db: Session, project_id: int) -> List[Dict[str, Any]]:
    """
    发现共享资源的项目关联
...
def discover_shared_rd_project_relations(db: Session, project_id: int) -> List[Dict[str, Any]]:
    """
    发现关联相同研发项目的项目关联
    ...
    try:
        ...
        linked_rd_projects = (
            db.query(RdProject).filter(RdProject.linked_project_id == project_id).all()
        )

        for rd_project in linked_rd_projects:
            # 查找其他关联到相同研发项目的非标项目
            other_linked_projects = (
                db.query(RdProject)
                .filter(
            ...
            for other_rd in other_linked_projects:
                if other_rd.linked_project_id:
                    related_project = (
                        db.query(Project).filter(Project.id == other_rd.linked_project_id).first()
                    )
                    if related_project:
                        relations.append(
                            {
...
def deduplicate_and_filter_relations(
    relations: List[Dict[str, Any]], min_confidence: float
) -> List[Dict[str, Any]]:
    """
    去重并过滤置信度
    ...
    for relation in relations:
        if relation["confidence"] >= min_confidence:
            ...
            if (
                key not in unique_relations
                or relation["confidence"] > unique_relations[key]["confidence"]
            ):
...
def calculate_discovery_relation_statistics(relations: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    计算关联关系统计信息

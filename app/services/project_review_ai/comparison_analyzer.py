...
class ProjectComparisonAnalyzer:
    ...
    def compare_with_history(
        self, review_id: int, similarity_type: str = "industry", limit: int = 5
    ) -> Dict[str, Any]:
        """
        与历史项目对比
        ...
        """
        # 1. 获取当前项目复盘
        current_review = self.db.query(ProjectReview).filter(ProjectReview.id == review_id).first()

        if not current_review:
            raise ValueError(f"复盘报告 {review_id} 不存在")

        # 2. 查找相似历史项目
        similar_reviews = self._find_similar_reviews(current_review, similarity_type, limit)

        # 3. 构建对比数据
        comparison_data = self._build_comparison_data(current_review, similar_reviews)
    ...
    def identify_improvements(self, review_id: int) -> List[Dict[str, Any]]:
        """
        识别改进点
    ...
    def _find_similar_reviews(
        self, current_review: ProjectReview, similarity_type: str, limit: int
    ) -> List[ProjectReview]:
        """查找相似的历史复盘"""
        query = self.db.query(ProjectReview).filter(
            ProjectReview.id != current_review.id, ProjectReview.status == "PUBLISHED"
        )
        ...
        if similarity_type == "industry":
            # 按行业相似度查找
            query = query.join(Project).filter(Project.industry == current_project.industry)
        elif similarity_type == "type":
            # 按项目类型查找
            query = query.join(Project).filter(Project.project_type == current_project.project_type)
    ...
    def _build_comparison_data(
        self, current: ProjectReview, similars: List[ProjectReview]
    ) -> Dict[str, Any]:
        """构建对比数据"""
        # 计算平均指标
        avg_schedule_variance = (
            sum(r.schedule_variance or 0 for r in similars) / len(similars) if similars else 0
        )
        avg_cost_variance = (
            sum(float(r.cost_variance or 0) for r in similars) / len(similars) if similars else 0
        )
        avg_change_count = (
            sum(r.change_count or 0 for r in similars) / len(similars) if similars else 0
        )
        avg_satisfaction = (
            sum(r.customer_satisfaction or 0 for r in similars) / len(similars) if similars else 0
        )
    ...
    def _analyze_comparison(self, comparison_data: Dict[str, Any]) -> Dict[str, Any]:
        """AI分析对比结果"""
        prompt = f"""# 项目对比分析任务
...
- benchmarks: 基准对比 {{"schedule": {{}}, "cost": {{}}, "quality": {{}}}}"""
        ...
        ai_response = self.ai_client.generate_solution(
            prompt=prompt, model="glm-5", temperature=0.6, max_tokens=2000
        )
        ...
        try:
            ...
            analysis = {"strengths": [], "weaknesses": [], "improvements": [], "benchmarks": {}}

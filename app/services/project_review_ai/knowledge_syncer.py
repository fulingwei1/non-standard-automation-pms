...
class ProjectKnowledgeSyncer:
    ...
    def sync_to_knowledge_base(self, review_id: int, auto_publish: bool = True) -> Dict[str, Any]:
        """
        同步复盘到知识库
        ...
        # 3. 创建或更新知识库记录
        existing_case = (
            self.db.query(PresaleKnowledgeCase)
            .filter(PresaleKnowledgeCase.case_name == case_data["case_name"])
            .first()
        )
    ...
    def _generate_knowledge_case(self, review: ProjectReview) -> Dict[str, Any]:
        """生成知识库案例"""
        ...
        ai_response = self.ai_client.generate_solution(
            prompt=prompt, model="glm-5", temperature=0.6, max_tokens=1500
        )
        ...
        case_data = {
            ...
            "customer_name": (
                project.customer.name if hasattr(project, "customer") and project.customer else None
            ),
            "project_amount": review.budget_amount,
            "project_summary": summary_data.get("summary", review.ai_summary or ""),
            "technical_highlights": summary_data.get(
                "technical_highlights", self._extract_technical_highlights(review)
            ),
    ...
    def _build_summary_prompt(self, review: ProjectReview, project: Project) -> str:
        """构建摘要生成提示词"""
        return f"""# 项目案例摘要生成任务
...
确保内容专业、客观、易于理解。"""
    ...
    def _parse_summary_response(self, ai_response: Dict[str, Any]) -> Dict[str, Any]:
        """解析AI摘要响应"""
    ...
    def _extract_tags(self, review: ProjectReview, project: Project) -> List[str]:
        """提取标签"""
        tags = []
    ...
    def _extract_technical_highlights(self, review: ProjectReview) -> str:
        """从最佳实践中提取技术亮点"""
    ...
    def _calculate_quality_score(self, review: ProjectReview) -> float:
        """计算案例质量评分"""
        score = 0.5  # 基础分
    ...
    def update_case_from_lessons(self, review_id: int, case_id: int) -> Dict[str, Any]:
        """从经验教训更新知识库案例"""
        # 获取经验教训
        lessons = (
            self.db.query(ProjectLesson)
            .filter(ProjectLesson.review_id == review_id, ProjectLesson.ai_confidence >= 0.7)
            .all()
        )
    ...
    def get_sync_status(self, review_id: int) -> Dict[str, Any]:
        """获取同步状态"""
        # 查询是否已同步
        review = self.db.query(ProjectReview).filter(ProjectReview.id == review_id).first()
        ...
        case = (
            self.db.query(PresaleKnowledgeCase)
            .filter(PresaleKnowledgeCase.case_name == case_name)
            .first()
        )
        ...
        if case:
            ...
            return {"synced": False, "message": "尚未同步到知识库"}

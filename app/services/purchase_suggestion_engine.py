...
class PurchaseSuggestionEngine:
    ...
    def generate_from_shortages(
        self, project_id: Optional[int] = None, min_urgency: str = "NORMAL"
    ...
    def generate_from_forecast(
        self, material_id: int, forecast_months: int = 3
    ...
    def _recommend_supplier(
        self, material_id: int, weight_config: Optional[Dict[str, Decimal]] = None
        ...
        material_suppliers = (
            ...
            .filter(
                and_(
                    MaterialSupplier.material_id == material_id, MaterialSupplier.is_active == True
                )
            )
            .all()
        )
        ...
        for ms in material_suppliers:
            score_data = self._calculate_supplier_score(ms.supplier_id, material_id, weight_config)
    ...
    def _calculate_supplier_score(
        self, supplier_id: int, material_id: int, weight_config: Dict[str, Decimal]
    ) -> Dict:
        """计算供应商评分"""
        scores = {
        ...
        latest_performance = (
            ...
            .filter(SupplierPerformance.supplier_id == supplier_id)
            .order_by(SupplierPerformance.period_end.desc())
            .first()
        )
        ...
        all_prices = (
            self.db.query(MaterialSupplier.price)
            .filter(
                and_(
            ...
            if current_supplier and current_supplier.price > 0:
                ...
                if current_supplier.price <= min_price:
                    ...
                    if avg_price > min_price:
                        scores["price_score"] = Decimal("100") - (
                            (current_supplier.price - min_price)
                            / (avg_price - min_price)
                            * Decimal("40")
                        )
                    else:
    ...
    def _calculate_avg_consumption(self, material_id: int, months: int = 6) -> Optional[Decimal]:
        ...
        result = (
            self.db.query(func.sum(PurchaseOrderItem.received_qty))
            .join(PurchaseOrder)
            .filter(
                and_(
    ...
    def _generate_suggestion_no(self) -> str:
        ...
        latest = (
            self.db.query(PurchaseSuggestion)
            .filter(PurchaseSuggestion.suggestion_no.like(f"{prefix}{date_str}%"))
            .order_by(PurchaseSuggestion.suggestion_no.desc())
            .first()
        )

...
class QualityRiskManagementService:
    ...
    def get_detection(self, detection_id: int) -> Optional[QualityRiskDetection]:
        ...
        """
        return (
            self.db.query(QualityRiskDetection)
            .filter(QualityRiskDetection.id == detection_id)
            .first()
        )
    ...
    def generate_test_recommendation(
        self, detection_id: int, current_user_id: int
        ...
        risk_analysis = {
            ...
            "rework_probability": (
                float(detection.rework_probability) if detection.rework_probability else 0
            ),
            "estimated_impact_days": detection.estimated_impact_days or 0,
        }

        # 项目信息（简化版）
        project_info = {"project_id": detection.project_id}
    ...
    def update_recommendation(
        self, recommendation_id: int, update_data: Dict[str, Any]
    ) -> Optional[QualityTestRecommendation]:
        """
        更新测试推荐
        ...
        """
        recommendation = (
            self.db.query(QualityTestRecommendation)
            .filter(QualityTestRecommendation.id == recommendation_id)
            .first()
        )
    ...
    def generate_quality_report(
        ...
        if include_recommendations:
            recommendations_data = self._get_recommendations_data(project_id, start_date, end_date)
    ...
    def _get_project_name(self, project_id: int) -> str:
        """根据项目ID查询项目名称"""
        project = self.db.query(Project.project_name).filter(Project.id == project_id).first()
    ...
    def _analyze_trends(self, detections: List[QualityRiskDetection]) -> Dict[str, Dict[str, Any]]:
        """分析趋势数据"""
        trend_data = {}
        for detection in detections:

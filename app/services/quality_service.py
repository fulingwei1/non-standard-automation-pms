...
class QualityService:
    ...
    def create_inspection(
        db: Session, inspection_data: QualityInspectionCreate, current_user_id: int
    ) -> QualityInspection:
        """
        创建质检记录
    ...
    def _generate_inspection_no(db: Session) -> str:
        ...
        last_record = (
            ...
            .filter(QualityInspection.inspection_no.like(f"{prefix}%"))
            .order_by(desc(QualityInspection.inspection_no))
            .first()
        )
    ...
    def get_quality_trend(
        ...
        prediction = QualityService._calculate_moving_average(
            [d.defect_rate for d in trend_data], window=3
        )
    ...
    def _aggregate_by_time(
        inspections: List[QualityInspection], group_by: str
        ...
        for key in sorted(data_dict.keys()):
            data = data_dict[key]
            defect_rate = (
                data["defect_qty"] / data["total_qty"] * 100 if data["total_qty"] > 0 else 0
            )
            trend_data.append(
                QualityTrendDataPoint(
                    date=data["date"],
        ...
        for inspection in inspections:
            ...
            data_points.append(
                SPCDataPoint(
                    ...
                    spec_upper_limit=(
                        float(inspection.spec_upper_limit) if inspection.spec_upper_limit else None
                    ),
                    spec_lower_limit=(
                        float(inspection.spec_lower_limit) if inspection.spec_lower_limit else None
                    ),
                )
            )
    ...
    def create_defect_analysis(
        db: Session, analysis_data: DefectAnalysisCreate, current_user_id: int
    ) -> DefectAnalysis:
        """创建不良品根因分析"""
        # 生成分析单号
    ...
    def _generate_analysis_no(db: Session) -> str:
        ...
        last_record = (
            db.query(DefectAnalysis)
            .filter(DefectAnalysis.analysis_no.like(f"{prefix}%"))
            .order_by(desc(DefectAnalysis.analysis_no))
            .first()
        )
    ...
    def _check_quality_alerts(db: Session, inspection: QualityInspection):
        ...
        """
        # 查询启用的预警规则
        rules = db.query(QualityAlertRule).filter(QualityAlertRule.enabled == 1).all()
    ...
    def _check_defect_rate_alert(
        db: Session, rule: QualityAlertRule, inspection: QualityInspection
    ):
        """检查不良率预警"""
        # 计算时间窗口内的不良率
    ...
    @staticmethod
    def _check_spc_alert(db: Session, rule: QualityAlertRule, inspection: QualityInspection):
        """检查SPC控制限预警"""
    ...
    @staticmethod
    def create_rework_order(db: Session, rework_data: dict, current_user_id: int) -> ReworkOrder:
        """创建返工单"""
        # 生成返工单号
        rework_order_no = QualityService._generate_rework_order_no(db)

        rework_order = ReworkOrder(
            rework_order_no=rework_order_no, created_by=current_user_id, **rework_data
        )
    ...
    def _generate_rework_order_no(db: Session) -> str:
        ...
        last_record = (
            ...
            .filter(ReworkOrder.rework_order_no.like(f"{prefix}%"))
            .order_by(desc(ReworkOrder.rework_order_no))
            .first()
        )
    ...
    def complete_rework_order(
        db: Session, rework_order_id: int, completion_data: dict
    ) -> ReworkOrder:
        """完成返工单"""
        rework_order = db.query(ReworkOrder).filter(ReworkOrder.id == rework_order_id).first()
    ...
    def get_quality_statistics(db: Session) -> Dict:
        ...
        inspections = (
            db.query(QualityInspection)
            .filter(QualityInspection.inspection_date >= start_date)
            .all()
        )
        ...
        # 返工单统计
        rework_orders = db.query(ReworkOrder).filter(ReworkOrder.created_at >= start_date).all()

        pending_rework = db.query(ReworkOrder).filter(ReworkOrder.status == "PENDING").count()

        # Top不良类型
        defect_stats = (
            db.query(QualityInspection.defect_type, func.count(QualityInspection.id).label("count"))
            .filter(
                QualityInspection.inspection_date >= start_date,
                QualityInspection.defect_type.isnot(None),
            )
        ...
        top_defects = [
            {"defect_type": stat.defect_type, "count": stat.count} for stat in defect_stats
        ]
        ...
        recent_inspections = (
            db.query(QualityInspection)
            .filter(QualityInspection.inspection_date >= seven_days_ago)
            .all()
        )
        ...
        return {
            ...
            "active_alerts_count": db.query(func.count(AlertRecord.id))
    ...
    def batch_tracing(db: Session, batch_no: str) -> Dict:
        """批次质量追溯"""
        inspections = (
            db.query(QualityInspection).filter(QualityInspection.batch_no == batch_no).all()
        )
        ...
        defect_analyses = (
            db.query(DefectAnalysis).filter(DefectAnalysis.inspection_id.in_(inspection_ids)).all()
        )

        # 查询相关的返工单
        rework_orders = (
            db.query(ReworkOrder)
            .filter(ReworkOrder.quality_inspection_id.in_(inspection_ids))
            .all()
        )
        ...
        if inspections and inspections[0].material_id:
            ...
            material = db.query(Material.material_name).filter(Material.id == material_id).first()
            if material:

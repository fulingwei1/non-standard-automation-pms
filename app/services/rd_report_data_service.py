...
def build_auxiliary_ledger_data(
    db: Session, year: int, project_id: Optional[int] = None
) -> Dict[str, Any]:
    """
    构建研发费用辅助账数据
    ...
    Returns:
        报表数据字典
    """
    query = db.query(RdCost).join(RdProject).filter(func.extract("year", RdCost.cost_date) == year)
    if project_id:
    ...
    return {"details": details, "title": f"{year}年研发费用辅助账"}
...
def build_deduction_detail_data(
    ...
    query = (
        db.query(RdCost)
        .join(RdProject)
        .filter(func.extract("year", RdCost.cost_date) == year, RdCost.deductible_amount > 0)
    )
    if project_id:
        query = query.filter(RdCost.rd_project_id == project_id)
...
def build_high_tech_data(db: Session, year: int) -> Dict[str, Any]:
    """
    构建高新企业研发费用表数据
    ...
    costs = (
        db.query(RdCost)
        .join(RdProject)
        .filter(func.extract("year", RdCost.cost_date) == year)
        .all()
    )
...
def build_intensity_data(db: Session, year: int) -> Dict[str, Any]:
    """
    构建研发投入强度报表数据
    ...
    """
    rd_costs = (
        db.query(func.sum(RdCost.cost_amount))
        .filter(func.extract("year", RdCost.cost_date) == year)
        .scalar()
        or 0
    )
    ...
    return {"details": details, "title": f"{year}年研发投入强度报表"}


def build_personnel_data(db: Session, year: int) -> Dict[str, Any]:
    """
    构建研发人员统计数据
...
def get_rd_report_data(
    db: Session, report_type: str, year: int, project_id: Optional[int] = None
) -> Dict[str, Any]:
    """
    获取研发费用报表数据

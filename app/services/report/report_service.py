...
class ReportService:
    ...
    def get_template_with_recipients(self, template_id: int) -> Optional[Dict]:
        ...
        recipients = (
            self.db.query(ReportRecipient).filter(ReportRecipient.template_id == template_id).all()
        )
    ...
    def generate_report_data(
        self, template_id: int, period: str, generated_by: str = GeneratedByEnum.SYSTEM.value
    ) -> Dict:
        """
        生成报表数据
    ...
    def _generate_user_monthly_report(
        self, template: ReportTemplate, year: int, month: int
    ) -> Dict:
        """生成人员月度工时报表"""
            ...
            summary_data.append(
                {
                    ...
                    "avg_hours_per_day": (
                        round(float(row.total_hours or 0) / row.work_days, 2)
                        if row.work_days > 0
                        else 0
                    ),
                }
        ...
        if template.config and "filters" in template.config:
            ...
            if "department_ids" in filters:
                detail_query = detail_query.filter(
                    Timesheet.department_id.in_(filters["department_ids"])
                )
    ...
    def _generate_dept_monthly_report(
        ...
        query = (
            ...
            .group_by(Timesheet.department_id, Timesheet.department_name)
        )
            ...
            summary_data.append(
                {
                    ...
                    "overtime_hours": float(row.overtime_hours or 0),
                    "avg_hours_per_user": (
                        round(float(row.total_hours or 0) / row.user_count, 2)
                        if row.user_count > 0
                        else 0
                    ),
                }
    ...
    def _generate_company_monthly_report(
        ...
        summary_data = {
            ...
            "avg_hours_per_user": (
                round(float(total_stats.total_hours or 0) / total_stats.total_users, 2)
                if total_stats.total_users > 0
                else 0
            ),
        }
    ...
    def _generate_overtime_monthly_report(
        ...
        for row in results:
            total_overtime = (
                ...
                + float(row.weekend_hours or 0)
                + float(row.holiday_hours or 0)
            )
            summary_data.append(
                {
                    "user_id": row.user_id,
    ...
    def list_archives(
        ...
        total = query.count()
        archives = (
            query.order_by(ReportArchive.generated_at.desc())
            .offset((page - 1) * page_size)
            .limit(page_size)
            .all()
        )
    ...
    def get_archive(self, archive_id: int) -> Optional[ReportArchive]:
        """获取单个报表归档"""
        return self.db.query(ReportArchive).filter(ReportArchive.id == archive_id).first()
    ...
    def increment_download_count(self, archive_id: int) -> bool:
        ...
        logger.info(
            f"下载报表归档: {archive.period} (ID: {archive.id}, 下载次数: {archive.download_count})"
        )
    ...
    def get_archives_by_ids(self, archive_ids: List[int]) -> List[ReportArchive]:
        """根据ID列表批量获取归档"""
        return self.db.query(ReportArchive).filter(ReportArchive.id.in_(archive_ids)).all()
    ...
    def delete_recipient(self, recipient_id: int) -> bool:
        """删除报表收件人"""
        recipient = (
            self.db.query(ReportRecipient).filter(ReportRecipient.id == recipient_id).first()
        )

...
class DeptReportMixin:
    ...
    def generate_dept_weekly_report(
        ...
        # 获取部门人员
        dept_members = (
            db.query(User).filter(User.department == department.dept_name, User.is_active).all()
        )
        ...
        return {
            "summary": summary,
            "members": {"total_count": len(dept_members), "active_count": len(dept_members)},
            "timesheet": {
    ...
    def generate_dept_monthly_report(
        ...
        # 获取部门参与的项目
        project_memberships = (
            db.query(ProjectMember).filter(ProjectMember.user_id.in_(dept_user_ids)).all()
        )
        project_ids = list(set(pm.project_id for pm in project_memberships))

        projects = db.query(Project).filter(Project.id.in_(project_ids), Project.is_active).all()
        ...
        key_metrics = {
            ...
            "avg_hours_per_member": (
                round(total_hours / len(dept_members), 2) if dept_members else 0
            ),

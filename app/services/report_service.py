...
class ReportService:
    ...
    @staticmethod
    def generate_report(
        db: Session, template_id: int, period: str, generated_by: str = GeneratedByEnum.SYSTEM.value
    ) -> Dict:
        """
        生成报表数据
    ...
    def _generate_user_monthly_report(
        db: Session, template: ReportTemplate, year: int, month: int
    ) -> Dict:
        """生成人员月度工时报表"""
    ...
    def increment_download_count(db: Session, archive_id: int):
        """增加下载次数"""
        archive = db.query(ReportArchive).filter(ReportArchive.id == archive_id).first()

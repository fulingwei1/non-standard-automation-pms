...
class RequirementExtractionService:
    ...
    def _extract_production_requirements(self, project: Project) -> Dict[str, Any]:
        ...
        return {
            ...
            "required_certifications": (
                ["电工证"] if "电气" in str(skill_mapping.get(product_category, [])) else []
            ),
        }
    ...
    def _extract_service_requirements(self, project: Project) -> Dict[str, Any]:
        ...
        service_req = service_requirements.get(
            industry,
            {
                "response_time": 8,
    ...
    def _calculate_match_score(
        ...
        matched_skills = [
            s for s in required_skills if any(s in e or e in s for e in engineer_skills)
        ]
        missing_skills = [
            s for s in required_skills if not any(s in e or e in s for e in engineer_skills)
        ]
        ...
        # 4. 综合匹配度
        overall_score = skill_score * 0.4 + capacity_score * 0.3 + availability_score * 0.3

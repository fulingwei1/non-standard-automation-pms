...
def check_workstation_availability(
    ...
    if conflicting_order:
        return (
            False,
            f"工位在 {conflicting_order.plan_start_date} 至 {conflicting_order.plan_end_date} 已被工单 {conflicting_order.work_order_no} 占用",
        )
...
def find_available_workstations(
    ...
    # 查询工位
    query = db.query(Workstation).filter(Workstation.is_active, Workstation.status == "IDLE")
    ...
    for ws in workstations:
        is_available, reason = check_workstation_availability(db, ws.id, start_date, end_date)

...
class ResourcePlanService:
    ...
    @staticmethod
    def detect_employee_conflicts(db: Session, employee_id: int) -> List[ResourceConflict]:
        """检测员工的所有资源冲突"""
        assignments = (
            db.query(ProjectStageResourcePlan)
            .filter(
            ...
            for a2 in assignments[i + 1 :]:
                ...
                if overlap:
                    ...
                    if total > 100:
                        # 获取相关信息
                        employee = db.query(User).filter(User.id == employee_id).first()
                        project1 = db.query(Project).filter(Project.id == a1.project_id).first()
                        project2 = db.query(Project).filter(Project.id == a2.project_id).first()
                        ...
                        conflicts.append(
                            ResourceConflict(
                                ...
                                this_project=ConflictProject(
                                    ...
                                    project_name=(project1.project_name if project1 else "Unknown"),
                                ...
                                conflict_with=ConflictProject(
                                    ...
                                    project_name=(project2.project_name if project2 else "Unknown"),
    ...
    @staticmethod
    def detect_project_conflicts(db: Session, project_id: int) -> List[ResourceConflict]:
        """检测项目的所有资源冲突"""
        # 获取项目的所有已分配资源计划
        plans = (
            db.query(ProjectStageResourcePlan)
...
class ResourcePlanningService:
    ...
    def get_department_workload_stats(
        self, department_id: int, start_date: Optional[date] = None, end_date: Optional[date] = None
    ) -> Dict[str, Any]:
        """
        获取部门工作量统计

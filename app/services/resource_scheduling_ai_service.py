...
class ResourceSchedulingAIService:
            ...
            for i in range(len(allocs)):
                for j in range(i + 1, len(allocs)):
                    ...
                    if overlap_start <= overlap_end:
                        # 有重叠
                        total_allocation = (alloc_a.allocation_percent or 0) + (
                            alloc_b.allocation_percent or 0
                        )
    ...
    def _create_conflict_record(
        ...
        # 生成冲突编码
        conflict_code = (
            f"RC-{resource_id}-{overlap_start.strftime('%Y%m%d')}-{overlap_end.strftime('%Y%m%d')}"
        )
        ...
        return [
            {
                ...
                "adjustments": {
                    "project_a": {
                        "action": "KEEP",
                        "allocation_percent": conflict.allocation_a_percent,
                    },
                    "project_b": {
                        "action": "REDUCE",
                        "allocation_percent": 100 - conflict.allocation_a_percent,
                    },
                },
                "pros": ["快速实施", "成本低"],
                "cons": ["影响项目进度"],
    ...
    def _create_suggestion_record(
        ...
        suggestion = ResourceSchedulingSuggestion(
            ...
            affected_projects=json.dumps(
                ai_suggestion.get("affected_projects", []), ensure_ascii=False
            ),
            affected_resources=json.dumps(
                ai_suggestion.get("affected_resources", []), ensure_ascii=False
            ),
            ...
            execution_steps=json.dumps(
                ai_suggestion.get("execution_steps", []), ensure_ascii=False
            ),
...
"""
    ...
    def _create_forecast_record(
        ...
        forecast = ResourceDemandForecast(
            ...
            driving_projects=json.dumps(
                ai_forecast.get("driving_projects", []), ensure_ascii=False
            ),
            ...
            hiring_suggestion=json.dumps(
                ai_forecast.get("hiring_suggestion", {}), ensure_ascii=False
            ),
    ...
    def analyze_resource_utilization(
        ...
        utilization_rate = (
            (total_actual_hours / total_available_hours * 100) if total_available_hours > 0 else 0
        )

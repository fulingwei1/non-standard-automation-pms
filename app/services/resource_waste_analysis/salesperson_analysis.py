...
class SalespersonAnalysisMixin:
    ...
    def get_salesperson_waste_ranking(
        ...
        for sp_id, stats in salesperson_stats.items():
            ...
            win_rate = (
                stats["won_leads"] / (stats["won_leads"] + stats["lost_leads"])
                if (stats["won_leads"] + stats["lost_leads"]) > 0
                else 0
            )
            ...
            # 资源效率：中标金额 / 总投入工时
            resource_efficiency = (
                float(stats["won_amount"]) / stats["total_hours"] if stats["total_hours"] > 0 else 0
            )

            # 主要丢标原因（Top 3）
            top_reasons = sorted(stats["loss_reasons"].items(), key=lambda x: x[1], reverse=True)[
                :3
            ]
    ...
    def get_department_comparison(
        ...
        for dept, stats in department_stats.items():
            ...
            waste_rate = (
                stats["wasted_hours"] / stats["total_hours"] if stats["total_hours"] > 0 else 0
            )

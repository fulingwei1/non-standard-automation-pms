...
class AICostEstimationService:
    ...
    async def estimate_cost(self, input_data: CostEstimationInput) -> CostEstimationResponse:
        ...
        pricing_recommendations = self._generate_pricing_recommendations(
            total_cost, input_data.target_margin_rate or Decimal("0.30")
        )
    ...
    def _calculate_risk_reserve(
        self, project_type: str, complexity: str, base_cost: Decimal
    ) -> Decimal:
        """计算风险储备金"""
    ...
    async def _generate_optimization_suggestions(
        self, input_data: CostEstimationInput, cost_breakdown: Dict[str, Decimal]
    ...
    def _generate_pricing_recommendations(
        self, total_cost: Decimal, target_margin_rate: Decimal
    ) -> PricingRecommendation:
        """生成定价推荐"""
        # 基于目标毛利率计算建议价格
        ...
        historical_count = (
            ...
            .filter(
                PresaleCostHistory.project_features.contains(
                    f'"project_type": "{input_data.project_type}"'
                )
            )
            .count()
        )
    ...
    async def optimize_cost(self, input_data: CostOptimizationInput) -> CostOptimizationResponse:
        ...
        total_saving_rate = (
            (total_saving / original_total * Decimal("100")) if original_total > 0 else Decimal("0")
        )
    ...
    def _analyze_price_sensitivity(
        self, cost: Decimal, pricing: PricingRecommendation, customer_budget: Optional[Decimal]
    ) -> Dict[str, Any]:
        """价格敏感度分析"""
        analysis = {
    ...
    def _calculate_competitiveness(
        self, pricing: PricingRecommendation, customer_budget: Optional[Decimal]
    ) -> Decimal:
        """计算竞争力评分"""
    ...
    async def get_historical_accuracy(self) -> HistoricalAccuracyResponse:
        ...
        accuracy_rate = (
            Decimal(str(accurate_count)) / Decimal(str(len(variance_rates))) * Decimal("100")
            if variance_rates
            else Decimal("0")
        )

...
class SalesTargetService:
    ...
    @staticmethod
    def create_target(
        db: Session, target_data: SalesTargetV2Create, created_by: int
    ) -> SalesTargetV2:
        """创建销售目标"""
        # 验证目标类型和对应的ID
        ...
        existing = (
            ...
            .filter(
                and_(
                    ...
                    or_(
                        (
                            SalesTargetV2.team_id == target_data.team_id
                            if target_data.team_id
                            else False
                        ),
                        (
                            SalesTargetV2.user_id == target_data.user_id
                            if target_data.user_id
                            else False
                        ),
                    ),
                )
    ...
    @staticmethod
    def update_target(
        db: Session, target_id: int, target_data: SalesTargetV2Update
    ) -> SalesTargetV2:
        """更新目标"""
    ...
    def delete_target(db: Session, target_id: int) -> bool:
        ...
        # 检查是否有子目标
        sub_targets = (
            db.query(SalesTargetV2).filter(SalesTargetV2.parent_target_id == target_id).count()
        )
        if sub_targets > 0:
    ...
    def breakdown_target(
        db: Session, target_id: int, breakdown_data: TargetBreakdownRequest, created_by: int
    ) -> List[SalesTargetV2]:
        """手动分解目标"""
    ...
    def auto_breakdown_target(
        db: Session, target_id: int, breakdown_data: AutoBreakdownRequest, created_by: int
    ) -> List[SalesTargetV2]:
        """自动分解目标"""
        parent_target = get_or_404(db, SalesTargetV2, target_id, detail="上级目标不存在")
    ...
    def get_breakdown_tree(db: Session, target_id: int) -> Dict[str, Any]:
        ...
        def build_tree(target_obj: SalesTargetV2) -> Dict[str, Any]:
            sub_targets = (
                db.query(SalesTargetV2)
                .filter(SalesTargetV2.parent_target_id == target_obj.id)
                .all()
            )
    ...
    def get_team_ranking(
        db: Session, target_year: int, target_month: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """获取团队排名"""
        query = db.query(SalesTargetV2).filter(
            and_(SalesTargetV2.target_type == "team", SalesTargetV2.target_year == target_year)
        )
    ...
    def get_personal_ranking(
        ...
        """获取个人排名"""
        query = db.query(SalesTargetV2).filter(
            and_(SalesTargetV2.target_type == "personal", SalesTargetV2.target_year == target_year)
        )
    ...
    @staticmethod
    def get_completion_trend(db: Session, target_id: int) -> List[Dict[str, Any]]:

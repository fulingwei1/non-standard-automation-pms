...
class SalesTeamService:
    ...
    def get_team_members(
        db: Session, team_id: int, is_active: Optional[bool] = True
    ...
    def update_member_role(
        db: Session, team_id: int, user_id: int, member_data: SalesTeamMemberUpdate
    ) -> SalesTeamMember:
        """更新成员角色"""
        member = (
            db.query(SalesTeamMember)
...
class SalesRegionService:
    ...
    def create_region(db: Session, region_data: SalesRegionCreate, created_by: int) -> SalesRegion:
        ...
        # 检查区域编码是否已存在
        existing = (
            db.query(SalesRegion).filter(SalesRegion.region_code == region_data.region_code).first()
        )
        if existing:
            raise HTTPException(
                status_code=400, detail=f"区域编码 {region_data.region_code} 已存在"
            )
    ...
    def get_regions(
        ...
        return (
            query.order_by(SalesRegion.sort_order, SalesRegion.id).offset(skip).limit(limit).all()
        )
    ...
    @staticmethod
    def assign_team(
        db: Session, region_id: int, team_id: int, leader_id: Optional[int] = None
    ) -> SalesRegion:
        """分配团队"""
        region = get_or_404(db, SalesRegion, region_id, detail="区域不存在")

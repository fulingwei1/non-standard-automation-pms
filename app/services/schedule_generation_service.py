...
class ScheduleGenerationService:
    ...
    def _analyze_historical_projects(self, project: Project) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence": (
                "HIGH" if len(durations) >= 5 else "MEDIUM" if len(durations) >= 3 else "LOW"
            ),
        }
    ...
    def _calculate_efficiency_factors(
        ...
        avg_multi_project = (
            sum(multi_project_efficiencies) / len(multi_project_efficiencies)
            if multi_project_efficiencies
            else 1.0
        )
    ...
    def _schedule_tasks(
        ...
        for phase_key, phase in phases_tasks.items():
            ...
            for task in phase["tasks"]:
                task_duration = int(
                    phase_duration * task["adjusted_duration_factor"] / total_factor
                )

...
class ScheduleOptimizationService:
            ...
            reusable["design_documents"].append(
                {
                    ...
                    "similarity": (
                        "HIGH" if sp.product_category == project.product_category else "MEDIUM"
                    ),
    ...
    def _calculate_time_savings(self, optimization_analysis: Dict) -> Dict[str, Any]:
        ...
        total_savings = sum(
            phase.get("potential_savings", 0) for phase in optimization_analysis.values()
        )

        total_current = sum(
            phase.get("current_duration", 0) for phase in optimization_analysis.values()
        )
        ...
        return {
            ...
            "savings_percentage": (
                round(total_savings / total_current * 100, 1) if total_current > 0 else 0
            ),
            "by_phase": {

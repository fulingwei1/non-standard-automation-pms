...
class SchedulingSuggestionService:
    ...
    def calculate_priority_score(
        cls, db: Session, project: Project, readiness: Optional[MaterialReadiness] = None
    ) -> Dict:
        """
        计算项目优先级评分
        ...
        return {"total_score": round(score, 2), "factors": factors, "max_score": 100}
    ...
    def generate_scheduling_suggestions(
        cls, db: Session, scope: str = "WEEKLY", project_ids: Optional[List[int]] = None
    ) -> List[Dict]:
        """
        生成智能排产建议
        ...
        for project in projects:
            ...
            readiness = (
                ...
                .filter(MaterialReadiness.project_id == project.id)
                .order_by(MaterialReadiness.analysis_time.desc())
                .first()
            )
            ...
            if readiness.can_start:
                ...
                suggestion = {
                    ...
                    "suggestion_type": (
                        "CAN_START"
                        if status == "READY"
                        else ("WAIT_RESOURCE" if status == "WAIT_RESOURCE" else "WAIT_MATERIAL")
                    ),
                }
            else:
                # 不能排产
    ...
    def _get_next_stage(cls, current_stage: str) -> Optional[str]:
        """获取下一阶段"""
        stage_order = {"FRAME": 1, "MECH": 2, "ELECTRIC": 3, "WIRING": 4, "DEBUG": 5, "COSMETIC": 6}
    ...
    def _get_blocking_items(
        cls, db: Session, readiness: MaterialReadiness, stage_code: str
    ) -> List[Dict]:
        """获取阻塞物料列表"""

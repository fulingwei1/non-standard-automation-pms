...
class ServiceTicketsService:
    ...
    def get_dashboard_statistics(self) -> ServiceDashboardStatistics:
        ...
        today_tickets = (
            ...
            .filter(func.date(ServiceTicket.created_at) == today)
            .count()
        )
            ...
            "members": [
                {
                    ...
                    "department": (
                        user.department
                        if isinstance(getattr(user, "department", None), str)
                        else (user.department.name if user.department else None)
                    ),
    ...
    def get_ticket_statistics(
        ...
        # 状态分布
        status_stats = (
            query.with_entities(ServiceTicket.status, func.count(ServiceTicket.id).label("count"))
        ...
        # 优先级分布
        priority_stats = (
            query.with_entities(ServiceTicket.priority, func.count(ServiceTicket.id).label("count"))
    ...
    def get_service_tickets(
        ...
        """获取服务工单列表"""
        query = self.db.query(ServiceTicket).options(joinedload(ServiceTicket.customer))
    ...
    def create_service_ticket(self, ticket_data, current_user=None) -> ServiceTicket:
        """创建服务工单"""
        # 生成工单编号
        ticket_type = getattr(ticket_data, "ticket_type", None) or getattr(
            ticket_data, "problem_type", "other"
        )
        ...
        service_ticket = ServiceTicket(
            ...
            problem_desc=getattr(ticket_data, "description", None)
            or getattr(ticket_data, "problem_desc", ""),
    ...
    def assign_ticket(
        self, ticket_id: int, assign_data=None, current_user=None
    ...
    def update_ticket_status(
        self, ticket_id: int, status: str, note: Optional[str] = None, current_user=None
    ) -> Optional[ServiceTicket]:
        """更新工单状态"""
        service_ticket = self.get_service_ticket(ticket_id)
        if not service_ticket:
    ...
    def close_ticket(
        self, ticket_id: int, close_data=None, current_user=None
    ) -> Optional[ServiceTicket]:
        """关闭服务工单"""
        service_ticket = self.get_service_ticket(ticket_id)
        if not service_ticket:
        ...
        service_ticket.solution = getattr(close_data, "solution", None) or getattr(
            close_data, "resolution_summary", None
        )
        service_ticket.customer_feedback = getattr(
            close_data, "customer_feedback", None
        ) or getattr(close_data, "feedback", None)
    ...
    def _auto_assign_ticket(self, service_ticket: ServiceTicket):
        ...
        try:
            ...
            if not engineers:
                logger.warning(
                    "未找到可分配工程师 (ticket_id=%s, type=%s, dept=%s)",
                    service_ticket.id,
                    service_ticket.ticket_type,
                    target_dept,
                )
                return
            ...
            if best_engineer:
                ...
                self.db.refresh(service_ticket)
                logger.info(
                    "工单已自动分配 (ticket_id=%s, engineer_id=%s, load=%d)",
                    service_ticket.id,
                    best_engineer.id,
                    min_load,
                )
    ...
    def _send_ticket_notification(self, service_ticket: ServiceTicket, action: str):
        ...
        try:
            ...
            logger.info(
                "工单通知已发送 (ticket_id=%s, action=%s, recipients=%s)",
                service_ticket.id,
                action,
                recipient_ids,
            )
            ...
            logger.error(
                "发送工单通知失败 (ticket_id=%s, action=%s): %s", service_ticket.id, action, str(e)
            )
    ...
    def _create_satisfaction_survey(self, service_ticket: ServiceTicket):
        ...
        try:
            ...
            count = (
                ...
                .filter(func.date(CustomerSatisfaction.created_at) == today)
                .count()
            )
            ...
            customer_name = (
                getattr(customer, "name", None) or getattr(customer, "customer_name", "未知客户")
                if customer
                else "未知客户"
            )
            ...
            self.db.commit()
            logger.info(
                "满意度调查已创建 (ticket_id=%s, survey_no=%s)", service_ticket.id, survey_no
            )
        except Exception as e:
    ...
    def _calculate_avg_response_time(self) -> float:
        ...
        response_times = (
            self.db.query(ServiceTicket)
            .filter(
                ServiceTicket.created_at >= thirty_days_ago, ServiceTicket.assigned_time.isnot(None)
            )
            .with_entities(
    ...
    def _calculate_satisfaction_rate(self) -> float:
        ...
        satisfactions = (
            ...
            .with_entities(CustomerSatisfaction.overall_score)
            .all()
        )

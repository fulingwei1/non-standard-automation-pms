...
class SessionService:
    ...
    def create_session(
        ...
        # 检测是否为可疑登录
        is_suspicious, risk_score = cls._assess_risk(db, user_id, ip_address, device_info, location)
    ...
    def get_session_by_jti(
        ...
        if token_type == "access":
            return db.query(UserSession).filter(UserSession.access_token_jti == jti).first()
        else:
            return db.query(UserSession).filter(UserSession.refresh_token_jti == jti).first()
    ...
    def revoke_session(
        ...
        # 将tokens加入黑名单
        cls._add_to_blacklist(session.access_token_jti, cls.ACCESS_TOKEN_EXPIRE_MINUTES * 60)
        cls._add_to_blacklist(session.refresh_token_jti, cls.REFRESH_TOKEN_EXPIRE_DAYS * 24 * 3600)
    ...
    def revoke_all_sessions(
        ...
        for session in sessions:
            ...
            cls._add_to_blacklist(
                session.refresh_token_jti, cls.REFRESH_TOKEN_EXPIRE_DAYS * 24 * 3600
            )
    ...
    def _assess_risk(
        ...
        if device_info and device_info.get("device_id"):
            known_devices = {s.device_id for s in recent_sessions if s.device_id}
        ...
        # 检查登录频率（防止暴力破解）
        recent_logins = [
            s for s in recent_sessions if s.login_at > datetime.utcnow() - timedelta(hours=1)
        ]
    ...
    def _cleanup_old_sessions(cls, db: Session, user_id: int):
        ...
        if len(active_sessions) >= cls.MAX_SESSIONS_PER_USER:
            ...
            for session in sessions_to_revoke:
                ...
                session.logout_at = datetime.utcnow()
                cls._add_to_blacklist(
                    session.access_token_jti, cls.ACCESS_TOKEN_EXPIRE_MINUTES * 60
                )
                cls._add_to_blacklist(
                    session.refresh_token_jti, cls.REFRESH_TOKEN_EXPIRE_DAYS * 24 * 3600
                )

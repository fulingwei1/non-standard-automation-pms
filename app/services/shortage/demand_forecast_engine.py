...
class DemandForecastEngine:
    ...
    def forecast_material_demand(
        ...
        historical_data = self._collect_historical_demand(
            material_id=material_id, days=historical_days, project_id=project_id
        )
        ...
        lower_bound, upper_bound = self._calculate_confidence_interval(
            forecast=forecasted_demand, std=historical_std, confidence=confidence_interval
        )
        ...
        forecast = MaterialDemandForecast(
            ...
            seasonal_pattern={"detected": seasonal_factor != 1.0, "factor": float(seasonal_factor)},
            status="ACTIVE",
    ...
    def validate_forecast_accuracy(self, forecast_id: int, actual_demand: Decimal) -> Dict:
        """
        验证预测准确率
        ...
        """
        forecast = (
            self.db.query(MaterialDemandForecast)
            .filter(MaterialDemandForecast.id == forecast_id)
            .first()
        )
        ...
        error_percentage = (
            abs(float(forecast_error / actual_demand * 100)) if actual_demand > 0 else 0
        )
    ...
    def _collect_historical_demand(
        self, material_id: int, days: int, project_id: Optional[int]
    ) -> List[Decimal]:
        """
        收集历史需求数据
    ...
    def _moving_average_forecast(self, data: List[Decimal], window: int = 7) -> Decimal:
        """
        移动平均预测
    ...
    def _exponential_smoothing_forecast(
        self, data: List[Decimal], alpha: Decimal = Decimal("0.3")
    ) -> Decimal:
        """
        指数平滑预测
    ...
    def _calculate_confidence_interval(
        self, forecast: Decimal, std: Decimal, confidence: float
    ...
    def _generate_forecast_no(self) -> str:
        ...
        count = (
            self.db.query(func.count(MaterialDemandForecast.id))
            .filter(MaterialDemandForecast.forecast_date == datetime.now().date())
            .scalar()
            or 0
        )
    ...
    def batch_forecast_for_project(
        self, project_id: int, forecast_horizon_days: int = 30
        ...
        # 获取项目所需的所有物料
        material_ids = (
            self.db.query(WorkOrder.material_id.distinct())
            .filter(
                and_(
    ...
    def get_forecast_accuracy_report(
        self, material_id: Optional[int] = None, days: int = 30
    ) -> Dict:
        """
        获取预测准确率报告
        ...
        within_ci_count = sum(
            1 for f in forecasts if f.lower_bound <= f.actual_demand <= f.upper_bound
        )
        ...
        for forecast in forecasts:
            ...
            if algo not in by_algorithm:
                by_algorithm[algo] = {"count": 0, "avg_accuracy": 0, "avg_mape": 0}

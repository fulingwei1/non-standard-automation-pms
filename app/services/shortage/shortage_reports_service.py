...
class ShortageReportsService:
    ...
    def create_shortage_report(
        self, report_data: ShortageReportCreate, current_user: User
    ) -> ShortageReport:
        """创建缺料报告"""
    ...
    def handle_shortage_report(
        self, report_id: int, handle_data: dict, current_user: User
    ) -> Optional[ShortageReport]:
        """处理缺料报告"""
        shortage_report = self.get_shortage_report(report_id)
        if not shortage_report:
    ...
    def resolve_shortage_report(
        self, report_id: int, resolve_data: dict, current_user: User
    ) -> Optional[ShortageReport]:
        """解决缺料报告"""
        shortage_report = self.get_shortage_report(report_id)
        if not shortage_report:
...
def calculate_alert_statistics(db: Session, target_date: date) -> Dict[str, Any]:
    ...
    """
    # 基础查询：筛选缺料类型预警
    base_query = db.query(AlertRecord).filter(AlertRecord.target_type == "SHORTAGE")

    new_alerts = base_query.filter(func.date(AlertRecord.created_at) == target_date).count()

    resolved_alerts = base_query.filter(
        AlertRecord.handle_end_at.isnot(None), func.date(AlertRecord.handle_end_at) == target_date
    ).count()
...
def calculate_report_statistics(db: Session, target_date: date) -> Dict[str, int]:
    """
    计算上报统计
    ...
    """
    new_reports = (
        db.query(func.count(ShortageReport.id))
        .filter(func.date(ShortageReport.report_time) == target_date)
        .scalar()
        or 0
    )
    ...
    return {"new_reports": new_reports, "resolved_reports": resolved_reports}


def calculate_kit_statistics(db: Session, target_date: date) -> Dict[str, Any]:
    """
    计算齐套统计
    ...
    Returns:
        齐套统计数据字典
    """
    kit_checks = db.query(KitCheck).filter(func.date(KitCheck.check_time) == target_date).all()

    total_work_orders = len(kit_checks)
    kit_complete_count = len([k for k in kit_checks if (k.kit_status or "").lower() == "complete"])
    kit_rate = (
        round(sum(float(k.kit_rate or 0) for k in kit_checks) / total_work_orders, 2)
...
def calculate_arrival_statistics(db: Session, target_date: date) -> Dict[str, Any]:
    """
    计算到货统计
    ...
    """
    expected_arrivals = (
        db.query(func.count(MaterialArrival.id))
        .filter(MaterialArrival.expected_date == target_date)
        .scalar()
        or 0
    )
    ...
    actual_arrivals = (
        db.query(func.count(MaterialArrival.id))
        .filter(MaterialArrival.actual_date == target_date)
        .scalar()
        or 0
    )
    ...
    delayed_arrivals = (
        db.query(func.count(MaterialArrival.id))
        .filter(MaterialArrival.actual_date == target_date, MaterialArrival.is_delayed)
        .scalar()
        or 0
    )

    on_time_rate = (
        round(((actual_arrivals - delayed_arrivals) / actual_arrivals) * 100, 2)
        if actual_arrivals
        else 0.0
    )
...
def calculate_response_time_statistics(db: Session, target_date: date) -> Dict[str, Any]:
    """
    ...
    alerts_for_response = (
        ...
        .filter(
            AlertRecord.target_type == "SHORTAGE", func.date(AlertRecord.created_at) == target_date
        )
        .all()
    )
    ...
    avg_response_minutes = (
        int(round(sum(response_minutes) / len(response_minutes), 0)) if response_minutes else 0
    )

    resolved_alerts_list = (
        db.query(AlertRecord)
        .filter(
    ...
    avg_resolve_hours = round(sum(resolve_hours) / len(resolve_hours), 2) if resolve_hours else 0.0

    return {"avg_response_minutes": avg_response_minutes, "avg_resolve_hours": avg_resolve_hours}


def calculate_stoppage_statistics(db: Session, target_date: date) -> Dict[str, Any]:
    """
    ...
    return {"stoppage_count": stoppage_count, "stoppage_hours": stoppage_hours}


def build_daily_report_data(db: Session, target_date: date) -> Dict[str, Any]:
    """
    构建缺料日报数据

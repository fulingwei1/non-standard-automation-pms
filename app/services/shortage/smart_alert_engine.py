...
class SmartAlertEngine:
    ...
    def scan_and_alert(
        ...
        """
        logger.info(
            f"开始扫描缺料预警: project_id={project_id}, material_id={material_id}, days_ahead={days_ahead}"
        )
        ...
        demands = self._collect_material_demands(
            project_id=project_id, material_id=material_id, days_ahead=days_ahead
        )
    ...
    def predict_impact(
        ...
        impact["estimated_delay_days"] = max(
            0, avg_lead_time - (required_date - datetime.now().date()).days
        )
    ...
    def _collect_material_demands(
        self, project_id: Optional[int], material_id: Optional[int], days_ahead: int
    ) -> List[Dict]:
        """收集物料需求数据"""
        demands = []
    ...
    def _get_average_lead_time(self, material_id: int) -> int:
        ...
        result = (
            ...
            .join(PurchaseOrder, PurchaseOrder.id == GoodsReceipt.order_id)
    ...
    def _calculate_risk_score(
        self, delay_days: int, cost_impact: Decimal, project_count: int, shortage_qty: Decimal
    ) -> Decimal:
        """计算风险评分 0-100"""
        score = 0
    ...
    def _generate_alert_no(self) -> str:
        ...
        count = (
            self.db.query(func.count(ShortageAlert.id))
            .filter(ShortageAlert.alert_date == datetime.now().date())
            .scalar()
            or 0
        )
    ...
    def _generate_urgent_purchase_plan(
        self, alert: ShortageAlert
    ) -> Optional[ShortageHandlingPlan]:
        """生成紧急采购方案"""
    ...
    def _score_cost(self, solution: ShortageHandlingPlan, alert: ShortageAlert) -> Decimal:
        ...
        cost_ratio = (
            float(solution.estimated_cost / alert.estimated_cost_impact)
            if alert.estimated_cost_impact > 0
            else 1
        )
        if cost_ratio < 0.5:
            return Decimal("100")
    ...
    def _generate_plan_no(self) -> str:
        ...
        today = datetime.now().strftime("%Y%m%d")
        count = (
            self.db.query(func.count(ShortageHandlingPlan.id))
            .filter(func.date(ShortageHandlingPlan.created_at) == datetime.now().date())
            .scalar()
            or 0
        )

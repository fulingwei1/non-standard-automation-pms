...
class ShortageAlertService:
    ...
    def get_alerts_with_filters(
        ...
        # 分页查询
        alerts = (
            query.order_by(desc(ShortageAlert.created_at))
    ...
    def get_handling_solutions(self, alert_id: int) -> List[ShortageHandlingPlan]:
        ...
        # 查询现有处理方案
        plans = (
            self.db.query(ShortageHandlingPlan)
            .filter(ShortageHandlingPlan.alert_id == alert_id)
            .order_by(desc(ShortageHandlingPlan.ai_score))
            .all()
        )
    ...
    def analyze_shortage_trend(
        self, days: int = 30, project_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        缺料趋势分析
        ...
        query = self.db.query(ShortageAlert).filter(ShortageAlert.alert_date >= start_date)
        ...
        avg_resolution_hours = total_resolution_hours / resolved_count if resolved_count > 0 else 0

        # 每日趋势数据
        trend_data = []
        current_date = start_date
        while current_date <= datetime.now().date():
            daily_alerts = [a for a in alerts if a.alert_date == current_date]
    ...
    def analyze_root_cause(self, days: int = 30) -> Dict[str, Any]:
        ...
        alerts = self.db.query(ShortageAlert).filter(ShortageAlert.alert_date >= start_date).all()
    ...
    def analyze_project_impact(
        self, alert_level: Optional[str] = None, status: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        缺料对项目的影响分析
        ...
        ).filter(ShortageAlert.status.in_(["PENDING", "PROCESSING"]))
        ...
        for row in results:
            ...
            # 获取关键物料
            critical_materials_query = (
                self.db.query(ShortageAlert.material_name)
                .filter(
                    and_(

...
class ShortageAnalyticsService:
    ...
    def get_dashboard_data(self, project_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        critical_alerts = alert_query.filter(
            MaterialShortage.alert_level == "CRITICAL", MaterialShortage.status != "RESOLVED"
        ).count()
        ...
        return {
            ...
            "substitutions": {"total": total_substitutions, "pending": pending_substitutions},
    ...
    def get_daily_report(
        self, report_date: Optional[date] = None, project_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        获取缺料日报（实时计算）
        ...
        for stats in by_project.values():
            project_list.append({**stats, "total_shortage_qty": float(stats["total_shortage_qty"])})
    ...
    def get_latest_daily_report(self) -> Optional[Dict[str, Any]]:
        ...
        report = (
            self.db.query(ShortageDailyReport)
            .filter(ShortageDailyReport.report_date == latest_date)
            .first()
        )
    ...
    def _build_shortage_daily_report(report: ShortageDailyReport) -> Dict[str, Any]:
        ...
        return {
            ...
            "response": {
                ...
                "avg_resolve_hours": (
                    float(report.avg_resolve_hours) if report.avg_resolve_hours else 0.0
                ),
            },
            "stoppage": {

...
def get_project_requirements(db: Session, project_id: int) -> List[TechnicalSpecRequirement]:
    """
    获取项目的所有规格要求
    ...
    return (
        ...
        .filter(TechnicalSpecRequirement.project_id == project_id)
        .all()
    )
...
def check_all_po_items(
    db: Session, project_id: int, requirements: List[TechnicalSpecRequirement], matcher: SpecMatcher
) -> List[SpecMatchResult]:
    """
    检查所有采购订单项
    ...
    po_items = (
        ...
        .join(PurchaseOrderItem.order)
        .filter(PurchaseOrderItem.order.has(project_id=project_id))
        .all()
    )
...
def check_all_bom_items(
    ...
    bom_items = (
        db.query(BomItem)
        .join(BomItem.header)
        .filter(BomItem.header.has(project_id=project_id))
        .all()
    )

...
class ProfileAggregator(StaffMatchingBase):
    ...
    def aggregate_employee_profile(cls, db: Session, employee_id: int) -> HrEmployeeProfile:
        ...
        if performances:
            ...
            if quality_scores:
                profile.quality_score = Decimal(
                    str(round(sum(quality_scores) / len(quality_scores), 2))
                )
            ...
            if perf_scores:
                profile.avg_performance_score = Decimal(
                    str(round(sum(perf_scores) / len(perf_scores), 2))
                )
        ...
        if user:
            ...
            active_assignments = (
                ...
                .filter(
                    ...
                    or_(ProjectMember.end_date is None, ProjectMember.end_date >= today),
                )
                .all()
            )

            total_allocation = sum(float(a.allocation_pct or 0) for a in active_assignments)

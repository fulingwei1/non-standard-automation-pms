...
class HelpersMixin:
    ...
    def _clear_default_template(self, project_type: str) -> None:
        ...
        self.db.query(StageTemplate).filter(
            and_(StageTemplate.project_type == project_type, StageTemplate.is_default)
    ...
    def _has_circular_dependency(
        self, node_id: int, new_dependency_ids: List[int], template_id: int
    ) -> bool:
        """检测是否存在循环依赖"""
        visited = set()
        ...
        def dfs(current_id: int) -> bool:
            ...
            node = self.db.query(NodeDefinition).filter(NodeDefinition.id == current_id).first()
    ...
    def _get_dependency_codes(self, node: NodeDefinition) -> List[str]:
        ...
        dep_nodes = (
            self.db.query(NodeDefinition)
            .filter(NodeDefinition.id.in_(node.dependency_node_ids))
            .all()
        )

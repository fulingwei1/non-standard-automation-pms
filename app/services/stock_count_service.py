...
class StockCountService:
    ...
    def get_count_task(self, task_id: int) -> Optional[StockCountTask]:
        """获取盘点任务详情"""
        return (
            self.db.query(StockCountTask)
            .filter(StockCountTask.id == task_id, StockCountTask.tenant_id == self.tenant_id)
            .first()
        )
    ...
    def get_count_details(self, task_id: int) -> List[StockCountDetail]:
        """获取盘点明细列表"""
        return (
            self.db.query(StockCountDetail)
            .filter(
                StockCountDetail.task_id == task_id, StockCountDetail.tenant_id == self.tenant_id
            )
            .all()
        )
    ...
    def record_actual_quantity(
        ...
        detail = (
            self.db.query(StockCountDetail)
            .filter(StockCountDetail.id == detail_id, StockCountDetail.tenant_id == self.tenant_id)
            .first()
        )
    ...
    def batch_record_quantities(
        self, records: List[Dict], counted_by: Optional[int] = None
    ) -> List[StockCountDetail]:
        """批量录入实盘数量"""
    ...
    def mark_for_recount(self, detail_id: int, recount_reason: str) -> StockCountDetail:
        """标记需要复盘"""
    ...
    def _update_task_statistics(self, task_id: int):
        ...
        task.total_diff_value = sum(d.diff_value or Decimal(0) for d in details)

    # ============ 盘点审批和调整 ============

    def approve_adjustment(self, task_id: int, approved_by: int, auto_adjust: bool = True) -> Dict:
        """批准库存调整"""
        ...
        for detail in details:
            if detail.difference != 0:
                ...
                adjustment = self._create_adjustment(
                    detail=detail, task=task, approved_by=approved_by
                )
    ...
    def _create_adjustment(
        self, detail: StockCountDetail, task: StockCountTask, approved_by: int
    ) -> StockAdjustment:
        """创建库存调整记录"""
    ...
    def get_count_summary(self, task_id: int) -> Dict:
        ...
        return {
            ...
            "value_analysis": {
                ...
                "accuracy_rate": (
                    float(len(matched_items) / task.total_items * 100)
    ...
    def _get_top_differences(self, details: List[StockCountDetail], limit: int = 10) -> List[Dict]:
        """获取差异最大的物料"""
    ...
    def analyze_count_history(
        ...
        query = self.db.query(StockCountTask).filter(
            StockCountTask.tenant_id == self.tenant_id, StockCountTask.status == "COMPLETED"
        )
            ...
            "trend": [
                {
                    ...
                    "accuracy_rate": (
                        float(t.matched_items / t.total_items * 100) if t.total_items > 0 else 0
                    ),

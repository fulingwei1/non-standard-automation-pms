...
class SupplierPerformanceEvaluator:
    ...
    def evaluate_supplier(
        ...
        performance.detail_data = {
            "delivery": {
                k: float(v) if isinstance(v, Decimal) else v for k, v in delivery_metrics.items()
            },
            "quality": {
                k: float(v) if isinstance(v, Decimal) else v for k, v in quality_metrics.items()
            },
            "price": {
                k: float(v) if isinstance(v, Decimal) else v for k, v in price_metrics.items()
            },
            "response": {
                k: float(v) if isinstance(v, Decimal) else v for k, v in response_metrics.items()
            },
        }
    ...
    def _calculate_delivery_metrics(
        self, orders: List[PurchaseOrder], period_start: date, period_end: date
    ) -> Dict:
        """计算准时交货指标"""
        if not orders:
            return {
        ...
        if total_delivered > 0:
            ...
            avg_delay_days = (
                total_delay_days / Decimal(late_orders) if late_orders > 0 else Decimal("0")
            )
        else:
            on_time_rate = Decimal("0")
            avg_delay_days = Decimal("0")
    ...
    def _calculate_price_competitiveness(
        self, supplier_id: int, period_start: date, period_end: date
    ) -> Dict:
        """计算价格竞争力"""
        # 获取该供应商在此期间的平均价格
        supplier_avg_price = (
            self.db.query(func.avg(PurchaseOrderItem.unit_price))
            .join(PurchaseOrder)
            .filter(
                and_(
                    PurchaseOrder.supplier_id == supplier_id,
    ...
    def _calculate_response_speed(
        ...
        if avg_hours <= Decimal("4"):  # 4小时内
            ...
            score = max(
                Decimal("30"),
                Decimal("60") - (avg_hours - Decimal("48")) / Decimal("24") * Decimal("5"),
            )
    ...
    def get_supplier_ranking(
        self, evaluation_period: str, limit: int = 10
        ...
        performances = (
            ...
            .order_by(SupplierPerformance.overall_score.desc())
            .limit(limit)
            .all()
        )
    ...
    def batch_evaluate_all_suppliers(self, evaluation_period: str) -> int:
        ...
        suppliers = (
            ...
            .filter(and_(Vendor.status == "ACTIVE", Vendor.vendor_type == "MATERIAL"))
            .all()
        )

...
class CompanyReportMixin:
    ...
    def _generate_company_monthly(
        ...
        for dept in departments:
            users = db.query(User).filter(User.department_id == dept.id, User.is_active).all()
            ...
            if total_hours > 0:
                dept_hours.append({"department": dept.name, "hours": round(total_hours, 2)})
        ...
        return {
            ...
            "sections": {
                ...
                "department_hours": {"title": "部门工时统计", "type": "table", "data": dept_hours},
            },
            "metrics": {
                "total_projects": len(projects),
                "active_projects": status_counts.get("IN_PROGRESS", 0),
                "health_projects": sum(v for k, v in health_counts.items() if k in ["H1", "H2"]),
            },
        }

...
class OvertimeCalculationService:
    ...
    def calculate_overtime_pay(
        self, user_id: int, work_date: date, hours: Decimal, overtime_type: str
    ) -> Decimal:
        """
        计算单个工时记录的加班工资
            ...
            daily_records.append(
                {
                    ...
                    "overtime_pay": float(
                        self.calculate_overtime_pay(user_id, ts.work_date, hours, ts.overtime_type)
                    ),
    ...
    def get_overtime_statistics(
        ...
        total_overtime_hours = sum(
            float(ts.hours or 0) for ts in timesheets if ts.overtime_type != "NORMAL"
        )
        total_overtime_pay = Decimal("0")
        ...
        for ts in timesheets:
            if ts.overtime_type != "NORMAL":
                ...
                pay = self.calculate_overtime_pay(
                    ts.user_id, ts.work_date, Decimal(str(ts.hours or 0)), ts.overtime_type
                )

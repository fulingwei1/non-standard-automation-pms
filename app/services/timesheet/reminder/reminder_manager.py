...
class TimesheetReminderManager:
    ...
    def update_reminder_config(self, config_id: int, **kwargs) -> Optional[TimesheetReminderConfig]:
        ...
        config = (
            ...
            .filter(TimesheetReminderConfig.id == config_id)
            .first()
        )
    ...
    def get_active_configs_by_type(
        self, reminder_type: ReminderTypeEnum
    ...
    def check_user_applicable(
        ...
        # 如果没有指定适用范围，则适用所有用户
        if (
            not config.apply_to_users
            and not config.apply_to_departments
            and not config.apply_to_roles
        ):
            return True
    ...
    def mark_reminder_sent(
        self, reminder_id: int, channels: List[str]
    ) -> Optional[TimesheetReminderRecord]:
        """标记提醒已发送"""
        record = (
            self.db.query(TimesheetReminderRecord)
            .filter(TimesheetReminderRecord.id == reminder_id)
            .first()
        )
    ...
    def dismiss_reminder(
        self, reminder_id: int, dismissed_by: int, reason: Optional[str] = None
    ) -> Optional[TimesheetReminderRecord]:
        """忽略提醒"""
        record = (
            self.db.query(TimesheetReminderRecord)
    ...
    def mark_reminder_resolved(self, reminder_id: int) -> Optional[TimesheetReminderRecord]:
        """标记提醒已解决"""
        record = (
            self.db.query(TimesheetReminderRecord)
            .filter(TimesheetReminderRecord.id == reminder_id)
    ...
    def get_pending_reminders(
        ...
        return (
            query.order_by(
                desc(TimesheetReminderRecord.priority), desc(TimesheetReminderRecord.created_at)
            )
            .limit(limit)
            .all()
        )
    ...
    def check_reminder_limit(
        self, user_id: int, reminder_type: ReminderTypeEnum, max_per_day: int
    ) -> bool:
        """检查提醒次数限制"""
    ...
    def resolve_anomaly(
        self, anomaly_id: int, resolved_by: int, resolution_note: Optional[str] = None
    ) -> Optional[TimesheetAnomalyRecord]:
        """解决异常"""
        record = (
            self.db.query(TimesheetAnomalyRecord)
            .filter(TimesheetAnomalyRecord.id == anomaly_id)
            .first()
        )
    ...
    def get_unresolved_anomalies(
        ...
        return query.order_by(desc(TimesheetAnomalyRecord.detected_at)).limit(limit).all()

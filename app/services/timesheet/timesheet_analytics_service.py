...
class TimesheetAnalyticsService:
    ...
    def analyze_trend(
        ...
        query = self.db.query(
            ...
            func.sum(case((Timesheet.overtime_type == "NORMAL", Timesheet.hours), else_=0)).label(
                "normal_hours"
            ),
            func.sum(case((Timesheet.overtime_type != "NORMAL", Timesheet.hours), else_=0)).label(
                "overtime_hours"
            ),
        ).filter(Timesheet.work_date.between(start_date, end_date), Timesheet.status == "APPROVED")
        ...
        if period_type == "DAILY":
            ...
            # 按月分组
            query = query.group_by(func.year(Timesheet.work_date), func.month(Timesheet.work_date))
        elif period_type == "QUARTERLY":
            # 按季度分组
            query = query.group_by(
                func.year(Timesheet.work_date), func.quarter(Timesheet.work_date)
            )
    ...
    def _calculate_trend(self, results: List) -> Tuple[str, float]:
        ...
        mid = len(results) // 2
        first_half_avg = (
            sum(r.total_hours for r in results[:mid] if r.total_hours) / mid if mid > 0 else 0
        )
        second_half_avg = sum(r.total_hours for r in results[mid:] if r.total_hours) / (
            len(results) - mid
        )
    ...
    def analyze_workload(
        ...
        query = query.group_by(
            Timesheet.user_id, Timesheet.user_name, Timesheet.department_name, Timesheet.work_date
        ...
        for user_id, workload in user_workload.items():
            ...
            # 计算饱和度
            saturation = (
                (workload["total_hours"] / standard_total_hours * 100)
                if standard_total_hours > 0
                else 0
            )
        ...
        heatmap_data = HeatmapData(rows=rows, columns=columns, data=data)

        # 统计信息
        total_users = len(user_workload)
        avg_hours = (
            sum(w["total_hours"] for w in user_workload.values()) / total_users
            if total_users > 0
            else 0
        )
        ...
        statistics = {
            ...
            "overload_rate": (
                round(len(overload_users) / total_users * 100, 2) if total_users > 0 else 0
            ),
        }
    ...
    def analyze_efficiency(
        ...
        """
        # 查询实际工时
        query = self.db.query(func.sum(Timesheet.hours).label("actual_hours")).filter(
            Timesheet.work_date.between(start_date, end_date), Timesheet.status == "APPROVED"
        )
    ...
    def analyze_overtime(
        ...
        query = self.db.query(
            ...
            func.sum(case((Timesheet.overtime_type == "WEEKEND", Timesheet.hours), else_=0)).label(
                "weekend_hours"
            ),
            func.sum(case((Timesheet.overtime_type == "HOLIDAY", Timesheet.hours), else_=0)).label(
                "holiday_hours"
            ),
            func.sum(Timesheet.hours).label("total_hours"),
        ).filter(Timesheet.work_date.between(start_date, end_date), Timesheet.status == "APPROVED")
        ...
        # 查询人均加班
        user_count_query = self.db.query(func.count(func.distinct(Timesheet.user_id))).filter(
            Timesheet.work_date.between(start_date, end_date), Timesheet.status == "APPROVED"
        )
        ...
        trend_results = (
            trend_query.group_by(func.date(Timesheet.work_date))
            .order_by(func.date(Timesheet.work_date))
            .all()
        )
    ...
    def analyze_department_comparison(
        ...
        results = (
            query.group_by(Timesheet.department_id, Timesheet.department_name)
        ...
        for r in results:
            ...
            overtime_rate = (
                (float(r.overtime_hours or 0) / total_hours * 100) if total_hours > 0 else 0
            )
    ...
    def analyze_project_distribution(
        ...
        results = (
            query.group_by(Timesheet.project_id, Timesheet.project_name)
            .order_by(func.sum(Timesheet.hours).desc())
            .all()
        )
        ...
        # 计算集中度指数（前3个项目的工时占比）
        top3_percentage = (
            sum(p["percentage"] for p in project_details[:3]) if len(project_details) >= 3 else 100
        )
        concentration_index = top3_percentage / 100

        pie_chart = PieChartData(labels=pie_labels, values=pie_values, colors=None)  # 使用默认颜色

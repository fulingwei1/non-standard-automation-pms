...
class TimesheetAggregationService:
    ...
    def generate_finance_report(
        self, year: int, month: int, project_id: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        生成财务报表（用于核算项目成本）
        ...
        timesheets = query.order_by(
            Timesheet.project_id, Timesheet.user_id, Timesheet.work_date
        ).all()
    ...
    def generate_rd_report(
        self, year: int, month: int, rd_project_id: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        生成研发报表（用于核算研发费用）
        ...
        timesheets = query.order_by(
            Timesheet.rd_project_id, Timesheet.user_id, Timesheet.work_date
        ).all()
    ...
    def generate_project_report(
        ...
        """
        query = self.db.query(Timesheet).filter(
            Timesheet.status == "APPROVED", Timesheet.project_id == project_id
        )
        ...
        for user_key in personnel_stats:
            if total_hours > 0:
                personnel_stats[user_key]["contribution_rate"] = (
                    personnel_stats[user_key]["total_hours"] / total_hours
                ) * 100
        ...
        for ts in timesheets:
            ...
            if day_key not in daily_stats:
                daily_stats[day_key] = {"date": day_key, "hours": 0, "personnel_count": 0}

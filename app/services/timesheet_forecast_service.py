...
class TimesheetForecastService:
    ...
    def forecast_project_hours(
        ...
        if forecast_method == "HISTORICAL_AVERAGE":
            ...
            return self._forecast_by_linear_regression(
                project_id, project_name, project_type, complexity, team_size, duration_days
            )
    ...
    def _forecast_by_historical_average(
        ...
        if similar_project_ids:
            ...
            similar_query = (
                ...
                .filter(
                    Timesheet.project_id.in_(similar_project_ids), Timesheet.status == "APPROVED"
                )
            ...
            similar_query = (
                ...
                .filter(Timesheet.status == "APPROVED", Timesheet.project_id.isnot(None))
                .group_by(Timesheet.project_id, Timesheet.project_name)
                .limit(10)
            )
        ...
        if not similar_results:
            ...
            predicted_hours = (
                team_size * duration_days * base_hours_per_person_per_day * complexity_factor
            )
        ...
        return ProjectForecastResponse(
            ...
            predicted_hours=Decimal(
                str(
                    round(
                        float(predicted_hours) if not (predicted_hours != predicted_hours) else 0.0,
                        2,
                    )
                )
            ),
    ...
    def _forecast_by_linear_regression(
        ...
        historical_data = (
            self.db.query(
                ...
                func.datediff(func.max(Timesheet.work_date), func.min(Timesheet.work_date)).label(
                    "duration"
                ),
            )
            .filter(Timesheet.status == "APPROVED", Timesheet.project_id.isnot(None))
            .group_by(Timesheet.project_id, Timesheet.project_name)
            .having(func.count(func.distinct(Timesheet.user_id)) > 0)
            .all()
        )
        ...
        if len(historical_data) < 3:
            # 数据不足，退回到历史平均法
            return self._forecast_by_historical_average(
                project_id, project_name, project_type, complexity, team_size, duration_days, None
            )
        ...
        if SKLEARN_AVAILABLE and len(X) >= 3:
            ...
            predicted_hours = (
                team_size * duration_days * avg_hours_per_person_per_day * complexity_factor
            )
            confidence_level = 65
            r_squared = None
            feature_importance = {}
    ...
    def _forecast_by_trend(
        ...
        trend_data = (
            ...
            .group_by(func.date(Timesheet.work_date))
    ...
    def forecast_completion(
        self, project_id: int, forecast_method: str = "TREND_FORECAST"
    ) -> CompletionForecastResponse:
        """
        ...
        # 查询已消耗工时
        consumed_result = (
            self.db.query(func.sum(Timesheet.hours).label("consumed_hours"))
            .filter(Timesheet.project_id == project_id, Timesheet.status == "APPROVED")
            .first()
        )
        ...
        forecast_curve = self._generate_forecast_curve(
            consumed_hours, float(remaining_hours), current_progress, predicted_days_remaining
        )
    ...
    def _generate_forecast_curve(
        ...
        for i in range(1, min(days_remaining + 1, 31)):
            ...
            forecast_value = (
                consumed_hours + (remaining_hours * i / days_remaining)
                if days_remaining > 0
                else consumed_hours
            )
    ...
    def forecast_workload_alert(
        ...
        query = self.db.query(
            ...
            Timesheet.work_date.between(recent_start, recent_end), Timesheet.status == "APPROVED"
        )
    ...
    def analyze_gap(
        ...
        # 实际应从项目计划、任务预估中查询
        # 这里基于历史工时推算
        historical_query = self.db.query(func.sum(Timesheet.hours).label("total_hours")).filter(
            Timesheet.work_date.between(start_date, end_date), Timesheet.status == "APPROVED"
        )
                ...
                projects_gap.append(
                    {
                        ...
                        "gap_hours": round(
                            (required_hours - available_hours) / len(project_ids), 2
                        ),
                    }
                )

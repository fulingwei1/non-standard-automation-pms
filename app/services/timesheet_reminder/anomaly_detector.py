...
class TimesheetAnomalyDetector:
    ...
    def detect_daily_over_12(
        ...
        query = (
            ...
            .group_by(Timesheet.user_id, Timesheet.work_date)
        )
    ...
    def detect_daily_invalid(
        ...
        results = query.having(
            or_(func.sum(Timesheet.hours) < 0, func.sum(Timesheet.hours) > 24)
        ).all()
            ...
            for result in results:
                ...
                existing = (
                    ...
                    .filter(
                        ...
                        TimesheetAnomalyRecord.is_resolved == False,
                        TimesheetAnomalyRecord.anomaly_data["week_start"].astext
    ...
    def detect_no_rest_7days(
        ...
        # 获取需要检查的用户列表
        query = (
            self.db.query(Timesheet.user_id, Timesheet.user_name)
            .filter(
                Timesheet.work_date >= start_date,
                Timesheet.work_date <= end_date,
                Timesheet.status != "CANCELLED",
        ...
        for user in users:
            ...
            work_dates = [
                row.work_date
                for row in self.db.query(Timesheet.work_date)
                .filter(
                    ...
                    if consecutive_days >= 7:
                        ...
                        existing = (
                            ...
                            .filter(
                                ...
                                == AnomalyTypeEnum.NO_REST_7DAYS,
                                TimesheetAnomalyRecord.is_resolved == False,
                                TimesheetAnomalyRecord.anomaly_data["period_start"].astext
                        ...
                        timesheet_ids = [
                            row.id
                            for row in self.db.query(Timesheet.id)
                            .filter(
                                Timesheet.user_id == user.user_id,
    ...
    def detect_progress_mismatch(
        ...
        for timesheet in timesheets:
            # 检查1: 有工时但进度未更新
            if timesheet.progress_before is not None and timesheet.progress_after is not None:
                ...
                if hours >= 4 and progress_change == 0:
                    existing = (
                        ...
                        .filter(
                            ...
                            TimesheetAnomalyRecord.anomaly_type
                            == AnomalyTypeEnum.PROGRESS_MISMATCH,
                            TimesheetAnomalyRecord.is_resolved == False,
                        )
                        .first()
                    )

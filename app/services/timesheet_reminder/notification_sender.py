...
class NotificationSender:
    ...
    def send_reminder_notification(
        self, reminder: TimesheetReminderRecord, channels: Optional[List[str]] = None
    ) -> Dict[str, bool]:
        """
        发送提醒通知
        ...
        logger.info(
            f"提醒通知发送完成: {reminder.reminder_no}, " f"成功渠道: {successful_channels}"
        )

        return results

    def _send_system_notification(self, reminder: TimesheetReminderRecord, user: User) -> bool:
        """发送系统通知"""
        try:
    ...
    def _send_wechat_notification(self, reminder: TimesheetReminderRecord, user: User) -> bool:
        ...
        try:
            ...
            if result.get("errcode") == 0:
                logger.info(
                    f"企业微信通知发送成功: {reminder.reminder_no} -> {user.wechat_user_id}"
                )
                return True
            else:
    ...
    def _generate_email_html(self, reminder: TimesheetReminderRecord, user: User) -> str:
        """生成邮件HTML内容"""
        extra_info = ""
    ...
    def send_batch_reminders(
        self, reminders: List[TimesheetReminderRecord], channels: Optional[List[str]] = None
    ) -> Dict[str, int]:
        """
        批量发送提醒

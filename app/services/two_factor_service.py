...
from ..models.two_factor import User2FABackupCode, User2FASecret
...
class TwoFactorService:
    ...
    def generate_qr_code(
        self, user: User, secret: str, issuer_name: str = "非标项目管理系统"
    ) -> bytes:
        """
        生成TOTP二维码（PNG格式）
    ...
    def enable_2fa_for_user(
        self, db: Session, user: User, totp_code: str
    ...
    def disable_2fa_for_user(self, db: Session, user: User, password: str) -> Tuple[bool, str]:
        """
        为用户禁用2FA（需要验证密码）
    ...
    def setup_2fa_for_user(self, db: Session, user: User) -> Tuple[str, bytes]:
        """
        为用户设置2FA（生成密钥和二维码）
        ...
        db.query(User2FASecret).filter(
            User2FASecret.user_id == user.id, User2FASecret.method == "totp"
        ).delete()

        # 4. 保存新配置
        secret_record = User2FASecret(
            user_id=user.id, secret_encrypted=encrypted_secret, method="totp", is_active=True
        )
    ...
    def verify_2fa_code(
        self, db: Session, user: User, code: str, ip_address: Optional[str] = None
    ) -> Tuple[bool, str]:
        """
        验证2FA码（支持TOTP码和备用码）
    ...
    def _generate_backup_codes(self, db: Session, user: User, count: int = 10) -> List[str]:
        """
        生成备用恢复码
        ...
        for _ in range(count):
            ...
            backup_code_record = User2FABackupCode(user_id=user.id, code_hash=code_hash, used=False)
    ...
    def _verify_backup_code(
        ...
        # 查询未使用的备用码
        backup_codes = (
            db.query(User2FABackupCode)
            .filter(User2FABackupCode.user_id == user.id, User2FABackupCode.used == False)
            .all()
        )
    ...
    def get_backup_codes_info(self, db: Session, user: User) -> dict:
        """
        获取备用码信息
        ...
        Returns:
            备用码统计信息
        """
        total = db.query(User2FABackupCode).filter(User2FABackupCode.user_id == user.id).count()
    ...
    def regenerate_backup_codes(
        self, db: Session, user: User, password: str
    ) -> Tuple[bool, str, Optional[List[str]]]:
        """
        重新生成备用码（需要验证密码）

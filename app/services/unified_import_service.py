            ...
            try:
                ...
                try:
                    if isinstance(work_date_val, datetime):
                        work_date = work_date_val.date()
                    elif isinstance(work_date_val, date):
                        work_date = work_date_val
                    else:
                        work_date = pd.to_datetime(work_date_val).date()
                except:
                    failed_rows.append({
                        "row_index": index + 2,
                        "error": "工作日期格式错误"
                ...
                progress_after = None
                if pd.notna(row.get('更新前进度(%)')):
                    try:
                        progress_before = int(row.get('更新前进度(%)'))
                    except:
                        pass
                if pd.notna(row.get('更新后进度(%)')):
                    try:
                        progress_after = int(row.get('更新后进度(%)'))
                    except:
                        pass

...
def predict(
    ...
    # 3. 获取客户历史数据
    coop_count, success_count = get_customer_cooperation_history(
        service, customer_id, customer_name
    )
    ...
    recommendations = _generate_recommendations(
        predicted_rate, dimension_scores, salesperson_win_rate, competitor_count, product_match_type
    )
...
def _generate_recommendations(
    ...
    if product_match_type == ProductMatchTypeEnum.NEW.value:
        recommendations.append(
            "【新产品风险】该产品不在公司优势产品列表中，建议评估技术风险和交付能力"
        )
    ...
    if predicted_rate < 0.30:
        recommendations.append(
            "【资源评估】预测中标率偏低(<30%)，建议评估是否继续投入资源，或调整策略"
        )
...
def batch_predict(
    service: "WinRatePredictionService", leads: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """批量预测多个线索的中标率
    ...
    for lead in leads:
        try:
            ...
            results.append({"lead_id": lead.get("lead_id"), "error": str(e)})

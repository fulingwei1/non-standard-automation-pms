...
class TestProductionPlanService:
    ...
    def test_build_plan_response(
        self, service, sample_plan, sample_project, sample_workshop, mock_db
    ):
        """测试构建计划响应"""
        # 设置mock查询
        mock_query = Mock()
    ...
    def test_list_plans(self, mock_pagination, mock_get_or_404, service, sample_plan, mock_db):
        ...
        # 调用方法
        result = service.list_plans(pagination=pagination, status="DRAFT")
    ...
    @patch("app.services.production.plan_service.get_or_404")
    def test_get_plan(
        self, mock_get_or_404, service, sample_plan, sample_project, sample_workshop, mock_db
    ):
        """测试获取生产计划详情"""
        # 设置mock
    ...
    def test_update_plan(
        ...
        # 创建更新输入
        plan_in = Mock()
        plan_in.model_dump.return_value = {"payment_name": "更新后的计划"}
    ...
    def test_approve_plan(self, mock_get_or_404, service, sample_plan, mock_db):
        ...
        result = service.approve_plan(
            plan_id=1, approved=True, approval_note="审批通过", current_user_id=2
        )

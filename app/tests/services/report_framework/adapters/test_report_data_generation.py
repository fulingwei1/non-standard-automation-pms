...
class TestReportDataGenerationAdapter:
    ...
    def test_generate_data_project_weekly(self, mock_router, adapter):
        ...
        mock_router.generate_report_by_type.return_value = {
            ...
            "end_date": "2024-01-07",
            "summary": {"total_hours": 100, "completed_tasks": 10},
            "details": [],
        }

        # 调用方法
        params = {"project_id": 1, "start_date": date(2024, 1, 1), "end_date": date(2024, 1, 7)}
    ...
    def test_generate_data_with_default_dates(self, mock_router, adapter):
        """测试使用默认日期生成数据"""
        # 设置mock
        mock_router.generate_report_by_type.return_value = {"summary": {}}
    ...
    def test_generate_data_with_error(self, mock_router, adapter):
        """测试生成数据时出错"""
        # 设置mock返回错误
        mock_router.generate_report_by_type.return_value = {"error": "数据生成失败"}

...
class TestTemplateReportAdapter:
    ...
    def test_generate_with_yaml_config(self, mock_service, adapter, sample_template, mock_db):
        ...
        # Mock engine
        mock_engine = Mock()
        mock_engine.generate.return_value = {"metadata": {}, "sections": []}
        adapter.engine = mock_engine

        # 调用方法
        params = {"template_id": 1, "project_id": 1}
    ...
    def test_convert_to_report_result(self, adapter):
        ...
        data = {
            "template_code": "TEST",
            "template_name": "测试报表",
            "summary": {"total": 100, "completed": 80},
            "sections": {
                "section1": [{"name": "项目A", "progress": 50}, {"name": "项目B", "progress": 75}],
                "section2": {"metric1": 100, "metric2": 200},
            },
        }

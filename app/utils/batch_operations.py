...
class BatchOperationResult:
    ...
    def add_failure(self, entity_id: Any, reason: str, id_field: str = "id"):
        """记录失败"""
        self.failed_count += 1
        self.failed_items.append({id_field: entity_id, "reason": reason})
...
class BatchOperationExecutor(Generic[ModelType]):
    ...
    def execute(
        ...
        if pre_filter_func:
            ...
            entities = self.db.query(self.model).filter(id_column.in_(entity_ids)).all()
        ...
        for entity_id in entity_ids:
            ...
            if not entity:
                # 使用模型特定的ID字段名
                id_field_name = (
                    f"{self.model.__tablename__.rstrip('s')}_id"

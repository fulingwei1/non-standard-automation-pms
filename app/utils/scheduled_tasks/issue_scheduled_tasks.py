...
def check_overdue_issues():
    ...
    try:
        with get_db_session() as db:
            ...
            overdue_issues = (
                db.query(Issue)
                .filter(Issue.status.in_(["OPEN", "IN_PROGRESS"]), Issue.due_date < now)
                .all()
            )
            ...
            logger.info(
                f"问题逾期检查完成: 发现 {len(overdue_issues)} 个逾期问题, 生成 {alert_count} 个预警"
            )
...
def check_blocking_issues():
    ...
    try:
        with get_db_session() as db:
            ...
            blocking_issues = (
                db.query(Issue)
                .filter(Issue.priority == "URGENT", Issue.status.in_(["OPEN", "IN_PROGRESS"]))
                .all()
            )
            ...
            logger.info(
                f"阻塞问题检查完成: 发现 {len(blocking_issues)} 个阻塞问题, 生成 {alert_count} 个预警"
            )
    ...
    try:
        with get_db_session() as db:
            ...
            timeout_issues = (
                db.query(Issue)
                .filter(
                    Issue.status.in_(["OPEN", "IN_PROGRESS"]), Issue.updated_at < timeout_threshold
                )
                .all()
            )
            ...
            logger.info(
                f"问题超时检查完成: 发现 {len(timeout_issues)} 个超时问题, 升级 {upgraded_count} 个问题优先级"
            )
...
def daily_issue_statistics_snapshot():
    ...
    try:
        with get_db_session() as db:
            ...
            processing_issues = db.query(Issue).filter(Issue.status == "IN_PROGRESS").count()
            resolved_issues = db.query(Issue).filter(Issue.status == "RESOLVED").count()

            # ��优先级统计
            urgent_issues = db.query(Issue).filter(Issue.priority == "URGENT").count()
            high_priority_issues = db.query(Issue).filter(Issue.priority == "HIGH").count()
            medium_priority_issues = db.query(Issue).filter(Issue.priority == "MEDIUM").count()
            low_priority_issues = db.query(Issue).filter(Issue.priority == "LOW").count()

            # 按项目统计
            project_stats = (
                db.query(Issue.project_id, func.count(Issue.id).label("issue_count"))
                .filter(Issue.project_id.isnot(None))
            ...
            logger.info(
                f"问题统计快照生成完成: 总问题数 {total_issues}, 待处理 {open_issues}, 进行中 {processing_issues}, 已解决 {resolved_issues}"
            )
        ...
        with get_db_session() as db:
            ...
            unassigned_issues = (
                ...
                .filter(
                    ...
                    Issue.created_at < assignment_timeout,
                )
                .all()
            )
            ...
            logger.info(
                f"问题分配超时检查完成: 发现 {len(unassigned_issues)} 个未分配问题, 生成 {alert_count} 个预警"
            )
...
def check_issue_resolution_timeout():
    ...
    try:
        with get_db_session() as db:
            ...
            now = datetime.now()
            overdue_resolution_issues = (
                db.query(Issue)
                .filter(
                    Issue.status == "IN_PROGRESS",
                    Issue.due_date.isnot(None),
                    Issue.due_date < now.date(),
                )
                .all()
            )
            ...
            logger.info(
                f"问题解决超时检查完成: 发现 {len(overdue_resolution_issues)} 个超时问题, 生成 {alert_count} 个预警"
            )

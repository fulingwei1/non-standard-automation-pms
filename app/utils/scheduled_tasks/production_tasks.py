...
def check_production_plan_alerts():
    ...
    try:
        with get_db_session() as db:
            ...
            plan_rule = (
                db.query(AlertRule)
                .filter(AlertRule.rule_code == "PRODUCTION_PLAN", AlertRule.is_enabled)
                .first()
            )
            ...
            # 查询所有执行中的生产计划
            active_plans = (
                db.query(ProductionPlan).filter(ProductionPlan.status == "EXECUTING").all()
            )
...
def check_work_report_timeout():
    ...
    try:
        with get_db_session() as db:
            ...
            timeout_rule = (
                db.query(AlertRule)
                .filter(AlertRule.rule_code == "WORK_REPORT_TIMEOUT", AlertRule.is_enabled)
                .first()
            )
            ...
            # 查询所有进行中的工单
            active_orders = (
                db.query(WorkOrder).filter(WorkOrder.status.in_(["IN_PROGRESS", "ASSIGNED"])).all()
            )
            ...
            for order in active_orders:
                ...
                last_report = (
                    db.query(WorkReport)
                    .filter(WorkReport.work_order_id == order.id)
                    .order_by(WorkReport.report_time.desc())
                    .first()
                )
...
def generate_production_daily_reports(target_date=None):
    ...
    try:
        with get_db_session() as db:
            ...
            # 检查是否已有报告
            existing = (
                db.query(ProductionDailyReport)
                .filter(ProductionDailyReport.report_date == target_date)
                .first()
            )

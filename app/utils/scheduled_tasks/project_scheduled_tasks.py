    ...
    with get_db_session() as db:
        try:
                ...
                for po in purchase_orders:
                    ...
                    for item in items:
                        ...
                        spec_requirement = (
                            ...
                            .filter(TechnicalSpecRequirement.id == item.technical_spec_id)
                            .first()
                        )
                        ...
                        # 创建匹配记录
                        match_result = service.check_spec_match(item.description, spec_requirement)

                        # 记录匹配结果
                        existing_record = (
                            db.query(SpecMatchRecord)
                            .filter(
            ...
            for project in projects:
                bom_headers = (
                    db.query(BomHeader)
                    .filter(BomHeader.project_id == project.id, BomHeader.is_latest)
                    .all()
                )
                ...
                for bom in bom_headers:
                    ...
                    for item in bom_items:
                        ...
                        match_result = service.check_spec_match(item.item_name, spec_requirement)
            ...
            logger.info(
                f"规格匹配检查完成: 检查 {total_checked} 项, 发现 {total_mismatched} 项不匹配"
            )
...
def calculate_project_health():
    ...
    try:
        ...
        with get_db_session() as db:
            ...
            logger.info(
                f"健康度计算完成: 总计 {result['total']} 个项目,  更新 {result['updated']} 个, 未变化 {result['unchanged']} 个"
            )
...
def calculate_progress_summary():
    ...
    try:
        with get_db_session() as db:
            ...
            for project in projects:
                ...
                milestone_progress_summary = (
                    db.query(
                        ...
                        func.sum(case((ProjectMilestone.status == "COMPLETED", 1), else_=0)).label(
                ...
                if total_tasks > 0 or total_milestones > 0:
                    ...
                    overall_progress = (task_completion_rate * 0.6) + (
            ...
            return {"updated_projects": updated_count, "timestamp": datetime.now().isoformat()}
...
def check_project_deadline_alerts():
    ...
    try:
        with get_db_session() as db:
            ...
            logger.info(
                f"项目截止日期预警检查完成: 发现 {len(projects)} 个即将到期项目, 生成 {alert_count} 个预警"
            )
...
def check_project_cost_overrun():
    ...
    try:
        with get_db_session() as db:
            ...
            return {"overrun_projects": alert_count, "timestamp": datetime.now().isoformat()}

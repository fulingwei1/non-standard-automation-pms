...
from app.models.report import ArchiveStatusEnum, GeneratedByEnum
                ...
                try:
                    ...
                    if template.output_format == OutputFormatEnum.EXCEL.value:
                        file_path = ReportExcelService.export_to_excel(
                            data=data, template_name=template.name
                        )
...
def send_report_to_recipients(db, archive):
    ...
    recipients = (
        ...
        .filter(ReportRecipient.template_id == archive.template_id, ReportRecipient.enabled == True)
        .all()
    )

...
class SpecMatcher:
    ...
    def match_specification(
        ...
        req_requirement_level = _get_attr(requirement, "requirement_level") or _get_attr(
            requirement, "requirementment_level"
        )

        # 1. 文本相似度匹配（规格型号）
        spec_similarity = self._text_similarity(req_specification.lower(), actual_spec.lower())
        ...
        if req_brand:
            if actual_brand:
                ...
                if not brand_match:
                    differences["brand"] = {"required": req_brand, "actual": actual_brand}
                scores.append(("brand", 100 if brand_match else 0))
            else:
                differences["brand"] = {"required": req_brand, "actual": None, "missing": True}
        ...
        if req_model:
            if actual_model:
                ...
                if not model_match:
                    differences["model"] = {"required": req_model, "actual": actual_model}
                scores.append(("model", 100 if model_match else 0))
            else:
                differences["model"] = {"required": req_model, "actual": None, "missing": True}
        ...
        if scores:
            ...
            weights = {"specification": 0.5, "brand": 0.2, "model": 0.2, "parameters": 0.1}
    ...
    def _compare_parameters(
        self, required: Dict[str, Any], actual: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        对比关键参数，返回差异
        ...
        for key, required_value in required.items():
            ...
            if actual_value is None:
                differences[key] = {"required": required_value, "actual": None, "missing": True}
            else:
                ...
                if req_num is not None and act_num is not None:
                    ...
                    if str(required_value).lower() != str(actual_value).lower():
                        differences[key] = {"required": required_value, "actual": actual_value}

        return differences

    def _calculate_param_score(self, required: Dict[str, Any], actual: Dict[str, Any]) -> float:
        """
        计算参数匹配度

...
def calculate_project_health():
    ...
    try:
        ...
        with get_db_session() as db:
            ...
            logger.info(
                f"[{datetime.now()}] 健康度计算完成: 总计 {result['total']} 个项目, "
                f"更新 {result['updated']} 个, 未变化 {result['unchanged']} 个"
            )

            return result
    except Exception as e:
        logger.error(f"[{datetime.now()}] 健康度计算失败: {str(e)}")
        import traceback
        traceback.print_exc()
        return {'error': str(e)}
...
def daily_health_snapshot():
    ...
    try:
        ...
        with get_db_session() as db:
            ...
            logger.info(f"[{datetime.now()}] 健康度快照生成完成: 生成 {snapshot_count} 个快照")
        ...
        logger.error(f"[{datetime.now()}] 健康度快照生成失败: {str(e)}")
        import traceback
        traceback.print_exc()
        return {'error': str(e)}

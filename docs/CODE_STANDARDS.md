...
- 每个模块在 `app/api/v1/api.py` 注册时必须使用清晰的模块前缀（如 `/production`、`/service`）。
- 仅在**模块内部**路由已统一包含模块根路径时，才允许在注册时使用 `prefix=""`。
- 若使用 `prefix=""`，请在模块的 `router.py` 或 `__init__.py` 内明确标注模块根路径，避免产生不一致或难以追踪的 API 路由。

# 非标自动化项目管理系统真正用起来路线图

## 目标

适用对象：一两百人的非标自动化设备公司。

判断标准不是页面多，而是销售、售前、项目、采购、生产、交付、财务每天能沿着同一条主线做事：

1. 销售录入商机和客户需求。
2. 售前技术支持接单、澄清需求、出方案、估成本、配合投标。
3. 赢单后生成项目交接包，项目经理接收范围、成本、风险、计划。
4. 项目阶段推进时，采购、生产、装配、调试、验收按项目同一条线更新。
5. 毛利、交期、风险、变更、验收和复盘能回到同一个项目闭环。

## 当前问题判断

系统不是没有功能，而是功能太散：

| 业务段 | 当前现象 | 对真实使用的影响 |
|---|---|---|
| 权限和菜单 | 页面、路由、权限点存在重复和历史入口 | 用户不知道该从哪里进，同一岗位看到太多旧入口 |
| 销售到售前 | 商机、方案、成本、投标、工单分散 | 销售觉得流程断，售前任务接不住 |
| 售前技术支持 | 需求调研、方案、技术参数、工单、投标、知识库分散 | 售前工程师需要跳多个页面，资料难复用 |
| 项目管理 | 项目看板、PMO、甘特、资源、成本、评审、风险、ECN、收尾分散 | 项目经理缺一个日常工作台 |
| 项目后续模块 | 采购、生产、装配、交付、验收都能按项目查，但入口分散 | 公司管理层看不到“一台设备从签单到验收”的完整状态 |

## 主入口设计

不要把所有页面删掉重做。先确定主入口，其它页面保留为子能力或旧路由重定向。

| 主入口 | 面向岗位 | 应合并的能力 | 当前落点 |
|---|---|---|---|
| 销售工作台 | 销售、销售经理 | 客户、商机、报价、合同、售前申请、赢单交接 | 销售模块现有工作台继续收敛 |
| 售前技术支持中心 | 销售、售前工程师、售前主管 | 需求调研、方案、技术参数、成本估算、投标、工单、知识库、模板 | `/presales/technical-solutions` 已开始合并 |
| 项目管理中心 | 项目经理、PMO、工程负责人 | 项目看板、项目详情、WBS、任务、甘特、资源、里程碑、项目评审 | `/project/management-center` 已开始合并 |
| 项目执行中心 | 机械、电气、软件、装配、调试 | 任务领取、进度填报、工时、问题、现场资源 | 当前分散在任务、工程师工作台、装配、生产执行 |
| 项目成本毛利中心 | 项目经理、财务、老板 | 预算、实际成本、工时成本、报价成本、毛利偏差、变更影响 | `/project/cost-center` 已有雏形 |
| 交付验收中心 | 项目经理、品质、交付、售后 | FAT、SAT、现场问题、验收单、整改闭环、售后交接 | 当前分散在生产验收、项目交付、售后 |

## 售前技术支持合并方案

近期目标：把售前从“页面集合”变成“接单到交付物”的工作台。

主入口：`/presales/technical-solutions`

建议页签：

| 页签 | 作用 | 来源页面 |
|---|---|---|
| 需求调研 | 客户需求澄清和现场调研 | `RequirementSurvey` |
| 技术方案 | 方案编制、版本、评审 | `PresaleProposals` |
| 技术参数 | 参数模板、行业分类、成本估算基础 | `TechnicalParameterManagement` |
| 成本估算 | 初版成本、毛利预警、报价输入 | `PresalesCostEstimation` |
| 投标支持 | 技术标、商务标协同、投标状态 | `BiddingCenter`、`PresaleBids` |
| 工单看板 | 销售发起支持、售前处理、经理派工 | `PresalesTasks`、`PresaleTicketBoard` |
| 知识模板 | 方案模板、案例、售前复用资产 | `PresaleTemplates` |

保留旧 URL，但逐步重定向到中心页签，避免老链接失效。

当前已落地：

- `/presales/technical-solutions` 已扩展为售前技术支持中心。
- `需求调研 / 技术方案 / 技术参数 / 成本估算 / 投标支持 / 知识模板 / 工单看板` 已能在中心内切换。
- `/requirement-survey`、`/presales/solutions`、`/presales/technical-parameters`、`/presales/cost-estimation`、`/bidding`、`/presales/bids`、`/presales/templates`、`/presale-templates`、`/presales-tasks`、`/presales/ticket-board` 已作为兼容入口跳到中心页签。
- `/sales/presales-tasks`、`/project-presales-tasks` 已作为兼容入口跳到 `tab=reviews`，并保留 `type/status/projectId` 等筛选条件。
- 全局 `/knowledge-base` 暂不并入售前中心，保留为公司级知识库。
- 售前工作台聚合上下文已支持线索级自动解析：销售从线索发起售前工单后，工单会继承线索客户名称和需求摘要；不带 `ticket_id` 进入售前中心也能带出对应工单、需求包、方案和成本基线。
- 售前工作台当前技术评估已跟随当前工单：同一线索/商机存在多张售前工单时，带 `ticket_id` 打开的上下文不会串到其它工单的最新评估。
- 需求调研页已接入售前聚合上下文：从线索/商机进入时会优先展示结构化需求包、关键需求项、技术规格和未闭环问题，同时保留原调研工单列表。
- 技术方案页已优先使用售前聚合上下文里的方案列表；聚合上下文没有方案或加载失败时再回退到原方案列表接口，减少商机/工单上下文下的空方案页。
- 投标支持页已优先使用售前聚合上下文里的投标记录；聚合上下文没有投标或加载失败时再回退到原投标列表接口，并保留成本支持跳转上下文。
- 投标详情“申请成本支持”已携带投标、工单、线索、商机、项目、预算金额和投标名称上下文；成本估算保存时会把投标来源写入方案成本明细，方便后续报价/项目交接追溯。
- 商机侧直接创建售前方案时，已自动绑定当前商机的售前工单；售前工作台能继续按当前工单带出方案和成本基线。
- 商机页“发起售前支持/技术评估”已优先复用同商机同类型活跃售前工单；即使工作台当前工单是其它类型，也会从工单列表兜底查找，避免重复造单。
- 售前工单详情已增加“生成项目立项申请”入口，会把线索、商机、工单、方案、客户、金额和需求摘要带到 PMO 立项页；PMO 立项页会优先读取售前聚合上下文预填方案、成本基线、需求和风险，创建后继续进入已有立项审批与项目生成流程。
- 售前任务详情已补齐完成后销售确认评价入口：技术人员完成工单后，销售可提交满意度和反馈，已评价工单会直接展示确认结果。
- 售前工单交付物已支持单独审核通过/退回：交付物状态、审核人、审核时间和审核意见会随工单详情返回，前端可在任务详情内直接处理。
- 售前工单完成已纳入交付物审核门禁：已有交付物必须全部通过审核后，才能标记工单完成；退回或待审核交付物会在前后端同时阻断。
- 售前交付物已支持关键/非关键标记：只有关键交付物未通过审核才阻断工单完成，非关键参考附件不影响工单收口。
- 售前成本估算页已优先使用售前工作台聚合上下文里的成本基线；拿不到聚合基线时再回退到方案列表接口，减少商机/项目上下文下的空成本页。

下一批建议：

1. 继续补售前中心页签间的上下文联动，例如投标支持到成本支持、报价成本的自动带参。
2. 继续加深售前到项目交接包，把投标结果、未闭环事项和初始里程碑汇总进项目创建后的工作区。

## 项目管理合并方案

项目管理的问题和售前类似：已经有很多子模块，但项目经理缺一个稳定日常入口。

当前主入口：`/project/management-center`

已落地页签：

| 页签 | 作用 | 来源页面 |
|---|---|---|
| 看板 | 所有项目、卡片、看板、列表、流水线视图 | `ProjectBoard` |
| 驾驶舱 | PMO 总览、健康监控 | `ProjectDashboardCenter` |
| 任务 | 项目任务和个人任务 | `TaskCenter` |
| 进度 | 排期、进度报告、里程碑、WBS 模板 | `ScheduleBoard`、`ProgressReport`、`MilestoneManagement`、`WBSTemplateManagement` |
| 计划资源 | 任务依赖、资源负载 | `GanttAndResource` |
| 成本 | 预算、工时成本、毛利联动 | `ProjectCostCenter` |
| 收尾 | 结项、复盘、经验教训 | `ProjectClosing` |
| AI工具 | 智能排计划、工程师调度 | `AIProjectTools` |

路由策略：

- `/projects`、`/board` 已重定向到 `management-center?tab=board`。
- `/pmo/dashboard`、`/project/dashboard-center` 已重定向到 `management-center?tab=dashboard`。
- `/progress-tracking/schedule|reports|milestones|wbs` 已重定向到 `management-center?tab=tracking` 的对应子页签。
- `/gantt-resource`、`/resource-overview` 已重定向到 `management-center?tab=planning`。
- `/project/cost-center`、`/time-cost-margin-flow` 已重定向到 `management-center?tab=cost`。
- `/project-closing`、`/pmo/closure`、`/projects/reviews`、`/lessons-learned` 已重定向到 `management-center?tab=closing`。
- `/projects/:id/*` 继续保留项目详情深链接，不强行塞进全局页签。

## 售前到项目的交接包

这是系统能不能真正跑起来的核心。

赢单后必须生成一份项目交接包，至少包括：

| 交接内容 | 来源 | 进入项目后的用途 |
|---|---|---|
| 客户和合同信息 | 销售、合同 | 项目基础信息 |
| 需求调研结论 | 售前需求调研 | 项目范围、验收标准 |
| 技术方案版本 | 方案管理 | 设计输入、技术评审 |
| 技术参数模板 | 技术参数 | BOM、测试规格、标准工时 |
| 成本估算和毛利底线 | 售前成本 | 项目预算、毛利预警 |
| 风险和假设 | 售前/投标 | 项目风险清单 |
| 交期和关键里程碑 | 销售/售前 | 项目计划初稿 |
| 未闭环事项 | 售前工单/评审 | 项目待办和阶段门 |

交接动作要做成系统按钮：`赢单交接 -> 生成项目 -> 生成WBS/里程碑初稿 -> 生成项目待办`。

当前已落地：

- 售前工单详情可发起 PMO 项目立项申请，并携带线索、商机、工单、方案、客户、金额和需求摘要。
- PMO 立项页会优先读取售前聚合上下文，预填方案、成本基线、需求和风险，创建后进入已有立项审批与项目生成流程。
- 项目工作区交接包已展示合同、商机、报价成本、售前方案、售前工单、技术评估、开放项、成本基准、投标结果和初始里程碑。
- PMO 立项审批生成/复用项目后，会把未关闭的售前未决事项自动生成项目任务；项目工作区最近任务会标识“售前遗留”，避免问题只停留在交接展示层。

## 分阶段落地

### Phase 1：入口收敛

目标：用户先知道从哪里进。

- 销售：商机、报价、合同、售前申请只保留一个主入口。
- 售前：`/presales/technical-solutions` 作为售前技术支持中心。
- 项目：`/project/management-center` 作为项目管理中心，旧入口跳转到对应页签。
- 菜单只放主入口，旧入口只做跳转和深链接。

### Phase 2：流程闭环

目标：销售到售前到项目能走通。

- 商机创建售前工单。
- 售前工单关联需求调研、方案、成本、投标。
- 赢单后生成项目交接包。
- 项目创建后自动带出范围、成本、风险、里程碑初稿。

### Phase 3：项目后续模块收口

目标：项目经理每天只看一个项目中心。

- 项目中心继续补齐技术评审、风险变更、验收交付、售后交接等专业深链入口。
- 采购、生产、装配、验收页面保留专业视图，但必须能从项目详情进入。
- 每个后续模块都要回写项目状态、风险、成本或验收结果。

### Phase 4：权限和角色验收

目标：100-200 人公司能按岗位使用。

最少角色：

| 角色 | 主入口 |
|---|---|
| 销售 | 销售工作台、售前申请、报价合同 |
| 售前工程师 | 售前技术支持中心 |
| 售前主管 | 售前派工、评审、负荷 |
| 项目经理 | 项目管理中心 |
| 工程师 | 我的任务、技术评审、工时 |
| 采购/仓库 | 采购执行、齐套、收发料 |
| 生产/装配 | 生产执行、装配任务、异常 |
| 品质/交付 | 验收、整改、交付计划 |
| 财务/老板 | 成本毛利、项目健康、经营看板 |

权限原则：

- 菜单权限按主入口配置。
- 操作权限按按钮/API 配置。
- 旧页面不单独暴露菜单权限，避免同一个岗位看到多套入口。

## 最近三步执行建议

1. 统一销售工作台里的“发起售前支持”动作，确保商机、方案评审、技术支持都落到同一张售前工单。
2. 做售前到项目交接包，把需求、方案、成本、风险、里程碑、未闭环事项带入项目中心。
3. 做一条端到端验收脚本：商机 -> 售前工单 -> 方案/成本 -> 赢单交接 -> 项目 -> 任务/评审 -> 验收/收尾。

这三步完成后，系统才会从“很多功能模块”变成“公司能按流程每天用的系统”。

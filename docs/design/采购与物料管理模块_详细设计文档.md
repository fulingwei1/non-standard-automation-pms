# 采购与物料管理模块 - 详细设计文档

> 适用范围：非标自动化测试设备/线体项目的物料采购管理
> 目标：实现BOM管理、采购全流程、到货跟踪、齐套率计算、缺料预警。

---

## 一、模块定位与边界

### 1.1 模块范围

**包含功能**：
- 物料主数据管理
- 供应商管理
- BOM编制与版本管理
- 采购申请与审批
- 采购订单管理
- 到货与入库管理
- 齐套率计算
- 缺料预警

**不包含**（由其他模块负责）：
- 外协加工管理 → 外协管理模块
- 仓库库存管理 → ERP/WMS系统
- 财务付款 → 财务系统
- 供应商评价详细流程 → 独立模块（可后期扩展）

### 1.2 业务流程概览

```
BOM发布 → 采购申请 → 采购审批 → 采购下单 → 到货确认 → 入库 → 齐套检查
                                    │
                                    ▼
                              缺料预警 ──► 项目健康度更新
```

### 1.3 与现有系统关系

```
项目管理 ──机台BOM──► 采购物料 ──缺料预警──► 预警异常
    │                    │
    │                    ├── 齐套率 ──► 阶段门校验
    │                    │
    ▼                    ▼
变更管理 ──BOM变更──► 采购调整
```

---

## 二、核心业务对象

### 2.1 对象关系

```
┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│ 物料主数据  │     │   供应商    │     │   项目      │
│  materials  │     │  suppliers  │     │  projects   │
└──────┬──────┘     └──────┬──────┘     └──────┬──────┘
       │                   │                   │
       │         ┌─────────┴─────────┐         │
       │         ▼                   ▼         │
       │   ┌─────────────┐     ┌─────────────┐ │
       └──►│    BOM      │     │   机台      │◄┘
           │  bom_items  │     │  machines   │
           └──────┬──────┘     └──────┬──────┘
                  │                   │
                  └─────────┬─────────┘
                            ▼
                    ┌─────────────┐
                    │   采购单    │
                    │  purchases  │
                    └──────┬──────┘
                           │
              ┌────────────┼────────────┐
              ▼            ▼            ▼
        ┌──────────┐ ┌──────────┐ ┌──────────┐
        │ 采购明细 │ │ 到货记录 │ │ 缺料预警 │
        │  items   │ │ receipts │ │  alerts  │
        └──────────┘ └──────────┘ └──────────┘
```

### 2.2 物料分类体系

```
物料分类（三级）
├── ME 机械件
│   ├── ME-01 机加工件
│   ├── ME-02 钣金件
│   ├── ME-03 标准件（螺丝螺母等）
│   └── ME-04 型材
├── EL 电气件
│   ├── EL-01 PLC/控制器
│   ├── EL-02 伺服/电机
│   ├── EL-03 传感器
│   ├── EL-04 气动元件
│   └── EL-05 线缆/接插件
├── PN 气动件
│   ├── PN-01 气缸
│   ├── PN-02 阀类
│   └── PN-03 气动附件
├── ST 标准件/通用件
│   ├── ST-01 轴承/导轨
│   ├── ST-02 联轴器
│   └── ST-03 其他标准件
└── OT 其他
    ├── OT-01 外购整机
    ├── OT-02 软件/授权
    └── OT-03 耗材
```

---

## 三、物料主数据管理

### 3.1 物料编码规则

```
物料编码 = 大类 + "-" + 中类 + "-" + 小类 + "-" + 流水号
示例：ME-01-01-0001（机加工件-轴类-传动轴-0001）
      EL-02-01-0015（电气件-伺服电机-松下A6-0015）
```

### 3.2 物料主数据表

```sql
CREATE TABLE materials (
    id INTEGER PRIMARY KEY AUTOINCREMENT,

    -- 编码与名称
    material_code VARCHAR(30) UNIQUE NOT NULL,     -- ME-01-01-0001
    material_name VARCHAR(100) NOT NULL,
    short_name VARCHAR(50),                        -- 简称

    -- 分类
    category_l1 VARCHAR(20) NOT NULL,              -- 大类：ME/EL/PN/ST/OT
    category_l2 VARCHAR(20),                       -- 中类
    category_l3 VARCHAR(20),                       -- 小类

    -- 规格参数
    specification TEXT,                            -- 规格描述
    model VARCHAR(100),                            -- 型号
    brand VARCHAR(50),                             -- 品牌
    unit VARCHAR(10) DEFAULT '个',                 -- 单位

    -- 采购属性
    is_standard BOOLEAN DEFAULT FALSE,             -- 是否标准件
    is_critical BOOLEAN DEFAULT FALSE,             -- 是否关键件
    standard_lead_time INTEGER,                    -- 标准交期(天)
    min_order_qty DECIMAL(10,2),                   -- 最小起订量
    reference_price DECIMAL(12,2),                 -- 参考单价

    -- 供应商
    default_supplier_id INTEGER,                   -- 默认供应商
    alt_supplier_ids TEXT,                         -- 备选供应商(JSON)

    -- 状态
    status VARCHAR(20) DEFAULT 'ACTIVE',           -- ACTIVE/INACTIVE/OBSOLETE
    obsolete_reason TEXT,

    -- 审计
    created_by INTEGER,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (default_supplier_id) REFERENCES suppliers(id)
);

-- 索引
CREATE INDEX idx_materials_code ON materials(material_code);
CREATE INDEX idx_materials_category ON materials(category_l1, category_l2);
CREATE INDEX idx_materials_status ON materials(status);
CREATE INDEX idx_materials_critical ON materials(is_critical);
```

### 3.3 物料替代关系

```sql
-- 物料替代关系表
CREATE TABLE material_alternatives (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    material_id INTEGER NOT NULL,                  -- 原物料
    alt_material_id INTEGER NOT NULL,              -- 替代物料
    priority INTEGER DEFAULT 1,                    -- 优先级(1最高)
    alt_type VARCHAR(20) DEFAULT 'EQUIVALENT',     -- EQUIVALENT/UPGRADE/DOWNGRADE
    remark TEXT,
    is_active BOOLEAN DEFAULT TRUE,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (material_id) REFERENCES materials(id),
    FOREIGN KEY (alt_material_id) REFERENCES materials(id)
);
```

---

## 四、供应商管理

### 4.1 供应商数据表

```sql
CREATE TABLE suppliers (
    id INTEGER PRIMARY KEY AUTOINCREMENT,

    -- 基本信息
    supplier_code VARCHAR(20) UNIQUE NOT NULL,     -- V00001
    supplier_name VARCHAR(100) NOT NULL,
    short_name VARCHAR(50),

    -- 分类
    supplier_type VARCHAR(20),                     -- 生产商/贸易商/代理商
    category VARCHAR(50),                          -- 机加工/钣金/电气/标准件

    -- 联系信息
    contact_name VARCHAR(50),
    contact_phone VARCHAR(20),
    contact_email VARCHAR(100),
    address TEXT,

    -- 商务信息
    payment_terms VARCHAR(50),                     -- 付款条款
    tax_rate DECIMAL(5,2),                         -- 税率
    bank_account TEXT,                             -- 银行账户信息

    -- 评价（简化版）
    delivery_rating DECIMAL(3,2) DEFAULT 3.0,      -- 交期评分 1-5
    quality_rating DECIMAL(3,2) DEFAULT 3.0,       -- 质量评分 1-5
    service_rating DECIMAL(3,2) DEFAULT 3.0,       -- 服务评分 1-5
    overall_rating DECIMAL(3,2) DEFAULT 3.0,       -- 综合评分

    -- 状态
    status VARCHAR(20) DEFAULT 'ACTIVE',           -- ACTIVE/INACTIVE/BLACKLIST
    cooperation_start DATE,
    blacklist_reason TEXT,

    -- 审计
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
);

-- 索引
CREATE INDEX idx_suppliers_code ON suppliers(supplier_code);
CREATE INDEX idx_suppliers_category ON suppliers(category);
CREATE INDEX idx_suppliers_status ON suppliers(status);
CREATE INDEX idx_suppliers_rating ON suppliers(overall_rating);
```

### 4.2 供应商物料关联

```sql
-- 供应商可供物料
CREATE TABLE supplier_materials (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    supplier_id INTEGER NOT NULL,
    material_id INTEGER NOT NULL,
    supplier_part_no VARCHAR(50),                  -- 供应商料号
    unit_price DECIMAL(12,2),                      -- 单价
    currency VARCHAR(10) DEFAULT 'CNY',
    lead_time INTEGER,                             -- 交期(天)
    min_order_qty DECIMAL(10,2),                   -- 最小起订量
    is_preferred BOOLEAN DEFAULT FALSE,            -- 是否首选
    last_purchase_date DATE,
    last_purchase_price DECIMAL(12,2),
    remark TEXT,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (supplier_id) REFERENCES suppliers(id),
    FOREIGN KEY (material_id) REFERENCES materials(id),
    UNIQUE(supplier_id, material_id)
);
```

---

## 五、BOM管理

### 5.1 BOM版本管理

```sql
-- BOM版本表
CREATE TABLE bom_versions (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL,
    version_no VARCHAR(10) NOT NULL,               -- V1/V2/V3
    version_name VARCHAR(100),
    status VARCHAR(20) DEFAULT 'DRAFT',            -- DRAFT/RELEASED/OBSOLETE
    released_by INTEGER,
    released_at DATETIME,
    obsolete_reason TEXT,
    ecn_id INTEGER,                                -- 关联的变更单
    item_count INTEGER DEFAULT 0,                  -- 物料行数
    total_cost DECIMAL(12,2),                      -- 总成本(预估)
    remark TEXT,
    created_by INTEGER,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (machine_id) REFERENCES machines(id),
    FOREIGN KEY (ecn_id) REFERENCES ecn(id),
    UNIQUE(machine_id, version_no)
);

CREATE INDEX idx_bom_versions_machine ON bom_versions(machine_id);
CREATE INDEX idx_bom_versions_status ON bom_versions(status);
```

### 5.2 BOM明细表

```sql
-- BOM明细表
CREATE TABLE bom_items (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    bom_version_id INTEGER NOT NULL,

    -- 物料信息
    material_id INTEGER,                           -- 关联物料主数据(可空)
    material_code VARCHAR(30),
    material_name VARCHAR(100) NOT NULL,
    specification TEXT,
    brand VARCHAR(50),

    -- 数量与单位
    quantity DECIMAL(10,2) NOT NULL,
    unit VARCHAR(10) DEFAULT '个',
    unit_price DECIMAL(12,2),                      -- 单价(预估)
    total_price DECIMAL(12,2),                     -- 总价(预估)

    -- 分类与标识
    category VARCHAR(20),                          -- 物料类别
    is_critical BOOLEAN DEFAULT FALSE,             -- 是否关键件
    is_long_lead BOOLEAN DEFAULT FALSE,            -- 是否长交期
    lead_time INTEGER,                             -- 预计交期(天)

    -- 来源
    source_type VARCHAR(20) DEFAULT 'PURCHASE',    -- PURCHASE/OUTSOURCE/STOCK
    supplier_id INTEGER,                           -- 指定供应商

    -- 备注
    remark TEXT,
    design_remark TEXT,                            -- 设计备注

    -- 审计
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (bom_version_id) REFERENCES bom_versions(id) ON DELETE CASCADE,
    FOREIGN KEY (material_id) REFERENCES materials(id),
    FOREIGN KEY (supplier_id) REFERENCES suppliers(id)
);

CREATE INDEX idx_bom_items_version ON bom_items(bom_version_id);
CREATE INDEX idx_bom_items_material ON bom_items(material_id);
CREATE INDEX idx_bom_items_critical ON bom_items(is_critical);
```

### 5.3 BOM状态流转

```
┌────────┐    发布     ┌──────────┐    废弃     ┌──────────┐
│ DRAFT  │ ──────────► │ RELEASED │ ──────────► │ OBSOLETE │
│  草稿  │             │  已发布  │             │  已废弃  │
└────────┘             └──────────┘             └──────────┘
     │                      │
     │                      │ ECN变更
     │                      ▼
     │               ┌──────────┐
     └──────────────►│ 新版本   │
                     │  DRAFT   │
                     └──────────┘
```

---

## 六、采购流程

### 6.1 采购流程图

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           采购全流程                                         │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  BOM发布        采购申请        审批         下单         到货         入库  │
│  ┌─────┐       ┌─────┐       ┌─────┐      ┌─────┐      ┌─────┐      ┌─────┐│
│  │BOM  │──────►│申请 │──────►│审批 │─────►│下单 │─────►│到货 │─────►│入库 ││
│  │发布 │       │生成 │       │流程 │      │执行 │      │确认 │      │完成 ││
│  └─────┘       └─────┘       └─────┘      └─────┘      └─────┘      └─────┘│
│     │             │             │            │            │            │    │
│     │             ▼             │            ▼            ▼            │    │
│     │        ┌─────────┐       │       ┌─────────┐  ┌─────────┐       │    │
│     │        │自动汇总 │       │       │供应商   │  │到货检验 │       │    │
│     │        │采购需求 │       │       │发货跟踪 │  │质量记录 │       │    │
│     │        └─────────┘       │       └─────────┘  └─────────┘       │    │
│     │                          │                                       │    │
│     └──────────────────────────┴───────────────────────────────────────┘    │
│                                      │                                      │
│                                      ▼                                      │
│                              ┌─────────────┐                                │
│                              │  齐套率计算  │                                │
│                              │  缺料预警    │                                │
│                              └─────────────┘                                │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 6.2 采购状态定义

```python
PURCHASE_STATUS = {
    # 申请阶段
    "DRAFT": "草稿",
    "PENDING_APPROVAL": "待审批",
    "APPROVED": "已审批",
    "REJECTED": "已驳回",

    # 执行阶段
    "TO_ORDER": "待下单",
    "ORDERED": "已下单",
    "PARTIAL_RECEIVED": "部分到货",
    "RECEIVED": "已到货",
    "CLOSED": "已关闭",

    # 异常
    "CANCELLED": "已取消",
}
```

### 6.3 采购单数据模型

```sql
-- 采购单主表
CREATE TABLE purchase_orders (
    id INTEGER PRIMARY KEY AUTOINCREMENT,

    -- 编码
    po_code VARCHAR(20) UNIQUE NOT NULL,           -- PO-250712-001
    po_type VARCHAR(20) DEFAULT 'NORMAL',          -- NORMAL/URGENT/SAMPLE

    -- 关联
    project_id INTEGER NOT NULL,
    machine_id INTEGER,
    bom_version_id INTEGER,
    supplier_id INTEGER,

    -- 金额
    total_amount DECIMAL(12,2),
    currency VARCHAR(10) DEFAULT 'CNY',
    tax_rate DECIMAL(5,2),
    tax_amount DECIMAL(12,2),

    -- 时间
    order_date DATE,                               -- 下单日期
    require_date DATE,                             -- 要求交期
    expect_date DATE,                              -- 预计到货
    actual_date DATE,                              -- 实际到货

    -- 状态
    status VARCHAR(20) DEFAULT 'DRAFT',
    approval_status VARCHAR(20),
    approved_by INTEGER,
    approved_at DATETIME,

    -- 收货
    received_amount DECIMAL(12,2) DEFAULT 0,
    received_rate INTEGER DEFAULT 0,               -- 到货率%

    -- 备注
    remark TEXT,
    buyer_id INTEGER,                              -- 采购员

    -- 审计
    created_by INTEGER,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (project_id) REFERENCES projects(id),
    FOREIGN KEY (machine_id) REFERENCES machines(id),
    FOREIGN KEY (supplier_id) REFERENCES suppliers(id),
    FOREIGN KEY (buyer_id) REFERENCES employees(id)
);

-- 采购单明细
CREATE TABLE purchase_order_items (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    po_id INTEGER NOT NULL,

    -- 物料
    bom_item_id INTEGER,                           -- 关联BOM行
    material_id INTEGER,
    material_code VARCHAR(30),
    material_name VARCHAR(100) NOT NULL,
    specification TEXT,

    -- 数量与价格
    quantity DECIMAL(10,2) NOT NULL,               -- 采购数量
    unit VARCHAR(10),
    unit_price DECIMAL(12,2),
    total_price DECIMAL(12,2),

    -- 交期
    require_date DATE,
    expect_date DATE,
    actual_date DATE,

    -- 收货
    received_qty DECIMAL(10,2) DEFAULT 0,
    pending_qty DECIMAL(10,2),                     -- 待收数量

    -- 状态
    status VARCHAR(20) DEFAULT 'PENDING',          -- PENDING/PARTIAL/RECEIVED/CANCELLED
    is_critical BOOLEAN DEFAULT FALSE,

    -- 备注
    remark TEXT,

    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (po_id) REFERENCES purchase_orders(id) ON DELETE CASCADE,
    FOREIGN KEY (bom_item_id) REFERENCES bom_items(id),
    FOREIGN KEY (material_id) REFERENCES materials(id)
);

-- 索引
CREATE INDEX idx_po_code ON purchase_orders(po_code);
CREATE INDEX idx_po_project ON purchase_orders(project_id);
CREATE INDEX idx_po_supplier ON purchase_orders(supplier_id);
CREATE INDEX idx_po_status ON purchase_orders(status);
CREATE INDEX idx_po_items_po ON purchase_order_items(po_id);
CREATE INDEX idx_po_items_material ON purchase_order_items(material_id);
```

### 6.4 到货记录

```sql
-- 到货记录表
CREATE TABLE purchase_receipts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,

    -- 编码
    receipt_code VARCHAR(20) UNIQUE NOT NULL,      -- RCV-250712-001

    -- 关联
    po_id INTEGER NOT NULL,
    po_item_id INTEGER NOT NULL,
    project_id INTEGER,
    machine_id INTEGER,

    -- 到货信息
    received_qty DECIMAL(10,2) NOT NULL,
    received_date DATE NOT NULL,
    warehouse VARCHAR(50),                         -- 入库仓库
    location VARCHAR(50),                          -- 库位

    -- 质量
    quality_status VARCHAR(20) DEFAULT 'PENDING',  -- PENDING/PASSED/FAILED
    inspection_date DATE,
    inspector_id INTEGER,
    reject_qty DECIMAL(10,2) DEFAULT 0,
    reject_reason TEXT,

    -- 单据
    delivery_note VARCHAR(50),                     -- 送货单号
    invoice_no VARCHAR(50),                        -- 发票号

    -- 备注
    remark TEXT,
    receiver_id INTEGER,                           -- 收货人

    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (po_id) REFERENCES purchase_orders(id),
    FOREIGN KEY (po_item_id) REFERENCES purchase_order_items(id),
    FOREIGN KEY (receiver_id) REFERENCES employees(id)
);

CREATE INDEX idx_receipts_po ON purchase_receipts(po_id);
CREATE INDEX idx_receipts_date ON purchase_receipts(received_date);
CREATE INDEX idx_receipts_quality ON purchase_receipts(quality_status);
```

---

## 七、齐套率计算

### 7.1 齐套率定义

```
齐套率 = 已到货物料行数 / BOM总物料行数 × 100%

或按金额加权：
加权齐套率 = Σ(已到货行金额) / Σ(BOM行金额) × 100%
```

### 7.2 齐套率计算逻辑

```python
def calculate_material_ready_rate(machine_id, method='count'):
    """
    计算机台物料齐套率

    Args:
        machine_id: 机台ID
        method: 计算方法 'count'按行数 / 'amount'按金额

    Returns:
        ready_rate: 齐套率(0-100)
        details: 详细信息
    """
    # 获取已发布BOM
    bom = get_released_bom(machine_id)
    if not bom:
        return 0, {"error": "BOM未发布"}

    bom_items = get_bom_items(bom.id)
    if not bom_items:
        return 100, {"message": "BOM无物料"}

    ready_items = []
    pending_items = []

    for item in bom_items:
        received = get_received_quantity(machine_id, item.material_code)
        required = item.quantity

        if received >= required:
            ready_items.append(item)
        else:
            pending_items.append({
                "item": item,
                "required": required,
                "received": received,
                "shortage": required - received
            })

    if method == 'count':
        ready_rate = len(ready_items) / len(bom_items) * 100
    else:  # amount
        total_amount = sum(i.total_price or 0 for i in bom_items)
        ready_amount = sum(i.total_price or 0 for i in ready_items)
        ready_rate = ready_amount / total_amount * 100 if total_amount > 0 else 0

    return round(ready_rate, 1), {
        "total_items": len(bom_items),
        "ready_items": len(ready_items),
        "pending_items": pending_items
    }
```

### 7.3 齐套状态视图

```sql
-- 机台齐套状态视图
CREATE VIEW v_machine_material_status AS
SELECT
    m.id AS machine_id,
    m.machine_code,
    m.project_id,
    bv.id AS bom_version_id,
    bv.version_no,
    COUNT(bi.id) AS total_items,
    SUM(CASE WHEN COALESCE(pr.received_qty, 0) >= bi.quantity THEN 1 ELSE 0 END) AS ready_items,
    ROUND(
        SUM(CASE WHEN COALESCE(pr.received_qty, 0) >= bi.quantity THEN 1 ELSE 0 END) * 100.0 / COUNT(bi.id),
        1
    ) AS ready_rate,
    SUM(CASE WHEN bi.is_critical = 1 AND COALESCE(pr.received_qty, 0) < bi.quantity THEN 1 ELSE 0 END) AS critical_shortage
FROM machines m
JOIN bom_versions bv ON bv.machine_id = m.id AND bv.status = 'RELEASED'
JOIN bom_items bi ON bi.bom_version_id = bv.id
LEFT JOIN (
    SELECT po_item_id, SUM(received_qty) AS received_qty
    FROM purchase_receipts
    WHERE quality_status = 'PASSED'
    GROUP BY po_item_id
) pr ON pr.po_item_id = bi.id
GROUP BY m.id, bv.id;
```

---

## 八、缺料预警

### 8.1 预警规则

| 预警级别 | 条件 | 颜色 | 响应 |
|:--------:|------|:----:|------|
| 🔴 严重 | 关键物料缺货且影响装配 | 红色 | 立即处理，通知PM/PMC |
| 🟠 紧急 | 物料交期延迟>3天 | 橙色 | 24小时内处理 |
| 🟡 注意 | 齐套率<80%且临近装配 | 黄色 | 关注跟进 |
| 🔵 提示 | 物料交期临近 | 蓝色 | 正常跟踪 |

### 8.2 预警数据模型

```sql
-- 缺料预警表
CREATE TABLE shortage_alerts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,

    -- 关联
    project_id INTEGER NOT NULL,
    machine_id INTEGER,
    bom_item_id INTEGER,
    po_item_id INTEGER,

    -- 物料信息
    material_code VARCHAR(30),
    material_name VARCHAR(100),

    -- 预警信息
    alert_level VARCHAR(20) NOT NULL,              -- CRITICAL/URGENT/WARNING/INFO
    alert_type VARCHAR(20),                        -- SHORTAGE/DELAY/LOW_STOCK
    shortage_qty DECIMAL(10,2),                    -- 缺口数量
    delay_days INTEGER,                            -- 延期天数

    -- 影响评估
    affect_stage VARCHAR(10),                      -- 影响的项目阶段
    affect_date DATE,                              -- 影响日期（如装配日期）
    impact_description TEXT,

    -- 处理
    status VARCHAR(20) DEFAULT 'OPEN',             -- OPEN/HANDLING/RESOLVED/CLOSED
    handler_id INTEGER,
    handle_time DATETIME,
    handle_result TEXT,
    resolve_plan TEXT,

    -- 通知
    notified_users TEXT,                           -- 已通知人员(JSON)
    last_notify_time DATETIME,

    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (project_id) REFERENCES projects(id),
    FOREIGN KEY (machine_id) REFERENCES machines(id),
    FOREIGN KEY (handler_id) REFERENCES employees(id)
);

CREATE INDEX idx_shortage_project ON shortage_alerts(project_id);
CREATE INDEX idx_shortage_level ON shortage_alerts(alert_level);
CREATE INDEX idx_shortage_status ON shortage_alerts(status);
```

### 8.3 预警生成逻辑

```python
def generate_shortage_alerts():
    """
    定时任务：生成缺料预警
    建议执行频率：每天早上8:00
    """
    alerts = []

    # 获取所有进行中的项目
    projects = get_active_projects()

    for project in projects:
        for machine in project.machines:
            # 跳过已出货机台
            if machine.actual_ship_date:
                continue

            # 获取齐套状态
            ready_rate, details = calculate_material_ready_rate(machine.id)

            # 规则1：关键物料缺货
            for item in details.get('pending_items', []):
                if item['item'].is_critical:
                    alerts.append({
                        'project_id': project.id,
                        'machine_id': machine.id,
                        'bom_item_id': item['item'].id,
                        'material_code': item['item'].material_code,
                        'material_name': item['item'].material_name,
                        'alert_level': 'CRITICAL',
                        'alert_type': 'SHORTAGE',
                        'shortage_qty': item['shortage'],
                        'affect_stage': 'S6',
                        'affect_date': machine.plan_assembly_date,
                        'impact_description': f"关键物料缺货{item['shortage']}，影响装配"
                    })

            # 规则2：齐套率过低
            if ready_rate < 80 and is_approaching_assembly(machine, days=14):
                alerts.append({
                    'project_id': project.id,
                    'machine_id': machine.id,
                    'alert_level': 'WARNING',
                    'alert_type': 'LOW_STOCK',
                    'impact_description': f"齐套率{ready_rate}%，装配日期临近"
                })

            # 规则3：交期延迟
            delayed_items = get_delayed_po_items(machine.id)
            for item in delayed_items:
                alerts.append({
                    'project_id': project.id,
                    'machine_id': machine.id,
                    'po_item_id': item.id,
                    'material_code': item.material_code,
                    'material_name': item.material_name,
                    'alert_level': 'URGENT' if item.delay_days > 3 else 'INFO',
                    'alert_type': 'DELAY',
                    'delay_days': item.delay_days,
                    'impact_description': f"物料延期{item.delay_days}天"
                })

    # 保存预警并发送通知
    save_alerts_and_notify(alerts)
    return alerts
```

### 8.4 预警与项目联动

```python
def update_project_health_from_shortage(project_id):
    """
    根据缺料预警更新项目健康度
    """
    # 获取未解决的严重预警
    critical_alerts = get_open_alerts(project_id, level='CRITICAL')

    if critical_alerts:
        # 有严重缺料 → 健康度H3阻塞
        update_project_health(project_id, 'H3')
        update_project_status(project_id, 'ST14')  # 缺料阻塞
        return

    # 获取未解决的紧急预警
    urgent_alerts = get_open_alerts(project_id, level='URGENT')

    if urgent_alerts:
        # 有紧急预警 → 健康度H2有风险
        update_project_health(project_id, 'H2')
        return
```

---

## 九、API接口设计

### 9.1 物料管理

```
GET    /materials                      # 物料列表
POST   /materials                      # 创建物料
GET    /materials/{id}                 # 物料详情
PATCH  /materials/{id}                 # 更新物料
GET    /materials/{id}/suppliers       # 物料供应商列表
GET    /materials/{id}/alternatives    # 替代物料
```

### 9.2 供应商管理

```
GET    /suppliers                      # 供应商列表
POST   /suppliers                      # 创建供应商
GET    /suppliers/{id}                 # 供应商详情
PATCH  /suppliers/{id}                 # 更新供应商
GET    /suppliers/{id}/materials       # 供应商物料
GET    /suppliers/{id}/orders          # 供应商订单历史
```

### 9.3 BOM管理

```
GET    /projects/{project_id}/machines/{id}/bom   # 机台BOM列表
POST   /projects/{project_id}/machines/{id}/bom  # 创建BOM版本
GET    /bom/{id}                       # BOM详情
PATCH  /bom/{id}                       # 更新BOM
POST   /bom/{id}/release               # 发布BOM
POST   /bom/{id}/items                 # 添加BOM行
PATCH  /bom/{id}/items/{item_id}       # 更新BOM行
DELETE /bom/{id}/items/{item_id}       # 删除BOM行
POST   /bom/{id}/import                # 导入BOM(Excel)
GET    /bom/{id}/export                # 导出BOM
```

### 9.4 采购管理

```
# 采购单
GET    /purchase-orders                # 采购单列表
POST   /purchase-orders                # 创建采购单
GET    /purchase-orders/{id}           # 采购单详情
PATCH  /purchase-orders/{id}           # 更新采购单
POST   /purchase-orders/{id}/submit    # 提交审批
POST   /purchase-orders/{id}/approve   # 审批
POST   /purchase-orders/{id}/order     # 确认下单
POST   /purchase-orders/{id}/cancel    # 取消

# 到货
POST   /purchase-orders/{id}/receive   # 登记到货
GET    /purchase-receipts              # 到货记录列表
```

### 9.5 齐套与预警

```
GET    /machines/{id}/material-status  # 机台齐套状态
GET    /projects/{id}/material-status  # 项目齐套汇总
GET    /shortage-alerts                # 缺料预警列表
PATCH  /shortage-alerts/{id}           # 处理预警
POST   /shortage-alerts/{id}/resolve   # 解决预警
```

---

## 十、页面设计

### 10.1 页面清单

| 页面 | 功能 | 访问角色 |
|------|------|----------|
| 物料列表 | 物料主数据管理 | 采购/PMC |
| 供应商列表 | 供应商管理 | 采购 |
| BOM编辑 | BOM编制与维护 | 工程师/PMC |
| 采购单列表 | 采购单管理 | 采购/PMC |
| 采购单详情 | 采购单详细信息 | 采购/PMC |
| 到货登记 | 到货确认 | 采购/仓库 |
| 齐套看板 | 各项目齐套状态 | PMC/PM |
| 缺料预警 | 预警处理 | PMC/采购 |

### 10.2 齐套看板页

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  齐套看板                                          [刷新] [导出] [预警设置] │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─ 齐套率分布 ─────────────────────┐  ┌─ 预警汇总 ────────────────────────┐│
│  │  100%: ████████ 8个              │  │  🔴 严重: 3    🟠 紧急: 5         ││
│  │  80-99%: ██████ 6个              │  │  🟡 注意: 8    🔵 提示: 12        ││
│  │  60-79%: ████ 4个                │  │                                   ││
│  │  <60%: ██ 2个                    │  │  [查看全部预警]                   ││
│  └──────────────────────────────────┘  └───────────────────────────────────┘│
│                                                                             │
│  项目/机台齐套明细                               筛选: [全部▼] [搜索...]     │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
│  │ 项目号       机台          齐套率    关键缺料  装配日期   状态    操作   ││
│  │ ─────────────────────────────────────────────────────────────────────── ││
│  │ PJ250712001  PN001         🔴 65%    2        08-15     缺料    [详情] ││
│  │              PN002         🟡 78%    1        09-01     跟踪    [详情] ││
│  │ PJ250710002  PN001         🟢 95%    0        08-20     正常    [详情] ││
│  │ PJ250708003  PN001         🟢 100%   0        已装配    完成    [详情] ││
│  │              PN002         🟡 82%    0        08-25     跟踪    [详情] ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 10.3 缺料预警页

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  缺料预警                                    [全部] [待处理] [处理中] [已解决]│
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
│  │ 🔴 严重 | PJ250712001-PN001 | 伺服电机 MSMF042L1U2M                     ││
│  │ 缺口: 2台  |  预计延期: 7天  |  影响装配: 2025-08-15                     ││
│  │ 当前供应商: 松下  |  采购员: 王采购                                      ││
│  │ ────────────────────────────────────────────────────────────────────    ││
│  │ 处理方案:                                                                ││
│  │ ○ 催货加急    ○ 更换供应商    ○ 使用替代物料    ○ 调整计划              ││
│  │                                                               [处理]    ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
│  │ 🟠 紧急 | PJ250712001-PN002 | PLC FX5U-32MT                             ││
│  │ 缺口: 1台  |  预计延期: 4天  |  影响装配: 2025-09-01                     ││
│  │ 当前供应商: 三菱代理  |  采购员: 李采购                                  ││
│  │                                                               [处理]    ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
│  │ 🟡 注意 | PJ250710002-PN001 | 气缸 CDQ2B32-50DZ                         ││
│  │ 齐套率: 78%  |  装配临近: 14天                                           ││
│  │                                                               [跟进]    ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 十一、权限矩阵

| 功能 | PMC | 采购 | 工程师 | 仓库 | PM | 财务 | 管理员 |
|------|:---:|:----:|:------:|:----:|:--:|:----:|:------:|
| 物料主数据 | R/W | R/W | R | R | R | R | R/W |
| 供应商管理 | R | R/W | R | R | R | R | R/W |
| BOM编制 | R/W | R | R/W | R | R | R | R/W |
| BOM发布 | W | - | - | - | W | - | W |
| 采购申请 | W | W | - | - | R | R | W |
| 采购审批 | A | - | - | - | A | A | A |
| 采购下单 | R | W | - | - | R | R | W |
| 到货登记 | R | W | - | W | R | R | W |
| 齐套查看 | R | R | R | R | R | R | R |
| 预警处理 | W | W | - | - | R | - | W |

---

## 十二、配置项

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| 采购单编码前缀 | PO | 采购单编码格式 |
| 到货单编码前缀 | RCV | 到货单编码格式 |
| 齐套率阈值 | 80% | 低于此值触发预警 |
| 关键物料预警天数 | 7 | 关键物料缺货预警提前天数 |
| 交期预警天数 | 3 | 交期延迟预警阈值 |
| 自动生成采购申请 | 开启 | BOM发布后是否自动生成采购申请 |

---

## 十三、与其他模块联动

| 触发模块 | 事件 | 采购模块响应 |
|----------|------|--------------|
| 项目管理 | 新建机台 | - |
| 进度跟踪 | BOM任务完成 | 可触发BOM发布 |
| 变更管理 | ECN审批通过 | 生成新BOM版本，调整采购 |
| 验收管理 | - | - |
| 外协管理 | 外协件到货 | 更新齐套状态 |

| 采购模块事件 | 响应模块 |
|--------------|----------|
| 关键物料缺货 | 项目管理：健康度→H3 |
| 齐套率达标 | 项目管理：可推进至S6 |
| 物料延期 | 预警模块：生成预警 |

---

*本文档与项目管理、变更管理、外协管理模块配套使用。*

...
2026-06-07 继续补了两个上下文断点：

- 需求调研页已读取 `opportunity_id` 和 `ticket_id`，并带到 `/api/v1/presale/tickets`，销售商机进入售前调研时不再丢上下文。
- 技术评审列表和新建评审已读取 `project_id`，项目进入工程/技术评审时默认按项目筛选和带入项目。
...
截至 2026-06-07，项目中心已完成第一批上下文贯通：

- `/projects/` 后端列表支持 `project_id`、`contract_id`、`opportunity_id` 筛选，并在响应里返回合同和商机上下文。
- `/project/management-center` 的项目看板、任务、计划资源、成本页签会保留并透传上述上下文。
- 任务中心实际请求已收口为 `page/page_size/project_id`，不再把前端内部筛选字段误传给后端。
- 甘特、预算、工时成本毛利页签会根据 `project_id` 自动定位到目标项目。
- 需求调研和技术评审已分别接入销售上下文和项目上下文，开始覆盖售前到项目后的工程交接断点。
- 采购订单、生产工单、生产计划已读取 `project_id`，列表请求按项目筛选，新建订单/工单/计划默认带入当前项目。
...
用户刚才强调的点是对的：不能只合并销售、售前和项目看板，后面的工程、BOM、采购、生产、仓储、质量、交付、售后也必须纳入同一条主线。

这里不要把所有页面硬塞进一个超级项目页。对 100-200 人非标自动化公司，更合适的是“两层工作台”：

1. 项目视角
   - `/project/management-center` 和 `/projects/:id/workspace` 是项目经理、老板、跨部门协同看同一个项目的入口。
   - 进入任何后段模块时都必须保留 `project_id / contract_id / opportunity_id`。

2. 专业部门视角
   - 采购、物料、生产、装配、仓储、质量、交付仍保留自己的专业中心。
   - 专业中心不是孤岛，必须支持从项目上下文进入，也必须能按项目筛选。

后段模块建议这样归并：

| 业务段 | 主入口 | 项目上下文要求 |
| --- | --- | --- |
| 工程/技术评审 | `/technical-reviews`，后续挂入项目工作区 | 列表按 `project_id` 筛选，新建默认带项目 |
| BOM/物料需求 | `/procurement/material-center` | BOM、物料需求、齐套率必须支持 `project_id` |
| 采购执行 | `/procurement/execution-center` | 采购申请、采购订单、收货必须支持 `project_id` |
| 生产执行 | `/production/execution-center` | 工单、派工、生产计划必须支持 `project_id` |
| 装配齐套 | `/production/assembly-center` | 装配任务、齐套率、缺料必须支持 `project_id` |
| 仓储 | `/workstation/warehouse` 与仓储列表 | 入库、出库、库存占用、盘点要能回溯项目 |
| 质量 | `/workstation/quality` 与质量列表 | 检验、问题、整改、验收要能按项目追踪 |
| 交付验收 | `/delivery/acceptance-center` | 现场调试、安装、验收要接项目和合同 |
| 售后服务 | `/projects/:projectId/after-sales` | 售后问题要能回到项目、设备、客户、验收记录 |

统一规则：

- 所有跨模块链接都保留已有查询参数，不丢 `project_id / contract_id / opportunity_id`。
- 列表型 API 优先支持 `project_id`；确实没有项目字段的主数据列表保留全局视角。
- 详情页从自身 ID 反查项目上下文，并提供“回到项目工作区”入口。
- 项目工作区只放项目相关切片；专业中心保留部门批量处理能力。
- 验收标准不是页面能打开，而是从一个项目点进去，BOM、采购、生产、质量、验收看到的是同一个项目的数据。
...
- 所有后段专业中心支持 `project_id` 上下文筛选，先覆盖技术评审、BOM、采购、生产、质量、交付验收。

验收：

- 项目经理打开项目能看到合同、报价、售前方案、范围、预算、节点。
- 项目成本和售前成本能对账。
- 从项目进入后段专业模块时，列表和新建动作默认落到当前项目，不需要用户重新筛选。
...
2. 把项目中心的专业深链继续按项目上下文接入：BOM、采购申请、采购订单、收货、生产工单、生产计划、质量检验、验收、交付、售后。
3. 把售前中心和项目中心的数据加载从多个散接口逐步迁到统一上下文接口。
4. 给每个后段专业中心补一条浏览器级冒烟路径：从 `/project/management-center?project_id=...` 进入后段页面，请求必须带项目上下文。

这四步完成后，系统才会从“很多模块都有”变成“从销售到交付能按一条线跑”。

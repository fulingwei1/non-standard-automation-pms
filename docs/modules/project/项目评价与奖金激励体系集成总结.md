...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/projects/{project_id}/evaluations` | 创建项目评价 |
| GET | `/projects/{project_id}/evaluations` | 获取评价列表 |
| GET | `/projects/{project_id}/evaluations/{eval_id}` | 获取评价详情 |
| PUT | `/projects/{project_id}/evaluations/{eval_id}` | 更新评价 |
| POST | `/projects/{project_id}/evaluations/{eval_id}/confirm` | 确认评价 |
| GET | `/projects/{project_id}/evaluations/latest` | 获取项目最新评价 |
...
评价维度配置当前未对外暴露独立 API（兼容层已移除）。
如需调整维度权重或评分规则，请通过服务层或管理后台配置入口统一维护。

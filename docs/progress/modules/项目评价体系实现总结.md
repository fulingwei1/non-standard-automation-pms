...
**文件**: `app/api/v1/endpoints/projects/evaluations/`

#### 已实现端点：
- `POST /api/v1/projects/{project_id}/evaluations` - 创建项目评价
- `GET /api/v1/projects/{project_id}/evaluations` - 获取评价列表
- `GET /api/v1/projects/{project_id}/evaluations/{id}` - 获取评价详情
- `GET /api/v1/projects/{project_id}/evaluations/latest` - 获取项目最新评价
- `PUT /api/v1/projects/{project_id}/evaluations/{id}` - 更新评价
- `POST /api/v1/projects/{project_id}/evaluations/{id}/confirm` - 确认评价

> 说明：旧兼容层已于 2026-01-31 移除，评价维度配置接口不再单独暴露。
...
```bash
POST /api/v1/projects/1/evaluations
{
...
```bash
GET /api/v1/projects/1/evaluations/latest
```
...
└── api/v1/endpoints/
    └── projects/evaluations/           # 项目评价API（项目子路由）

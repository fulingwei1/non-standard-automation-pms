...
| TEN-05 | 隔离装饰器/工具全家 0 使用死代码 | P2 | 待修 | app/core/decorators/tenant_isolation.py；app/core/permissions/tenant_access.py；app/core/middleware/tenant_middleware.py 的 TenantAwareQuery/require_same_tenant | 删 0.5d（复核：实际更大） | 静态已证；复核见备注 | 复核：审计"0.5d 快删"低估了工作量——这几处死代码都配有真实的专门单元测试（tests/test_tenant_isolation_decorator.py、tests/test_tenant_access.py、tests/unit/test_core_common_coverage.py、tests/middleware/test_tenant_middleware.py、tests/unit/test_tenant_middleware_utils.py 合计约 30 个用例），删源码必须同步清理/重写这些测试，不是纯删除；本轮未做，留待后续单独排期 |

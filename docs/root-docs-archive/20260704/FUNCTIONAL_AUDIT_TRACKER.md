...
**2026-07-04 追加进展**：RPT-01 已修复报表中心“待实现”桩静默空 200；now 报表类型配置和角色权限矩阵只展示真实可生成的 6 类报表，绕过前端直调未实现类型时返回 error，不再生成空报表记录。RPT-08 已修复 PPT 生成器硬编码 demo；now 主生成入口必须显式传入 `deck_spec`，生成内容完全来自调用方数据，旧硬编码营销文案已移除，并补齐 builder 兼容路径。RPT-07 已收敛 template_report 三套实现；now adapter、旧根服务、数据服务都走 `TemplateReportCore`，断链 import 已移除。RPT-05/SALES-17 已补报价版本、合同和财务报表含税/不含税建模；now 报价/报价版本保存 `amount_without_tax/tax_rate/tax_amount/amount_with_tax`，合同从报价继承税口径，财务月趋势/成本分析/项目盈利/现金流显式返回不含税、税额和含税字段。SALES-19 已修复已收款发票作废红冲链路；now 作废已开票单生成 `RED_CREDIT` 负票并保留原回款审计痕迹，合同累计开票额度排除红冲负票。PROJ-23 已修复终验收后售后移交事实不完整；now 主链 `transfer_to_after_sales` 同时生成 ACTIVE 质保档、定期保养计划，并回填项目/机台质保与客户归属。PROJ-24 已修复项目复盘 AI 降级语义错配；now 识别 `*-mock` 后只写规则复盘底稿并标 `degraded`，不再把预售方案 mock 当成真实复盘 AI 入库。PROJ-26 已修复自动组队未吃立项信息和经验固定满分；now 按已批准立项资源/预计工时生成角色与工时，经验分读取历史任务质量、准时和返工信号。PROD-18 已补生产排程显式工单依赖；now `constraints.dependencies` 可约束后置工单不得早于前置完工，甘特图回显依赖任务。PROD-19 已补委外交付/收货闭环；now 外协订单可挂生产工单，创建交付会拦截超交，`PUT /outsourcing-deliveries/{delivery_id}/receive` 确认收货并同步实收数量/订单状态。PROD-20 已补 ECN 到采购影响传导；now `sync-to-purchase` 会按受影响物料自动生成采购影响单，`MODIFY` 标记采购待评审而非空 pass/假处理。PROD-21 已补移动扫码开工离线队列和 iOS 降级；now 离线开工暂存到本地队列并联网自动补传，无 `BarcodeDetector` 时给手输/系统相机提示而非 alert 中断。PROD-23 已收口采购/报工状态机双轨；now 采购提交/审批/统一审批适配器共用采购订单状态机，生产报工开工和完工审批复用工单状态机校验。PROD-24 已修正委外成本归集口径；now 外协实际成本按质检合格数量×单价确认，合格量为 0 时删除旧成本并重算项目实际成本。ADMIN-18 已修复合同附件下载任意文件读取；now 新旧两个合同附件下载入口都把路径解析到 `UPLOAD_DIR` 内，绝对路径/路径穿越返回 403。AS-14 已修复设备保养提醒调度与终验转售后定期保养计划联动。HR-02 已修复离职审批后账号仍可登录；now 审批 resignation 会同步停用绑定 `User` 并返回停用账号数。HR-04 已补组织员工/部门删除接口；now 前端 `DELETE /org/employees/{id}` 软停用员工，`DELETE /org/departments/{id}` 在无子部门/在职员工时软停用部门。HR-06/07 已完成考勤假数据止损；now `/admin/attendance` 返回显式空态，不再合成迟到/请假/出勤率，打卡/单条记录未接真实域时返回 501。HR-08 已下架考勤页未接入的请假/加班假入口；now 只展示部门统计/考勤记录。HR-09 已接通节假日 DB 消费链；work-log 规则引擎 now 有 DB 时优先读取 `holidays`，无 DB 时保留静态日历兜底。HR-21 已修复时薪全级 miss 静默兜底与费率改删无留痕；now API 返回来源/兜底标记，PUT 生成新版本并到期旧版本，DELETE 软停用保留历史查询。

**2026-07-05 进展**：PERM-15/16/17（数据权限打包三项）已全部闭环。PERM-15 修复 `check_customer_access` CUSTOMER 域恒 True 放行，改为与 `sales_permissions.py` 一致的降级为项目口径校验，无匹配项目直接拒绝（线上 0 角色配置 CUSTOMER 域，属防御性修复）。PERM-16 复核发现 `get_user_data_scopes` 并非真死代码而是"死在实践中"（垃圾种子导致恒返回空），清空 `role_data_scopes`/`data_scope_rules` 垃圾种子，不建管理 API（无实际需求，`Role.data_scope` 单字段已是唯一生效机制）。PERM-17 按 6 个域拆解并行修复（production/purchase+outsourcing/bom+ecn/inventory+warehouse/budget+cost+finance_reports/timesheet），复用 sales 域已验证的 `GenericFilterService.filter_by_scope` 简单口径，只挂载有真实归属概念的逐行列表端点，聚合报表/共享主数据/已有等效审批链路口径的端点保留跳过并逐项记录理由；6 组新增测试+既有回归全绿。此次是审计以来首次对现网用户产生实质行为变更：15 个非 ALL 域用户（8 DEPARTMENT/4 OWN/3 PROJECT）在这些域今后只能看到自己范围内数据。

多租户 Batch 路线 TEN-02/03/04 同日一并推进（TEN-06 之后按既定顺序：TEN-02 查询层→TEN-03 Project 域切片）。TEN-02 用 `do_orm_execute`+`with_loader_criteria` 重做全局租户过滤，替换掉只重写 `__iter__`、在 SQLAlchemy 2.0 下对 `.all()/.first()/.count()` 全部失效的旧 `TenantQuery`；过程中连续踩中两个 SQLAlchemy lambda-SQL 缓存坑，其中一个是会导致跨请求复用上一次绑定租户 ID 的真实数据泄露级 bug（已用最小复现脚本定位并改用"回调外部算好普通表达式"的写法绕开）。顺带发现 `Role/ApiPermission/DataScopeRule/MenuPermission` 的 tenant_id=NULL 是"系统级共享"语义（与 `User` 的 NULL=超管相反），一开始按统一严格相等实现导致既有角色隔离回归测试失败，已修正为按模型区分。TEN-03 只做了 Project 域切片：`projects.tenant_id` 实测是比审计描述更严重的幽灵列（ORM 模型从未声明该字段，不是"未消费"是完全读写不了），现已补声明+存量归户，顺带解锁了一批此前卡在这个问题上的既有租户隔离测试。TEN-04 因 TEN-02 是全局框架级过滤而基本自动解决，不再需要逐端点手工挂载。TEN-05 复核后发现审计"0.5d"估时不现实（配套约 30 个专门单元测试，删源码必须同步处理测试），本轮未做，留待后续。TEN-07（配额/生命周期执行）随后同日补上：登录流程接入 `TenantService.check_tenant_login_allowed`，暂停/过期租户下的非超管用户登录返回 403；建用户流程接入 `check_user_quota`，达到 `max_users` 上限拒绝创建。管理端（TEN-01）早就能设置这些字段，此前从未被任何流程读取。TEN-03 全量铺开（customers/sales_orders/contracts/invoices 等其余表）仍未动，是这条线下一步的候选。
...
| PROJ-23 | 验收通过后无售后/ITR 移交联动（售后需人工重建） | P2 | 已验证 | acceptance/order_workflow.py；acceptance_completion_service.py；project_data_flow_service.py；tests/unit/test_project_after_sales_handover_proj23.py；tests/unit/test_acceptance_aftersales_handover.py | 2d | 静态已证；✅契约+主链服务层回归（2026-07-04） | 详#18；主链终验收已调用售后移交；移交 now 创建 ACTIVE 质保档、4 条定期保养计划，回填项目质保日期和机台客户/质保；异步 SAT 分支也有幂等质保移交；ITR 继续通过现有验收/工单 read model 读取上下文；关联 AS-10/AS-12 |
| PROJ-24 | 项目复盘可用，但 AI 降级 mock 语义错配（预售文案进复盘） | P3 | 已验证 | project_review_ai/report_generator.py；ai_client_service.py:is_mock_response；tests/unit/test_review_report_generator.py | 1-2d | 静态已证；✅契约测试+离线 API 回归（2026-07-04） | 详#19；`*-mock` now `ai_generated=False`、metadata 标 `degraded/degraded_reason`，复盘内容改用项目数据规则底稿，不再写入预售方案 mock；附带：change_impact_ai_service.py 653 行死代码无端点 |
| PROJ-25 | 交付风险 AI（ai_delivery，规则真算） | — | 已验证 | — | — | 静态已证（正面确认） | 无缺陷；但只认 EXECUTING，ST 码项目不进扫描（随 PROJ-05 清洗解决） |
| PROJ-26 | 团队组建未接入立项；经验维度写死 20 分 | P3 | 已验证 | team_generation_service.py；tests/unit/test_team_generation_proj26.py；test_openapi_route_contracts.py | 1-2d | 静态已证；✅单元+路由注册回归（2026-07-04） | 详#20；自动组队 now 回查已批准 `PmoProjectInitiation`，按资源要求补视觉/软件/测试等角色，按立项预计工时分摊；经验分从历史 `EngineerTaskAssignment` 质量/准时/返工和能力画像计算，不再固定 20 分 |
...
| PROD-18 | 排产主算法真实但无工序依赖（单工序工单模型，dependencies=[]） | P2 | 已验证 | production_schedule_service.py；tests/unit/test_production_schedule_dependencies_prod18.py；tests/unit/test_production_schedule_n2.py | 4-6d | 静态已证；✅单元回归（2026-07-04） | 短期闭环：生成请求支持 `constraints.dependencies`，排程按依赖拓扑校正并把后置工单开始时间约束到前置完工后；`constraints_met.dependencies` 入库，甘特图把前置工单映射成前置任务 ID 回显；完整工艺路线/多工序工单模型仍待后续专项 |
| PROD-19 | 委外：超交无校验、无收货确认端点、订单不挂工单 | P2 | 已验证 | outsourcing/deliveries.py；outsourcing/orders.py；models/outsourcing.py；schemas/outsourcing.py；tests/unit/test_outsourcing_delivery_prod19.py | 3-4d | 静态已证；✅单元+路由注册回归（2026-07-04） | 详#21；订单 now 可挂 `work_order_id` 并校验项目归属；交付创建按订单剩余数量拒绝超交；新增收货确认端点同步 `received_quantity/received_by/RECEIVED` 状态；附带修复供应商名字段和 Decimal 税额混算 |
| PROD-20 | ECN 影响传导到采购靠手工（受影响记录手建、MODIFY 分支空 pass） | P2 | 已验证 | ecn_integration_service.py；tests/unit/test_ecn_purchase_sync_prod20.py；ecn/integration.py | 3d | 静态已证；✅单元+路由注册回归（2026-07-04） | 详#22；`sync_to_purchase()` now 从 `EcnAffectedMaterial` 反查有效采购订单行并幂等生成 `EcnAffectedOrder`；`MODIFY` 写 `CHANGE_REQUIRED`、处理人/时间和采购单 ECN 备注，保留原采购状态等待采购评审；`sync-to-purchase` 路由已确认注册 |
| PROD-21 | 车间移动端无离线队列、扫码 iOS 不兼容 | P2 | 已验证 | MobileScanStart.jsx；mobileScanStartHelpers.js；mobileScanStartHelpers.test.js | 4-5d | 静态已证；✅前端单测+构建回归（2026-07-04） | 详#23；开工接口网络失败/离线时写本地 `mobile.scanStart.offlineStartQueue.v1` 队列，恢复在线自动补传；无 `BarcodeDetector`/iOS 时展示系统相机/手动输入降级提示，不再 alert 中断；完整图片二维码解码仍需后续引入 jsQR/原生扫码能力 |
...
| PROD-23 | 状态机治理双轨（采购两套并行审批改 PO.status；报工内联判断不走状态机） | P2 | 已验证 | purchase/order_state_machine.py；orders_refactored.py；approval_engine/adapters/purchase.py；purchase_service.py；work_reports.py；tests/unit/test_state_governance_prod23.py | 2d | 静态已证；✅单元+采购 workflow/API+报工端点回归（2026-07-04） | 集群6；采购订单提交/审批、旧 PurchaseService 与统一审批适配器 now 共用 `transition_purchase_order_status()`；报工开工与完工审批 now 调用 `work_order_state_machine.validate_transition()`，不再靠内联状态判断绕过状态机 |
| PROD-24 | 委外成本归集口径偏差（审批即全额入账，不随质检合格量冲减） | P3 | 已验证 | cost_collection_service.py；outsourcing_workflow_service.py；tests/unit/test_outsourcing_cost_collection_prod24.py | 2d | 静态已证；✅单元+成本归集+委外质检回归（2026-07-04） | 外协成本 now 读取订单明细 `qualified_quantity`，按合格数量×单价确认实际成本并按比例确认税额；合格量为 0 时不创建成本，已有旧成本会删除并重算项目实际成本，避免审批早于质检时全额入账 |
...
| HR-03 | 数据权限绑部门名字符串，组织变动不随动 | P1 | 已验证 | data_scope/generic_filter.py；tests/unit/test_data_scope_filter_coverage.py | 2-3d+清洗0.5d | 静态已证；✅红后绿单元+PERM-17 相关回归（2026-07-04） | DEPT 范围 now 优先按 `user.department_id/dept_id/owner_dept_id` 过滤，旧部门名仅在无 ID 时兜底；调岗用户不再被旧部门名字符串带偏。存量字符串清洗仍归 HR-05/组织数据治理 |
| HR-04 | 部门/员工删除前端调不存在端点（405/404） | P2 | 已验证 | hr.js；organization/employees.py；organization/departments_refactored.py；tests/api/test_org_delete_hr04.py | 1d | 静态已证；✅API+路由注册+组织回归（2026-07-04） | 前端既有 `employeeApi.delete`/`departmentApi.delete` now 有后端承接；员工删除为软停用并置 `employment_status=resigned`；部门删除为软停用，存在启用子部门或在职员工时返回 400 |
| HR-05 | 员工-部门无外键、双主数据字符串关联（同义词并存） | P2 | 已验证 | organization.py；organization/employees.py；organization/departments_refactored.py；tests/api/test_org_department_id_hr05.py；migrations/20260704_employee_department_id_sqlite.sql | 2-3d | 静态已证；✅红后绿 API+组织回归（2026-07-04） | `Employee` now 有 `department_id` 外键/索引，员工创建更新传 ID 会同步旧 `department` 名；部门统计/删除保护/用户列表优先按 ID，旧字符串仅在 ID 为空时兜底。精确部门名回填迁移已补；复杂同义词/历史字符串清洗不自动猜测 |
...
| HR-08 | 请假缺失、加班僵尸模型、补卡缺失 | P1 | 已验证 | AttendanceManagement.jsx；AttendanceManagement.test.jsx | 5-8d/摘Tab0.5d | 静态已证；✅前端入口止损回归（2026-07-04） | 考勤页 now 只保留“部门统计/考勤记录”两个已接入页签，页头不再宣称请假/加班管理；未做完整请假/加班/补卡域，后续需按正式考勤域补模型、审批和排产输入 |
| HR-09 | 节假日双轨、DB 模型零消费（真消费的是硬编码字典） | P3 | 已验证 | holiday_utils.py；work_log_ai/rule_engine.py；tests/unit/test_holiday_db_consumption_hr09.py | 1d | 静态已证；✅DB 节假日消费回归（2026-07-04） | `holiday_utils` now 支持可选 DB 并优先读取 `HolidayService`；工作日志规则引擎有 `self.db` 时传入 DB，真实 `holidays` 配置可决定 HOLIDAY/WORKDAY；无 DB 调用仍保留静态中国节假日日历兜底 |
...
| HR-11 | 绩效评分维度写死常量（五维至少两维对全岗恒定） | P1 | 已验证 | performance_calculator.py；tests/unit/test_performance_collector_integration_hr11_12.py | 3-5d(与HR-10打包) | 静态已证；✅红后绿单元+采集器回归（2026-07-04） | `PerformanceCalculator` now 消费 `PerformanceDataAggregator`：设计/调试进入技术分，任务完成/准时进入执行分，BOM+ECN 进入质量分，知识贡献进入知识分；无采集数据时保留旧兜底 |
| HR-12 | 绩效采集器与算分器双轨不联通（空数据回落 75） | P2 | 已验证 | performance_collector/aggregator.py；performance_calculator.py；tests/unit/test_performance_collector_integration_hr11_12.py | 并入 HR-11 | 静态已证；✅算分器消费采集器回归（2026-07-04） | 采集器输出 now 被正式工程师绩效落库链路消费；`calculate_and_save_result` 会把采集指标写进 `performance_result.indicator_scores` |
| HR-13 | 绩效申诉缺失（模型完整但零写入无端点） | P2 | 已验证 | appeal_adjustment.py:13；performance/appeals.py；tests/unit/test_performance_appeals_hr13.py | 2d | 静态已证；✅红后绿单元+路由契约（2026-07-04） | 申诉提交/列表/处理 now 写 `performance_appeal`，接受申诉时同步更新 `performance_result` 并写 `PerformanceAdjustmentHistory`；后续如需统一审批引擎，可在处理端点前接审批流 |
| HR-14 | 三套绩效体系并行隔绝、服务大面积复制粘贴 | P2 | 已验证 | performance_service/calculation.py；manager_performance_service.py；tests/unit/test_performance_unification_hr14.py | 3d | 静态已证；✅红后绿单元+月度/经理绩效回归（2026-07-04） | 月度员工/经理评价完成 now upsert 正式 `PerformanceResult`：创建/复用 `MONTHLY-YYYY-MM` 周期，写总分、等级、部门、`indicator_scores`，个人绩效/奖金/排名下游可统一读正式结果；深度去重仍可后续结构优化 |
| HR-15 | 绩效合同绕 ORM 用裸 sqlite3（连接串写死） | P2 | 已验证 | performance/contract.py；models/performance/contract.py；tests/unit/test_performance_contract_import.py | 1d | 静态已证；✅红后绿单元+静态扫描（2026-07-04） | `performance/contract.py` 去掉 import 期 DDL 与裸 `sqlite3.connect`，新增 `PerformanceContract/PerformanceContractItem` ORM 模型并注册；合约 CRUD、条目权重、提交、签署主流程均走注入 Session；MISC-12 继续合并到本项 |
...
| HR-19 | 奖金系数硬编码非规则驱动（等级/角色/售前系数全写死） | P2 | 已验证 | services/bonus/base.py；services/bonus/performance.py；services/bonus/presale.py；tests/unit/test_bonus_rule_coefficients_hr19.py | 1d | 静态已证；✅红后绿单元+相邻回归（2026-07-04） | 绩效等级系数 now 可从 `BonusRule.trigger_condition.performance_coefficients` 配置；售前紧急/满意度系数 now 可从 `urgency_coefficients` / `satisfaction_coefficients` 配置；角色系数原已支持 `role_coefficients`；未配置时保留旧默认兼容 |
| HR-20 | 时薪费率体系：本体真实，旁路 14 处写死 | P1 | 已验证 | report_labor_cost.py；template_report/analysis_reports.py；presale_analytics/resource_analysis.py；presale_analytics/dashboard.py；dashboard/adapters/presales.py；resource_waste_analysis/*；cost_endpoints/labor_cost_detail.py；loss_deep_analysis_service.py；cost/labor_cost_service.py；tests/unit/test_hourly_rate_consumers_hr20.py | 1.5d | 静态已证；✅红后绿单元+相邻回归+硬编码扫描（2026-07-04） | 旧模板成本分析、售前资源投入/浪费看板、resource_waste_analysis、人工成本 by-engineer、未中标投入分析均改为按 `Timesheet.user_id + work_date` / Worker 绑定 User 读取 `HourlyRateService`；成本统计仅计 `APPROVED` 工时；缺失用户走统一可追踪兜底，不再散落 200/300 固定时薪 |
...
| HR-23 | 冲突调解：真算法架在无写入者的空表上（resource_conflicts 零写入） | P2 | 已验证 | analytics/resource_conflicts.py；conflict_mediation_service.py；tests/unit/test_resource_conflict_persistence_hr23.py | 检测落库1d/收敛2-3d | 静态已证；✅红后绿单元+冲突调解回归（2026-07-04） | 项目冲突检查 now 扫描全局同员工重叠分配并幂等 upsert `resource_conflicts`；跨项目 70%+60% 会落库；调解服务补齐识别/解决/升级/历史直调入口 |
...
| PERM-15 | 数据权限 ALL/DEPT/OWN 过滤大量静默降级；CUSTOMER 恒 True | P1 | 已验证 | app/services/data_scope/generic_filter.py check_customer_access；tests/unit/test_data_scope_filter_coverage.py | 3-5d | 静态已证；✅单元回归（2026-07-05） | 关联 HR-03；三系统性根因②；复核：DEPT/CUSTOM 分支原本就是失败时静默降级为 OWN（安全方向，非泄漏），真正的失败开放漏洞只有 CUSTOMER 恒 True 一处，现已改为与 sales_permissions.py 一致的"客户门户降级为项目口径"，无匹配项目直接拒绝；线上当前 0 个角色配置 CUSTOMER 域，本次为防御性修复非在网漏洞止血；PERM-16/17（RoleDataScope 配置层+挂载率）另行处理，见下一行 |
| PERM-16 | RoleDataScope 配置层坏死（无 API、DB 垃圾种子 is_active 全 NULL） | P1 | 已验证 | migrations/20260705_permission_data_scope_seed_cleanup_sqlite.sql | 3-5d | 静态已证；✅复核（2026-07-05） | 复核：`get_user_data_scopes` 并非死代码（engperf_scope.py/sales_scope.py/data_scope_service_enhanced.py 均调用），只是因垃圾种子恒返回 `{}` 而"死在实践中"；`RoleDataScope`/`DataScopeRule` 在 ghost_tables_baseline 中已标记幽灵表，无任何真实业务消费，权衡后不建管理 API（当前无自定义数据权限规则的产品需求），只清空 6+3 行垃圾种子，实际生效机制统一为 `Role.data_scope` 单字段 |
| PERM-17 | 数据权限挂载率：制造/供应链/财务全域 0 行级过滤 | P1 | 已验证 | 6 个域分别挂载 DataScopeService.filter_by_scope：production(material_requisitions.py/work_reports.py)、purchase/outsourcing(requests_refactored.py/receipts.py/outsourcing/orders.py/payments/crud.py)、bom/ecn(bom/list.py/machine_bom.py/ecn/core.py/statistics.py)、inventory/warehouse(warehouse/crud.py/count.py/stock_count_service.py)、budget/cost/finance_reports(budgets.py/items.py/collection.py/finance_reports.py)、timesheet(reports.py/reports_unified.py/statistics.py)；tests/**/test_perm17_*.py 等 6 组新测试 | 与 15/16 打包 | 静态已证；✅6 域并行 TDD+单元/API 回归（2026-07-05） | 复用 sales 域已验证的 `Role.data_scope`→`GenericFilterService.filter_by_scope` 简单口径（不依赖 PERM-16 的 RoleDataScope）；仅挂载有真实归属概念的逐行列表端点，明确跳过：纯聚合/分析报表（月度趋势/成本分析/现金流等无项目维度输出）、共享主数据（物料目录/供应商/标准成本/分摊规则/原始库存）、已有等效审批链路口径的端点（工时待审批、ECN/采购审批队列）、纯 raw SQL 无 ORM Query 对象的端点（需另行改写，未来跟进）；行为变更：线上 15 个非 ALL 域用户（8 DEPARTMENT/4 OWN/3 PROJECT）在这些域今后只能看到自己范围内数据，此前无过滤形同全公司可见 |
...
| PERM-22 | 前端路由/菜单/按钮权限：system/hr/finance 路由零守卫 | P2 | 已验证 | frontend/src/routes/modules/{system,hr,finance}Routes.jsx；permissionProtectedRoutes.test.jsx | 1-2d | 静态已证；✅前端路由守卫回归+build（2026-07-05） | system/hr/finance 管理类直链 now 通过 ModuleProtectedRoute 校验；个人绩效/奖金自助页不挂 HR 管理权限，避免误拦 |
...
| ADMIN-07 | 行政管理（用品/车辆/资产/费用）全硬编码+写端点缺失（404） | P1 | 已修待验 | admin_compat.py（重写）；models/admin_office.py；migrations/20260704_admin_office_sqlite.sql；tests/unit/test_admin_office_real.py | 2-3d | 静态已证；✅契约测试回归（2026-07-04） | 四件套建真表做实：用品申领审批扣库存、用车审批置 IN_USE、资产 CRUD+统计、费用真聚合；对齐前端 adminApi 既有调用面（camelCase 兼容）；/stats 仍委托 admin_stats（ADMIN-05 范围）；会议室预定端点前端有调用但历史即缺，另行排期 |
...
| ADMIN-20 | 日志管理/轮转不存在（仅 stdout，logs/ 空） | P2 | 已验证 | core/logging_config.py；tests/unit/test_logging_file_rotation_admin20.py | 1d | 静态已证；✅日志文件输出+轮转配置回归（2026-07-05） | `setup_logging()` now 同时输出 stdout 与 `logs/app.log`，支持 `APP_LOG_DIR/APP_LOG_FILE/APP_LOG_MAX_BYTES/APP_LOG_BACKUP_COUNT` 配置；敏感信息过滤同步作用于文件日志 |
...
| TEN-02 | TenantQuery 在 SQLA 2.0 形同虚设（只重写 __iter__，.all() 走 _iter 绕过） | P1 | 已验证 | app/core/database/tenant_scope.py（新增，替代已删除的 tenant_query.py）；app/models/base.py get_session_factory；app/core/middleware/tenant_middleware.py（新增 is_superuser 上下文变量）；app/models/project/core.py（补 tenant_id，见 TEN-03）；tests/unit/test_tenant_query_scope_ten02.py | 2-3d | 静态已证；✅单元回归（2026-07-05） | 改用 `do_orm_execute`+`with_loader_criteria` 全局过滤，覆盖 `.all()/.first()/.count()` 等原实现全部绕过的方法；踩了两个 SQLAlchemy 2.0 "lambda SQL" 缓存坑（回调闭包不能调用外部函数/不能靠闭包变量分支，否则要么直接报错要么更隐蔽地跨请求复用上一次绑定值导致真实跨租户泄露风险），最终改为在事件回调外部把分支判断做完、只传"算好的普通表达式"给 `with_loader_criteria`，不传可调用对象；发现 `Role/ApiPermission/DataScopeRule/MenuPermission` 的 tenant_id=NULL 语义是"系统级共享"而非"无归属"，与 `User`（NULL=超管）相反，已用既有回归测试 `test_role_tenant_isolation_contracts.py` 验证两种语义都正确处理 |
| TEN-03 | 96.8% 业务表无 tenant_id；projects.tenant_id 是幽灵列 | P1 | 部分已验证 | app/models/project/core.py；app/models/project/customer.py；app/models/sales/contracts.py；app/models/sales/invoices.py；app/models/business_support/sales_order.py；migrations/20260705_project_tenant_backfill_sqlite.sql；migrations/20260705_ten03_core_tables_tenant_id_sqlite.sql；app/core/database/tenant_scope.py（新增 before_flush 自动补全钩子）；tests/unit/test_ten03_core_tables_tenant_scope.py | 先 Project 3-4d/全量 2-3 周 | 静态已证；✅Project 域切片+customers/contracts/invoices/sales_orders 四表回归（2026-07-05） | 复核：`projects.tenant_id` 比审计描述更严重——DB 早有该列和索引，但 ORM 模型从未声明，不是"未消费"而是完全不可读写；已补模型声明+存量 105 条项目归户（全部 tenant_id=1），解锁了此前因这个幽灵列直接报错的 ~47 个既有租户隔离测试（tests/security/test_tenant_*.py、test_multi_model_isolation.py）里的一部分——其余仍卡在无关的既有测试债（如 create_access_token 签名早已变更但测试代码未跟进），非本次改动引入。第二批铺开审计点名"全无 tenant_id"的 customers/contracts/invoices/sales_orders 四张表（与 projects 幽灵列不同，是真的从零加列+存量 110/116/99/60 条归户）；同步给 tenant_scope.py 加 `before_flush` 钩子，新建对象若带 tenant_id 字段但未显式赋值就用当前请求租户上下文自动补全——否则新表上线后任何遗漏传参的创建入口都会产生对创建者自己也不可见的"隐形数据"（不是隔离是丢数据），这个钩子对已有的 18+1 张表同样生效，属于纯正向加固；用 `git stash` 前后对比验证过 sales/contract/invoice 相关集成测试的失败集合完全一致，未引入新回归；customers/sales_orders/contracts/invoices 之外的其余表（约 550 张）仍未铺开，全量铺开留待后续 |
| TEN-04 | 业务域查询层租户过滤≈0（项目/销售/采购/财务/报表 0 处） | P1 | 已验证 | 随 TEN-02 一并解决 | 并入 02/03 | 静态已证；✅随 TEN-02 回归 | TEN-02 是全局框架级过滤（任意模型只要声明了 tenant_id 就自动生效），不再需要在每个业务域端点手工加过滤——已不是"挂载率"问题；后续新增 tenant_id 列的表（TEN-03 铺开）会自动被现有过滤覆盖，无需改动 tenant_scope.py |
| TEN-05 | 隔离装饰器/工具全家 0 使用死代码 | P2 | 已验证 | 已删除 app/core/decorators/tenant_isolation.py；app/core/permissions/tenant_access.py；tenant_middleware.py 的 TenantAwareQuery/require_same_tenant；同步删除/摘除 tests/test_tenant_isolation_decorator.py、tests/test_tenant_access.py、tests/unit/test_tenant_middleware_utils.py（整份）与 test_core_common_coverage.py/test_tenant_middleware.py/test_core_modules.py 里对应段落 | 删 0.5d（复核：实际更大） | 静态已证；✅回归（2026-07-05） | 复核属实：审计"0.5d 快删"低估了工作量，实际配套约 30 个专门单元测试需同步清理，非纯删除；顺手把 test_tenant_middleware.py 里 Mock(spec=User) 自动虚构属性导致断言恒真的既有小坑修掉 |
...
| TEN-07 | 配额/套餐/生命周期全不执行（暂停/过期租户照常登录） | P2 | 已验证 | app/services/tenant_service.py（新增 check_tenant_login_allowed/check_user_quota）；app/api/v1/endpoints/auth.py login；app/api/v1/endpoints/users/crud_refactored.py create_user；tests/unit/test_tenant_lifecycle_ten07.py；tests/api/test_auth_tenant_lifecycle_ten07.py | 1-2d | 静态已证；✅单元+接线集成回归（2026-07-05） | 登录流程 now 读取 `Tenant.status/expired_at`，SUSPENDED 或已过期租户下的非超管用户登录返回 403（`TENANT_SUSPENDED`/`TENANT_EXPIRED`）；建用户流程 now 读取 `max_users`，达到配额返回 400 拒绝创建；超管（tenant_id=None）不受两项限制；租户不存在按拒绝处理，不静默放行 |

...
| TEN-03 | 96.8% 业务表无 tenant_id；projects.tenant_id 是幽灵列 | P1 | 部分已验证 | app/models/project/core.py；migrations/20260705_project_tenant_backfill_sqlite.sql | 先 Project 3-4d/全量 2-3 周 | 静态已证；✅Project 域切片已回归（2026-07-05） | 复核：`projects.tenant_id` 比审计描述更严重——DB 早有该列和索引，但 ORM 模型从未声明，不是"未消费"而是完全不可读写；已补模型声明+存量 105 条项目归户（全部 tenant_id=1），解锁了此前因这个幽灵列直接报错的 ~47 个既有租户隔离测试（tests/security/test_tenant_*.py、test_multi_model_isolation.py）里的一部分——其余仍卡在无关的既有测试债（如 create_access_token 签名早已变更但测试代码未跟进），非本次改动引入；customers/sales_orders/contracts/invoices 等其余表仍无 tenant_id，全量铺开留待后续 |

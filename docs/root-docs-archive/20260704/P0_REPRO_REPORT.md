...
| 9 | 现场调试假实现 | API：`POST /field/tasks/1/checkin` 后查 `field_checkins`；进度/问题/完工同步查 `field_tasks/field_issues` | 修复前返回「签到已记录」HTTP 200 但行数 3→3 无新增；修复后签到新增行，进度写回任务，问题写入 `field_issues` 并回填任务备注，完工写状态/进度/签名/完成时间 | **已复现并回归** | `test_p0_09_field_checkin_fake.py`（3 用例） |
...
| 12 | BOM→工单断链 | 静态：工单模型有无 bom 字段；`WorkOrderBom` 有无业务读写 | 修复前 WorkOrder 模型无 bom_* 字段；`WorkOrderBom` 仅在 models/exports 出现，无业务读写；修复后工单创建/更新会绑定 BOM 并固化 `mat_work_order_bom` 快照 | **已复现并回归** | `test_p0_12_bom_workorder_broken.py`（2 用例）；`test_work_order_bom_snapshot.py`（2 用例） |
| 13 | 售后无设备档案 | PRAGMA：`service_tickets`/`machines` 列；服务工单/服务记录业务绑定 | 修复前 `service_tickets` 无 machine 外键；`machines` 无 serial_no/customer_id/warranty；修复后 `machines` 有客户/SN/质保字段，服务工单/服务记录可绑定 `machine_id`，机台服务历史优先按 machine_id 查询 | **已复现并回归** | `test_p0_13_device_archive_missing.py`（4 用例）；`test_device_archive_as10.py`（2 用例） |
| 14 | 派工冲突空转 | DB：依赖表是否存在 + API conflict-detect + 安装调试派工入口 | 修复前 `engineer_task_assignments` 表不存在且 conflict-detect 恒返回 0；修复后事实表存在，重叠跨项目派工返回冲突，安装调试派工入口对撞期工程师返回 409 且不改派工单状态 | **已复现并回归** | `test_p0_14_dispatch_conflict.py`（3 用例）+ `test_dispatch_conflict_guard_as04.py` |

...
const MachineForm = ({ isOpen, onClose, onSubmit, initialData = {}, projectId }) => {
    ...
    const handleChange = (e) => {
        ...
        setFormData(prev => ({
            ...prev,
            [name]: name === 'progress_pct' || name === 'machine_no' ? Number(value) : value
        }));
    };
    ...
    return (
        <div className="modal-overlay">
            <div className="glass-panel modal-content animate-fade" style={{ maxWidth: '600px', width: '90%', padding: '0' }}>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h2 style={{ fontSize: '1.25rem', fontWeight: 600 }}>{initialData.id ? '编辑设备' : '添加设备'}</h2>
                ...
                <form onSubmit={handleSubmit} style={{ padding: '24px' }}>
                    ...
                    <div style={{ marginTop: '32px', display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                        <button type="button" onClick={onClose} className="nav-item" style={{ background: 'rgba(255,255,255,0.05)' }}>取消</button>
                        <button type="submit" className="btn-primary">保存设备</button>
                    </div>
                </form>
            </div>
        </div>

...
const ProjectForm = ({ isOpen, onClose, onSubmit, initialData = {} }) => {
    ...
    const handleChange = (e) => {
        ...
        setFormData(prev => ({
            ...prev,
            [name]: name.includes('amount') || name.includes('_id') ? (value === '' ? '' : Number(value)) : value
        }));
    };
    ...
    return (
        <div className="modal-overlay">
            <div className="glass-panel modal-content animate-fade" style={{ maxWidth: '800px', width: '90%', padding: '0' }}>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h2 style={{ fontSize: '1.25rem', fontWeight: 600 }}>{initialData.id ? '编辑项目' : '新建项目'}</h2>
                ...
                <form onSubmit={handleSubmit} style={{ padding: '24px' }}>
                    ...
                    <div style={{ marginTop: '32px', display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                        ...
                        <button type="submit" className="btn-primary">保存项目</button>
                    </div>
                </form>
            </div>

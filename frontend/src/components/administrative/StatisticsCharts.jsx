 ...
 */
export function SimpleLineChart({ data, height = 200, color = 'text-blue-400' }) {
  const maxValue = useMemo(() => {
    return Math.max(...data.map(d => d.value), 1)
  }, [data])
        ...
        {segments.map((segment, index) => (
          <div key={index} className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-slate-500">{segment.percentage.toFixed(1)}%</div>
            </div>
          </div>
        ))}
      </div>
 ...
 */
export function TrendComparisonCard({ title, current, previous, unit = '', formatValue }) {
  const trend = useMemo(() => {
            ...
            <div key={index} className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-white font-medium">{format(item.value)}</span>
 ...
 * Monthly Trend Chart
 */
export function MonthlyTrendChart({ data, valueKey = 'value', labelKey = 'month', height = 200 }) {
  const maxValue = useMemo(() => {
          ...
          <div key={index} className="flex-1 flex flex-col items-center gap-1">
            <div className="w-full flex flex-col items-center gap-1">
              ...
              <div
                ...
                style={{ height: `${(item[valueKey] / maxValue) * (height - 40)}px` }}
              />
            </div>

...
const presetRanges = [
  { key: '7d', label: '近7天', getRange: () => ({ start: subDays(new Date(), 7), end: new Date() }) },
  { key: '30d', label: '近30天', getRange: () => ({ start: subDays(new Date(), 30), end: new Date() }) },
  { key: '90d', label: '近90天', getRange: () => ({ start: subDays(new Date(), 90), end: new Date() }) },
  { key: 'thisMonth', label: '本月', getRange: () => ({ start: startOfMonth(new Date()), end: new Date() }) },
  { key: 'lastMonth', label: '上月', getRange: () => {
    const lastMonth = subMonths(new Date(), 1)
    return { start: startOfMonth(lastMonth), end: endOfMonth(lastMonth) }
  }},
  { key: 'ytd', label: '年初至今', getRange: () => ({ start: startOfYear(new Date()), end: new Date() }) },
      ...
      {isOpen && (
        <div className="absolute top-full mt-2 right-0 z-50 w-72 bg-slate-800 border border-slate-700 rounded-lg shadow-xl">
          ...
          <div className="p-2 border-b border-slate-700">
            <div className="text-xs text-slate-400 px-2 py-1 mb-1">快捷选择</div>
            <div className="grid grid-cols-2 gap-1">
              {presetRanges.filter(p => p.key !== 'custom').map((preset) => (
                <button
                  key={preset.key}

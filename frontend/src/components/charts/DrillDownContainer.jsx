...
import { ChevronRight, ArrowLeft, Home, Maximize2, Minimize2 } from 'lucide-react'
...
export default function DrillDownContainer({
  ...
  const [breadcrumb, setBreadcrumb] = useState([{ key: 'root', label: '全部' }])
  ...
  const handleDrillDown = useCallback((item) => {
    const newBreadcrumb = [...breadcrumb, { key: item.key || item.id, label: item.label || item.name }]
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {typeof children === 'function'
                ? children({ level: currentLevel, data: drillData, onDrillDown: handleDrillDown })
                : children}
            </motion.div>
          </AnimatePresence>
        </div>

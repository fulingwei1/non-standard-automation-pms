...
export function CostTrendChart({ 
  ...
  const pricePoints = useMemo(() => {
    ...
    return sortedData.map((item, index) => {
      ...
      return { x, y, value: item.total_price || 0, version: item.version_no, date: item.created_at }
    })
  ...
  const costPoints = useMemo(() => {
    ...
    return sortedData.map((item, index) => {
      ...
      return { x, y, value: item.total_cost || 0, version: item.version_no, date: item.created_at }
    })
  ...
  const marginPoints = useMemo(() => {
    ...
    return sortedData.map((item, index) => {
      const x = padding.left + (index / (sortedData.length - 1 || 1)) * (chartWidth - padding.left - padding.right)
      const y = padding.top + (1 - ((item.gross_margin || 0) - minMargin) / marginRange) * (chartHeight - padding.top - padding.bottom)
      return { x, y, value: item.gross_margin || 0, version: item.version_no, date: item.created_at }
    })
  ...
  const pricePath = useMemo(() => {
    ...
    return pricePoints.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.x} ${p.y}`).join(' ')
  }, [pricePoints])
  ...
  return (
    <div className="relative" style={{ height: `${height}px`, width: '100%', overflowX: 'auto' }}>
      <svg width={chartWidth} height={chartHeight} className="overflow-visible">
        ...
        {pricePoints.length > 1 && (
          <g>
            ...
            />
            {showPoints && pricePoints.map((point, index) => (
              <g key={`price-${index}`}>
                <circle
                  cx={point.x}
                  cy={point.y}
        ...
        {costPoints.length > 1 && (
          <g>
            <path
              ...
              className="transition-all"
            />
            {showPoints && costPoints.map((point, index) => (
              <g key={`cost-${index}`}>
                <circle
                  cx={point.x}
                  cy={point.y}
        ...
        {marginPoints.length > 1 && (
          <g>
            <path
              ...
              className="transition-all"
            />
            {showPoints && marginPoints.map((point, index) => (
              <g key={`margin-${index}`}>
                <circle
                  cx={point.x}
                  cy={point.y}

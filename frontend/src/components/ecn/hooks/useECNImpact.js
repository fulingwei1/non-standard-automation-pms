...
export function useECNImpact(ecnId, refetchECN) {
  ...
  const [responsibilityForm, setResponsibilityForm] = useState(initialResponsibilityForm)
  ...
  const handleAnalyzeBomImpact = useCallback(async () => {
    ...
    try {
      const result = await ecnApi.analyzeBomImpact(ecnId, { include_cascade: true })
  ...
  const handleSaveMaterial = useCallback(async () => {
    try {
      if (editingMaterial) {
        await ecnApi.updateAffectedMaterial(ecnId, editingMaterial.id, materialForm)
      } else {

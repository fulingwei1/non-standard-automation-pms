...
const statusConfig = {
  ...
  in_progress: { icon: PlayCircle, color: 'text-blue-400', bg: 'bg-blue-500/20' },
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 mb-1">
        ...
        <span className="text-xs text-white font-medium truncate">{task.titleCn}</span>
      </div>
...
// Day Cell Component
function DayCell({ date, tasks, isCurrentMonth, isToday, onTaskClick, selectedTaskId, viewMode }) {
  ...
  const dueTasks = dayTasks.filter(task => 
    isSameDay(parseDate(task.plannedEnd), date) && task.status !== 'completed'
  )

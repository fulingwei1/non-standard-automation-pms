...
export function BiddingTimeline({ timeline, className }) {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'completed':
        return { icon: CheckCircle, color: 'bg-emerald-500', textColor: 'text-emerald-500' }
      case 'in_progress':
        return { icon: Clock, color: 'bg-blue-500', textColor: 'text-blue-500' }
      case 'pending':
        return { icon: Flag, color: 'bg-slate-600', textColor: 'text-slate-500' }
      case 'overdue':
        return { icon: AlertTriangle, color: 'bg-red-500', textColor: 'text-red-500' }
      default:
        return { icon: Flag, color: 'bg-slate-600', textColor: 'text-slate-500' }
    }
  }
        ...
        return (
          <div key={index} className="flex gap-3">
            ...
            <div className="flex-1">
              <p className={cn('text-sm', item.status === 'completed' ? 'text-white' : 'text-slate-400')}>
                {item.event}
              </p>

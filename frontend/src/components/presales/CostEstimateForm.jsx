...
export default function CostEstimateForm({ bidding, onSave, onCancel }) {
  ...
  const calculateTotals = (newCosts) => {
    const total = Object.values(newCosts).reduce(
    ...
    Object.keys(updatedCosts).forEach((key) => {
      updatedCosts[key].percentage =
        total > 0
          ? (((updatedCosts[key].amount || 0) / total) * 100).toFixed(1)
          : 0;
    });
    ...
    const margin =
      total > 0
        ? (((total * 1.3 - total) / (total * 1.3)) * 100).toFixed(1)
        : 0;
      ...
      <div className="space-y-3">
        {costCategories.map((category) => {
          ...
          return (
            <Card
              key={category.id}
              className="bg-surface-50/50 border border-white/5"
            >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input
                          ...
                          value={cost.amount || ""}
                          onChange={(e) =>
                            updateCost(category.id, "amount", e.target.value)
                          }
                          placeholder="0.00"
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          value={cost.notes}
                          onChange={(e) =>
                            updateCost(category.id, "notes", e.target.value)
                          }
                          placeholder="说明..."
                          className="bg-surface-100 border-white/10"
                        />

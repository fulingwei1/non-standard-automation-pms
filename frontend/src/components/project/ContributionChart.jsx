...
export default function ContributionChart({ contributions = [] }) {
  const stats = useMemo(() => {
    ...
    const totalScore = contributions.reduce((sum, c) => sum + (c.contribution_score || 0), 0)
    const totalHours = contributions.reduce((sum, c) => sum + (c.actual_hours || 0), 0)
    const totalBonus = contributions.reduce((sum, c) => sum + (c.bonus_amount || 0), 0)
    const totalTasks = contributions.reduce((sum, c) => sum + (c.task_count || 0), 0)

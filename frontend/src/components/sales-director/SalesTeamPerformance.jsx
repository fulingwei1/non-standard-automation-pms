...
export default function SalesTeamPerformance({
  ...
  const renderMemberCard = (member, index) => {
    const grade = getPerformanceGrade(member.performanceScore || 0);
    ...
    return (
      ...
      >
        <Card className={cn(
          ...
          isTopPerformer && 'border-yellow-200 bg-gradient-to-br from-yellow-50/50 to-transparent'
        )}>
          <CardContent className="p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{formatCurrency(member.totalRevenue || 0)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-600">签单数</span>
                <span className="font-medium">{member.dealCount || 0}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-600">转化率</span>
                <span className="font-medium">{formatPercentage(member.conversionRate || 0)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-600">平均订单额</span>
                <span className="font-medium">{formatCurrency(member.avgDealSize || 0)}</span>
              </div>
            </div>
    ...
    return (
      <Card className="mb-6">
          ...
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={configSaving || !metricValidation.isValid}
              >
                {configSaving ? '保存中...' : '保存'}
              </Button>
            </div>
        ...
        <CardContent className="space-y-4">
          ...
          {/* 当前指标列表 */}
          <div>
            <h4 className="font-medium mb-3">当前指标 (权重总和: {metricValidation.totalWeight.toFixed(2)})</h4>
            <div className="space-y-2">
              {configDraft.map((metric, index) => (
                <div key={index} className="flex items-center gap-3 p-3 border rounded-lg">
                  <span className="font-medium flex-1">{metric.label}</span>
                  <input
                    type="number"
                    step="0.1"
                    min="0"
          ...
          {/* 添加新指标 */}
          <div>
            <h4 className="font-medium mb-3">添加指标</h4>
            <div className="flex gap-2">
              <select
                value={metricToAdd}
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-bold text-gray-900">{teamPerformance.length}</div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">优秀成员</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="font-medium text-gray-700">当前排名指标</span>
            </div>
      ...
      {sortedTeam.length === 0 && (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">没有找到团队成员</h3>

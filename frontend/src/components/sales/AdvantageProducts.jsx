...
const CATEGORY_BG = {
  ...
  AUTOMATION_LINE: 'bg-indigo-500/10 border-indigo-500/20 hover:bg-indigo-500/20',
...
// 单个类别展示组件
function CategorySection({ category, products, isExpanded, onToggle, onProductSelect }) {
  const IconComponent = CATEGORY_ICONS[category.code] || Package
  const colorClass = CATEGORY_COLORS[category.code] || 'from-slate-500 to-gray-500'
  const bgClass = CATEGORY_BG[category.code] || 'bg-slate-500/10 border-slate-500/20'
...
export default function AdvantageProducts({
  ...
  // 统计信息
  const totalProducts = groupedData.reduce((sum, g) => sum + g.products.length, 0)
    ...
    <motion.div variants={fadeIn} className="space-y-4">
      ...
      {!compact && (
        <div className="flex items-center justify-between">
          ...
          >
            {expandedCategories.size === groupedData.length ? '全部收起' : '全部展开'}
          </button>
        </div>
      )}
...
function getMockData() {
  return [
    {
      category: { id: 1, code: 'HOME_APPLIANCE', name: '白色家电', product_count: 19 },
      products: [
        { id: 1, product_code: 'KC2701', product_name: '离线双工位FCT', category_name: '白色家电' },
        { id: 2, product_code: 'KC2702', product_name: '离线显示双工位FCT', category_name: '白色家电' },
        { id: 3, product_code: 'KC2706', product_name: '在线式双轨双工位FCT', category_name: '白色家电' },
      ],
    },
    {
      category: { id: 2, code: 'AUTOMOTIVE', name: '汽车电子', product_count: 34 },
      products: [
        { id: 4, product_code: 'KC2101', product_name: '域控制器测试系统', category_name: '汽车电子' },
        { id: 5, product_code: 'KC2102', product_name: '车载导航整机测试系统', category_name: '汽车电子' },
        { id: 6, product_code: 'KC2106', product_name: '电机控制器测试系统', category_name: '汽车电子' },
      ],
    },
    {
      category: { id: 3, code: 'NEW_ENERGY', name: '新能源', product_count: 24 },
      products: [
        { id: 7, product_code: 'KC2901', product_name: 'PACK/模组EOL测试系统', category_name: '新能源' },
        { id: 8, product_code: 'KC2909', product_name: 'BMS测试系统', category_name: '新能源' },
      ],
    },
    {
      category: { id: 4, code: 'SEMICONDUCTOR', name: '半导体', product_count: 12 },
      products: [
        { id: 9, product_code: 'KC3105', product_name: '功率器件老化测试设备', category_name: '半导体' },
        { id: 10, product_code: 'KC3107', product_name: '功率器件ATE测试设备', category_name: '半导体' },
      ],
    },
  ]
}
...
// 紧凑版本 - 用于下拉选择
export function AdvantageProductSelect({ value, onChange, placeholder = '选择优势产品' }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className={cn('text-sm', selectedProduct ? 'text-white' : 'text-slate-500')}>
              ...
              products.map(product => (
                <button
                  ...
                  className={cn(
                    ...
                    value === product.id ? 'text-primary-400 bg-primary-500/10' : 'text-slate-300'
                  )}
                >

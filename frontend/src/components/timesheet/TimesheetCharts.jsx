...
// 简单的柱状图组件
export function BarChart({ data, width = 400, height = 200, color = '#3b82f6' }) {
...
// 简单的折线图组件
export function LineChart({ data, width = 400, height = 200, color = '#10b981' }) {
  if (!data || data.length === 0) {
    return (
      <div className="flex items-center justify-center h-full text-slate-400">
        暂无数据
...
// 日历热力图（简化版）
export function CalendarHeatmap({ data, title = '工时日历' }) {
  // 这里实现一个简化的日历视图
  // 完整的热力图需要更复杂的实现

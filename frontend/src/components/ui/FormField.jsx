...
export const FormField = forwardRef(
  ({ label, name, error, touched, required, className, children, ...props }, ref) => {
    const showError = touched && error

    return (
      <div className={cn('space-y-2', className)}>
        {label && (
          <Label htmlFor={name} className={cn(required && 'after:content-["*"] after:text-red-400 after:ml-1')}>
            {label}
          </Label>
        )}
        {children ? (
          ...
          <Input
            ...
            id={name}
            name={name}
            className={cn(showError && 'border-red-500 focus:border-red-500 focus:ring-red-500')}

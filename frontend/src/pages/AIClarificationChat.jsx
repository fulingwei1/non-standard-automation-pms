...
export default function AIClarificationChat() {
  ...
  const handleCreateQuestions = async () => {
    ...
    try {
      ...
      if (sourceType === 'lead') {
        ...
        response = await technicalAssessmentApi.createAIClarificationForOpportunity(
          parseInt(sourceId),
          { questions: questionsJson }
        )
      }
  ...
  const handleSubmitAnswers = async (clarificationId) => {
    const currentClarification = clarifications.find(c => c.id === clarificationId)
    ...
    <div className="min-h-screen bg-gray-900 text-white p-6">
      ...
      <div className="mt-6 space-y-6">
        ...
        {clarifications.map((clarification) => {
          ...
          const answersArray = clarification.answers ? JSON.parse(clarification.answers) : []
          const hasAnswers = answersArray.length > 0

          return (
            <Card key={clarification.id} className="bg-gray-800 border-gray-700">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="flex items-center gap-2">
                    <MessageSquare className="w-5 h-5" />
                    第 {clarification.round} 轮澄清
                  </CardTitle>
                  <Badge className={hasAnswers ? 'bg-green-500' : 'bg-yellow-500'}>
                    {hasAnswers ? '已回复' : '待回复'}
                  </Badge>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2 pl-6">
                    {questionsArray.map((question, idx) => (
                      <div key={idx} className="p-3 bg-gray-700 rounded-lg">
                        <div className="text-sm mb-2">{idx + 1}. {question}</div>
                        {hasAnswers ? (
                ...
                {!hasAnswers && (
                  ...
                  >
                    {sending ? <Loader className="w-4 h-4 mr-2 animate-spin" /> : <Send className="w-4 h-4 mr-2" />}
                    提交回答
                  </Button>
                )}
              </CardContent>
        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2">AI生成的问题（每行一个问题）</label>
                <Textarea

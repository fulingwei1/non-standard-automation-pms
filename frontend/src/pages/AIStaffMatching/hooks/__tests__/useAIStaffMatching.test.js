...
import { useAIStaffMatching } from '../useAIStaffMatching';
import { projectApi, staffApi } from '../../../../services/api';
...
vi.mock('../../../../services/api', () => {
    return {
        ...
        staffApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useAIStaffMatching Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [projectApi, staffApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useAIStaffMatching());

...
export default function AcceptanceExecution() {
  ...
  const handleUpdateItem = async () => {
    ...
    try {
      ...
      alert(
        "更新检查项失败: " + (error.response?.data?.detail || error.message),
      );
    }
  };
  ...
  return (
    <div className="space-y-6 p-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-2xl font-bold">
                  {totalChecked > 0
                    ? ((passedCount / totalChecked) * 100).toFixed(1)
                    : 0}
                  %
                </div>
              </div>
              <Progress
                value={
                  totalChecked > 0 ? (passedCount / totalChecked) * 100 : 0
                }
                className="w-16 h-16"
              />
            </div>
                      ...
                      {(categoryItems || []).map((item) => (
                        ...
                        >
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <Badge
                              className={
                                resultStatusConfigs[item.result_status]
                                  ?.color || "bg-slate-500"
                              }
                            >
                              {resultStatusConfigs[item.result_status]?.label ||
                                item.result_status}
                            </Badge>
                          </div>
      ...
      <Dialog open={showItemDialog} onOpenChange={setShowItemDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{selectedItem?.item_name} - 检查结果</DialogTitle>
          </DialogHeader>
          <DialogBody>
            {selectedItem && (
              <div className="space-y-4">
                ...
                {selectedItem.standard_value && (
                  <div>
                    ...
                    <div>
                      {selectedItem.standard_value} {selectedItem.unit || ""}
                    </div>
                  </div>
                )}
                <div>

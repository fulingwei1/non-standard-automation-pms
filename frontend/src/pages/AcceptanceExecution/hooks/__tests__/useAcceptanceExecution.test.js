...
import { useAcceptanceExecution } from '../useAcceptanceExecution';
import { acceptanceApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        acceptanceApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), listExecutions: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), startExecution: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), submitResult: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useAcceptanceExecution Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [acceptanceApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useAcceptanceExecution());

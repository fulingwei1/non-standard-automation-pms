              ...
              <TableBody>
                {filteredOrders.map((order) => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell>
                      <Badge className={typeConfigs[order.acceptance_type]?.color || 'bg-slate-500'}>
                        {typeConfigs[order.acceptance_type]?.label || order.acceptance_type}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        {order.pass_rate && (
                          <Progress value={order.pass_rate} className="h-1.5 w-16" />
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {order.status === 'IN_PROGRESS' && (
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => navigate(`/acceptance-orders/${order.id}/execute`)}
                          >
          ...
          <DialogBody>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium mb-2 block">验收类型</label>
                  <Select
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">验收地点</label>
                <Input
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Badge className={typeConfigs[selectedOrder.acceptance_type]?.color}>
                      {typeConfigs[selectedOrder.acceptance_type]?.label}
                    </Badge>
                  </div>
                  <div>
                    <div className="text-sm text-slate-500 mb-1">计划日期</div>
                    <div>{selectedOrder.planned_date ? formatDate(selectedOrder.planned_date) : '-'}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-500 mb-1">通过率</div>
                    <div className="flex items-center gap-2">
                      <span className="text-lg font-bold">{selectedOrder.pass_rate || 0}%</span>
                      {selectedOrder.pass_rate && (
                        <Progress value={selectedOrder.pass_rate} className="h-2 flex-1" />
                      )}
                    </div>
                  </div>
                  <div>
                    ...
                    {selectedOrder.overall_result && (
                      <Badge className={resultConfigs[selectedOrder.overall_result]?.color}>
                ...
                <div className="grid grid-cols-4 gap-4">
                  <div>
                    ...
                    <div className="text-lg font-bold">{selectedOrder.total_items || 0}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-500 mb-1">通过</div>
                    <div className="text-lg font-bold text-emerald-600">{selectedOrder.passed_items || 0}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-500 mb-1">不通过</div>
                    <div className="text-lg font-bold text-red-600">{selectedOrder.failed_items || 0}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-500 mb-1">不适用</div>
                    <div className="text-lg font-bold text-slate-400">{selectedOrder.na_items || 0}</div>
                  </div>
                </div>
              </div>
            )}

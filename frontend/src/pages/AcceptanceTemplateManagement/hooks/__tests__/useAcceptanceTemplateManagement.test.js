...
import { useAcceptanceTemplateManagement } from '../useAcceptanceTemplateManagement';
import { acceptanceTemplateApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        acceptanceTemplateApi: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), query: vi.fn(), aiMatch: vi.fn(), assign: vi.fn() }
    };
});

describe('useAcceptanceTemplateManagement Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [acceptanceTemplateApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useAcceptanceTemplateManagement());

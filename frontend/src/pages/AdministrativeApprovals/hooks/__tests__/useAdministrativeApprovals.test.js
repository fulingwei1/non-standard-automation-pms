...
import { useAdministrativeApprovals } from '../useAdministrativeApprovals';
import { adminApprovalApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        adminApprovalApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), approve: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), reject: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useAdministrativeApprovals Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [adminApprovalApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useAdministrativeApprovals());

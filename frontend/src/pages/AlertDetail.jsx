    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      <PageHeader
        title={
          <div className="flex items-center gap-3">
            ...
            <span>预警详情</span>
            <span className="font-mono text-sm text-slate-400">{alert.alert_no}</span>
          </div>
        }
      />
      ...
      <div className="container mx-auto px-4 py-6 space-y-6">
        ...
        <motion.div variants={fadeIn} initial="hidden" animate="visible">
          <Card className={cn('border', levelConfig.bgColor, levelConfig.borderColor)}>
            <CardContent className="p-6">
              <div className="flex items-start justify-between">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <levelConfig.icon className={cn('w-6 h-6', levelConfig.textColor)} />
                    <Badge
                      variant="outline"
                      className={cn(levelConfig.textColor, levelConfig.borderColor)}
                    >
                      {levelConfig.label}
                    </Badge>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-white">{alert.rule_code || alert.rule_name}</p>
                </div>
                <div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="prose prose-invert max-w-none">
                  <p className="text-slate-300 whitespace-pre-line">{alert.suggestion}</p>
                </div>
              </CardContent>
            </Card>
          </motion.div>
                    ...
                    <div key={index} className="flex gap-4 pb-4 border-b border-slate-700 last:border-0">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-white font-medium">{action.created_by_name || '系统'}</span>
                          <span className="text-xs text-slate-500">{action.created_at}</span>
                        </div>
                        <p className="text-slate-400 text-sm">{action.action_content}</p>
                      </div>
                    </div>
                  ))}
                </div>

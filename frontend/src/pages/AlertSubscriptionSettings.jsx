                    ...
                    <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-lg font-semibold text-white">
                            {subscription.alert_type
                              ? alertTypeOptions.find(o => o.value === subscription.alert_type)?.label || subscription.alert_type
                              : '全部类型'}
                          </h3>
                          <Badge
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-slate-400 mb-3">
                          <div>
                            ...
                            <span className="text-white">
                              {subscription.project_name || subscription.project_id || '全部项目'}
                            </span>
                          </div>
                          <div>
                            <span className="text-slate-500">通知渠道:</span>{' '}
                            <span className="text-white">
                              {(subscription.notify_channels || ['SYSTEM']).map(ch => 
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        <Button
                          ...
                          className="gap-2"
                          title={subscription.is_active ? '禁用订阅' : '启用订阅'}
                        >
                          {subscription.is_active ? (
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogBody className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-500">选择"全部类型"将接收所有类型的预警</p>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="project_id">项目范围</Label>
                  <Select
                    value={formData.project_id?.toString() || ''}
                    onValueChange={(val) => setFormData(prev => ({ ...prev, project_id: val ? parseInt(val) : null }))}
                  >
                    <SelectTrigger className="bg-surface-2">
                  ...
                  <p className="text-xs text-slate-500">选择"全部项目"将接收所有项目的预警</p>
                </div>
              </div>
            </div>
            ...
            {/* 接收级别和通知渠道 */}
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-white">接收级别和通知渠道</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="min_level">最低接收级别</Label>
                  <Select
                    value={formData.min_level}
                    onValueChange={(val) => setFormData(prev => ({ ...prev, min_level: val }))}
                  >
                    <SelectTrigger className="bg-surface-2">
                      <SelectValue />
                    </SelectTrigger>
                  ...
                  <p className="text-xs text-slate-500">只接收此级别及以上的预警</p>
                </div>
                <div className="space-y-2">
                  <Label>通知渠道</Label>
            ...
            {/* 免打扰时段 */}
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-white">免打扰时段（可选）</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, quiet_start: e.target.value }))}
                    placeholder="例如: 22:00"
                    className="bg-surface-2"
                  />
                  <p className="text-xs text-slate-500">格式: HH:mm (24小时制)</p>
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, quiet_end: e.target.value }))}

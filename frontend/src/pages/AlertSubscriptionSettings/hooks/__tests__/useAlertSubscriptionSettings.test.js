...
import { useAlertSubscriptionSettings } from '../useAlertSubscriptionSettings';
import { alertSubscriptionApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        alertSubscriptionApi: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), query: vi.fn(), aiMatch: vi.fn(), assign: vi.fn() }
    };
});

describe('useAlertSubscriptionSettings Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [alertSubscriptionApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useAlertSubscriptionSettings());

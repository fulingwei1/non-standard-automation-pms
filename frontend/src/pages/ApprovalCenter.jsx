...
function ApprovalCard({ approval, onView, onApprove, onReject }) {
  ...
  const currentStep = approval.approvalFlow.findIndex(
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <Badge
                variant="outline"
                className={cn("text-[10px]", type.color)}
              >
                {type.label}
              </Badge>
              {approval.priority !== "normal" && (
                <Badge
                  variant="outline"
                  className={cn("text-[10px] border", priority.color)}
                >
                  {priority.label}
                </Badge>
              )}
        ...
        {approval.impact && (
          <div className="flex gap-3 mb-3 text-xs">
            <span className="px-2 py-1 rounded bg-surface-2 text-slate-400">
              成本：
              <span className="text-amber-400">{approval.impact.cost}</span>
            </span>
            <span className="px-2 py-1 rounded bg-surface-2 text-slate-400">
              工期：
              <span className="text-amber-400">{approval.impact.schedule}</span>
            </span>
          </div>
        )}
  ...
  const loadApprovalDetails = async () => {
    ...
    try {
      ...
      if (approvalType === "quote_workflow") {
        ...
        const historyRes = await contractApi.getApprovalHistory(
          originalData.id,
        );
        setApprovalStatus(statusRes.data || statusRes);
        setApprovalHistory(historyRes.data || historyRes || []);
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="text-white">
                {approval.applicant} ({approval.applicantDept})
              </p>
            </div>
            <div>
            ...
            {approval.amount && (
              <div>
                ...
                <p className="text-white">
                  ¥{(approval.amount / 10000).toFixed(0)}万
                </p>
              </div>
            )}
          </div>
          ...
          {approval.approvalFlow && approval.approvalFlow.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {approval.approvalFlow.map((step, index) => {
                  const isCurrent =
                    step.step_order === approval.currentStep?.step_order;
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {approvalHistory.map((history, index) => (
                  <div
                    ...
                    className="p-2 rounded-lg bg-surface-50 text-sm"
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-white">
                        {history.action || history.status}
                      </span>
                      <span className="text-xs text-slate-400">
                        {history.created_at
...
export default function ApprovalCenter() {
  ...
  const loadApprovals = useCallback(async () => {
    try {
      ...
      try {
        const poRes = await purchaseApi.orders.list({
          status: "SUBMITTED",
          page_size: 100,
        });
      ...
      try {
        const quoteRes = await quoteApi.list({
          status: "SUBMITTED",
          page_size: 100,
        });
        ...
        for (const quote of quotes) {
          ...
          try {
            ...
            console.warn(
              `Failed to get approval status for quote ${quote.id}:`,
              err,
            );
          }
              ...
              status === "PENDING"
                ...
                : status === "APPROVED"
                  ...
                  : status === "REJECTED"
                    ...
                    : "waiting",
            priority:
              parseFloat(quote.total_price || 0) > 1000000
                ? "urgent"
                : "normal",
      ...
      try {
        ...
        for (const contract of contracts) {
          ...
          try {
            ...
            console.warn(
              `Failed to get approval status for contract ${contract.id}:`,
              err,
            );
          }
          ...
          allApprovals.push({
            ...
            applicant:
              contract.owner?.real_name || contract.owner_name || "未知",
              ...
              status === "PENDING"
                ...
                : status === "APPROVED"
                  ...
                  : status === "REJECTED"
                    ...
                    : "waiting",
            priority:
              parseFloat(contract.contract_amount || 0) > 5000000
                ? "urgent"
                : "normal",
      ...
      try {
        ...
        for (const invoice of invoices) {
          ...
          try {
            ...
            console.warn(
              `Failed to get approval status for invoice ${invoice.id}:`,
              err,
            );
          }
              ...
              status === "PENDING"
                ...
                : status === "APPROVED"
                  ...
                  : status === "REJECTED"
                    ...
                    : "waiting",
            priority:
              parseFloat(invoice.amount || invoice.total_amount || 0) > 500000
                ? "urgent"
                : "normal",
    ...
    try {
      ...
      switch (approvalType) {
        ...
        case "quote":
          ...
          break;
        case "quote_workflow":
          await quoteApi.approvalAction(originalData.id, {
            action: "APPROVE",
            comment: approvalComment,
          });
          break;
        case "contract_workflow":
          await contractApi.approvalAction(originalData.id, {
            action: "APPROVE",
            comment: approvalComment,
          });
          break;
        case "invoice_workflow":
          await invoiceApi.approvalAction(originalData.id, {
            action: "APPROVE",
            comment: approvalComment,
          });
          break;
      ...
      const errorMessage =
        err.response?.data?.detail || err.message || "审批失败，请稍后重试";
    ...
    try {
      ...
      switch (approvalType) {
        case "ecn":
          ...
          break;
        case "purchase_request":
          await purchaseApi.requests.approve(originalData.id, {
            ...rejectData,
            approved: false,
          });
          break;
        case "purchase_order":
          await purchaseApi.orders.approve(originalData.id, {
            ...rejectData,
            approved: false,
          });
          break;
        case "quote":
          ...
          await quoteApi.approve(originalData.id, {
            ...rejectData,
            approved: false,
          });
          break;
      ...
      const errorMessage =
        err.response?.data?.detail || err.message || "驳回失败，请稍后重试";
      toast.error(errorMessage);
    }
  };

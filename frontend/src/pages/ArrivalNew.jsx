...
import { shortageApi, materialApi, purchaseApi, supplierApi } from '../services/api'
...
export default function ArrivalNew() {
  ...
  useEffect(() => {
    ...
    if (location.state?.material_id) {
      const material = materials.find(m => m.id === location.state.material_id)
      if (material) {
        setFormData(prev => ({
          ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submitData = {
        ...
        purchase_order_item_id: formData.purchase_order_item_id ? parseInt(formData.purchase_order_item_id) : null,
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                />
                {errors.expected_qty && (
                  <div className="text-sm text-red-400 mt-1">{errors.expected_qty}</div>
                )}
              </div>
              ...
              <div>
                ...
                {errors.expected_date && (
                  <div className="text-sm text-red-400 mt-1">{errors.expected_date}</div>
                )}
              </div>
            </div>

            ...
            <Select value={filterSupplier} onValueChange={setFilterSupplier}>
              ...
              <SelectContent>
                ...
                {suppliers.map((supplier) => {
                  const supplierId = supplier.id?.toString() || supplier.supplier_id?.toString()
                  if (!supplierId || supplierId === '') return null
                  return (
                    <SelectItem key={supplier.id || supplier.supplier_id} value={supplierId}>
              ...
              <TableBody>
                {filteredArrivals.map((arrival) => (
                  ...
                  >
                    ...
                    <TableCell className="text-slate-300">{arrival.supplier_name || '-'}</TableCell>
                    <TableCell className="text-slate-300">{arrival.expected_qty || 0}</TableCell>
            ...
            {selectedArrival && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <div className="text-sm text-slate-400 mb-1">到货单号</div>
                    <div className="font-mono text-slate-200">{selectedArrival.arrival_no}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-400 mb-1">状态</div>
                    <Badge className={statusConfigs[selectedArrival.status]?.color}>
                      {statusConfigs[selectedArrival.status]?.label}
                    </Badge>
                  </div>
                  <div>
                    <div className="text-sm text-slate-400 mb-1">物料编码</div>
                    <div className="font-mono text-slate-200">{selectedArrival.material_code}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-400 mb-1">物料名称</div>
                    <div className="text-slate-200">{selectedArrival.material_name}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-400 mb-1">供应商</div>
                    <div className="text-slate-200">{selectedArrival.supplier_name || '-'}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-400 mb-1">预期数量</div>
                    <div className="font-medium text-slate-200">{selectedArrival.expected_qty || 0}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-400 mb-1">预期日期</div>
                    <div className="text-slate-200">{selectedArrival.expected_date ? formatDate(selectedArrival.expected_date) : '-'}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-400 mb-1">实际日期</div>
                    <div className={selectedArrival.actual_date ? "text-emerald-400" : "text-slate-400"}>
                      {selectedArrival.actual_date ? formatDate(selectedArrival.actual_date) : '未到货'}
                    </div>
                  </div>
                  {selectedArrival.is_delayed && (
                    <div>
                      <div className="text-sm text-slate-400 mb-1">延迟天数</div>
                      <Badge variant="destructive">{selectedArrival.delay_days || 0} 天</Badge>
                    </div>
                  )}
                  {selectedArrival.received_qty !== undefined && (
                    <div>
                      <div className="text-sm text-slate-500 mb-1">已收货数量</div>
                      <div className="font-medium text-emerald-600">{selectedArrival.received_qty || 0}</div>
                    </div>
                  )}
                </div>

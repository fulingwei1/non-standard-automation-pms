...
const alertLevelConfig = {
  L1: { label: '停工预警', color: 'bg-red-600', textColor: 'text-red-600', bgLight: 'bg-red-50 border-red-500' },
  L2: { label: '紧急预警', color: 'bg-orange-500', textColor: 'text-orange-600', bgLight: 'bg-orange-50 border-orange-500' },
  L3: { label: '提前预警', color: 'bg-yellow-500', textColor: 'text-yellow-600', bgLight: 'bg-yellow-50 border-yellow-500' },
  L4: { label: '常规预警', color: 'bg-blue-500', textColor: 'text-blue-600', bgLight: 'bg-blue-50 border-blue-500' },
}
    ...
    <div className="space-y-6 p-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className={cn("text-2xl font-bold", getKitRateColor(stats.avg_kit_rate || 0))}>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className={cn("text-2xl font-bold", getKitRateColor(stats.avg_blocking_rate || 0))}>
              ...
              return (
                <div key={stage.stage_code} className="relative">
                  ...
                  <div className={cn(
                    ...
                    isBlocked ? "border-red-300 bg-red-50" : "border-emerald-300 bg-emerald-50"
                  )}>
                    <div className={cn(
                      "w-12 h-12 rounded-full flex items-center justify-center mb-2",
                      isBlocked ? "bg-red-100" : "bg-emerald-100"
                    )}>
                      <Icon className={cn("w-6 h-6", isBlocked ? "text-red-600" : "text-emerald-600")} />
                    </div>
                    <div className="text-sm font-medium text-center mb-1">{stage.stage_name}</div>
                    <div className={cn("text-lg font-bold", getKitRateColor(stage.avg_kit_rate))}>
                      {stage.avg_kit_rate}%
                    </div>
                    <div className="flex gap-2 mt-2 text-xs">
                      <span className="text-emerald-600">{stage.can_start_count} 可开</span>
                      <span className="text-red-600">{stage.blocked_count} 阻塞</span>
                    </div>
                  </div>
                </div>
              )
      ...
      <div className="grid grid-cols-4 gap-4">
        {['L1', 'L2', 'L3', 'L4'].map((level) => {
          ...
          return (
            <Card key={level} className={cn(count > 0 && "border-l-4", count > 0 && config.bgLight.split(' ')[1])}>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm text-slate-500 mb-1">{config.label}</div>
                    <div className={cn("text-2xl font-bold", count > 0 ? config.textColor : "text-slate-400")}>
                      {count}
                    </div>
                  </div>
                  <AlertTriangle className={cn("w-8 h-8", count > 0 ? config.textColor : "text-slate-300")} />
                </div>
              </CardContent>
            </Card>
          )
        ...
        <Card>
          ...
          <CardContent>
            {recentAnalyses.length > 0 ? (
                ...
                {recentAnalyses.map((analysis) => (
                  <div
                    ...
                    className={cn(
                      ...
                      analysis.can_start ? "border-emerald-200 bg-emerald-50/50" : "border-red-200 bg-red-50/50"
                    )}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-medium">{analysis.project_name || analysis.project_no}</span>
                          {analysis.machine_no && (
                            <Badge variant="outline">{analysis.machine_no}</Badge>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-1">
                            <span className="text-sm text-slate-500">整体:</span>
                            <span className={cn("font-medium", getKitRateColor(analysis.overall_kit_rate))}>
                              {analysis.overall_kit_rate}%
                            </span>
                          </div>
                          <div className="flex items-center gap-1">
                            <span className="text-sm text-slate-500">阻塞:</span>
                            <span className={cn("font-medium", getKitRateColor(analysis.blocking_kit_rate))}>
              ...
              <div className="text-center py-8 text-slate-400">暂无分析记录</div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {pendingSuggestions.length > 0 ? (
                ...
                {pendingSuggestions.map((suggestion) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <div className="font-medium">{suggestion.project_name || suggestion.project_no}</div>
                        {suggestion.machine_no && (
                          <span className="text-sm text-slate-500">{suggestion.machine_no}</span>
                        )}
                      </div>
                      <Badge variant="outline" className="bg-blue-100">
                        {suggestion.suggestion_type === 'CAN_START' ? '可立即开工' :
                         suggestion.suggestion_type === 'WAIT_MATERIAL' ? '等待物料' :
                         suggestion.suggestion_type === 'PARTIAL_START' ? '部分开工' : suggestion.suggestion_type}
                      </Badge>
                    </div>
              ...
              <div className="text-center py-8 text-slate-400">暂无待处理建议</div>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              缺料预警明细
              <Badge variant="outline" className="ml-2">{alerts.total} 条</Badge>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              ...
              <TableBody>
                {alerts.items.map((alert) => {
                  const config = alertLevelConfig[alert.alert_level] || alertLevelConfig.L4
                  return (
                    <TableRow key={alert.shortage_id}>
                      ...
                      <TableCell>
                        ...
                        {alert.machine_no && (
                          <div className="text-xs text-slate-500">{alert.machine_no}</div>
                        )}
                      </TableCell>
                      <TableCell>
                        <div>{alert.material_name}</div>
                        <div className="text-xs text-slate-500">{alert.material_code}</div>
                      </TableCell>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="p-3 bg-slate-50 rounded-lg">
                  ...
                  <div className={cn("text-xl font-bold", getKitRateColor(analysisDetail.overall_kit_rate))}>
                    {analysisDetail.overall_kit_rate}%
                  </div>
                </div>
                <div className="p-3 bg-slate-50 rounded-lg">
                  <div className="text-sm text-slate-500">阻塞齐套率</div>
                  <div className={cn("text-xl font-bold", getKitRateColor(analysisDetail.blocking_kit_rate))}>
                    {analysisDetail.blocking_kit_rate}%
                  </div>
                </div>
                <div className="p-3 bg-slate-50 rounded-lg">
                  <div className="text-sm text-slate-500">开工状态</div>
                  <div className={cn("text-xl font-bold", analysisDetail.can_start ? "text-emerald-600" : "text-red-600")}>
                ...
                <div className="space-y-3">
                  {analysisDetail.stage_kit_rates?.map((stage) => (
                    <div key={stage.stage_code} className="flex items-center gap-4">
                      <div className="w-24 text-sm font-medium">{stage.stage_name}</div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <Progress value={stage.kit_rate} className="h-2 flex-1" />
                          <span className={cn("text-sm font-medium w-12", getKitRateColor(stage.kit_rate))}>
                            {stage.kit_rate}%
                          </span>
                        </div>
                        <div className="text-xs text-slate-500">
                          ...
                          {stage.can_start ? (
                            <span className="text-emerald-600 ml-1">可开始</span>
                          ) : (
              ...
              {analysisDetail.shortage_details?.length > 0 && (
                <div>
                  <h4 className="font-medium mb-3">缺料明细 ({analysisDetail.shortage_details.length} 项)</h4>
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {analysisDetail.shortage_details.map((detail) => (
                        <TableRow key={detail.id}>
                          <TableCell className="font-mono text-sm">{detail.material_code}</TableCell>
                          <TableCell>{detail.material_name}</TableCell>
                          <TableCell>{detail.stage_name || detail.assembly_stage}</TableCell>
                          <TableCell>{detail.required_qty}</TableCell>
                          <TableCell>{detail.available_qty}</TableCell>
                          <TableCell className="text-red-600 font-medium">{detail.shortage_qty}</TableCell>
                          <TableCell>
                            {detail.is_blocking ? (
                          ...
                          <TableCell>
                            <Badge className={alertLevelConfig[detail.alert_level]?.color || 'bg-slate-500'}>
                              {detail.alert_level}
                            </Badge>
                          </TableCell>
                        </TableRow>
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleRejectSuggestion} className="bg-red-500 hover:bg-red-600">
              确认拒绝
            </Button>
          </DialogFooter>
        </DialogContent>

  ...
  const loadBiddingDetail = useCallback(async () => {
    ...
    try {
      ...
      const transformedBidding = {
        ...
        bidAmount: tenderData.our_bid_amount ? tenderData.our_bid_amount * 10000 : tenderData.budget_amount ? tenderData.budget_amount * 10000 : 0,
        estimatedCost: tenderData.budget_amount ? tenderData.budget_amount * 10000 * 0.74 : 0, // Assume 74% cost
        estimatedMargin: tenderData.our_bid_amount && tenderData.budget_amount
          ? ((tenderData.our_bid_amount - tenderData.budget_amount * 0.74) / tenderData.our_bid_amount * 100).toFixed(0)
          : 26,
        bidDeadline: tenderData.deadline || '',
        daysLeft: daysLeft > 0 ? daysLeft : 0,
        status: mapTenderResult(tenderData.result),
        documentStatus: tenderData.result === 'SUBMITTED' ? 'submitted' : 'draft',
        progress: tenderData.result === 'SUBMITTED' ? 100 : tenderData.result === 'PREPARING' ? 60 : 20,
        projectType: '线体',
        ...
        evaluation: [
          {
            id: 1,
            stage: '资格审查',
            status: tenderData.result === 'WON' || tenderData.result === 'LOST' ? 'completed' : 'in_progress',
          ...
          },
          {
            ...
            notes: tenderData.technical_score ? `技术得分: ${tenderData.technical_score}` : '技术评标进行中'
          },
          {
            id: 3,
            stage: '商务评标',
            ...
            dueDate: tenderData.bid_opening_date || '',
            notes: tenderData.commercial_score ? `商务得分: ${tenderData.commercial_score}` : '待技术评标完成'
          },
          {
            id: 4,
            stage: '综合评定',
            ...
            dueDate: tenderData.bid_opening_date || '',
            notes: tenderData.total_score ? `总分: ${tenderData.total_score}` : '待商务评标完成'
          }
        ],
        competitors: [],
        risks: [],

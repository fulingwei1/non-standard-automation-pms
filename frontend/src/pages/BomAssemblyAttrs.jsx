...
export default function BomAssemblyAttrs() {
  ...
  const getStageOption = (code) => stageOptions.find(s => s.value === code) || stageOptions[1]
        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <Select value={selectedProject} onValueChange={(v) => { setSelectedProject(v); setSelectedBom(''); }}>
                <SelectTrigger>
            ...
            <div>
              ...
              <Select value={selectedBom} onValueChange={setSelectedBom} disabled={!selectedProject}>
                <SelectTrigger>
          ...
          <CardContent>
            {loading ? (
                  ...
                  {filteredAttrs.map((attr) => {
                    ...
                    return (
                      <TableRow key={attr.id || attr.bom_item_id}>
                        <TableCell className="font-mono text-sm">{attr.material_code}</TableCell>
                        <TableCell>{attr.material_name}</TableCell>
                        <TableCell>{attr.required_qty}</TableCell>
                        <TableCell>
                          <Select
                            value={edited.assembly_stage || 'MECH'}
                            onValueChange={(v) => handleAttrChange(attr.bom_item_id, 'assembly_stage', v)}
                          >
                            <SelectTrigger className="h-8">
                              <SelectValue>
                                <div className="flex items-center gap-1">
                                  <div className={cn("w-3 h-3 rounded-full", stageOpt.color)} />
                                  <span className="text-sm">{stageOpt.label}</span>
                                </div>
                              </SelectValue>
                            </SelectTrigger>
                            <SelectContent>
                        ...
                        <TableCell>
                          <Select
                            ...
                            onValueChange={(v) => handleAttrChange(attr.bom_item_id, 'importance_level', v)}
                          >
                            <SelectTrigger className="h-8">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              {importanceOptions.map((imp) => (
                                <SelectItem key={imp.value} value={imp.value}>
                                  <Badge className={imp.color}>{imp.label}</Badge>
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        ...
                        <TableCell>
                          <Switch
                            ...
                            onCheckedChange={(v) => handleAttrChange(attr.bom_item_id, 'is_blocking', v)}
                          />
                        </TableCell>
                        <TableCell>
                          <Switch
                            checked={edited.can_postpone ?? false}
                            onCheckedChange={(v) => handleAttrChange(attr.bom_item_id, 'can_postpone', v)}
                          />
                        </TableCell>
                        <TableCell>
                          <Switch
                            checked={edited.has_substitute ?? false}
                            onCheckedChange={(v) => handleAttrChange(attr.bom_item_id, 'has_substitute', v)}
                          />
                        </TableCell>
                        <TableCell>
                          <Input
              ...
              <div className="text-center py-8 text-slate-400">
                {assemblyAttrs.length === 0 ? '暂无装配属性配置，请使用"自动分配"或"套用模板"初始化' : '没有匹配的记录'}
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <Label>覆盖已有配置</Label>
                <p className="text-sm text-slate-500">是否覆盖已经配置过的物料</p>
              </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-amber-700">
                  ...
                  <p>自动分配会根据物料分类映射配置来设置装配阶段。请确保已配置好物料分类映射。</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <Dialog open={templateDialogOpen} onOpenChange={setTemplateDialogOpen}>
        <DialogContent>
                  ...
                  {templates.map((tpl) => (
                    <SelectItem key={tpl.id} value={tpl.id.toString()}>
                      <div>
                        ...
                        {tpl.equipment_type && (
                          <span className="text-slate-500 ml-2">({tpl.equipment_type})</span>
                        )}
                        {tpl.is_default && (
          ...
          <DialogFooter>
            ...
            <Button onClick={handleApplyTemplate} disabled={loading || !selectedTemplate}>
              <FileDown className="w-4 h-4 mr-2" />
              应用模板
            </Button>
          </DialogFooter>

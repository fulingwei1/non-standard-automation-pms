...
import { useBusinessSupportWorkstation } from '../useBusinessSupportWorkstation';
import { supportApi, ticketApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        supportApi: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), query: vi.fn(), aiMatch: vi.fn(), assign: vi.fn() },
        ticketApi: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), query: vi.fn(), aiMatch: vi.fn(), assign: vi.fn() }
    };
});

describe('useBusinessSupportWorkstation Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [supportApi, ticketApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useBusinessSupportWorkstation());

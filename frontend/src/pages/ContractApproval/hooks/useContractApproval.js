...
export function useContractApproval() {
    ...
    const loadApprovals = useCallback(async () => {
        try {
            setLoading(true);
            const response = await contractApprovalApi.list({ tab, page_size: 50 });
    ...
    const approve = useCallback(async (id, comment) => {
        try { await contractApprovalApi.approve(id, { comment }); await loadApprovals(); return { success: true }; }
        catch (err) { return { success: false, error: err.response?.data?.detail || err.message }; }
    }, [loadApprovals]);

    const reject = useCallback(async (id, reason) => {
        try { await contractApprovalApi.reject(id, { reason }); await loadApprovals(); return { success: true }; }
        catch (err) { return { success: false, error: err.response?.data?.detail || err.message }; }
    }, [loadApprovals]);

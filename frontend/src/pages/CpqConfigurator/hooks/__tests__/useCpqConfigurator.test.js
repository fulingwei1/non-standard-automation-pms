...
import { useCpqConfigurator } from '../useCpqConfigurator';
import { cpqApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        cpqApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), calculatePrice: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), getRules: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useCpqConfigurator Hook', () => {
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useCpqConfigurator());

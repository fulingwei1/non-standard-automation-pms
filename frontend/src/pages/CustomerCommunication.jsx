...
import {
  Plus, Search, Filter, Eye, Edit, Phone, Mail, MessageSquare,
  Calendar, User, Clock, FileText, CheckCircle2, XCircle, RefreshCw,
  MapPin, Video, TrendingUp, Download, Star, AlertCircle,
  ChevronRight, Send, Tag,
} from 'lucide-react'
...
const mockCommunications = [
  ...
  {
    id: 2,
    communication_no: 'COM-20260105-002',
    communication_type: '邮件',
    customer_name: '惠州XX电池',
    customer_contact: '张工',
    customer_email: 'zhang@example.com',
    project_code: 'PJ250103003',
    project_name: 'ICT在线测试设备',
    communication_date: '2026-01-05',
    communication_time: '10:00',
    topic: '操作培训',
    subject: '操作培训需求',
    content: '客户需要对新员工进行设备操作培训，已发送培训资料，并安排下周进行现场培训',
    follow_up_required: true,
    follow_up_task: '安排现场操作培训',
    follow_up_due_date: '2026-01-12',
    follow_up_status: '已安排',
    tags: ['培训', '服务'],
    importance: '中',
    created_by: '当前用户',
  ...
  {
    id: 3,
    communication_no: 'COM-20260104-003',
    communication_type: '现场',
    customer_name: '深圳XX科技',
    customer_contact: '王工',
    customer_phone: '137****3456',
    project_code: 'PJ250101001',
    project_name: 'BMS老化测试设备',
    communication_date: '2026-01-04',
    communication_time: '15:00',
    duration: 120,
    location: '广东省深圳市XX科技园',
    topic: '定期回访',
    subject: '质保期设备回访',
    content: '对质保期内的设备进行定期回访，检查设备运行状态，客户反馈设备运行正常，无异常问题',
  ...
  {
    id: 4,
    communication_no: 'COM-20260103-004',
    communication_type: '微信',
    customer_name: '东莞XX电子',
    customer_contact: '李工',
    topic: '技术支持',
    subject: '设备参数咨询',
    content: '客户通过微信咨询设备参数设置问题，已提供详细的技术支持',
    follow_up_required: false,
    tags: ['技术支持', '咨询'],
    importance: '低',
    created_by: '当前用户',
    created_at: '2026-01-03 16:20:00',
  },
]

const communicationTypeConfig = {
  '电话': { label: '电话', icon: Phone, color: 'text-blue-400', bg: 'bg-blue-500/20' },
  '邮件': { label: '邮件', icon: Mail, color: 'text-purple-400', bg: 'bg-purple-500/20' },
  '现场': { label: '现场', icon: MapPin, color: 'text-green-400', bg: 'bg-green-500/20' },
  '微信': { label: '微信', icon: MessageSquare, color: 'text-emerald-400', bg: 'bg-emerald-500/20' },
  '会议': { label: '会议', icon: Video, color: 'text-orange-400', bg: 'bg-orange-500/20' },
  '其他': { label: '其他', icon: MessageSquare, color: 'text-slate-400', bg: 'bg-slate-500/20' },
}

const importanceConfig = {
  '低': { label: '低', color: 'text-slate-400', bg: 'bg-slate-500/20' },
  '中': { label: '中', color: 'text-amber-400', bg: 'bg-amber-500/20' },
  '高': { label: '高', color: 'text-red-400', bg: 'bg-red-500/20' },
}

const topicConfig = {
  '设备运行问题': { label: '设备运行问题', color: 'text-red-400' },
  '操作培训': { label: '操作培训', color: 'text-blue-400' },
  '定期回访': { label: '定期回访', color: 'text-green-400' },
  '技术支持': { label: '技术支持', color: 'text-purple-400' },
  '满意度调查': { label: '满意度调查', color: 'text-amber-400' },
  '其他': { label: '其他', color: 'text-slate-400' },
}
...
export default function CustomerCommunication() {
  ...
  const loadStatistics = useCallback(async () => {
    try {
      ...
      setStats({
        ...
        pendingFollowUp: communications.filter(c => 
          c.follow_up_required && (c.follow_up_status === '待处理' || c.follow_up_status === 'PENDING')
        ).length,
  ...
  const filteredCommunications = useMemo(() => {
    ...
    if (followUpFilter === 'REQUIRED') {
      ...
      result = result.filter(comm => comm.follow_up_required && comm.follow_up_status === '待处理')
    }

    // Date filter
    if (dateFilter.start) {
      result = result.filter(comm => comm.communication_date >= dateFilter.start)
    }
    if (dateFilter.end) {
      result = result.filter(comm => comm.communication_date <= dateFilter.end)
    }

    return result.sort((a, b) => {
      return new Date(b.communication_date + ' ' + (b.communication_time || '00:00')) - 
             new Date(a.communication_date + ' ' + (a.communication_time || '00:00'))
    })
  }, [communications, searchQuery, typeFilter, topicFilter, importanceFilter, followUpFilter, dateFilter])
        ...
        actions={
          <div className="flex gap-2">
            <Button
              ...
              size="sm"
              className="gap-2"
              onClick={() => { loadCommunications(); loadStatistics(); toast.success('数据已刷新'); }}
              disabled={loading}
            >
          ...
          <motion.div variants={fadeIn}>
            <Card className="bg-amber-500/10 border-amber-500/20">
              <CardContent className="p-4">
                ...
                <div className="text-2xl font-bold text-amber-400">{stats.pendingFollowUp}</div>
              </CardContent>
            </Card>
          </motion.div>
          <motion.div variants={fadeIn}>
            <Card className="bg-emerald-500/10 border-emerald-500/20">
              <CardContent className="p-4">
                ...
                <div className="text-2xl font-bold text-emerald-400">{stats.byType['电话'] || 0}</div>
              </CardContent>
            </Card>
          </motion.div>
        </motion.div>
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  {(searchQuery || typeFilter !== 'ALL' || topicFilter !== 'ALL' || 
                    importanceFilter !== 'ALL' || followUpFilter !== 'ALL' || 
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-3">
          {loading ? (
            ...
            filteredCommunications.map((comm) => {
              const typeConfig = communicationTypeConfig[comm.communication_type] || communicationTypeConfig['其他']
              const TypeIcon = typeConfig.icon
              const importance = importanceConfig[comm.importance] || importanceConfig['中']
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 space-y-3">
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            <span className="font-mono text-sm text-slate-300">{comm.communication_no}</span>
                            ...
                            <Badge variant="outline" className={cn('text-xs', topic.color)}>
                              {topic.label}
                            </Badge>
                          ...
                          {/* Content */}
                          <div>
                            <h3 className="text-white font-medium mb-1">{comm.subject}</h3>
                            <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400 mb-2">
                              <span className="flex items-center gap-1">
                                <User className="w-3 h-3" />
                                {comm.customer_name}
                                {comm.customer_contact && ` - ${comm.customer_contact}`}
                              </span>
                              {comm.project_name && (
                                <span className="flex items-center gap-1">
                              ...
                              <span className="flex items-center gap-1">
                                ...
                                {comm.communication_date} {comm.communication_time && comm.communication_time}
                              </span>
                              {comm.duration && (
                                <span className="flex items-center gap-1">
                          ...
                          {comm.follow_up_required && comm.follow_up_task && (
                            <div className="p-2 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                              <div className="flex items-center gap-2 text-xs">
                                ...
                                <span className="text-amber-400 font-medium">跟进任务:</span>
                                <span className="text-slate-300">{comm.follow_up_task}</span>
                                {comm.follow_up_due_date && (
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl bg-slate-900 border-slate-700 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>记录与客户的沟通内容，系统将自动生成记录号</DialogDescription>
        </DialogHeader>
        <DialogBody>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm text-slate-400 mb-1 block">沟通方式 *</label>
                <select
              ...
              <div>
                <label className="text-sm text-slate-400 mb-1 block">沟通主题 *</label>
                <select
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="text-sm text-slate-400 mb-1 block">沟通日期 *</label>
                <Input
                  type="date"
              ...
              <div>
                <label className="text-sm text-slate-400 mb-1 block">沟通时间</label>
                <Input
                  type="time"
              ...
              <div>
                <label className="text-sm text-slate-400 mb-1 block">沟通时长（分钟）</label>
                <Input
                  type="number"
            ...
            <div>
              <label className="text-sm text-slate-400 mb-1 block">沟通内容 *</label>
              <Textarea
            ...
            <div className="space-y-3">
              ...
              {formData.follow_up_required && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm text-slate-400 mb-1 block">跟进任务</label>
                    <Input
                      value={formData.follow_up_task}
                      onChange={(e) => setFormData({ ...formData, follow_up_task: e.target.value })}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm text-slate-400 mb-1 block">重要性</label>
                <select
...
function CommunicationDetailDialog({ communication, onClose, onUpdate }) {
  const typeConfig = communicationTypeConfig[communication.communication_type] || communicationTypeConfig['其他']
  const TypeIcon = typeConfig.icon
  const importance = importanceConfig[communication.importance] || importanceConfig['中']
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-white">{communication.customer_contact || '-'}</p>
              </div>
            ...
            {communication.project_name && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-white">{communication.project_code || '-'}</p>
                </div>
                <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              {communication.communication_time && (
                <div>
                  ...
                  <p className="text-white">{communication.communication_time}</p>
                </div>
              )}
            ...
            {communication.follow_up_required && (
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                <p className="text-sm text-amber-400 font-medium mb-2">后续跟进</p>
                {communication.follow_up_task && (
                  <p className="text-white mb-1">跟进任务: {communication.follow_up_task}</p>
                )}
                {communication.follow_up_due_date && (
                  <p className="text-slate-400 text-sm">截止日期: {communication.follow_up_due_date}</p>
                )}

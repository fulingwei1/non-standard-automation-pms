...
export default function CustomerServiceDashboard() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const issuesData =
        issuesResponse.data?.items || issuesResponse.data || [];
      ...
      const ticketsData =
        ticketsResponse.data?.items || ticketsResponse.data || [];
      ...
      const acceptanceData =
        acceptanceResponse.data?.items || acceptanceResponse.data || [];
      ...
      const transformedTodos = [
        ...issuesData.slice(0, 3).map((issue) => ({
          ...
          time: new Date(issue.created_at).toLocaleTimeString("zh-CN", {
            hour: "2-digit",
            minute: "2-digit",
          }),
        ...
        ...acceptanceData.slice(0, 2).map((acc) => ({
          ...
          time: acc.scheduled_date
            ? new Date(acc.scheduled_date).toLocaleTimeString("zh-CN", {
                hour: "2-digit",
                minute: "2-digit",
              })
            : "待定",
        ...
        ...ticketsData.slice(0, 2).map((ticket) => ({
          ...
          time: ticket.scheduled_time
            ? new Date(ticket.scheduled_time).toLocaleTimeString("zh-CN", {
                hour: "2-digit",
                minute: "2-digit",
              })
            : "待定",
      ...
      // Calculate stats
      const pendingIssues = issuesData.filter(
        (i) => i.status === "PENDING",
      ).length;
      const inProgressServices = ticketsData.filter(
      ...
      const urgentIssues = issuesData.filter(
        (i) => i.priority === "URGENT" || i.severity === "CRITICAL",
      ).length;
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <Badge
                      className={cn(
                        "text-xs",
                        issueStatusConfig[issue.status]?.color ||
                          "bg-slate-500",
                      )}
                    >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, impact_scope: e.target.value })
                }

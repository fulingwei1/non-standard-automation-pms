...
import { useCustomerServiceDashboard } from '../useCustomerServiceDashboard';
import { customerServiceApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        customerServiceApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), getDashboard: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useCustomerServiceDashboard Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [customerServiceApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useCustomerServiceDashboard());

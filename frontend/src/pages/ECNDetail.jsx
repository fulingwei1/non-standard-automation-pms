...
export default function ECNDetail() {
  ...
  const handleAnalyzeBomImpact = async () => {
    try {
      ...
      const result = await ecnApi.analyzeBomImpact(id, {
        include_cascade: true,
      });
      if (result.data?.has_impact) {
        alert(
          `BOM影响分析完成：影响${result.data.total_affected_items}项物料，成本影响¥${result.data.total_cost_impact?.toLocaleString()}`,
        );
      } else {
  ...
  const handleCheckObsoleteRisk = async () => {
    try {
      ...
      if (result.data?.has_obsolete_risk) {
        alert(
          `呆滞料风险检查完成：发现${result.data.obsolete_risks?.length || 0}个风险，总成本¥${result.data.total_obsolete_cost?.toLocaleString()}`,
        );
      } else {
      ...
      alert(
        "呆滞料检查失败: " + (error.response?.data?.detail || error.message),
      );
    } finally {
      setAnalyzingBom(false);
    }
  ...
  const handleFindSimilarEcns = async () => {
    try {
      setLoadingKnowledge(true);
      const result = await ecnApi.getSimilarEcns(id, {
        top_n: 5,
        min_similarity: 0.3,
      });
      ...
      <PageHeader
        title={
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="sm" onClick={() => navigate("/ecns")}>
              ...
              return (
                <div key={step.status} className="flex items-center flex-1">
                  <div className="flex flex-col items-center flex-1">
                    ...
                    <div
                      className={`text-xs mt-2 ${isActive ? "font-semibold" : "text-slate-400"}`}
                    >
                      {step.label}
                    </div>
                  </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <div className="mt-2">
                      <div className="w-full bg-slate-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${evaluationSummary.total_evaluations > 0 ? (evaluationSummary.submitted_count / evaluationSummary.total_evaluations) * 100 : 0}%`,
                          }}
                        />
                      </div>
                    </div>
        ...
        <TabsContent value="affected" className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                  <div>
                    ...
                    <div className="text-xl font-bold text-red-600">
                      ¥
                      {bomImpactSummary.total_cost_impact?.toLocaleString() ||
                        0}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {bomImpactSummary.bom_impacts.map((impact, idx) => (
                        <div key={idx} className="p-2 bg-white rounded text-sm">
                          ...
                          项受影响, 成本影响¥
                          {impact.cost_impact?.toLocaleString()}, 交期影响
                          {impact.schedule_impact_days}天
                        </div>
                      ))}
                    </div>
                  )}
          ...
          {responsibilitySummary &&
            responsibilitySummary.has_responsibility && (
              <Card className="border-green-200 bg-green-50/30">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {responsibilitySummary.responsibilities?.map(
                      (resp, idx) => (
                        <div
                          key={idx}
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-2">
                ...
                {(ecn.status === "COMPLETED" || ecn.status === "CLOSED") &&
                  ecn.solution && (
                    <Button
                      variant="outline"
                      onClick={() => {
                  ...
                  {solutionRecommendations.map((rec, idx) => (
                    <Card key={idx} className="p-3">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          handleApplySolutionTemplate(rec.template_id)
                        }
                        className="w-full"
                      >
                        应用此方案
          ...
          <DialogBody className="space-y-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setVerifyForm({ ...verifyForm, verify_result: value })
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setVerifyForm({ ...verifyForm, verify_note: e.target.value })
                }
                placeholder="请输入验证说明"
              />
            </div>
          ...
          <DialogBody className="space-y-4">
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setCloseForm({ ...closeForm, close_note: e.target.value })
                }
                placeholder="请输入关闭说明"
              />
            </div>
          ...
          <DialogBody className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setMaterialForm({ ...materialForm, change_type: value })
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <DialogBody className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setEvaluationForm({ ...evaluationForm, eval_result: value })
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <DialogBody className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setOrderForm({ ...orderForm, action_type: value })
                }
              >
                <SelectTrigger>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {responsibilityForm.length > 1 && (
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          const newForm = responsibilityForm.filter(
                            (_, i) => i !== index,
                          );
              ...
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`text-lg font-bold ${
                    Math.abs(
                      responsibilityForm.reduce(
                        (sum, r) =>
                          sum + parseFloat(r.responsibility_ratio || 0),
                        0,
                      ) - 100,
                    ) < 0.01
                      ? "text-green-600"
          ...
          <DialogBody className="space-y-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setRcaForm({ ...rcaForm, root_cause: value })
                }
              >
                <SelectTrigger>
      ...
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setShowSolutionTemplateDialog(false)}
            >
              取消
            </Button>

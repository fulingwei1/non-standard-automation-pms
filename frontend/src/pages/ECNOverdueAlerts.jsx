...
export default function ECNOverdueAlerts() {
  ...
  const handleSelectAll = () => {
    if (selectedAlerts.size === filteredAlerts.length) {
      ...
      setSelectedAlerts(new Set(filteredAlerts.map(a => `${a.type}-${a.ecn_id}-${a.task_id || a.approval_level || a.dept || ''}`)))
    }
  }
  ...
  const handleBatchProcess = async () => {
    ...
    if (!confirm(`确认发送提醒通知给相关人员？将处理 ${selectedAlerts.size} 个超时提醒。`)) {
      return
    }
    ...
    try {
      ...
      if (res.data) {
        ...
        if (fail_count === 0) {
          ...
          if (window.toast) {
            window.toast.success(`批量处理成功！已发送提醒通知给 ${success_count} 个提醒的相关人员。`)
          } else {
            alert(`批量处理成功！已发送提醒通知给 ${success_count} 个提醒的相关人员。`)
          }
        } else {
          if (window.toast) {
            window.toast.warning(`批量处理完成：成功 ${success_count} 个，失败 ${fail_count} 个。`)
          } else {
            alert(`批量处理完成：成功 ${success_count} 个，失败 ${fail_count} 个。`)
          }
        }
      } else {
        if (window.toast) {
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-8 text-slate-400">
              {filterType === 'all' ? '暂无超时提醒' : `暂无${alertTypeConfigs[filterType]?.label || ''}提醒`}
            </div>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-12">
                        <Checkbox
                          checked={selectedAlerts.size === groupedAlerts.EVALUATION_OVERDUE.length && 
                                   groupedAlerts.EVALUATION_OVERDUE.length > 0 &&
                                   groupedAlerts.EVALUATION_OVERDUE.every(a => 
                                     selectedAlerts.has(`${a.type}-${a.ecn_id}-${a.dept || ''}`)
                                   )}
                          onCheckedChange={(checked) => {
                            if (checked) {
                              const newSelected = new Set(selectedAlerts)
                              groupedAlerts.EVALUATION_OVERDUE.forEach(a => {
                                newSelected.add(`${a.type}-${a.ecn_id}-${a.dept || ''}`)
                              })
                              setSelectedAlerts(newSelected)
                            } else {
                              const newSelected = new Set(selectedAlerts)
                              groupedAlerts.EVALUATION_OVERDUE.forEach(a => {
                                newSelected.delete(`${a.type}-${a.ecn_id}-${a.dept || ''}`)
                              })
                              setSelectedAlerts(newSelected)
                            }
                          }}
                    ...
                    {groupedAlerts.EVALUATION_OVERDUE.map((alert, index) => {
                      ...
                      return (
                        <TableRow key={index}>
                          ...
                          <TableCell className="font-mono text-sm">{alert.ecn_no}</TableCell>
                          <TableCell>{alert.ecn_title}</TableCell>
                          <TableCell>
                            <Badge className="bg-amber-500">{alert.dept}</Badge>
                          </TableCell>
                          <TableCell>
                            <Badge className="bg-red-500">{alert.overdue_days} 天</Badge>
                          </TableCell>
                          <TableCell>
                            <Button
                              variant="ghost"
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-12">
                        <Checkbox
                          checked={selectedAlerts.size === groupedAlerts.APPROVAL_OVERDUE.length && 
                                   groupedAlerts.APPROVAL_OVERDUE.length > 0 &&
                                   groupedAlerts.APPROVAL_OVERDUE.every(a => 
                                     selectedAlerts.has(`${a.type}-${a.ecn_id}-${a.approval_level || ''}`)
                                   )}
                          onCheckedChange={(checked) => {
                            if (checked) {
                              const newSelected = new Set(selectedAlerts)
                              groupedAlerts.APPROVAL_OVERDUE.forEach(a => {
                                newSelected.add(`${a.type}-${a.ecn_id}-${a.approval_level || ''}`)
                              })
                              setSelectedAlerts(newSelected)
                            } else {
                              const newSelected = new Set(selectedAlerts)
                              groupedAlerts.APPROVAL_OVERDUE.forEach(a => {
                                newSelected.delete(`${a.type}-${a.ecn_id}-${a.approval_level || ''}`)
                              })
                              setSelectedAlerts(newSelected)
                            }
                          }}
                    ...
                    {groupedAlerts.APPROVAL_OVERDUE.map((alert, index) => {
                      ...
                      return (
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <Badge className="bg-blue-500">{alert.approval_role}</Badge>
                          </TableCell>
                          <TableCell>
                            <Badge className="bg-red-500">{alert.overdue_days} 天</Badge>
                          </TableCell>
          ...
          {groupedAlerts.TASK_OVERDUE.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>执行任务超过计划完成日期未完成</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-12">
                        <Checkbox
                          checked={selectedAlerts.size === groupedAlerts.TASK_OVERDUE.length && 
                                   groupedAlerts.TASK_OVERDUE.length > 0 &&
                                   groupedAlerts.TASK_OVERDUE.every(a => 
                                     selectedAlerts.has(`${a.type}-${a.ecn_id}-${a.task_id || ''}`)
                                   )}
                          onCheckedChange={(checked) => {
                            if (checked) {
                              const newSelected = new Set(selectedAlerts)
                              groupedAlerts.TASK_OVERDUE.forEach(a => {
                                newSelected.add(`${a.type}-${a.ecn_id}-${a.task_id || ''}`)
                              })
                              setSelectedAlerts(newSelected)
                            } else {
                              const newSelected = new Set(selectedAlerts)
                              groupedAlerts.TASK_OVERDUE.forEach(a => {
                                newSelected.delete(`${a.type}-${a.ecn_id}-${a.task_id || ''}`)
                              })
                              setSelectedAlerts(newSelected)
                            }
                          }}

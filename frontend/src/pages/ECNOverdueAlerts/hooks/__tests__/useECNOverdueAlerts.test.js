...
import { useECNOverdueAlerts } from '../useECNOverdueAlerts';
import { ecnApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        ecnApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), getOverdueAlerts: vi.fn().mockResolvedValue({ data: [] }) }
    };
});

describe('useECNOverdueAlerts Hook', () => {
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useECNOverdueAlerts());

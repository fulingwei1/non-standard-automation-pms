...
export default function ExecutiveDashboard() {
  ...
  const [costData, _setCostData] = useState([]);
  const [trendData, setTrendData] = useState([]);
  const [_projectStageData, _setProjectStageData] = useState([]);
  const [milestoneData, setMilestoneData] = useState({
    completionRate: 0,
    healthIndex: 0
  });
  const [_projectProgressData, _setProjectProgressData] = useState([]);
  const [_budgetData, _setBudgetData] = useState([]);
  const [_paymentData, _setPaymentData] = useState([]);
  const [_profitData, _setProfitData] = useState([]);
  const [salesFunnelData, _setSalesFunnelData] = useState([]);
  const [_customerDistributionData, _setCustomerDistributionData] = useState([]);
        ...
        if (dashboardRes.data) {
          ...
          if (dashboardRes.data.health_distribution) {
            ...
            const total = Object.values(
              dashboardRes.data.health_distribution
          ...
          if (dashboardRes.data.monthly) {
            ...
            setTrendData(
              Object.keys(monthly).map((month) => ({
                month,
                revenue:
                monthly[month].revenue || monthly[month].contract_amount || 0,
                profit: monthly[month].profit || 0,
                amount:
                monthly[month].amount || monthly[month].contract_amount || 0,
                count:
                monthly[month].count || monthly[month].new_contracts || 0
              }))
            );
          }
        }
        ...
        {kpiCards.map((kpi, index) => {
          ...
          return (
            <motion.div key={index} variants={fadeIn}>
              <Card
                className={cn("bg-gradient-to-br border", colorMap[kpi.color])}>
                ...
                <CardContent className="p-5">
                  <div className="flex items-start justify-between">
                    ...
                    <div
                      className={cn("p-3 rounded-xl", iconColorMap[kpi.color])}>

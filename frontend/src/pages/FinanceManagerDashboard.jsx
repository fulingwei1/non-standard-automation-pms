...
export default function FinanceManagerDashboard() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const purchaseOrdersData = purchaseOrdersResponse.data?.items || purchaseOrdersResponse.data || []

      // Calculate financial statistics
      const monthlyRevenue = invoicesData
        .filter(inv => inv.status === 'PAID' && new Date(inv.paid_date || inv.created_at).getMonth() === now.getMonth())
        .reduce((sum, inv) => sum + (parseFloat(inv.amount) || 0), 0)
      
      const monthlyCost = costsData
        .filter(cost => new Date(cost.cost_date).getMonth() === now.getMonth())
        .reduce((sum, cost) => sum + (parseFloat(cost.amount) || 0), 0)

      const yearRevenue = invoicesData
        .filter(inv => inv.status === 'PAID' && new Date(inv.paid_date || inv.created_at).getFullYear() === now.getFullYear())
        .reduce((sum, inv) => sum + (parseFloat(inv.amount) || 0), 0)

      const yearCost = costsData
        .filter(cost => new Date(cost.cost_date).getFullYear() === now.getFullYear())
        .reduce((sum, cost) => sum + (parseFloat(cost.amount) || 0), 0)
      ...
      const pendingInvoices = invoicesData.filter(inv => inv.status === 'PENDING' || inv.status === 'ISSUED')
      const pendingInvoicesAmount = pendingInvoices.reduce((sum, inv) => sum + (parseFloat(inv.amount) || 0), 0)
      ...
      const transformedPayments = purchaseOrdersData
        ...
        .map(po => ({
          ...
          daysPending: po.created_at ? Math.floor((now - new Date(po.created_at)) / (1000 * 60 * 60 * 24)) : 0,
      ...
      const transformedProjectCosts = await Promise.all(projectsData.slice(0, 20).map(async (project) => {
        try {
          const summaryResponse = await costApi.getProjectSummary(project.id)
          const summary = summaryResponse.data || {}
          const totalCost = parseFloat(summary.total_amount || 0)
          const budget = parseFloat(project.budget || project.contract_amount || 0)
      ...
      setFinancialStats({
        ...
        overdueReceivable: 0,
        collectionRate: yearRevenue > 0 ? ((yearRevenue - pendingInvoicesAmount) / yearRevenue) * 100 : 0,
        accountsPayable: transformedPayments.reduce((sum, p) => sum + p.amount, 0),
        overduePayable: 0,
        pendingPayments: transformedPayments.length,
        pendingAmount: transformedPayments.reduce((sum, p) => sum + p.amount, 0),
        urgentPayments: transformedPayments.filter(p => p.priority === 'high' || p.priority === 'urgent').length,
        pendingInvoices: pendingInvoices.length,
        invoicedAmount: pendingInvoicesAmount,
        paidInvoices: paidInvoices.length,
        paidAmount: paidInvoicesAmount,
        activeProjects: projectsData.length,
        projectsOverBudget: transformedProjectCosts.filter(p => p.used > 100).length,
        totalProjectCost: transformedProjectCosts.reduce((sum, p) => sum + p.actual, 0),
        totalProjectBudget: transformedProjectCosts.reduce((sum, p) => sum + p.budget, 0),
        teamSize: 6,
        activeTeamMembers: 6,
      })
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-slate-400 mb-1">年度营收目标</p>
                      ...
                      <div className="mt-2">
                        ...
                        <div className="flex items-center justify-between text-xs mt-1">
                          ...
                          <span className="text-emerald-400">
                            已完成: {formatCurrency(mockFinancialStats.yearRevenue)}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div>
                      <p className="text-sm text-slate-400 mb-1">年度成本预算</p>
                      <p className="text-2xl font-bold text-white">
                      ...
                      <div className="mt-2">
                        ...
                        <div className="flex items-center justify-between text-xs mt-1">
                          <span className="text-slate-400">
                            使用率: {mockFinancialStats.yearBudgetUsed.toFixed(1)}%
                          </span>
                  ...
                  <div className="pt-4 border-t border-slate-700/50">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                          {mockFinancialStats.yearBudgetUsed < 100 ? '良好' : '超支'}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <Tabs defaultValue="revenue" className="w-full">
                  ...
                  <TabsContent value="revenue" className="space-y-4 mt-4">
                    {mockMonthlyTrends.map((item, index) => {
                      const maxRevenue = Math.max(...mockMonthlyTrends.map(t => t.revenue))
                          ...
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              {index > 0 && (
                                <span className={cn(
                                  'text-xs',
                                  item.revenue > mockMonthlyTrends[index - 1].revenue
                                    ? 'text-emerald-400'
                                    : 'text-red-400'
                                )}>
                                  {item.revenue > mockMonthlyTrends[index - 1].revenue ? '↑' : '↓'}
                                  {Math.abs(
                                    ((item.revenue - mockMonthlyTrends[index - 1].revenue) / mockMonthlyTrends[index - 1].revenue) * 100
                                  ).toFixed(1)}%
                                </span>
                              )}
                            </div>
                  ...
                  <TabsContent value="cost" className="space-y-4 mt-4">
                    {mockMonthlyTrends.map((item, index) => {
                      const maxCost = Math.max(...mockMonthlyTrends.map(t => t.cost))
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              {index > 0 && (
                                <span className={cn(
                                  ...
                                  {item.cost < mockMonthlyTrends[index - 1].cost ? '↓' : '↑'}
                                  {Math.abs(
                                    ((item.cost - mockMonthlyTrends[index - 1].cost) / mockMonthlyTrends[index - 1].cost) * 100
                                  ).toFixed(1)}%
                                </span>
                              )}
                            </div>
                  ...
                  <TabsContent value="profit" className="space-y-4 mt-4">
                    {mockMonthlyTrends.map((item, index) => {
                      const maxProfit = Math.max(...mockMonthlyTrends.map(t => t.profit))
                          ...
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              {index > 0 && (
                                <span className={cn(
                                  'text-xs',
                                  item.profit > mockMonthlyTrends[index - 1].profit
                                    ? 'text-emerald-400'
                                    : 'text-red-400'
                                )}>
                                  {item.profit > mockMonthlyTrends[index - 1].profit ? '↑' : '↓'}
                                  {Math.abs(
                                    ((item.profit - mockMonthlyTrends[index - 1].profit) / mockMonthlyTrends[index - 1].profit) * 100
                                  ).toFixed(1)}%
                                </span>
                              )}
                            </div>
                  ...
                  <TabsContent value="cashflow" className="space-y-4 mt-4">
                    {mockMonthlyTrends.map((item, index) => {
                      const maxCashFlow = Math.max(...mockMonthlyTrends.map(t => t.cashFlow))
                          ...
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              {index > 0 && (
                                <span className={cn(
                                  'text-xs',
                                  item.cashFlow > mockMonthlyTrends[index - 1].cashFlow
                                    ? 'text-emerald-400'
                                    : 'text-red-400'
                                )}>
                                  {item.cashFlow > mockMonthlyTrends[index - 1].cashFlow ? '↑' : '↓'}
                                  {Math.abs(
                                    ((item.cashFlow - mockMonthlyTrends[index - 1].cashFlow) / mockMonthlyTrends[index - 1].cashFlow) * 100
                                  ).toFixed(1)}%
                                </span>
                              )}
                            </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <div className="flex items-center gap-2">
                              <span className={cn(
                                'font-medium',
                                project.variance > 0 ? 'text-red-400' : 'text-emerald-400'
                              )}>
              ...
              <CardContent className="space-y-3">
                {pendingPayments.map((item) => {
                  ...
                  const typeColors = {
                    ...
                    outsourcing: 'bg-purple-500/20 text-purple-400 border-purple-500/30',
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          ...
                          <p className="font-medium text-white text-sm">{item.orderNo}</p>
                          <p className="text-xs text-slate-400 mt-1">
                            {item.projectName || item.category || item.department}
                          </p>
                          <p className="text-xs text-slate-500 mt-1">
                            {item.supplier || item.submitter} · {item.submitTime.split(' ')[1]}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center justify-between text-xs mt-2">
                        <span className="text-slate-400">
                          {item.daysPending > 0 ? `待审批${item.daysPending}天` : '今日提交'}
                        </span>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-4 border-t border-slate-700/50 grid grid-cols-2 gap-3">
                  <div className="text-center p-3 bg-slate-800/40 rounded-lg">
                    ...
                    <div className="text-xs text-slate-400 mt-1">可用现金流</div>
                  </div>
                  <div className="text-center p-3 bg-slate-800/40 rounded-lg">
                    <div className="text-lg font-bold text-red-400">
                      {mockFinancialStats.projectsOverBudget}
                    </div>
                    <div className="text-xs text-slate-400 mt-1">超预算项目</div>
                  </div>
                </div>
              </CardContent>
            </Card>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="text-right">
                          <div className={cn('text-sm font-bold', performanceColors[member.performance])}>

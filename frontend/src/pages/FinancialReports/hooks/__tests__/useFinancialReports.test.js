...
vi.mock('../../../../services/api', () => {
    return {
        reportApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), exportFinancial: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), generateFinancial: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), listFinancial: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useFinancialReports Hook', () => {

...
import { useGoodsReceiptNew } from '../useGoodsReceiptNew';
...
vi.mock('../../../../services/api', () => {
    return {
        ...
        purchaseOrderApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), getPending: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useGoodsReceiptNew Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [goodsReceiptApi, purchaseOrderApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useGoodsReceiptNew());

...
import { Users, FileText, Calendar, AlertCircle, TrendingUp, CheckCircle, Plus, Edit, Eye } from 'lucide-react'
...
export default function HRManagement() {
  ...
  const loadTransactions = async () => {
    try {
      setLoading(true)
      const response = await hrManagementApi.transactions.list({ page: 1, page_size: 10 })
      const data = response.data?.data || response.data || response
  ...
  const loadContracts = async () => {
    try {
      const response = await hrManagementApi.contracts.list({ page: 1, page_size: 10 })
      const data = response.data?.data || response.data || response
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{dashboardData.total_active || 0}</p>
                </div>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{dashboardData.probation_count || 0}</p>
                </div>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{dashboardData.pending_transactions || 0}</p>
                </div>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{dashboardData.expiring_contracts_60days || 0}</p>
                </div>
                    ...
                    <tbody>
                      {transactions.map((t) => (
                        <tr key={t.id} className="border-b hover:bg-gray-50">
                          <td className="p-2">{t.employee_name || t.employee_code}</td>
                          <td className="p-2">
                            <Badge variant="outline">{getTransactionTypeLabel(t.transaction_type)}</Badge>
                          </td>
                          <td className="p-2">{t.transaction_date ? formatDate(t.transaction_date) : '-'}</td>
                          <td className="p-2">
                            <Badge variant={t.status === 'pending' ? 'secondary' : t.status === 'approved' ? 'default' : 'outline'}>
                              {t.status === 'pending' ? '待审批' : t.status === 'approved' ? '已审批' : t.status}
                            </Badge>
                          </td>
                          <td className="p-2">
                            {t.status === 'pending' && (
                    ...
                    <tbody>
                      {contracts.map((c) => (
                        <tr key={c.id} className="border-b hover:bg-gray-50">
                          <td className="p-2">{c.employee_name || c.employee_code}</td>
                          <td className="p-2">{c.contract_no}</td>
                          <td className="p-2">{c.start_date ? formatDate(c.start_date) : '-'}</td>
                          <td className="p-2">{c.end_date ? formatDate(c.end_date) : '-'}</td>
                          <td className="p-2">
                          ...
                          <td className="p-2">
                            {c.days_until_expiry !== null && c.days_until_expiry !== undefined && (
                              <span className={c.days_until_expiry <= 30 ? 'text-red-600 font-semibold' : ''}>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setTransactionForm({ ...transactionForm, employee_id: e.target.value })}
              />
            </div>
            <div>
              <Label>事务类型 *</Label>

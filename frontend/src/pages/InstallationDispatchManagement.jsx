...
export default function InstallationDispatchManagement() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      toast.error(
        "加载派工单失败: " + (error.response?.data?.detail || error.message),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleBatchAssign = async () => {
    ...
    try {
      ...
      toast.error(
        "派工失败: " + (error.response?.data?.detail || error.message),
      );
    }
  };
  ...
  const handleCreate = async () => {
    try {
      ...
      const selectedProject = projects.find(
        (p) => p.id.toString() === createData.project_id,
      );
      if (!selectedProject) {
        toast.error("请选择项目");
        return;
      ...
      const createPayload = {
        ...
        machine_id: createData.machine_id
          ? parseInt(createData.machine_id)
          : null,
        estimated_hours: createData.estimated_hours
          ? parseFloat(createData.estimated_hours)
          : null,
      };
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      toast.error(
        "开始任务失败: " + (error.response?.data?.detail || error.message),
      );
    }
  };
  ...
  const handleUpdateProgress = async (progress, notes) => {
    ...
    try {
      ...
      toast.error(
        "更新进度失败: " + (error.response?.data?.detail || error.message),
      );
    }
  };
  ...
  const handleComplete = async (completeData) => {
    if (!selectedOrder) return;
    try {
      await installationDispatchApi.orders.complete(
        selectedOrder.id,
        completeData,
      );
      toast.success("任务已完成");
      ...
      toast.error(
        "完成任务失败: " + (error.response?.data?.detail || error.message),
      );
    }
  };
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (filterProject) {
      filtered = filtered.filter(
        (o) => o.project_id === parseInt(filterProject),
      );
    }
    if (filterTaskType) {
    ...
    <div className="space-y-6 p-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>共 {filteredOrders.length} 个派工单</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <Table>
              ...
              <TableBody>
                {filteredOrders.map((order) => {
                  ...
                  const priority =
                    priorityConfig[order.priority] || priorityConfig.NORMAL;
                  const taskType =
                    taskTypeConfig[order.task_type] || taskTypeConfig.OTHER;
                  return (
                    <TableRow
                      key={order.id}
                      className={cn(
        ...
        <DialogContent>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setAssignData({
                      ...assignData,
                      assigned_to_id: val ? parseInt(val) : null,
                    })
                  }
                >
                  <SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!assignData.assigned_to_id}
            >
              确认派工
            </Button>
          </DialogFooter>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(val) =>
                      setCreateData({ ...createData, machine_id: val })
                    }
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={
                          createData.project_id ? "选择机台" : "请先选择项目"
                        }
                      />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setCreateData({ ...createData, task_type: val })
                  }
                >
                  <SelectTrigger>
                ...
                {selectedOrder.status === "PENDING" && (
                  <Button
                    onClick={() => {
                      ...
                      setAssignData({
                        assigned_to_id: selectedOrder.assigned_to_id,
                        remark: "",
                      });
                    }}
                  >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setProgressData({
                          ...
                          execution_notes: selectedOrder.execution_notes || "",
                        });
                      }}
                    >
      ...
      <Dialog
        open={showAssignDialog && selectedOrder}
        onOpenChange={(open) => {

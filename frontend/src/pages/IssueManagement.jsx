...
export default function IssueManagement() {
  ...
  const handleExport = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `问题列表_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="text-white">
                  已选择{" "}
                  <span className="font-bold">{selectedIssues.length}</span>{" "}
                  个问题
                </div>
                <div className="flex gap-2">
                  <Button
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const newStatus = prompt(
                        "请输入新状态 (OPEN/PROCESSING/RESOLVED/CLOSED):",
                      );
                      if (newStatus) {
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <Badge
                                className={cn(priorityColors[issue.priority])}
                              >
                                {issue.priority}
                              </Badge>
      ...
      <DialogContent className="max-w-4xl bg-surface-50 border-white/10 text-white max-h-[90vh]">
        ...
        <div className="space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="text-white bg-surface-100 p-3 rounded-lg">
              {issue.description}
            </div>
          </div>
...
function CreateIssueDialog({ onClose, onSubmit }) {
  ...
  const handleCreateFromTemplate = async () => {
    ...
    try {
      ...
      const response = await issueTemplateApi.createIssue(
        selectedTemplate.id,
        templateData,
      );
      const newIssue = response.data || response;
      ...
      alert(
        "从模板创建问题失败: " + (err.response?.data?.detail || err.message),
      );
    }
  };
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-2 pt-6">
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, is_blocking: e.target.checked })
                }
                className="w-4 h-4"
              />
...
function IssueBoardView({ issues, onIssueClick, onStatusChange, onRefresh }) {
  ...
  const handleDragEnd = async (event) => {
    ...
    try {
      ...
      setLocalIssues(issues);
      alert(
        "更新问题状态失败: " + (error.response?.data?.detail || error.message),
      );
    }
  };

  const draggedIssue = activeId
    ? localIssues.find((i) => `issue-${i.id}` === activeId)
    : null;
            ...
            <CardContent className="p-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <Badge
                  className={cn(
                    "text-xs",
                    severityColors[draggedIssue.severity],
                  )}
                >
                ...
                <Badge
                  className={cn(
                    "text-xs",
                    priorityColors[draggedIssue.priority],
                  )}
                >
      ...
      >
        <CardContent className="p-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {issue.is_blocking && (
              <Badge className="text-xs bg-red-500/20 text-red-400">阻塞</Badge>
            )}
          </div>
            ...
            <CardContent>
              <SimplePieChart
                data={[
                  ...
                  {
                    label: "重要",
                    value: statistics.major || 0,
                    color: "#f59e0b",
                  },
                  {
                    label: "一般",
                    value: statistics.minor || 0,
                    color: "#3b82f6",
                  },
                ].filter((item) => item.value > 0)}
                size={250}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="bg-surface-50 border-white/5">
            ...
            <CardContent>
              <SimplePieChart
                data={
                  causeData.top_causes?.slice(0, 10).map((cause, idx) => ({
          ...
          <Card className="bg-surface-50 border-white/5">
            <CardHeader>
              <CardTitle>
                Top {causeData.top_causes?.length || 0} 问题原因
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {causeData.top_causes?.map((cause, idx) => (
                  <div key={idx} className="flex items-center gap-4">
                    <div
                      className="w-32 text-sm text-white truncate"
                      title={cause.cause}
                    >
                      {cause.cause || "未知"}
                    </div>

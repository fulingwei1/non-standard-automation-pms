...
import { useLessonsLearnedLibrary } from '../useLessonsLearnedLibrary';
import { projectReviewApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        projectReviewApi: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), query: vi.fn(), aiMatch: vi.fn(), assign: vi.fn(), getLessonCategories: vi.fn(), getLessonsStatistics: vi.fn(), searchLessonsLearned: vi.fn() }
    };
});
...
describe('useLessonsLearnedLibrary Hook', () => {
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useLessonsLearnedLibrary());

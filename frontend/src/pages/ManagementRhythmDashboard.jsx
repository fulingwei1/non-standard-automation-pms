...
const healthStatusConfig = {
  ...
  YELLOW: { label: '有风险', color: 'bg-yellow-500', textColor: 'text-yellow-700' },
...
export default function ManagementRhythmDashboard() {
  ...
  const renderLevelCard = (level, data) => {
    ...
    const daysUntil = data?.next_meeting_date ? calculateDaysUntil(data.next_meeting_date) : null
                ...
                {data.next_meeting_date && (
                  <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">{formatDate(data.next_meeting_date)}</span>
                    </div>
                    {daysUntil !== null && (
                      <Badge variant={daysUntil <= 3 ? 'destructive' : 'default'}>
                        {daysUntil > 0 ? `${daysUntil}天后` : daysUntil === 0 ? '今天' : '已过期'}
                      </Badge>
                    )}
                  </div>
                )}
                ...
                {data.last_meeting_date && (
                  <div className="pt-3 border-t">
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span>上次会议: {formatDate(data.last_meeting_date)}</span>
                    </div>
                  </div>
                )}

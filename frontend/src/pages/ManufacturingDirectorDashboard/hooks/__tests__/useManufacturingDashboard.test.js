...
import { useManufacturingDashboard } from '../useManufacturingDashboard';
import { productionApi, projectApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        productionApi: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), query: vi.fn(), aiMatch: vi.fn(), assign: vi.fn() },
        projectApi: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), query: vi.fn(), aiMatch: vi.fn(), assign: vi.fn() }
    };
});

describe('useManufacturingDashboard Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [productionApi, projectApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useManufacturingDashboard());

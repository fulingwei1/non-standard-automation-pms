...
export default function MaterialRequisitionDetail() {
  ...
  const handleApprove = async () => {
    ...
    try {
      await productionApi.materialRequisitions.approve(requisition.id, approveData)
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="font-mono text-sm">{requisition.requisition_no}</div>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <Badge className={statusConfigs[requisition.status]?.color || 'bg-slate-500'}>
                  {statusConfigs[requisition.status]?.label || requisition.status}
                </Badge>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="font-medium">{requisition.project_name || '-'}</div>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-sm">
                  {requisition.apply_time ? formatDate(requisition.apply_time) : '-'}
                </div>
              </div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="font-mono">{requisition.work_order_no || '-'}</div>
            </div>
            <div>
            ...
            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.id} className="border rounded-lg p-3">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-sm text-slate-500 mb-1">申请数量</div>
                      <div>{item.apply_qty || 0} {item.unit || ''}</div>
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-2 block">批准数量</label>
                      <Input
                        type="number"
                        value={approveData.approved_qty[item.id] || item.apply_qty || 0}
            ...
            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.id} className="border rounded-lg p-3">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-sm text-slate-500 mb-1">批准数量</div>
                      <div>{item.approved_qty || 0} {item.unit || ''}</div>
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-2 block">发料数量</label>
                      <Input
                        type="number"
                        value={issueData.issued_qty[item.id] || item.approved_qty || 0}
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">发料说明</label>
                <textarea
                  className="w-full min-h-[80px] p-3 border rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
                  value={issueData.issue_comment}
                  onChange={(e) => setIssueData({ ...issueData, issue_comment: e.target.value })}
                  placeholder="发料说明..."
                />
              </div>
            </div>

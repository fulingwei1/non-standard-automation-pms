...
import { useOpportunityManagement } from '../useOpportunityManagement';
import { salesApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        salesApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), createOpportunity: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), listOpportunities: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), updateOpportunity: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useOpportunityManagement Hook', () => {
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useOpportunityManagement());

...
export default function OutsourcingOrderDetail() {
  ...
  const totalQuantity = order.items?.reduce((sum, item) => sum + (parseFloat(item.quantity) || 0), 0) || 0
  const deliveredQuantity = order.items?.reduce((sum, item) => sum + (parseFloat(item.delivered_quantity) || 0), 0) || 0
  const qualifiedQuantity = order.items?.reduce((sum, item) => sum + (parseFloat(item.qualified_quantity) || 0), 0) || 0
  const rejectedQuantity = order.items?.reduce((sum, item) => sum + (parseFloat(item.rejected_quantity) || 0), 0) || 0
  const deliveryRate = totalQuantity > 0 ? (deliveredQuantity / totalQuantity * 100) : 0
  const passRate = deliveredQuantity > 0 ? (qualifiedQuantity / deliveredQuantity * 100) : 0
  return (
    <div className="space-y-6 p-6">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div>{order.required_date ? formatDate(order.required_date) : '-'}</div>
            </div>
            <div>
          ...
          <Tabs defaultValue="items" className="w-full">
            ...
            <TabsContent value="items" className="mt-4">
              {order.items && order.items.length > 0 ? (
                  ...
                  <TableBody>
                    {order.items.map((item) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell>
                          <span className={parseFloat(item.delivered_quantity || 0) >= parseFloat(item.quantity || 0) ? 'text-emerald-600' : ''}>
                            {item.delivered_quantity || 0}
                          </span>
                        </TableCell>
                        <TableCell className="text-emerald-600">{item.qualified_quantity || 0}</TableCell>
                        <TableCell className="text-red-600">{item.rejected_quantity || 0}</TableCell>
                        <TableCell>
                ...
                <div className="text-center py-8 text-slate-400">暂无订单明细</div>
              )}
            </TabsContent>
            <TabsContent value="deliveries" className="mt-4">
              {deliveries.length === 0 ? (
                <div className="text-center py-8 text-slate-400">暂无交付记录</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {deliveries.map((delivery) => (
                      <TableRow key={delivery.id}>
                        <TableCell className="font-mono text-sm">{delivery.delivery_no}</TableCell>
                        ...
                        <TableCell>{delivery.logistics_company || '-'}</TableCell>
                        <TableCell className="font-mono text-sm">{delivery.tracking_no || '-'}</TableCell>
                        <TableCell>
                          <Badge variant={delivery.status === 'RECEIVED' ? 'default' : 'outline'}>
                            {delivery.status === 'RECEIVED' ? '已收货' : '待收货'}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-slate-500 text-sm">
                          {delivery.received_at ? formatDate(delivery.received_at) : '-'}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
            ...
            <TabsContent value="inspections" className="mt-4">
              {inspections.length === 0 ? (
                <div className="text-center py-8 text-slate-400">暂无质检记录</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {inspections.map((inspection) => (
                      <TableRow key={inspection.id}>
                        <TableCell className="font-mono text-sm">{inspection.inspection_no}</TableCell>
                        <TableCell>{formatDate(inspection.inspect_date)}</TableCell>
                        <TableCell>{inspection.inspect_type || '-'}</TableCell>
                        <TableCell>{inspection.inspector_name || '-'}</TableCell>
                        <TableCell>{inspection.inspect_quantity || 0}</TableCell>
                        <TableCell className="text-emerald-600">{inspection.qualified_quantity || 0}</TableCell>
                        <TableCell className="text-red-600">{inspection.rejected_quantity || 0}</TableCell>
                        <TableCell>
                          <span className={inspection.pass_rate >= 95 ? 'text-emerald-600' : inspection.pass_rate >= 80 ? 'text-amber-600' : 'text-red-600'}>
                            {inspection.pass_rate?.toFixed(1) || 0}%
                          </span>
                        </TableCell>
                        <TableCell>
                          <Badge className={inspectResultConfigs[inspection.inspect_result]?.color || 'bg-slate-500'}>
                            {inspectResultConfigs[inspection.inspect_result]?.label || inspection.inspect_result}
                          </Badge>
                        </TableCell>
            ...
            <TabsContent value="progress" className="mt-4">
              {progressLogs.length === 0 ? (
                <div className="text-center py-8 text-slate-400">暂无进度记录</div>
              ) : (
                    ...
                    <Card key={progress.id}>
                      <CardContent className="pt-4">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <div className="font-medium">{formatDate(progress.report_date)}</div>
                          ...
                          <div className="text-right">
                            ...
                            <Progress value={progress.progress_pct || 0} className="mt-2 w-24" />
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4 mt-3">
                          <div>
                            <div className="text-sm text-slate-500">完成数量</div>
                            <div className="font-medium">{progress.completed_quantity || 0}</div>
                          </div>
                          <div>
                            <div className="text-sm text-slate-500">预计完成</div>
                            <div className="font-medium">
                              {progress.estimated_complete ? formatDate(progress.estimated_complete) : '-'}
                            </div>
                          </div>
                        </div>
                        {progress.issues && (
                          <div className="mt-3 p-3 bg-amber-50 rounded-lg">
                            <div className="text-sm font-medium text-amber-800 mb-1">问题/风险</div>
                            <div className="text-sm text-amber-700">{progress.issues}</div>
                          </div>
                        )}
                        {progress.risk_level && (
                          <div className="mt-2">
                            <Badge
                              className={cn(
                                ...
                                progress.risk_level === 'MEDIUM' && 'bg-amber-500',
                                progress.risk_level === 'LOW' && 'bg-blue-500',
                                'bg-slate-500'
                              )}
                            >
                              {progress.risk_level === 'HIGH' ? '高风险' :
                               progress.risk_level === 'MEDIUM' ? '中风险' :
                               progress.risk_level === 'LOW' ? '低风险' : progress.risk_level}
                            </Badge>
                          </div>
                        )}
                      </CardContent>

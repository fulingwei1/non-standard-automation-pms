...
import { formatCurrency } from "../../../components/payment-management";

export function AgingAnalysis({ agingData }) {
    const agingItems = [
                    ...
                    <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-slate-400 text-sm">{item.count || 0} 笔付款</p>
                            </div>
                            <div className="text-right">
                                <div className="text-lg font-bold text-white">{formatCurrency(item.amount || 0)}</div>
                                <Progress value={totalAmount > 0 ? ((item.amount || 0) / totalAmount) * 100 : 0} className="w-24 h-2 mt-1" />
                            </div>
                        </div>
                    </CardContent>
                </Card>

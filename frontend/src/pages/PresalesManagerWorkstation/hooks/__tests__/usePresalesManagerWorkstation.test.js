...
import { usePresalesManagerWorkstation } from '../usePresalesManagerWorkstation';
import { presaleApi, taskApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        presaleApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), getTeamMembers: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), listOpportunities: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) },
        taskApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), getTeamTasks: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('usePresalesManagerWorkstation Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [presaleApi, taskApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => usePresalesManagerWorkstation());

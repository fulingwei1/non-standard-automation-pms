...
export default function ProcurementEngineerWorkstation() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const transformedOrders = ordersData.map(order => {
        ...
        else if (daysLeft < 0 && order.status !== 'RECEIVED') arrivalStatus = 'delayed'
      ...
      setStats({
        ...
        onTimeRate: {
          ...
          value: transformedOrders.length > 0 
            ? Math.round((transformedOrders.filter(o => o.daysLeft >= 0 || o.arrivalStatus === 'completed').length / transformedOrders.length) * 100)
            : 94,
        },
      })
  ...
  const delayedOrders = purchaseOrders.filter(o => o.daysLeft < 0 && o.arrivalStatus !== 'completed').length
  const inTransitOrders = purchaseOrders.filter(o => o.arrivalStatus === 'in_transit').length
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <PageHeader
        ...
        actions={
          <Button onClick={() => navigate('/purchases?action=create')} className="bg-blue-600 hover:bg-blue-700">
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {[
                { icon: Plus, label: '新建采购单', color: 'text-blue-400', path: '/purchases', action: () => navigate('/purchases?action=create') },
                { icon: ShoppingCart, label: '采购订单', color: 'text-slate-400', path: '/purchases', action: () => navigate('/purchases') },
                { icon: Truck, label: '到货管理', color: 'text-emerald-400', path: '/purchases/receipts', action: () => navigate('/purchases/receipts') },
                { icon: CheckCircle2, label: '来料检验', color: 'text-cyan-400', path: '/purchases/receipts', action: () => navigate('/purchases/receipts') },
                { icon: Building2, label: '供应商管理', color: 'text-purple-400', path: '/suppliers', action: () => navigate('/suppliers') },
                { icon: DollarSign, label: '成本分析', color: 'text-amber-400', path: '/cost-analysis', action: () => navigate('/cost-analysis') },
                { icon: AlertTriangle, label: '缺料预警', color: 'text-red-400', path: '/shortage-alert', action: () => navigate('/shortage-alert') },
                { icon: BarChart3, label: '采购报表', color: 'text-indigo-400', path: '#', action: () => {} },
              ].map((item, idx) => (
                <Button
                  key={idx}
                  variant="ghost"
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              <div className="flex items-center justify-between rounded-lg bg-amber-500/20 px-3 py-2">
                ...
                <span className="font-semibold text-amber-400">{shortages.length}</span>
              </div>
            </CardContent>
          </Card>
        </div>

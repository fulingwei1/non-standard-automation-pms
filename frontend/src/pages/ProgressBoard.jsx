            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 rounded-md border bg-white">
                  <div className="text-xs text-slate-500 mb-1">预计完工日期</div>
                ...
                <div className="p-3 rounded-md border bg-white">
                  ...
                  <div className="text-xs text-slate-400">预测范围 {forecastData.forecast_horizon_days} 天</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 rounded-md border bg-white">
              ...
              <div className="flex items-center gap-3">
                <div className={cn(
                  ...
                  confidenceTone[forecastData.confidence] || 'bg-slate-100 text-slate-600'
                )}>
                ...
                delayedTasks.map(task => (
                  ...
                  >
                    ...
                    <div className="text-xs text-slate-500 mt-1 flex items-center justify-between">
                      <span>预测完成: {formatDate(task.predicted_finish_date)}</span>
                      <span>当前进度 {task.progress_percent}%</span>
                    </div>
                  </div>
                ))
              ) : (
                <div className="text-sm text-slate-500">暂无延迟任务，保持节奏。</div>
              )}
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div className="p-3 border rounded-md bg-white">
                ...
                <div className="text-2xl font-semibold text-red-600">
                  {dependencyData.issues?.filter(issue => issue.severity === 'HIGH').length || 0}
                </div>
                <div className="text-xs text-slate-500">需要立即处理的依赖冲突</div>
              </div>
              <div className="p-3 border rounded-md bg-white">
              ...
              {(dependencyData.issues || []).slice(0, 6).map((issue, index) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="font-medium">{issue.task_name || '未指派任务'}</div>
                    <Badge
                      className={cn(
                        'uppercase',
                        issue.severity === 'HIGH'

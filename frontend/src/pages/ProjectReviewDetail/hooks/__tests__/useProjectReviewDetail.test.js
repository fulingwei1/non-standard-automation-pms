...
import { useProjectReviewDetail } from '../useProjectReviewDetail';
import { projectReviewApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        projectReviewApi: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), query: vi.fn(), aiMatch: vi.fn(), assign: vi.fn(), addComment: vi.fn(), getComments: vi.fn(), submitDecision: vi.fn() }
    };
});

describe('useProjectReviewDetail Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [projectReviewApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useProjectReviewDetail(1));

...
vi.mock('../../../../services/api', () => {
    return {
        projectRolesApi: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), query: vi.fn(), aiMatch: vi.fn(), assign: vi.fn() }
    };
});
...
describe('useProjectRoles Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [projectRolesApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useProjectRoles());

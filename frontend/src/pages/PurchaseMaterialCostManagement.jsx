...
export default function PurchaseMaterialCostManagement() {
  ...
  const loadCosts = async () => {
    setLoading(true)
    try {
      const res = await salesTemplateApi.listPurchaseMaterialCosts({ page: 1, page_size: 1000 })
  ...
  const filterCosts = () => {
    ...
    if (standardFilter !== 'all') {
      filtered = filtered.filter(c => c.is_standard_part === (standardFilter === 'standard'))
    }
  ...
  const handleSave = async () => {
    try {
      ...
      if (selectedCost) {
        await salesTemplateApi.updatePurchaseMaterialCost(selectedCost.id, submitData)
      } else {
          ...
          <div className="flex gap-2">
            {reminder && (
              <Button
                ...
                className={reminder.is_due ? "bg-amber-500 hover:bg-amber-600" : ""}
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="font-medium text-amber-400 mb-1">物料成本更新提醒</div>
              <div className="text-sm text-slate-300 mb-3">
                距离上次更新已超过 {reminder.reminder_interval_days} 天，请及时更新物料成本信息，确保报价成本准确性。
              </div>
              <div className="flex gap-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-slate-400">
                    距离下次提醒还有 <strong className="text-blue-300">{reminder.days_until_next}</strong> 天
                    {reminder.next_reminder_date && ` (${formatDate(reminder.next_reminder_date)})`}
                  </div>
                </div>
              </div>
                ...
                <TableRow key={cost.id}>
                  <TableCell>
                    <div>
                      ...
                      {cost.material_code && (
                        <div className="text-xs text-slate-400">{cost.material_code}</div>
                      )}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <Badge className={cost.match_priority > 0 ? 'bg-blue-500' : 'bg-slate-500'}>
                      {cost.match_priority}
                    </Badge>
                  </TableCell>
                  <TableCell>{cost.usage_count || 0}</TableCell>
                  <TableCell>
                    <Badge className={cost.is_active ? 'bg-green-500' : 'bg-slate-500'}>
      ...
      <Dialog open={showCreateDialog || showEditDialog} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>采购部提交历史采购物料成本信息</DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="flex items-center gap-2 pt-6">
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, is_standard_part: e.target.checked })}
                  className="rounded"
                />
                <Label>标准件</Label>
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={!formData.material_name || !formData.unit_cost}>
              保存
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              确定要删除物料成本 "{selectedCost?.material_name}" 吗？此操作不可恢复。
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-xs text-slate-400 mt-1">
                    当前设置为每 {reminder.reminder_interval_days || 30} 天提醒一次
                  </div>
                </div>
                <div>
                  <Label>下次提醒日期</Label>
                  <Input
                    value={reminder.next_reminder_date ? formatDate(reminder.next_reminder_date) : '-'}
                    disabled
                    className="bg-slate-800"
                  />
                </div>
              ...
              {reminder.is_due && (
                <div className="bg-amber-900/20 border border-amber-500/50 rounded-lg p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <div className="font-medium text-amber-400 mb-1">提醒已到期</div>

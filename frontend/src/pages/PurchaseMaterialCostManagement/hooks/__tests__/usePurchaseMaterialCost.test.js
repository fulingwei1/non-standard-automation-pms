...
import { usePurchaseMaterialCost } from '../usePurchaseMaterialCost';
import { materialApi, purchaseApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        materialApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), listWithCost: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), updatePrice: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) },
        purchaseApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), listSuppliers: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('usePurchaseMaterialCost Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [materialApi, purchaseApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => usePurchaseMaterialCost());

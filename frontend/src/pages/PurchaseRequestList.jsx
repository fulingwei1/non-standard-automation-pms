    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-sm font-semibold text-slate-200">{request.request_no}</h3>
      ...
      <div className="space-y-2 mb-3">
        <div className="flex items-center gap-2 text-xs text-slate-400">
          ...
          <span>总金额：¥{request.total_amount?.toLocaleString() || '0.00'}</span>
        </div>
        ...
        {request.request_reason && (
          <p className="text-xs text-slate-400 line-clamp-2">{request.request_reason}</p>
        )}
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-border/50">
        ...
        <div className="flex gap-1">
          <Button variant="ghost" size="sm" className="h-7 px-2" onClick={() => onView(request)} title="查看详情">
            <Eye className="w-3.5 h-3.5" />
          </Button>
          {request.status === 'DRAFT' && onEdit && (
            <Button variant="ghost" size="sm" className="h-7 px-2" onClick={() => onEdit(request)} title="编辑">
              <Edit3 className="w-3.5 h-3.5" />
            </Button>
          )}
          {request.status === 'DRAFT' && onDelete && (
            <Button variant="ghost" size="sm" className="h-7 px-2" onClick={() => onDelete(request)} title="删除">
              <Trash2 className="w-3.5 h-3.5" />
            </Button>
          )}
          {request.status === 'DRAFT' && onSubmit && (
            <Button variant="ghost" size="sm" className="h-7 px-2 text-blue-400" onClick={() => onSubmit(request)} title="提交">
              <Send className="w-3.5 h-3.5" />
            </Button>
          )}
          {request.status === 'SUBMITTED' && onApprove && (
            <Button variant="ghost" size="sm" className="h-7 px-2 text-emerald-400" onClick={() => {
...
export default function PurchaseRequestList() {
  ...
  const filteredRequests = useMemo(() => {
    ...
    return requests.filter(req => {
      if (searchQuery && !req.request_no?.toLowerCase().includes(searchQuery.toLowerCase())) {
        return false
      }
      if (statusFilter !== 'all' && req.status !== statusFilter) {
        return false
      }
      if (projectFilter !== 'all' && req.project_id !== parseInt(projectFilter)) {
        return false
      }
      return true
    })
  ...
  // Stats
  const stats = useMemo(() => {
    if (!Array.isArray(requests)) return { total: 0, draft: 0, submitted: 0, approved: 0, rejected: 0 }
    return {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      <div className="container mx-auto px-4 py-6 space-y-6">
        <PageHeader
          ...
          actions={
            <Button onClick={() => navigate('/purchase-requests/new')} className="bg-blue-600 hover:bg-blue-700">
            ...
            <DialogContent className="bg-slate-900 border-slate-700">
              ...
              <DialogBody>
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-slate-200">{selectedRequestForApprove.request_no}</p>
                  </div>
                  <div>
              ...
              <DialogFooter>
                <Button
                  variant="outline"
                  onClick={() => {
                    handleApprove(selectedRequestForApprove, false, approveData.note)
                  }}
                ...
                <Button
                  onClick={() => {
                    handleApprove(selectedRequestForApprove, true, approveData.note)
                  }}

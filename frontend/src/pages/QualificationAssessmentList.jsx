...
export default function QualificationAssessmentList() {
  ...
  const loadAssessments = async () => {
    ...
    try {
      ...
      if (response.data?.code === 200) {
        // 获取每个员工的评估记录
        const employeeIds = response.data.data?.items?.map(q => q.employee_id) || []
        const assessmentPromises = employeeIds.map(id => 
          qualificationApi.getAssessments(id).catch(() => ({ data: { data: { items: [] } } }))
        )
        const assessmentResults = await Promise.all(assessmentPromises)
        const allAssessments = assessmentResults.flatMap(res => 
          res.data?.data?.items || []
        )
  ...
  const handleExport = () => {
    try {
      const exportData = assessments.map(assessment => ({
        ...
        评估日期: assessment.assessed_at ? formatDate(assessment.assessed_at) : '-',
      ...
      link.setAttribute('download', `评估记录列表_${new Date().toISOString().split('T')[0]}.csv`)
  ...
  const getResultBadge = (result) => {
    ...
    const badge = badges[result] || { label: result, color: 'bg-gray-100 text-gray-800' }
    ...
    >
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Select
              ...
              onValueChange={(value) => setFilters({ ...filters, assessment_type: value })}
            >
              <SelectTrigger>
              ...
              {assessments.length === 0 ? (
                ...
                assessments.map((assessment) => (
                  <TableRow key={assessment.id}>
                    ...
                    <TableCell>
                      {assessment.assessed_at ? formatDate(assessment.assessed_at) : '-'}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => navigate(`/qualifications/employees/${assessment.employee_id}/view`)}
                      >
      ...
      {Object.keys(assessmentsByEmployee).length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {Object.entries(assessmentsByEmployee).slice(0, 4).map(([employeeId, employeeAssessments]) => (
            <Card key={employeeId}>
              <CardHeader>

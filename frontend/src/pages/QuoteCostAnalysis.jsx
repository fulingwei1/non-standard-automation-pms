...
export default function QuoteCostAnalysis() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (versionsList.length >= 2) {
        setSelectedVersions([versionsList[versionsList.length - 2], versionsList[versionsList.length - 1]])
  ...
  const structureByCategory = useMemo(() => {
    ...
    return costStructure.by_category.map(cat => ({
      ...cat,
      percentage: costStructure.total_cost > 0 
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => navigate(`/sales/quotes/${id}/cost`)}>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <Card>
                      <CardHeader className="pb-3">
                        <CardTitle className="text-sm font-medium text-slate-400">总价变化</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className={cn(
                          "text-2xl font-bold",
                          comparison.comparison?.price_change >= 0 ? "text-green-400" : "text-red-400"
                        )}>
                          {comparison.comparison?.price_change >= 0 ? '+' : ''}
                          {formatCurrency(comparison.comparison?.price_change || 0)}
                        </div>
                    ...
                    <Card>
                      <CardHeader className="pb-3">
                        <CardTitle className="text-sm font-medium text-slate-400">成本变化</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className={cn(
                          "text-2xl font-bold",
                          comparison.comparison?.cost_change >= 0 ? "text-red-400" : "text-green-400"
                        )}>
                          {comparison.comparison?.cost_change >= 0 ? '+' : ''}
                          {formatCurrency(comparison.comparison?.cost_change || 0)}
                        </div>
                        <div className="text-sm text-slate-400 mt-1">
                    ...
                    <Card>
                      <CardHeader className="pb-3">
                        <CardTitle className="text-sm font-medium text-slate-400">毛利率变化</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className={cn(
                          "text-2xl font-bold",
                          comparison.comparison?.margin_change >= 0 ? "text-green-400" : "text-red-400"
                        )}>
                    ...
                    <TableBody>
                      {versions.map((version, index) => {
                        ...
                        const marginChange = prevVersion
                          ? (version.gross_margin || 0) - (prevVersion.gross_margin || 0)
                          : 0
                        ...
                        return (
                          <TableRow key={version.id}>
                            ...
                            <TableCell>{formatCurrency(version.total_price || 0)}</TableCell>
                            <TableCell>{formatCurrency(version.cost_total || 0)}</TableCell>
                              ...
                              {index > 0 && (
                                <div className="space-y-1">
                                  <div className={cn(
                                    ...
                                    价格: {priceChange >= 0 ? '+' : ''}{formatCurrency(priceChange)}
                                  </div>
                                  <div className={cn(
                                    "text-sm",
                                    marginChange >= 0 ? "text-green-400" : "text-red-400"
                                  )}>
                                    毛利率: {marginChange >= 0 ? '+' : ''}{marginChange.toFixed(2)}%
                                  </div>
                                </div>
                              )}
                            </TableCell>
              ...
              {costStructure && structureByCategory.length > 0 ? (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <Card>
                      <CardHeader className="pb-3">
                        <CardTitle className="text-sm font-medium text-slate-400">成本分类数</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="text-2xl font-bold">
                    ...
                    <Card>
                      <CardHeader className="pb-3">
                        <CardTitle className="text-sm font-medium text-slate-400">平均占比</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="text-2xl font-bold">
                          {costStructure.by_category?.length > 0
                            ? (100 / costStructure.by_category.length).toFixed(1)
                            : 0}%
                        </div>
                      </CardContent>
                    </Card>
                        ...
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm w-16 text-right">{category.percentage}%</span>
                            </div>
                          </TableCell>
                          <TableCell>

...
import { useSalesProjectTrack } from '../useSalesProjectTrack';
import { salesProjectApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        salesProjectApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), addFollowUp: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), updateStage: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useSalesProjectTrack Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [salesProjectApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useSalesProjectTrack());

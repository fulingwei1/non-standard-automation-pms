...
export default function SalesTarget() {
  ...
  const getStatusBadge = (status) => {
    const configs = {
      ACTIVE: { label: '进行中', color: 'bg-blue-500', textColor: 'text-blue-400' },
      COMPLETED: { label: '已完成', color: 'bg-emerald-500', textColor: 'text-emerald-400' },
      CANCELLED: { label: '已取消', color: 'bg-slate-500', textColor: 'text-slate-400' },
    }
          ...
          <CardContent className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              <Select
                ...
                onValueChange={(value) => setFilters(prev => ({ ...prev, target_scope: value || '' }))}
              >
                <SelectTrigger>
              ...
              <Select
                ...
                onValueChange={(value) => setFilters(prev => ({ ...prev, target_type: value || '' }))}
              >
                <SelectTrigger>
              ...
              <Select
                ...
                onValueChange={(value) => setFilters(prev => ({ ...prev, target_period: value || '' }))}
              >
                <SelectTrigger>
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-sm text-slate-400">{target.period_value}</span>
                          </div>
                          <div className="text-sm text-slate-400 mb-2">
                            {target.target_scope === 'PERSONAL' && target.user_name && (
                              <span>负责人: {target.user_name}</span>
                            )}
                            {target.target_scope === 'DEPARTMENT' && target.department_name && (
                              <span>部门: {target.department_name}</span>
                            )}
                            {target.description && (
                              <span className="ml-4">描述: {target.description}</span>
                            )}
                          </div>
                        </div>
                        <Button
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData(prev => ({ ...prev, target_scope: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData(prev => ({ ...prev, target_period: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>

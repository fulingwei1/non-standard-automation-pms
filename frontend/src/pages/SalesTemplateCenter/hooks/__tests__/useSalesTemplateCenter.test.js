...
import { useSalesTemplateCenter } from '../useSalesTemplateCenter';
import { salesTemplateApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        salesTemplateApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), duplicate: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), getCategories: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useSalesTemplateCenter Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [salesTemplateApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useSalesTemplateCenter());

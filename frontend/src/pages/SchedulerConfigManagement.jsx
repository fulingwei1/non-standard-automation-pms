...
export default function SchedulerConfigManagement() {
  ...
  const handleSync = async (force = false) => {
    try {
      ...
      toast.error('配置同步失败: ' + (err.response?.data?.detail || err.message))
    } finally {
      setSyncing(false)
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await schedulerApi.updateConfig(editingConfig.task_id, updateData)
      ...
      toast.error('更新配置失败: ' + (err.response?.data?.detail || err.message))
    }
  }
  ...
  const formatCronConfig = (cronConfig) => {
    ...
    if (parts.length === 0) {
      // 检查是否是每小时执行
      if (Object.keys(cronConfig).length === 1 && cronConfig.minute !== undefined) {
  ...
  if (error && configs.length === 0) {
    return (
      <div className="space-y-6">
        <PageHeader title="定时服务配置管理" description="配置所有后台定时服务的执行频率和启用状态" />
        <ApiIntegrationError
          error={error}
              ...
              {editingConfig.cron_config && (
                <div className="space-y-2 p-4 bg-muted rounded-lg">
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    ...
                    示例：hour=7, minute=0 表示每天7:00执行；minute=0 表示每小时整点执行
                  </p>
                </div>
              )}

...
vi.mock('../../../../services/api', () => {
    return {
        serviceAnalyticsApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), getData: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useServiceAnalytics Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [serviceAnalyticsApi];

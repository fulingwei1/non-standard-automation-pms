...
export default function ServiceRecord() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (dateFilter.start) {
      result = result.filter(
        (record) => record.service_date >= dateFilter.start,
      );
    }
    if (dateFilter.end) {
    ...
    return result.sort((a, b) => {
      return (
        new Date(b.service_date + " " + b.service_start_time) -
        new Date(a.service_date + " " + a.service_start_time)
      );
    });
  ...
  const handleCreateRecord = async (recordData) => {
    try {
      ...
      const photosToUpload =
        recordData.photos?.filter((p) => p.isPending && p.file) || [];
      const existingPhotos =
        recordData.photos?.filter((p) => !p.isPending) || [];
      ...
      if (photosToUpload.length > 0) {
        ...
        for (const photo of photosToUpload) {
          try {
            await serviceApi.records.uploadPhoto(
              recordId,
              photo.file,
              photo.description,
            );
        ...
        >
          {loading ? (
            ...
            filteredRecords.map((record) => {
              const typeConfig =
                serviceTypeConfig[record.service_type] ||
                serviceTypeConfig["安装调试"];
              const TypeIcon = typeConfig.icon;
              const status =
                statusConfig[record.status] || statusConfig["进行中"];
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      if (uploadedPhotos.length > 0) {
        ...
        toast.success(
          `已添加 ${uploadedPhotos.length} 张照片（将在创建记录后上传）`,
        );
      }
    } catch (error) {
          ...
          <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      service_engineer_phone: e.target.value,
                    })
                  }
                  placeholder="输入工程师电话"
                  className="bg-slate-800/50 border-slate-700"
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setFormData({ ...formData, service_content: e.target.value })
                }
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setFormData({ ...formData, service_result: e.target.value })
                }
                placeholder="描述服务结果..."
                rows={3}
                className="bg-slate-800/50 border-slate-700"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, issues_found: e.target.value })
                  }
                  placeholder="记录发现的问题..."
                  rows={3}
                  className="bg-slate-800/50 border-slate-700"
...
function RecordDetailDialog({ record, onClose, onUpdate, onGenerateReport }) {
  ...
  const handleDeletePhoto = async (recordId, photoIndex) => {
    ...
    try {
      ...
      toast.error(
        "照片删除失败: " + (error.response?.data?.detail || error.message),
      );
    }
  };

  const typeConfig =
    serviceTypeConfig[record.service_type] || serviceTypeConfig["安装调试"];
  const TypeIcon = typeConfig.icon;
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-white">
                  {record.service_engineer} ({record.service_engineer_phone})
                </p>
              </div>
              <div>
                <p className="text-sm text-slate-400 mb-1">客户联系人</p>
                <p className="text-white">
                  {record.customer_contact} ({record.customer_phone})
                </p>
              </div>
            </div>

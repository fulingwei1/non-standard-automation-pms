      ...
      try {
        ...
        if (batchError.response?.status === 404) {
          ...
          if (failCount === 0) {
            ...
            toast.warn(
              `分配完成：成功 ${successCount} 个，失败 ${failCount} 个`,
            );
          }
        } else {
          throw batchError;
        ...
        actions={
          <div className="flex gap-2">
            ...
            {selectedTickets.size > 0 && (
              ...
              >
                <Download
                  className={cn("w-4 h-4", exporting && "animate-spin")}
                />
          ...
          {loading ? (
            ...
            <>
                  ...
                  <CardContent className="p-3">
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
                        checked={
                          selectedTickets.size === sortedTickets.length &&
                          sortedTickets.length > 0
                        }
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleExport(Array.from(selectedTickets))
                          }
                          disabled={exporting}
                        >
                          <Download
              ...
              {sortedTickets.map((ticket) => {
                const status =
                  statusConfig[ticket.status] || statusConfig["待分配"];
                const urgency =
                  urgencyConfig[ticket.urgency] || urgencyConfig["普通"];
                const problemType =
                  problemTypeConfig[ticket.problem_type] ||
                  problemTypeConfig["其他"];
...
function CreateTicketDialog({ onClose, onSubmit }) {
  ...
  const handleSubmit = async () => {
    ...
    // 验证电话号码格式（如果填写了）
    if (
      formData.reported_phone &&
      !/^1[3-9]\d{9}$/.test(formData.reported_phone.replace(/\s+/g, ""))
    ) {
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, reported_phone: e.target.value })
                }
                placeholder="输入报告人电话"
                className="bg-slate-800/50 border-slate-700"
              />
...
function TicketDetailDialog({ ticket, onClose, onAssign, onCloseTicket }) {
  ...
  const problemType =
    problemTypeConfig[ticket.problem_type] || problemTypeConfig["其他"];

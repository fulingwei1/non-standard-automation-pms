                ...
                {filteredAlerts.map((alert) => {
                  ...
                  return (
                    <TableRow key={alert.id} className="border-slate-700">
                      ...
                      <TableCell className="text-slate-200">{alert.material_name}</TableCell>
                      <TableCell className="text-slate-300">{alert.required_qty || 0}</TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <span className="text-slate-300">{alert.required_date ? formatDate(alert.required_date) : '-'}</span>
                          {urgent && (
                      ...
                      <TableCell>
                        <Badge className={levelConfigs[alert.alert_level]?.color || 'bg-slate-500'}>
                          {levelConfigs[alert.alert_level]?.label || alert.alert_level}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge className={statusConfigs[alert.status]?.color || 'bg-slate-500'}>
      ...
      <Dialog open={showDetailDialog} onOpenChange={setShowDetailDialog}>
        <DialogContent className="max-w-4xl bg-slate-900 border-slate-700">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="font-medium text-slate-200">{selectedAlert.project_name || '-'}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-mono text-slate-200">{selectedAlert.material_code}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-medium text-slate-200">{selectedAlert.required_qty || 0}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-400 mb-1">可用数量</div>
                    <div className={cn(
                      'text-slate-200',
                      selectedAlert.available_qty < selectedAlert.required_qty && 'text-red-400'
                    )}>
                  ...
                  <div>
                    ...
                    <div className="font-medium text-red-400">{selectedAlert.shortage_qty || 0}</div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-400 mb-1">需求日期</div>
                    <div className="text-slate-200">{selectedAlert.required_date ? formatDate(selectedAlert.required_date) : '-'}</div>
                  </div>
                  <div>
                  ...
                  {selectedAlert.resolved_at && (
                    <div>
                      ...
                      <div className="text-slate-200">{formatDate(selectedAlert.resolved_at)}</div>
                    </div>
                  )}
                </div>
          ...
          <DialogFooter>
            ...
            {selectedAlert && selectedAlert.status !== 'RESOLVED' && selectedAlert.status !== 'CLOSED' && (
              <Button onClick={() => {
            ...
            {selectedAlert && (
              <div className="space-y-4">
                <div>
                  ...
                  <div className="font-medium text-slate-200">{selectedAlert.material_name}</div>
                </div>
                <div>
                  <div className="text-sm text-slate-400 mb-1">缺料数量</div>
                  <div className="font-medium text-red-400">{selectedAlert.shortage_qty || 0}</div>
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block text-slate-400">处理状态</label>
                  <Select
                    value={handleData.status}
                    onValueChange={(val) => setHandleData({ ...handleData, status: val })}
                  >
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block text-slate-400">解决方案</label>
                  <textarea
                    ...
                    onChange={(e) => setHandleData({ ...handleData, solution: e.target.value })}
                    placeholder="填写解决方案..."
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setHandleData({ ...handleData, remark: e.target.value })}
                    placeholder="备注信息"

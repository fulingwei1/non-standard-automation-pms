...
import { useShortageAlert } from '../useShortageAlert';
import { shortageAlertApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        shortageAlertApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), acknowledge: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useShortageAlert Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [shortageAlertApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useShortageAlert());

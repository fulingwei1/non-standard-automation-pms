...
const mockSolutions = [
  {
    ...
    salesPerson: '张销售',
    tags: ['新能源', 'BMS', '老化测试'],
    description: 'BMS电池管理系统老化测试设备技术方案，支持多通道同时测试，自动数据记录分析。',
    deliverables: [
      { name: '技术方案文档', status: 'completed' },
      { name: '设备配置清单', status: 'completed' },
      { name: '报价明细', status: 'in_progress' },
    ],
    reviews: 2,
    comments: 5,
  },
  {
    id: 2,
    code: 'SOL-2026-002',
    name: '电池Pack自动化测试线技术方案',
    customer: '深圳XX科技',
    customerId: 'C001',
    version: 'V0.9',
    status: 'in_progress',
    deviceType: 'hybrid',
    deviceTypeName: '综合测试线',
    progress: 60,
    amount: 220,
    deadline: '2026-01-15',
    ...
    creator: '李工',
    opportunity: '电池Pack测试线项目',
    opportunityId: 'OP002',
    salesPerson: '张销售',
    tags: ['新能源', '电池Pack', '自动化'],
    description: '完整的电池Pack自动化测试线方案，包括ICT、FCT、老化、EOL等多工站集成。',
    deliverables: [
      { name: '技术方案文档', status: 'in_progress' },
      { name: '设备布局图', status: 'in_progress' },
      { name: '设备配置清单', status: 'pending' },
      { name: '报价明细', status: 'pending' },
    ],
    reviews: 0,
    comments: 3,
  },
  {
    ...
    creator: '李工',
    opportunity: 'EOL功能测试设备',
    opportunityId: 'OP003',
    salesPerson: '李销售',
    tags: ['消费电子', 'EOL', '功能测试'],
    description: 'EOL下线功能测试设备方案，支持多规格产品兼容测试。',
    deliverables: [
      { name: '技术方案文档', status: 'completed' },
      { name: '设备配置清单', status: 'completed' },
      { name: '报价明细', status: 'completed' },
    ],
    reviews: 3,
    comments: 8,
  },
  {
    ...
    creator: '李工',
    opportunity: '视觉检测系统项目',
    opportunityId: 'OP004',
    salesPerson: '王销售',
    tags: ['智能制造', '视觉', 'AI检测'],
    description: '基于深度学习的视觉检测系统方案，支持缺陷检测、尺寸测量等功能。',
    deliverables: [
      { name: '技术方案文档', status: 'completed' },
      { name: '算法说明', status: 'completed' },
      { name: '设备配置清单', status: 'completed' },
    ],
    reviews: 2,
    comments: 6,
  },
  {
    ...
    updatedAt: '2026-01-02',
    creator: '李工',
    opportunity: 'ICT测试设备项目',
    opportunityId: 'OP005',
    salesPerson: '张销售',
    tags: ['储能', 'ICT', 'PCBA测试'],
    description: 'ICT在线测试设备方案草稿，针对储能产品PCBA测试需求。',
    deliverables: [
      { name: '技术方案文档', status: 'in_progress' },
    ],
    reviews: 0,
  ...
  },
  {
    ...
    creator: '李工',
    opportunity: '汽车ECU烧录设备',
    opportunityId: 'OP006',
    salesPerson: '李销售',
    tags: ['汽车电子', '烧录', 'ECU'],
    description: '汽车ECU烧录设备技术方案（已归档）。',
    deliverables: [
      { name: '技术方案文档', status: 'completed' },
      { name: '设备配置清单', status: 'completed' },
    ],
...
export default function SolutionList() {
  ...
  const filteredSolutions = mockSolutions.filter((solution) => {
    ...
    const matchesDeviceType = selectedDeviceType === 'all' || solution.deviceType === selectedDeviceType
    const matchesSearch =
  ...
  const stats = {
    ...
    draft: mockSolutions.filter((s) => s.status === 'draft').length,
    inProgress: mockSolutions.filter((s) => s.status === 'in_progress').length,
    reviewing: mockSolutions.filter((s) => s.status === 'reviewing').length,
    published: mockSolutions.filter((s) => s.status === 'published').length,
  }

...
import { useSubstitutionDetail } from '../useSubstitutionDetail';
import { substitutionApi } from '../../../../services/api';

// Mock API
vi.mock('../../../../services/api', () => {
    return {
        substitutionApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), get: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), create: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), update: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), delete: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), query: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), aiMatch: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), assign: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }), approve: vi.fn().mockResolvedValue({ data: { items: [], total: 0 } }) }
    };
});

describe('useSubstitutionDetail Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [substitutionApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useSubstitutionDetail(1));

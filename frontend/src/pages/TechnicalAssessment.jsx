...
const decisionConfig = {
  RECOMMEND: { label: '推荐立项', color: 'bg-green-500', textColor: 'text-green-400' },
  CONDITIONAL: { label: '有条件立项', color: 'bg-yellow-500', textColor: 'text-yellow-400' },
  DEFER: { label: '暂缓', color: 'bg-orange-500', textColor: 'text-orange-400' },
  NOT_RECOMMEND: { label: '不建议立项', color: 'bg-red-500', textColor: 'text-red-400' },
}

const statusConfig = {
  PENDING: { label: '待评估', color: 'bg-gray-500', textColor: 'text-gray-400' },
  IN_PROGRESS: { label: '评估中', color: 'bg-blue-500', textColor: 'text-blue-400' },
  COMPLETED: { label: '已完成', color: 'bg-green-500', textColor: 'text-green-400' },
...
export default function TechnicalAssessment() {
  ...
  const loadAssessment = async () => {
    try {
      ...
      if (sourceType === 'lead') {
        const response = await technicalAssessmentApi.getLeadAssessments(parseInt(sourceId))
        assessments = response.data.items || response.data || []
      } else if (sourceType === 'opportunity') {
        const response = await technicalAssessmentApi.getOpportunityAssessments(parseInt(sourceId))
        assessments = response.data.items || response.data || []
      }
  ...
  const handleApplyAssessment = async () => {
    try {
      let response
      if (sourceType === 'lead') {
        response = await technicalAssessmentApi.applyForLead(parseInt(sourceId), {})
      } else {
        response = await technicalAssessmentApi.applyForOpportunity(parseInt(sourceId), {})
      }
  ...
  const handleExportReport = () => {
    ...
    const report = {
      ...
      评估时间: assessment.evaluated_at ? new Date(assessment.evaluated_at).toLocaleString() : '未评估',
  ...
  const dimensionScores = assessment?.dimension_scores ? JSON.parse(assessment.dimension_scores) : null
  const risks = assessment?.risks ? JSON.parse(assessment.risks) : []
  const similarCases = assessment?.similar_cases ? JSON.parse(assessment.similar_cases) : []
  const conditions = assessment?.conditions ? JSON.parse(assessment.conditions) : []
    ...
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: sourceType === 'lead' ? '线索管理' : '商机管理', path: sourceType === 'lead' ? '/leads' : '/opportunities' },
          { label: '技术评估', path: '' },
        ]}
      />
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                {!assessment && (
                  <Button onClick={handleApplyAssessment} className="bg-blue-600 hover:bg-blue-700">
                    申请技术评估
                  </Button>
                )}
                {assessment && assessment.status === 'PENDING' && (
                  <Button onClick={handleEvaluate} disabled={evaluating} className="bg-green-600 hover:bg-green-700">
                    {evaluating ? '评估中...' : '执行评估'}
                  </Button>
                )}
              </div>
            ...
            {assessment ? (
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  <Badge className={statusConfig[assessment.status]?.color || 'bg-gray-500'}>
                    {statusConfig[assessment.status]?.label || assessment.status}
                  </Badge>
                  {assessment.evaluator_name && (
                    <span className="text-gray-400">评估人: {assessment.evaluator_name}</span>
                  )}
                  {assessment.evaluated_at && (
                    <span className="text-gray-400">评估时间: {new Date(assessment.evaluated_at).toLocaleString()}</span>
                  )}
                </div>
                
                {assessment.total_score !== null && (
                  <div className="flex items-center gap-4">
                    <div className="text-3xl font-bold">{assessment.total_score}</div>
            ...
            <CardContent>
              <div className="space-y-3">
                {assessments.map((item, idx) => {
                  const itemScores = item.dimension_scores ? JSON.parse(item.dimension_scores) : null
                  return (
                    <div 
                      key={item.id} 
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          {item.total_score !== null && (
                            <span className="text-lg font-bold">{item.total_score}分</span>
                          )}
                          {item.decision && (
                            <Badge className={decisionConfig[item.decision]?.color || 'bg-gray-500'}>
                              {decisionConfig[item.decision]?.label || item.decision}
                            </Badge>
                          )}
                        </div>
                        <span className="text-xs text-gray-400">
                          {item.evaluated_at ? new Date(item.evaluated_at).toLocaleString() : '未评估'}
                        </span>
                      </div>
                      {itemScores && (
                      ...
                      <div className="space-y-6">
                            ...
                            return (
                              <div key={dimension} className="space-y-1">
                                <div className="flex items-center justify-between text-sm">
                                  <span className="text-gray-300">{dimensionNames[dimension] || dimension}</span>
                        ...
                        {assessment.decision && (
                          <div className="mt-6 p-4 bg-gray-700 rounded-lg">
                            ...
                            <Badge className={decisionConfig[assessment.decision]?.color || 'bg-gray-500'}>
                              ...
                              {Object.keys(dimensionLabels).map((dim) => {
                                ...
                                return (
                                  <div key={dim} className="space-y-2">
                                    <div className="text-sm text-gray-300">{dimensionNames[dim]}</div>
                                  ...
                                  <div className="flex-1">
                                    <div className="flex items-center gap-2">
                                      <div 
                                        ...
                                        style={{ width: `${(a.total_score / 100) * 100}%` }}
                                      />
                                      <span className="text-sm font-semibold w-12 text-right">{a.total_score}分</span>
                                    </div>
                                  </div>
                                  <div className="text-xs text-gray-400 w-32">
                                    {a.evaluated_at ? new Date(a.evaluated_at).toLocaleDateString() : '未评估'}
                                  </div>
                                </div>
                              ))}
                          </div>
                        ...
                        {similarCases.map((case_, idx) => (
                          <div key={idx} className="p-4 bg-gray-700 rounded-lg">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <Badge>相似度: {(case_.similarity_score * 100).toFixed(0)}%</Badge>
                            </div>
                            <div className="text-sm text-gray-300 mb-2">{case_.core_failure_reason}</div>
        ...
        {assessment && assessment.status === 'PENDING' && (
          <Card className="bg-gray-800 border-gray-700">
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-2">需求数据 (JSON格式)</label>
                  <textarea
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="enableAI" className="text-sm">启用AI分析（需要配置API密钥）</label>
                </div>
              </div>
            </CardContent>
          </Card>

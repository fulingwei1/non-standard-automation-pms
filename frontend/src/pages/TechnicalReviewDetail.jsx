...
export default function TechnicalReviewDetail() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <PageHeader
        title={isNew ? '创建技术评审' : `技术评审 - ${review?.review_name || ''}`}
        description={isNew ? '创建新的技术评审' : `评审编号: ${review?.review_no || ''}`}
        action={
      ...
      <div className="container mx-auto px-4 py-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="bg-slate-900/50 border-slate-800">
            ...
            <TabsTrigger value="materials">材料 ({materials.length})</TabsTrigger>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({ ...formData, review_name: e.target.value })}
                      placeholder="请输入评审名称"
                      className="bg-slate-800/50 border-slate-700"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setFormData({ ...formData, recorder_id: value })}
                      className="bg-slate-800/50 border-slate-700"
                    >
                  ...
                  <div className="space-y-2">
                    {participants.map((p) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-slate-200">
                              {users.find((u) => u.id === p.user_id)?.real_name ||
                        ...
                        <Badge className={getStatusBadge(p.attendance || 'PENDING').color}>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-400">
                              {m.material_type} | {(m.file_size / 1024 / 1024).toFixed(2)}MB{' '}
                              {m.is_required && '(必需)'}
                            </p>
                          </div>
                        </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-slate-400">{i.issue_no}</span>
                          </div>
                        ...
                        <div className="flex items-center gap-4 text-sm text-slate-400">
                          ...
                          <span>
                            责任人:{' '}
                            {users.find((u) => u.id === i.assignee_id)?.real_name ||
                              users.find((u) => u.id === i.assignee_id)?.username ||
                              `用户${i.assignee_id}`}
                          </span>
                          <span>期限: {formatDate(i.deadline, 'YYYY-MM-DD')}</span>
                        </div>
                        {i.solution && (
                          <p className="text-sm text-slate-300">解决方案: {i.solution}</p>
                        )}
                      </div>
                    ))}
                  </div>

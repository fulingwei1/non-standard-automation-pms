...
import { useWorkerWorkstation } from '../useWorkerWorkstation';
...
vi.mock('../../../../services/api', () => {
    return {
        ...
        workerApi: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), query: vi.fn(), aiMatch: vi.fn(), assign: vi.fn() }
    };
});

describe('useWorkerWorkstation Hook', () => {
  ...
  beforeEach(() => {
    ...
    const apiObjects = [taskApi, workerApi];
  ...
  it('should load data', async () => {
    const { result } = renderHook(() => useWorkerWorkstation());

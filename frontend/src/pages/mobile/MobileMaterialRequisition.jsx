...
export default function MobileMaterialRequisition() {
  ...
  const [items, setItems] = useState([{ material_id: '', material_code: '', material_name: '', qty: 1, unit: '' }])
  ...
  const handleAddItem = () => {
    setItems([...items, { material_id: '', material_code: '', material_name: '', qty: 1, unit: '' }])
  }
  ...
  const handleSubmit = async () => {
    // 验证
    const invalidItems = items.filter(item => !item.material_id || !item.qty || item.qty <= 0)
    ...
    try {
      ...
      setError('领料申请失败: ' + (error.response?.data?.detail || error.message))
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="p-4 space-y-4">
        ...
        {success && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              <div className="text-sm font-medium text-emerald-800">领料申请成功！</div>
            </div>
          </div>
        )}
            ...
            <div className="space-y-4">
              ...
              {items.map((item, index) => (
                <div key={index} className="p-4 border rounded-lg space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">物料 {index + 1}</span>
                    {items.length > 1 && (
                      <Button
                        variant="ghost"
                        size="sm"
                  ...
                  {item.material_id && (
                    <div className="bg-slate-50 rounded-lg p-3 space-y-1">
                      ...
                      <div className="font-mono text-sm">{item.material_code}</div>

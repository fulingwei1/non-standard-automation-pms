...
export default function MobileWorkerTaskList() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      // 筛选已派工的任务
      const myTasks = allOrders.filter(order => 
      ...
      if (filterStatus) {
        const filtered = myTasks.filter(order => order.status === filterStatus)
        setTasks(filtered)
      } else {
        setTasks(myTasks)
      }
  ...
  const stats = {
    ...
    inProgress: tasks.filter(t => t.status === 'STARTED' || t.status === 'IN_PROGRESS').length,
          ...
          tasks.map((task) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-mono text-xs text-slate-500">{task.work_order_no}</span>
                  </div>
                  <h3 className="font-medium text-base mb-1">{task.task_name}</h3>
              ...
              <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                ...
                {(task.status === 'STARTED' || task.status === 'IN_PROGRESS' || task.status === 'PAUSED') && (
                  <>
                    <Button
                      size="sm"
                      variant="outline"
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        navigate(`/mobile/progress-report?workOrderId=${task.id}`)
                      }}
                    >
                      报进度
                    </Button>
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        navigate(`/mobile/complete-report?workOrderId=${task.id}`)
                      }}
                    >
                      完工
                    </Button>

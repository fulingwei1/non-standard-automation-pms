...
import { ArrowDownToLine, ArrowLeft, CheckCircle2, XCircle, Play } from "lucide-react";
...
const STATUS_MAP = {
  ...
  PENDING: { label: "待入库", color: "bg-yellow-500", next: "RECEIVING", nextLabel: "开始入库" },
  RECEIVING: { label: "入库中", color: "bg-blue-500", next: "COMPLETED", nextLabel: "完成入库" },
...
export default function InboundDetail() {
  ...
  const fetchOrder = async () => {
    try {
      setLoading(true);
      const res = await warehouseApi.inbound.get(id);
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      await warehouseApi.inbound.updateStatus(id, newStatus);
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        <Card>
          ...
          <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div><span className="text-text-muted">已收数量:</span> {order.received_quantity}</div>

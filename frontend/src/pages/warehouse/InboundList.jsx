...
export default function InboundList() {
  ...
  return (
    <div className="min-h-screen bg-surface-50">
      <PageHeader
        title="入库管理"
        subtitle={`共 ${data.total} 条入库记录`}
        icon={<ArrowDownToLine className="h-6 w-6" />}
        actions={
          <Button onClick={() => navigate("/warehouse/inbound/new")} className="gap-2">
            <Plus className="h-4 w-4" /> 新建入库单
          </Button>
        }
      />
              ...
              {loading ? (
                ...
                <TableRow key={row.id} className="cursor-pointer hover:bg-surface-300/50" onClick={() => navigate(`/warehouse/inbound/${row.id}`)}>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); navigate(`/warehouse/inbound/${row.id}`); }}>

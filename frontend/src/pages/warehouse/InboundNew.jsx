...
export default function InboundNew() {
  ...
  const handleSubmit = async () => {
    const validItems = items.filter((i) => i.material_code && i.planned_quantity > 0);
    if (validItems.length === 0) {
      toast?.error?.("请至少添加一个物料明细") || alert("请至少添加一个物料明细");
      return;
    }
    setSubmitting(true);
    try {
      const payload = {
        ...
        items: validItems.map((i) => ({ ...i, planned_quantity: parseFloat(i.planned_quantity) })),
      };
      await warehouseApi.inbound.create(payload);
      toast?.success?.("入库单创建成功") || alert("创建成功");
      navigate("/warehouse/inbound");
    } catch (e) {
      console.error(e);
      toast?.error?.("创建失败") || alert("创建失败");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <Label>来源单号</Label>
              <Input value={form.source_no} onChange={(e) => setForm({ ...form, source_no: e.target.value })} placeholder="采购单号等" />
            </div>
            <div className="space-y-2">
              <Label>供应商</Label>
              <Input value={form.supplier_name} onChange={(e) => setForm({ ...form, supplier_name: e.target.value })} placeholder="供应商名称" />
            </div>
            <div className="space-y-2">
              <Label>备注</Label>
          ...
          <CardContent>
            <div className="space-y-3">
              <div className="grid grid-cols-12 gap-2 text-xs text-text-muted font-medium px-1">
                <div className="col-span-2">物料编码*</div>
                <div className="col-span-3">物料名称</div>
                <div className="col-span-2">规格型号</div>
                <div className="col-span-1">单位</div>
                <div className="col-span-2">计划数量*</div>
                <div className="col-span-1">备注</div>
                <div className="col-span-1"></div>
              </div>
              {items.map((item, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                  <Input className="col-span-2" value={item.material_code} onChange={(e) => updateItem(idx, "material_code", e.target.value)} placeholder="物料编码" />
                  <Input className="col-span-3" value={item.material_name} onChange={(e) => updateItem(idx, "material_name", e.target.value)} placeholder="物料名称" />
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={handleSubmit} disabled={submitting} className="gap-2">
            <Save className="h-4 w-4" /> {submitting ? "提交中..." : "创建入库单"}
          </Button>
        </div>
      </main>
    </div>

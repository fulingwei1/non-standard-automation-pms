...
import { Package, Search, RefreshCw, AlertTriangle, Download } from "lucide-react";
...
export default function InventoryList() {
  ...
  const getStockStatus = (row) => {
    if (row.min_stock > 0 && row.available_quantity <= row.min_stock) {
      return { label: "低库存", color: "text-red-400 bg-red-400/10" };
    }
    if (row.max_stock > 0 && row.quantity > row.max_stock) {
      return { label: "超储", color: "text-yellow-400 bg-yellow-400/10" };
    }
      ...
      <main className="container mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-surface-200 rounded-xl border border-white/5 overflow-hidden">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={10} className="text-center py-8 text-text-muted">加载中...</TableCell></TableRow>
              ) : data.items.length === 0 ? (
                <TableRow><TableCell colSpan={10} className="text-center py-8 text-text-muted">暂无数据</TableCell></TableRow>
              ) : data.items.map((row) => {

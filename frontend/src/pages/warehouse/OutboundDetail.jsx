...
export default function OutboundDetail() {
  ...
  const fetch = async () => { try { setLoading(true); const r = await warehouseApi.outbound.get(id); setOrder(r.data || r); } catch(e){} finally { setLoading(false); } };
  useEffect(() => { fetch(); }, [id]);

  const changeStatus = async (s) => { try { await warehouseApi.outbound.updateStatus(id, s); toast?.success?.("更新成功"); fetch(); } catch(e) { toast?.error?.("失败"); } };

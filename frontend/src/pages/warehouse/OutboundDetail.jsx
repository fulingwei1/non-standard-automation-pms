...
const STATUS_MAP = {
  ...
  PENDING: { label: "待出库", next: "PICKING", nextLabel: "开始拣货" },
  PICKING: { label: "拣货中", next: "COMPLETED", nextLabel: "完成出库" },
...
export default function OutboundDetail() {
  ...
  const fetch = async () => { try { setLoading(true); const r = await warehouseApi.outbound.get(id); setOrder(r.data || r); } catch (_e) { /* ignore */ } finally { setLoading(false); } };
  useEffect(() => { fetch(); }, [id]);

  const changeStatus = async (s) => { try { await warehouseApi.outbound.updateStatus(id, s); toast?.success?.("更新成功"); fetch(); } catch (_e) { toast?.error?.("失败"); } };
  ...
  return (
    <div className="min-h-screen bg-surface-50">
      <PageHeader title={`出库单 ${order.order_no}`} subtitle={<>{si.label}{order.is_urgent && <AlertTriangle className="inline h-4 w-4 text-red-400 ml-1" />}</>}
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        <Card><CardHeader><CardTitle>基本信息</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div><span className="text-text-muted">部门:</span> {order.department || "-"}</div>
            ...
            <div><span className="text-text-muted">已拣:</span> {order.picked_quantity}</div>
        ...
        <Card><CardHeader><CardTitle>物料明细 ({order.items?.length || 0})</CardTitle></CardHeader>
          <CardContent><Table><TableHeader><TableRow>
            ...
            <TableHead>单位</TableHead><TableHead>计划数量</TableHead><TableHead>实拣数量</TableHead><TableHead>备注</TableHead>
            ...
            {(order.items || []).map((item, idx) => (
              <TableRow key={item.id}><TableCell>{idx + 1}</TableCell><TableCell className="font-mono text-sm">{item.material_code}</TableCell>
                ...
                <TableCell>{item.picked_quantity}</TableCell><TableCell>{item.remark || "-"}</TableCell></TableRow>
            ))}

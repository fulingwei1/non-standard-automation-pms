...
const STATUS_MAP = {
  ...
  PENDING: { label: "待出库", color: "bg-yellow-500" },
  PICKING: { label: "拣货中", color: "bg-blue-500" },
...
export default function OutboundList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch outbound orders:", e);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-surface-50">
      <PageHeader
        title="出库管理"
        subtitle={`共 ${data.total} 条出库记录`}
        icon={<ArrowUpFromLine className="h-6 w-6" />}
        actions={
          <Button onClick={() => navigate("/warehouse/outbound/new")} className="gap-2">
            <Plus className="h-4 w-4" /> 新建出库单
          </Button>
        }
      />
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>领料部门</TableHead>
                <TableHead>目标单号</TableHead>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={8} className="text-center py-8 text-text-muted">加载中...</TableCell></TableRow>
              ) : data.items.length === 0 ? (
                <TableRow><TableCell colSpan={8} className="text-center py-8 text-text-muted">暂无数据</TableCell></TableRow>
              ) : data.items.map((row) => (
                <TableRow key={row.id} className="cursor-pointer hover:bg-surface-300/50" onClick={() => navigate(`/warehouse/outbound/${row.id}`)}>
                  <TableCell className="font-mono text-sm">
                    {row.order_no}
                    {row.is_urgent && <AlertTriangle className="inline ml-1 h-3 w-3 text-red-400" />}
                  </TableCell>
                  <TableCell>{TYPE_MAP[row.order_type] || row.order_type}</TableCell>
                  <TableCell>{row.department || "-"}</TableCell>
                  <TableCell>{row.target_no || "-"}</TableCell>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); navigate(`/warehouse/outbound/${row.id}`); }}>

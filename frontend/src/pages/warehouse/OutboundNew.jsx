...
export default function OutboundNew() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await warehouseApi.outbound.create(payload);
      toast?.success?.("出库单创建成功") || alert("创建成功");
      navigate("/warehouse/outbound");
    } catch (e) {
      console.error(e);
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <Label>目标单号</Label>
              <Input value={form.target_no} onChange={(e) => setForm({ ...form, target_no: e.target.value })} placeholder="工单号等" />
            </div>
            <div className="space-y-2">
              <Label>领料部门</Label>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={handleSubmit} disabled={submitting} className="gap-2">
            <Save className="h-4 w-4" /> {submitting ? "提交中..." : "创建出库单"}
          </Button>
        </div>
      </main>
    </div>

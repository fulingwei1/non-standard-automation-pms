...
export default function StockAlerts() {
  ...
  const fetchSummary = async () => {
    try {
      const res = await warehouseApi.alerts.summary();
  ...
  return (
    <div className="min-h-screen bg-surface-50">
      <PageHeader
        title="库存预警"
        subtitle={`共 ${summary.total_alerts} 条预警`}
          ...
          <Card>
            <CardContent className="pt-6 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-text-muted">低库存预警</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6 flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-bold">{summary.overstock_count}</p>
                <p className="text-sm text-text-muted">超储预警</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-text-muted">总预警数</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex gap-3 items-center">
          ...
          <Select value={alertType} onValueChange={(v) => { setAlertType(v); setPage(1); }}>
            ...
            <SelectContent>
              ...
              <SelectItem value="OVERSTOCK">超储</SelectItem>
            </SelectContent>
          </Select>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">缺口/超储</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={8} className="text-center py-8 text-text-muted">暂无预警 🎉</TableCell></TableRow>
              ) : data.items.map((row, idx) => (
                <TableRow key={idx}>
                  <TableCell>
                    <Badge variant="outline" className={row.alert_type === "LOW" ? "text-red-400 bg-red-400/10" : "text-yellow-400 bg-yellow-400/10"}>
                      {row.alert_type === "LOW" ? "低库存" : "超储"}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className={`text-right font-medium ${row.alert_type === "LOW" ? "text-red-400" : "text-yellow-400"}`}>
                    {row.shortage}
                  </TableCell>
                </TableRow>
              ))}

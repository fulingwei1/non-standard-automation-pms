...
import { ClipboardCheck, Plus, Search, Eye, RefreshCw, Play } from "lucide-react";
...
const STATUS_MAP = {
  ...
  IN_PROGRESS: { label: "盘点中", color: "bg-blue-500" },
...
export default function StockCount() {
  ...
  const handleCreate = async () => {
    setSubmitting(true);
    try {
      await warehouseApi.stockCount.create({
        ...createForm,
        warehouse_id: createForm.warehouse_id ? parseInt(createForm.warehouse_id) : null,
        planned_date: createForm.planned_date || null,
      });
      toast?.success?.("盘点单创建成功");
      setShowCreate(false);
      setCreateForm({ warehouse_id: null, count_type: "FULL", planned_date: "", remark: "" });
      fetchData();
    } catch (e) {
      toast?.error?.("创建失败");
    } finally {
  ...
  const viewDetail = async (order) => {
    try {
      const res = await warehouseApi.stockCount.get(order.id);
  ...
  const handleStatusChange = async (orderId, status) => {
    try {
      await warehouseApi.stockCount.updateStatus(orderId, status);
      toast?.success?.("状态更新成功");
      fetchData();
      if (showDetail) {
        const res = await warehouseApi.stockCount.get(orderId);
        setSelectedOrder(res.data || res);
      }
    } catch (e) {
      toast?.error?.("操作失败");
  ...
  return (
    <div className="min-h-screen bg-surface-50">
      <PageHeader
        title="盘点管理"
        subtitle={`共 ${data.total} 条盘点记录`}
        icon={<ClipboardCheck className="h-6 w-6" />}
        actions={
          <Button onClick={() => setShowCreate(true)} className="gap-2">
            <Plus className="h-4 w-4" /> 新建盘点单
          </Button>
        }
      />
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>盘点项数</TableHead>
                <TableHead>一致/差异</TableHead>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={7} className="text-center py-8 text-text-muted">加载中...</TableCell></TableRow>
              ) : data.items.length === 0 ? (
                <TableRow><TableCell colSpan={7} className="text-center py-8 text-text-muted">暂无盘点记录</TableCell></TableRow>
              ) : data.items.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>
                    <span className="text-green-400">{row.matched_items}</span>
                    {" / "}
                    <span className="text-red-400">{row.diff_items}</span>
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline" className={`${STATUS_MAP[row.status]?.color || ""} text-white text-xs`}>
                      {STATUS_MAP[row.status]?.label || row.status}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right space-x-1">
                    <Button variant="ghost" size="sm" onClick={() => viewDetail(row)}>
                      <Eye className="h-4 w-4" />
                    </Button>
                    {row.status === "DRAFT" && (
                      <Button variant="ghost" size="sm" onClick={() => handleStatusChange(row.id, "IN_PROGRESS")}>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={createForm.count_type} onValueChange={(v) => setCreateForm({ ...createForm, count_type: v })}>
                ...
                <SelectContent>
                  {COUNT_TYPES.map((t) => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label>计划日期</Label>
              <Input type="date" value={createForm.planned_date} onChange={(e) => setCreateForm({ ...createForm, planned_date: e.target.value })} />
            </div>
            <div className="space-y-2">
              <Label>备注</Label>
              <Input value={createForm.remark} onChange={(e) => setCreateForm({ ...createForm, remark: e.target.value })} placeholder="备注" />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={showDetail} onOpenChange={setShowDetail}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>盘点单详情 - {selectedOrder?.count_no}</DialogTitle></DialogHeader>
          {selectedOrder && (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4 text-sm">
                ...
                <div><span className="text-text-muted">盘点项:</span> {selectedOrder.total_items} (一致 {selectedOrder.matched_items} / 差异 {selectedOrder.diff_items})</div>
              </div>
                  ...
                  <TableBody>
                    {selectedOrder.items.map((item) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className={`text-right ${item.diff_quantity && item.diff_quantity !== 0 ? "text-red-400 font-medium" : ""}`}>
                          {item.diff_quantity ?? "-"}
                        </TableCell>
                        <TableCell className="text-text-muted text-sm">{item.diff_reason || "-"}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          <DialogFooter>
            {selectedOrder?.status === "IN_PROGRESS" && (
              <Button onClick={() => handleStatusChange(selectedOrder.id, "COMPLETED")}>完成盘点</Button>
            )}

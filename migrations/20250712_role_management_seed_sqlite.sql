...
-- PM 归属于 CTO
UPDATE roles 
SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'CTO')
WHERE role_code = 'PM';

-- 工程师归属于 PM
UPDATE roles 
SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'PM')
WHERE role_code IN ('ME', 'EE', 'SW', 'DEBUG');

-- 质量归属于 QA_MGR
UPDATE roles 
SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'QA_MGR')
WHERE role_code = 'QA';

-- 采购归属于 PU_MGR
UPDATE roles 
SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'PU_MGR')
WHERE role_code = 'PU';

-- 销售归属于 SALES_DIR
UPDATE roles 
SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'SALES_DIR')
WHERE role_code = 'SA';

-- 财务归属于 CFO
UPDATE roles 
SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'CFO')
WHERE role_code = 'FI';
...
-- 采购专员与财务专员互斥
INSERT OR REPLACE INTO role_exclusions (role_id_a, role_id_b, exclusion_type, reason, is_active)

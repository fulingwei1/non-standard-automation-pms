...
-- PM 归属于 CTO
UPDATE roles SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'CTO')
WHERE role_code = 'PM';

-- 工程师归属于 PM
UPDATE roles SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'PM')
WHERE role_code IN ('ME', 'EE', 'SW', 'DEBUG');

-- 质量归属于 QA_MGR
UPDATE roles SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'QA_MGR')
WHERE role_code = 'QA';

-- 采购归属于 PU_MGR
UPDATE roles SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'PU_MGR')
WHERE role_code = 'PU';

-- 销售归属于 SALES_DIR
UPDATE roles SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'SALES_DIR')
WHERE role_code = 'SA';

-- 财务归属于 CFO
UPDATE roles SET parent_role_id = (SELECT id FROM roles WHERE role_code = 'CFO')
WHERE role_code = 'FI';
...
SELECT
    a.id, b.id, 'MUTUAL',
    '职责分离：采购专员与财务总监不得兼任', 1
FROM roles a, roles b
WHERE a.role_code = 'PU' AND b.role_code = 'CFO';

-- 注意：同项目内的QA与PM互斥需要在业务逻辑中按项目维度判断
-- 这里仅记录规则说明
INSERT INTO role_exclusions (role_id_a, role_id_b, exclusion_type, reason, is_active)
SELECT
    a.id, b.id, 'MUTUAL',
    '验收独立性：同一项目内质量主管与项目经理不得兼任（项目维度）', 1
FROM roles a, roles b
...
-- 根部门
INSERT OR REPLACE INTO departments (dept_code, dept_name, parent_id, level, sort_order, is_active) VALUES
('ROOT', '公司', NULL, 0, 0, 1);

-- 一级部门
INSERT OR REPLACE INTO departments (dept_code, dept_name, parent_id, level, sort_order, is_active) VALUES
('TECH', '技术中心', (SELECT id FROM departments WHERE dept_code = 'ROOT'), 1, 1, 1);

INSERT OR REPLACE INTO departments (dept_code, dept_name, parent_id, level, sort_order, is_active) VALUES
('SALES', '销售部', (SELECT id FROM departments WHERE dept_code = 'ROOT'), 1, 2, 1);

INSERT OR REPLACE INTO departments (dept_code, dept_name, parent_id, level, sort_order, is_active) VALUES
('FINANCE', '财务部', (SELECT id FROM departments WHERE dept_code = 'ROOT'), 1, 3, 1);

INSERT OR REPLACE INTO departments (dept_code, dept_name, parent_id, level, sort_order, is_active) VALUES
('PURCHASE', '采购部', (SELECT id FROM departments WHERE dept_code = 'ROOT'), 1, 4, 1);

INSERT OR REPLACE INTO departments (dept_code, dept_name, parent_id, level, sort_order, is_active) VALUES
('QA', '质量部', (SELECT id FROM departments WHERE dept_code = 'ROOT'), 1, 5, 1);

INSERT OR REPLACE INTO departments (dept_code, dept_name, parent_id, level, sort_order, is_active) VALUES
('PMC', '计划部', (SELECT id FROM departments WHERE dept_code = 'ROOT'), 1, 6, 1);

-- 技术中心下属部门
INSERT OR REPLACE INTO departments (dept_code, dept_name, parent_id, level, sort_order, is_active)
...
-- 机械部默认角色：机械工程师
INSERT OR REPLACE INTO department_default_roles (department_id, role_id, is_primary)
SELECT d.id, r.id, 1

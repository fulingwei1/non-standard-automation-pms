...
-- 先尝试使用新表结构（permission_code），如果失败则使用旧表结构（perm_code）
-- 新表结构版本
INSERT IGNORE INTO permissions (permission_code, permission_name, module, resource, action, description, is_active) VALUES
...
-- 如果使用旧表结构（perm_code），也插入（兼容性）
-- 注意：如果表使用 perm_code，请取消下面的注释并执行
INSERT IGNORE INTO permissions (perm_code, perm_name, module, action)
SELECT permission_code, permission_name, module, action
FROM permissions
WHERE module = 'ecn' AND permission_code IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM permissions WHERE perm_code = permission_code)
LIMIT 0;  -- 先不执行，如果新表结构失败，手动执行下面的语句

-- 旧表结构版本（如果新表结构不存在，取消注释执行）
/*
INSERT IGNORE INTO permissions (perm_code, perm_name, module, action) VALUES
...
WHERE r.role_code = 'ADMIN' 
  AND p.module = 'ecn'
  AND ((p.permission_code IS NOT NULL AND p.permission_code LIKE 'ecn:%')
       OR (p.perm_code IS NOT NULL AND p.perm_code LIKE 'ecn:%'));

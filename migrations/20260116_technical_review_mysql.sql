...
CREATE TABLE IF NOT EXISTS technical_reviews (
    ...
    review_no           VARCHAR(50) NOT NULL UNIQUE COMMENT '评审编号：RV-PDR-202501-0001',
    review_type         VARCHAR(20) NOT NULL COMMENT '评审类型：PDR/DDR/PRR/FRR/ARR',
    review_name         VARCHAR(200) NOT NULL COMMENT '评审名称',
    
    -- 关联项目
    project_id          BIGINT NOT NULL COMMENT '关联项目ID',
    project_no          VARCHAR(50) NOT NULL COMMENT '项目编号',
    equipment_id        BIGINT COMMENT '关联设备ID（多设备项目）',
    
    -- 评审基本信息
    status              VARCHAR(20) NOT NULL DEFAULT 'DRAFT' COMMENT '状态：draft/pending/in_progress/completed/cancelled',
    scheduled_date      DATETIME NOT NULL COMMENT '计划评审时间',
    actual_date         DATETIME COMMENT '实际评审时间',
    location            VARCHAR(200) COMMENT '评审地点',
    meeting_type        VARCHAR(20) NOT NULL DEFAULT 'ONSITE' COMMENT '会议形式：onsite/online/hybrid',
    
    -- 评审人员
    host_id             BIGINT NOT NULL COMMENT '主持人ID',
    presenter_id        BIGINT NOT NULL COMMENT '汇报人ID',
    recorder_id         BIGINT NOT NULL COMMENT '记录人ID',
    
    -- 评审结论
    conclusion          VARCHAR(30) COMMENT '评审结论：pass/pass_with_condition/reject/abort',
    conclusion_summary  TEXT COMMENT '结论说明',
    condition_deadline  DATE COMMENT '有条件通过的整改期限',
    next_review_date    DATE COMMENT '下次复审日期',
    
    -- 问题统计
    issue_count_a       INT DEFAULT 0 COMMENT 'A类问题数',
    issue_count_b       INT DEFAULT 0 COMMENT 'B类问题数',
    issue_count_c       INT DEFAULT 0 COMMENT 'C类问题数',
    issue_count_d       INT DEFAULT 0 COMMENT 'D类问题数',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='技术评审主表';
...
CREATE TABLE IF NOT EXISTS review_participants (
    ...
    role                VARCHAR(20) NOT NULL COMMENT '角色：host/expert/presenter/recorder/observer',
    is_required         TINYINT(1) NOT NULL DEFAULT 1 COMMENT '是否必须参与',
    
    -- 出席信息
    attendance          VARCHAR(20) COMMENT '出席状态：pending/confirmed/absent/delegated',
    delegate_id         BIGINT COMMENT '代理人ID（请假时）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='评审参与人表';
...
CREATE TABLE IF NOT EXISTS review_materials (
    id                  BIGINT PRIMARY KEY AUTO_INCREMENT,
    review_id           BIGINT NOT NULL COMMENT '评审ID',
    material_type       VARCHAR(20) NOT NULL COMMENT '材料类型：drawing/bom/report/document/other',
    material_name       VARCHAR(200) NOT NULL COMMENT '材料名称',
    file_path           VARCHAR(500) NOT NULL COMMENT '文件路径',
    file_size           BIGINT NOT NULL COMMENT '文件大小（字节）',
    version             VARCHAR(20) COMMENT '版本号',
    is_required         TINYINT(1) NOT NULL DEFAULT 1 COMMENT '是否必须材料',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='评审材料表';
...
CREATE TABLE IF NOT EXISTS review_checklist_records (
    id                  BIGINT PRIMARY KEY AUTO_INCREMENT,
    review_id           BIGINT NOT NULL COMMENT '评审ID',
    checklist_item_id  BIGINT COMMENT '检查项ID（关联模板，可为空）',
    category            VARCHAR(50) NOT NULL COMMENT '检查类别',
    check_item          VARCHAR(500) NOT NULL COMMENT '检查项内容',
    result              VARCHAR(20) NOT NULL COMMENT '检查结果：pass/fail/na',
    
    -- 问题信息（不通过时）
    issue_level         VARCHAR(10) COMMENT '问题等级：A/B/C/D（不通过时）',
    issue_desc          TEXT COMMENT '问题描述',
    issue_id            BIGINT COMMENT '关联问题ID（自动创建的问题）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='评审检查项记录表';
...
CREATE TABLE IF NOT EXISTS review_issues (
    id                  BIGINT PRIMARY KEY AUTO_INCREMENT,
    review_id           BIGINT NOT NULL COMMENT '评审ID',
    issue_no            VARCHAR(50) NOT NULL UNIQUE COMMENT '问题编号：RV-ISSUE-202501-0001',
    issue_level         VARCHAR(10) NOT NULL COMMENT '问题等级：A/B/C/D',
    category            VARCHAR(50) NOT NULL COMMENT '问题类别',
    ...
    -- 状态与处理
    status              VARCHAR(20) NOT NULL DEFAULT 'OPEN' COMMENT '状态：open/processing/resolved/verified/closed',
    ...
    -- 关联
    linked_issue_id     BIGINT COMMENT '关联问题管理系统ID',

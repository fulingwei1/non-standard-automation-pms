...
-- 示例转换SQL（根据实际情况调整）：
-- UPDATE employees 
-- SET employee_code = 'EMP-' || SUBSTR(employee_code, -5)
...
-- 员工编码唯一性（如果不存在）
-- CREATE UNIQUE INDEX IF NOT EXISTS idx_employees_code_unique ON employees(employee_code);

-- 客户编码唯一性（如果不存在）
-- CREATE UNIQUE INDEX IF NOT EXISTS idx_customers_code_unique ON customers(customer_code);

-- 物料编码唯一性（如果不存在）
-- CREATE UNIQUE INDEX IF NOT EXISTS idx_materials_code_unique ON materials(material_code);

...
# New batch2 test files (comprehensive tests)
NEW_TESTS=(
...
for f in "${ALL_TESTS[@]}"; do
    ...
    if [ $RC -eq 0 ]; then
        PASSED=$((PASSED + 1))
        echo "PASS ($i/$TOTAL): $(basename $f)"
    else
        FAILED=$((FAILED + 1))
        echo "FAIL ($i/$TOTAL): $(basename $f)"
    fi
done
...
python3 -m coverage report --sort=cover > /tmp/coverage_final.txt
echo "Full report: /tmp/coverage_final.txt"

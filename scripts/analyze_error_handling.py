...
class ErrorHandlingAnalyzer:
            ...
            if "try" in line and "{" in line:
                ...
                if catch_start != -1:
                    ...
                    if "{" in catch_line:
                        ...
                        catch_inner = re.search(
                            r"catch\s*\([^)]+\)\s*\{([^}]*)\}", catch_body, re.DOTALL
                        )
                        if not catch_inner:
                            # 多行情况
                        ...
                        # 检查是否为空 catch
                        if not catch_inner_text or re.match(
                            r"^\s*(//.*)?$", catch_inner_text, re.MULTILINE
                        ):
                            stats["empty_catch"] += 1
                            stats["patterns"].append(
                                {
                                    ...
                                    "type": "empty_catch",
                                    "code": "\n".join(
                                        lines[try_start : min(catch_end + 1, try_start + 5)]
                                    ),
                                }
                            )
                            i = catch_end + 1
                        ...
                        # 只有 console.error
                        if re.search(r"console\.(error|warn)", catch_inner_text) and not re.search(
                            r"(alert|toast|handleApiError|setError|showError)", catch_inner_text
                        ):
                        ...
                        # 没有用户反馈
                        if not re.search(
                            r"(alert|toast|handleApiError|setError|showError|message)",
                            catch_inner_text,
                            re.IGNORECASE,
                        ):
    ...
    def _analyze_backend_file(self, content: str, file_path: Path) -> Dict:
        ...
        for match in matches:
            ...
            if except_match:
                ...
                # 只有 print
                if re.search(r"print\s*\(", except_body) and not re.search(
                    r"logger\.", except_body
                ):
    ...
    def analyze(self) -> Dict:
        """分析整个代码库"""
        total_stats = {"frontend": defaultdict(int), "backend": defaultdict(int), "issues": []}
        ...
        if backend_dir.exists():
            for file_path in backend_dir.rglob("*.py"):
                stats = self._analyze_backend_file(file_path.read_text(encoding="utf-8"), file_path)
                for key, value in stats.items():
                    if isinstance(value, int):
    ...
    def generate_report(self, stats: Dict) -> str:
        ...
        report.append(
            f"- 总计: {stats['frontend']['try_blocks'] + stats['backend']['try_blocks']}\n"
        )

...
def auto_assign_roles(dry_run: bool = True, replace: bool = False, verbose: bool = False):
    ...
    with get_db_session() as session:
        ...
        for user in users:
            ...
            # 获取用户当前角色
            current_roles = session.query(UserRole).filter(
                UserRole.user_id == user.id
            ).all()
            ...
            if current_roles and not replace:
                if verbose:
                    role_names = [session.query(Role).filter(Role.id == rid).first().role_name 
            ...
            if not role_to_assign:
                ...
                try:
                    ...
                    if employee:
                        hr_profile = session.query(EmployeeHrProfile).filter(
                ...
                # 先进行职位+职级匹配（获取基础角色）
                base_role = get_role_by_position_and_level(
            ...
            if verbose:
                print(f"  ✓ 分配角色: {role_to_assign.role_name} ({role_to_assign.role_code}) - {assignment_method}")
            
            # 执行分配
            if not dry_run:
                try:
                    assigned = assign_role_to_user(
                        session, user, role_to_assign, 
                        replace=replace,

...
def check_project_data_quality(project_code: str = "PJ250114"):
    ...
    with get_db_session() as db:
        ...
        # 检查文档文件路径
        fake_paths = [doc for doc in documents if doc.file_path.startswith('/documents/') or doc.file_path.startswith('/docs/')]
        ...
        if duplicate_costs:
            ...
            for key, count in list(duplicate_costs.items())[:10]:  # 只显示前10个
                ...
                print(f"      - {cost_type}: {formatCurrency(amount)} - {desc[:30]}... ({count} 次)")
        else:
            print("   ✓ 没有重复的成本记录")
        ...
        duplicate_milestones = {name: count for name, count in milestone_name_counts.items() if count > 1}
        ...
        duplicate_issues = {title: count for title, count in issue_title_counts.items() if count > 1}
        ...
        duplicate_machines = {code: count for code, count in machine_code_counts.items() if count > 1}

    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                functions.append(
                    (str(file_path), line_count, start_line, f"{func_type} {func_name}")
                )
    except Exception:
        pass  # 忽略无法解析的文件
        ...
        for i, line in enumerate(lines, 1):
            ...
            if in_function:
                ...
                if (
                    brace_count == 0
                    and paren_count == 0
                    and (";" in line or line.strip().endswith(")"))
                ):
                    line_count = i - function_start + 1
                    functions.append(
                        (str(file_path), line_count, function_start, f"function {function_name}")
                    )
                    in_function = False
                    function_name = ""
    except Exception:

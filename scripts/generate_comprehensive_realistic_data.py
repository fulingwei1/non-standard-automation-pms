...
def generate_failure_cases(db, users):
    ...
    for case_data in failure_cases_data:
        # 检查是否已存在
        existing = (
            db.query(FailureCase).filter(FailureCase.case_code == case_data["case_code"]).first()
        )
        if existing:
        ...
        case = FailureCase(
            ...
            created_by=(
                (
                    users.get("pm")
                    or users.get("sales")
                    or (list(users.values())[0] if users else None)
                ).id
                if users
                else None
            ),
...
def generate_leads_with_assessments(db, customers, users):
    ...
    for lead_data in leads_data:
        ...
        req_detail = LeadRequirementDetail(
            ...
            target_object_type=(
                "BMS"
                if "BMS" in lead_data["demand"]
                else "主板" if "主板" in lead_data["demand"] else "汽车零部件"
            ),
            ...
            cycle_time_seconds=(
                Decimal("15")
                if "15秒" in lead_data["demand"]
                else Decimal("8") if "8秒" in lead_data["demand"] else Decimal("12")
            ),
            workstation_count=(
                8 if "8通道" in lead_data["demand"] else 2 if "双工位" in lead_data["demand"] else 1
            ),
        ...
        if lead_data["assessment_status"] == "IN_PROGRESS":
            assessment = TechnicalAssessment(
                ...
                decision="PENDING",
                risks=json.dumps(["需求可能变更", "交付周期紧张", "技术难度较高"]),
                evaluated_at=None,
            )
            db.add(assessment)
            db.flush()
...
def generate_opportunities_and_projects(db, customers, leads, users):
    ...
    for i, lead in enumerate(leads_to_convert):
        ...
        if existing_opp:
            ...
            # 检查是否有对应的项目
            existing_project = (
                db.query(Project).filter(Project.opportunity_id == existing_opp.id).first()
            )
            if existing_project:
            ...
            quote_items = [
                {
                    ...
                    "type": "EQUIPMENT",
                    "qty": 1,
                    "price": opp.est_amount * Decimal("0.6"),
                },
                {
                    "name": "测试治具",
                    "type": "FIXTURE",
                    "qty": 4,
                    "price": opp.est_amount * Decimal("0.2"),
                },
                {
                    "name": "软件系统",
                    "type": "SOFTWARE",
                    "qty": 1,
                    "price": opp.est_amount * Decimal("0.15"),
                },
                {
                    "name": "安装调试",
                    "type": "SERVICE",
                    "qty": 1,
                    "price": opp.est_amount * Decimal("0.05"),
                },
            ]
        ...
        if random.random() > 0.2:  # 80%概率签单
            ...
            project = Project(
                ...
                actual_start_date=(
                    date.today() - timedelta(days=random.randint(0, 20)) if stage != "S2" else None
                ),
            ...
            milestone_templates = [
                ...
                },
                {
                    ...
                    "days_offset": 20,
                    "status": "COMPLETED" if stage != "S2" else "PENDING",
                },
                {
                    "name": "BOM发布",
                    "type": "BOM_RELEASED",
                    "days_offset": 30,
                    "status": "COMPLETED" if stage in ["S3", "S4", "S5", "S6"] else "PENDING",
                },
                {
                    "name": "物料到齐",
                    "type": "MATERIAL_ARRIVED",
                    "days_offset": 60,
                    "status": "COMPLETED" if stage in ["S4", "S5", "S6"] else "PENDING",
                },
                {
                    "name": "机械加工完成",
                    "type": "MACHINING_COMPLETED",
                    "days_offset": 90,
                    "status": "COMPLETED" if stage in ["S5", "S6"] else "PENDING",
                },
                {
                    "name": "装配完成",
                    "type": "ASSEMBLY_COMPLETED",
                    "days_offset": 120,
                    "status": "COMPLETED" if stage == "S6" else "PENDING",
                },
                {
                    "name": "调试完成",
            ...
            print(
                f"  ✓ 商机: {opp.opp_code} → 项目: {project.project_code} (阶段: {stage}, 健康度: {health})"
            )
        else:
...
def generate_production_data(db, projects, users):
    ...
    for proc_data in processes_data:
        # 检查是否已存在
        existing = (
            db.query(ProcessDict).filter(ProcessDict.process_code == proc_data["code"]).first()
        )
        if existing:
            ...
            for j, proc in enumerate(processes[:proc_count]):  # 每个项目多个工单
                ...
                if wo.status in ["STARTED", "COMPLETED"]:
                    ...
                    start_report = WorkReport(
                        ...
                        report_type="START",
                        report_time=wo.assigned_at
                        or datetime.now() - timedelta(days=random.randint(1, 5)),
                        progress_percent=0,
                    ...
                    if wo.status == "COMPLETED":
                        complete_report = WorkReport(
                            ...
                            report_time=wo.actual_end_time
                            or datetime.now() - timedelta(days=random.randint(0, 2)),
                            progress_percent=100,
                            work_hours=wo.actual_hours,
                            completed_qty=wo.completed_qty,
                            qualified_qty=wo.qualified_qty,
                            defect_qty=(
                                max(0, wo.completed_qty - wo.qualified_qty)
                                if wo.completed_qty
                                else 0
                            ),
                            status="APPROVED",
    ...
    db.flush()
    print(
        f"  ✓ 创建 {len(workshops)} 个车间, {len(processes)} 个工序, {len(workers)} 个工人, {len(work_orders)} 个工单"
    )
...
def generate_alert_and_exception_data(db, projects, users):
    ...
    for project in projects:
        # 根据项目健康度决定预警数量
        alert_probability = (
            0.3 if project.health == "H1" else 0.6 if project.health == "H2" else 0.9
        )
    ...
    for project in projects:
        ...
        exception_probability = (
            0.2 if project.health == "H1" else 0.5 if project.health == "H2" else 0.8
        )
    ...
    print(
        f"  ✓ 创建 {len(alert_rules)} 个预警规则, {len(alert_records)} 个预警记录, {len(exceptions)} 个异常事件"
    )
...
def generate_users_if_needed(db):
    ...
    for user_template in user_templates:
        ...
        if not user:
            ...
            emp = (
                db.query(Employee)
                .filter(Employee.employee_code == f"EMP{user_template['username'][-1].upper()}")
                .first()
            )
            if not emp:
                emp = Employee(
...
def main():
    ...
    with get_db_session() as db:
        try:
            ...
            # 5. 生成商机和项目
            opportunities, projects = generate_opportunities_and_projects(
                db, customers, leads, users
            )

            # 6. 生成生产数据
            workshops, processes, workers, work_orders = generate_production_data(
                db, projects, users
            )

            # 7. 生成预警和异常数据
            alert_rules, alert_records, exceptions = generate_alert_and_exception_data(
                db, projects, users
            )

...
def import_employees():
    ...
    try:
        ...
        existing_codes = set(
            row[0] for row in cursor.execute("SELECT employee_code FROM employees")
        )
        ...
        for idx, row in df.iterrows():
            ...
            # 创建员工档案（如果不存在）
            cursor.execute(
                "SELECT id FROM hr_employee_profile WHERE employee_id = ?",
                (employee_id,),
            )

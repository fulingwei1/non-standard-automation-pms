...
class HistoricalDataImporter:
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 必需字段
        required_fields = ["presale_ticket_id", "predicted_win_rate", "actual_result"]
        ...
        try:
            for _, row in df.iterrows():
                ...
                history = PresaleWinRateHistory(
                    ...
                    prediction_id=(
                        int(row.get("prediction_id"))
                        if pd.notna(row.get("prediction_id"))
                        else None
                    ),
    ...
    def generate_sample_data(self, output_file: str, num_records: int = 100):
        ...
        for i in range(1, num_records + 1):
            ...
            record = {
                ...
                "actual_win_date": (
                    datetime.now().strftime("%Y-%m-%d") if actual_result == "won" else ""
                ),
                "actual_lost_date": (
                    datetime.now().strftime("%Y-%m-%d") if actual_result == "lost" else ""
                ),
                "updated_by": 1,
            }

...
def migrate_scoring_rules(source_db_path: str, target_db_session):
    ...
    for rule in rules:
        # 检查是否已存在
        existing = (
            target_db_session.query(ScoringRule)
            .filter(ScoringRule.version == rule["version"])
            .first()
        )
...
def migrate_failure_cases(source_db_path: str, target_db_session, user_id_map: Dict[str, int]):
    ...
    for case in cases:
        # 检查是否已存在
        existing = (
            target_db_session.query(FailureCase)
            .filter(FailureCase.case_code == case["caseCode"])
            .first()
        )

        if not existing:
            created_by = (
                user_id_map.get(case.get("createdById")) if case.get("createdById") else None
            )
...
def migrate_assessments(
    ...
    for eval_data in evaluations:
        ...
        evaluator_id = (
            user_id_map.get(eval_data.get("evaluatorId")) if eval_data.get("evaluatorId") else None
        )
        ...
        new_assessment = TechnicalAssessment(
            ...
            evaluated_at=(
                datetime.fromisoformat(eval_data["createdAt"].replace("Z", "+00:00"))
                if eval_data.get("createdAt")
                else None
            ),
        )
...
def build_project_mapping(source_db_path: str, target_db_session) -> tuple:
    ...
    for project in projects:
        ...
        if customer_name:
            # 先查找线索
            lead = (
                target_db_session.query(Lead)
                .filter(Lead.customer_name == customer_name)
            ...
            # 再查找商机
            opp = (
                target_db_session.query(Opportunity)
...
def main():
    """主函数"""
    presales_db_path = (
        project_root / "presales-project" / "presales-evaluation-system" / "prisma" / "dev.db"
    )
    ...
    try:
        ...
        # 2. 构建项目映射
        project_to_lead_map, project_to_opp_map = build_project_mapping(
            str(presales_db_path), target_db
        )
        ...
        # 5. 迁移评估结果
        migrate_assessments(
            str(presales_db_path), target_db, user_id_map, project_to_lead_map, project_to_opp_map
        )

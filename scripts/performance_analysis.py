...
def analyze_database_performance() -> Dict[str, Any]:
    ...
    try:
        ...
        slow_queries = db.execute(text("""
            ...
            LIMIT 10;
        """)).fetchall() if has_pg_stat_statements(db) else []
...
def analyze_code_complexity() -> Dict[str, Any]:
    ...
    results['complex_functions'] = sorted(complex_functions, key=lambda x: x['complexity_lines'], reverse=True)[:15]
...
def generate_performance_report() -> str:
    ...
    if 'large_tables' in db_results:
        ...
        for table in db_results['large_tables'][:10]:
            report.append(f"| {table.tablename} | {table.attname} | {table.n_distinct} | {table.correlation:.2f} |")
        report.append("")
    ...
    if 'slow_queries' in db_results:
        ...
        if db_results['slow_queries']:
            ...
            for query in db_results['slow_queries']:
                report.append(f"| `{query.query[:50]}...` | {query.mean_time:.2f} | {query.calls} | {query.total_time:.2f} |")
        else:
    ...
    for file_info in code_results['large_files'][:10]:
        report.append(f"| {file_info['file']} | {file_info['lines']} | {file_info['size_mb']:.2f} |")
    report.append("")
    ...
    for func_info in code_results['complex_functions'][:10]:
        report.append(f"| {func_info['file']} | {func_info['function']} | {func_info['line_number']} | {func_info['complexity_lines']} |")
    report.append("")

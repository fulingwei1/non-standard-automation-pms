...
def seed():
    ...
    for p in periods:
        c.execute(
            """INSERT OR REPLACE INTO performance_period
    ...
    indicators = [
        ...
        (
            ...
            20,
            json.dumps(
                {"90-100": 20, "80-89": 16, "70-79": 12, "60-69": 8, "<60": 4}, ensure_ascii=False
            ),
            20,
            0,
            json.dumps(["engineer", "manager"], ensure_ascii=False),
            None,
            1,
            1,
        ),
        (
            ...
            15,
            json.dumps(
                {"95-100": 15, "85-94": 12, "75-84": 9, "65-74": 6, "<65": 3}, ensure_ascii=False
            ),
            15,
            0,
            json.dumps(["engineer", "manager"], ensure_ascii=False),
        ...
        (
            ...
            15,
            0,
            json.dumps(["engineer"], ensure_ascii=False),
            None,
            1,
            3,
        ),
        ...
        (
            ...
            15,
            0,
            json.dumps(["manager"], ensure_ascii=False),
            None,
            1,
            6,
        ),
    ...
    for ind in indicators:
        c.execute(
            """INSERT OR REPLACE INTO performance_indicator
    ...
    for r in results:
        c.execute(
            """INSERT OR REPLACE INTO performance_result
    ...
    for e in evaluations:
        c.execute(
            """INSERT OR REPLACE INTO performance_evaluation
    ...
    # ==================== 5. 绩效排名快照 ====================
    c.execute(
        """SELECT name FROM sqlite_master WHERE type='table' AND name='performance_ranking_snapshot'"""
    )
    if c.fetchone():
        rankings = [
        ...
        if "company_rank" in col_names:
            for r in rankings:
                c.execute(
                    """INSERT OR REPLACE INTO performance_ranking_snapshot
                    (id, period_id, user_id, user_name, department_id, department_name,

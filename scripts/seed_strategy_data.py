...
def seed():
    ...
    for h in kpi_history:
        c.execute(
            """INSERT OR REPLACE INTO kpi_history 
    ...
    for w in works:
        c.execute(
            """INSERT OR REPLACE INTO annual_key_works
    ...
    dept_objectives = [
        (
            ...
            None,  # 研发部
            json.dumps(
                ["完成3个标准化测试平台研发", "申请3项发明专利", "建立技术预研机制"],
                ensure_ascii=False,
            ),
            json.dumps(
                ["Q2完成电池包EOL平台V2", "Q3完成ICT标准化模块", "每季度1项专利申请"],
                ensure_ascii=False,
            ),
            json.dumps(
                [
                    {"kpi_name": "研发项目完成率", "target": 90, "unit": "%"},
                    {"kpi_name": "专利申请数", "target": 3, "unit": "项"},
                ],
                ensure_ascii=False,
            ),
            "active",
        ...
        (
            ...
            None,  # 销售部
            json.dumps(
                ["完成1.8亿新签合同", "拓展半导体行业客户", "TOP5客户复购率>80%"],
                ensure_ascii=False,
            ),
            json.dumps(
                ["Q1新签5000万", "Q2锁定2个半导体意向客户", "每月客户回访"], ensure_ascii=False
            ),
            json.dumps(
                [
                    {"kpi_name": "新签合同额", "target": 18000, "unit": "万元"},
                    {"kpi_name": "新客户数", "target": 8, "unit": "个"},
                ],
                ensure_ascii=False,
            ),
            "active",
        ...
        (
            ...
            None,  # 生产部
            json.dumps(
                ["项目交付周期缩短20%", "一次通过率>95%", "产能提升25%"], ensure_ascii=False
            ),
            json.dumps(["Q1建立标准工序库", "Q2导入SPC", "Q3实现产线柔性化"], ensure_ascii=False),
            json.dumps(
                [
                    {"kpi_name": "平均交付周期", "target": 75, "unit": "天"},
                    {"kpi_name": "一次通过率", "target": 95, "unit": "%"},
                ],
                ensure_ascii=False,
            ),
            "active",
        ...
        (
            ...
            None,  # 采购部
            json.dumps(
                ["关键物料采购成本降低10%", "供应商准时交付率>95%", "建立战略供应商体系"],
                ensure_ascii=False,
            ),
            json.dumps(
                ["Q1完成供应商分级", "Q2签订年度框架协议", "Q3导入第二供应商"], ensure_ascii=False
            ),
            json.dumps(
                [
                    {"kpi_name": "采购成本降幅", "target": 10, "unit": "%"},
                    {"kpi_name": "准时交付率", "target": 95, "unit": "%"},
                ],
                ensure_ascii=False,
            ),
            "active",
    ...
    for obj in dept_objectives:
        c.execute(
            """INSERT OR REPLACE INTO department_objectives
    ...
    for p in personal:
        c.execute(
            """INSERT OR REPLACE INTO personal_kpis
    ...
    reviews = [
        (
            ...
            73,
            json.dumps(
                ["营收增速低于预期", "半导体拓展尚未突破", "PMS系统推广顺利"], ensure_ascii=False
            ),
            json.dumps(
                ["新能源平台方案通过评审", "毛利率环比改善", "3项专利已提交"], ensure_ascii=False
            ),
            json.dumps(
                ["加大半导体行业投入", "优化报价流程提升毛利", "加快PMS培训进度"],
                ensure_ascii=False,
            ),
            json.dumps(["批准半导体专项预算80万", "成立报价优化小组"], ensure_ascii=False),
            json.dumps(
                [
                    {"task": "半导体行业调研报告", "owner": "销售部", "deadline": "2026-04-15"},
                    {"task": "报价优化方案", "owner": "技术部", "deadline": "2026-04-30"},
                ],
                ensure_ascii=False,
            ),
            "全体高管参加，讨论了Q1执行情况和Q2重点",
            '["符总","张总","李总","王总","赵总"]',
            120,
            "2026-06-28",
        ),
    ...
    for e in events:
        c.execute(
            """INSERT OR REPLACE INTO strategy_calendar_events
    ...
    # ==================== 10. 同比分析 ====================
    c.execute(
        """INSERT OR REPLACE INTO strategy_comparisons
        ...
        (
            ...
            json.dumps(
                ["学习成长维度提升最大(+13分)", "PMS系统推广成效显著", "技术专利申请超进度"],
                ensure_ascii=False,
            ),
            json.dumps(
                ["客户维度提升幅度最小(+6分)", "半导体行业拓展需加速", "回款效率仍需改善"],
                ensure_ascii=False,
            ),
            json.dumps(
                ["加大半导体行业资源投入", "建立客户分级回款机制", "Q2重点推进质量管理升级"],
                ensure_ascii=False,
            ),
            now,
            now,

...
def test_issue_ticket_relation():
    ...
    with get_db_session() as db:
        ...
        try:
            # 查询有关联工单的问题
            issues_with_ticket = db.query(Issue).filter(Issue.service_ticket_id.isnot(None)).count()
            print(f"   ✅ 有关联工单的问题数量: {issues_with_ticket}")

            # 查询一个具体的问题（如果有）
            issue_with_ticket = db.query(Issue).filter(Issue.service_ticket_id.isnot(None)).first()
        ...
        try:
            ...
            if ticket:
                ...
                # 查询关联的问题
                related_issues = db.query(Issue).filter(Issue.service_ticket_id == ticket.id).all()
        ...
        try:
            # SQLite
            result = db.execute(
                "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_issues_service_ticket_id'"
            ).fetchall()
            if result:
                ...
                # MySQL
                result = db.execute(
                    "SHOW INDEX FROM issues WHERE Key_name='idx_issues_service_ticket_id'"
                ).fetchall()
                if result:
                    print("   ✅ 索引 idx_issues_service_ticket_id 存在")
                else:

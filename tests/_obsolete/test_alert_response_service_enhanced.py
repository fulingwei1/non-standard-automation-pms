...
class TestCalculateResponseTimes:
    ...
    def test_calculate_response_times_single_alert(self, mock_alert):
        ...
        alert = mock_alert(
            triggered_at=base_time, acknowledged_at=base_time + timedelta(minutes=30)
        )
    ...
    def test_calculate_response_times_missing_triggered_at(self, mock_alert):
        """测试缺少triggered_at"""
        alert = mock_alert(triggered_at=None, acknowledged_at=datetime.now())
    ...
    def test_calculate_response_times_missing_acknowledged_at(self, mock_alert):
        """测试缺少acknowledged_at"""
        alert = mock_alert(triggered_at=datetime.now(), acknowledged_at=None)
...
class TestCalculateResolveTimes:
    ...
    def test_calculate_resolve_times_single_alert(self, mock_alert):
        """测试单个告警"""
        base_time = datetime(2024, 1, 1, 8, 0, 0)
        alert = mock_alert(acknowledged_at=base_time, handle_end_at=base_time + timedelta(hours=2))
...
class TestGenerateResponseRankings:
    ...
    def test_generate_rankings_basic(self):
        ...
        result = alert_response_service.generate_response_rankings(project_metrics, handler_metrics)
    ...
    def test_generate_rankings_limit_to_5(self):
        """测试限制前5名"""
        project_metrics = {
            f"项目{i}": {"project_id": i, "avg_hours": float(i), "count": 10} for i in range(1, 11)
        }

        handler_metrics = {
            f"用户{i}": {"user_id": i, "avg_hours": float(i), "count": 10} for i in range(1, 11)
        }
...
class TestAlertResponseServiceCalculateDailyMetrics:
    ...
    @patch("app.services.alert_response_service.calculate_handler_metrics")
    @patch("app.services.alert_response_service.generate_response_rankings")
    def test_calculate_daily_metrics_with_data(
        self,
        mock_rankings,
        mock_handler,
        mock_project,
        ...
        mock_response.return_value = [{"hours": 1.0}, {"hours": 2.0}, {"hours": 3.0}]
        mock_resolve.return_value = [{"hours": 3.0}, {"hours": 4.0}]
    ...
    @patch("app.services.alert_response_service.calculate_handler_metrics")
    @patch("app.services.alert_response_service.generate_response_rankings")
    def test_calculate_daily_metrics_no_data(
        self,
        mock_rankings,
        mock_handler,
        mock_project,

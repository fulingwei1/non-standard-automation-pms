...
class TestCostAlertService:
    ...
    def test_check_budget_execution_project_not_found(self, db_session):
        """测试项目不存在"""
        result = CostAlertService.check_budget_execution(db=db_session, project_id=99999)
    ...
    def test_check_budget_execution_no_budget(self, db_session, test_project):
        ...
        result = CostAlertService.check_budget_execution(db=db_session, project_id=test_project.id)
    ...
    def test_check_budget_execution_warning_level(self, db_session, test_project, test_budget):
        """测试警告级别预警（执行率80-90%）"""
        with (
            patch("app.services.budget_execution_check_service.get_actual_cost") as mock_cost,
            patch(
    ...
    def test_check_budget_execution_with_trigger_source(
        self, db_session, test_project, test_budget
    ):
        """测试带触发来源的预警"""
        with (
            patch("app.services.budget_execution_check_service.get_actual_cost") as mock_cost,
            ...
            result = CostAlertService.check_budget_execution(
                db=db_session, project_id=test_project.id, trigger_source="PURCHASE", source_id=123
            )
    ...
    def test_check_all_projects_budget_specific_projects(self, db_session, test_project):
        ...
        with patch.object(CostAlertService, "check_budget_execution") as mock_check:
            ...
            result = CostAlertService.check_all_projects_budget(
                db=db_session, project_ids=[test_project.id]
            )
    ...
    def test_check_all_projects_budget_all_active(self, db_session, test_project):
        ...
        with patch.object(CostAlertService, "check_budget_execution") as mock_check:
            ...
            result = CostAlertService.check_all_projects_budget(db=db_session, project_ids=None)
    ...
    def test_check_all_projects_budget_with_alerts(self, db_session, test_project):
        ...
        with patch.object(CostAlertService, "check_budget_execution") as mock_check:
            ...
            result = CostAlertService.check_all_projects_budget(
                db=db_session, project_ids=[test_project.id, 99999]  # 第二个项目不存在，会被跳过
            )
    ...
    def test_check_all_projects_budget_empty_list(self, db_session):
        """测试批量检查 - 空列表"""
        result = CostAlertService.check_all_projects_budget(db=db_session, project_ids=[])

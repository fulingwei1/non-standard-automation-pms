...
def test_db():
    ...
    # 创建测试用户
    test_user = User(
        id=1, username="test_user", email="test@example.com", password_hash="hashed_password"
    )
    db.add(test_user)
    db.commit()
                            ...
                            "process_flow": [
                                {
                                    ...
                                    "name": "上料",
                                    "description": "自动上料系统",
                                    "equipment_required": ["上料机"],
                                },
                                {
                                    "step_number": 2,
                                    "name": "视觉检测",
                                    "description": "外观检测",
                                    "equipment_required": ["视觉系统"],
                                },
                                {
                                    "step_number": 3,
                                    "name": "组装",
                                    "description": "自动组装",
                                    "equipment_required": ["机器人"],
                                },
                            ],
    ...
    async def test_analyze_simple_requirement(self, sample_requirement_text):
        ...
        with patch.object(analyzer, "_call_openai_api", new_callable=AsyncMock) as mock_api:
            mock_api.return_value = (
                '{"structured_requirement": {"project_type": "自动化"}, "equipment_list": []}'
            )
...
class TestQuestionGeneration:
    ...
    async def test_questions_have_categories(self, sample_requirement_text):
        ...
        with patch.object(analyzer, "_call_openai_api", new_callable=AsyncMock) as mock_api:
            ...
            assert questions[0].category in [
                "技术参数",
                "功能需求",
                "约束条件",
                "验收标准",
    ...
    async def test_questions_with_structured_data(self, sample_requirement_text):
        ...
        with patch.object(analyzer, "_call_openai_api", new_callable=AsyncMock) as mock_api:
            ...
            questions = await analyzer.generate_clarification_questions(
                sample_requirement_text, structured_data
            )
...
class TestStructuredOutput:
    ...
    def test_equipment_item_schema(self):
        """测试16: 设备项Schema验证"""
        data = {"name": "工业机器人", "type": "六轴机器人", "quantity": 2, "priority": "high"}
    ...
    def test_process_step_schema(self):
        """测试17: 工艺步骤Schema验证"""
        data = {"step_number": 1, "name": "上料", "description": "自动上料"}
    ...
    def test_technical_parameter_schema(self):
        """测试18: 技术参数Schema验证"""
        data = {"parameter_name": "精度", "value": "±0.05mm", "unit": "mm", "is_critical": True}
    ...
    def test_equipment_quantity_validation(self):
        ...
        with pytest.raises(ValueError):
            EquipmentItem(name="机器人", type="六轴", quantity=0)  # 数量必须≥1

    def test_process_step_number_validation(self):
        """测试20: 步骤序号验证"""
        with pytest.raises(ValueError):
            ProcessStep(step_number=0, name="步骤", description="描述")  # 步骤序号必须≥1
...
class TestAPIIntegration:
    ...
    async def test_service_analyze_requirement(self, test_db, sample_requirement_text):
        ...
        request = RequirementAnalysisRequest(
            presale_ticket_id=1, raw_requirement=sample_requirement_text, ai_model="gpt-4"
        )

        with patch.object(
            service.analyzer, "analyze_requirement", new_callable=AsyncMock
        ) as mock_analyze:
            with patch.object(
                service.analyzer, "generate_clarification_questions", new_callable=AsyncMock
            ) as mock_questions:
                with patch.object(
                    service.analyzer, "perform_feasibility_analysis", new_callable=AsyncMock
                ) as mock_feasibility:
                    # 模拟返回
    ...
    def test_service_get_analysis(self, test_db):
        ...
        analysis = PresaleAIRequirementAnalysis(
            presale_ticket_id=1, raw_requirement="测试需求", confidence_score=0.8, created_by=1
        )
    ...
    async def test_service_refine_requirement(self, test_db):
        ...
        # 创建初始分析
        analysis = PresaleAIRequirementAnalysis(
            presale_ticket_id=1, raw_requirement="初始需求", confidence_score=0.6, created_by=1
        )
        test_db.add(analysis)
        test_db.commit()

        request = RequirementRefinementRequest(
            analysis_id=analysis.id, additional_context="补充信息"
        )

        with patch.object(
            service.analyzer, "analyze_requirement", new_callable=AsyncMock
        ) as mock_analyze:
            mock_analyze.return_value = {"structured_requirement": {}, "confidence_score": 0.85}
    ...
    def test_service_get_confidence(self, test_db):
        ...
        # 创建高置信度分析
        analysis = PresaleAIRequirementAnalysis(
            presale_ticket_id=1, raw_requirement="测试", confidence_score=0.90, created_by=1
        )
        test_db.add(analysis)
        test_db.commit()
...
class TestAcceptanceCriteria:
    ...
    def test_requirement_update(self, test_db):
        ...
        # 创建初始分析
        analysis = PresaleAIRequirementAnalysis(
            presale_ticket_id=1, raw_requirement="测试", created_by=1
        )
        test_db.add(analysis)
        test_db.commit()
        ...
        request = RequirementUpdateRequest(analysis_id=analysis.id, equipment_list=equipment)
    ...
    def test_confidence_score_breakdown(self, test_db):
        ...
        analysis = PresaleAIRequirementAnalysis(
            presale_ticket_id=1, raw_requirement="测试", confidence_score=0.80, created_by=1
        )
        test_db.add(analysis)
        test_db.commit()

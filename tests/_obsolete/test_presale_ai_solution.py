...
def test_user(db_session):
    """创建测试用户"""
    user = User(username="test_user", email="test@example.com", password_hash="hashed_password_123")
    db_session.add(user)
    db_session.commit()
...
def test_template(db_session, test_user):
    ...
    template = PresaleSolutionTemplate(
        ...
        solution_content={
            ...
            "equipment_list": [{"name": "装配机器人", "model": "ROBOT-500", "quantity": 4}],
        },
...
def test_match_templates_by_industry(db_session, test_user, test_template):
    ...
    request = TemplateMatchRequest(presale_ticket_id=1, industry="汽车", top_k=3)
...
def test_match_templates_by_equipment_type(db_session, test_user, test_template):
    """测试2: 按设备类型匹配模板"""
    service = PresaleAIService(db_session)
    request = TemplateMatchRequest(presale_ticket_id=1, equipment_type="装配", top_k=3)
...
def test_match_templates_by_keywords(db_session, test_user, test_template):
    """测试3: 按关键词匹配模板"""
    service = PresaleAIService(db_session)
    request = TemplateMatchRequest(presale_ticket_id=1, keywords="汽车 机器人", top_k=3)
...
def test_match_templates_empty_result(db_session, test_user):
    """测试4: 无匹配结果"""
    service = PresaleAIService(db_session)
    request = TemplateMatchRequest(presale_ticket_id=1, industry="不存在的行业", top_k=3)
...
def test_generate_architecture_basic(db_session, test_user):
    ...
    result = service.generate_architecture(
        requirements={"type": "装配线"}, diagram_type="architecture"
    )
...
def test_generate_topology_diagram(db_session, test_user):
    ...
    result = service.generate_architecture(
        requirements={"equipment": ["robot", "plc"]}, diagram_type="topology"
    )
...
def test_generate_signal_flow_diagram(db_session, test_user):
    ...
    result = service.generate_architecture(
        requirements={"signals": ["input", "output"]}, diagram_type="signal_flow"
    )
...
def test_generate_architecture_with_solution_id(db_session, test_user):
    """测试22: 关联方案ID生成架构图"""
    # 先创建一个方案
    solution = PresaleAISolution(presale_ticket_id=1, created_by=test_user.id)
    ...
    result = service.generate_architecture(
        requirements={"type": "test"}, diagram_type="architecture", solution_id=solution.id
    )
...
def test_generate_bom_basic(db_session, test_user):
    ...
    equipment_list = [{"name": "机器人", "model": "ROBOT-500", "quantity": 2}]

    result = service.generate_bom(
        equipment_list=equipment_list, include_cost=True, include_suppliers=True
    )
...
def test_generate_bom_without_cost(db_session, test_user):
    ...
    equipment_list = [{"name": "PLC", "quantity": 1}]

    result = service.generate_bom(
        equipment_list=equipment_list, include_cost=False, include_suppliers=False
    )
...
def test_generate_bom_without_suppliers(db_session, test_user):
    ...
    equipment_list = [{"name": "传感器", "quantity": 10}]

    result = service.generate_bom(
        equipment_list=equipment_list, include_cost=True, include_suppliers=False
    )
...
def test_generate_bom_item_structure(db_session, test_user):
    ...
    equipment = {"name": "机器人", "model": "ROBOT-600", "quantity": 1, "unit": "台"}
...
def test_generate_bom_total_cost(db_session, test_user):
    ...
    equipment_list = [{"name": "设备A", "quantity": 2}, {"name": "设备B", "quantity": 1}]

    result = service.generate_bom(equipment_list=equipment_list, include_cost=True)
...
def test_generate_bom_with_solution_id(db_session, test_user):
    ...
    equipment_list = [{"name": "测试设备", "quantity": 1}]

    result = service.generate_bom(equipment_list=equipment_list, solution_id=solution.id)
...
def test_generate_bom_empty_list(db_session, test_user):
    ...
    result = service.generate_bom(equipment_list=[], include_cost=True)
...
def test_generate_bom_performance(db_session, test_user):
    ...
    equipment_list = [{"name": f"设备{i}", "quantity": 1} for i in range(10)]
...
def test_get_solution(db_session, test_user):
    ...
    solution = PresaleAISolution(
        presale_ticket_id=1, solution_description="Test solution", created_by=test_user.id
    )
    db_session.add(solution)
    db_session.commit()
    db_session.refresh(solution)
...
def test_update_solution(db_session, test_user):
    ...
    updated = service.update_solution(solution.id, {"solution_description": "Updated description"})
...
def test_review_solution(db_session, test_user):
    """测试33: 审核方案"""
    solution = PresaleAISolution(presale_ticket_id=1, status="draft", created_by=test_user.id)
    db_session.add(solution)
    db_session.commit()
    db_session.refresh(solution)
    ...
    reviewed = service.review_solution(solution.id, test_user.id, "approved", "方案通过审核")
...
def test_get_template_library(db_session, test_user, test_template):
    ...
    templates = service.get_template_library(industry="汽车", is_active=True)
...
def test_update_template(db_session, test_user, test_template):
    ...
    updated = template_service.update_template(test_template.id, {"name": "更新后的模板名"})

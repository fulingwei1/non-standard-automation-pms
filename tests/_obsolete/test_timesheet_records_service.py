...
class TestTimesheetRecordsService(unittest.TestCase):
    ...
    def test_list_timesheets_basic(self, mock_access_filter):
        ...
        mock_query.offset.return_value.limit.return_value.all.return_value = [mock_ts1, mock_ts2]
        ...
        items, total = self.service.list_timesheets(
            current_user=self.current_user, offset=0, limit=10
        )
    ...
    def test_update_timesheet_success(self, mock_get_or_404, mock_save_obj):
        ...
        # 准备更新数据
        update_data = TimesheetUpdate(work_hours=Decimal("9.0"), description="更新后的描述")
    ...
    def test_check_duplicate_timesheet_with_project(self):
        ...
        # 执行并验证
        with self.assertRaises(HTTPException) as ctx:
            self.service._check_duplicate_timesheet(1, date(2025, 1, 15), 100, None)

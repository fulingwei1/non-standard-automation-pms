...
class TestAIPlanningAPI:
    ...
    def test_get_template_detail_api(self, db: Session, auth_headers):
        ...
        response = client.get(f"/api/v1/ai-planning/templates/{template.id}", headers=auth_headers)
    ...
    def test_get_wbs_suggestions_api(self, sample_project, auth_headers):
        """测试：获取WBS建议API"""
        response = client.get(f"/api/v1/ai-planning/wbs/{sample_project.id}", headers=auth_headers)
    ...
    def test_accept_wbs_api(self, db: Session, sample_project, auth_headers):
        ...
        response = client.patch(f"/api/v1/ai-planning/wbs/{wbs.id}/accept", headers=auth_headers)
    ...
    def test_get_resource_allocations_api(self, sample_project, auth_headers):
        """测试：获取资源分配API"""
        response = client.get(
            f"/api/v1/ai-planning/allocations/{sample_project.id}", headers=auth_headers
        )
    ...
    def test_get_project_schedule_api(self, sample_project, auth_headers):
        """测试：获取项目进度API"""
        response = client.get(
            f"/api/v1/ai-planning/schedule/{sample_project.id}", headers=auth_headers
        )
    ...
    def test_get_accuracy_statistics_api(self, auth_headers):
        """测试：准确性统计API"""
        response = client.get("/api/v1/ai-planning/statistics/accuracy", headers=auth_headers)
    ...
    def test_get_performance_statistics_api(self, auth_headers):
        """测试：性能统计API"""
        response = client.get("/api/v1/ai-planning/statistics/performance", headers=auth_headers)
    ...
    def test_api_error_handling(self, auth_headers):
        """测试：API错误处理"""
        # 测试不存在的模板
        response = client.get("/api/v1/ai-planning/templates/99999", headers=auth_headers)

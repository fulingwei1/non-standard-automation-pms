...
from app.models import Project, TaskUnified, User
from app.models.ai_planning import AIResourceAllocation, AIWbsSuggestion
...
def test_project(db: Session):
    ...
    project = Project(
        project_code=code, project_name="资源测试项目", project_type="WEB_DEV", status="ST01"
    )
    db.add(project)
    db.commit()
    db.refresh(project)
...
def test_wbs(db: Session, test_project):
    ...
    wbs = AIWbsSuggestion(
        ...
        required_skills=json.dumps(
            [{"skill": "Python", "level": "Senior"}, {"skill": "Django", "level": "Middle"}],
            ensure_ascii=False,
        ),
    )
    db.add(wbs)
...
class TestAIResourceOptimizerEnhanced:
    ...
    async def test_allocate_resources_empty_users(self, db: Session, test_wbs, mock_glm_service):
        ...
        allocations = await optimizer.allocate_resources(
            wbs_suggestion_id=test_wbs.id, available_user_ids=[]
        )
    ...
    async def test_allocate_resources_invalid_wbs(self, db: Session, mock_glm_service):
        ...
        allocations = await optimizer.allocate_resources(
            wbs_suggestion_id=99999, available_user_ids=[1, 2, 3]  # 不存在的ID
        )
    ...
    async def test_allocate_resources_with_constraints(
        self, db: Session, test_wbs, test_users, mock_glm_service
    ):
        """测试：带约束条件的资源分配"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)

        constraints = {"max_allocations": 2, "min_skill_match": 60.0}
    ...
    def test_calculate_skill_match_no_requirements(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：无技能要求时的匹配度计算"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    def test_calculate_skill_match_with_requirements(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：有技能要求时的匹配度"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    def test_calculate_experience_match_no_tasks(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：无历史任务的经验匹配度"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    def test_calculate_experience_match_with_tasks(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：有历史任务的经验匹配度"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    def test_calculate_availability_zero_workload(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：无工作负载的可用性"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    def test_calculate_availability_high_workload(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：高工作负载的可用性"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    def test_calculate_performance_score_no_history(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：无历史绩效数据"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    def test_calculate_performance_score_perfect_delivery(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：完美按时交付的绩效"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    def test_generate_recommendation_reason_high_scores(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：高评分的推荐理由"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    def test_generate_recommendation_reason_low_availability(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：低可用性的推荐理由"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    def test_analyze_strengths(self, db: Session, test_users, test_wbs, mock_glm_service):
        ...
        strengths = optimizer._analyze_strengths(
            test_users[0], test_wbs, skill_match=85.0, performance=85.0
        )
    ...
    def test_analyze_weaknesses(self, db: Session, test_users, test_wbs, mock_glm_service):
        ...
        weaknesses = optimizer._analyze_weaknesses(
            test_users[0], test_wbs, skill_match=40.0, availability=30.0  # 低技能匹配  # 低可用性
        )
    ...
    def test_optimize_allocations_priority_assignment(
        self, db: Session, test_wbs, mock_glm_service
    ):
        """测试：优先级分配"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)
    ...
    @pytest.mark.asyncio
    async def test_analyze_user_match_complete_flow(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：完整的用户匹配分析流程"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)

        allocation = await optimizer._analyze_user_match(test_users[0], test_wbs, None)
    ...
    @pytest.mark.asyncio
    async def test_analyze_user_match_low_score_rejection(
        self, db: Session, test_users, test_wbs, mock_glm_service
    ):
        """测试：低综合评分拒绝分配"""
        optimizer = AIResourceOptimizer(db, mock_glm_service)

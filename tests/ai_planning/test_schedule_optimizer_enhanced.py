...
def test_project(db: Session):
    ...
    project = Project(
        project_code=code, project_name="排期测试项目", project_type="WEB_DEV", status="ST01"
    )
    db.add(project)
    db.commit()
    db.refresh(project)
...
class TestAIScheduleOptimizerEnhanced:
    ...
    def test_optimize_schedule_empty_tasks(self, db: Session, test_project):
        ...
        result = optimizer.optimize_schedule(
            project_id=test_project.id, start_date=date(2026, 3, 1)
        )
    ...
    def test_optimize_schedule_invalid_project(self, db: Session):
        ...
        result = optimizer.optimize_schedule(
            project_id=99999, start_date=date(2026, 3, 1)  # 不存在的ID
        )
    ...
    def test_optimize_schedule_default_start_date(self, db: Session, test_project, test_wbs_chain):
        ...
        result = optimizer.optimize_schedule(
            project_id=test_project.id, start_date=None  # 使用默认值
        )
    ...
    def test_optimize_schedule_with_constraints(self, db: Session, test_project, test_wbs_chain):
        ...
        constraints = {"max_duration": 60, "critical_path_limit": 0.3}

        result = optimizer.optimize_schedule(
            project_id=test_project.id, start_date=date(2026, 3, 1), constraints=constraints
        )
    ...
    def test_generate_gantt_data_structure(self, db: Session, test_wbs_chain):
        ...
        gantt_data = optimizer._generate_gantt_data(test_wbs_chain, cpm_result, start_date)
    ...
    def test_analyze_resource_load_with_allocations(
        self, db: Session, test_project, test_wbs_chain
    ):
        """测试：有资源分配的负载分析"""
        # 创建一些资源分配
    ...
    def test_generate_recommendations_with_critical_path(self, db: Session, test_wbs_chain):
        ...
        critical_path = [{"task_id": t.id, "task_name": t.task_name} for t in test_wbs_chain]

        recommendations = optimizer._generate_recommendations(test_wbs_chain, critical_path, [], {})
    ...
    def test_generate_recommendations_resource_imbalance(self, db: Session, test_wbs_chain):
        ...
        # 模拟不均衡的资源负载
        resource_load = {1: {"total_hours": 400}, 2: {"total_hours": 100}}  # 显著低于平均值

        recommendations = optimizer._generate_recommendations(test_wbs_chain, [], [], resource_load)
    ...
    def test_generate_recommendations_high_risk_tasks(self, db: Session, test_project):
        ...
        recommendations = optimizer._generate_recommendations([high_risk_task], [], [], {})
    ...
    def test_optimize_schedule_complete_integration(
        self, db: Session, test_project, test_wbs_chain
    ):
        """测试：完整的排期优化集成"""
        optimizer = AIScheduleOptimizer(db)

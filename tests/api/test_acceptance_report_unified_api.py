...
class TestAcceptanceReportUnifiedAPI:
    ...
    def test_list_available_reports(self, pm_auth_headers):
        """测试获取可用报表列表"""
        response = client.get("/api/v1/reports/available", headers=pm_auth_headers)
    ...
    def test_get_acceptance_report_schema(self, pm_auth_headers):
        """测试获取验收报表Schema"""
        response = client.get(
    ...
    def test_generate_acceptance_report_json(self, pm_auth_headers, db_session):
        """测试生成验收报表（JSON格式）"""

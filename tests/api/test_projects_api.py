...
class TestProjectsAPI:
    ...
    def test_get_projects_templates(self, api_client, db_session):
        ...
        template1 = ProjectTemplateFactory(
            template_name="ICT测试模板", project_type="NEW", is_active=True
        )
        template2 = ProjectTemplateFactory(
            template_name="FCT测试模板", project_type="REPEAT", is_active=True
        )
    ...
    def test_get_projects_templates_template_id(self, api_client, db_session):
        ...
        template = ProjectTemplateFactory(
            template_code=_TPL_DETAIL_001, template_name="详情测试模板", project_type="NEW"
        )
    ...
    def test_put_projects_templates_template_id(self, api_client, db_session):
        ...
        # 准备测试数据
        template = ProjectTemplateFactory(template_name="原始名称", description="原始描述")
        ...
        with get_session() as session:
            updated_template = (
                session.query(ProjectTemplate).filter(ProjectTemplate.id == template.id).first()
            )
    ...
    def test_post_projects_templates_template_id_versions(self, api_client, db_session):
        ...
        response = api_client.post(
            f"/api/v1/projects/templates/{template.id}/versions", json=version_data
        )
    ...
    @pytest.mark.skip(reason="测试与实际API不匹配")
    def test_put_projects_templates_template_id_versions_version_id_publish(
        ...
        version = ProjectTemplateVersionFactory(
            template_id=template.id, version_no="V1", status="DRAFT"
        )
        ...
        with get_session() as session:
            published_version = (
                session.query(ProjectTemplateVersion)
                .filter(ProjectTemplateVersion.id == version.id)
                .first()
            )
    ...
    def test_get_projects_templates_template_id_versions_compare(self, api_client, db_session):
        ...
        version1 = ProjectTemplateVersionFactory(
            template_id=template.id, version_no="V1", template_config='{"stage": "S1"}'
        )
        version2 = ProjectTemplateVersionFactory(
            template_id=template.id, version_no="V2", template_config='{"stage": "S2"}'
        )
    ...
    @pytest.mark.skip(reason="测试与实际API不匹配")
    def test_post_projects_templates_template_id_versions_version_id_rollback(
        self, api_client, db_session
    ):
        ...
        template = ProjectTemplateFactory()
        version = ProjectTemplateVersionFactory(
            template_id=template.id, version_no="V1", status="ACTIVE"
        )
    ...
    def test_put_projects_project_id_archive(self, api_client, db_session):
        ...
        # 调用端点
        response = api_client.put(
            f"/api/v1/projects/{project.id}/archive", json={"reason": "项目已完成，进行归档"}
        )
    ...
    def test_put_projects_project_id_unarchive(self, api_client, db_session):
        ...
        # 调用端点
        response = api_client.put(
            f"/api/v1/projects/{project.id}/unarchive", json={"reason": "需要重新激活项目"}
        )
    ...
    def test_post_projects_project_id_clone(self, api_client, db_session):
        ...
        response = api_client.post(
            ...
            json={
                "project_code": f"PJ_CLONE_001-{uuid.uuid4().hex[:8]}",
                "project_name": "克隆项目",
            },
        )
        assert response.status_code in [200, 201, 400, 404]
    ...
    def test_put_projects_project_id_status(self, api_client, db_session):
        ...
        project = ProjectWithCustomerFactory()
        response = api_client.put(
            f"/api/v1/projects/{project.id}/status", json={"status": "ST02", "reason": "状态更新"}
        )
        assert response.status_code in [200, 400, 404]
    ...
    def test_put_projects_project_id_health(self, api_client, db_session):
        ...
        response = api_client.put(
            f"/api/v1/projects/{project.id}/health", json={"health": "H2", "reason": "存在风险"}
        )
        assert response.status_code in [200, 400, 404]
    ...
    def test_post_projects_project_id_health_calculate(self, api_client, db_session):
        ...
        # 调用端点
        response = api_client.post(
            f"/api/v1/projects/{project.id}/health/calculate", params={"auto_update": False}
        )
    ...
    def test_post_projects_batch_assign_pm(self, api_client, db_session):
        ...
        response = api_client.post(
            ...
            json={"project_ids": [project1.id, project2.id], "pm_id": pm.id},
        )
        assert response.status_code in [200, 400]
    ...
    def test_post_projects_project_id_payment_plans(self, api_client, db_session):
        ...
        response = api_client.post(f"/api/v1/projects/{project.id}/payment-plans", json=plan_data)
        assert response.status_code == 201
        data = response.json()
        assert data["code"] == 200
        assert "data" in data
    ...
    def test_put_projects_payment_plans_plan_id(self, api_client, db_session):
        ...
        update_data = {"planned_amount": 120000.00, "planned_date": "2026-02-15"}
        response = api_client.put(f"/api/v1/projects/payment-plans/{plan.id}", json=update_data)
        assert response.status_code in [200, 400, 404]

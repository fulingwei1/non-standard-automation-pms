...
class TestQualityRiskDetectionAPI:
    ...
    def test_analyze_work_logs_success(self, client, auth_headers, db: Session):
        ...
        response = client.post(
            "/api/v1/quality-risk/detections/analyze", json=payload, headers=auth_headers
        )
    ...
    def test_list_detections(self, client, auth_headers):
        """测试查询检测记录列表"""
        response = client.get("/api/v1/quality-risk/detections", headers=auth_headers)
    ...
    def test_list_detections_with_filters(self, client, auth_headers):
        """测试带过滤条件查询"""
        params = {"project_id": 1, "risk_level": "HIGH", "limit": 10}

        response = client.get(
            "/api/v1/quality-risk/detections", params=params, headers=auth_headers
        )
    ...
    def test_get_detection_not_found(self, client, auth_headers):
        """测试获取不存在的检测记录"""
        response = client.get("/api/v1/quality-risk/detections/999999", headers=auth_headers)
    ...
    def test_update_detection_status(self, client, auth_headers, db: Session):
        """测试更新检测状态"""
        # 假设存在ID=1的记录
        payload = {"status": "CONFIRMED", "resolution_note": "已确认为真实风险"}

        response = client.patch(
            "/api/v1/quality-risk/detections/1", json=payload, headers=auth_headers
        )
...
class TestQualityTestRecommendationAPI:
    ...
    def test_list_recommendations(self, client, auth_headers):
        """测试查询推荐列表"""
        response = client.get("/api/v1/quality-risk/recommendations", headers=auth_headers)
    ...
    def test_list_recommendations_with_filters(self, client, auth_headers):
        """测试带过滤条件查询推荐"""
        params = {"project_id": 1, "priority_level": "HIGH", "status": "PENDING"}

        response = client.get(
            "/api/v1/quality-risk/recommendations", params=params, headers=auth_headers
        )
    ...
    def test_update_recommendation(self, client, auth_headers):
        """测试更新推荐"""
        payload = {"status": "ACCEPTED", "acceptance_note": "已接受推荐"}

        response = client.patch(
            "/api/v1/quality-risk/recommendations/1", json=payload, headers=auth_headers
        )
...
class TestQualityReportAPI:
    ...
    def test_generate_quality_report(self, client, auth_headers):
        ...
        response = client.post(
            "/api/v1/quality-risk/reports/generate", json=payload, headers=auth_headers
        )
    ...
    def test_get_statistics_summary(self, client, auth_headers):
        """测试获取统计摘要"""
        response = client.get("/api/v1/quality-risk/statistics/summary", headers=auth_headers)
    ...
    def test_get_statistics_with_filters(self, client, auth_headers):
        """测试带过滤条件的统计"""
        params = {"project_id": 1, "days": 60}

        response = client.get(
            "/api/v1/quality-risk/statistics/summary", params=params, headers=auth_headers
        )

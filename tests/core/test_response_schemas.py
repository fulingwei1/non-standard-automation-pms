...
def test_error_response():
    """测试错误响应"""
    response = error_response(message="资源不存在", code=404)
...
def test_paginated_response_edge_cases():
    """测试分页响应边界情况"""
    # 空列表
    response = paginated_response(items=[], total=0, page=1, page_size=20)
    assert response.pages == 0

    # 最后一页
    response = paginated_response(items=[{"id": 1}], total=21, page=2, page_size=20)

...
from app.models.purchase import PurchaseOrder, PurchaseRequest, PurchaseRequestItem
    ...
    def test_low_stock_alert_and_emergency_purchase(self, integration_test_data):
        ...
        for alert in shortage_alerts:
            ...
            purchase_order = PurchaseOrder(
                ...
                required_date=(
                    datetime.utcnow() + timedelta(days=material.lead_time_days // 2)
                ).date(),  # 缩短交期
        ...
        # 验证结果
        processing_alerts = (
            db.query(ShortageAlert).filter(ShortageAlert.status == "PROCESSING").count()
        )

...
class TestInventoryQueryPerformance:
    ...
    def test_stock_realtime_query_performance(self, integration_test_data):
        ...
        low_stock_materials = (
            db.query(Material)
            .filter(Material.current_stock < Material.safety_stock, Material.is_active == True)
            .limit(50)
            .all()
        )
    ...
    def test_batch_stock_query_performance(self, integration_test_data):
        ...
        materials = (
            db.query(Material).filter(Material.id.in_(material_ids[:100])).all()  # 查询100个
        )
...
class TestDemandForecastPerformance:
    ...
    def test_forecast_with_one_year_data(self, integration_test_data):
        ...
        for i in range(365):
            ...
            historical_data.append({"date": date, "consumption": consumption})
        ...
        for i in range(30):
            ...
            forecasts.append({"date": forecast_date, "predicted_demand": avg_consumption})
...
class TestConcurrentStockUpdate:
    ...
    def test_concurrent_stock_updates(self, integration_test_data):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(update_stock, material_id, update_qty) for _ in range(update_count)
            ]
    ...
    def test_high_concurrency_simulation(self, integration_test_data):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(lambda: time.sleep(0.001)) for _ in range(concurrent_requests)
            ]

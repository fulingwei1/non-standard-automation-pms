...
class TestAlertNotificationDelivery:
    ...
    def test_email_notification(self, client: TestClient, db: Session, auth_headers, test_employee):
        ...
        response = client.post(
            "/api/v1/alerts/notifications", json=notification_data, headers=auth_headers
        )
    ...
    def test_system_notification(
        ...
        response = client.post(
            "/api/v1/alerts/notifications", json=system_notif_data, headers=auth_headers
        )
        assert response.status_code in [200, 201, 404]

    def test_notification_delivery_status(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):
        """测试：通知发送状态"""

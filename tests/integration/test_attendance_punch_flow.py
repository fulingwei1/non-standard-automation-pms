...
class TestAttendancePunchFlow:
    ...
    def test_late_and_early_leaving(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):
        """测试：迟到早退处理"""
        # 1. 迟到打卡
    ...
    def test_outdoor_punch(self, client: TestClient, db: Session, auth_headers, test_employee):
        ...
        response = client.post(
            "/api/v1/hr/outing-applications", json=outing_application, headers=auth_headers
        )
    ...
    def test_punch_correction_application(
        ...
        response = client.post(
            "/api/v1/hr/attendance/corrections", json=correction_data, headers=auth_headers
        )
        assert response.status_code in [200, 201, 404]
    ...
    def test_overtime_record(self, client: TestClient, db: Session, auth_headers, test_employee):
        ...
        response = client.post(
            "/api/v1/hr/overtime-applications", json=overtime_application, headers=auth_headers
        )
        assert response.status_code in [200, 201, 404]
        ...
        if overtime_id:
            response = client.get(
                f"/api/v1/hr/overtime-records/{overtime_id}", headers=auth_headers
            )
            assert response.status_code in [200, 404]
    ...
    def test_attendance_statistics(
        ...
        response = client.get(
            "/api/v1/hr/attendance/statistics", params=stats_params, headers=auth_headers
        )
        assert response.status_code in [200, 404]
        ...
        response = client.get(
            "/api/v1/hr/attendance/department-statistics",
            params=dept_stats_params,
            headers=auth_headers,
        )
        assert response.status_code in [200, 404]
        ...
        response = client.post(
            "/api/v1/hr/attendance/export", json=export_request, headers=auth_headers
        )
        assert response.status_code in [200, 201, 404]

    def test_attendance_anomaly_handling(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):
        """测试：考勤异常处理"""
        # 1. 系统自动检测异常
        ...
        response = client.post(
            "/api/v1/hr/attendance/detect-anomalies",
            json=anomaly_detection_request,
            headers=auth_headers,
        )
        assert response.status_code in [200, 201, 404]
        ...
        if response.status_code == 200:
            ...
            if isinstance(anomalies, list) and len(anomalies) > 0:
                ...
                if anomaly_id:
                    ...
                    response = client.post(
                        "/api/v1/hr/attendance/handle-anomaly",
                        json=handling_data,
                        headers=auth_headers,
                    )
                    assert response.status_code in [200, 201, 404]

    def test_flexible_working_attendance(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):
        """测试：弹性工作制考勤"""
        # 1. 设置员工为弹性工作制
        flexible_setting = {
            "employee_id": test_employee.id,
            "work_mode": "flexible",
            "core_work_hours": {"start": "10:00:00", "end": "16:00:00"},
            "min_daily_hours": 8.0,
            "effective_date": str(date.today()),
        }

        response = client.post(
            "/api/v1/hr/work-mode-settings", json=flexible_setting, headers=auth_headers
        )
        assert response.status_code in [200, 201, 404]

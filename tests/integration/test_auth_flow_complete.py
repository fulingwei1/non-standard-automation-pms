...
class TestTokenFlow:
    ...
    def test_token_expiration(self):
        ...
        expired_token = create_access_token(
            data={"sub": "1"}, expires_delta=timedelta(seconds=-10)  # 负数表示已过期
        )
    ...
    def test_token_with_custom_expiration(self):
        ...
        # 1. 生成30分钟过期的Token
        token = create_access_token(data={"sub": "1"}, expires_delta=timedelta(minutes=30))
...
class TestPermissionCheckFlow:
    ...
    def test_user_with_role(self):
        ...
        role = Role(id=2, role_code="PM", role_name="项目经理", data_scope="PROJECT")
    ...
    def test_permission_check_with_role(self, mock_db, test_user_with_role, test_permissions):
        ...
        perms = mock_db.query(RoleApiPermission).filter(RoleApiPermission.role_id == role.id).all()
    ...
    def test_superuser_bypass_permission_check(self):
        """测试超级管理员绕过权限检查"""
        admin = User(
            id=1, username="admin", is_active=True, is_superuser=True, password_hash="test_hash_123"
        )
...
class TestDataScopeFlow:
    ...
    def test_dept_scope_access(self, roles_with_scopes):
        ...
        # 部门范围只能访问本部门数据
        can_access = (
            dept_role.data_scope == "DEPT" and user_dept == target_dept
        ) or dept_role.data_scope == "ALL"
...
class TestCompleteAuthFlow:
    ...
    def test_end_to_end_auth_flow(self, mock_db):
        ...
        # 3. 分配权限
        permission = ApiPermission(id=1, perm_code="project:view", perm_name="查看项目")

        role_perm = RoleApiPermission(role_id=role.id, permission_id=permission.id)
        ...
        # 获取角色权限
        role_perms = (
            mock_db.query(RoleApiPermission).filter(RoleApiPermission.role_id == role.id).all()
        )

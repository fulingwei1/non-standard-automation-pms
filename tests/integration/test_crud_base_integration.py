...
class TestResourceService(
    BaseService[TestResource, TestResourceCreate, TestResourceUpdate, TestResourceResponse]
):
...
class TestCRUDBaseIntegration:
    ...
    async def test_create_and_get_workflow(self, db_session):
        ...
        create_data = TestResourceCreate(
            name="测试资源1", code="TEST001", description="这是一个测试资源"
        )
    ...
    async def test_list_with_filters_and_keyword(self, db_session):
        ...
        # 测试筛选
        result = await service.list(filters={"status": "ACTIVE"}, skip=0, limit=10)
        assert result["total"] == 2
        assert all(item.status == "ACTIVE" for item in result["items"])

        # 测试关键词搜索
        result = await service.list(
            keyword="项目", keyword_fields=["name", "code"], skip=0, limit=10
        )
        assert result["total"] == 2
    ...
    async def test_update_workflow(self, db_session):
        ...
        # 创建资源
        created = await service.create(TestResourceCreate(name="原始名称", code="TEST001"))

        # 更新资源
        updated = await service.update(
            created.id, TestResourceUpdate(name="更新后的名称", status="INACTIVE")
        )
    ...
    async def test_delete_workflow(self, db_session):
        ...
        # 创建资源
        created = await service.create(TestResourceCreate(name="待删除资源", code="TEST001"))
    ...
    async def test_pagination_workflow(self, db_session):
        ...
        # 创建多个资源
        for i in range(25):
            await service.create(TestResourceCreate(name=f"资源{i}", code=f"TEST{i:03d}"))
    ...
    async def test_get_by_field(self, db_session):
        ...
        # 创建资源
        created = await service.create(TestResourceCreate(name="测试资源", code="UNIQUE001"))
    ...
    async def test_create_many_workflow(self, db_session):
        ...
        # 批量创建
        items = [TestResourceCreate(name=f"资源{i}", code=f"BATCH{i:03d}") for i in range(5)]
...
class TestCRUDBaseErrorHandling:
    ...
    async def test_update_not_found(self, db_session):
        ...
        with pytest.raises(Exception):
            await service.update(99999, TestResourceUpdate(name="不存在的资源"))
...
class TestCRUDBasePerformance:
    ...
    async def test_bulk_operations(self, db_session):
        ...
        # 批量创建
        items = [TestResourceCreate(name=f"资源{i}", code=f"PERF{i:05d}") for i in range(100)]

...
class TestExpenseApprovalFlow:
    ...
    def test_expense_approval(self, client: TestClient, db: Session, auth_headers, test_employee):
        ...
        response = client.post(
            "/api/v1/finance/expense-applications/1/approve", json=data, headers=auth_headers
        )
        assert response.status_code in [200, 404]

    def test_expense_reimbursement(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):

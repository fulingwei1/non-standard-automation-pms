...
class TestPerformanceEvaluationFlow:
    def test_performance_plan_creation(
    ...
    def test_self_evaluation(self, client: TestClient, db: Session, auth_headers, test_employee):
        ...
        response = client.post(
            "/api/v1/hr/performance-plans/1/self-evaluate", json=data, headers=auth_headers
        )
    ...
    def test_manager_evaluation(self, client: TestClient, db: Session, auth_headers, test_employee):
        ...
        response = client.post(
            "/api/v1/hr/performance-plans/1/manager-evaluate", json=data, headers=auth_headers
        )
        assert response.status_code in [200, 404]
    ...
    def test_performance_ranking(
        ...
        response = client.get(
            "/api/v1/hr/performance-ranking?year=2024&quarter=1", headers=auth_headers
        )
        assert response.status_code in [200, 404]

...
class TestProjectCostTracking:
    def test_cost_budget_creation(self, client: TestClient, db: Session, auth_headers, test_employee):
        project_data = {"project_name": "测试项目", "project_code": f"PRJ-001-{uuid.uuid4().hex[:8]}", "customer_id": 1, "start_date": str(date.today()), "contract_amount": 5000000.00, "project_manager_id": test_employee.id}

...
class TestProjectTeamCollaboration:
    ...
    def test_create_project_team(
        ...
        members = [
            ...
            {
                "employee_id": test_employee.id + 2,
                "role_name": "开发工程师",
                "join_date": str(date.today()),
            },
            {
                "employee_id": test_employee.id + 3,
                "role_name": "测试工程师",
                "join_date": str(date.today()),
            },
        ]
    ...
    def test_member_permission_assignment(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):
        """测试：成员权限分配"""
        # 1. 创建项目
    ...
    def test_member_collaboration_tasks(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):
        """测试：成员协作任务"""
        # 1. 创建项目
    ...
    def test_member_role_change(self, client: TestClient, db: Session, auth_headers, test_employee):
        ...
        # 3. 变更角色
        update_data = {"role_name": "技术负责人", "change_reason": "工作表现优秀，晋升为技术负责人"}
    ...
    def test_cross_project_member_sharing(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):
        """测试：跨项目成员共享"""
        # 1. 创建两个项目
        ...
        response1 = client.post(
            f"/api/v1/projects/{project1_id}/members", json=member_data, headers=auth_headers
        )
    ...
    def test_member_work_hours_statistics(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):
        """测试：成员工时统计"""
        # 1. 创建项目并添加成员
        ...
        for entry in timesheet_entries:
            response = client.post("/api/v1/timesheets", json=entry, headers=auth_headers)
    ...
    def test_team_communication_log(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):
        """测试：团队沟通记录"""
        # 1. 创建项目

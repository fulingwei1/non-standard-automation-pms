...
class TestSalesFunnelConversion:
    ...
    def test_lead_scoring(self, client: TestClient, db: Session, auth_headers, test_employee):
        ...
        if lead_id:
            score_data = {
                "lead_id": lead_id,
                "scoring_criteria": {"budget": 9, "authority": 8, "need": 9, "timeline": 7},
                "total_score": 82,
            }

            response = client.post(
                f"/api/v1/sales/leads/{lead_id}/score", json=score_data, headers=auth_headers
            )
    ...
    def test_lead_nurturing(self, client: TestClient, db: Session, auth_headers, test_employee):
        ...
        if lead_id:
            ...
            for activity in nurturing_activities:
                response = client.post(
                    f"/api/v1/sales/leads/{lead_id}/nurturing", json=activity, headers=auth_headers
                )
                assert response.status_code in [200, 201, 404]

    def test_opportunity_creation_and_progress(
    ...
    def test_win_loss_analysis(self, client: TestClient, db: Session, auth_headers, test_employee):
        ...
        response = client.post(
            "/api/v1/sales/opportunities", json=win_opportunity, headers=auth_headers
        )
        assert response.status_code in [200, 201]
    ...
    def test_conversion_rate_statistics(
        self, client: TestClient, db: Session, auth_headers, test_employee
    ):
        """测试：转化率统计"""
        response = client.get(

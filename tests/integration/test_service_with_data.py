...
def check_data(token: str):
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/projects?page=1&page_size=1", headers=headers, timeout=10
        )
        if response.status_code == 200:
            data = response.json()
            if data.get("total", 0) > 0:
                ...
                print_success(
                    f"找到项目: {project.get('project_name', 'N/A')} (ID: {project.get('id', 'N/A')})"
                )
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/customers?page=1&page_size=1", headers=headers, timeout=10
        )
        if response.status_code == 200:
            data = response.json()
            if data.get("total", 0) > 0:
                ...
                print_success(
                    f"找到客户: {customer.get('customer_name', 'N/A')} (ID: {customer.get('id', 'N/A')})"
                )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("total", 0) > 0:
                ...
                print_success(
                    f"找到用户: {user.get('name', user.get('username', 'N/A'))} (ID: {user.get('id', 'N/A')})"
                )
...
def test_api(
    method: str, endpoint: str, token: str, data: Optional[dict] = None, description: str = ""
):
    """测试API"""
...
def main():
    ...
    # 获取服务工单列表
    test_api(
        "GET", "/service/service-tickets?page=1&page_size=10", token, description="获取服务工单列表"
    )

    # 获取服务工单统计
    test_api("GET", "/service/service-tickets/statistics", token, description="获取服务工单统计")

    # 获取服务记录列表
    test_api(
        "GET", "/service/service-records?page=1&page_size=10", token, description="获取服务记录列表"
    )

    # 获取服务记录统计
    test_api("GET", "/service/service-records/statistics", token, description="获取服务记录统计")
    ...
    if project_id and customer_id and user_id:
        ...
        ticket_result = test_api(
            "POST", "/service/service-tickets", token, ticket_data, description="创建服务工单"
        )
        ...
        if ticket_result and isinstance(ticket_result, dict) and "id" in ticket_result:
            ...
            test_api(
                "GET",
                f"/service/service-tickets/{ticket_id}",
                token,
        ...
        record_result = test_api(
            "POST", "/service/service-records", token, record_data, description="创建服务记录"
        )
        ...
        if record_result and isinstance(record_result, dict) and "id" in record_result:
            ...
            test_api(
                "GET",
                f"/service/service-records/{record_id}",
                token,

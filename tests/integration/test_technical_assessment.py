        ...
        if response.status_code >= 200 and response.status_code < 300:
            ...
            try:
                ...
                if isinstance(result, dict):
                    if "items" in result:
                        print(
                            f"返回 {len(result.get('items', []))} 条记录，共 {result.get('total', 0)} 条"
                        )
                    elif "id" in result:
...
def main():
    ...
    if not rules or len(rules) == 0:
        ...
        if rule_result:
            ...
            if rule_id:
                test_api(
                    "PUT", f"/sales/scoring-rules/{rule_id}/activate", token, None, "激活评分规则"
                )
    ...
    if leads and "items" in leads and len(leads["items"]) > 0:
        ...
        assessment_result = test_api(
            "POST", f"/sales/leads/{lead_id}/assessments/apply", token, {}, "申请技术评估（线索）"
        )
        ...
        if assessment_result:
            ...
            if assessment_id:
                ...
                # 获取评估列表
                test_api(
                    "GET", f"/sales/leads/{lead_id}/assessments", token, None, "获取线索的评估列表"
                )
                ...
                test_api(
                    ...
                    token,
                    {"requirement_data": requirement_data, "enable_ai": False},
                    "执行技术评估",
                )
    ...
    if lead_id:
        ...
        test_api(
            "POST", f"/sales/leads/{lead_id}/open-items", token, open_item, "创建未决事项（线索）"
        )
        ...
        test_api(
            ...
            "/sales/open-items",
            token,
            {"source_type": "LEAD", "source_id": lead_id, "page": 1, "page_size": 10},
            "获取未决事项列表",
        )

...
class TestAuthMiddlewareNormalFlow:
    ...
    def test_valid_token_success(self, client, mock_user, mock_db):
        """测试有效token验证成功"""
        with (
            patch("app.models.base.get_session", return_value=mock_db),
            patch(
                "app.core.auth.verify_token_and_get_user",
            ...
            response = client.get(
                "/api/v1/protected", headers={"Authorization": "Bearer valid_token_123"}
            )
    ...
    def test_user_info_stored_in_request_state(self, client, mock_user, mock_db):
        ...
        with (
            ...
            response = client.get(
                "/api/v1/protected", headers={"Authorization": "Bearer valid_token"}
            )
...
class TestAuthMiddlewareErrorHandling:
    ...
    def test_invalid_authorization_format_no_bearer(self, client):
        ...
        response = client.get("/api/v1/protected", headers={"Authorization": "invalid_token_123"})
    ...
    def test_invalid_authorization_format_empty_token(self, client):
        """测试空token"""
        response = client.get("/api/v1/protected", headers={"Authorization": "Bearer "})
    ...
    def test_invalid_authorization_format_multiple_parts(self, client):
        """测试多余的空格导致格式错误"""
        response = client.get(
            "/api/v1/protected", headers={"Authorization": "Bearer token part2 part3"}
        )
    ...
    def test_expired_token(self, client, mock_db):
        ...
        with (
            ...
            patch(
                "app.core.auth.verify_token_and_get_user",
                new_callable=AsyncMock,
                side_effect=mock_verify_expired,
            ),
        ):

            response = client.get(
                "/api/v1/protected", headers={"Authorization": "Bearer expired_token"}
            )
    ...
    def test_invalid_token(self, client, mock_db):
        ...
        with (
            ...
            patch(
                "app.core.auth.verify_token_and_get_user",
                new_callable=AsyncMock,
                side_effect=mock_verify_invalid,
            ),
        ):
    ...
    def test_user_not_found(self, client, mock_db):
        ...
        with (
            ...
            patch(
                "app.core.auth.verify_token_and_get_user",
                new_callable=AsyncMock,
                side_effect=mock_verify_no_user,
            ),
        ):

            response = client.get(
                "/api/v1/protected", headers={"Authorization": "Bearer token_for_deleted_user"}
            )
    ...
    def test_database_error(self, client, mock_db):
        ...
        with (
            ...
            patch(
                "app.core.auth.verify_token_and_get_user",
                new_callable=AsyncMock,
                side_effect=mock_verify_db_error,
            ),
        ):
...
class TestAuthMiddlewareBoundaryConditions:
    ...
    def test_very_long_token(self, client, mock_user, mock_db):
        ...
        with (
            ...
            response = client.get(
                "/api/v1/protected", headers={"Authorization": f"Bearer {long_token}"}
            )
    ...
    def test_special_characters_in_token(self, client, mock_db):
        ...
        with (
            ...
            patch(
                "app.core.auth.verify_token_and_get_user",
                new_callable=AsyncMock,
                side_effect=mock_verify_special,
            ),
        ):

            response = client.get(
                "/api/v1/protected", headers={"Authorization": f"Bearer {special_token}"}
            )
...
class TestAuthMiddlewareSecurity:
    ...
    def test_sql_injection_in_token(self, client, mock_db):
        ...
        with (
            ...
            patch(
                "app.core.auth.verify_token_and_get_user",
                new_callable=AsyncMock,
                side_effect=mock_verify_sql,
            ),
        ):

            response = client.get(
                "/api/v1/protected", headers={"Authorization": f"Bearer {malicious_token}"}
            )
    ...
    def test_xss_in_token(self, client, mock_db):
        ...
        with (
            ...
            patch(
                "app.core.auth.verify_token_and_get_user",
                new_callable=AsyncMock,
                side_effect=mock_verify_xss,
            ),
        ):

            response = client.get(
                "/api/v1/protected", headers={"Authorization": f"Bearer {xss_token}"}
            )
    ...
    def test_multiple_authorization_headers(self, client, mock_user, mock_db):
        ...
        with (
            ...
            response = client.get("/api/v1/protected", headers={"Authorization": "Bearer token1"})
    ...
    def test_timing_attack_resistance(self, client, mock_db):
        ...
        with (
            ...
            patch(
                "app.core.auth.verify_token_and_get_user",
                new_callable=AsyncMock,
                side_effect=mock_verify_timing,
            ),
        ):

            start1 = time.time()
            response1 = client.get("/api/v1/protected", headers={"Authorization": "Bearer wrong1"})
            time1 = time.time() - start1

            start2 = time.time()
            response2 = client.get("/api/v1/protected", headers={"Authorization": "Bearer wrong2"})
...
class TestAuthMiddlewareIntegration:
    ...
    def test_db_session_cleanup(self, client, mock_db):
        ...
        with (
            patch("app.models.base.get_session", return_value=mock_db) as mock_get_session,
            patch(
                "app.core.auth.verify_token_and_get_user",
                new_callable=AsyncMock,
                side_effect=mock_verify_success,
            ),
        ):

            client.get("/api/v1/protected", headers={"Authorization": "Bearer valid_token"})

...
class TestTenantMiddlewareErrorHandling:
    ...
    def test_user_without_tenant_id(self):
        ...
        # 显式置 None：Mock(spec=User) 对 spec 里声明过的属性会自动生成一个
        # 子 Mock 而不是抛 AttributeError，不显式赋值的话 getattr 永远拿不到
        # 真正的 None，断言会恒假失败（与本测试要验证的逻辑无关的既有坑）。
        user.tenant_id = None
...
class TestTenantMiddlewareIntegration:
    ...
    def test_middleware_order(self, test_app):
        ...
        tenant_middleware_exists = any("TenantContextMiddleware" in str(m) for m in middlewares)

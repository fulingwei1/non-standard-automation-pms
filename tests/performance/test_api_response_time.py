...
class TestAPIResponseTime:
    ...
    def test_list_apis_response_time(self, client, auth_headers):
        """列表API响应时间"""
        endpoints = ["/api/projects", "/api/users", "/api/tasks", "/api/reports"]
    ...
    def test_detail_apis_response_time(self, client, auth_headers):
        """详情API响应时间"""
        endpoints = ["/api/projects/1", "/api/users/1", "/api/tasks/1"]
    ...
    def test_create_apis_response_time(self, client, auth_headers):
        ...
        for i in range(20):
            ...
            response = client.post(
                "/api/projects",
                json={"name": f"Perf Test Project {i}", "code": f"PTP{i:04d}"},

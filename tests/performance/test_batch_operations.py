...
class TestBatchOperations:
    ...
    def test_batch_create(self, client, auth_headers):
        ...
        for size in batch_sizes:
            data = [{"name": f"Project {i}", "code": f"P{i:05d}"} for i in range(size)]

            start = time.time()
            response = client.post(
                "/api/projects/batch", json={"items": data}, headers=auth_headers
            )
    ...
    def test_batch_update(self, client, auth_headers):
        ...
        response = client.put(
            "/api/projects/batch",
            json={"ids": ids, "data": {"status": "updated"}},
    ...
    def test_batch_delete(self, client, auth_headers):
        ...
        start = time.time()
        response = client.delete("/api/projects/batch", json={"ids": ids}, headers=auth_headers)
        elapsed = time.time() - start

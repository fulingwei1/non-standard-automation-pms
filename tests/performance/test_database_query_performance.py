...
class TestDatabaseQueryPerformance:
    ...
    def test_complex_join_query_performance(self, test_db: Session):
        ...
        for _ in range(iterations):
            ...
            results = (
                test_db.query(
                    Project.id, Project.name, func.count(ProjectMember.id).label("member_count")
                )
    ...
    def test_aggregation_query_performance(self, test_db: Session):
        ...
        for _ in range(iterations):
            ...
            stats = (
                ...
                .filter(Project.status.in_(["active", "pending"]))
                .first()
            )
    ...
    def test_index_efficiency(self, test_db: Session):
        ...
        for _ in range(iterations):
            start = time.time()
            projects = (
                test_db.query(Project).filter(Project.description.like("%500%")).limit(10).all()
            )
            elapsed = time.time() - start
    ...
    def test_bulk_update_performance(self, test_db: Session):
        ...
        # 批量更新
        test_db.query(Project).filter(Project.id.in_(project_ids)).update(
            {"status": "updated"}, synchronize_session=False
        )
        test_db.commit()

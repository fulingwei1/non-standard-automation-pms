...
class TestFileUploadPerformance:
    ...
    def test_excel_export_performance(self, client, auth_headers):
        ...
        for count in record_counts:
            start = time.time()
            response = client.get(f"/api/projects/export?limit={count}", headers=auth_headers)
            elapsed = time.time() - start
    ...
    def test_pdf_generation_performance(self, client, auth_headers):
        ...
        for pages in page_counts:
            start = time.time()
            response = client.post(
                "/api/reports/pdf", json={"project_id": 1, "pages": pages}, headers=auth_headers
            )
            elapsed = time.time() - start
    ...
    def test_file_download_performance(self, client, auth_headers):
        ...
        for file_id in file_ids:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"  文件{file_id}: {size/1024:.2f}KB, {elapsed*1000:.2f}ms, {throughput:.2f}MB/s"
                )

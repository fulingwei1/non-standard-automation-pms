...
class TestSearchPerformance:
    ...
    def test_fulltext_search_performance(self, client, auth_headers, test_db):
        ...
        for term in search_terms:
            ...
            for _ in range(20):
                start = time.time()
                response = client.get(f"/api/projects/search?q={term}", headers=auth_headers)
    ...
    def test_fuzzy_search_performance(self, client, auth_headers):
        ...
        for i in range(50):
            start = time.time()
            response = client.get(f"/api/projects/search?q=test&fuzzy=true", headers=auth_headers)
            elapsed = time.time() - start
            times.append(elapsed)
    ...
    def test_multi_field_search_performance(self, client, auth_headers):
        ...
        for i in range(30):
            ...
            response = client.post(
                "/api/search",
                json={"name": "test", "code": "TP", "status": "active"},
    ...
    def test_search_result_pagination(self, client, auth_headers):
        ...
        for page in [1, 5, 10, 20]:
            start = time.time()
            response = client.get(
                f"/api/projects/search?q=test&page={page}&page_size=20", headers=auth_headers
            )
            elapsed = time.time() - start
    ...
    def test_search_with_filters(self, client, auth_headers):
        """带过滤器的搜索性能"""
        filters = {"status": ["active", "pending"], "budget_min": 10000, "budget_max": 100000}

        times = []
        for _ in range(30):
            start = time.time()
            response = client.post(
                "/api/projects/search", json={"q": "test", "filters": filters}, headers=auth_headers
            )
            elapsed = time.time() - start
            times.append(elapsed)

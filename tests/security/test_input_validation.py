...
class TestInputValidation:
    ...
    def test_email_validation(self, client, admin_headers):
        ...
        for email in invalid_emails:
            response = client.post(
                "/api/users",
                json={"username": "test", "email": email, "password": "Test@123"},
    ...
    def test_phone_validation(self, client, auth_headers):
        ...
        for phone in invalid_phones:
            response = client.put("/api/users/1", json={"phone": phone}, headers=auth_headers)
    ...
    def test_length_validation(self, client, auth_headers):
        ...
        response = client.post(
            "/api/projects", json={"name": long_string, "code": "TEST"}, headers=auth_headers
        )
    ...
    def test_numeric_validation(self, client, auth_headers):
        ...
        for num in invalid_numbers:
            response = client.post("/api/projects", json={"budget": num}, headers=auth_headers)
    ...
    def test_special_characters(self, client, auth_headers):
        ...
        for char in special_chars:
            response = client.post(
                "/api/projects", json={"name": f"Project {char}"}, headers=auth_headers
            )

...
from tests.fixtures.tenant_fixtures import superuser, tenant_a, tenant_b, user_a, user_b
...
class TestMultiModelTenantIsolation:
    ...
    def test_user_model_tenant_isolation(
        self, db: Session, tenant_a: Tenant, tenant_b: Tenant, user_a: User, user_b: User
    ):
        """测试用户模型的租户隔离"""
        # 查询租户A的用户
        tenant_a_users = db.query(User).filter(User.tenant_id == tenant_a.id).all()
        ...
        # 查询租户B的用户
        tenant_b_users = db.query(User).filter(User.tenant_id == tenant_b.id).all()

        # 验证不包含租户A的用户
        user_ids = [u.id for u in tenant_b_users]
        assert user_a.id not in user_ids, "租户B的用户查询不应包含租户A的用户"

    def test_role_model_tenant_isolation(self, db: Session, tenant_a: Tenant, tenant_b: Tenant):
        """测试角色模型的租户隔离"""
        # 创建租户A的角色
        role_a = Role(
        ...
        # 查询租户A的角色
        tenant_a_roles = db.query(Role).filter(Role.tenant_id == tenant_a.id).all()
        role_ids = [r.id for r in tenant_a_roles]
        assert role_b.id not in role_ids, "租户A的角色查询不应包含租户B的角色"

        # 查询租户B的角色
        tenant_b_roles = db.query(Role).filter(Role.tenant_id == tenant_b.id).all()
    ...
    def test_api_key_model_tenant_isolation(
        ...
        # 查询租户A的API Keys
        tenant_a_keys = db.query(APIKey).filter(APIKey.tenant_id == tenant_a.id).all()
        key_ids = [k.id for k in tenant_a_keys]
        assert api_key_b.id not in key_ids, "租户A的API Key查询不应包含租户B的"

        # 查询租户B的API Keys
        tenant_b_keys = db.query(APIKey).filter(APIKey.tenant_id == tenant_b.id).all()
    ...
    def test_api_endpoint_tenant_isolation(
        self, client: TestClient, db: Session, user_a: User, model_info: Tuple[str, str]
    ):
        """参数化测试：验证各API端点的租户隔离"""
        ...
        if response.status_code == 200:
            ...
            for item in items:
                if "tenant_id" in item:
                    assert (
                        item["tenant_id"] == user_a.tenant_id or item["tenant_id"] is None
...
class TestCrossModelTenantIsolation:
    ...
    def test_project_member_tenant_isolation(
        ...
        # 创建租户A的项目
        project_a = create_project(db, tenant_a.id, user_a.id, "CROSS_A_001", "跨模型测试项目A")

        # 创建租户B的项目
        project_b = create_project(db, tenant_b.id, user_b.id, "CROSS_B_001", "跨模型测试项目B")

        # 尝试将租户A的用户添加到租户B的项目（应该被业务逻辑阻止）
        # 注意：这里假设有租户ID验证逻辑
        member = ProjectMember(project_id=project_b.id, user_id=user_a.id, role_code="MEMBER")
        db.add(member)
    ...
    def test_task_assignment_tenant_isolation(
        self, db: Session, tenant_a: Tenant, user_a: User, user_b: User
    ):
        """测试任务分配的租户隔离"""
        from tests.fixtures.tenant_fixtures import create_project

        # 创建租户A的项目
        project_a = create_project(db, tenant_a.id, user_a.id, "TASK_A_001", "任务测试项目A")
    ...
    def test_report_data_tenant_isolation(self, db: Session, tenant_a: Tenant, tenant_b: Tenant):
        """测试报表数据的租户隔离"""
        # 验证报表统计数据只包含本租户的数据
        # 例如：项目统计、成本统计、人员统计等
        ...
        if hasattr(Project, "tenant_id"):
            tenant_a_count = db.query(Project).filter(Project.tenant_id == tenant_a.id).count()
...
class TestTenantIsolationPerformance:
    ...
    def test_large_dataset_tenant_filtering(self, db: Session, tenant_a: Tenant, tenant_b: Tenant):
        """测试大数据集下的租户过滤性能"""
        import time
        ...
        # 为租户A创建100个项目
        for i in range(100):
            create_project(db, tenant_a.id, user_a.id, f"PERF_A_{i:05d}", f"性能测试项目A{i}")

        # 为租户B创建100个项目
        for i in range(100):
            create_project(db, tenant_b.id, user_b.id, f"PERF_B_{i:05d}", f"性能测试项目B{i}")

        # 测试查询性能
        start_time = time.time()

        if hasattr(Project, "tenant_id"):
            tenant_a_projects = db.query(Project).filter(Project.tenant_id == tenant_a.id).all()
            ...
            assert query_time < 1.0, f"租户过滤查询性能不达标: {query_time:.3f}秒"

    def test_index_effectiveness_for_tenant_filtering(self, db: Session, tenant_a: Tenant):
        """测试租户ID索引的有效性"""
        ...
        for table_name in tables_to_check:
            try:
                ...
                if has_tenant_id:
                    ...
                    has_tenant_index = any("tenant_id" in idx["column_names"] for idx in indexes)

                    # 建议：tenant_id 应该有索引以提高查询性能
                    if not has_tenant_index:
                        pytest.skip(
                            f"表 {table_name} 的 tenant_id 字段缺少索引，" "可能影响查询性能"
                        )
            except Exception as e:
                # 如果表不存在或其他错误，跳过
                continue
...
class TestTenantDataMigration:
    ...
    def test_legacy_data_without_tenant_id(self, db: Session, superuser: User):
    ...
    def test_tenant_data_export_isolation(self, db: Session, tenant_a: Tenant, tenant_b: Tenant):
        """测试租户数据导出的隔离"""
        # 验证导出功能只导出本租户的数据
        # 这是一个重要的安全测试
        pass

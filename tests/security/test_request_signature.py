...
from app.core.request_signature import RequestSignatureVerifier, generate_client_signature
...
class TestRequestSignature:
    ...
    def test_compute_signature_consistency(self):
        ...
        sig1 = RequestSignatureVerifier.compute_signature(method, path, timestamp, body, secret)
        sig2 = RequestSignatureVerifier.compute_signature(method, path, timestamp, body, secret)
    ...
    def test_compute_signature_different_secret(self):
        ...
        sig1 = RequestSignatureVerifier.compute_signature(method, path, timestamp, body, "secret1")
        sig2 = RequestSignatureVerifier.compute_signature(method, path, timestamp, body, "secret2")
    ...
    def test_verify_expired_signature(self):
        ...
        # 验证签名（应该因过期失败）
        with pytest.raises(Exception) as exc_info:
            RequestSignatureVerifier.verify_signature(request, signature, timestamp, body, secret)
    ...
    def test_generate_client_signature(self):
        ...
        signature, timestamp = generate_client_signature(method, url, body, secret)
    ...
    def _create_mock_request(self, method: str, path: str, query: str = "") -> Request:
        """创建模拟的Request对象"""

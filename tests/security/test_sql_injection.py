...
class TestSQLInjectionProtection:
    ...
    def test_classic_sql_injection(self, client, auth_headers):
        ...
        for payload in payloads:
            response = client.get(f"/api/users/search?q={payload}", headers=auth_headers)

            # 应该返回正常响应或400错误,不应该暴露数据库错误
            assert response.status_code in [200, 400, 404], f"SQL注入防护失败: {payload}"
    ...
    def test_union_based_injection(self, client, auth_headers):
        ...
        for payload in payloads:
            response = client.get(f"/api/projects/{payload}", headers=auth_headers)

            assert response.status_code in [400, 404], f"UNION注入防护失败: {payload}"
    ...
    def test_error_based_injection(self, client, auth_headers):
        ...
        for payload in payloads:
            response = client.get(f"/api/users/{payload}", headers=auth_headers)

            # 不应该暴露数据库版本信息
            if response.status_code == 500:
                text = response.text.lower()
                assert "mysql" not in text and "postgres" not in text, "暴露了数据库信息"
    ...
    def test_parameterized_queries(self, client, auth_headers):
        """测试参数化查询保护"""
        # 正常查询应该成功
        response = client.get("/api/projects/search?q=test", headers=auth_headers)
        assert response.status_code == 200
        ...
        for char in special_chars:
            response = client.get(f"/api/projects/search?q={char}", headers=auth_headers)
            assert response.status_code in [200, 400], f"未正确处理特殊字符: {char}"
    ...
    def test_stored_procedure_injection(self, client, auth_headers):
        ...
        for payload in payloads:
            response = client.post(
                "/api/reports/generate", json={"filter": payload}, headers=auth_headers
            )
            assert response.status_code in [400, 404]
    ...
    def test_nosql_injection(self, client, auth_headers):
        ...
        for payload in payloads:
            response = client.post(
                "/api/users/search", json={"username": payload}, headers=auth_headers
            )

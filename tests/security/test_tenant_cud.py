...
class TestTenantCreateIsolation:
    ...
    def test_created_resource_auto_assigned_to_user_tenant(
        self, client: TestClient, db: Session, user_a: User
    ):
        """测试创建的资源自动分配给用户所属租户"""
        ...
        response = client.post(
            "/api/v1/projects", json=project_data, headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_user_cannot_create_resource_for_other_tenant(
        ...
        if response.status_code in [200, 201]:
            ...
            assert data.get("tenant_id") == user_a.tenant_id, "不应该允许用户为其他租户创建资源"

    def test_superuser_can_create_for_any_tenant(
        self, client: TestClient, db: Session, superuser: User, tenant_a: Tenant, tenant_b: Tenant
    ):
        """测试超级管理员可以为任意租户创建资源"""
        from app.core.security import create_access_token
        ...
        response_a = client.post(
            "/api/v1/projects", json=project_data_a, headers={"Authorization": f"Bearer {token}"}
        )

        if response_a.status_code in [200, 201]:
            data_a = response_a.json()
            assert data_a.get("tenant_id") == tenant_a.id, "超级管理员应该能为租户A创建资源"
        ...
        response_b = client.post(
            "/api/v1/projects", json=project_data_b, headers={"Authorization": f"Bearer {token}"}
        )

        if response_b.status_code in [200, 201]:
            data_b = response_b.json()
            assert data_b.get("tenant_id") == tenant_b.id, "超级管理员应该能为租户B创建资源"
...
class TestTenantUpdateIsolation:
    ...
    def test_user_cannot_update_other_tenant_project(
        self, client: TestClient, db: Session, user_a: User, project_b: Project
    ):
        """测试用户无法更新其他租户的项目"""
        from app.core.security import create_access_token
        ...
        # 尝试更新租户B的项目
        update_data = {"project_name": "尝试修改租户B项目"}
        ...
        assert project_b.project_name != "尝试修改租户B项目", "其他租户的数据不应该被修改"

    def test_user_can_update_own_tenant_project(
        self, client: TestClient, db: Session, user_a: User, project_a: Project
    ):
        """测试用户可以更新本租户的项目"""
        from app.core.security import create_access_token
        ...
        update_data = {"project_name": new_name}
        ...
        if response.status_code == 200:
            data = response.json()
            assert data.get("project_name") == new_name, "应该能更新自己租户的项目"

            # 验证租户ID没有被修改
            assert data.get("tenant_id") == user_a.tenant_id, "更新时租户ID不应该改变"

    def test_user_cannot_change_tenant_id_on_update(
        self, client: TestClient, db: Session, user_a: User, project_a: Project, tenant_b: Tenant
    ):
        """测试用户无法通过更新修改资源的租户ID"""
        from app.core.security import create_access_token
        ...
        # 尝试修改租户ID
        update_data = {"tenant_id": tenant_b.id, "project_name": "尝试转移租户"}  # 尝试改成租户B
        ...
        if response.status_code == 200:
            data = response.json()
            # 租户ID应该保持不变
            assert data.get("tenant_id") == original_tenant_id, "不应该允许用户修改资源的租户ID"

        # 验证数据库中的租户ID没有改变
        db.refresh(project_a)
        assert project_a.tenant_id == original_tenant_id, "数据库中的租户ID不应该被修改"
...
class TestTenantDeleteIsolation:
    ...
    def test_user_cannot_delete_other_tenant_project(
        ...
        response = client.delete(
            f"/api/v1/projects/{project_b.id}", headers={"Authorization": f"Bearer {token}"}
        )
        ...
        db.refresh(project_b)
        assert (
            db.query(Project).filter(Project.id == project_b.id).first() is not None
        ), "其他租户的数据不应该被删除"
    ...
    def test_user_can_delete_own_tenant_project(
        ...
        test_project = create_project(
            db, user_a.tenant_id, user_a.id, "DELETE_TEST_001", "待删除测试项目"
        )

        token = create_access_token(user_a.id)

        response = client.delete(
            f"/api/v1/projects/{test_project.id}", headers={"Authorization": f"Bearer {token}"}
        )

        # 应该成功或返回204
        if response.status_code in [200, 204]:
            # 验证数据已被删除（软删除或硬删除）
            deleted_project = db.query(Project).filter(Project.id == test_project.id).first()

            # 如果是软删除，应该有 deleted_at 字段
            # 如果是硬删除，应该查询不到
            if deleted_project:
                assert (
                    hasattr(deleted_project, "deleted_at")
                    and deleted_project.deleted_at is not None
    ...
    def test_superuser_can_delete_any_tenant(
        ...
        project_a = create_project(
            db, tenant_a.id, superuser.id, "SUPER_DEL_A_001", "超管删除租户A项目"
        )
        project_b = create_project(
            db, tenant_b.id, superuser.id, "SUPER_DEL_B_001", "超管删除租户B项目"
        )
        ...
        response_a = client.delete(
            f"/api/v1/projects/{project_a.id}", headers={"Authorization": f"Bearer {token}"}
        )
        assert response_a.status_code in [200, 204], "超级管理员应该能删除租户A的项目"

        # 删除租户B的项目
        response_b = client.delete(
            f"/api/v1/projects/{project_b.id}", headers={"Authorization": f"Bearer {token}"}
        )
        assert response_b.status_code in [200, 204], "超级管理员应该能删除租户B的项目"

    def test_batch_delete_respects_tenant_isolation(
        self, client: TestClient, db: Session, user_a: User, multiple_projects_b: list
    ):
        """测试批量删除也遵循租户隔离"""
        from app.core.security import create_access_token
        ...
        for project_id in project_ids:
            ...
            assert project is not None, f"其他租户的项目不应该被删除，项目ID: {project_id}"
...
class TestTenantCascadeOperations:
    ...
    def test_delete_with_cascade_respects_tenant_isolation(
        self, db: Session, user_a: User, project_b: Project
    ):
        """测试级联删除也遵循租户隔离"""
        from app.models.task import Task

        # 为租户B的项目创建任务
        task = Task(project_id=project_b.id, task_name="租户B任务", created_by=project_b.created_by)
        db.add(task)
        db.commit()

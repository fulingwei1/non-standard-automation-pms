...
class TestBasicTenantIsolation:
    ...
    def test_list_projects_only_returns_same_tenant(
        ...
        for item in items:
            assert (
                item.get("tenant_id") == tenant_a_id
    ...
    def test_database_level_isolation(
        self, db: Session, user_a: User, project_a: Project, project_b: Project
    ):
        """测试数据库层面的租户隔离"""
        # 直接查询数据库（模拟无过滤器的场景）
        all_projects = db.query(Project).filter(Project.tenant_id == user_a.tenant_id).all()
        ...
        assert project_b.id not in project_ids, "数据库查询不应该包含其他租户的项目"
    ...
    def test_tenant_admin_isolation(
        ...
        assert response_other.status_code in [403, 404], "租户管理员不应该能访问其他租户的项目"

    def test_null_tenant_id_handling(self, db: Session, superuser: User):
        ...
        # 普通用户不应该能访问系统级项目
        user_projects = db.query(Project).filter(Project.tenant_id == 1).all()  # 假设租户1

        project_ids = [p.id for p in user_projects]
        assert system_project.id not in project_ids, "普通用户不应该能访问系统级项目"

        # 超级管理员可以访问所有项目（包括系统级）
        all_projects = db.query(Project).all()
        all_project_ids = [p.id for p in all_projects]
        assert system_project.id in all_project_ids, "超级管理员应该能访问系统级项目"


class TestTenantIsolationEdgeCases:
    """租户隔离边界情况测试"""

    def test_inactive_tenant_access(self, client: TestClient, db: Session, tenant_c: Tenant):
        """测试暂停租户的访问控制"""
        from app.core.security import create_access_token
        from tests.fixtures.tenant_fixtures import create_project, create_user, tenant_c

        # 创建暂停租户的用户和项目
        user_c = create_user(db, "user_c", tenant_c.id)
        project_c = create_project(db, tenant_c.id, user_c.id, "PROJ_C_001", "租户C项目")

        token = create_access_token(user_c.id)

        # 尝试访问暂停租户的项目
        response = client.get(
            f"/api/v1/projects/{project_c.id}", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_expired_tenant_access(self, db: Session, tenant_a: Tenant):
        """测试过期租户的访问控制"""
    ...
    def test_tenant_id_mismatch_in_nested_resources(
        ...
        # 创建租户B项目的任务
        task_b = Task(
            project_id=project_b.id, task_name="租户B任务", created_by=project_b.created_by
        )
        db.add(task_b)
        db.commit()

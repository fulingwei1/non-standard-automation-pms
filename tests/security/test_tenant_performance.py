...
pytestmark = pytest.mark.skipif(
    ...
    reason="Project 模型当前不包含 tenant_id，项目级租户隔离性能测试暂不适用",
)
...
class TestTenantQueryPerformance:
    ...
    def test_single_tenant_query_performance(self, db: Session, tenant_a: Tenant):
        """测试单租户查询性能"""
        # 创建测试数据
        ...
        if hasattr(Project, "tenant_id"):
            results = db.query(Project).filter(Project.tenant_id == tenant_a.id).all()
        else:
        ...
        assert query_time < 2.0, f"单租户查询性能不达标: {query_time:.3f}秒 (期望 < 2.0秒)"

        print(f"\n✓ 单租户查询1000条记录耗时: {query_time:.3f}秒")

    def test_multi_tenant_query_performance(self, db: Session):
        """测试多租户环境下的查询性能"""
        # 创建10个租户，每个租户100个项目
        tenants = []
        for i in range(10):
            tenant = create_tenant(db, f"perf_tenant_{i}", f"性能测试租户{i}")
            ...
            # 批量创建项目
            for j in range(100):
                create_project(db, tenant.id, user.id, f"MULTI_{i}_{j:05d}", f"多租户项目{i}-{j}")
        ...
        for tenant in tenants:
            ...
            if hasattr(Project, "tenant_id"):
                results = db.query(Project).filter(Project.tenant_id == tenant.id).all()
            else:
                results = db.query(Project).all()
        ...
        # 平均查询时间应该在0.5秒内
        assert avg_time < 0.5, f"多租户平均查询性能不达标: {avg_time:.3f}秒 (期望 < 0.5秒)"

        print(f"\n✓ 多租户环境（10个租户，共1000条记录）平均查询耗时: {avg_time:.3f}秒")

    def test_tenant_filtering_with_pagination(self, db: Session, tenant_a: Tenant):
        """测试分页查询的租户过滤性能"""
        user = create_user(db, "page_user", tenant_a.id)

        # 创建500个项目
        for i in range(500):
            create_project(db, tenant_a.id, user.id, f"PAGE_{i:06d}", f"分页测试项目{i}")
        ...
        for page in range(total_pages):
            ...
            if hasattr(Project, "tenant_id"):
                results = (
                    db.query(Project)
                    .filter(Project.tenant_id == tenant_a.id)
                    .offset(page * page_size)
                    .limit(page_size)
                    .all()
                )
            else:
                results = db.query(Project).offset(page * page_size).limit(page_size).all()
        ...
        # 单页查询应该在0.1秒内完成
        assert avg_page_time < 0.1, f"分页查询性能不达标: {avg_page_time:.3f}秒 (期望 < 0.1秒)"

        print(f"\n✓ 分页查询（500条记录，20条/页）平均耗时: {avg_page_time:.3f}秒")

    def test_complex_query_with_tenant_filter(self, db: Session, tenant_a: Tenant):
        """测试复杂查询（JOIN + 过滤）的性能"""
        user = create_user(db, "complex_user", tenant_a.id)

        # 创建300个项目
        for i in range(300):
            create_project(db, tenant_a.id, user.id, f"COMPLEX_{i:06d}", f"复杂查询测试{i}")
        ...
        if hasattr(Project, "tenant_id"):
            ...
            results = db.query(Project).join(User, Project.created_by == User.id).all()

        end_time = time.time()
        query_time = end_time - start_time

        # 复杂查询应该在1秒内完成
        assert query_time < 1.0, f"复杂查询性能不达标: {query_time:.3f}秒 (期望 < 1.0秒)"
...
class TestTenantIndexPerformance:
    ...
    def test_tenant_id_index_usage(self, db: Session, tenant_a: Tenant):
        """测试tenant_id索引的使用情况"""
        ...
        for i in range(1000):
            create_project(db, tenant_a.id, user.id, f"INDEX_{i:06d}", f"索引测试{i}")
    ...
    def test_composite_index_performance(self, db: Session, tenant_a: Tenant):
        ...
        if hasattr(Project, "tenant_id"):
            results = (
                db.query(Project)
                .filter(Project.tenant_id == tenant_a.id, Project.status == "IN_PROGRESS")
                .all()
            )
        else:
            results = db.query(Project).filter(Project.status == "IN_PROGRESS").all()

        end_time = time.time()
        query_time = end_time - start_time

        # 组合查询应该在0.2秒内完成
        assert query_time < 0.2, f"组合索引查询性能不达标: {query_time:.3f}秒 (期望 < 0.2秒)"
...
class TestTenantConcurrentAccess:
    ...
    def test_concurrent_tenant_queries(self, db: Session):
        """测试多租户并发查询"""
        import threading
        ...
        for i in range(5):
            ...
            for j in range(100):
                create_project(db, tenant.id, user.id, f"CONC_{i}_{j:05d}", f"并发项目{i}-{j}")
        ...
        def query_tenant(tenant_id: int):
            try:
                ...
                start = time.time()
                if hasattr(Project, "tenant_id"):
                    items = local_db.query(Project).filter(Project.tenant_id == tenant_id).all()
                else:
...
class TestTenantMemoryUsage:
    ...
    def test_large_result_set_memory(self, db: Session, tenant_a: Tenant):
        """测试大结果集的内存使用"""
        import sys
    ...
    def test_streaming_query_performance(self, db: Session, tenant_a: Tenant):
        """测试流式查询性能（减少内存占用）"""
        user = create_user(db, "stream_user", tenant_a.id)

        # 创建1000个项目
        for i in range(1000):
            create_project(db, tenant_a.id, user.id, f"STREAM_{i:06d}", f"流式查询测试{i}")
        ...
        if hasattr(Project, "tenant_id"):
            query = db.query(Project).filter(Project.tenant_id == tenant_a.id).yield_per(100)
        else:
...
class TestTenantAPIPerformance:
    ...
    def test_api_list_endpoint_performance(self, client: TestClient, db: Session, tenant_a: Tenant):
        """测试API列表端点的性能"""
        ...
        for i in range(200):
            create_project(db, tenant_a.id, user.id, f"API_{i:06d}", f"API性能测试{i}")
        ...
        response = client.get(
            "/api/v1/projects?page=1&page_size=50", headers={"Authorization": f"Bearer {token}"}
        )
        ...
        # API响应应该在1秒内完成
        assert api_time < 1.0, f"API查询性能不达标: {api_time:.3f}秒 (期望 < 1.0秒)"

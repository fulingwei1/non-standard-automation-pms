...
class TestProjectCrudService:
    ...
    def sample_customer(self):
        """示例客户"""
        return Customer(
            id=1, customer_name="测试客户", contact_person="张三", contact_phone="13800138000"
        )
    ...
    def sample_user(self):
        """示例用户"""
        return User(
            id=10, username="pm_wang", real_name="王项目经理", password_hash="test_hash_123"
        )
    ...
    def test_create_project_duplicate_code(self, service, mock_db, sample_project):
        """测试创建项目时编码重复"""
        project_in = ProjectCreate(
            project_code="PRJ001", project_name="重复项目", customer_id=1, pm_id=10  # 已存在的编码
        )
    ...
    def test_update_project(self, service, mock_db, sample_project):
        """测试更新项目"""
        update_data = {"project_name": "更新后的项目名", "progress_pct": 75.0}
    ...
    def test_update_project_with_customer_change(
        self, service, mock_db, sample_project, sample_customer
    ):
        """测试更新项目时更改客户"""
        update_data = {"customer_id": 2}

        new_customer = Customer(
            id=2, customer_name="新客户", contact_person="李四", contact_phone="13900139000"
        )
    ...
    def test_update_customer_redundant_fields(
        self, service, mock_db, sample_project, sample_customer
    ):
        """测试更新客户冗余字段"""
        new_customer = Customer(
            id=2, customer_name="新客户公司", contact_person="赵六", contact_phone="13700137000"
        )
        mock_db.query.return_value.get.return_value = new_customer
    ...
    def test_update_pm_redundant_fields(self, service, mock_db, sample_project):
        """测试更新项目经理冗余字段"""
        new_pm = User(id=20, username="new_pm", real_name="新经理", password_hash="test_hash_123")
    ...
    def test_update_pm_redundant_fields_no_realname(self, service, mock_db, sample_project):
        """测试项目经理没有真实姓名时使用用户名"""
        new_pm = User(
            id=20, username="username_only", real_name=None, password_hash="test_hash_123"
        )
        mock_db.query.return_value.get.return_value = new_pm
        sample_project.pm_id = 20

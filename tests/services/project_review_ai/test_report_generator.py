...
class TestExtractProjectData:
    ...
    def test_extract_complete_project_data(
        self, generator, mock_db, sample_project, sample_timesheets, sample_costs, sample_changes
    ):
        """测试提取完整项目数据"""
        # 设置mock查询 - 项目查询
        project_query = Mock()
    ...
    def test_extract_cost_variance_over_budget(self, generator, mock_db, sample_project):
        ...
        costs = [Mock(spec=ProjectCost, project_id=1, amount=Decimal("60000"))]
...
class TestFormatChanges:
    ...
    def test_format_single_change(self, generator):
        """测试单个变更"""
        changes = [
            {"title": "需求变更", "type": "REQUIREMENT", "impact": "HIGH", "status": "APPROVED"}
        ]
...
class TestParseAIResponse:
    ...
    def test_parse_valid_json(self, generator):
        ...
        project_data = {
            "project": {"id": 1, "code": "PRJ-001", "budget": 100000.0},
            "statistics": {
                "plan_duration": 30,
                "actual_duration": 28,
                "schedule_variance": -2,
    ...
    def test_parse_invalid_json(self, generator):
        """测试解析无效JSON（降级为纯文本）"""
        ai_response = {"content": "这是一段普通文本，不是JSON格式"}

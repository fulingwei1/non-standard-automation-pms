...
class TestCreatePrediction:
    ...
    def test_create_prediction_traditional_method(
        ...
        prediction = service.create_prediction(
            project_id=sample_project.id, use_ai=False, created_by=1, notes="测试预测"
        )
    ...
    def test_create_prediction_ai_method(
        ...
        prediction = service.create_prediction(
            project_id=sample_project.id, use_ai=True, prediction_version="V2.0"
        )
    ...
    def test_create_prediction_with_high_risk_generates_suggestions(
        ...
        prediction = service.create_prediction(project_id=sample_project.id, use_ai=True)

        assert prediction.risk_level == "HIGH"

        # 验证生成了优化建议
        suggestions = (
            db_session.query(CostOptimizationSuggestion)
            .filter(CostOptimizationSuggestion.prediction_id == prediction.id)
            .all()
        )
    ...
    def test_create_prediction_saves_cpi_trend_data(
        ...
        prediction = service.create_prediction(project_id=sample_project.id, use_ai=False)
...
class TestGetLatestPrediction:
    ...
    def test_get_latest_prediction_exists(self, db_session, sample_project, sample_predictions):
        """测试获取最新预测 - 存在预测"""
        service = ProjectCostPredictionService(db_session)
...
class TestGetPredictionHistory:
    ...
    def test_get_prediction_history_no_limit(self, db_session, sample_project, sample_predictions):
        """测试获取全部历史"""
        service = ProjectCostPredictionService(db_session)
...
class TestPrivateMethods:
    ...
    def test_calculate_health_score_critical_risk(self, db_session, sample_critical_prediction):
        """测试严重风险项目的健康评分"""
        service = ProjectCostPredictionService(db_session)
        suggestions_summary = {"pending": 0, "approved": 0, "in_progress": 0}

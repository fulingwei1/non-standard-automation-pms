...
class TestInventoryManagementService:
    ...
    def setup_test_data(self, db: Session, test_tenant):
        ...
        return {"material": material, "category": category, "tenant_id": test_tenant.id}
    ...
    def test_weighted_avg_cost(self, db: Session, setup_test_data):
        ...
        stock = result["stock"]
        expected_price = (
            Decimal("100") * Decimal("50") + Decimal("200") * Decimal("60")
        ) / Decimal("300")
    ...
    def test_insufficient_stock_error(self, db: Session, setup_test_data):
        ...
        with pytest.raises(InsufficientStockError):
            service.issue_material(
                material_id=material.id, quantity=Decimal("100"), location="仓库A"
            )
    ...
    def test_issue_material_fifo(self, db: Session, setup_test_data):
        ...
        result = service.issue_material(
            material_id=material.id, quantity=Decimal("150"), location="仓库A", cost_method="FIFO"
        )
    ...
    def test_issue_material_lifo(self, db: Session, setup_test_data):
        ...
        # LIFO出库
        result = service.issue_material(
            material_id=material.id, quantity=Decimal("150"), location="仓库A", cost_method="LIFO"
        )
    ...
    def test_return_material(self, db: Session, setup_test_data):
        ...
        service.issue_material(material_id=material.id, quantity=Decimal("50"), location="仓库A")

        # 退料
        result = service.return_material(
            material_id=material.id, quantity=Decimal("20"), location="仓库A"
        )
    ...
    def test_reserve_material(self, db: Session, setup_test_data):
        ...
        reservation = service.reserve_material(
            material_id=material.id, quantity=Decimal("30"), project_id=1
        )
    ...
    def test_cancel_reservation(self, db: Session, setup_test_data):
        ...
        reservation = service.reserve_material(material_id=material.id, quantity=Decimal("30"))

        # 取消预留
        service.cancel_reservation(reservation_id=reservation.id, cancel_reason="测试取消")
    ...
    def test_reserve_insufficient_stock(self, db: Session, setup_test_data):
        ...
        # 尝试预留超过可用库存的数量
        with pytest.raises(InsufficientStockError):
            service.reserve_material(material_id=material.id, quantity=Decimal("100"))
    ...
    def test_calculate_turnover_rate(self, db: Session, setup_test_data):
        ...
        # 出库
        service.issue_material(material_id=material.id, quantity=Decimal("600"), location="仓库A")
    ...
    def test_get_available_quantity(self, db: Session, setup_test_data):
        ...
        # 预留部分
        service.reserve_material(material_id=material.id, quantity=Decimal("30"))

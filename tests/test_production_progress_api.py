...
class TestRealtimeProgressAPI:
    ...
    def test_get_realtime_overview(self, auth_headers, test_work_order):
        ...
        response = client.get("/api/v1/production/progress/realtime", headers=auth_headers)
...
class TestWorkOrderTimelineAPI:
    ...
    def test_get_nonexistent_work_order(self, auth_headers):
        """测试获取不存在的工单"""
        response = client.get(
            "/api/v1/production/progress/work-orders/99999/timeline", headers=auth_headers
        )
...
class TestProgressLogAPI:
    ...
    def test_create_progress_log(self, auth_headers, test_work_order, db: Session):
        ...
        response = client.post(
            "/api/v1/production/progress/log", json=log_data, headers=auth_headers
        )
    ...
    def test_create_log_invalid_work_order(self, auth_headers):
        """测试创建无效工单的日志"""
        log_data = {"work_order_id": 99999, "current_progress": 50, "status": "IN_PROGRESS"}
...
class TestBottleneckAPI:
    ...
    def test_get_bottlenecks(self, auth_headers, test_bottleneck_workstation):
        ...
        response = client.get("/api/v1/production/progress/bottlenecks", headers=auth_headers)
    ...
    def test_get_bottlenecks_with_level_filter(self, auth_headers):
        """测试按等级筛选瓶颈"""
        response = client.get(
            "/api/v1/production/progress/bottlenecks?min_level=2", headers=auth_headers
        )
    ...
    def test_get_bottlenecks_with_limit(self, auth_headers):
        """测试限制返回数量"""
        response = client.get(
            "/api/v1/production/progress/bottlenecks?limit=5", headers=auth_headers
        )
...
class TestAlertAPI:
    ...
    def test_get_alerts(self, auth_headers, test_alert):
        ...
        response = client.get("/api/v1/production/progress/alerts", headers=auth_headers)
    ...
    def test_dismiss_alert(self, auth_headers, test_alert, db: Session):
        ...
        dismiss_data = {"resolution_note": "已处理完成"}
    ...
    def test_dismiss_nonexistent_alert(self, auth_headers):
        """测试关闭不存在的预警"""
        response = client.post(
            "/api/v1/production/progress/alerts/99999/dismiss", json={}, headers=auth_headers
        )
...
class TestDeviationAPI:
    ...
    def test_get_deviation_analysis(self, auth_headers, test_delayed_work_order):
        ...
        response = client.get("/api/v1/production/progress/deviation", headers=auth_headers)
...
class TestPermissions:
    ...
    def test_read_permission_required(self, db: Session):
        ...
        # 创建无权限用户
        user = User(
            username="no_perm_user", email="noperm@test.com", password_hash="hashed", is_active=True
        )
        db.add(user)
        db.commit()

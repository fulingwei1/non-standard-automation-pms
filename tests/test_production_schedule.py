...
try:
    ...
    pytest.skip(
        "Production schedule models not available (ResourceConflict renamed)",
...
class TestProductionScheduleService:
    ...
    def setup_test_data(self, db: Session):
        ...
        # 创建车间
        workshop = Workshop(
            workshop_code="WS001", workshop_name="测试车间", workshop_type="MACHINING"
        )
        db.add(workshop)
        db.flush()
        ...
        # 创建工序
        process = ProcessDict(process_code="P001", process_name="组装", process_type="ASSEMBLY")
        db.add(process)
        db.flush()
...
class TestProductionSchedulePerformance:
    ...
    def setup_large_dataset(self, db: Session):
        ...
        # 创建工序
        process = ProcessDict(process_code="P001", process_name="加工", process_type="MACHINING")
        db.add(process)
        db.flush()
        ...
        return {"workshop": workshop, "work_orders": work_order_list, "process": process}
...
class TestProductionScheduleAPI:
    ...
    def test_generate_schedule_api(self, client, db: Session, auth_headers):
        ...
        # 准备数据
        workshop = Workshop(
            workshop_code="WS001", workshop_name="测试车间", workshop_type="ASSEMBLY"
        )
        db.add(workshop)
        db.flush()
    ...
    def test_preview_schedule_api(self, client, db: Session, auth_headers):
        ...
        response = client.get(
            f"/api/v1/production/schedule/preview?plan_id=1001", headers=auth_headers
        )
    ...
    def test_conflicts_api(self, client, db: Session, auth_headers):
        """测试冲突检测API"""
        response = client.get(
            "/api/v1/production/schedule/conflicts?plan_id=1001", headers=auth_headers
        )
    ...
    def test_gantt_data_api(self, client, db: Session, auth_headers):
        """测试甘特图数据API"""
        response = client.get(
            "/api/v1/production/schedule/gantt?plan_id=1001", headers=auth_headers
        )

...
class TestProjectReviewReportGenerator:
    ...
    def test_generate_report(self, db: Session, sample_project):
        ...
        result = generator.generate_report(project_id=sample_project.id, review_type="POST_MORTEM")
...
class TestProjectLessonExtractor:
    ...
    def test_extract_lessons(self, db: Session, sample_review):
        ...
        lessons = extractor.extract_lessons(review_id=sample_review.id, min_confidence=0.5)
...
class TestProjectComparisonAnalyzer:
    ...
    def test_compare_with_history(self, db: Session, sample_review):
        ...
        result = analyzer.compare_with_history(
            review_id=sample_review.id, similarity_type="type", limit=3
        )
...
class TestProjectKnowledgeSyncer:
    ...
    def test_sync_to_knowledge_base(self, db: Session, sample_review):
        ...
        result = syncer.sync_to_knowledge_base(review_id=sample_review.id, auto_publish=True)
        ...
        # 验证知识库记录已创建
        case = (
            db.query(PresaleKnowledgeCase)
            .filter(PresaleKnowledgeCase.id == result["knowledge_case_id"])
            .first()
        )
...
class TestEndToEndWorkflow:
    ...
    def test_full_workflow(self, db: Session, sample_project):
        ...
        review = ProjectReview(
            review_no="REV_TEST_001", reviewer_id=1, reviewer_name="测试员", **report_data
        )
        db.add(review)
        db.commit()
        db.refresh(review)
        ...
        case = (
            db.query(PresaleKnowledgeCase)
            .filter(PresaleKnowledgeCase.id == sync_result["knowledge_case_id"])
            .first()
        )
        assert case is not None

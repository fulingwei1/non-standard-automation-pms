...
class TestQualityInspection:
    ...
    def test_create_inspection(self, db: Session, test_user: User, test_material: Material):
        ...
        inspection = QualityService.create_inspection(
            db=db, inspection_data=inspection_data, current_user_id=test_user.id
        )
...
class TestSPCControl:
    ...
    def test_spc_out_of_control_detection(
        self, db: Session, test_user: User, test_material: Material
    ):
        """测试失控点检测"""
        # 创建正常数据和异常数据
        ...
        abnormal_inspection = QualityService.create_inspection(
            db, inspection_data_abnormal, test_user.id
        )
...
class TestParetoAnalysis:
    ...
    def test_pareto_top_defects(self, db: Session, test_user: User):
        """测试帕累托Top不良类型识别"""
        # 创建不同不良类型的数据
        defect_types = [("划伤", 50), ("变形", 30), ("尺寸偏差", 15), ("表面瑕疵", 3), ("其他", 2)]
...
class TestReworkOrder:
    ...
    def test_complete_rework_order(self, db: Session, test_user: User, test_work_order: WorkOrder):
        ...
        completed_rework = QualityService.complete_rework_order(
            db, rework_order.id, completion_data
        )
    ...
    def test_complete_already_completed_rework(
        self, db: Session, test_user: User, test_work_order: WorkOrder
    ):
        """测试重复完成返工单"""
        rework_data = {
            "original_work_order_id": test_work_order.id,
...
class TestQualityWorkflow:
    ...
    def test_quality_inspection_to_rework_flow(
        self, db: Session, test_user: User, test_work_order: WorkOrder
    ):

...
def test_template(db: Session):
    ...
    template = ReportTemplate(
        ...
        description="测试用模板",
        config={"fields": ["user_name", "total_hours", "work_days"], "filters": {}},
...
def test_generate_user_monthly_report(db: Session, test_template):
    """测试生成人员月度报表"""
    try:
        data = ReportService.generate_report(db=db, template_id=test_template.id, period="2026-01")
...
def test_generate_dept_monthly_report(db: Session):
    ...
    try:
        data = ReportService.generate_report(db=db, template_id=template.id, period="2026-01")
...
def test_generate_report_with_invalid_template(db: Session):
    ...
    with pytest.raises(ValueError, match="报表模板不存在"):
        ReportService.generate_report(db=db, template_id=99999, period="2026-01")
...
def test_export_to_excel_basic(tmp_path):
    ...
    data = {
        ...
        "month": 1,
        "summary": [
            {"user_name": "张三", "department": "研发部", "total_hours": 160.0, "work_days": 20}
        ],
        "detail": [],
    }
    ...
    file_path = ReportExcelService.export_to_excel(
        data=data, template_name="测试报表", output_dir=output_dir
    )
...
def test_export_with_detail_sheet(tmp_path):
    ...
    data = {
        ...
        "year": 2026,
        "month": 1,
        "summary": [{"user_name": "张三", "total_hours": 160.0}],
        "detail": [{"user_name": "张三", "work_date": "2026-01-10", "hours": 8.0}],
    }

    output_dir = str(tmp_path)
    file_path = ReportExcelService.export_to_excel(
        data=data, template_name="明细报表", output_dir=output_dir
    )
...
def test_export_with_chart_sheet(tmp_path):
    ...
    output_dir = str(tmp_path)
    file_path = ReportExcelService.export_to_excel(
        data=data, template_name="图表报表", output_dir=output_dir
    )
...
def test_export_empty_data(tmp_path):
    ...
    data = {"period": "2026-01", "year": 2026, "month": 1, "summary": [], "detail": []}

    output_dir = str(tmp_path)
    file_path = ReportExcelService.export_to_excel(
        data=data, template_name="空报表", output_dir=output_dir
    )

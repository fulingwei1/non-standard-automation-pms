...
def test_detect_conflicts_success(
    ...
    service = ResourceSchedulingAIService(db_session)
...
def test_list_conflicts(db: Session, test_user: User):
    ...
    # 查询
    conflicts = (
        db.query(ResourceConflictDetection)
        .filter(ResourceConflictDetection.severity == "HIGH")
        .all()
    )
...
def test_update_conflict_resolve(db: Session):
    ...
    updated = (
        db.query(ResourceConflictDetection)
        .filter(ResourceConflictDetection.id == conflict.id)
        .first()
    )
...
def test_dashboard_summary_with_data(db: Session):
    ...
    critical_conflicts = (
        db.query(func.count(ResourceConflictDetection.id))

...
class TestSalesRegionService:
    ...
    def test_create_duplicate_region_code(self, db: Session):
        """测试创建重复区域编码"""
        region_data = SalesRegionCreate(region_code="R002", region_name="华南区")
    ...
    def test_get_region(self, db: Session):
        """测试获取区域详情"""
        region_data = SalesRegionCreate(region_code="R003", region_name="华北区")
    ...
    def test_update_region(self, db: Session):
        """测试更新区域"""
        region_data = SalesRegionCreate(region_code="R004", region_name="西南区")

        region = SalesRegionService.create_region(db, region_data, created_by=1)

        update_data = SalesRegionUpdate(region_name="西南大区", description="覆盖四川、重庆等地")
    ...
    def test_assign_team(self, db: Session):
        """测试分配团队"""
        # 创建区域
        region_data = SalesRegionCreate(region_code="R005", region_name="华中区")
        region = SalesRegionService.create_region(db, region_data, created_by=1)

        # 创建团队
        team_data = SalesTeamCreate(team_code="T801", team_name="华中团队", team_type="REGION")
    ...
    def test_hierarchical_regions(self, db: Session):
        """测试层级区域"""
        # 创建父区域
        parent_data = SalesRegionCreate(region_code="R100", region_name="中国区", level=1)
        parent = SalesRegionService.create_region(db, parent_data, created_by=1)

        # 创建子区域
        sub_data = SalesRegionCreate(
            region_code="R101", region_name="华东省", parent_region_id=parent.id, level=2
        )
    ...
    def test_get_regions_list(self, db: Session):
        ...
        # 创建多个区域
        for i in range(5):
            region_data = SalesRegionCreate(region_code=f"R20{i}", region_name=f"区域{i}")
            SalesRegionService.create_region(db, region_data, created_by=1)

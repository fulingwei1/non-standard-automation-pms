...
class TestSalesTargetService:
    ...
    def test_create_team_target(self, db: Session):
        """测试创建团队目标"""
        # 先创建团队
        team_data = SalesTeamCreate(team_code="T301", team_name="销售一部", team_type="REGION")
    ...
    def test_update_target(self, db: Session):
        ...
        update_data = SalesTargetV2Update(
            sales_target=Decimal("6000000"), actual_sales=Decimal("3000000")
        )
    ...
    def test_manual_breakdown(self, db: Session):
        ...
        breakdown_data = TargetBreakdownRequest(
            breakdown_items=[
                TargetBreakdownItem(
                    target_type="team", team_id=team1.id, sales_target=Decimal("6000000")
                ),
                TargetBreakdownItem(
                    target_type="team", team_id=team2.id, sales_target=Decimal("4000000")
                ),
            ]
        )

        sub_targets = SalesTargetService.breakdown_target(
            db, target.id, breakdown_data, created_by=1
        )
    ...
    def test_get_breakdown_tree(self, db: Session):
        ...
        breakdown_data = TargetBreakdownRequest(
            breakdown_items=[
                TargetBreakdownItem(
                    target_type="team", team_id=team.id, sales_target=Decimal("5000000")
                )
            ]
        )
...
class TestSalesTargetStatistics:
    ...
    def test_team_ranking(self, db: Session):
        ...
        for i in range(3):
            team_data = SalesTeamCreate(
                team_code=f"T70{i}", team_name=f"RankTeam{i}", team_type="REGION"
            )
            team = SalesTeamService.create_team(db, team_data, created_by=1)

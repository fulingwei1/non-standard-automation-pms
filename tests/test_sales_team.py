...
class TestSalesTeamService:
    ...
    def test_create_duplicate_team_code(self, db: Session):
        """测试创建重复团队编码"""
        team_data = SalesTeamCreate(team_code="T002", team_name="华南团队", team_type="REGION")
    ...
    def test_get_team(self, db: Session):
        """测试获取团队详情"""
        team_data = SalesTeamCreate(team_code="T003", team_name="华北团队", team_type="REGION")
    ...
    def test_update_team(self, db: Session):
        """测试更新团队"""
        team_data = SalesTeamCreate(team_code="T004", team_name="西南团队", team_type="REGION")

        team = SalesTeamService.create_team(db, team_data, created_by=1)

        update_data = SalesTeamUpdate(team_name="西南大区", description="负责西南区域销售业务")
    ...
    def test_delete_team(self, db: Session):
        """测试删除团队"""
        team_data = SalesTeamCreate(team_code="T005", team_name="西北团队", team_type="REGION")
    ...
    def test_get_team_tree(self, db: Session):
        """测试获取团队组织树"""
        # 创建父团队
        parent_data = SalesTeamCreate(team_code="T100", team_name="销售总部", team_type="REGION")
        parent = SalesTeamService.create_team(db, parent_data, created_by=1)

        # 创建子团队
        sub_data = SalesTeamCreate(
            team_code="T101", team_name="华东分部", team_type="REGION", parent_team_id=parent.id
        )
...
class TestSalesTeamMember:
    ...
    def test_add_member(self, db: Session):
        """测试添加团队成员"""
        # 先创建团队
        team_data = SalesTeamCreate(team_code="T201", team_name="产品团队", team_type="PRODUCT")
        team = SalesTeamService.create_team(db, team_data, created_by=1)

        # 添加成员
        member_data = SalesTeamMemberCreate(team_id=team.id, user_id=1, role="MEMBER")
    ...
    def test_remove_member(self, db: Session):
        """测试移除团队成员"""
        team_data = SalesTeamCreate(team_code="T202", team_name="行业团队", team_type="INDUSTRY")
        team = SalesTeamService.create_team(db, team_data, created_by=1)

        member_data = SalesTeamMemberCreate(team_id=team.id, user_id=2, role="MEMBER")
    ...
    def test_update_member_role(self, db: Session):
        """测试更新成员角色"""
        team_data = SalesTeamCreate(team_code="T203", team_name="大客户团队", team_type="OTHER")
        team = SalesTeamService.create_team(db, team_data, created_by=1)

        member_data = SalesTeamMemberCreate(team_id=team.id, user_id=3, role="MEMBER")
        SalesTeamService.add_member(db, member_data)

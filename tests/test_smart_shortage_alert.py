...
class TestSmartAlertEngine:
    ...
    def test_score_solution_feasibility(self, db: Session, test_alert):
        ...
        plan = ShortageHandlingPlan(
            alert_id=test_alert.id, solution_type="URGENT_PURCHASE", plan_no="TEST001"
        )
    ...
    def test_score_solution_risk(self, db: Session, test_alert):
        ...
        plan = ShortageHandlingPlan(
            alert_id=test_alert.id, solution_type="URGENT_PURCHASE", plan_no="TEST004", risks=[]
        )
...
class TestDemandForecastEngine:
    ...
    def test_calculate_confidence_interval(self, db: Session):
        ...
        lower, upper = engine._calculate_confidence_interval(
            forecast=forecast, std=std, confidence=95.0
        )
    ...
    def test_validate_forecast_accuracy(self, db: Session, test_material, test_forecast):
        """测试预测准确率验证"""
        engine = DemandForecastEngine(db)
        ...
        result = engine.validate_forecast_accuracy(
            forecast_id=test_forecast.id, actual_demand=actual_demand
        )
...
class TestShortageAlertAPI:
    ...
    async def test_get_shortage_alerts(self, client, auth_headers):
        """测试获取预警列表"""
        response = await client.get("/api/v1/shortage/smart/alerts", headers=auth_headers)
    ...
    async def test_get_alert_detail(self, client, auth_headers, test_alert):
        ...
        response = await client.get(
            f"/api/v1/shortage/smart/alerts/{test_alert.id}", headers=auth_headers
        )
    ...
    async def test_trigger_scan(self, client, auth_headers):
        ...
        response = await client.post(
            "/api/v1/shortage/smart/scan", headers=auth_headers, json={"days_ahead": 30}
        )
    ...
    async def test_get_handling_solutions(self, client, auth_headers, test_alert):
        """测试获取处理方案"""
        response = await client.get(
            f"/api/v1/shortage/smart/alerts/{test_alert.id}/solutions", headers=auth_headers
        )
    ...
    async def test_resolve_alert(self, client, auth_headers, test_alert):
        ...
        response = await client.post(
            ...
            json={"resolution_type": "PURCHASE", "resolution_note": "已安排采购"},
        )
    ...
    async def test_get_material_forecast(self, client, auth_headers, test_material):
        """测试获取物料预测"""
        response = await client.get(
            f"/api/v1/shortage/smart/forecast/{test_material.id}", headers=auth_headers
        )
    ...
    async def test_get_shortage_trend(self, client, auth_headers):
        """测试缺料趋势分析"""
        response = await client.get(
            "/api/v1/shortage/smart/analysis/trend?days=30", headers=auth_headers
        )
    ...
    async def test_get_root_cause(self, client, auth_headers):
        """测试根因分析"""
        response = await client.get(
            "/api/v1/shortage/smart/analysis/root-cause?days=30", headers=auth_headers
        )
    ...
    async def test_get_project_impact(self, client, auth_headers):
        """测试项目影响分析"""
        response = await client.get("/api/v1/shortage/smart/impact/projects", headers=auth_headers)
    ...
    async def test_subscribe_notifications(self, client, auth_headers):
        ...
        response = await client.post(
            ...
            headers=auth_headers,
            json={"alert_levels": ["URGENT", "CRITICAL"], "notification_channels": ["EMAIL"]},
        )
...
def test_material(db: Session):
    """测试物料"""
    material = Material(
        material_code="MAT001", material_name="测试物料", standard_price=Decimal("100")
    )
    db.add(material)
    db.commit()
    return material
...
def test_project(db: Session):
    """测试项目"""
    project = Project(project_no="PRJ001", project_name="测试项目")
    db.add(project)
    db.commit()
    return project

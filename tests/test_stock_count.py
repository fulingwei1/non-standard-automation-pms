...
class TestStockCountService:
    ...
    def setup_test_data(self, db: Session, test_tenant):
        """准备测试数据"""
        # 创建物料分类
        category = MaterialCategory(category_code="TEST-CAT", category_name="测试分类", level=1)
        db.add(category)
        db.flush()
        ...
        return {"materials": materials, "category": category, "tenant_id": test_tenant.id}
    ...
    def test_create_full_count_task(self, db: Session, setup_test_data):
        ...
        task = service.create_count_task(
            count_type="FULL", count_date=date.today(), location="仓库A", created_by=1
        )
    ...
    def test_create_category_count_task(self, db: Session, setup_test_data):
        ...
        task = service.create_count_task(
            count_type="CYCLE", count_date=date.today(), category_id=category.id, created_by=1
        )
    ...
    def test_record_actual_quantity(self, db: Session, setup_test_data):
        ...
        updated = service.record_actual_quantity(
            detail_id=detail.id, actual_quantity=detail.system_quantity, counted_by=1
        )
    ...
    def test_record_actual_quantity_with_difference(self, db: Session, setup_test_data):
        ...
        updated = service.record_actual_quantity(
            detail_id=detail.id, actual_quantity=actual_qty, counted_by=1
        )
    ...
    def test_batch_record_quantities(self, db: Session, setup_test_data):
        ...
        updated_details = service.batch_record_quantities(records=records, counted_by=1)
    ...
    def test_mark_for_recount(self, db: Session, setup_test_data):
        ...
        # 标记复盘
        updated = service.mark_for_recount(detail_id=detail.id, recount_reason="差异较大,需要复盘")
    ...
    def test_start_count_task(self, db: Session, setup_test_data):
        ...
        task = service.create_count_task(count_type="FULL", count_date=date.today(), created_by=1)
    ...
    def test_approve_adjustment_no_difference(self, db: Session, setup_test_data):
        ...
        # 审批
        result = service.approve_adjustment(task_id=task.id, approved_by=1)
    ...
    def test_approve_adjustment_with_difference(self, db: Session, setup_test_data):
        ...
        # 审批并自动调整
        result = service.approve_adjustment(task_id=task.id, approved_by=1, auto_adjust=True)
    ...
    def test_get_count_tasks(self, db: Session, setup_test_data):
        ...
        # 创建多个任务
        for i in range(3):
            service.create_count_task(count_type="FULL", count_date=date.today(), created_by=1)
    ...
    def test_get_count_tasks_by_status(self, db: Session, setup_test_data):
        ...
        # 创建任务并设置不同状态
        task1 = service.create_count_task(count_type="FULL", count_date=date.today(), created_by=1)

        task2 = service.create_count_task(count_type="FULL", count_date=date.today(), created_by=1)

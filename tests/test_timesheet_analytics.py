...
try:
    ...
    pytest.skip(f"Timesheet analytics dependencies not available: {e}", allow_module_level=True)
...
def test_data(db):
    ...
    for i in range(1, 4):
        project = Project(
            id=i, project_code=f"PRJ-{i:03d}", project_name=f"测试项目{i}", status="ONGOING"
        )
        db.add(project)
...
def test_01_trend_analysis_monthly(db, test_data):
    ...
    result = service.analyze_trend(
        period_type="MONTHLY", start_date=test_data["start_date"], end_date=test_data["end_date"]
    )
...
def test_11_forecast_completion(db, test_data):
    ...
    result = service.forecast_completion(project_id=1, forecast_method="TREND_FORECAST")
...
def test_12_workload_alert_high(db, test_data):
    ...
    alerts = service.forecast_workload_alert(user_ids=[2, 4], forecast_days=30)
...
def test_13_workload_alert_filter(db, test_data):
    ...
    alerts = service.forecast_workload_alert(alert_level="CRITICAL", forecast_days=30)
...
def test_14_gap_analysis(db, test_data):
    ...
    result = service.analyze_gap(
        period_type="MONTHLY", start_date=date.today(), end_date=date.today() + timedelta(days=30)
    )
...
def test_16_empty_date_range(db):
    ...
    result = service.analyze_trend(
        period_type="MONTHLY", start_date=future_start, end_date=future_end
    )
...
def test_19_chart_data_structure(db, test_data):
    ...
    result = service.analyze_trend(
        period_type="DAILY", start_date=test_data["start_date"], end_date=test_data["end_date"]
    )

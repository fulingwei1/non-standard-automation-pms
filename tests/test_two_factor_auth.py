...
from app.models.two_factor import User2FABackupCode, User2FASecret
...
class TestTwoFactorService:
    ...
    def test_setup_2fa_for_user(self, db: Session):
        ...
        # 验证数据库记录
        secret_record = db.query(User2FASecret).filter(User2FASecret.user_id == user.id).first()
    ...
    def test_enable_2fa_for_user_success(self, db: Session):
        ...
        # 验证备用码
        backup_count = (
            db.query(User2FABackupCode).filter(User2FABackupCode.user_id == user.id).count()
        )
    ...
    def test_disable_2fa_for_user(self, db: Session):
        ...
        # 验证数据已删除
        secret_count = db.query(User2FASecret).filter(User2FASecret.user_id == user.id).count()
    ...
    def test_verify_2fa_with_backup_code(self, db: Session):
        ...
        backup_record = (
            ...
            .filter(User2FABackupCode.user_id == user.id, User2FABackupCode.used == True)
            .first()
        )
...
class TestTwoFactorAPI:
    ...
    def test_enable_2fa_endpoint(self, client, test_user, auth_headers, db):
        ...
        response = client.post(
            "/api/v1/auth/2fa/enable", headers=auth_headers, json={"totp_code": code}
        )
    ...
    def test_disable_2fa_endpoint(self, client, test_user_with_2fa, auth_headers_2fa):
        """测试禁用2FA端点"""
        response = client.post(
            "/api/v1/auth/2fa/disable", headers=auth_headers_2fa, json={"password": "test_password"}
        )
    ...
    def test_get_backup_codes_info_endpoint(self, client, test_user_with_2fa, auth_headers_2fa):
        """测试获取备用码信息端点"""
        response = client.get("/api/v1/auth/2fa/backup-codes", headers=auth_headers_2fa)
    ...
    def test_2fa_login_flow(self, client, test_user_with_2fa, db):
        ...
        login_response = client.post(
            "/api/v1/auth/login",
            data={"username": test_user_with_2fa.username, "password": "test_password"},
        )
        ...
        # 2. 获取用户的TOTP密钥
        secret_record = (
            db.query(User2FASecret).filter(User2FASecret.user_id == test_user_with_2fa.id).first()
        )
        ...
        verify_response = client.post(
            "/api/v1/auth/2fa/login", json={"temp_token": login_data["temp_token"], "code": code}
        )

...
class TestInvoiceModel:
    ...
    def test_invoice_code_unique(self, db_session, sample_customer):
        """测试发票编码唯一性"""
        i1 = Invoice(
            invoice_code="INV001", amount=Decimal("1000.00"), customer_id=sample_customer.id
        )
        db_session.add(i1)
        db_session.commit()

        i2 = Invoice(
            invoice_code="INV001", amount=Decimal("2000.00"), customer_id=sample_customer.id
        )
        db_session.add(i2)
    ...
    def test_invoice_delete(self, db_session, sample_customer):
        """测试删除发票"""
        invoice = Invoice(
            invoice_code="INV_DEL", amount=Decimal("1000.00"), customer_id=sample_customer.id
        )
        db_session.add(invoice)
        db_session.commit()
        iid = invoice.id

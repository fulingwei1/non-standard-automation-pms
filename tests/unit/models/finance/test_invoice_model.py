...
class TestInvoiceModel:
    ...
    def test_create_invoice(self, db_session, sample_contract):
        """测试创建发票"""
        invoice = Invoice(
            invoice_code="INV001",
            amount=Decimal("10000.00"),
            contract_id=sample_contract.id,
            project_id=sample_contract.project_id,
            buyer_name=sample_contract.customer.customer_name,
    ...
    def test_invoice_code_unique(self, db_session, sample_contract):
        """测试发票编码唯一性"""
        i1 = Invoice(
            invoice_code="INV001", amount=Decimal("1000.00"), contract_id=sample_contract.id
        )
        db_session.add(i1)
        db_session.commit()

        i2 = Invoice(
            invoice_code="INV001", amount=Decimal("2000.00"), contract_id=sample_contract.id
        )
        db_session.add(i2)
    ...
    def test_invoice_amounts(self, db_session, sample_contract):
        """测试发票金额"""
        invoice = Invoice(
            invoice_code="INV002",
            amount=Decimal("10000.00"),
    ...
    def test_invoice_type(self, db_session, sample_contract):
        """测试发票类型"""
        ...
        count = db_session.query(Invoice).filter(Invoice.invoice_type.in_(types)).count()
    ...
    def test_invoice_dates(self, db_session, sample_contract):
        """测试发票日期"""
        issue_date = date.today()
    ...
    def test_invoice_customer_relationship(self, db_session, sample_invoice):
        ...
        assert sample_invoice.contract.customer is not None
    ...
    def test_invoice_delete(self, db_session, sample_contract):
        """测试删除发票"""
        invoice = Invoice(
            invoice_code="INV_DEL", amount=Decimal("1000.00"), contract_id=sample_contract.id
        )
        db_session.add(invoice)
        db_session.commit()
        iid = invoice.id
    ...
    def test_invoice_number_and_code(self, db_session, sample_contract):
        """测试发票号码和编码"""
        invoice = Invoice(
            invoice_code="INV004",
        ...
        assert invoice.invoice_code == "INV004"

    def test_multiple_invoices(self, db_session, sample_contract):
        """测试多个发票"""
        invoices = [
            Invoice(
    ...
    def test_invoice_description(self, db_session, sample_contract):
        """测试发票描述"""
        desc = "项目阶段性付款发票"
        invoice = Invoice(
            invoice_code="INV005",

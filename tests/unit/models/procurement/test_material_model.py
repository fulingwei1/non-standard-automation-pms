...
class TestMaterialModel:
    ...
    def test_material_code_unique(self, db_session):
        """测试物料编码唯一性"""
        m1 = Material(material_code="MAT001", material_name="物料1")
        db_session.add(m1)
        db_session.commit()

        m2 = Material(material_code="MAT001", material_name="物料2")
        db_session.add(m2)
    ...
    def test_material_type(self, db_session):
        ...
        for i, mt in enumerate(types):
            material = Material(
                material_code=f"MAT_TYPE_{i}", material_name=f"{mt}测试", material_type=mt
            )
            db_session.add(material)
        db_session.commit()

        count = db_session.query(Material).filter(Material.material_type.in_(types)).count()
        assert count == len(types)

    def test_material_unit(self, db_session):
        """测试物料单位"""
        material = Material(material_code="MAT002", material_name="单位测试", unit="千克")
        db_session.add(material)
        db_session.commit()
    ...
    def test_material_specification(self, db_session):
        ...
        material = Material(material_code="MAT004", material_name="规格测试", specification=spec)
        db_session.add(material)
        db_session.commit()
    ...
    def test_material_delete(self, db_session):
        """测试删除物料"""
        material = Material(material_code="MAT_DEL", material_name="待删除")
        db_session.add(material)
        db_session.commit()
        mid = material.id

...
class TestProjectMemberModel:
    ...
    def test_project_member_unique_constraint(self, db_session, sample_project, sample_user):
        ...
        member1 = ProjectMember(
            project_id=sample_project.id, user_id=sample_user.id, role_code="工程师"
        )
    ...
    def test_project_member_relationships(self, db_session, sample_project, sample_user):
        """测试项目成员关系"""
        member = ProjectMember(
            project_id=sample_project.id, user_id=sample_user.id, role_code="项目经理"
        )
        db_session.add(member)
        db_session.commit()
    ...
    def test_project_member_is_active(self, db_session, sample_project, sample_user):
        """测试成员激活状态"""
        member = ProjectMember(
            project_id=sample_project.id, user_id=sample_user.id, role_code="工程师", is_active=True
        )
        db_session.add(member)
        db_session.commit()
    ...
    def test_project_member_update(self, db_session, sample_project, sample_user):
        """测试更新项目成员"""
        member = ProjectMember(
            project_id=sample_project.id, user_id=sample_user.id, role_code="初级工程师"
        )
        db_session.add(member)
        db_session.commit()
    ...
    def test_project_member_delete(self, db_session, sample_project, sample_user):
        """测试删除项目成员"""
        member = ProjectMember(
            project_id=sample_project.id, user_id=sample_user.id, role_code="临时成员"
        )
        db_session.add(member)
        db_session.commit()
        member_id = member.id
    ...
    def test_project_member_default_values(self, db_session, sample_project, sample_user):
        """测试成员默认值"""
        member = ProjectMember(
            project_id=sample_project.id, user_id=sample_user.id, role_code="成员"
        )
        db_session.add(member)
        db_session.commit()
    ...
    def test_multiple_members_same_project(self, db_session, sample_project):
        ...
        user1 = User(
            username="user1", password_hash=get_password_hash("pass"), email="user1@test.com"
        )
        user2 = User(
            username="user2", password_hash=get_password_hash("pass"), email="user2@test.com"
        )

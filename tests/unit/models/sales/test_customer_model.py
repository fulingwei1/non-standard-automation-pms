...
class TestCustomerModel:
    ...
    def test_customer_code_unique_constraint(self, db_session):
        """测试客户编码唯一性约束"""
        customer1 = Customer(customer_name="客户A", customer_code="CUST001", customer_type="企业")
        db_session.add(customer1)
        db_session.commit()

        customer2 = Customer(
            customer_name="客户B", customer_code="CUST001", customer_type="企业"  # 相同编码
        )
    ...
    def test_customer_credit_level(self, db_session):
        """测试客户信用等级"""
        customer = Customer(customer_name="优质客户", customer_code="CUST004", credit_level="AAA")
        db_session.add(customer)
        db_session.commit()
    ...
    def test_customer_status(self, db_session):
        """测试客户状态"""
        customer = Customer(customer_name="状态测试客户", customer_code="CUST005", status="ACTIVE")
        db_session.add(customer)
        db_session.commit()
    ...
    def test_customer_update(self, db_session):
        """测试更新客户信息"""
        customer = Customer(customer_name="初始名称", customer_code="CUST008", customer_type="个人")
        db_session.add(customer)
        db_session.commit()
    ...
    def test_customer_delete(self, db_session):
        """测试删除客户"""
        customer = Customer(customer_name="待删除客户", customer_code="CUST_DEL")
        db_session.add(customer)
        db_session.commit()

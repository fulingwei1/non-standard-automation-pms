...
class TestCreateAnnualWork:
    ...
    def test_create_success(self):
        ...
        data = AnnualKeyWorkCreate(csf_id=1, code="KW001", name="重点工作1", year=2024)
        ...
        with patch(
            "app.services.strategy.annual_work_service.crud.AnnualKeyWork", return_value=mock_work
        ):
...
class TestListAnnualWorks:
    ...
    def test_list_all_works(self):
        ...
        with patch(
            "app.services.strategy.annual_work_service.crud.apply_pagination"
        ) as mock_pagination:
...
class TestUpdateAnnualWork:
    ...
    def test_update_success(self):
        ...
        with patch(
            "app.services.strategy.annual_work_service.crud.get_annual_work", return_value=mock_work
        ):
            db.commit = MagicMock()
            db.refresh = MagicMock()
    ...
    def test_update_non_existing_work(self):
        ...
        with patch(
            "app.services.strategy.annual_work_service.crud.get_annual_work", return_value=None
        ):
    ...
    def test_update_partial_fields(self):
        ...
        mock_work = AnnualKeyWork(id=1, name="旧名称", description="保持不变", is_active=True)
    ...
    def test_list_with_all_filters(self):
        ...
        with patch(
            ...
            items, total = list_annual_works(
                db, csf_id=1, strategy_id=10, year=2024, status="IN_PROGRESS"
            )

...
class TestCalculateFormula:
    ...
    def test_decimal_parameters(self):
        """测试Decimal参数"""
        result = calculate_formula("a + b", {"a": Decimal("10.5"), "b": Decimal("20.3")})
...
class TestCollectKPIValue:
    ...
    def test_collect_auto_source(self):
        ...
        # Mock采集器
        with patch(
            "app.services.strategy.kpi_collector.calculation.get_collector"
    ...
    def test_collect_manual_type(self):
        ...
        data_source = KPIDataSource(
            id=1, kpi_id=1, source_type="MANUAL", is_primary=True, is_active=True
        )
    ...
    def test_collect_collector_not_found(self):
        ...
        with patch(
            "app.services.strategy.kpi_collector.calculation.get_collector", return_value=None
        ):
            result = collect_kpi_value(db, 1)
...
class TestBatchCollectKPIs:
    ...
    def test_batch_collect_with_strategy_filter(self):
        ...
        kpis = [KPI(id=1, code="KPI001", data_source_type="AUTO", is_active=True)]
...
class TestEdgeCases:
    ...
    def test_collect_specific_data_source(self):
        ...
        kpi = KPI(id=1, is_active=True)
        data_source = KPIDataSource(
            id=2, kpi_id=1, source_type="AUTO", source_module="project", is_active=True
        )

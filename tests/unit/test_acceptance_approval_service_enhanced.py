...
class TestAcceptanceApprovalServiceEnhanced(unittest.TestCase):
    ...
    def test_submit_single_order_success(self):
        ...
        results, errors = self.service.submit_orders_for_approval(
            order_ids=[1], initiator_id=50, urgency="NORMAL", comment="请审批"
        )
    ...
    def test_submit_order_not_found(self):
        ...
        # 执行
        results, errors = self.service.submit_orders_for_approval(order_ids=[999], initiator_id=50)
    ...
    def test_submit_order_invalid_status(self):
        """测试提交状态不允许的验收单"""
        order = AcceptanceOrder(id=1, order_no="ACC-2024-001", status="DRAFT")  # 草稿状态不允许提交

        self.db_mock.query.return_value.filter.return_value.first.return_value = order

        results, errors = self.service.submit_orders_for_approval(order_ids=[1], initiator_id=50)
    ...
    def test_submit_order_no_result(self):
        """测试提交没有验收结论的验收单"""
        order = AcceptanceOrder(
            id=1, order_no="ACC-2024-001", status="COMPLETED", overall_result=None  # 没有验收结论
        )
    ...
    def test_submit_multiple_orders_mixed(self):
        ...
        order2 = AcceptanceOrder(id=2, order_no="ACC-002", status="DRAFT")  # 状态不对
        ...
        # 执行
        results, errors = self.service.submit_orders_for_approval(
            order_ids=[1, 2, 3], initiator_id=50
        )
    ...
    def test_submit_order_with_urgency_high(self):
        ...
        results, errors = self.service.submit_orders_for_approval(
            order_ids=[1], initiator_id=50, urgency="HIGH", comment="紧急审批"
        )
    ...
    def test_get_pending_tasks_with_type_filter(self):
        ...
        # 执行 - 只筛选FAT类型
        items, total = self.service.get_pending_tasks(user_id=50, acceptance_type="FAT")
    ...
    def test_get_pending_tasks_pagination(self):
        ...
        # 执行 - 获取第2页，每页3条
        items, total = self.service.get_pending_tasks(user_id=50, offset=3, limit=3)
    ...
    def test_perform_approval_action_approve(self):
        ...
        result = self.service.perform_approval_action(
            task_id=101, action="approve", approver_id=50, comment="同意"
        )
    ...
    def test_perform_approval_action_reject(self):
        ...
        result = self.service.perform_approval_action(
            task_id=101, action="reject", approver_id=50, comment="不符合要求"
        )
    ...
    def test_perform_approval_action_invalid_action(self):
        ...
        with self.assertRaises(ValueError) as context:
            self.service.perform_approval_action(
                task_id=101, action="invalid_action", approver_id=50
            )
    ...
    def test_perform_approval_action_no_comment(self):
        ...
        result = self.service.perform_approval_action(task_id=101, action="approve", approver_id=50)

        self.assertEqual(result["task_id"], 101)
        self.engine_mock.approve.assert_called_once_with(task_id=101, approver_id=50, comment=None)
    ...
    def test_batch_approval_all_success(self):
        ...
        results, errors = self.service.batch_approval(
            task_ids=[101, 102, 103], action="approve", approver_id=50, comment="批量同意"
        )
    ...
    def test_batch_approval_partial_failure(self):
        """测试批量审批部分失败"""
        # 第二次调用抛出异常
        self.engine_mock.approve.side_effect = [MagicMock(), Exception("审批失败"), MagicMock()]

        results, errors = self.service.batch_approval(
            task_ids=[101, 102, 103], action="approve", approver_id=50
        )
    ...
    def test_batch_approval_reject(self):
        ...
        results, errors = self.service.batch_approval(
            task_ids=[101, 102], action="reject", approver_id=50, comment="批量拒绝"
        )
    ...
    def test_batch_approval_invalid_action(self):
        """测试批量审批使用无效操作"""
        results, errors = self.service.batch_approval(
            task_ids=[101, 102], action="invalid", approver_id=50
        )
    ...
    def test_get_approval_status_without_instance(self):
        """测试获取无审批实例的状态"""
        order = AcceptanceOrder(id=1, order_no="ACC-001", acceptance_type="SAT", status="COMPLETED")
    ...
    def test_withdraw_approval_success(self):
        ...
        instance = ApprovalInstance(
            id=201, entity_type="ACCEPTANCE_ORDER", entity_id=1, status="PENDING", initiator_id=50
        )
        ...
        result = self.service.withdraw_approval(order_id=1, user_id=50, reason="需要修改")

        self.assertEqual(result["order_id"], 1)
        self.assertEqual(result["status"], "withdrawn")
        self.engine_mock.withdraw.assert_called_once_with(instance_id=201, user_id=50)
    ...
    def test_get_approval_history_basic(self):
        ...
        order = AcceptanceOrder(
            id=1, order_no="ACC-001", acceptance_type="FAT", overall_result="PASSED"
        )
    ...
    def test_get_approval_history_with_status_filter(self):
        ...
        self.service.get_approval_history(user_id=50, status_filter="APPROVED")
    ...
    def test_get_approval_history_with_type_filter(self):
        ...
        order2 = AcceptanceOrder(
            id=2, order_no="ACC-002", acceptance_type="SAT", overall_result="PASSED"
        )
        ...
        items, total = self.service.get_approval_history(user_id=50, acceptance_type="FAT")
    ...
    def test_get_approval_history_pagination(self):
        ...
        for i in range(5):
            task = ApprovalTask(
                id=300 + i, status="APPROVED", action="approve", completed_at=datetime.now()
            )
        ...
        query_mock.join.return_value.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = tasks[
            2:4
        ]
        ...
        orders = [
            AcceptanceOrder(
                id=i, order_no=f"ACC-{i:03d}", acceptance_type="FAT", overall_result="PASSED"
            )
            for i in range(2, 4)
        ]
        self.db_mock.query.return_value.filter.return_value.first.side_effect = orders

        items, total = self.service.get_approval_history(user_id=50, offset=2, limit=2)
    ...
    def test_get_approval_history_result_name_mapping(self):
        ...
        order = AcceptanceOrder(
            id=1, order_no="ACC-001", acceptance_type="FINAL", overall_result="FAILED"
        )

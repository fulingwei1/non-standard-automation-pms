...
class TestAcceptanceApprovalService(unittest.TestCase):
    ...
    def test_submit_multiple_orders_mixed_results(self):
        ...
        # 执行
        results, errors = self.service.submit_orders_for_approval(order_ids=[1, 2], initiator_id=50)
    ...
    def test_get_pending_tasks_success(self):
        ...
        self.engine_mock.get_pending_tasks.assert_called_once_with(
            user_id=60, entity_type="ACCEPTANCE_ORDER"
        )
    ...
    def test_get_pending_tasks_with_type_filter(self):
        ...
        # 执行 - 只要FAT类型
        items, total = self.service.get_pending_tasks(user_id=60, acceptance_type="FAT")
    ...
    def test_get_pending_tasks_pagination(self):
        ...
        # 执行 - 第一页
        items, total = self.service.get_pending_tasks(user_id=60, offset=0, limit=10)

        self.assertEqual(total, 25)
        self.assertEqual(len(items), 10)

        # 执行 - 第二页
        items, total = self.service.get_pending_tasks(user_id=60, offset=10, limit=10)
    ...
    def test_perform_approval_action_approve(self):
        ...
        result = self.service.perform_approval_action(
            task_id=1, action="approve", approver_id=60, comment="同意"
        )
        ...
        self.engine_mock.approve.assert_called_once_with(task_id=1, approver_id=60, comment="同意")
    ...
    def test_perform_approval_action_reject(self):
        ...
        result = self.service.perform_approval_action(
            task_id=1, action="reject", approver_id=60, comment="不符合要求"
        )
        ...
        self.engine_mock.reject.assert_called_once_with(
            task_id=1, approver_id=60, comment="不符合要求"
        )

    def test_perform_approval_action_invalid_action(self):
        """测试无效的审批操作"""
        with self.assertRaises(ValueError) as context:
            self.service.perform_approval_action(task_id=1, action="invalid_action", approver_id=60)
    ...
    def test_perform_approval_action_without_comment(self):
        ...
        result = self.service.perform_approval_action(
            task_id=1, action="approve", approver_id=60, comment=None
        )

        self.assertEqual(result["task_id"], 1)
        self.engine_mock.approve.assert_called_once_with(task_id=1, approver_id=60, comment=None)
    ...
    def test_batch_approval_all_success(self):
        ...
        results, errors = self.service.batch_approval(
            task_ids=[1, 2, 3], action="approve", approver_id=60, comment="批量同意"
        )
    ...
    def test_batch_approval_partial_failure(self):
        ...
        results, errors = self.service.batch_approval(
            task_ids=[1, 2, 3], action="approve", approver_id=60
        )
    ...
    def test_batch_approval_reject_action(self):
        """测试批量拒绝"""
        results, errors = self.service.batch_approval(
            task_ids=[1, 2], action="reject", approver_id=60, comment="批量拒绝"
        )
    ...
    def test_batch_approval_invalid_action(self):
        """测试批量审批时使用无效操作"""
        results, errors = self.service.batch_approval(
            task_ids=[1, 2], action="invalid", approver_id=60
        )
    ...
    def test_batch_approval_empty_list(self):
        """测试批量审批空列表"""
        results, errors = self.service.batch_approval(task_ids=[], action="approve", approver_id=60)
    ...
    def test_withdraw_approval_success(self):
        ...
        result = self.service.withdraw_approval(order_id=1, user_id=50, reason="需要修改验收单")

        self.assertEqual(result["order_id"], 1)
        self.assertEqual(result["order_no"], "ACC-2024-001")
        self.assertEqual(result["status"], "withdrawn")

        self.engine_mock.withdraw.assert_called_once_with(instance_id=999, user_id=50)
    ...
    def test_get_approval_history_with_status_filter(self):
        ...
        # 执行
        items, total = self.service.get_approval_history(user_id=60, status_filter="REJECTED")
    ...
    def test_get_approval_history_with_type_filter(self):
        ...
        # 执行 - 筛选SAT类型
        items, total = self.service.get_approval_history(user_id=60, acceptance_type="SAT")
    ...
    def test_get_approval_history_pagination(self):
        ...
        # 执行
        items, total = self.service.get_approval_history(user_id=60, offset=0, limit=10)

...
class TestTriggerInvoiceOnAcceptanceEdgeCases:
    ...
    def test_invoice_service_not_available(self, mock_service_class):
        """开票服务不可用时不应抛出异常"""
        mock_db = MagicMock()
        mock_service_class.side_effect = ImportError("No module named 'invoice_service'")
...
class TestHandleAcceptanceStatusTransitionIntegration:
    ...
    def test_fat_passed_with_db_mock(self, mock_service_class, db_session: Session):
        ...
        # 验证调用了正确的方法
        mock_service.handle_fat_passed.assert_called_once_with(order.project_id, order.machine_id)
...
class TestTriggerBonusCalculationIntegration:
    ...
    @patch("app.services.bonus.BonusCalculator")
    def test_bonus_not_triggered_on_failed(self, mock_calculator_class, db_session: Session):
        """验收失败不应触发奖金计算"""
        # 创建测试数据
        project = Project(
...
class TestHandleProgressIntegrationIntegration:
    ...
    def test_acceptance_failed_blocks_milestones(self, mock_service_class, db_session: Session):
        """验收失败应阻塞里程碑"""
        # 创建测试数据
        project = Project(
    ...
    @patch("app.services.progress_integration_service.ProgressIntegrationService")
    def test_acceptance_passed_unblocks_milestones(self, mock_service_class, db_session: Session):
        """验收通过应解除里程碑阻塞"""
        # 创建测试数据
        project = Project(

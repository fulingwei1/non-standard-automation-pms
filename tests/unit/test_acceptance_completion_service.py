...
class TestTriggerInvoice:
    ...
    def test_exception_handling(self, db):
        with patch('app.services.acceptance_completion_service.InvoiceAutoService', side_effect=Exception("err")):
            result = trigger_invoice_on_acceptance(db, 1, auto_trigger=True)
            assert result["success"] is False


class TestHandleAcceptanceStatusTransition:
    @patch('app.services.acceptance_completion_service.StatusTransitionService')
    ...
    def test_exception_safe(self, db):
        with patch('app.services.acceptance_completion_service.StatusTransitionService', side_effect=Exception):
...
class TestHandleProgressIntegration:
    @patch('app.services.acceptance_completion_service.ProgressIntegrationService')
...
class TestTriggerBonusCalculation:
    ...
    @patch('app.services.acceptance_completion_service.BonusCalculator')

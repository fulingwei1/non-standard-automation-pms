...
# 跳过如果模块不存在
pytest.importorskip("app.services.acceptance_handler", reason="Module not implemented")
...
class TestAcceptanceHandler:
    ...
    def test_module_exports(self):
        ...
        for func_name in expected_functions:
            assert hasattr(acceptance_handler, func_name), f"Missing function: {func_name}"
            assert callable(
                getattr(acceptance_handler, func_name)

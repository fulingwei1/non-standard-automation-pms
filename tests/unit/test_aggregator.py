...
class TestPerformanceDataAggregator:
    def setup_method(self):
        self.db = MagicMock()
        with (
            patch("app.services.performance_collector.aggregator.WorkLogCollector"),
    ...
    def test_collect_all_data_success(self):
        ...
        self.agg.work_log_collector.extract_self_evaluation_from_work_logs = MagicMock(
            return_value={"total_logs": 1}
        )
        ...
        self.agg.knowledge_collector.collect_knowledge_contribution_data = MagicMock(
            return_value={}
        )
    ...
    def test_generate_collection_report(self):
        # Mock all collectors
        self.agg.work_log_collector.extract_self_evaluation_from_work_logs = MagicMock(
            return_value={"total_logs": 0}
        )
        self.agg.project_collector.collect_task_completion_data = MagicMock(
            return_value={"total_tasks": 0}
        )
        self.agg.project_collector.collect_project_participation_data = MagicMock(
            return_value={"total_projects": 0}
        )
        self.agg.ecn_collector.collect_ecn_responsibility_data = MagicMock(return_value={})
        self.agg.bom_collector.collect_bom_data = MagicMock(return_value={})
        self.agg.design_collector.collect_design_review_data = MagicMock(
            return_value={"total_reviews": 0}
        )
        self.agg.design_collector.collect_debug_issue_data = MagicMock(return_value={})
        self.agg.knowledge_collector.collect_knowledge_contribution_data = MagicMock(
            return_value={}

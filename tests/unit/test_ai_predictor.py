...
class TestSummarizeEVMHistory(unittest.TestCase):
    ...
    def setUp(self):
        with patch(
            "app.services.project_cost_prediction.ai_predictor.os.getenv", return_value="test_key"
        ):
    ...
    def test_summarize_single_period(self):
        """测试单个周期数据"""
        history = [{"period": "2024-01", "cpi": 1.05, "spi": 0.98, "ac": 50000, "ev": 52500}]
...
class TestBuildPrompts(unittest.TestCase):
    ...
    def test_build_risk_analysis_prompt(self):
        """测试构建风险分析提示词"""
        project_data = {"project_code": "PRJ-002", "bac": 500000}
        evm_history = [{"period": "2024-01", "cpi": 1.0, "spi": 1.0, "ac": 50000, "ev": 50000}]
        current_evm = {"cpi": 0.95, "spi": 0.98, "ac": 100000, "percent_complete": 20}

        prompt = self.predictor._build_risk_analysis_prompt(project_data, evm_history, current_evm)
    ...
    def test_build_optimization_prompt(self):
        """测试构建优化建议提示词"""
        project_data = {"project_code": "PRJ-003", "bac": 800000}
        prediction_result = {"predicted_eac": 900000}
...
class TestCallGLM5API(unittest.TestCase):
    ...
    def test_call_api_with_custom_parameters(self, mock_post):
        ...
        self.predictor._call_glm5_api("测试", temperature=0.8, max_tokens=2000)
...
class TestParseEACPrediction(unittest.TestCase):
    ...
    def test_parse_incomplete_json(self):
        ...
        ai_response = json.dumps({"predicted_eac": 1100000})
        ...
        ai_response = json.dumps(
            {
                ...
                "risk_level": "HIGH",
                "risk_score": 75,
                "risk_factors": [{"factor": "CPI下降", "impact": "HIGH", "weight": 0.8}],
    ...
    def test_parse_valid_suggestions_array(self):
        ...
        ai_response = json.dumps(
            [
                ...
                {"title": "重新谈判供应商价格", "type": "VENDOR_NEGOTIATION", "priority": "MEDIUM"},
            ]
        )
...
class TestPredictEAC(unittest.TestCase):
    ...
    def test_predict_eac_with_additional_context(self, mock_post):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {
            "choices": [{"message": {"content": json.dumps({"predicted_eac": 1200000})}}]
        }
        mock_post.return_value = mock_response

        project_data = {"bac": 1000000}
        evm_history = []
        additional_context = {"market_condition": "稳定", "team_experience": "高"}

        result = self.predictor.predict_eac(project_data, evm_history, additional_context)
...
class TestAnalyzeCostRisks(unittest.TestCase):
    ...
    def test_analyze_risks_success(self, mock_post):
            ...
            "choices": [
                {
                    "message": {
                        "content": json.dumps(
                            {"overrun_probability": 60, "risk_level": "HIGH", "risk_score": 70}
                        )
                    }
                }
            ]
        ...
        result = self.predictor.analyze_cost_risks(project_data, evm_history, current_evm)
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_empty_project_data(self):
        ...
        # 构建提示词时应该能处理缺失字段
        prompt = self.predictor._build_eac_prediction_prompt(project_data, evm_history, None)

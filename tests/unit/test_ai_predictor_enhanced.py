...
class TestPredictEAC:
    ...
    def test_predict_eac_success(
        self, mock_api, predictor, sample_project_data, sample_evm_history, mock_ai_eac_response
    ):
        """测试成功预测EAC"""
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_predict_eac_with_additional_context(
        self, mock_api, predictor, sample_project_data, sample_evm_history, mock_ai_eac_response
    ):
        """测试带额外上下文的EAC预测"""
        mock_api.return_value = mock_ai_eac_response
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_predict_eac_temperature_setting(
        self, mock_api, predictor, sample_project_data, sample_evm_history
    ):
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_predict_eac_bounds_validation(
        self, mock_api, predictor, sample_project_data, sample_evm_history, mock_ai_eac_response
    ):
        """测试EAC预测边界值验证"""
        mock_api.return_value = mock_ai_eac_response
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_predict_eac_api_failure_fallback(
        self, mock_api, predictor, sample_project_data, sample_evm_history
    ):
        """测试API失败时的回退机制"""
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_predict_eac_includes_reasoning(
        self, mock_api, predictor, sample_project_data, sample_evm_history, mock_ai_eac_response
    ):
        """测试预测结果包含推理过程"""
        mock_api.return_value = mock_ai_eac_response
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_predict_eac_key_assumptions(
        self, mock_api, predictor, sample_project_data, sample_evm_history, mock_ai_eac_response
    ):
        """测试预测结果包含关键假设"""
        mock_api.return_value = mock_ai_eac_response
...
class TestAnalyzeCostRisks:
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_analyze_risks_success(
        self,
        mock_api,
        predictor,
        sample_project_data,
        ...
        result = predictor.analyze_cost_risks(
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_analyze_risks_temperature(
        self, mock_api, predictor, sample_project_data, sample_evm_history, sample_current_evm
    ):
        """测试风险分析使用正确的temperature"""
        mock_api.return_value = "{}"

        predictor.analyze_cost_risks(sample_project_data, sample_evm_history, sample_current_evm)

        call_args = mock_api.call_args
        assert call_args[1]["temperature"] == 0.4

    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_analyze_risks_factors_structure(
        self,
        mock_api,
        predictor,
        sample_project_data,
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_analyze_risks_trend_analysis(
        self,
        mock_api,
        predictor,
        sample_project_data,
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_analyze_risks_early_warnings(
        self,
        mock_api,
        predictor,
        sample_project_data,
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_analyze_risks_failure_fallback(
        self, mock_api, predictor, sample_project_data, sample_evm_history, sample_current_evm
    ):
        """测试解析失败时的默认值"""
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_analyze_risks_score_range(
        self,
        mock_api,
        predictor,
        sample_project_data,
...
class TestGenerateOptimizationSuggestions:
    ...
    def test_generate_suggestions_success(
        self, mock_api, predictor, sample_project_data, mock_ai_optimization_response
    ):
        """测试成功生成优化建议"""
    ...
    def test_generate_suggestions_temperature(self, mock_api, predictor, sample_project_data):
        ...
        predictor.generate_optimization_suggestions(sample_project_data, {}, {})

        call_args = mock_api.call_args
        assert call_args[1]["temperature"] == 0.6

    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_generate_suggestions_structure(
        self, mock_api, predictor, sample_project_data, mock_ai_optimization_response
    ):
        """测试建议结构完整性"""
        mock_api.return_value = mock_ai_optimization_response

        result = predictor.generate_optimization_suggestions(sample_project_data, {}, {})
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_generate_suggestions_roi_calculation(
        self, mock_api, predictor, sample_project_data, mock_ai_optimization_response
    ):
        """测试ROI计算包含在建议中"""
        mock_api.return_value = mock_ai_optimization_response
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_generate_suggestions_dict_to_list_conversion(
        self, mock_api, predictor, sample_project_data
    ):
        """测试单个字典转换为列表"""
        single_suggestion = {"title": "测试建议", "type": "PROCESS_IMPROVEMENT", "priority": "HIGH"}
...
class TestResponseParsing:
    ...
    def test_parse_risk_valid_json(self, predictor):
        """测试解析有效的风险分析JSON"""
        response = json.dumps(
            {"risk_level": "CRITICAL", "overrun_probability": 85, "risk_score": 88}
        )
    ...
    def test_parse_optimization_array(self, predictor):
        """测试解析优化建议数组"""
        response = json.dumps(
            [{"title": "建议1", "priority": "HIGH"}, {"title": "建议2", "priority": "MEDIUM"}]
        )
...
class TestPromptBuilding:
    ...
    def test_build_eac_prompt_includes_project_info(
        self, predictor, sample_project_data, sample_evm_history
    ):
        """测试EAC提示词包含项目信息"""
    ...
    def test_build_risk_prompt_includes_cpi(
        self, predictor, sample_project_data, sample_evm_history, sample_current_evm
    ):
        """测试风险提示词包含CPI信息"""
    ...
    def test_build_optimization_prompt_includes_eac(self, predictor, sample_project_data):
        ...
        prompt = predictor._build_optimization_prompt(sample_project_data, prediction, risk)

        assert "1100000" in prompt
        assert "HIGH" in prompt

    def test_build_eac_prompt_with_context(
        self, predictor, sample_project_data, sample_evm_history
    ):
        """测试包含额外上下文的提示词"""
...
class TestIntegration:
    ...
    @patch.object(GLM5CostPredictor, "_call_glm5_api")
    def test_consistent_results_across_methods(
        self, mock_api, predictor, sample_project_data, sample_evm_history, sample_current_evm
    ):
        """测试各方法结果的一致性"""
        ...
        risk = predictor.analyze_cost_risks(
            sample_project_data, sample_evm_history, sample_current_evm
        )

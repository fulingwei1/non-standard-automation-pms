...
class TestNotifyTimesheetAnomaly:
    def test_no_anomalies_returns_zero(self, mock_db):
        ...
        with patch(
            "app.services.timesheet_reminder.anomaly_reminders.TimesheetQualityService",
    ...
    def test_sends_notification_for_anomaly(self, mock_db):
        ...
        with (
            ...
            patch(
                "app.services.timesheet_reminder.anomaly_reminders.create_timesheet_notification"
            ) as mock_notify,
        ):
            result = notify_timesheet_anomaly(mock_db, days=1)

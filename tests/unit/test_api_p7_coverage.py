...
class TestPurchaseWorkflow:
    ...
    def test_submit_orders_for_approval_success(self):
        ...
        with patch(
            "app.api.v1.endpoints.purchase.workflow.ApprovalEngineService",
    ...
    def test_get_approval_history(self):
        ...
        with patch(
            ...
            result = get_approval_history(order_id=1, db=db, current_user=_mock_user())
        assert result is not None
...
class TestPurchaseOrdersRefactored:
    ...
    def test_create_purchase_order(self):
        ...
        with (
            patch("app.api.v1.endpoints.purchase.orders_refactored.save_obj") as mock_save,
            patch(
                "app.api.v1.endpoints.purchase.orders_refactored.generate_order_no",
                return_value="PO-001",
            ),
        ):
...
class TestDepartmentsRefactored:
    ...
    def test_read_departments_empty(self):
        ...
        db.query.return_value.filter.return_value.all.return_value = []
        with patch(
            "app.api.v1.endpoints.organization.departments_refactored.apply_pagination"
        ) as mock_pag:
            mock_pag.return_value.all.return_value = []
    ...
    def test_read_department_not_found(self):
        ...
        db = _mock_db()
        with patch(
            "app.api.v1.endpoints.organization.departments_refactored.get_or_404"
        ) as mock_get:
    ...
    def test_create_department(self):
        ...
        with patch(
            "app.api.v1.endpoints.organization.departments_refactored.save_obj"
        ) as mock_save:
    ...
    def test_get_department_statistics(self):
        from app.api.v1.endpoints.organization.departments_refactored import (
...
class TestProjectRisks:
    ...
    def test_create_risk(self):
        ...
        mock_schema = MagicMock()
        with (
            patch("app.api.v1.endpoints.projects.risks.get_or_404", return_value=mock_project),
            patch("app.api.v1.endpoints.projects.risks.save_obj") as mock_save,
        ):
            mock_save.return_value = MagicMock(id=1)
            result = create_risk(
                project_id=1, risk_in=mock_schema, db=db, current_user=_mock_user()
            )
        assert result is not None
    ...
    def test_delete_risk(self):
        ...
        with (
            patch("app.api.v1.endpoints.projects.risks.get_or_404", return_value=mock_risk),
...
class TestProductionPlans:
    ...
    def test_create_production_plan(self):
        ...
        mock_schema.project_id = None
        with (
            patch("app.api.v1.endpoints.production.plans.save_obj") as mock_save,
    ...
    def test_submit_production_plan_success(self):
        ...
        with (
            patch("app.api.v1.endpoints.production.plans.get_or_404", return_value=mock_plan),
        ...
        with patch("app.api.v1.endpoints.sales.quote_costs.get_or_404") as mock_get:
            ...
            with pytest.raises(HTTPException) as exc:
                get_cost_calculations(
                    quote_id=999, version_id=None, db=db, current_user=_mock_user()
                )
            assert exc.value.status_code == 404
...
class TestShortageSubstitutions:
    ...
    def test_list_substitutions_empty(self):
        ...
        db.query.return_value.count.return_value = 0
        with patch(
            "app.api.v1.endpoints.shortage.handling.substitutions.apply_pagination"
        ) as mock_pag:
            mock_pag.return_value.all.return_value = []
    ...
    def test_create_substitution(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.shortage.handling.substitutions._generate_substitution_no",
                return_value="SUB-001",
            ),
            patch("app.api.v1.endpoints.shortage.handling.substitutions.save_obj") as mock_save,
            patch(
                "app.api.v1.endpoints.shortage.handling.substitutions._build_substitution_response",
                return_value={"id": 1},
            ),
        ):
            result = create_substitution(
                substitution_in=mock_schema, db=db, current_user=_mock_user()
            )
        assert result is not None
    ...
    def test_tech_approve_substitution(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.shortage.handling.substitutions.get_or_404",
            ...
            result = tech_approve_substitution(
                substitution_id=1, comment="OK", db=db, current_user=_mock_user()
            )
        assert result is not None
...
class TestCustomerRegistrations:
    ...
    def test_get_customer_registrations_empty(self):
        from app.api.v1.endpoints.business_support_orders.customer_registrations import (
        ...
        db.query.return_value.count.return_value = 0
        with patch(
            "app.api.v1.endpoints.business_support_orders.customer_registrations.apply_keyword_filter",
            return_value=db.query.return_value,
        ):
            db.query.return_value.filter.return_value.count.return_value = 0
    ...
    def test_get_customer_registration_not_found(self):
        ...
        db = _mock_db()
        with patch(
            "app.api.v1.endpoints.business_support_orders.customer_registrations.get_or_404"
        ) as mock_get:
            mock_get.side_effect = HTTPException(status_code=404, detail="入驻申请不存在")
            with pytest.raises(HTTPException) as exc:
                asyncio.run(
                    get_customer_registration(registration_id=999, db=db, current_user=_mock_user())
                )
            assert exc.value.status_code == 404
    ...
    def test_create_customer_registration(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.business_support_orders.customer_registrations.generate_registration_no",
                return_value="REG-001",
            ),
            patch(
                "app.api.v1.endpoints.business_support_orders.customer_registrations._to_registration_response",
                return_value={"id": 1},
            ),
        ):
            result = asyncio.run(
...
class TestSalesCustomers:
    ...
    def test_read_customers_empty(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.sales.customers.apply_keyword_filter",
                return_value=db.query.return_value,
            ),
            patch(
                "app.api.v1.endpoints.sales.customers.security.filter_sales_data_by_scope",
                return_value=db.query.return_value,
            ),
        ):
            db.query.return_value.count.return_value = 0
    ...
    def test_create_customer(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.sales.customers.generate_customer_code",
                return_value="CUS20240101001",
            ),
...
class TestProjectWorkloadCrud:
    ...
    def test_get_project_team_workload_not_found(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.projects.workload.crud.check_project_access_or_raise"
            ) as mock_check,
            ...
            with pytest.raises(HTTPException) as exc:
                get_project_team_workload(
                    project_id=999, start_date=None, end_date=None, db=db, current_user=_mock_user()
                )
            assert exc.value.status_code == 404
    ...
    def test_get_project_workload_gantt(self):
        ...
        mock_project.id = 1
        with (
            patch("app.api.v1.endpoints.projects.workload.crud.check_project_access_or_raise"),
            patch(
                "app.api.v1.endpoints.projects.workload.crud.get_or_404", return_value=mock_project
            ),
        ):
            db.query.return_value.filter.return_value.all.return_value = []
...
class TestRdProjectExpenses:
    ...
    def test_create_rd_cost(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.rd_project.expenses.get_or_404",
                side_effect=[mock_project, mock_cost_type],
            ),
            patch(
                "app.api.v1.endpoints.rd_project.expenses.generate_cost_no", return_value="COST-001"
            ),
    ...
    def test_calculate_labor_cost(self):
        ...
        with patch(
            "app.api.v1.endpoints.rd_project.expenses.get_or_404",
            side_effect=[mock_project, mock_user],
        ):
            ...
            result = calculate_labor_cost(
                calc_request=mock_calc_req, db=db, current_user=_mock_user()
            )
        assert result is not None
...
class TestShortageArrivals:
    ...
    def test_list_arrivals_empty(self):
        ...
        db.query.return_value.count.return_value = 0
        with (
            patch("app.api.v1.endpoints.shortage.handling.arrivals.apply_pagination") as mock_pag,
            patch(
                "app.api.v1.endpoints.shortage.handling.arrivals.apply_keyword_filter",
                return_value=db.query.return_value,
            ),
        ):
            mock_pag.return_value.all.return_value = []
    ...
    def test_create_arrival(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.shortage.handling.arrivals.get_or_404",
                return_value=mock_material,
            ),
            patch(
                "app.api.v1.endpoints.shortage.handling.arrivals._generate_arrival_no",
                return_value="ARR-001",
            ),
            patch("app.api.v1.endpoints.shortage.handling.arrivals.save_obj"),
            patch(
                "app.api.v1.endpoints.shortage.handling.arrivals._build_arrival_response",
                return_value={"id": 1},
            ),
...
class TestSalesContractsBasic:
    ...
    def test_read_contracts_empty(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.sales.contracts.basic.apply_keyword_filter",
                return_value=db.query.return_value,
            ),
            patch("app.api.v1.endpoints.sales.contracts.basic.apply_pagination") as mock_pag,
            patch(
                "app.api.v1.endpoints.sales.contracts.basic.security.filter_sales_data_by_scope",
                return_value=db.query.return_value,
            ),
        ):
            mock_pag.return_value.all.return_value = []
    ...
    def test_create_contract_no_quote(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.sales.contracts.basic.generate_contract_code",
                return_value="CON-001",
            ),
            patch(
                "app.api.v1.endpoints.sales.contracts.basic.security.filter_sales_data_by_scope",
                return_value=db.query.return_value,
            ...
            result = create_contract(
                contract_in=mock_schema, db=db, skip_g3_validation=True, current_user=_mock_user()
            )
        assert result is not None
    ...
    def test_update_contract_not_found(self):
        ...
        with pytest.raises(HTTPException) as exc:
            update_contract(
                contract_id=999, contract_in=MagicMock(), db=db, current_user=_mock_user()
            )
        assert exc.value.status_code == 404
...
class TestReconciliations:
    ...
    def test_get_reconciliations_empty(self):
        ...
        with patch(
            "app.api.v1.endpoints.business_support_orders.reconciliations.apply_keyword_filter",
            return_value=db.query.return_value,
        ):
            db.query.return_value.count.return_value = 0
            db.query.return_value.filter.return_value.count.return_value = 0
    ...
    def test_get_reconciliation_not_found(self):
        ...
        db = _mock_db()
        with patch(
            "app.api.v1.endpoints.business_support_orders.reconciliations.get_or_404"
        ) as mock_get:
            mock_get.side_effect = HTTPException(status_code=404, detail="对账单不存在")
            with pytest.raises(HTTPException) as exc:
                asyncio.run(
                    get_reconciliation(reconciliation_id=999, db=db, current_user=_mock_user())
                )
            assert exc.value.status_code == 404

    def test_update_reconciliation_not_found(self):
        from app.api.v1.endpoints.business_support_orders.reconciliations import (
...
class TestSalesTargets:
    ...
    def test_get_sales_targets_empty(self):
        ...
        mock_service_instance.calculate_target_performance.return_value = (
            Decimal("0"),
            Decimal("0"),
        )
        with (
            patch("app.api.v1.endpoints.sales.targets.apply_pagination") as mock_pag,
            patch("app.api.v1.endpoints.sales.targets.get_user_role_code", return_value="SALES"),
            patch(
                "app.api.v1.endpoints.sales.targets.SalesTeamService",
    ...
    def test_create_sales_target(self):
        ...
        with (
            patch("app.api.v1.endpoints.sales.targets.save_obj") as mock_save,
    ...
    def test_update_sales_target_not_found(self):
        ...
        db = _mock_db()
        with (
            patch("app.api.v1.endpoints.sales.targets.get_or_404") as mock_get,
            ...
            with pytest.raises(HTTPException) as exc:
                update_sales_target(
                    target_id=999, target_in=MagicMock(), db=db, current_user=_mock_user()
                )
            assert exc.value.status_code == 404
...
class TestOpportunityCrud:
    ...
    def test_read_opportunities_empty(self):
        ...
        with (
            ...
            patch(
                "app.api.v1.endpoints.sales.opportunity_crud.security.filter_sales_data_by_scope",
                return_value=db.query.return_value,
            ),
        ):
            db.query.return_value.filter.return_value.count.return_value = 0
...
class TestOrganizationUnits:
    ...
    def test_create_org_unit(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.organization.units.OrganizationUnit", return_value=mock_unit
            ),
            patch("app.api.v1.endpoints.organization.units.OrganizationUnitResponse") as mock_resp,
            patch(
                "app.api.v1.endpoints.organization.units.success_response",
...
class TestProjectsArchive:
    ...
    def test_archive_project_already_archived(self):
        ...
        with patch(
            "app.utils.permission_helpers.check_project_access_or_raise", return_value=mock_project
        ):
    ...
    def test_get_archived_projects_empty(self):
        ...
        db.query.return_value.filter.return_value.count.return_value = 0
        with (
            patch("app.api.v1.endpoints.projects.archive.apply_pagination") as mock_pag,
            patch(
                "app.api.v1.endpoints.projects.archive.apply_keyword_filter",
                return_value=db.query.return_value,
            ),
        ):
            mock_pag.return_value.all.return_value = []
    ...
    def test_batch_archive_projects_empty_list(self):
        ...
        db = _mock_db()
        with patch(
            "app.services.data_scope.DataScopeService.filter_projects_by_scope",
            return_value=db.query.return_value,
        ):
            db.query.return_value.filter.return_value.all.return_value = []
            result = batch_archive_projects(
                project_ids=[], archive_reason=None, db=db, current_user=_mock_user()
            )
        assert result is not None
...
class TestSalesTeamPK:
    ...
    def test_create_team_pk(self):
        ...
        with (
            patch("app.api.v1.endpoints.sales.team.pk.save_obj"),
...
class TestSalesLeadsCrud:
    ...
    def test_read_leads_empty(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.sales.leads.crud.apply_keyword_filter",
                return_value=db.query.return_value,
            ),
            patch(
                "app.api.v1.endpoints.sales.leads.crud.security.filter_sales_data_by_scope",
                return_value=db.query.return_value,
            ),
        ):
            db.query.return_value.filter.return_value.count.return_value = 0
    ...
    def test_create_lead(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.sales.leads.crud.generate_lead_code", return_value="LEAD-001"
            ),

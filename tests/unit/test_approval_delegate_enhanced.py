...
class TestUpdateDelegate:
    ...
    def test_update_delegate_success(self, service, db):
        ...
        result = service.update_delegate(
            delegate_id=1, scope="TEMPLATE", template_ids=[100, 200], reason="更新原因"
        )
    ...
    def test_update_delegate_only_allowed_fields(self, service, db):
        ...
        result = service.update_delegate(
            delegate_id=1, scope="ALL", invalid_field="should_not_update"
        )
        ...
        # invalid_field不应该被设置
        assert (
            not hasattr(delegate, "invalid_field") or delegate.invalid_field != "should_not_update"
        )
...
class TestSendDelegateNotification:
    ...
    def test_send_delegate_notification_approved(self, service, db):
        ...
        with patch(
            "app.services.approval_engine.notify.ApprovalNotifyService"
        ) as MockNotifyService:

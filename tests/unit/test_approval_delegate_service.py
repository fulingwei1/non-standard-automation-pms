...
class TestApprovalDelegateService_GetActiveDelegate:
    ...
    def test_get_active_delegate_none_configured(self, delegate_service: ApprovalDelegateService):
    ...
    def test_get_active_delegate_all_scope(
        ...
        today = date.today()
        next_week = date(today.year, today.month + 1 if today.month < 12 else 1, today.day)
...
class TestApprovalDelegateService_UpdateDelegate:
    ...
    def test_update_delegate_success(
        ...
        # Update reason
        updated = delegate_service.update_delegate(delegate_id=created.id, reason="Updated reason")

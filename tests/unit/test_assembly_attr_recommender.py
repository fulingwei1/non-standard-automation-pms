...
class TestRecommend:
    ...
    def test_history_wins(self, *mocks):
        history_rec = AssemblyAttrRecommendation(
            "FRAME", True, False, confidence=95.0, source="HISTORY", reason="test"
        )
...
class TestMatchFromCategory:
    def test_with_mapping(self):
        ...
        mapping = MagicMock(
            stage_code="MECH", is_blocking=True, can_postpone=False, importance_level="HIGH"
        )
...
class TestBatchRecommend:
    def test_batch(self):
        db = MagicMock()
        material = MagicMock(
            id=1, material_name="气缸 SC50", category_id=None, default_supplier_id=None
        )

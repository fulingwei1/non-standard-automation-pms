...
class TestGetDepartmentManagerUserId(TestApprovalAdapterBase):
    ...
    def test_get_department_manager_with_valid_department(
        self, mock_dept_cls, mock_employee_cls, mock_user_cls
    ):
        """测试有效部门返回负责人ID"""
        # 构造真实数据对象
        dept = Mock()
    ...
    @patch("app.models.organization.Employee")
    @patch("app.models.organization.Department")
    def test_get_department_manager_user_not_found(
        self, mock_dept_cls, mock_employee_cls, mock_user_cls
    ):
        """测试用户记录不存在返回None"""
        dept = Mock()
...
class TestGetDepartmentManagerUserIdsByCodes(TestApprovalAdapterBase):
    ...
    @patch("app.models.organization.Employee")
    @patch("app.models.organization.Department")
    def test_get_managers_with_valid_departments(
        self, mock_dept_cls, mock_employee_cls, mock_user_cls
    ):
        """测试有效部门编码返回负责人ID列表"""
        # 构造部门数据

...
class TestBatchRevenue:
    @patch.object(
        RevenueService, "get_project_revenue", side_effect=[Decimal("100"), Decimal("200")]
    )
    def test_batch(self, m, mock_db):
        assert RevenueService.get_projects_revenue(mock_db, [1, 2]) == {
            1: Decimal("100"),
            2: Decimal("200"),
        }
    ...
    @patch.object(
        RevenueService, "get_projects_revenue", return_value={1: Decimal("100"), 2: Decimal("200")}
    )

...
class TestBatchOperationExecutor:
    ...
    def executor(self, mock_model, mock_db, mock_user):
        """创建执行器实例"""
        return BatchOperationExecutor(model=mock_model, db=mock_db, current_user=mock_user)

    def test_初始化(self, mock_model, mock_db, mock_user):
        """测试初始化"""
        executor = BatchOperationExecutor(
            model=mock_model, db=mock_db, current_user=mock_user, id_field="custom_id"
        )
    ...
    def test_execute_empty_list(self, executor):
        """测试执行空列表"""
        result = executor.execute(entity_ids=[], operation_func=lambda x: None)
    ...
    def test_execute_successful_operation(self, executor, mock_db):
        ...
        # 执行批量操作
        result = executor.execute(entity_ids=[1, 2], operation_func=update_status)
    ...
    def test_execute_entity_not_found(self, executor, mock_db):
        ...
        # 执行批量操作，请求ID包括不存在的ID
        result = executor.execute(entity_ids=[1, 2, 3], operation_func=lambda x: None)
    ...
    def test_execute_operation_exception(self, executor, mock_db):
        ...
        result = executor.execute(entity_ids=[1, 2], operation_func=failing_operation)
    ...
    def test_execute_commit_failure(self, executor, mock_db):
        ...
        result = executor.execute(entity_ids=[1], operation_func=lambda x: None)
    ...
    def test_batch_update(self, executor, mock_db):
        ...
        result = executor.batch_update(entity_ids=[1], update_func=update_name)
    ...
    def test_batch_delete_soft(self, executor, mock_db):
        ...
        result = executor.batch_delete(entity_ids=[1], soft_delete=True)
    ...
    def test_batch_delete_hard(self, executor, mock_db):
        ...
        result = executor.batch_delete(entity_ids=[1], soft_delete=False)
    ...
    def test_batch_status_update(self, executor, mock_db):
        ...
        result = executor.batch_status_update(entity_ids=[1], new_status="COMPLETED")
    ...
    def test_batch_status_update_custom_field(self, executor, mock_db):
        ...
        result = executor.batch_status_update(
            entity_ids=[1], new_status="CLOSED", status_field="state"
        )
    ...
    def test_with_scope_filter_func(self, mock_model, mock_db, mock_user):
        ...
        executor = BatchOperationExecutor(
            model=mock_model, db=mock_db, current_user=mock_user, scope_filter_func=scope_filter
        )

        result = executor.execute(entity_ids=[1, 2, 3, 4], operation_func=lambda x: None)
    ...
    def test_with_pre_filter_func(self, executor, mock_db):
        ...
        result = executor.execute(
            entity_ids=[1, 2, 3, 4], operation_func=lambda x: None, pre_filter_func=pre_filter
        )
...
class TestIntegrationScenarios:
    ...
    def test_批量更新任务状态场景(self):
        ...
        # 创建执行器
        executor = BatchOperationExecutor(model=mock_model, db=mock_db, current_user=mock_user)

...
from app.models.enums.workflow import ChangeSourceEnum, ChangeTypeEnum
...
class TestAnalyzeChangeImpact(unittest.TestCase):
    ...
    async def test_analyze_change_impact_success(self):
        ...
        # Mock所有分析方法
        with (
            patch.object(self.service, "_gather_analysis_context", return_value={}),
            patch.object(
                self.service,
                "_analyze_schedule_impact",
    ...
    async def test_analyze_change_impact_exception_handling(self):
        ...
        with patch.object(
            self.service, "_gather_analysis_context", side_effect=Exception("测试异常")
        ):
...
class TestFindAffectedTasks(unittest.TestCase):
    ...
    def test_find_affected_tasks_max_limit(self):
        ...
        context = {"change": {"time_impact": 5}, "tasks": tasks}
...
class TestFindAffectedMilestones(unittest.TestCase):
    ...
    def test_find_affected_milestones_with_delay(self):
        ...
        context = {
            "milestones": [
                {"id": 1, "name": "里程碑1", "plan_date": datetime(2024, 6, 1).isoformat()},
                {"id": 2, "name": "里程碑2", "plan_date": datetime(2024, 12, 1).isoformat()},
            ]
        }

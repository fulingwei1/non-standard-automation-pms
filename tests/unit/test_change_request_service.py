...
class TestCreateChangeRequest:
    ...
    def test_create_with_attachments(self):
        """测试5：创建带附件的变更"""
        attachments = [{"name": "需求文档.pdf", "url": "/uploads/doc.pdf", "size": 1024}]
...
class TestStatusTransition:
    ...
    def test_valid_transition_submitted_to_assessing(self):
        ...
        assert (
            validate_status_transition(ChangeStatusEnum.SUBMITTED, ChangeStatusEnum.ASSESSING)
            is True
        )
    ...
    def test_invalid_transition_rejected_to_approved(self):
        ...
        assert (
            validate_status_transition(ChangeStatusEnum.REJECTED, ChangeStatusEnum.APPROVED)
            is False
        )
    ...
    def test_invalid_transition_closed_to_implementing(self):
        ...
        assert (
            validate_status_transition(ChangeStatusEnum.CLOSED, ChangeStatusEnum.IMPLEMENTING)
            is False
        )
    ...
    def test_valid_transition_implementing_to_verifying(self):
        ...
        assert (
            validate_status_transition(ChangeStatusEnum.IMPLEMENTING, ChangeStatusEnum.VERIFYING)
            is True
        )
...
class TestApprovalWorkflow:
    ...
    def test_approval_with_attachments(self):
        """测试15：带附件的审批"""
        attachments = [{"name": "审批意见.pdf", "url": "/uploads/approval.pdf"}]
        approval_record = ChangeApprovalRecord(
            change_request_id=1,
            approver_id=1,
            approver_name="审批人",

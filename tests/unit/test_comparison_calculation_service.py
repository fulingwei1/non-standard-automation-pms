...
class TestComparisonCalculationService:
    ...
    def test_calculate_mom_comparison_success(
        self, comparison_calculation_service, test_metric_definition
    ):
        """测试计算环比 - 成功场景"""
        with patch.object(
            comparison_calculation_service.metric_service, "calculate_metric"
        ) as mock_calc:
            ...
            result = comparison_calculation_service.calculate_mom_comparison(
                metric_code="TEST_METRIC", year=2024, month=2
            )
    ...
    def test_calculate_mom_comparison_january(
        self, comparison_calculation_service, test_metric_definition
    ):
        """测试计算环比 - 1月（跨年）"""
        with patch.object(
            ...
            result = comparison_calculation_service.calculate_mom_comparison(
                metric_code="TEST_METRIC", year=2024, month=1
            )

            assert result is not None
            assert result["previous_period"] == "2023-12"

    def test_calculate_mom_comparison_decrease(
        self, comparison_calculation_service, test_metric_definition
    ):
        """测试计算环比 - 下降"""
        with patch.object(
    ...
    def test_calculate_mom_comparison_zero_previous(
        self, comparison_calculation_service, test_metric_definition
    ):
        """测试计算环比 - 上月为0"""
        with patch.object(
    ...
    def test_calculate_yoy_comparison_success(
        self, comparison_calculation_service, test_metric_definition
    ):
        """测试计算同比 - 成功场景"""
        with patch.object(
    ...
    def test_calculate_annual_yoy_comparison_success(
        self, comparison_calculation_service, test_metric_definition
    ):
        """测试计算年度同比 - 成功场景"""
        with patch.object(
            ...
            result = comparison_calculation_service.calculate_annual_yoy_comparison(
                metric_code="TEST_METRIC", year=2024
            )
    ...
    def test_calculate_comparisons_batch_monthly(
        self, comparison_calculation_service, test_metric_definition
    ):
        """测试批量计算对比 - 月度"""
        with patch.object(
            ...
            result = comparison_calculation_service.calculate_comparisons_batch(
                metric_codes=["TEST_METRIC"], year=2024, month=2, enable_mom=True, enable_yoy=True
            )
    ...
    def test_calculate_comparisons_batch_annual(
        self, comparison_calculation_service, test_metric_definition
    ):
        """测试批量计算对比 - 年度"""
        with patch.object(
    ...
    def test_calculate_comparisons_batch_metric_not_found(self, comparison_calculation_service):
        """测试批量计算对比 - 指标不存在"""
        result = comparison_calculation_service.calculate_comparisons_batch(
            metric_codes=["NONEXISTENT_METRIC"], year=2024, month=2
        )

        assert result is not None
        assert "NONEXISTENT_METRIC" not in result or len(result) == 0

    def test_calculate_comparisons_batch_multiple_metrics(
        self, comparison_calculation_service, db_session
    ):
        """测试批量计算对比 - 多个指标"""
        # 创建多个指标
        for i in range(2):
            ...
            with patch.object(
                ...
                result = comparison_calculation_service.calculate_comparisons_batch(
                    metric_codes=["METRIC_0", "METRIC_1"], year=2024, month=2
                )

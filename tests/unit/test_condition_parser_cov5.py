...
class TestConditionEvaluator:
    ...
    def test_eq_operator(self):
        expr = json.dumps(
            {"operator": "AND", "items": [{"field": "level", "op": "==", "value": "HIGH"}]}
        )
    ...
    def test_in_operator(self):
        expr = json.dumps(
            {
                "operator": "AND",
                "items": [{"field": "status", "op": "in", "value": ["A", "B", "C"]}],
            }
        )
    ...
    def test_between_operator(self):
        expr = json.dumps(
            {"operator": "AND", "items": [{"field": "score", "op": "between", "value": [60, 100]}]}
        )
    ...
    def test_contains_operator(self):
        expr = json.dumps(
            {"operator": "AND", "items": [{"field": "name", "op": "contains", "value": "foo"}]}
        )

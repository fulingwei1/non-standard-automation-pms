...
class TestGetTitle:
    ...
    def test_get_title_with_customer(self):
        ...
        contract = make_contract(id=1, contract_code="CT-2025-001", customer=customer)
...
class TestValidateSubmit:
    ...
    def test_validate_submit_success_draft(self):
        """测试草稿状态可以提交"""
        db = make_db()
        contract = make_contract(id=1, status="DRAFT", contract_amount=Decimal("100000"))
    ...
    def test_validate_submit_success_rejected(self):
        """测试被驳回状态可以重新提交"""
        db = make_db()
        contract = make_contract(id=1, status="REJECTED", contract_amount=Decimal("100000"))

...
from app.models.sales import PurchaseMaterialCost, QuoteItem
...
def test_quote_item():
    """创建测试报价项"""
    item = QuoteItem(item_name="测试物料", qty=10, cost=Decimal("100.00"))
    return item
...
class TestCostMatchSuggestionService:
    ...
    def test_check_cost_anomalies_high_cost(self, db_session, test_cost_query, test_quote_item):
        """测试检查成本异常 - 成本偏高"""
        # 创建历史成本记录
        historical_cost = PurchaseMaterialCost(material_name="测试物料", unit_cost=Decimal("50.00"))
    ...
    def test_find_matching_cost_exact_match(self, db_session, test_cost_query, test_quote_item):
        ...
        cost = PurchaseMaterialCost(
            material_name="测试物料", unit_cost=Decimal("100.00"), match_priority=100
        )
        db_session.add(cost)
        db_session.commit()
    ...
    def test_find_matching_cost_fuzzy_match(self, db_session, test_cost_query):
        ...
        # 创建模糊匹配的成本记录
        cost = PurchaseMaterialCost(material_name="测试物料A型", unit_cost=Decimal("100.00"))
        db_session.add(cost)
        db_session.commit()
    ...
    def test_calculate_summary_success(self):
        """测试计算汇总 - 成功场景"""
        items = [QuoteItem(id=1, qty=10, cost=Decimal("100.00"))]
        suggestions = [
            CostMatchSuggestion(
                item_id=1,
                item_name="物料1",
    ...
    def test_process_cost_match_suggestions_success(self, db_session, test_cost_query):
        """测试处理成本匹配建议 - 成功场景"""
        items = [QuoteItem(id=1, item_name="测试物料", qty=10, cost=Decimal("100.00"))]

        suggestions, matched_count, unmatched_count, warnings, total_cost = (
            process_cost_match_suggestions(db_session, items, test_cost_query)
        )
    ...
    def test_process_cost_match_suggestions_empty_items(self, db_session, test_cost_query):
        """测试处理成本匹配建议 - 空列表"""
        suggestions, matched_count, unmatched_count, warnings, total_cost = (
            process_cost_match_suggestions(db_session, [], test_cost_query)
        )

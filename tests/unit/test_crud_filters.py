...
class TestQueryBuilder:
    ...
    def test_build_with_exact_filter(self):
        ...
        db = MagicMock()
        query, count = QueryBuilder.build_list_query(FakeModel, db, filters={"status": "ACTIVE"})
        assert query is not None
    ...
    def test_build_with_none_filter(self):
        ...
        db = MagicMock()
        query, count = QueryBuilder.build_list_query(FakeModel, db, filters={"description": None})
        assert query is not None
    ...
    def test_build_with_nonexistent_order_field(self):
        ...
        query, count = QueryBuilder.build_list_query(FakeModel, db, order_by="nonexistent_field")
        assert query is not None

...
class TestCustomerService(unittest.TestCase):
    ...
    def test_list_customers_with_keyword(self):
        ...
        with patch.object(self.service.repository, "list") as mock_list:
            ...
            result = self.service.list_customers(page=1, page_size=20, keyword="阿里")
    ...
    def test_list_customers_with_filters(self):
        ...
        with patch.object(self.service.repository, "list") as mock_list:
            ...
            result = self.service.list_customers(
                page=1, page_size=20, customer_type="企业", industry="IT", status="ACTIVE"
            )
    ...
    def test_list_customers_partial_filters(self):
        ...
        with patch.object(self.service.repository, "list") as mock_list:
            ...
            result = self.service.list_customers(
                customer_type="企业", industry=None, status=None  # None应该被忽略
            )
    ...
    def test_after_update_with_auto_sync_enabled(self):
        ...
        mock_customer = Customer(
            id=1, customer_code="C001", customer_name="测试客户", credit_level="B", status="ACTIVE"
        )
    ...
    def test_after_update_with_auto_sync_disabled(self):
        """测试禁用自动同步"""
        mock_customer = Customer(
            id=2, customer_code="C002", customer_name="测试客户2", credit_level="B", status="ACTIVE"
        )
    ...
    def test_after_update_sync_exception_handling(self):
        """测试同步异常处理（不应影响更新操作）"""
        mock_customer = Customer(
            id=3, customer_code="C003", customer_name="测试客户3", credit_level="B", status="ACTIVE"
        )
    ...
    def test_after_update_re_enable_auto_sync(self):
        """测试重新启用自动同步"""
        mock_customer = Customer(
            id=4, customer_code="C004", customer_name="测试客户4", credit_level="B", status="ACTIVE"
        )
...
class TestCustomerServiceIntegration(unittest.TestCase):
    ...
    def test_delete_workflow_with_projects(self):
        ...
        with patch.object(self.service.repository, "get") as mock_get:
            mock_customer = Customer(
                id=1, customer_code="C001", customer_name="测试", credit_level="B", status="ACTIVE"
            )
    ...
    def test_update_workflow_with_sync(self):
        """测试完整的更新工作流（包含同步）"""
        mock_customer = Customer(
            id=1, customer_code="C001", customer_name="原名称", credit_level="B", status="ACTIVE"
        )
        ...
        # Mock repository方法
        with (
            patch.object(self.service.repository, "get") as mock_get,

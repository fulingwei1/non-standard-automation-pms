...
class TestScopeTypeMapping:
    ...
    def test_normalize_all(self):
        assert (
            DataScopeServiceEnhanced.normalize_scope_type(ScopeType.ALL.value)
            == DataScopeEnum.ALL.value
        )

    def test_normalize_business_unit(self):
        assert (
            DataScopeServiceEnhanced.normalize_scope_type(ScopeType.BUSINESS_UNIT.value)
            == DataScopeEnum.DEPT.value
        )

    def test_normalize_department(self):
        assert (
            DataScopeServiceEnhanced.normalize_scope_type(ScopeType.DEPARTMENT.value)
            == DataScopeEnum.DEPT.value
        )

    def test_normalize_team(self):
        assert (
            DataScopeServiceEnhanced.normalize_scope_type(ScopeType.TEAM.value)
            == DataScopeEnum.DEPT.value
        )

    def test_normalize_project(self):
        assert (
            DataScopeServiceEnhanced.normalize_scope_type(ScopeType.PROJECT.value)
            == DataScopeEnum.PROJECT.value
        )

    def test_normalize_own(self):
        assert (
            DataScopeServiceEnhanced.normalize_scope_type(ScopeType.OWN.value)
            == DataScopeEnum.OWN.value
        )
...
class TestGetAccessibleOrgUnits:
    ...
    def test_department_scope_with_dept_ancestor(self):
        ...
        dept_ancestor = MagicMock()
        dept_ancestor.id = 3
        db.query.return_value.filter.return_value.first.return_value = org

        with (
            patch.object(DataScopeServiceEnhanced, "get_user_org_units", return_value=[5]),
            patch.object(
                DataScopeServiceEnhanced, "_find_ancestor_by_type", return_value=dept_ancestor
            ),
            patch.object(
                DataScopeServiceEnhanced, "_get_subtree_ids_optimized", return_value={3, 4, 5}
            ),
        ):
            result = DataScopeServiceEnhanced.get_accessible_org_units(
                db, 1, ScopeType.DEPARTMENT.value
            )
        assert 3 in result
    ...
    def test_business_unit_scope_with_bu_ancestor(self):
        ...
        org = MagicMock()
        org.id = 5
        bu_ancestor = MagicMock()
        bu_ancestor.id = 1
        db.query.return_value.filter.return_value.first.return_value = org
        with (
            patch.object(DataScopeServiceEnhanced, "get_user_org_units", return_value=[5]),
            patch.object(
                DataScopeServiceEnhanced, "_find_ancestor_by_type", return_value=bu_ancestor
            ),
            patch.object(
                DataScopeServiceEnhanced, "_get_subtree_ids_optimized", return_value={1, 2, 3}
            ),
        ):
            result = DataScopeServiceEnhanced.get_accessible_org_units(
                db, 1, ScopeType.BUSINESS_UNIT.value
            )
        assert 1 in result
...
class TestGetSubtreeIds:
    ...
    def test_optimized_no_path_falls_back_to_recursive(self):
        ...
        db.query.return_value.filter.return_value.first.return_value = org
        with patch.object(
            DataScopeServiceEnhanced, "_get_subtree_ids_recursive", return_value={1, 2}
        ) as mock_rec:
            result = DataScopeServiceEnhanced._get_subtree_ids_optimized(db, 1)
...
class TestCanAccessData:
    ...
    def test_dept_scope_accessible(self):
        ...
        with (
            patch("app.services.data_scope_service_enhanced.PermissionService") as mock_ps,
            patch.object(
                DataScopeServiceEnhanced, "get_accessible_org_units", return_value=[10, 20]
            ),
        ):
            mock_ps.get_user_data_scopes.return_value = {"project": ScopeType.DEPARTMENT.value}
            result = DataScopeServiceEnhanced.can_access_data(db, user, "project", data)

...
class TestGenerateDeptWeeklyReport:
    def test_dept_not_found(self):
        ...
        result = DeptReportMixin.generate_dept_weekly_report(
            db, 999, date(2025, 1, 6), date(2025, 1, 12)
        )
    ...
    def test_basic_report(self):
        ...
        result = DeptReportMixin.generate_dept_weekly_report(
            db, 1, date(2025, 1, 6), date(2025, 1, 12)
        )
...
class TestGenerateDeptMonthlyReport:
    def test_dept_not_found(self):
        ...
        result = DeptReportMixin.generate_dept_monthly_report(
            db, 999, date(2025, 1, 1), date(2025, 1, 31)
        )
        assert result == {"error": "部门不存在"}
    ...
    def test_basic_monthly_report(self):
        ...
        user = MagicMock(
            id=1, real_name="张三", username="zhangsan", department="技术部", is_active=True
        )
        user.position = "工程师"
        ...
        ts = MagicMock(user_id=1, hours=160, project_id=1, work_date=date(2025, 1, 15))

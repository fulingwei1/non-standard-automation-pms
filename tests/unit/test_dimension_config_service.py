...
class TestDimensionConfigServiceCore(unittest.TestCase):
    ...
    def test_get_config_with_department_priority(self):
        ...
        result = self.service.get_config(
            job_type="mechanical", job_level="senior", department_id=100
        )
    ...
    def test_get_config_fallback_to_global(self):
        """测试部门配置不存在时回退到全局配置"""
        global_config = self._create_mock_config(
            config_id=2, job_type="mechanical", is_global=True, technical_weight=30
        )
        ...
        # 执行
        result = self.service.get_config(job_type="mechanical", department_id=100)
    ...
    def test_get_config_with_default_effective_date(self):
        """测试使用默认生效日期（今天）"""
        config = self._create_mock_config(config_id=1, job_type="electrical", is_global=True)
    ...
    def test_get_config_with_job_level_match(self):
        ...
        config = self._create_mock_config(
            config_id=1, job_type="test", job_level="junior", is_global=True
        )
        ...
        # 执行
        result = self.service.get_config(job_type="test", job_level="junior")
    ...
    def test_create_config_success_global(self):
        ...
        with patch(
            ...
            result = self.service.create_config(
                data=data, operator_id=1, department_id=None, require_approval=False
            )
    ...
    def test_create_config_department_level(self):
        ...
        with patch(
            ...
            # 执行
            result = self.service.create_config(
                data=data, operator_id=1, department_id=10, require_approval=True
            )
    ...
    def test_create_config_department_permission_denied(self):
        ...
        # 执行并验证异常
        with self.assertRaises(ValueError) as context:
            self.service.create_config(data=data, operator_id=2, department_id=10)
    ...
    def test_create_config_operator_incomplete(self):
        ...
        # 执行并验证异常
        with self.assertRaises(ValueError) as context:
            self.service.create_config(data=data, operator_id=3, department_id=10)
    ...
    def test_approve_config_success(self):
        """测试审批配置成功"""
        config = self._create_mock_config(
            config_id=1, job_type="mechanical", department_id=10, is_global=False
        )
        ...
        result = self.service.approve_config(
            config_id=1, approver_id=1, approved=True, approval_reason="符合要求"
        )
    ...
    def test_approve_config_reject(self):
        ...
        # 执行
        result = self.service.approve_config(
            config_id=1, approver_id=1, approved=False, approval_reason="权重配置不合理"
        )
    ...
    def test_approve_config_not_found(self):
        ...
        # 执行并验证异常
        with self.assertRaises(ValueError) as context:
            self.service.approve_config(config_id=999, approver_id=1)

        self.assertIn("配置不存在", str(context.exception))

    def test_approve_config_global_config_error(self):
        """测试全局配置不需要审批"""
        config = self._create_mock_config(config_id=1, job_type="mechanical", is_global=True)
        ...
        # 执行并验证异常
        with self.assertRaises(ValueError) as context:
            self.service.approve_config(config_id=1, approver_id=1)
    ...
    def test_approve_config_permission_denied(self):
        ...
        # 执行并验证异常
        with self.assertRaises(ValueError) as context:
            self.service.approve_config(config_id=1, approver_id=2)
    ...
    def test_approve_config_admin_by_role(self):
        ...
        # 执行（应该成功）
        result = self.service.approve_config(config_id=1, approver_id=2, approved=True)
    ...
    def test_format_config_success(self):
        """测试格式化配置"""
        config = self._create_mock_config(config_id=1, job_type="mechanical", technical_weight=30)
    ...
    def test_build_config_list(self):
        ...
        job_type_distribution = {
            ...
            "electrical": {"count": 3, "levels": {"senior": 1, "junior": 2}},
        }

...
class TestAutoAssignEvaluation:
    ...
    def test_no_users_returns_none(self):
        ...
        eval_ = MagicMock()
        eval_.eval_dept = "Engineering"
        with patch(
            "app.services.ecn_auto_assign_service.find_users_by_department", return_value=[]
        ):
            result = auto_assign_evaluation(db, ecn, eval_)
        assert result is None
    ...
    def test_picks_head_over_manager(self):
        ...
        with patch(
            "app.services.ecn_auto_assign_service.find_users_by_department",
            return_value=[manager, head],
        ):
            result = auto_assign_evaluation(db, ecn, eval_)
        assert result == 3
    ...
    def test_returns_none_when_only_normal_users(self):
        ...
        with patch(
            "app.services.ecn_auto_assign_service.find_users_by_department", return_value=[normal]
        ):
            result = auto_assign_evaluation(db, ecn, eval_)
        assert result is None
    ...
    def test_with_project_picks_project_member_head(self):
        ...
        pm_q = MagicMock()
        pm_q.filter.return_value = pm_q
        pm_q.all.return_value = [pm]
        user_q = MagicMock()
        user_q.filter.return_value = user_q
        user_q.all.return_value = [head]
...
class TestAutoAssignApproval:
    ...
    def test_picks_head_from_role_users(self):
        ...
        with patch(
            "app.services.ecn_auto_assign_service.find_users_by_role", return_value=[manager, head]
        ):
            result = auto_assign_approval(db, ecn, approval)
        assert result == 3
    ...
    def test_picks_manager_when_no_head_in_role(self):
        ...
        ecn = _mock_ecn(1, project_id=None)
        approval = MagicMock()
        approval.approval_role = "Reviewer"
        supervisor = _mock_user(7, position="主管")
        normal = _mock_user(8, position="成员")
        with patch(
            "app.services.ecn_auto_assign_service.find_users_by_role",
            return_value=[normal, supervisor],
        ):
            result = auto_assign_approval(db, ecn, approval)
        assert result == 7
...
class TestAutoAssignTask:
    ...
    def test_picks_head_for_task(self):
        ...
        with patch(
            "app.services.ecn_auto_assign_service.find_users_by_department", return_value=[head]
        ):
            result = auto_assign_task(db, ecn, task)
        assert result == 10
...
class TestAutoAssignPending:
    ...
    def test_pending_evaluations_none_pending(self):
        ...
        ecn = _mock_ecn(1)
        q_ecn = MagicMock()
        q_ecn.filter.return_value = q_ecn
        q_ecn.first.return_value = ecn
        q_eval = MagicMock()
        q_eval.filter.return_value = q_eval
        q_eval.all.return_value = []
    ...
    def test_pending_approvals_assigns(self):
        ...
        q_approval.all.return_value = [approval]
    ...
    def test_pending_tasks_assigns(self):
        ...
        q_ecn.first.return_value = ecn
        q_task = MagicMock()
        q_task.filter.return_value = q_task
        q_task.all.return_value = [task]

...
class TestFindUsersByRole:
    ...
    def test_find_users_with_role(self, db_session: Session):
        ...
        # 创建拥有该角色的用户
        user1 = User(
            username="manager1", real_name="经理1", is_active=True, password_hash="test_hash_123"
        )
        user2 = User(
            username="manager2", real_name="经理2", is_active=False, password_hash="test_hash_123"
        )
...
class TestAutoAssignApproval:
    ...
    def test_assign_to_project_role_lead(self, db_session: Session):
        ...
        # 创建用户
        pm1 = User(
            username="pm1", real_name="项目经理1", is_active=True, password_hash="test_hash_123"
        )
        pm2 = User(
            username="pm2", real_name="项目经理2", is_active=False, password_hash="test_hash_123"
        )
    ...
    def test_assign_to_role_manager(self, db_session: Session):
        ...
        # 创建用户
        manager = User(
            username="fin_mgr", real_name="财务经理", is_active=True, password_hash="test_hash_123"
        )
...
class TestEcnWorkflowIntegration:
    ...
    def test_ecn_create_to_approval_workflow(self, db_session: Session):
        ...
        # 创建用户
        engineer = User(
            username="engineer", real_name="工程师", is_active=True, password_hash="test_hash_123"
        )
        manager = User(
            username="manager", real_name="经理", is_active=True, password_hash="test_hash_123"
        )
    ...
    def test_ecn_with_bom_impact_analysis(self, db_session: Session):
        ...
        # 验证BOM影响记录已保存
        bom_impacts = db_session.query(EcnBomImpact).filter(EcnBomImpact.ecn_id == ecn.id).all()

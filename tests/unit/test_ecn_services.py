    ...
    def test_affected_materials_different_change_types(self, db_session: Session):
        ...
        for i, change_type in enumerate(change_types):
            mat = EcnAffectedMaterial(
                ...
                old_quantity=Decimal("10") if change_type in ["UPDATE", "REPLACE"] else None,
                new_quantity=Decimal("20") if change_type in ["ADD", "UPDATE", "REPLACE"] else None,
        ...
        affected_mats = (
            db_session.query(EcnAffectedMaterial).filter(EcnAffectedMaterial.ecn_id == ecn.id).all()
        )

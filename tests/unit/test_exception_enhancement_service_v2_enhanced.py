...
class TestExceptionEnhancementService(unittest.TestCase):
    ...
    def test_escalate_exception_create_new_flow(self):
        ...
        exception = ProductionException(
            id=exception_id, exception_no="EXC-001", title="测试异常", status="REPORTED"
        )
        ...
        with patch("app.utils.db_helpers.get_or_404", return_value=exception):
            ...
            # Mock User查询
            escalated_user = User(id=2, username="张三", password_hash="test_hash_123")
    ...
    def test_escalate_exception_update_existing_flow(self):
        ...
        with patch("app.utils.db_helpers.get_or_404", return_value=exception):
            ...
            # Mock User查询
            escalated_user = User(id=3, username="李四", password_hash="test_hash_123")
            user_query = MagicMock()
            user_query.filter.return_value.first.return_value = escalated_user
    ...
    def test_escalate_exception_invalid_level(self):
        """测试无效的升级级别使用默认值"""
        exception_id = 1
        exception = ProductionException(id=exception_id, exception_no="EXC-004", status="REPORTED")
    ...
    def test_escalate_exception_without_escalated_user(self):
        """测试没有指定升级人员的情况"""
        exception = ProductionException(id=1, exception_no="EXC-003", status="REPORTED")
        ...
        with patch("app.utils.db_helpers.get_or_404", return_value=exception):
            ...
            result = self.service.escalate_exception(
                exception_id=1, escalation_level="LEVEL_1", reason="测试", escalated_to_id=None
            )
    ...
    def test_get_exception_flow_success(self):
        ...
        # 构造完整的flow对象
        exception = ProductionException(id=exception_id, exception_no="EXC-003", title="测试异常3")
        escalated_user = User(id=2, username="张三", password_hash="test_hash_123")
        verifier = User(id=3, username="验证员", password_hash="test_hash_123")
    ...
    def test_calculate_flow_duration_in_progress(self):
        ...
        flow = ExceptionHandlingFlow(pending_at=past_time, processing_at=None, resolved_at=None)
    ...
    def test_calculate_flow_duration_partial_stages(self):
        ...
        with patch(
            "app.services.production.exception.exception_enhancement_service.datetime"
        ) as mock_datetime:
    ...
    def test_get_exception_statistics_full_data(self):
        ...
        def query_side_effect(*args, **kwargs):
            ...
            if call_count[0] == 1:  # 第一次：总数查询
                ...
                stats_query = MagicMock()
                stats_query.filter.return_value.group_by.return_value.all.return_value = (
                    status_stats
                )
                return stats_query
    ...
    def test_create_pdca_success(self):
        """测试成功创建PDCA记录"""
        exception_id = 1
        exception = ProductionException(id=exception_id, exception_no="EXC-100", title="测试异常")
        ...
        with (
            patch("app.utils.db_helpers.get_or_404", return_value=exception),
            ...
            # Mock用户查询
            user = User(id=2, username="负责人", password_hash="test_hash_123")
            user_query = MagicMock()
    ...
    def test_advance_pdca_stage_plan_to_do(self):
        """测试PDCA从P阶段推进到D阶段"""
        pdca = ExceptionPDCA(id=1, exception_id=1, pdca_no="PDCA-001", current_stage=PDCAStage.PLAN)
        ...
        with patch("app.utils.db_helpers.get_or_404", return_value=pdca):
            user = User(id=3, username="执行人", password_hash="test_hash_123")
    ...
    def test_advance_pdca_stage_do_to_check(self):
        """测试PDCA从D阶段推进到C阶段"""
        pdca = ExceptionPDCA(id=1, exception_id=1, current_stage=PDCAStage.DO)
        ...
        with patch("app.utils.db_helpers.get_or_404", return_value=pdca):
            user = User(id=4, username="检查人", password_hash="test_hash_123")
            exception = ProductionException(id=1, exception_no="EXC-001")
    ...
    def test_advance_pdca_stage_check_to_act(self):
        """测试PDCA从C阶段推进到A阶段"""
        pdca = ExceptionPDCA(id=1, exception_id=1, current_stage=PDCAStage.CHECK)
        ...
        with patch("app.utils.db_helpers.get_or_404", return_value=pdca):
            user = User(id=5, username="行动人", password_hash="test_hash_123")
            exception = ProductionException(id=1, exception_no="EXC-001")
    ...
    def test_advance_pdca_stage_act_to_completed(self):
        """测试PDCA从A阶段推进到完成"""
        pdca = ExceptionPDCA(id=1, exception_id=1, current_stage=PDCAStage.ACT)

        request = MagicMock(stage="COMPLETED", summary="总结", lessons_learned="经验教训")
    ...
    def test_advance_pdca_stage_invalid_transition(self):
        """测试PDCA无效的阶段转换"""
        pdca = ExceptionPDCA(id=1, exception_id=1, current_stage=PDCAStage.PLAN)
    ...
    def test_build_pdca_response_all_stages_completed(self):
        ...
        exception = ProductionException(id=1, exception_no="EXC-001")
        users = {
            1: User(id=1, username="计划者", password_hash="test_hash_123"),
            2: User(id=2, username="执行者", password_hash="test_hash_123"),
            3: User(id=3, username="检查者", password_hash="test_hash_123"),
            4: User(id=4, username="行动者", password_hash="test_hash_123"),
        }
    ...
    def test_analyze_recurrence_single_type(self):
        ...
        results = self.service.analyze_recurrence(exception_type="EQUIPMENT", days=7)
    ...
    def test_analyze_recurrence_all_types(self):
        ...
        results = self.service.analyze_recurrence(exception_type=None, days=30)

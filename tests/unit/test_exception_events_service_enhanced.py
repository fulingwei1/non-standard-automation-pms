...
class TestGetExceptionEvents:
    ...
    def test_get_exception_events_basic(
        self,
        mock_get_params,
        mock_apply_pagination,
        mock_apply_filter,
        mock_joinedload,
        service,
        mock_db,
    ...
    @patch("app.services.alert.exception_events_service.apply_pagination")
    @patch("app.services.alert.exception_events_service.get_pagination_params")
    def test_get_exception_events_with_filters(
        self,
        mock_get_params,
        mock_apply_pagination,
        mock_apply_filter,
...
class TestGetExceptionEvent:
    ...
    @patch("app.services.alert.exception_events_service.joinedload")
    def test_get_exception_event_found(
        self, mock_joinedload, service, mock_db, mock_exception_event
    ):
        """测试找到异常事件"""
        mock_query = MagicMock()
        mock_db.query.return_value.options.return_value = mock_query
...
class TestCreateExceptionEvent:
    ...
    @patch(
        "app.services.alert.exception_events_service.ExceptionEventsService._send_exception_notification"
    )
    @patch(
        "app.services.alert.exception_events_service.ExceptionEventsService._auto_assign_handler"
    )
    @patch("app.utils.db_helpers.save_obj")
    def test_create_exception_event_basic(
        self, mock_save_obj, mock_auto_assign, mock_send_notification, service, mock_db, mock_user
    ):
        """测试基本创建"""
    ...
    @patch("app.utils.db_helpers.save_obj")
    def test_create_exception_event_with_occurred_at(
        self, mock_save_obj, mock_auto_assign, mock_send_notification, service, mock_db, mock_user
    ):
        """测试指定发生时间"""
...
class TestUpdateExceptionEvent:
    ...
    def test_update_exception_event_success(
        self, service, mock_db, mock_exception_event, mock_user
    ):
        """测试成功更新"""
        service.get_exception_event = MagicMock(return_value=mock_exception_event)

        event_data = ExceptionEventUpdate(title="更新后的标题", severity="high")

        result = service.update_exception_event(
            event_id=1, event_data=event_data, current_user=mock_user
        )
    ...
    def test_update_exception_event_not_found(self, service, mock_db, mock_user):
        ...
        result = service.update_exception_event(
            event_id=999, event_data=ExceptionEventUpdate(title="新标题"), current_user=mock_user
        )
...
class TestResolveExceptionEvent:
    ...
    def test_resolve_exception_event_success(
        self, mock_send_notification, service, mock_db, mock_exception_event, mock_user
    ):
        """测试成功解决"""
        ...
        with patch("app.services.alert.exception_events_service.datetime") as mock_datetime:
            ...
            result = service.resolve_exception_event(
                event_id=1, resolve_data=resolve_data, current_user=mock_user
            )
    ...
    def test_resolve_exception_event_already_resolved(
        self, service, mock_db, mock_exception_event, mock_user
    ):
        """测试已解决的事件"""
...
class TestVerifyExceptionEvent:
    ...
    def test_verify_exception_event_verified(
        self, service, mock_db, mock_exception_event, mock_user
    ):
        """测试验证通过"""
        mock_exception_event.status = "resolved"
        ...
        verify_data = ExceptionEventVerify(is_verified=True, verification_note="验证通过")

        result = service.verify_exception_event(
            event_id=1, verify_data=verify_data, current_user=mock_user
        )
    ...
    def test_verify_exception_event_reopened(
        self, service, mock_db, mock_exception_event, mock_user
    ):
        """测试验证不通过重新打开"""
        mock_exception_event.status = "resolved"
        ...
        verify_data = ExceptionEventVerify(is_verified=False, verification_note="问题仍然存在")
    ...
    def test_verify_exception_event_wrong_status(
        self, service, mock_db, mock_exception_event, mock_user
    ):
        """测试错误状态无法验证"""
        mock_exception_event.status = "pending"
...
class TestAddExceptionAction:
    ...
    def test_add_exception_action_success(self, mock_save_obj, service, mock_db, mock_user):
        ...
        result = service.add_exception_action(
            event_id=1, action_data=action_data, current_user=mock_user
        )
...
class TestEscalateExceptionEvent:
    ...
    @patch(
        "app.services.alert.exception_events_service.ExceptionEventsService._send_escalation_notification"
    )
    def test_escalate_exception_event_success(
        self, mock_send_escalation, service, mock_db, mock_exception_event, mock_user
    ):
        """测试成功升级"""
        service.get_exception_event = MagicMock(return_value=mock_exception_event)
        ...
        result = service.escalate_exception_event(
            event_id=1, escalation_data=escalation_data, current_user=mock_user
        )
...
class TestCreateExceptionFromIssue:
    ...
    def test_create_exception_from_issue_success(self, mock_save_obj, service, mock_db, mock_user):
        ...
        result = service.create_exception_from_issue(issue_id=1, current_user=mock_user)
    ...
    def test_create_exception_from_issue_not_found(self, service, mock_db, mock_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            service.create_exception_from_issue(issue_id=999, current_user=mock_user)
...
class TestSendExceptionNotification:
    ...
    def test_send_exception_notification_success(
        self, mock_get_service, service, mock_db, mock_exception_event
    ):
        """测试成功发送通知"""
    ...
    @patch("app.services.alert.exception_events_service.get_notification_service")
    def test_send_exception_notification_multiple_recipients(
        self, mock_get_service, service, mock_db, mock_exception_event
    ):
        """测试多个接收人"""
        mock_exception_event.responsible_user_id = 1
...
class TestSendEscalationNotification:
    ...
    @patch("app.services.alert.exception_events_service.get_notification_service")
    def test_send_escalation_notification_success(
        self, mock_get_service, service, mock_db, mock_exception_event
    ):
        """测试成功发送升级通知"""
...
class TestSimplifiedAliases:
    ...
    @patch(
        "app.services.alert.exception_events_service.ExceptionEventsService.create_exception_event"
    )
    ...
    @patch(
        "app.services.alert.exception_events_service.ExceptionEventsService.get_exception_events"
    )

...
class TestProjectFinanceService:
    ...
    def test_with_projects(self, mock_scope):
        ...
        cost_query.with_entities.return_value.group_by.return_value.all.return_value = [
            ("MATERIAL", 3000),
            ("LABOR", 2000),
        ]
        cost_query.with_entities.return_value.group_by.return_value.order_by.return_value.limit.return_value.all.return_value = [
            (1, 3000)
        ]
        # Mock project query

...
class TestGetUsersHourlyRates:
    ...
    def test_get_rates_single_user(self, db_session: Session, test_user):
        """测试单个用户"""
        rates = HourlyRateService.get_users_hourly_rates(db_session, user_ids=[test_user.id])
    ...
    def test_get_rates_with_invalid_user(self, db_session: Session, test_user):
        """测试包含无效用户ID"""
        rates = HourlyRateService.get_users_hourly_rates(db_session, user_ids=[test_user.id, 99999])
        assert len(rates) == 2
...
class TestGetHourlyRateHistory:
    ...
    def test_get_history_by_user(self, db_session: Session, test_user):
        """测试按用户筛选历史"""
        history = HourlyRateService.get_hourly_rate_history(db_session, user_id=test_user.id)
        assert isinstance(history, list)

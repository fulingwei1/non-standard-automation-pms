...
class TestInventoryAnalysisServiceTurnoverRate(unittest.TestCase):
    ...
    def test_get_turnover_rate_data_basic(self):
        ...
        result = self.service.get_turnover_rate_data(self.db, self.start_date, self.end_date)
...
class TestInventoryAnalysisServiceABCAnalysis(unittest.TestCase):
    ...
    def test_get_abc_analysis_data_basic(self):
        ...
        result = self.service.get_abc_analysis_data(self.db, self.start_date, self.end_date)
    ...
    def test_get_abc_analysis_data_abc_summary(self):
        ...
        mock_results = [
            Mock(
                material_code=f"M{str(i).zfill(3)}",
        ...
        # 检查金额占比
        self.assertAlmostEqual(result["abc_summary"]["A"]["amount_percent"], 70, delta=5)
...
class TestInventoryAnalysisServiceCostOccupancy(unittest.TestCase):
    ...
    def test_get_cost_occupancy_data_basic(self):
        ...
        self.db.query.return_value.outerjoin.return_value.filter.return_value.group_by.return_value.all.return_value = (
            mock_category_results
        )
        ...
        self.db.query.return_value.outerjoin.return_value.filter.return_value.order_by.return_value.limit.return_value.all.return_value = (
            mock_top_materials
        )
    ...
    def test_get_cost_occupancy_data_with_category_filter(self):
        ...
        query_mock = (
            self.db.query.return_value.outerjoin.return_value.filter.return_value.group_by.return_value
        )
...
class TestInventoryAnalysisServiceEdgeCases(unittest.TestCase):
    ...
    def test_turnover_rate_data_with_null_values(self):
        ...
        result = self.service.get_turnover_rate_data(self.db, date(2024, 1, 1), date(2024, 12, 31))
    ...
    def test_safety_stock_compliance_limit_returns(self):
        ...
        warning_materials = [
            Mock(
                id=i,
                material_code=f"W{str(i).zfill(3)}",
                material_name=f"预警{i}",
                category_name="分类A",
                current_stock=Decimal("30"),
                safety_stock=Decimal("100"),
                unit="kg",
        ...
        out_materials = [
            Mock(
                id=i,
                material_code=f"O{str(i).zfill(3)}",
                material_name=f"缺货{i}",
                category_name="分类A",
                current_stock=Decimal("0"),
                safety_stock=Decimal("50"),
                unit="kg",

...
class TestBuildSummaryPrompt(TestProjectKnowledgeSyncer):
    ...
    def test_build_summary_prompt_with_none_fields(self):
        ...
        project = self._create_mock_project()
        review = self._create_mock_review(
            project=project, success_factors=None, problems=None, best_practices=None
        )
...
后续文本"""
    ...
    def test_parse_summary_response_invalid_json(self):
        """测试解析无效JSON时的降级处理"""
        ai_response = {"content": "这不是一个有效的JSON字符串，只是普通文本"}
...
class TestExtractTags(TestProjectKnowledgeSyncer):
    ...
    def test_extract_tags_all_dimensions(self):
        """测试提取所有维度的标签"""
        project = self._create_mock_project(industry="制造业", project_type="工程项目")
        review = self._create_mock_review(
            project=project,
    ...
    def test_extract_tags_small_project(self):
        """测试小型项目标签"""
        project = self._create_mock_project()
        review = self._create_mock_review(project=project, budget_amount="300000")  # < 500000
    ...
    def test_extract_tags_large_project(self):
        """测试大型项目标签"""
        project = self._create_mock_project()
        review = self._create_mock_review(project=project, budget_amount="5000000")  # >= 2000000
    ...
    def test_extract_tags_medium_satisfaction(self):
        """测试中等满意度标签"""
        project = self._create_mock_project()
        review = self._create_mock_review(project=project, customer_satisfaction=3)
    ...
    def test_extract_tags_slight_delay(self):
        """测试轻微延期标签"""
        project = self._create_mock_project()
        review = self._create_mock_review(project=project, schedule_variance=5)  # <= 7
    ...
    def test_extract_tags_significant_delay(self):
        """测试显著延期标签"""
        project = self._create_mock_project()
        review = self._create_mock_review(project=project, schedule_variance=15)  # > 7
    ...
    def test_extract_tags_cost_overrun(self):
        ...
        project = self._create_mock_project()
        review = self._create_mock_review(
            project=project, budget_amount="1000000", cost_variance="100000"  # 10%
        )
    ...
    def test_extract_tags_severe_overrun(self):
        ...
        project = self._create_mock_project()
        review = self._create_mock_review(
            project=project, budget_amount="1000000", cost_variance="200000"  # 20%
        )
...
class TestExtractTechnicalHighlights(TestProjectKnowledgeSyncer):
    ...
    def test_extract_technical_highlights_short_content(self):
        """测试短内容的技术亮点提取"""
        review = self._create_mock_review(best_practices="采用微服务架构，实现系统解耦")

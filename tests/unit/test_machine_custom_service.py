...
from app.services.machine_custom.service import DOC_TYPE_PERMISSION_MAP, MachineCustomService
...
class TestMachineCustomService(unittest.TestCase):
    ...
    def test_check_document_permission_has_permission(self, mock_check_permission):
        ...
        mock_check_permission.assert_called_once_with(
            self.mock_user, "machine:doc_circuit", self.mock_db
        )
    ...
    def test_check_document_permission_unknown_type(self, mock_check_permission):
        ...
        mock_check_permission.assert_called_once_with(
            self.mock_user, "machine:doc_other", self.mock_db  # 默认权限
        )
    ...
    def test_upload_document_success(
        self, mock_filepath_class, mock_file, mock_save_obj, mock_check_permission
    ):
        """测试成功上传文档"""
        # 准备
        mock_check_permission.return_value = True
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("app.services.machine_custom.service.FilePath")
    def test_upload_document_file_save_error(
        self, mock_filepath_class, mock_file, mock_check_permission
    ):
        """测试文件保存失败"""
        mock_check_permission.return_value = True
    ...
    def test_get_machine_documents_grouped(self, mock_check_permission):
        ...
        result = self.service.get_machine_documents(
            machine=self.mock_machine, user=self.mock_user, group_by_type=True
        )
    ...
    def test_get_machine_documents_not_grouped(self, mock_check_permission):
        ...
        result = self.service.get_machine_documents(
            machine=self.mock_machine, user=self.mock_user, group_by_type=False
        )
    ...
    def test_get_document_download_path_success(
        self, mock_settings, mock_filepath_class, mock_check_permission
    ):
        """测试成功获取下载路径"""
        mock_check_permission.return_value = True
        ...
        mock_file_path.resolve.return_value.relative_to.return_value = FilePath(
            "documents/machines/1/test.pdf"
        )

        mock_filepath_class.side_effect = [mock_file_path, MagicMock()]

        # 执行
        file_path, file_name = self.service.get_document_download_path(
            mock_document, self.mock_user
        )
    ...
    @patch("app.services.machine_custom.service.FilePath")
    @patch("app.services.machine_custom.service.settings")
    def test_get_document_download_path_file_not_exists(
        self, mock_settings, mock_filepath_class, mock_check_permission
    ):
        """测试文件不存在"""
        mock_check_permission.return_value = True
    ...
    @patch("app.services.machine_custom.service.FilePath")
    @patch("app.services.machine_custom.service.settings")
    def test_get_document_download_path_security_check(
        self, mock_settings, mock_filepath_class, mock_check_permission
    ):
        """测试路径安全检查（防止目录遍历攻击）"""
        mock_check_permission.return_value = True

...
class TestManagementRhythmDashboardAdapter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 创建适配器实例
        self.adapter = ManagementRhythmDashboardAdapter(db=self.db, current_user=self.current_user)
    ...
    def test_get_stats_with_snapshot_data(self):
        ...
        strategic_query.filter.return_value.order_by.return_value.first.return_value = (
            mock_strategic_snapshot
        )

...
class TestBOMService:
    ...
    def _make_db_with_bom(
        self, bom_status="APPROVED", bom_name="TestBOM", has_items=True, vendor_found=True
    ):
    ...
    def test_approve_bom_raises_when_not_found(self):
        ...
        with (
            patch("app.services.material.bom_service.select", return_value=MagicMock()),
            ...
            with pytest.raises(ValueError, match="BOM不存在"):
                _run(
                    BOMService.approve_bom_and_create_purchase_orders(db, bom_id=999, approved_by=1)
                )
    ...
    def test_approve_bom_returns_no_items_message(self):
        ...
        with (
            ...
            result = _run(
                BOMService.approve_bom_and_create_purchase_orders(db, bom_id=1, approved_by=1)
            )
    ...
    def test_approve_bom_success_creates_purchase_orders(self):
        ...
        with (
            ...
            db.execute = AsyncMock(
                side_effect=[mock_bom_result, mock_items_result, mock_vendor_result]
            )
            result = _run(
                BOMService.approve_bom_and_create_purchase_orders(db, bom_id=1, approved_by=1)
            )

...
class TestCheckTransferAvailable(unittest.TestCase):
    ...
    @patch(
        "app.services.material_transfer_service.MaterialTransferService.get_project_material_stock"
    )
    def test_check_sufficient_stock(self, mock_get_stock):
        """测试库存充足的情况"""
        mock_get_stock.return_value = {"available_qty": Decimal("100"), "source": "项目物料表"}

        result = self.service.check_transfer_available(self.db, 1, 1, Decimal("50"))
    ...
    def test_check_insufficient_stock(self, mock_get_stock):
        """测试库存不足的情况"""
        mock_get_stock.return_value = {"available_qty": Decimal("30"), "source": "项目物料表"}
    ...
    def test_check_exact_stock(self, mock_get_stock):
        """测试库存刚好够用"""
        mock_get_stock.return_value = {"available_qty": Decimal("50"), "source": "项目物料表"}
    ...
    def test_check_zero_stock(self, mock_get_stock):
        """测试零库存"""
        mock_get_stock.return_value = {"available_qty": Decimal("0"), "source": "项目物料表"}

        result = self.service.check_transfer_available(self.db, 1, 1, Decimal("10"))
...
class TestExecuteStockUpdate(unittest.TestCase):
    ...
    def test_execute_update_with_actual_qty(self, mock_update):
        ...
        # 使用实际数量40，而非计划的50
        result = self.service.execute_stock_update(self.db, mock_transfer, actual_qty=Decimal("40"))
...
class TestRecordTransaction(unittest.TestCase):
    ...
    def test_record_transaction_success(self):
        ...
        self.service._record_transaction(self.db, 1, 10, Decimal("50"), "TRANSFER_IN", "测试备注")
    ...
    def test_record_transaction_with_exception(self):
        ...
        # 不应该抛出异常
        try:
            self.service._record_transaction(self.db, 1, 10, Decimal("50"), "TRANSFER_IN", "测试")
        except Exception:
...
class TestValidateTransferBeforeExecute(unittest.TestCase):
    ...
    @patch(
        "app.services.material_transfer_service.MaterialTransferService.check_transfer_available"
    )

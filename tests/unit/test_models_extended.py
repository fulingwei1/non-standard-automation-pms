...
class TestMachineModel:
    ...
    def test_machine_required_fields(self, db_session: Session, test_project_with_customer):
        """测试设备必填字段"""
        timestamp = datetime.now().timestamp()
        machine = Machine(
            project_id=test_project_with_customer.id,
    ...
    def test_machine_stage_and_status(self, db_session: Session, test_project_with_customer):
        timestamp = datetime.now().timestamp()
        machine = Machine(
            project_id=test_project_with_customer.id,
    ...
    def test_machine_specification(self, db_session: Session, test_project_with_customer):
        timestamp = datetime.now().timestamp()
        machine = Machine(
            project_id=test_project_with_customer.id,
...
class TestMaterialModel:
    ...
    def test_material_repr(self, db_session: Session):
        """测试物料字符串表示"""
        timestamp = datetime.now().timestamp()
        material = Material(material_code=f"MATREP{timestamp}", material_name="repr测试")
        db_session.add(material)
        db_session.commit()
...
class TestSupplierModel:
    ...
    def test_supplier_repr(self, db_session: Session):
        """测试供应商字符串表示"""
        timestamp = datetime.now().timestamp()
        supplier = Supplier(supplier_code=f"SUPREP{timestamp}", supplier_name="repr测试")
        db_session.add(supplier)
        db_session.commit()
...
class TestBomHeaderModel:
    ...
    def test_bom_header_required_fields(self, db_session: Session, test_project_with_customer):
        """测试BOM表头必填字段"""
        timestamp = datetime.now().timestamp()
        bom = BomHeader(
...
class TestPurchaseOrderModel:
    ...
    @pytest.fixture
    def test_purchase_order_data(self, test_user, test_project_with_customer, test_supplier):
        """测试采购订单数据"""
        timestamp = datetime.now().timestamp()
        return {
    ...
    def test_purchase_order_creation(self, db_session: Session, test_purchase_order_data):
        po = PurchaseOrder(
...
class TestPurchaseOrderItemModel:
    ...
    def test_purchase_order_item_creation(self, db_session: Session, test_purchase_order_item_data):
    ...
    def test_purchase_order_item_required_fields(self, db_session: Session, test_po_for_items):
        """测试采购订单明细必填字段"""
        timestamp = datetime.now().timestamp()
        item = PurchaseOrderItem(
            order_id=test_po_for_items.id,
...
class TestEcnModel:
    ...
    def test_ecn_required_fields(self, db_session: Session, test_project_with_customer, test_user):
        """测试ECN必填字段"""
        timestamp = datetime.now().timestamp()
        ecn = Ecn(
    ...
    def test_ecn_status(self, db_session: Session, test_project_with_customer, test_user):
        """测试ECN状态"""
        timestamp = datetime.now().timestamp()
        ecn = Ecn(
    ...
    def test_ecn_priority(self, db_session: Session, test_project_with_customer, test_user):
        """测试ECN优先级"""
        timestamp = datetime.now().timestamp()
        ecn = Ecn(
    ...
    def test_ecn_impact(self, db_session: Session, test_project_with_customer, test_user):
        """测试ECN影响"""
        timestamp = datetime.now().timestamp()
        ecn = Ecn(
    ...
    def test_ecn_machine_relationship(self, db_session: Session, test_project_with_customer):
        """测试ECN-设备关系"""
        timestamp = datetime.now().timestamp()
        machine = Machine(
            project_id=test_project_with_customer.id,
...
class TestEcnEvaluationModel:
    ...
    def test_ecn_evaluation_creation(self, db_session: Session, test_ecn_evaluation_data):
    ...
    def test_ecn_evaluation_user(self, db_session: Session, test_ecn_for_ecn_evaluation, test_user):
        """测试ECN评估人"""
        ecn_evaluation = EcnEvaluation(
            ecn_id=test_ecn_for_ecn_evaluation.id,
            eval_dept="工程部",
    ...
    def test_ecn_evaluation_repr(self, db_session: Session, test_ecn_for_ecn_evaluation, test_user):
        """测试ECN评估字符串表示"""
        ecn_evaluation = EcnEvaluation(
            ecn_id=test_ecn_for_ecn_evaluation.id,
            eval_dept="工程部",
...
class TestEcnTaskModel:
    ...
    def test_ecn_task_required_fields(self, db_session: Session, test_ecn_for_task, test_user):
        """测试ECN任务必填字段"""
        task = EcnTask(
            ecn_id=test_ecn_for_task.id,
            task_no=2,
...
class TestAcceptanceOrderModel:
    ...
    def test_acceptance_order_creation(self, db_session: Session, test_acceptance_order_data):
    ...
    def test_acceptance_order_dates(self, db_session: Session, test_machine_for_acceptance):
        """测试验收订单日期"""
        today = date.today()
        now = datetime.now()
        timestamp = datetime.now().timestamp()
    ...
    def test_acceptance_order_location(self, db_session: Session, test_machine_for_acceptance):
        """测试验收订单地点"""
        timestamp = datetime.now().timestamp()
        order = AcceptanceOrder(
    ...
    def test_acceptance_order_status(self, db_session: Session, test_machine_for_acceptance):
        """测试验收订单状态"""
        timestamp = datetime.now().timestamp()
        order = AcceptanceOrder(
    ...
    def test_acceptance_order_statistics(self, db_session: Session, test_machine_for_acceptance):
        """测试验收订单统计"""
        today = date.today()
        timestamp = datetime.now().timestamp()
        order = AcceptanceOrder(
    ...
    def test_acceptance_order_conclusion(self, db_session: Session, test_machine_for_acceptance):
        """测试验收订单结论"""
        timestamp = datetime.now().timestamp()
        order = AcceptanceOrder(
...
class TestAcceptanceOrderItemModel:
    ...
    def test_acceptance_order_item_sort(self, db_session: Session, test_acceptance_order_for_items):
        """测试验收订单明细排序"""
        timestamp = datetime.now().timestamp()
        item = AcceptanceOrderItem(
            order_id=test_acceptance_order_for_items.id,
    ...
    def test_acceptance_order_item_repr(self, db_session: Session, test_acceptance_order_for_items):
        """测试验收订单明细字符串表示"""
        item = AcceptanceOrderItem(
            order_id=test_acceptance_order_for_items.id,
...
class TestOutsourcingOrderModel:
    ...
    def test_outsourcing_order_creation(self, db_session: Session, test_outsourcing_order_data):
    ...
    def test_outsourcing_order_required_fields(self, db_session: Session, test_outsourcing_vendor):
        """测试外协订单必填字段"""
        timestamp = datetime.now().timestamp()
        order = OutsourcingOrder(
    ...
    def test_outsourcing_order_amounts(self, db_session: Session, test_outsourcing_vendor):
        """测试外协订单金额"""
        timestamp = datetime.now().timestamp()
        order = OutsourcingOrder(
    ...
    def test_outsourcing_order_dates(self, db_session: Session, test_outsourcing_vendor):
        """测试外协订单日期"""
        today = date.today()
        timestamp = datetime.now().timestamp()
        order = OutsourcingOrder(
    ...
    def test_outsourcing_order_status(self, db_session: Session, test_outsourcing_vendor):
        """测试外协订单状态"""
        timestamp = datetime.now().timestamp()
        order = OutsourcingOrder(
    ...
    def test_outsourcing_order_delivery(self, db_session: Session, test_outsourcing_vendor):
        """测试外协订单交付"""
        timestamp = datetime.now().timestamp()
        order = OutsourcingOrder(

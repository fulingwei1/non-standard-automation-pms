...
class TestNotificationDispatcherCore(unittest.TestCase):
    ...
    def test_resolve_recipients_by_ids_invalid_types(self):
        """测试过滤无效类型的ID"""
        # Mock数据库查询
        user1 = User(id=1, username="user1", is_active=True, password_hash="test_hash_123")
        user2 = User(id=2, username="user2", is_active=True, password_hash="test_hash_123")
    ...
    def test_resolve_recipient_id_from_user(self):
        ...
        user = User(id=456, password_hash="test_hash_123")
    ...
    def test_build_notification_request_with_user(self):
        ...
        user = User(id=200, password_hash="test_hash_123")
    ...
    def test_dispatch_success(self, mock_failure, mock_success, mock_get_service):
        ...
        user = User(id=100, password_hash="test_hash_123")
    ...
    def test_dispatch_alert_notifications_success(
        ...
        """测试批量分发预警通知成功"""
        # Mock解析接收者
        user1 = User(id=1, username="user1", email="user1@test.com", password_hash="test_hash_123")
        user2 = User(id=2, username="user2", email="user2@test.com", password_hash="test_hash_123")
    ...
    def test_dispatch_alert_notifications_fallback_to_direct(
        ...
        user = User(id=1, username="user1", email="user1@test.com", password_hash="test_hash_123")
        ...
        with (
            patch(
                "app.services.notification_dispatcher.resolve_recipients"
            ) as mock_resolve_recipients,
            patch("app.services.notification_dispatcher.resolve_channels") as mock_resolve_channels,
            patch(
                "app.services.notification_dispatcher.resolve_channel_target"
    ...
    def test_dispatch_alert_notifications_no_target(self, mock_enqueue):
        """测试无法解析目标时跳过"""
        # Mock数据库查询
        user = User(id=1, username="user1", email=None, password_hash="test_hash_123")  # 无邮箱
        mock_user_query = MagicMock()
        mock_user_query.filter.return_value.filter.return_value.all.return_value = [user]
    ...
    def test_dispatch_alert_notifications_exception_in_resolve_channels(self):
        ...
        with (
            ...
            with (
                patch("app.services.notification_queue.enqueue_notification") as mock_enqueue,
                patch(
                    "app.services.notification_dispatcher.resolve_channel_target"
                ) as mock_resolve_target,
                patch(
                    "app.services.notification_dispatcher.channel_allowed"
                ) as mock_channel_allowed,
            ):
...
class TestNotificationDispatcherEdgeCases(unittest.TestCase):
    ...
    def test_resolve_recipients_by_ids_with_duplicates(self):
        """测试重复的用户ID"""
        user = User(id=1, username="user1", is_active=True, password_hash="test_hash_123")

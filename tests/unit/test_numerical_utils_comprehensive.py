...
class TestCalcSpiSafe:
    ...
    def test_with_decimal_inputs(self):
        """Decimal输入"""
        result = calc_spi_safe(ev=Decimal("100"), pv=Decimal("80"))
        assert result == Decimal("1.250000")
...
class TestCalcCpi:
    ...
    def test_with_decimal_inputs(self):
        """Decimal输入"""
        result = calc_cpi(ev=Decimal("150.50"), ac=Decimal("100.25"))
...
class TestCalcEac:
    ...
    def test_with_decimal_inputs(self):
        """Decimal输入"""
        result = calc_eac(bac=Decimal("1000"), ev=Decimal("500"), ac=Decimal("600"))
        assert isinstance(result, Decimal)
...
class TestCalcVac:
    ...
    def test_with_decimal_inputs(self):
        """Decimal输入"""
        result = calc_vac(bac=Decimal("1000.50"), eac=Decimal("900.25"))
...
class TestCalcPriceWithVat:
    ...
    def test_with_decimal_inputs(self):
        """Decimal输入"""
        result = calc_price_with_vat(price=Decimal("100.50"), vat_rate=Decimal("0.13"))
...
class TestCalcPriceBreakdown:
    ...
    def test_normal_breakdown(self):
        """正常报价分解"""
        result = calc_price_breakdown(hardware=5000, labor=3000, outsource=2000, margin_rate=0.30)
        # 成本合计 = 10000
    ...
    def test_zero_margin(self):
        """零利润率"""
        result = calc_price_breakdown(hardware=1000, labor=0, outsource=0, margin_rate=0)
        assert result["price"] == result["cost_subtotal"]
        assert result["profit"] == Decimal("0.00")

    def test_high_margin(self):
        """高利润率"""
        result = calc_price_breakdown(hardware=1000, labor=0, outsource=0, margin_rate=0.50)
        # 报价 = 1000 / (1 - 0.5) = 2000
    ...
    def test_margin_rate_one_raises_error(self):
        ...
        with pytest.raises(ValueError, match="毛利率必须在.*区间"):
            calc_price_breakdown(hardware=1000, labor=0, outsource=0, margin_rate=1.0)

    def test_negative_margin_raises_error(self):
        """负毛利率抛出异常"""
        with pytest.raises(ValueError, match="毛利率必须在.*区间"):
            calc_price_breakdown(hardware=1000, labor=0, outsource=0, margin_rate=-0.1)

    def test_margin_over_one_raises_error(self):
        """毛利率大于1时抛出异常"""
        with pytest.raises(ValueError, match="毛利率必须在.*区间"):
            calc_price_breakdown(hardware=1000, labor=0, outsource=0, margin_rate=1.5)
...
class TestPageResult:
    ...
    def test_custom_values(self):
        """自定义值"""
        page_result = PageResult(
            items=[1, 2, 3], total=100, page=2, page_size=20, total_pages=5, items_count=3
        )

...
class TestOutsourcingAdapterCore(unittest.TestCase):
    ...
    def test_get_entity_data_with_null_dates(self):
        """测试日期为None的情况"""
        order = self._create_sample_order(required_date=None, estimated_date=None, actual_date=None)
    ...
    def test_generate_summary_minimal_info(self):
        """测试最小信息（无可选字段）"""
        order = self._create_sample_order(required_date=None, project_id=None, machine_id=None)
    ...
    def test_get_cc_user_ids_with_project_manager(self):
        ...
        with patch.object(
            self.adapter, "get_department_manager_user_ids_by_codes", return_value=[]
        ):
    ...
    def test_get_cc_user_ids_with_prod_dept_managers(self):
        ...
        with patch.object(
            self.adapter, "get_department_manager_user_ids_by_codes", return_value=[300, 301]
        ):
            result = self.adapter.get_cc_user_ids(1)
    ...
    def test_get_cc_user_ids_deduplication(self):
        ...
        # 生产部门负责人也是500，应该去重
        with patch.object(
            self.adapter, "get_department_manager_user_ids_by_codes", return_value=[500, 501]
        ):
            result = self.adapter.get_cc_user_ids(1)
    ...
    def test_get_cc_user_ids_project_without_manager(self):
        ...
        with patch.object(
            self.adapter, "get_department_manager_user_ids_by_codes", return_value=[600]
        ):
            result = self.adapter.get_cc_user_ids(1)

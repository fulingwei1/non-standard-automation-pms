...
class TestChatPromptBuilding:
    ...
    def test_build_technical_prompt(self):
        """测试技术问题提示词"""
        prompt = self.service._build_chat_prompt("性能参数是什么", QuestionType.TECHNICAL, None)
    ...
    def test_build_competitor_prompt(self):
        """测试竞品对比提示词"""
        prompt = self.service._build_chat_prompt("竞品对比", QuestionType.COMPETITOR, None)
        assert "竞品对比分析" in prompt
        assert "优势和差异化" in prompt

    def test_build_case_prompt(self):
        """测试案例问题提示词"""
        prompt = self.service._build_chat_prompt("成功案例", QuestionType.CASE, None)
        assert "成功案例" in prompt
        assert "项目背景和需求" in prompt

    def test_build_pricing_prompt(self):
        """测试报价问题提示词"""
        prompt = self.service._build_chat_prompt("价格多少", QuestionType.PRICING, None)
    ...
    def test_build_prompt_with_context(self):
        ...
        prompt = self.service._build_chat_prompt("价格多少", QuestionType.PRICING, context)
        assert "ABC公司" in prompt
        assert "自动化改造" in prompt

    def test_build_other_prompt(self):
        """测试其他类型问题提示词"""
        prompt = self.service._build_chat_prompt("你好", QuestionType.OTHER, None)
        assert "你好" in prompt
    ...
    async def test_chat_without_ticket_id(self):
        ...
        with patch.object(self.service, "_call_ai_service", new_callable=AsyncMock) as mock_ai:
            ...
            with patch("app.services.presale_mobile_service.save_obj"):
                result = await self.service.chat(user_id=1, question="问题")
    ...
    async def test_voice_question_success(self):
        ...
        with patch.object(self.service, "_speech_to_text", new_callable=AsyncMock) as mock_stt:
            ...
            with patch.object(self.service, "chat", new_callable=AsyncMock) as mock_chat:
                ...
                with patch.object(
                    self.service, "_text_to_speech", new_callable=AsyncMock
                ) as mock_tts:
...
class TestQuickEstimate:
    ...
    async def test_quick_estimate_with_photo(self):
        """测试有照片的快速估价"""
        with patch.object(
            self.service, "_recognize_equipment", new_callable=AsyncMock
        ) as mock_recognize:
            mock_recognize.return_value = {"equipment_name": "识别的设备", "confidence": 90}

            with patch("app.services.presale_mobile_service.save_obj"):
                result = await self.service.quick_estimate(
                    user_id=1, equipment_description="测试设备", equipment_photo_base64="photo_data"
                )
    ...
    async def test_estimate_price_range_calculation(self):
        """测试估价范围计算"""
        with patch("app.services.presale_mobile_service.save_obj"):
            result = await self.service.quick_estimate(user_id=1, equipment_description="测试设备")
...
class TestVisitRecords:
    ...
    async def test_voice_to_visit_record(self):
        ...
        with patch.object(self.service, "_speech_to_text", new_callable=AsyncMock) as mock_stt:
            ...
            with patch.object(
                self.service, "_extract_visit_info", new_callable=AsyncMock
            ) as mock_extract:
...
class TestOfflineDataSync:
    ...
    def test_sync_chat_data_success(self):
        ...
        result = self.service.sync_offline_data(
            user_id=1, data_type="chat", local_id="local_123", data_payload=chat_data
        )
    ...
    def test_sync_visit_data_success(self):
        ...
        result = self.service.sync_offline_data(
            user_id=1, data_type="visit", local_id="local_456", data_payload=visit_data
        )
    ...
    def test_sync_estimate_data_success(self):
        ...
        result = self.service.sync_offline_data(
            user_id=1, data_type="estimate", local_id="local_789", data_payload=estimate_data
        )
    ...
    def test_sync_already_synced_data(self):
        ...
        result = self.service.sync_offline_data(
            user_id=1, data_type="chat", local_id="local_123", data_payload={"test": "data"}
        )
    ...
    def test_sync_data_failure(self):
        ...
        with patch.object(self.service, "_sync_chat_data", side_effect=Exception("同步错误")):
            result = self.service.sync_offline_data(
                user_id=1, data_type="chat", local_id="local_error", data_payload={"test": "data"}
            )
    ...
    def test_sync_chat_data_internal(self):
        ...
        with patch(
            "app.services.presale_mobile_service.PresaleMobileAssistantChat", return_value=mock_chat
        ):
    ...
    def test_sync_visit_data_internal(self):
        ...
        with patch(
            "app.services.presale_mobile_service.PresaleVisitRecord", return_value=mock_visit
        ):
    ...
    def test_sync_estimate_data_internal(self):
        ...
        with patch(
            "app.services.presale_mobile_service.PresaleMobileQuickEstimate",
...
class TestEdgeCases:
    ...
    async def test_chat_with_empty_context(self):
        ...
        with patch.object(self.service, "_call_ai_service", new_callable=AsyncMock) as mock_ai:
            mock_ai.return_value = "回答"
            with patch("app.services.presale_mobile_service.save_obj"):
                result = await self.service.chat(user_id=1, question="测试", context=None)
...
if __name__ == "__main__":
    pytest.main(
        [__file__, "-v", "--cov=app.services.presale_mobile_service", "--cov-report=term-missing"]
    )

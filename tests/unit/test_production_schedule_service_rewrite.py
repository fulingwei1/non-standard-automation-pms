...
class TestProductionScheduleServiceCore(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Mock db.add/db.add_all to auto-populate ORM fields that are normally set after flush
        def populate_orm_fields(obj):
            if getattr(obj, "__table__", None) is None:
                return
            if hasattr(obj, "id") and obj.id is None:
                obj.id = self._next_orm_id
                self._next_orm_id += 1
            if hasattr(obj, "created_at") and obj.created_at is None:
                obj.created_at = datetime.now()
            if hasattr(obj, "updated_at") and obj.updated_at is None:
                obj.updated_at = datetime.now()
            if hasattr(obj, "is_manually_adjusted") and obj.is_manually_adjusted is None:
                obj.is_manually_adjusted = False

        self.db.add.side_effect = populate_orm_fields
        self.db.add_all.side_effect = lambda items: [populate_orm_fields(item) for item in items]
    ...
    def test_get_conflict_summary_by_plan(self):
        ...
        conflict1.id = 1
        conflict1.schedule_id = 1
        conflict1.conflicting_schedule_id = None
        conflict1.conflict_type = "EQUIPMENT"
        conflict1.resource_type = "equipment"
        conflict1.resource_id = 1
        conflict1.conflict_description = "设备 1 时间冲突"
        conflict1.severity = "HIGH"
        conflict1.conflict_start_time = datetime(2024, 1, 1, 9, 0)
        conflict1.conflict_end_time = datetime(2024, 1, 1, 10, 0)
        conflict1.overlap_duration_hours = 1.0
        conflict1.resolution_suggestion = None
        conflict1.status = "UNRESOLVED"
        conflict1.detected_at = datetime.now()
        ...
        # 服务会先初始化冲突查询，再查询方案下的 schedule_id
        self.db.query.side_effect = [
            mock_query_conflicts,
            mock_query_schedules,
        ]
    ...
    def test_get_schedule_history(self):
        ...
        # page=1 时 offset=0，分页工具只会追加 limit
        mock_paginated_query = MagicMock()
        mock_ordered_query.limit.return_value = mock_paginated_query

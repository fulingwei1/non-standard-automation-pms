...
class TestHandleShortageAlertCreated:
    ...
    def test_low_level_alert(self, service, db):
        alert = MagicMock(
            project_id=1,
            alert_level="level1",
            alert_data=json.dumps({"impact_type": "none", "estimated_delay_days": 0}),
        )
...
class TestHandleEcnApproved:
    ...
    def test_major_impact_adjusts_tasks(self, service, db):
        ...
        task = MagicMock(
            id=1, task_name="Test", plan_end=datetime.now(), actual_start=None, plan_start=None
        )
...
class TestCheckMilestoneCompletionRequirements:
    ...
    def test_delivery_with_unapproved(self, service):
        ...
        milestone = MagicMock(milestone_type="DELIVERY", deliverables=deliverables, project_id=1)
...
class TestHandleAcceptancePassed:
    ...
    def test_unblocks_milestones(self, service, db):
        order = MagicMock(overall_result="PASSED", acceptance_type="FAT", project_id=1, id=1)

...
def test_validate_status_transition_submitted_to_assessing(service):
    """测试从已提交到评估中"""
    assert (
        service.validate_status_transition(ChangeStatusEnum.SUBMITTED, ChangeStatusEnum.ASSESSING)
        is True
    )
...
def test_validate_status_transition_assessing_to_pending_approval(service):
    ...
    assert (
        service.validate_status_transition(
            ChangeStatusEnum.ASSESSING, ChangeStatusEnum.PENDING_APPROVAL
        )
        is True
    )
...
def test_validate_status_transition_pending_to_approved(service):
    ...
    assert (
        service.validate_status_transition(
            ChangeStatusEnum.PENDING_APPROVAL, ChangeStatusEnum.APPROVED
        )
        is True
    )
...
def test_validate_status_transition_approved_to_implementing(service):
    """测试从已批准到实施中"""
    assert (
        service.validate_status_transition(ChangeStatusEnum.APPROVED, ChangeStatusEnum.IMPLEMENTING)
        is True
    )
...
def test_validate_status_transition_implementing_to_verifying(service):
    ...
    assert (
        service.validate_status_transition(
            ChangeStatusEnum.IMPLEMENTING, ChangeStatusEnum.VERIFYING
        )
        is True
    )
...
def test_validate_status_transition_verifying_to_closed(service):
    """测试从验证中到已关闭"""
    assert (
        service.validate_status_transition(ChangeStatusEnum.VERIFYING, ChangeStatusEnum.CLOSED)
        is True
    )
...
def test_validate_status_transition_invalid_rejected_to_approved(service):
    """测试非法转换：已拒绝到已批准"""
    assert (
        service.validate_status_transition(ChangeStatusEnum.REJECTED, ChangeStatusEnum.APPROVED)
        is False
    )
...
def test_validate_status_transition_invalid_closed_to_implementing(service):
    """测试非法转换：已关闭到实施中"""
    assert (
        service.validate_status_transition(ChangeStatusEnum.CLOSED, ChangeStatusEnum.IMPLEMENTING)
        is False
    )
...
def test_validate_status_transition_cancelled_to_any(service):
    """测试取消状态不能转换"""
    assert (
        service.validate_status_transition(ChangeStatusEnum.CANCELLED, ChangeStatusEnum.SUBMITTED)
        is False
    )
...
def test_list_change_requests_filter_by_status(service, mock_db):
    ...
    with patch("app.services.project_change_requests.service.apply_pagination") as mock_pagination:
        ...
        service.list_change_requests(offset=0, limit=10, status=ChangeStatusEnum.PENDING_APPROVAL)

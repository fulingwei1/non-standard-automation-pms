...
class TestProjectContributionService(unittest.TestCase):
    ...
    def test_calculate_member_contribution_new_record(self):
        ...
        result = self.service.calculate_member_contribution(
            project_id=1, user_id=101, period="2024-02"
        )
    ...
    def test_calculate_member_contribution_with_completed_tasks(self):
        ...
        self.db.query.return_value.filter.return_value.all.side_effect = [mock_tasks, [], []]
        self.service.bonus_service.get_project_bonus_calculations.return_value = []
    ...
    def test_get_project_contributions_with_period(self):
        ...
        self.db.query.return_value.filter.return_value.filter.return_value.order_by.return_value.all.return_value = (
            mock_contributions
        )
    ...
    def test_get_user_project_contributions_with_start_period(self):
        ...
        result = self.service.get_user_project_contributions(user_id=101, start_period="2024-01")
    ...
    def test_get_user_project_contributions_with_end_period(self):
        ...
        result = self.service.get_user_project_contributions(user_id=101, end_period="2024-12")
    ...
    def test_get_user_project_contributions_with_both_periods(self):
        ...
        filter_mock.filter.return_value.filter.return_value.order_by.return_value.all.return_value = (
            mock_contributions
        )

        result = self.service.get_user_project_contributions(
            user_id=101, start_period="2024-01", end_period="2024-12"
        )
    ...
    def test_rate_member_contribution_valid_rating(self):
        ...
        existing = ProjectMemberContribution(
            project_id=1, user_id=101, period="2024-02", contribution_score=Decimal("5.0")
        )
        self.db.query.return_value.filter.return_value.first.return_value = existing

        result = self.service.rate_member_contribution(
            project_id=1, user_id=101, period="2024-02", pm_rating=4, rater_id=201
        )
    ...
    def test_rate_member_contribution_invalid_rating_low(self):
        ...
        with self.assertRaises(ValueError) as context:
            self.service.rate_member_contribution(
                project_id=1, user_id=101, period="2024-02", pm_rating=0, rater_id=201
            )
    ...
    def test_rate_member_contribution_invalid_rating_high(self):
        ...
        with self.assertRaises(ValueError) as context:
            self.service.rate_member_contribution(
                project_id=1, user_id=101, period="2024-02", pm_rating=6, rater_id=201
            )
    ...
    def test_rate_member_contribution_no_existing_record(self):
        ...
        result = self.service.rate_member_contribution(
            project_id=1, user_id=101, period="2024-02", pm_rating=3, rater_id=201
        )
    ...
    def test_rate_member_contribution_boundary_values(self):
        """测试评分 - 边界值（1和5）"""
        existing = ProjectMemberContribution(
            project_id=1, user_id=101, period="2024-02", contribution_score=Decimal("10.0")
        )
        self.db.query.return_value.filter.return_value.first.return_value = existing
    ...
    def test_generate_contribution_report_user_name_fallback(self):
        ...
        contributions = [
            # 有employee.name
            self._create_contribution_with_user(
                user_id=101, employee_name="张三", real_name="zhangsan", username="user1"
            ),
            # 只有real_name
            self._create_contribution_with_user(
                user_id=102, employee_name=None, real_name="李四", username="user2"
            ),
            # 只有username
            self._create_contribution_with_user(
                user_id=103, employee_name=None, real_name=None, username="user3"
            ),
            # 都没有
            self._create_contribution_with_user(
                user_id=104, employee_name=None, real_name=None, username=None
            ),
        ]
        self.db.query.return_value.filter.return_value.order_by.return_value.all.return_value = (
            contributions
    ...
    def test_generate_contribution_report_top_contributors_limit(self):
        ...
        mock_contributions = [
            self._create_contribution_with_user(
                user_id=100 + i, username=f"user{i}", score=Decimal(str(15 - i))  # 分数递减
            )
            for i in range(15)
        ]
        self.db.query.return_value.filter.return_value.order_by.return_value.all.return_value = (

...
class TestProjectCostPredictionService(unittest.TestCase):
    ...
    def test_create_prediction_traditional_method(self):
        ...
        result = self.service.create_prediction(
            project_id=self.project_id, use_ai=False, created_by=1, notes="测试预测"
        )
    ...
    def test_create_prediction_with_ai(self):
        ...
        # 执行预测
        result = self.service.create_prediction(
            project_id=self.project_id, use_ai=True, prediction_version="V2.0"
        )
    ...
    def test_create_prediction_with_high_risk_generates_suggestions(self):
        ...
        # 执行预测
        result = self.service.create_prediction(project_id=self.project_id, use_ai=True)
    ...
    def test_create_prediction_record_calculation(self):
        ...
        # 验证超支百分比计算
        expected_percentage = (expected_overrun / self.latest_evm.budget_at_completion) * Decimal(
            "100"
        )
    ...
    def test_generate_optimization_suggestions_with_exception(self):
        """测试生成建议时的异常处理"""
        self.service.ai_predictor = MagicMock()
        self.service.ai_predictor.generate_optimization_suggestions.side_effect = Exception(
            "API Error"
        )

...
class TestProjectEvaluationService(unittest.TestCase):
    ...
    def test_get_dimension_weights_normalization(self):
        ...
        self.db.query.return_value.filter.return_value.all.return_value = [mock_dim1, mock_dim2]
    ...
    def test_get_level_thresholds_from_db(self):
        ...
        mock_config.scoring_rules = {"S": 90, "A": 80, "B": 70, "C": 60, "D": 0}
    ...
    def test_calculate_total_score_default_weights(self):
        """测试使用默认权重计算总分"""
        with patch.object(
            self.service, "get_dimension_weights", return_value=self.service.DEFAULT_WEIGHTS
        ):
    ...
    def test_determine_evaluation_level_s(self):
        """测试S级评价"""
        with patch.object(
            self.service, "get_level_thresholds", return_value=self.service.DEFAULT_LEVEL_THRESHOLDS
        ):
    ...
    def test_create_evaluation_basic(self):
        """测试创建基本评价记录"""
        with (
            patch.object(self.service, "calculate_total_score", return_value=Decimal("8.5")),

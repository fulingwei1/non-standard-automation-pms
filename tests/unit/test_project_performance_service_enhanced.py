...
class TestCheckPermission:
    ...
    def test_superuser_can_view_all(self, perf_service, superuser, regular_user):
        """测试超级管理员可以查看所有用户的绩效"""
        result = perf_service.check_performance_view_permission(superuser, regular_user.id)
        assert result is True

    def test_user_can_view_own_performance(self, perf_service, regular_user):
        """测试用户可以查看自己的绩效"""
        result = perf_service.check_performance_view_permission(regular_user, regular_user.id)
        assert result is True
    ...
    def test_dept_manager_can_view_dept_member(
        ...
        """测试部门经理可以查看本部门成员的绩效"""
        result = perf_service.check_performance_view_permission(dept_manager, regular_user.id)
        assert result is True
    ...
    def test_dept_manager_cannot_view_other_dept(
        ...
        result = perf_service.check_performance_view_permission(dept_manager, other_user.id)
    ...
    def test_project_manager_can_view_team_member(
        ...
        result = perf_service.check_performance_view_permission(project_manager, regular_user.id)
        assert result is True
    ...
    def test_regular_user_cannot_view_others(
        ...
        result = perf_service.check_performance_view_permission(regular_user, other_user.id)
        assert result is False

    def test_permission_check_with_nonexistent_user(self, perf_service, regular_user):
        """测试查看不存在用户的权限"""
        result = perf_service.check_performance_view_permission(regular_user, 99999)
        assert result is False

    def test_permission_check_without_roles(self, perf_service, db_session, test_department):
        """测试没有角色的用户权限检查"""
        from app.core.security import get_password_hash
...
class TestTeamAndDepartment:
    ...
    def test_get_team_members(self, perf_service, test_department, regular_user, db_session):
        """测试获取团队成员列表"""
        from app.core.security import get_password_hash
    ...
    def test_get_department_members(self, perf_service, test_department, regular_user):
        """测试获取部门成员列表"""
...
class TestEvaluatorType:
    ...
    def test_evaluator_type_with_chinese_role(self, perf_service, db_session, test_department):
        """测试中文角色名称的识别"""
        from app.core.security import get_password_hash
        from app.models.user import Role, UserRole
...
class TestProjectPerformance:
    ...
    def test_get_project_performance_nonexistent_project(self, perf_service, test_period):
        """测试获取不存在项目的绩效"""
        with pytest.raises(ValueError, match="项目不存在"):
            perf_service.get_project_performance(99999, test_period.id)

    def test_get_project_performance_no_period(self, perf_service, test_project, db_session):
        """测试没有周期时获取绩效"""
        # 删除所有finalized周期
...
class TestProjectProgressReport:
    ...
    def test_get_progress_report_monthly(self, perf_service, test_project):
        """测试月报类型"""
        result = perf_service.get_project_progress_report(test_project.id, report_type="MONTHLY")
        assert result["report_type"] == "MONTHLY"

    def test_get_progress_report_custom_date(self, perf_service, test_project):
        """测试自定义报告日期"""
        custom_date = date(2024, 6, 15)
        result = perf_service.get_project_progress_report(test_project.id, report_date=custom_date)
...
class TestComparePerformance:
    ...
    def test_compare_performance_success(self, perf_service, test_period, regular_user, db_session):
        """测试成功对比绩效"""
        from app.core.security import get_password_hash
        ...
        comparison = perf_service.compare_performance([regular_user.id, user2.id], test_period.id)
    ...
    def test_compare_performance_missing_user(
        ...
        # 包含一个不存在的用户ID
        comparison = perf_service.compare_performance([regular_user.id, 99999], test_period.id)

        # 应该只返回存在的用户
        assert len(comparison["comparison_data"]) == 1

    def test_compare_performance_no_results(self, perf_service, test_period, regular_user):
        """测试对比没有绩效结果的用户"""
        comparison = perf_service.compare_performance([regular_user.id], test_period.id)

...
def test_create_risk_basic(
    mock_get_or_404, mock_save_obj, risk_service, mock_db, mock_project, mock_user
):
    """测试基本风险创建"""
    mock_get_or_404.return_value = mock_project
    mock_db.query.return_value.filter.return_value.count.return_value = 0
...
def test_get_risk_list_with_occurred_filter_true(
    mock_get_or_404, risk_service, mock_db, mock_project
):
    """测试筛选已发生风险"""
    mock_get_or_404.return_value = mock_project
    mock_filter = MagicMock()
...
def test_update_risk_basic_fields(risk_service, mock_db, mock_risk, mock_user):
    ...
    update_data = {"risk_name": "更新后的风险名称", "description": "更新后的描述"}

    result = risk_service.update_risk(
        project_id=1, risk_id=1, update_data=update_data, current_user=mock_user
    )
...
def test_update_risk_probability_and_impact(risk_service, mock_db, mock_risk, mock_user):
    ...
    update_data = {"probability": 5, "impact": 5}

...
class TestCalculateFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_projects = [
            self._create_mock_project(
                1, "项目A", "进行中", "需求分析", "健康", 1, "张三", 1, "客户A", 100000, 50
            ),
            self._create_mock_project(
                2, "项目B", "进行中", "开发", "风险", 2, "李四", 2, "客户B", 200000, 30
            ),
            self._create_mock_project(
                3, "项目C", "已完成", "验收", "健康", 1, "张三", 1, "客户A", 150000, 100
            ),
            self._create_mock_project(
                4, "项目D", None, None, None, None, None, None, None, None, 0
            ),
        ]
    ...
    def test_calculate_monthly_statistics(self):
        ...
        projects = [
            self._create_mock_project(
                1, "项目1", "进行中", "开发", "健康", 1, "张三", 1, "客户A", 100000, 50
            ),
            self._create_mock_project(
                2, "项目2", "进行中", "开发", "健康", 1, "张三", 1, "客户A", 200000, 50
            ),
        ]
...
class TestProjectStatisticsServiceBase(unittest.TestCase):
    ...
    def test_group_by_field_with_aggregate(self):
        ...
        result = self.service.group_by_field(mock_query, "cost_type", func.sum, "amount")

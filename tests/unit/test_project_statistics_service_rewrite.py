...
class TestCalculateFunctions(unittest.TestCase):
    ...
    def _create_grouped_query(self, rows):
        """创建适配 SQL GROUP BY 实现的查询 mock"""
        mock_query = MagicMock()
        (
            mock_query.with_entities.return_value.filter.return_value.group_by.return_value.all
        ).return_value = rows
        return mock_query
    ...
    def test_calculate_status_statistics(self):
        """测试状态统计"""
        mock_query = self._create_grouped_query([
            ("进行中", 2),
            ("已完成", 1),
        ])

...
class TestPurchaseOrderFromBomService:
    ...
    def test_determine_supplier_for_item_bom_item_supplier(
        ...
        """测试确定供应商 - 使用BOM项中的供应商"""
        result = determine_supplier_for_item(db_session, test_bom_item, default_supplier_id=None)
    ...
    def test_determine_supplier_for_item_no_supplier(self, db_session, test_bom_item):
        ...
        if test_bom_item.material_id:
            material = (
                db_session.query(Material).filter(Material.id == test_bom_item.material_id).first()
            )
            if material:
    ...
    def test_group_items_by_supplier(self, db_session, test_bom, test_bom_item, test_supplier):
        """测试按供应商分组物料"""
        result = group_items_by_supplier(db_session, [test_bom_item], default_supplier_id=None)
    ...
    def test_calculate_order_item(self, test_bom_item):
        """测试计算订单明细项"""
        result = calculate_order_item(test_bom_item, item_no=1, remaining_qty=Decimal("10.0"))

...
from app.schemas.production.quality import DefectAnalysisCreate, QualityInspectionCreate
...
class TestGetQualityTrend:
    ...
    def test_returns_trend_data_and_statistics(self):
        ...
        result = QualityService.get_quality_trend(
            db, start_date=datetime(2025, 1, 1), end_date=datetime(2025, 1, 31)
        )
    ...
    def test_filters_by_material_id(self):
        ...
        result = QualityService.get_quality_trend(
            db, start_date=datetime(2025, 1, 1), end_date=datetime(2025, 1, 31), material_id=1
        )
...
class TestParetoAnalysis:
    ...
    def test_filters_by_material_id(self):
        ...
        result = QualityService.pareto_analysis(
            db, start_date=datetime(2025, 1, 1), end_date=datetime(2025, 12, 31), material_id=1
        )
...
class TestGetQualityStatistics:
    ...
    def test_returns_statistics_dashboard_data(self):
        ...
        db.query.side_effect = [
            ...
            MagicMock(
                filter=lambda *a, **kw: MagicMock(
                    group_by=lambda *a: MagicMock(
                        order_by=lambda *a: MagicMock(
                            limit=lambda *a: MagicMock(all=lambda: [defect_stat])
                        )
                    )
                )
            ),  # 第四次：不良类型统计
            MagicMock(
                filter=lambda *a, **kw: MagicMock(all=lambda: inspections)
            ),  # 第五次：最近7天趋势
        ]
    ...
    def test_calculates_correct_metrics(self):
        ...
        db.query.side_effect = [
            ...
            MagicMock(
                filter=lambda *a, **kw: MagicMock(
                    group_by=lambda *a: MagicMock(
                        order_by=lambda *a: MagicMock(limit=lambda *a: MagicMock(all=lambda: []))
                    )
                )
            ),

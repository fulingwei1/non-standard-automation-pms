...
class TestBuildKeywordConditions:
    ...
    def test_single_keyword_single_field(self, mock_model):
        """测试单个关键词单个字段"""
        conditions = build_keyword_conditions(mock_model, "test", "name")
    ...
    def test_single_keyword_multiple_fields(self, mock_model):
        """测试单个关键词多个字段"""
        conditions = build_keyword_conditions(mock_model, "test", ["name", "code", "description"])
    ...
    def test_multiple_keywords_single_field(self, mock_model):
        """测试多个关键词单个字段"""
        conditions = build_keyword_conditions(mock_model, ["test1", "test2"], "name")

        assert len(conditions) == 2

    def test_multiple_keywords_multiple_fields(self, mock_model):
        """测试多个关键词多个字段"""
        conditions = build_keyword_conditions(mock_model, ["test1", "test2"], ["name", "code"])
    ...
    def test_empty_keyword(self, mock_model):
        """测试空关键词"""
        conditions = build_keyword_conditions(mock_model, "", "name")

        assert len(conditions) == 0

    def test_none_keyword(self, mock_model):
        """测试None关键词"""
        conditions = build_keyword_conditions(mock_model, None, "name")

        assert len(conditions) == 0

    def test_use_like_instead_of_ilike(self, mock_model):
        """测试使用like而不是ilike"""
        conditions = build_keyword_conditions(mock_model, "test", "name", use_ilike=False)
    ...
    def test_non_existent_field(self, mock_model):
        """测试不存在的字段"""
        conditions = build_keyword_conditions(mock_model, "test", "non_existent_field")
...
class TestBuildLikeConditions:
    ...
    def test_single_pattern_single_field(self, mock_model):
        """测试单个模式单个字段"""
        conditions = build_like_conditions(mock_model, "test%", "name")
    ...
    def test_custom_wildcard_pattern(self, mock_model):
        """测试自定义通配符模式"""
        conditions = build_like_conditions(mock_model, "%test%suffix", "name")

        mock_model.name.ilike.assert_called_once_with("%test%suffix")

    def test_multiple_patterns(self, mock_model):
        """测试多个模式"""
        conditions = build_like_conditions(mock_model, ["%test1%", "%test2%"], "name")

        assert len(conditions) == 2

    def test_use_like_instead_of_ilike(self, mock_model):
        """测试使用like"""
        conditions = build_like_conditions(mock_model, "test%", "name", use_ilike=False)
...
class TestApplyKeywordFilter:
    ...
    def test_apply_keyword_filter_with_keyword(self, mock_query, mock_model):
        """测试应用关键词过滤"""
        result = apply_keyword_filter(mock_query, mock_model, "test", "name")
    ...
    def test_apply_keyword_filter_without_keyword(self, mock_query, mock_model):
        """测试无关键词时原样返回"""
        result = apply_keyword_filter(mock_query, mock_model, None, "name")
    ...
    def test_apply_keyword_filter_empty_string(self, mock_query, mock_model):
        """测试空字符串关键词"""
        result = apply_keyword_filter(mock_query, mock_model, "", "name")

        mock_query.filter.assert_not_called()

    def test_apply_keyword_filter_multiple_fields(self, mock_query, mock_model):
        """测试多个字段"""
        result = apply_keyword_filter(mock_query, mock_model, "test", ["name", "code"])
...
class TestApplyLikeFilter:
    ...
    def test_apply_like_filter_with_pattern(self, mock_query, mock_model):
        """测试应用LIKE过滤"""
        result = apply_like_filter(mock_query, mock_model, "test%", "name")
    ...
    def test_apply_like_filter_without_pattern(self, mock_query, mock_model):
        """测试无模式时原样返回"""
        result = apply_like_filter(mock_query, mock_model, None, "name")
...
class TestIntegrationScenarios:
    ...
    def test_search_and_pagination_workflow(self):
        ...
        # 1. 应用关键词过滤
        query = apply_keyword_filter(mock_query, mock_model, "test", ["name", "code"])
...
class TestEdgeCases:
    ...
    def test_very_long_keyword(self):
        ...
        long_keyword = "a" * 1000
        conditions = build_keyword_conditions(mock_model, long_keyword, "name")
    ...
    def test_special_sql_characters(self):
        ...
        # SQL通配符
        keyword = "test%_"
        conditions = build_keyword_conditions(mock_model, keyword, "name")

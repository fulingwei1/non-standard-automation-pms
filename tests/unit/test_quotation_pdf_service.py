...
from app.models.presale_ai_quotation import QuotationStatus, QuotationType
...
class TestQuotationPDFService(unittest.TestCase):
    ...
    def test_generate_pdf_includes_logo_when_provided(self, mock_image, mock_doc_template):
        """测试提供logo时包含在PDF中"""
        quotation = self._create_mock_quotation()
        company_info = {"logo_path": "/path/to/logo.png", "name": "测试公司"}
    ...
    def test_generate_comparison_pdf_creates_timestamped_filename(
        self, mock_datetime, mock_doc_template
    ):
        """测试生成带时间戳的对比PDF文件名"""
        ...
        quotations = [
            self._create_mock_quotation(
                quotation_type=QuotationType.BASIC, total=Decimal("5000.00")
            ),
            self._create_mock_quotation(
                quotation_type=QuotationType.STANDARD, total=Decimal("10000.00")
            ),
            self._create_mock_quotation(
                quotation_type=QuotationType.PREMIUM, total=Decimal("15000.00")
            ),
        ]
    ...
    @patch("app.services.quotation_pdf_service.SimpleDocTemplate")
    @patch("app.services.quotation_pdf_service.Paragraph")
    def test_generate_comparison_pdf_includes_recommendation(
        self, mock_paragraph, mock_doc_template
    ):
        """测试包含推荐方案"""
        quotations = [
            self._create_mock_quotation(quotation_type=QuotationType.BASIC),
        ...
        paragraph_calls = [str(call) for call in mock_paragraph.call_args_list]
        self.assertTrue(
            any("标准版" in str(call) or "推荐" in str(call) for call in paragraph_calls)
        )
    ...
    def test_generate_pdf_with_zero_values(self, mock_doc_template):
        """测试零值处理"""
        quotation = self._create_mock_quotation(tax=Decimal("0.00"), discount=Decimal("0.00"))
        mock_doc = MagicMock()
        mock_doc_template.return_value = mock_doc
    ...
    def test_generate_comparison_pdf_with_company_info(self, mock_doc_template):
        ...
        company_info = {"name": "测试科技有限公司", "logo_path": "/path/to/logo.png"}
...
class TestQuotationPDFServiceIntegration(unittest.TestCase):
    ...
    def test_complete_workflow_single_quotation(self, mock_doc_template):
        """测试完整的单个报价单生成流程"""
        quotation = self._create_mock_quotation(QuotationType.STANDARD, Decimal("10000.00"), 5)

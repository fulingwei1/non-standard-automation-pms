...
class TestQuoteAdapterCore(unittest.TestCase):
    ...
    def test_submit_for_approval_success(self):
        ...
        with patch("app.services.approval_engine.adapters.quote.ApprovalEngineService") as MockEngine:
            ...
            self.assertEqual(call_kwargs["template_code"], "SALES_QUOTE")
            self.assertEqual(call_kwargs["entity_type"], "QUOTE")
            self.assertEqual(call_kwargs["entity_id"], 100)
            self.assertEqual(call_kwargs["initiator_id"], 10)
    ...
    def test_submit_for_approval_default_title_summary(self):
        ...
        with patch("app.services.approval_engine.adapters.quote.ApprovalEngineService") as MockEngine:
            ...
            # 验证使用当前实现的默认标题和摘要
            call_kwargs = mock_engine.submit.call_args[1]
            self.assertEqual(call_kwargs["title"], "报价审批 - QT-2024-002")
            self.assertEqual(call_kwargs["summary"], "报价审批：QT-2024-002")

...
try:
    ...
    from app.models.sales.quotes import Quote, QuoteApproval, QuoteVersion
...
class TestQuoteApprovalAdapter:
    ...
    def test_get_entity_data_margin_already_percentage(self):
        """测试获取实体数据 - 毛利率已是百分比形式"""
        db = make_db()
        version = make_quote_version(gross_margin="25.5")  # 大于1，保持不变
    ...
    def test_get_title(self):
        """测试生成审批标题"""
        db = make_db()
        quote = make_quote(quote_code="QUOTE-123", customer_name="测试公司")
        db.query.return_value.filter.return_value.first.return_value = quote
    ...
    def test_get_summary(self):
        """测试生成审批摘要"""
        db = make_db()
        version = make_quote_version(total_price="200000", gross_margin="22.5", lead_time_days=60)
        quote = make_quote(customer_name="客户X", current_version=version)
        db.query.return_value.filter.return_value.first.return_value = quote
    ...
    def test_submit_for_approval_success(self, mock_workflow_engine):
        ...
        result = adapter.submit_for_approval(
            quote_version, initiator_id=10, title="测试报价审批", summary="测试摘要"
        )
    ...
    def test_update_quote_approval_approve(self):
        ...
        result = adapter.update_quote_approval_from_action(
            task, action="APPROVE", comment="同意此报价"
        )
    ...
    def test_update_quote_approval_reject(self):
        ...
        adapter = QuoteApprovalAdapter(db)
        result = adapter.update_quote_approval_from_action(
            task, action="REJECT", comment="毛利率过低"
        )
    ...
    def test_update_quote_approval_not_found(self):
        ...
        adapter = QuoteApprovalAdapter(db)
        result = adapter.update_quote_approval_from_action(task, action="APPROVE")

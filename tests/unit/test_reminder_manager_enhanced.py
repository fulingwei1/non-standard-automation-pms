...
class TestTimesheetReminderManager(unittest.TestCase):
    ...
    def test_update_reminder_config_success(self):
        ...
        result = self.manager.update_reminder_config(
            config_id=1, priority="HIGH", is_active=False, rule_name="更新后的名称"
        )
    ...
    def test_update_reminder_config_ignore_none_values(self):
        ...
        result = self.manager.update_reminder_config(
            config_id=1, priority="HIGH", rule_name=None  # 应该被忽略
        )
    ...
    def test_check_user_applicable_by_department(self):
        ...
        self.assertFalse(
            self.manager.check_user_applicable(config, user_id=100, department_id=None)
        )
    ...
    def test_check_user_applicable_multiple_conditions(self):
        ...
        # 满足用户ID
        self.assertTrue(
            self.manager.check_user_applicable(config, user_id=100, department_id=99, role_ids=[99])
        )
        # 满足部门
        self.assertTrue(
            self.manager.check_user_applicable(config, user_id=999, department_id=10, role_ids=[99])
        )
        # 满足角色
        self.assertTrue(
            self.manager.check_user_applicable(config, user_id=999, department_id=99, role_ids=[5])
        )
        # 都不满足
        self.assertFalse(
            self.manager.check_user_applicable(config, user_id=999, department_id=99, role_ids=[99])
        )
    ...
    def test_dismiss_reminder_success(self):
        ...
        with patch("app.services.timesheet_reminder.reminder_manager.datetime") as mock_dt:
            ...
            result = self.manager.dismiss_reminder(reminder_id=1, dismissed_by=200, reason="误报")
    ...
    def test_resolve_anomaly_success(self):
        ...
        with patch("app.services.timesheet_reminder.reminder_manager.datetime") as mock_dt:
            ...
            result = self.manager.resolve_anomaly(
                anomaly_id=1, resolved_by=200, resolution_note="已修正工时"
            )

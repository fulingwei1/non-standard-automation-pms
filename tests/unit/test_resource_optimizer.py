...
class TestAIResourceOptimizerSimplified(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 创建优化器实例
        self.optimizer = AIResourceOptimizer(db=self.db, glm_service=self.glm_service)
    ...
    def test_analyze_strengths_high_skill(self):
        ...
        strengths = self.optimizer._analyze_strengths(
            MagicMock(), MagicMock(), skill_match=85.0, performance=70.0
        )
    ...
    def test_analyze_strengths_none(self):
        """测试无优势"""
        strengths = self.optimizer._analyze_strengths(
            MagicMock(), MagicMock(), skill_match=60.0, performance=60.0
        )
    ...
    def test_analyze_weaknesses_low_skill(self):
        ...
        weaknesses = self.optimizer._analyze_weaknesses(
            MagicMock(), MagicMock(), skill_match=50.0, availability=80.0
        )
    ...
    def test_analyze_weaknesses_none(self):
        """测试无劣势"""
        weaknesses = self.optimizer._analyze_weaknesses(
            MagicMock(), MagicMock(), skill_match=70.0, availability=70.0
        )

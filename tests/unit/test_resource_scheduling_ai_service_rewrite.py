...
class TestResourceSchedulingAIService(unittest.TestCase):
    ...
    def test_analyze_resource_utilization_underutilized(self, mock_save):
        ...
        self.service.ai_client.generate_solution.return_value = {
            "content": json.dumps(
                {"key_insights": ["利用率偏低"], "optimization_suggestions": ["考虑增加任务分配"]}
            )
        }
    ...
    def test_analyze_utilization_zero_hours(self, mock_save):
        ...
        # Mock AI
        self.service.ai_client.generate_solution.return_value = {"content": "{}"}
    ...
    def test_full_workflow_detect_and_suggest(self, mock_save):
        ...
        self.db.query.side_effect = [
            ...
            MagicMock(
                filter=MagicMock(return_value=MagicMock(first=MagicMock(return_value=project_a)))
            ),
            MagicMock(
                filter=MagicMock(return_value=MagicMock(first=MagicMock(return_value=project_b)))
            ),
        ]

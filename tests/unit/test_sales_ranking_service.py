...
def test_config(db_session: Session):
    ...
    config = SalesRankingConfig(
        config_name="测试配置", metrics=SalesRankingService.DEFAULT_METRICS, is_active=True
    )
...
class TestSalesRankingService:
    ...
    def test_save_config_new(self, sales_ranking_service, db_session):
        ...
        result = sales_ranking_service.save_config(config_name="新配置", metrics=metrics)
        ...
        # 验证配置已创建
        config = db_session.query(SalesRankingConfig).filter_by(id=result["config_id"]).first()
    ...
    def test_save_config_update(self, sales_ranking_service, test_config):
        ...
        result = sales_ranking_service.save_config(
            config_id=test_config.id, config_name="更新配置", metrics=updated_metrics
        )
    ...
    def test_save_config_invalid_weights(self, sales_ranking_service):
        ...
        result = sales_ranking_service.save_config(config_name="无效配置", metrics=metrics)
    ...
    def test_save_config_primary_weight_invalid(self, sales_ranking_service):
        ...
        result = sales_ranking_service.save_config(config_name="主要权重不足", metrics=metrics)

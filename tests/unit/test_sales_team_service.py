...
class TestSalesTeamService:
    ...
    def test_calculate_target_performance_lead_count(self, sales_team_service, test_sales_user):
        target = SalesTarget(
        ...
        actual_value, completion_rate = sales_team_service.calculate_target_performance(target)
    ...
    def test_build_personal_target_map_success(self, sales_team_service, test_sales_user):
        target = SalesTarget(
            target_scope="PERSONAL",
            user_id=test_sales_user.id,
            target_type="CONTRACT_AMOUNT",
        ...
        result = sales_team_service.build_personal_target_map([test_sales_user.id], "2024-01", None)
    ...
    def test_get_followup_statistics_map_success(self, sales_team_service, test_sales_user):
        """测试获取跟进统计"""
        result = sales_team_service.get_followup_statistics_map([test_sales_user.id], None, None)
    ...
    def test_get_recent_followups_map_with_date_range(self, sales_team_service, test_sales_user):
        ...
        result = sales_team_service.get_recent_followups_map(
            [test_sales_user.id], start_datetime=start, end_datetime=end
        )
        assert isinstance(result, dict)
    ...
    def test_get_customer_distribution_map_success(self, sales_team_service, test_sales_user):
        """测试获取客户分布"""
        from datetime import date

        result = sales_team_service.get_customer_distribution_map(
            [test_sales_user.id], date(2024, 1, 1), date(2024, 12, 31)
        )
        assert isinstance(result, dict)
    ...
    def test_get_lead_quality_stats_map_success(self, sales_team_service, test_sales_user):
        """测试获取线索质量统计"""
        from datetime import datetime

        result = sales_team_service.get_lead_quality_stats_map([test_sales_user.id], None, None)
        assert isinstance(result, dict)
    ...
    def test_get_opportunity_stats_map_success(self, sales_team_service, test_sales_user):
        """测试获取商机统计"""
        from datetime import datetime

        result = sales_team_service.get_opportunity_stats_map([test_sales_user.id], None, None)
        assert isinstance(result, dict)
    ...
    def test_get_followup_statistics_map_with_date_range(self, sales_team_service, test_sales_user):
        """测试带日期范围的跟进统计"""
        from datetime import datetime, timedelta

        start = datetime.now() - timedelta(days=30)
        end = datetime.now()
        result = sales_team_service.get_followup_statistics_map([test_sales_user.id], start, end)
        assert isinstance(result, dict)

    def test_get_lead_quality_stats_map_with_date_range(self, sales_team_service, test_sales_user):
        """测试带日期范围的线索质量统计"""
        from datetime import datetime, timedelta

        start = datetime.now() - timedelta(days=30)
        end = datetime.now()
        result = sales_team_service.get_lead_quality_stats_map([test_sales_user.id], start, end)
        assert isinstance(result, dict)

    def test_get_opportunity_stats_map_with_date_range(self, sales_team_service, test_sales_user):
        """测试带日期范围的商机统计"""
        from datetime import datetime, timedelta

        start = datetime.now() - timedelta(days=30)
        end = datetime.now()
        result = sales_team_service.get_opportunity_stats_map([test_sales_user.id], start, end)
        assert isinstance(result, dict)

    def test_build_personal_target_map_yearly(self, sales_team_service, test_sales_user):
        """测试年度目标映射"""
        target = SalesTarget(
            target_scope="PERSONAL",
            user_id=test_sales_user.id,
        ...
        result = sales_team_service.build_personal_target_map([test_sales_user.id], None, "2024")

        assert test_sales_user.id in result
        assert "yearly" in result[test_sales_user.id]

    def test_build_personal_target_map_collection_amount(self, sales_team_service, test_sales_user):
        """测试回款金额目标映射"""
        target = SalesTarget(
            target_scope="PERSONAL",
            user_id=test_sales_user.id,
        ...
        result = sales_team_service.build_personal_target_map([test_sales_user.id], "2024-03", None)

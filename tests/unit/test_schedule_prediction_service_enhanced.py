...
class TestSchedulePredictionService(unittest.TestCase):
    ...
    def test_predict_completion_date_with_ai_success(self):
        ...
        with (
            ...
            patch(
                ...
                return_value=prediction_record,
            ),
        ):
    ...
    def test_extract_features_basic(self):
        ...
        self.service._get_similar_projects_stats = MagicMock(
            return_value={"avg_duration": 90, "delay_rate": 0.3}
        )
    ...
    def test_extract_features_with_project_data(self):
        ...
        project_data = {"days_elapsed": 50, "complexity": "high"}
    ...
    def test_predict_with_ai_fallback_to_linear(self):
        """测试AI失败时降级到线性预测"""
        features = {"current_progress": 50.0, "velocity_ratio": 0.9, "remaining_days": 30}
    ...
    def test_predict_with_ai_invalid_json_response(self):
        """测试AI返回无效JSON时的处理"""
        features = {"current_progress": 50.0, "velocity_ratio": 0.8, "remaining_days": 30}

        ai_response = {"content": "这不是有效的JSON格式"}
        self.service.ai_client.generate_solution.return_value = ai_response
    ...
    def test_predict_linear_on_schedule(self):
        """测试线性预测 - 按计划进行"""
        features = {"velocity_ratio": 1.0, "remaining_days": 30}
    ...
    def test_predict_linear_ahead_schedule(self):
        """测试线性预测 - 进度超前"""
        features = {"velocity_ratio": 1.5, "remaining_days": 20}
    ...
    def test_predict_linear_behind_schedule(self):
        """测试线性预测 - 进度滞后"""
        features = {"velocity_ratio": 0.5, "remaining_days": 30}
    ...
    def test_predict_linear_calculation_accuracy(self):
        """测试线性预测计算准确性"""
        features = {"velocity_ratio": 0.8, "remaining_days": 40}
    ...
    def test_generate_catch_up_solutions_success(self):
        ...
        with patch(
            "app.services.schedule_prediction_service.CatchUpSolution", return_value=solution_mock
        ):
            solutions = self.service.generate_catch_up_solutions(
                project_id=1, prediction_id=100, delay_days=10
            )
    ...
    def test_generate_catch_up_solutions_with_project_data(self):
        ...
        with patch(
            ...
            solutions = self.service.generate_catch_up_solutions(
                project_id=2, prediction_id=101, delay_days=15, project_data=project_data
            )
    ...
    def test_generate_catch_up_solutions_ai_fallback(self):
        ...
        with patch(
            ...
            solutions = self.service.generate_catch_up_solutions(
                project_id=3, prediction_id=102, delay_days=10
            )
    ...
    def test_create_alert_basic(self):
        ...
        with (
            patch("app.services.schedule_prediction_service.save_obj"),
            patch(
                "app.services.schedule_prediction_service.ScheduleAlert", return_value=alert_mock
            ),
        ):
    ...
    def test_check_and_create_alerts_delay_warning(self):
        ...
        with patch.object(self.service, "create_alert", return_value=alert_mock) as mock_create:
            alerts = self.service.check_and_create_alerts(
                project_id=1, prediction_id=100, delay_days=10, progress_deviation=-5.0
            )
        ...
        self.assertTrue(
            any(
                call_args[1]["alert_type"] == "delay_warning"
    ...
    def test_check_and_create_alerts_progress_deviation(self):
        ...
        with patch.object(self.service, "create_alert", return_value=alert_mock) as mock_create:
            alerts = self.service.check_and_create_alerts(
                project_id=2, prediction_id=101, delay_days=1, progress_deviation=-15.0
            )

        # Should create velocity drop alert
        self.assertTrue(
            any(
                call_args[1]["alert_type"] == "velocity_drop"
                for call_args in mock_create.call_args_list
            )
        )
    ...
    def test_check_and_create_alerts_multiple_alerts(self):
        ...
        with patch.object(self.service, "create_alert", return_value=alert_mock) as mock_create:
            alerts = self.service.check_and_create_alerts(
                project_id=3, prediction_id=102, delay_days=15, progress_deviation=-20.0
            )
    ...
    def test_check_and_create_alerts_no_alerts(self):
        ...
        with patch.object(self.service, "create_alert") as mock_create:
            alerts = self.service.check_and_create_alerts(
                project_id=4, prediction_id=103, delay_days=1, progress_deviation=2.0
            )
    ...
    def test_get_project_alerts_with_severity_filter(self):
        """测试按严重程度过滤预警"""
        mock_alerts = []
        self.db_mock.query.return_value.filter.return_value.filter.return_value.order_by.return_value.limit.return_value.all.return_value = (
            mock_alerts
        )

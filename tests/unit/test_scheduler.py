...
class TestResolveCallable:
    ...
    def test_resolve_callable_function_not_found(self):
        ...
        task = {"module": "app.utils.scheduler", "callable": "nonexistent_function"}
...
class TestWrapJobCallable:
    ...
    def test_wrap_job_callable_success(self, mock_record_success, mock_logger):
        ...
        def test_func():
            ...
            task = {"id": "test_job", "name": "Test Job", "owner": "test", "category": "test"}
...
class TestInitScheduler:
    ...
    @patch("app.utils.scheduler.logger")
    def test_init_scheduler_with_tasks(
        self, mock_logger, mock_wrap, mock_resolve, mock_load_db, mock_scheduler
    ):
        """Test initializing scheduler with tasks"""
        from app.utils.scheduler import SCHEDULER_TASKS, init_scheduler

...
class TestSessionServiceCreate(unittest.TestCase):
    ...
    def test_create_session_basic(
        self, mock_parse_ua, mock_location, mock_risk, mock_cleanup, mock_cache
    ):
        """测试基本会话创建"""
        # 准备mock返回值
        mock_parse_ua.return_value = {"browser": "Chrome 120", "os": "macOS 14", "device": "Mac"}
    ...
    @patch("app.services.session_service.SessionService._cleanup_old_sessions")
    @patch("app.services.session_service.SessionService._assess_risk")
    def test_create_session_no_ip_ua(self, mock_risk, mock_cleanup, mock_cache):
        """测试无IP和UA的会话创建"""
        mock_risk.return_value = (False, 0)
...
class TestSessionServiceQuery(unittest.TestCase):
    ...
    def test_get_user_sessions_active_only(self):
        ...
        sessions = SessionService.get_user_sessions(
            db=self.mock_db, user_id=self.user_id, active_only=True
        )
    ...
    def test_get_user_sessions_with_current_jti(self):
        ...
        current_jti = "jti1"
        mock_sessions = [
            MagicMock(id=1, user_id=self.user_id, is_active=True, access_token_jti=current_jti),
            MagicMock(id=2, user_id=self.user_id, is_active=True, access_token_jti="jti2"),
        ]
        ...
        sessions = SessionService.get_user_sessions(
            db=self.mock_db, user_id=self.user_id, current_jti=current_jti
        )
    ...
    def test_get_session_by_jti_access(self):
        ...
        session = SessionService.get_session_by_jti(db=self.mock_db, jti=jti, token_type="access")
    ...
    def test_get_session_by_jti_refresh(self):
        ...
        session = SessionService.get_session_by_jti(db=self.mock_db, jti=jti, token_type="refresh")
    ...
    def test_get_session_by_jti_not_found(self):
        ...
        session = SessionService.get_session_by_jti(
            db=self.mock_db, jti="nonexistent", token_type="access"
        )
...
class TestSessionServiceUpdate(unittest.TestCase):
    ...
    def test_update_session_activity_basic(self, mock_get_session, mock_cache):
        """测试更新会话活动时间"""
        jti = "refresh_jti_123"
        mock_session = MagicMock(id=1, last_activity_at=datetime.utcnow() - timedelta(hours=1))
        mock_get_session.return_value = mock_session

        result = SessionService.update_session_activity(db=self.mock_db, jti=jti)
    ...
    @patch("app.services.session_service.SessionService._cache_session")
    @patch("app.services.session_service.SessionService.get_session_by_jti")
    def test_update_session_activity_with_new_access_jti(self, mock_get_session, mock_cache):
        """测试刷新token时更新JTI"""
        jti = "refresh_jti_123"
        ...
        result = SessionService.update_session_activity(
            db=self.mock_db, jti=jti, new_access_jti=new_access_jti
        )
    ...
    def test_update_session_activity_not_found(self, mock_get_session):
        ...
        result = SessionService.update_session_activity(db=self.mock_db, jti="nonexistent")
...
class TestSessionServiceRevoke(unittest.TestCase):
    ...
    def test_revoke_session_success(self, mock_blacklist, mock_remove_cache):
        ...
        result = SessionService.revoke_session(
            db=self.mock_db, session_id=session_id, user_id=self.user_id
        )
    ...
    def test_revoke_session_not_found(self, mock_blacklist, mock_remove_cache):
        ...
        result = SessionService.revoke_session(
            db=self.mock_db, session_id=999, user_id=self.user_id
        )
    ...
    def test_revoke_all_sessions_success(self, mock_blacklist, mock_remove_cache):
        """测试撤销所有会话"""
        mock_sessions = [
            MagicMock(id=1, access_token_jti="jti1", refresh_token_jti="rjti1", is_active=True),
            MagicMock(id=2, access_token_jti="jti2", refresh_token_jti="rjti2", is_active=True),
        ]
        ...
        count = SessionService.revoke_all_sessions(db=self.mock_db, user_id=self.user_id)
    ...
    @patch("app.services.session_service.SessionService._remove_session_cache")
    @patch("app.services.session_service.SessionService._add_to_blacklist")
    def test_revoke_all_sessions_except_current(self, mock_blacklist, mock_remove_cache):
        """测试撤销除当前外的所有会话"""
        ...
        count = SessionService.revoke_all_sessions(
            db=self.mock_db, user_id=self.user_id, except_jti=current_jti
        )
...
class TestSessionServiceCleanup(unittest.TestCase):
    ...
    def test_cleanup_expired_sessions(self, mock_remove_cache):
        ...
        now = datetime.utcnow()
        expired_sessions = [
            MagicMock(id=1, is_active=True, expires_at=now - timedelta(days=1)),
            MagicMock(id=2, is_active=True, expires_at=now - timedelta(hours=1)),
        ]
...
class TestSessionServiceHelpers(unittest.TestCase):
    ...
    def test_assess_risk_new_ip(self):
        ...
        mock_query = mock_db.query.return_value
        mock_query.filter.return_value = mock_query
        mock_query.order_by.return_value.limit.return_value.all.return_value = [old_session]
...
class TestSessionServiceEdgeCases(unittest.TestCase):
    ...
    def test_assess_risk_none_values(self):
        ...
        is_suspicious, risk_score = SessionService._assess_risk(
            db=mock_db, user_id=1, ip_address=None, device_info=None, location=None
        )

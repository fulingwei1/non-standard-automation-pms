...
class TestSessionServiceCreateSession(unittest.TestCase):
    ...
    def test_create_session_success(
        self, mock_cache, mock_cleanup, mock_assess, mock_location, mock_parse_ua, mock_datetime
    ):
        """测试成功创建会话"""
        # 设置mock
    ...
    def test_create_session_without_optional_params(
        ...
        session = SessionService.create_session(
            self.mock_db, self.user_id, self.access_jti, self.refresh_jti
        )
    ...
    def test_create_session_suspicious_login(
        ...
        session = SessionService.create_session(
            self.mock_db, self.user_id, self.access_jti, self.refresh_jti, self.ip, self.user_agent
        )
...
class TestSessionServiceGetSessions(unittest.TestCase):
    ...
    def test_get_user_sessions_active_only(self):
        ...
        sessions = SessionService.get_user_sessions(self.mock_db, self.user_id, active_only=True)
    ...
    def test_get_user_sessions_all(self):
        ...
        sessions = SessionService.get_user_sessions(self.mock_db, self.user_id, active_only=False)
    ...
    def test_get_session_by_jti_access(self):
        ...
        session = SessionService.get_session_by_jti(self.mock_db, "jti123", token_type="access")
...
class TestSessionServiceUpdateActivity(unittest.TestCase):
    ...
    @patch.object(SessionService, "_cache_session")
    def test_update_activity_with_refresh_jti(self, mock_cache, mock_get_session, mock_datetime):
        """测试使用refresh JTI更新活动"""
        ...
        result = SessionService.update_session_activity(self.mock_db, "refresh_jti")
    ...
    @patch.object(SessionService, "get_session_by_jti")
    @patch.object(SessionService, "_cache_session")
    def test_update_activity_with_new_access_jti(self, mock_cache, mock_get_session, mock_datetime):
    ...
    def test_update_activity_fallback_to_access(self, mock_get_session):
        ...
        result = SessionService.update_session_activity(self.mock_db, "jti")
    ...
    def test_update_activity_session_not_found(self, mock_get_session):
        ...
        result = SessionService.update_session_activity(self.mock_db, "nonexistent_jti")
...
class TestSessionServiceRevokeSession(unittest.TestCase):
    ...
    def test_revoke_session_success(self, mock_remove_cache, mock_blacklist, mock_datetime):
        """测试成功撤销会话"""
    ...
    def test_revoke_all_sessions_except_current(
        ...
        session1 = MagicMock(
            id=1, access_token_jti="current_jti", refresh_token_jti="current_refresh"
        )
        session2 = MagicMock(id=2, access_token_jti="other_jti", refresh_token_jti="other_refresh")
        ...
        count = SessionService.revoke_all_sessions(self.mock_db, 1, except_jti="current_jti")
    ...
    def test_revoke_all_sessions_no_exception(
        ...
        sessions = [
            MagicMock(id=i, access_token_jti=f"jti{i}", refresh_token_jti=f"refresh{i}")
            for i in range(3)
        ]
...
class TestSessionServiceCleanupOldSessions(unittest.TestCase):
    ...
    @patch("app.services.session_service.datetime")
    @patch.object(SessionService, "_add_to_blacklist")
    def test_cleanup_old_sessions_under_limit(self, mock_blacklist, mock_datetime):
        """测试会话数未超限时不清理"""
        now = datetime(2024, 1, 1, 12, 0, 0)
        mock_datetime.utcnow.return_value = now
    ...
    @patch("app.services.session_service.datetime")
    @patch.object(SessionService, "_add_to_blacklist")
    def test_cleanup_old_sessions_over_limit(self, mock_blacklist, mock_datetime):
        """测试会话数超限时清理最旧的会话"""
        now = datetime(2024, 1, 1, 12, 0, 0)
        mock_datetime.utcnow.return_value = now
        ...
        # 创建6个会话（超过限制5个）
        sessions = [
            MagicMock(id=i, access_token_jti=f"access_{i}", refresh_token_jti=f"refresh_{i}")
            for i in range(6)
        ]
...
class TestSessionServiceCacheMethods(unittest.TestCase):
    ...
    def test_cache_session_redis_exception(self, mock_redis_client):
        ...
        session = MagicMock(
            id=123, user_id=1, is_active=True, access_token_jti="jti", refresh_token_jti="refresh"
        )
    ...
    def test_add_to_blacklist_success(self, mock_redis_client):
        ...
        mock_redis.setex.assert_called_once_with("jwt:blacklist:jti123", 3600, "1")

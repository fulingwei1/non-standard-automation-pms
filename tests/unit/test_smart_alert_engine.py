...
class TestCalculateRiskScore(unittest.TestCase):
    ...
    def test_low_risk_score(self):
        """测试低风险评分"""
        score = self.engine._calculate_risk_score(
            delay_days=5, cost_impact=Decimal("5000"), project_count=1, shortage_qty=Decimal("5")
        )
...
class TestPredictImpact(unittest.TestCase):
    ...
    def test_predict_impact_basic(self, mock_lead_time, mock_projects):
        """测试基础影响预测"""
        # Mock相关方法
        mock_projects.return_value = [{"id": 100, "name": "项目A", "required_qty": 200.0}]
        ...
        self.assertEqual(
            impact["estimated_cost_impact"], Decimal("50") * Decimal("100") * Decimal("1.5")
        )
...
class TestGenerateSolutionPlans(unittest.TestCase):
    ...
    def test_generate_urgent_purchase_plan(self, mock_plan_no):
        ...
        alert = ShortageAlert(id=1, shortage_qty=Decimal("100"))
    ...
    def test_generate_reschedule_plan(self, mock_plan_no):
        ...
        alert = ShortageAlert(
            id=1, required_date=datetime.now().date() + timedelta(days=10), estimated_delay_days=5
        )
...
class TestScanAndAlert(unittest.TestCase):
    ...
    def test_scan_and_alert_with_shortage(
        ...
        # 执行测试
        alerts = self.engine.scan_and_alert(project_id=None, material_id=None, days_ahead=30)
    ...
    def test_scan_and_alert_no_shortage(
        self, mock_demands, mock_available, mock_in_transit, mock_create_alert, mock_solutions
    ):
        """测试扫描无缺料"""
        mock_demands.return_value = [
            {

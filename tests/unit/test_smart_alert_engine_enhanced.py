...
class TestSmartAlertEngine(unittest.TestCase):
    ...
    def test_calculate_risk_score_medium_risk(self):
        """测试：中等风险应返回中等分数"""
        score = self.engine._calculate_risk_score(
            delay_days=10, cost_impact=Decimal("30000"), project_count=2, shortage_qty=Decimal("50")
        )
    ...
    def test_calculate_risk_score_low_risk(self):
        """测试：低风险应返回低分数"""
        score = self.engine._calculate_risk_score(
            delay_days=0, cost_impact=Decimal("5000"), project_count=1, shortage_qty=Decimal("5")
        )
    ...
    def test_generate_urgent_purchase_plan(self, mock_plan_no):
        """测试：生成紧急采购方案"""
        alert = ShortageAlert(
            id=1, shortage_qty=Decimal("100"), estimated_cost_impact=Decimal("10000")
        )
    ...
    def test_generate_partial_delivery_plan_with_stock(self, mock_plan_no):
        """测试：有库存时生成分批交付方案"""
        alert = ShortageAlert(id=1, available_qty=Decimal("50"), shortage_qty=Decimal("100"))
    ...
    def test_generate_partial_delivery_plan_no_stock(self):
        """测试：无库存时不应生成分批交付方案"""
        alert = ShortageAlert(id=1, available_qty=Decimal("0"), shortage_qty=Decimal("100"))
    ...
    def test_generate_reschedule_plan(self, mock_plan_no):
        """测试：生成重排期方案"""
        alert = ShortageAlert(id=1, required_date=date.today(), estimated_delay_days=7)
    ...
    def test_collect_material_demands_with_filters(self):
        ...
        demands = self.engine._collect_material_demands(
            project_id=10, material_id=100, days_ahead=30
        )
    ...
    def test_collect_material_demands_no_results(self):
        ...
        demands = self.engine._collect_material_demands(
            project_id=None, material_id=None, days_ahead=30
        )
    ...
    def test_find_affected_projects(self):
        """测试：查找受影响项目"""
        mock_result = [
            MagicMock(id=1, name="项目A", required_qty=Decimal("500")),
            MagicMock(id=2, name="项目B", required_qty=Decimal("300")),
        ]
        ...
        projects = self.engine._find_affected_projects(material_id=100, project_id=None)
    ...
    def test_predict_impact(self, mock_risk_score, mock_lead_time, mock_projects):
        """测试：预测缺料影响"""
        mock_projects.return_value = [{"id": 1, "name": "项目A", "required_qty": 100.0}]
    ...
    def test_generate_solutions_multiple_plans(
        self, mock_score, mock_reschedule, mock_partial, mock_transfer, mock_substitute, mock_urgent
    ):
        """测试：生成多个处理方案并排序"""

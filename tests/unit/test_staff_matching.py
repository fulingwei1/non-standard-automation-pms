...
class TestMatchingEngine(unittest.TestCase):
    ...
    def _create_mock_staffing_need(self, need_id=1, project_id=100, priority="P2"):
        ...
        need.preferred_skills = [{"tag_id": 3, "tag_name": "PostgreSQL", "min_score": 3}]
        need.required_domains = [{"tag_id": 10, "tag_name": "电商领域", "min_score": 3}]
        need.required_attitudes = [{"tag_id": 20, "tag_name": "责任心", "min_score": 4}]
        need.status = "PENDING"
        return need
    ...
    def test_match_candidates_success(self):
        ...
        # 执行匹配
        result = MatchingEngine.match_candidates(db=self.db, staffing_need_id=1, top_n=5)
    ...
    def test_match_candidates_staffing_need_not_found(self):
        ...
        with self.assertRaises(ValueError) as context:
            MatchingEngine.match_candidates(db=self.db, staffing_need_id=999)
    ...
    def test_match_candidates_no_candidates(self):
        ...
        self.db.query.side_effect = [need_query, project_query, candidate_query]

        result = MatchingEngine.match_candidates(db=self.db, staffing_need_id=1)
    ...
    def test_match_candidates_with_top_n(self):
        ...
        result = MatchingEngine.match_candidates(
            db=self.db, staffing_need_id=1, top_n=2  # 只返回前2名
        )
    ...
    def test_get_candidate_employees_exclude_overloaded(self):
        ...
        result = MatchingEngine._get_candidate_employees(
            db=self.db, staffing_need=staffing_need, include_overloaded=False
        )
    ...
    def test_get_candidate_employees_include_overloaded(self):
        ...
        result = MatchingEngine._get_candidate_employees(
            db=self.db, staffing_need=staffing_need, include_overloaded=True
        )
    ...
    def test_calculate_candidate_scores_all_dimensions(self):
        ...
        scores = MatchingEngine._calculate_candidate_scores(
            db=self.db, employee=employee, profile=profile, staffing_need=staffing_need
        )
    ...
    def test_calculate_candidate_scores_no_profile(self):
        ...
        scores = MatchingEngine._calculate_candidate_scores(
            db=self.db, employee=employee, profile=None, staffing_need=staffing_need  # 无档案
        )
...
class TestScoreCalculatorIntegration(unittest.TestCase):
    ...
    def test_skill_score_with_matched_skills(self):
        ...
        profile = Mock()
        required_skills = [{"tag_id": 1, "tag_name": "Python", "min_score": 4}]
        preferred_skills = []
    ...
    def test_skill_score_no_requirements(self):
        ...
        result = SkillScoreCalculator.calculate_skill_score(
            db=self.db, employee_id=1, profile=None, required_skills=[], preferred_skills=[]
        )
    ...
    def test_workload_score_fully_available(self):
        ...
        score = WorkloadScoreCalculator.calculate_workload_score(
            profile=profile, required_allocation=50.0
        )
    ...
    def test_quality_score_no_performance(self):
        ...
        score = QualityScoreCalculator.calculate_quality_score(db=self.db, employee_id=1)
    ...
    def test_domain_score_no_requirements(self):
        ...
        score = DomainScoreCalculator.calculate_domain_score(
            db=self.db, employee_id=1, profile=None, required_domains=[]
        )
    ...
    def test_attitude_score_from_profile(self):
        ...
        score = AttitudeScoreCalculator.calculate_attitude_score(
            db=self.db, employee_id=1, profile=profile, required_attitudes=[]
        )
    ...
    def test_special_score_no_abilities(self):
        ...
        score = SpecialScoreCalculator.calculate_special_score(
            db=self.db, employee_id=1, profile=None
        )

...
def transition_project_stage(
    project, target_stage: str, allow_backward: bool = False, reason: Optional[str] = None
):
    """
    项目阶段状态机核心逻辑：执行阶段转换
...
def transition_approval(approval, target_status: str):
    ...
    if current in APPROVAL_TERMINAL_STATES:
        raise ValueError(f"Cannot transition from terminal state '{current}' to '{target_status}'")

    # 检查转换是否被允许
    allowed = APPROVAL_TRANSITIONS.get(current, [])
    if target_status not in allowed:
        raise ValueError(
            f"Invalid approval transition: '{current}' → '{target_status}'. " f"Allowed: {allowed}"
        )
...
def transition_contract(contract, target_status: str, milestones_all_paid: bool = True):
    ...
    if target_status not in allowed:
        raise ValueError(
            f"Invalid contract transition: '{current}' → '{target_status}'. " f"Allowed: {allowed}"
        )

    # COMPLETED 需要所有里程碑付款已收
    if target_status == "COMPLETED" and not milestones_all_paid:
        raise ValueError("Cannot complete contract: not all milestone payments have been received")
...
class TestProjectStageMachine:
    ...
    def test_backward_without_reason_raises(self):
        ...
        with pytest.raises(ValueError, match="Reason is required for backward transition"):
            transition_project_stage(project, target_stage="S5", allow_backward=True, reason=None)

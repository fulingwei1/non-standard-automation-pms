...
def test_project(db_session: Session):
    """创建测试项目"""
    project = Project(
        project_code="PJ001", project_name="测试项目", stage="S2", status="ST05", health="H1"
    )
...
class TestStatusTransitionService:
    ...
    def test_handle_contract_signed_not_found(self, status_transition_service):
        """测试处理合同签订 - 合同不存在"""
        with patch.object(
            status_transition_service.contract_handler, "handle_contract_signed"
        ) as mock_handler:
    ...
    def test_handle_contract_signed_without_auto_create(
        self, status_transition_service, test_project
    ):
        """测试处理合同签订 - 不自动创建项目"""
        with patch.object(
            status_transition_service.contract_handler, "handle_contract_signed"
    ...
    def test_handle_bom_published_success(self, status_transition_service, test_project):
        """测试处理BOM发布 - 成功场景"""
        with patch.object(
            status_transition_service.material_handler, "handle_bom_published"
        ) as mock_handler:
    ...
    def test_handle_material_shortage_success(self, status_transition_service, test_project):
        """测试处理物料缺货 - 成功场景"""
        with patch.object(
            status_transition_service.material_handler, "handle_material_shortage"
        ) as mock_handler:
            mock_handler.return_value = True
            result = status_transition_service.handle_material_shortage(
                test_project.id, is_critical=True
            )
            assert result is True
            mock_handler.assert_called_once_with(
                project_id=test_project.id,
    ...
    def test_handle_material_shortage_non_critical(self, status_transition_service, test_project):
        ...
        with patch.object(
            ...
            mock_handler.return_value = True
            result = status_transition_service.handle_material_shortage(
                test_project.id, is_critical=False
            )
            assert result is True
            mock_handler.assert_called_once_with(
                project_id=test_project.id,
    ...
    def test_handle_material_ready_success(self, status_transition_service, test_project):
        """测试处理物料齐套 - 成功场景"""
        with patch.object(
            status_transition_service.material_handler, "handle_material_ready"
        ) as mock_handler:
            mock_handler.return_value = True
    ...
    def test_handle_fat_passed_success(self, status_transition_service, test_project):
        """测试处理FAT通过 - 成功场景"""
        with patch.object(
            status_transition_service.acceptance_handler, "handle_fat_passed"
        ) as mock_handler:
            mock_handler.return_value = True
    ...
    def test_handle_fat_failed_success(self, status_transition_service, test_project):
        """测试处理FAT失败 - 成功场景"""
        with patch.object(
            status_transition_service.acceptance_handler, "handle_fat_failed"
        ) as mock_handler:
            mock_handler.return_value = True
    ...
    def test_handle_fat_failed_with_issues(self, status_transition_service, test_project):
        ...
        with patch.object(
            ...
            mock_handler.return_value = True
            result = status_transition_service.handle_fat_failed(
                test_project.id, machine_id=1, issues=issues
            )
            assert result is True
            mock_handler.assert_called_once_with(
                project_id=test_project.id,
    ...
    def test_handle_sat_passed_success(self, status_transition_service, test_project):
        """测试处理SAT通过 - 成功场景"""
        with patch.object(
            status_transition_service.acceptance_handler, "handle_sat_passed"
        ) as mock_handler:
            mock_handler.return_value = True
    ...
    def test_handle_sat_failed_success(self, status_transition_service, test_project):
        """测试处理SAT失败 - 成功场景"""
        with patch.object(
            status_transition_service.acceptance_handler, "handle_sat_failed"
        ) as mock_handler:
            mock_handler.return_value = True
    ...
    def test_handle_final_acceptance_passed_success(self, status_transition_service, test_project):
        """测试处理终验收通过 - 成功场景"""
        with patch.object(
            status_transition_service.acceptance_handler, "handle_final_acceptance_passed"
        ) as mock_handler:
            mock_handler.return_value = True
    ...
    def test_handle_ecn_schedule_impact_success(self, status_transition_service, test_project):
        """测试处理ECN影响交期 - 成功场景"""
        with patch.object(
            status_transition_service.ecn_handler, "handle_ecn_schedule_impact"
        ) as mock_handler:
            mock_handler.return_value = True
            result = status_transition_service.handle_ecn_schedule_impact(
                project_id=test_project.id, ecn_id=1, impact_days=5
            )
            assert result is True
            mock_handler.assert_called_once_with(
                project_id=test_project.id,
    ...
    def test_check_auto_stage_transition_s3(self, status_transition_service, db_session):
        """测试检查阶段自动流转 - S3阶段"""
        project = Project(project_code="PJ002", project_name="测试项目2", stage="S3", status="ST08")
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
        ...
        with patch("app.services.stage_transition_checks.check_s3_to_s4_transition") as mock_check:
            ...
            result = status_transition_service.check_auto_stage_transition(
                project.id, auto_advance=False
            )
    ...
    def test_check_auto_stage_transition_s4(self, status_transition_service, db_session):
        """测试检查阶段自动流转 - S4阶段"""
        project = Project(project_code="PJ003", project_name="测试项目3", stage="S4", status="ST10")
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
    ...
    def test_check_auto_stage_transition_s5(self, status_transition_service, db_session):
        """测试检查阶段自动流转 - S5阶段"""
        project = Project(project_code="PJ004", project_name="测试项目4", stage="S5", status="ST12")
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
    ...
    def test_check_auto_stage_transition_s7(self, status_transition_service, db_session):
        """测试检查阶段自动流转 - S7阶段"""
        project = Project(project_code="PJ005", project_name="测试项目5", stage="S7", status="ST20")
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
    ...
    def test_check_auto_stage_transition_s8(self, status_transition_service, db_session):
        """测试检查阶段自动流转 - S8阶段"""
        project = Project(project_code="PJ006", project_name="测试项目6", stage="S8", status="ST25")
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
    ...
    def test_check_auto_stage_transition_with_auto_advance(
        self, status_transition_service, db_session
    ):
        """测试检查阶段自动流转 - 自动推进"""
        project = Project(project_code="PJ007", project_name="测试项目7", stage="S3", status="ST08")
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)

        with (
            patch("app.services.stage_transition_checks.check_s3_to_s4_transition") as mock_check,
    ...
    def test_check_auto_stage_transition_no_transition(self, status_transition_service, db_session):
        """测试检查阶段自动流转 - 不满足流转条件"""
        project = Project(project_code="PJ008", project_name="测试项目8", stage="S1", status="ST01")
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
    ...
    def test_log_status_change(self, status_transition_service, test_project):
        ...
        # 验证状态日志已创建
        logs = (
            status_transition_service.db.query(ProjectStatusLog)
            .filter(ProjectStatusLog.project_id == test_project.id)
            .all()
        )
    ...
    def test_log_status_change_minimal(self, status_transition_service, test_project):
        """测试记录状态变更 - 最小参数"""
        status_transition_service._log_status_change(project_id=test_project.id)
        status_transition_service.db.commit()  # 提交事务以保存日志

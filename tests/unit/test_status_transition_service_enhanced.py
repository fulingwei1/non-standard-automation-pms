...
class TestStatusTransitionService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher_contract = patch(
            "app.services.status_handlers.contract_handler.ContractStatusHandler"
        )
        self.patcher_material = patch(
            "app.services.status_handlers.material_handler.MaterialStatusHandler"
        )
        self.patcher_acceptance = patch(
            "app.services.status_handlers.acceptance_handler.AcceptanceStatusHandler"
        )
...
class TestBOMPublishedHandler(TestStatusTransitionService):
    ...
    def test_handle_bom_published_success(self):
        ...
        self.mock_material_handler.handle_bom_published.assert_called_once_with(
            project_id=1, machine_id=10, log_status_change=self.service._log_status_change
        )
    ...
    def test_handle_bom_published_without_machine(self):
        ...
        self.assertTrue(result)
        self.mock_material_handler.handle_bom_published.assert_called_once_with(
            project_id=1, machine_id=None, log_status_change=self.service._log_status_change
        )
...
class TestMaterialShortageHandler(TestStatusTransitionService):
    ...
    def test_handle_material_shortage_critical(self):
        ...
        self.mock_material_handler.handle_material_shortage.assert_called_once_with(
            project_id=1, is_critical=True, log_status_change=self.service._log_status_change
        )
    ...
    def test_handle_material_shortage_non_critical(self):
        ...
        self.assertTrue(result)
        self.mock_material_handler.handle_material_shortage.assert_called_once_with(
            project_id=1, is_critical=False, log_status_change=self.service._log_status_change
        )
...
class TestMaterialReadyHandler(TestStatusTransitionService):
    ...
    def test_handle_material_ready_success(self):
        ...
        self.mock_material_handler.handle_material_ready.assert_called_once_with(
            project_id=1, log_status_change=self.service._log_status_change
        )
...
class TestECNHandler(TestStatusTransitionService):
    ...
    def test_handle_ecn_schedule_impact_positive_days(self):
        ...
        result = self.service.handle_ecn_schedule_impact(project_id=1, ecn_id=100, impact_days=10)
    ...
    def test_handle_ecn_schedule_impact_negative_days(self):
        ...
        result = self.service.handle_ecn_schedule_impact(project_id=1, ecn_id=100, impact_days=-5)
    ...
    def test_handle_ecn_schedule_impact_zero_days(self):
        ...
        result = self.service.handle_ecn_schedule_impact(project_id=1, ecn_id=100, impact_days=0)
...
class TestAutoStageTransition(TestStatusTransitionService):
    ...
    @patch("app.services.status_transition_service.Project")
    def test_check_auto_stage_transition_s4_to_s5_with_missing(
        self, mock_project_model, mock_check
    ):
        """测试S4→S5阶段流转（有缺失项）"""
        mock_project = MagicMock()
        mock_project.id = 1
    ...
    @patch("app.services.status_transition_service.Project")
    def test_check_auto_stage_transition_s5_to_s6_auto_advance(
        self, mock_project_model, mock_check, mock_execute
    ):
        """测试S5→S6阶段流转（自动推进）"""
        mock_project = MagicMock()
        mock_project.id = 1
        ...
        mock_execute.return_value = (
            True,
            {"success": True, "current_stage": "S5", "new_stage": "S6"},
        )
...
class TestLogStatusChange(TestStatusTransitionService):
    ...
    def test_log_status_change_minimal_params(self, mock_datetime, mock_log_class):
        ...
        self.service._log_status_change(project_id=1, change_type="AUTO_TRANSITION")
...
class TestEdgeCases(TestStatusTransitionService):
    ...
    def test_handle_ecn_schedule_impact_large_impact(self):
        ...
        result = self.service.handle_ecn_schedule_impact(
            project_id=1, ecn_id=100, impact_days=365  # 延期一年
        )

...
class TestSubmitCore(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_template = ApprovalTemplate(
            id=1, template_code="QUOTE_APPROVAL", template_name="报价审批", is_active=True
        )

        self.mock_user = User(
            id=100, username="zhangsan", real_name="张三", password_hash="test_hash_123"
        )

        self.mock_flow = ApprovalFlowDefinition(id=10, flow_name="默认流程", template_id=1)
    ...
    def test_submit_with_adapter_validation(self):
        ...
        with patch(
            "app.services.approval_engine.engine.submit.get_adapter", return_value=mock_adapter
        ):
            result = self.submitter.submit(
                template_code="QUOTE_APPROVAL",
                entity_type="QUOTE",
    ...
    def test_submit_user_without_real_name(self):
        ...
        # 创建无真实姓名的用户
        user_no_name = User(
            id=100, username="testuser", real_name=None, password_hash="test_hash_123"
        )
    ...
    def test_submit_context_building(self):
        ...
        mock_adapter = MagicMock()
        mock_adapter.validate_submit.return_value = (True, None)
        mock_adapter.get_entity_data.return_value = {"quote_no": "Q2024001", "amount": 5000}

...
# Skip this module if dependencies don't exist
...
class TestGenerateHrReportExcel:
    ...
    def test_generate_hr_report_empty_data(
        ...
        with patch("app.services.timesheet_report_service.HourlyRateService"):
            ...
            # 只有表头，没有数据行
            assert ws.max_row == 4  # 标��行(1-2) + 空行(3) + 表头行(4)
...
class TestGenerateProjectReportExcel:
    ...
    def test_generate_project_report_success(
        ...
        project_data = {
            "project_name": "测试项目A",
            "personnel_stats": [
                {"user_name": "张三", "total_hours": 160.0, "contribution_rate": 50.0}
            ],
            "daily_stats": [
                {
                    "date": "2024-01-15",
        ...
        result = timesheet_report_service.generate_project_report_excel(
            project_id=1, start_date=date(2024, 1, 1), end_date=date(2024, 1, 31)
        )
    ...
    def test_generate_project_report_error(
        ...
        # 生成报表
        result = timesheet_report_service.generate_project_report_excel(
            project_id=999, start_date=date(2024, 1, 1), end_date=date(2024, 1, 31)
        )
...
class TestGenerateProjectReportExcelAlternate:
    """测试生成项目报表（替代方法）"""

...
class TestTaskOwnerAssignment:
    ...
    def test_assign_owner_by_role_code(self, db_session: Session):
        ...
        # 创建用户
        user = User(employee_id=1, username="test_engineer", password_hash="test", is_active=True)
        db_session.add(user)
        db_session.flush()
        ...
        found_role = (
            ...
            .filter(Role.role_code == "ENGINEER", Role.is_active == True)
            .first()
        )
        ...
        found_user_role = (
            ...
            .filter(UserRole.role_id == found_role.id)
            .join(User)
    ...
    def test_no_matching_role(self, db_session: Session):
        ...
        found_role = (
            db_session.query(Role)
            .filter(Role.role_code == "NON_EXISTENT_ROLE", Role.is_active == True)
            .first()
        )
...
class TestEquipmentMaintenanceRecipients:
    ...
    def test_get_workshop_manager(self, db_session: Session):
        ...
        # 创建车间主管用户
        manager = User(
            employee_id=2, username="workshop_manager", password_hash="test", is_active=True
        )
        ...
        # 测试获取车间主管
        found_workshop = (
            db_session.query(Workshop).filter(Workshop.id == equipment.workshop_id).first()
        )
        ...
        found_manager = (
            ...
            .filter(User.id == found_workshop.manager_id, User.is_active == True)
            .first()
        )
    ...
    def test_equipment_without_workshop(self, db_session: Session):
        """测试设备没有关联车间的情况"""
        equipment = Equipment(
            equipment_code="EQ002", equipment_name="独立设备", workshop_id=None, is_active=True
        )
        db_session.add(equipment)
        db_session.commit()
...
class TestProjectEvaluationLevelThresholds:
    ...
    @pytest.mark.skip(
        reason="Service uses default thresholds, not custom config - business logic issue"
    )
    def test_get_custom_thresholds_from_config(self, db_session: Session):
        ...
        level_config = ProjectEvaluationDimension(
            ...
            scoring_rules={"thresholds": {"S": 95, "A": 85, "B": 75, "C": 65, "D": 0}},
            is_active=True,
        )
...
class TestFinancialMetricsCalculation:
    ...
    def test_calculate_total_cost(self, db_session: Session):
        ...
        for cost in costs:
            ...
            total_cost = (
                db_session.query(func.sum(ProjectCost.amount))
...
class TestHourlyRateService:
    ...
    def test_get_user_personal_rate(self, db_session: Session):
        ...
        # 创建用户
        user = User(employee_id=3, username="test_user_rate", password_hash="test", is_active=True)
        db_session.add(user)
        db_session.flush()

        # 创建用户个人时薪配置
        config = HourlyRateConfig(
            config_type="USER", user_id=user.id, hourly_rate=Decimal("150"), is_active=True
        )
    ...
    def test_get_role_based_rate(self, db_session: Session):
        ...
        # 创建角色
        role = Role(role_code="SENIOR_ENGINEER", role_name="高级工程师", is_active=True)
        db_session.add(role)
        db_session.flush()

        # 创建用户
        user = User(employee_id=4, username="test_senior_eng", password_hash="test", is_active=True)
        db_session.add(user)
        db_session.flush()
        ...
        # 创建角色时薪配置
        config = HourlyRateConfig(
            config_type="ROLE", role_id=role.id, hourly_rate=Decimal("120"), is_active=True
        )
        db_session.add(config)
        db_session.commit()
    ...
    def test_user_rate_priority_over_role(self, db_session: Session):
        ...
        # 创建角色
        role = Role(role_code="DEVELOPER", role_name="开发工程师", is_active=True)
        db_session.add(role)
        db_session.flush()

        # 创建用户
        user = User(employee_id=5, username="test_dev", password_hash="test", is_active=True)
        db_session.add(user)
        db_session.flush()
        ...
        role_config = HourlyRateConfig(
            config_type="ROLE", role_id=role.id, hourly_rate=Decimal("100"), is_active=True
        )
        db_session.add(role_config)

        # 创建用户个人时薪配置（180元）
        user_config = HourlyRateConfig(
            config_type="USER", user_id=user.id, hourly_rate=Decimal("180"), is_active=True
        )
    ...
    def test_default_rate_fallback(self, db_session: Session):
        ...
        # 创建用户（没有任何时薪配置）
        user = User(employee_id=6, username="test_no_config", password_hash="test", is_active=True)
        db_session.add(user)
        db_session.flush()

        # 创建默认时薪配置
        default_config = HourlyRateConfig(
            config_type="DEFAULT", hourly_rate=Decimal("80"), is_active=True
        )
    ...
    def test_hardcoded_default_fallback(self, db_session: Session):
        ...
        user = User(employee_id=7, username="test_hardcoded", password_hash="test", is_active=True)
        db_session.add(user)
        db_session.commit()
    ...
    def test_batch_get_users_hourly_rates(self, db_session: Session):
        ...
        config1 = HourlyRateConfig(
            config_type="USER", user_id=user1.id, hourly_rate=Decimal("200"), is_active=True
        )
        db_session.add(config1)
        db_session.commit()

        # 批量获取
        rates = HourlyRateService.get_users_hourly_rates(db_session, [user1.id, user2.id])
    ...
    def test_rate_effective_date(self, db_session: Session):
        ...
        # 创建用户
        user = User(employee_id=10, username="test_date_user", password_hash="test", is_active=True)
        db_session.add(user)
        db_session.flush()

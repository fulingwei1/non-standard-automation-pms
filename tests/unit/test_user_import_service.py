...
class TestUserImportServiceRowValidation(unittest.TestCase):
    ...
    def test_validate_row_missing_username(self):
        """测试缺少用户名"""
        row = pd.Series({"username": "", "real_name": "测试用户", "email": "test@example.com"})
        error = UserImportService.validate_row(
            row, 2, self.mock_db, self.existing_usernames, self.existing_emails
        )
    ...
    def test_validate_row_missing_real_name(self):
        """测试缺少真实姓名"""
        row = pd.Series({"username": "testuser", "real_name": None, "email": "test@example.com"})
        error = UserImportService.validate_row(
            row, 2, self.mock_db, self.existing_usernames, self.existing_emails
        )
        self.assertIsNotNone(error)
    ...
    def test_validate_row_missing_email(self):
        """测试缺少邮箱"""
        row = pd.Series({"username": "testuser", "real_name": "测试用户", "email": ""})
        error = UserImportService.validate_row(
            row, 2, self.mock_db, self.existing_usernames, self.existing_emails
        )
        self.assertIsNotNone(error)
    ...
    def test_validate_row_username_too_short(self):
        """测试用户名过短"""
        row = pd.Series(
            {"username": "ab", "real_name": "测试用户", "email": "test@example.com"}  # 只有2个字符
        )
        error = UserImportService.validate_row(
            row, 2, self.mock_db, self.existing_usernames, self.existing_emails
        )
    ...
    def test_validate_row_username_too_long(self):
        """测试用户名过长"""
        row = pd.Series(
            {"username": "a" * 51, "real_name": "测试用户", "email": "test@example.com"}  # 51个字符
        )
        error = UserImportService.validate_row(
            row, 2, self.mock_db, self.existing_usernames, self.existing_emails
        )
    ...
    def test_validate_row_duplicate_username_in_file(self):
        ...
        row = pd.Series(
            {"username": "testuser", "real_name": "测试用户", "email": "test@example.com"}
        )
        error = UserImportService.validate_row(
            row, 2, self.mock_db, self.existing_usernames, self.existing_emails
        )
    ...
    def test_validate_row_invalid_email_format(self):
        """测试邮箱格式不正确"""
        row = pd.Series({"username": "testuser", "real_name": "测试用户", "email": "invalid-email"})
...
class TestUserImportServiceUserCreation(unittest.TestCase):
    ...
    def test_create_user_from_row_basic(self, mock_hash):
        ...
        user = UserImportService.create_user_from_row(self.mock_db, row, operator_id=1)
    ...
    def test_create_user_from_row_is_active_true(self, mock_hash):
        ...
        test_cases = ["true", "True", "1", "是", "启用", True, 1]
...
class TestUserImportServiceBatchImport(unittest.TestCase):
    ...
    def test_import_users_success(self, mock_hash):
        ...
        result = UserImportService.import_users(self.mock_db, df, operator_id=1)
    ...
    def test_import_users_with_tenant(self, mock_hash):
        ...
        result = UserImportService.import_users(self.mock_db, df, operator_id=1, tenant_id=10)
...
class TestUserImportServiceFileReading(unittest.TestCase):
    ...
    def test_read_file_xlsx(self, mock_read_excel):
        """测试读取XLSX文件"""
        mock_df = pd.DataFrame({"Username": ["test1"], "Email": ["test1@example.com"]})
...
class TestUserImportServiceEdgeCases(unittest.TestCase):
    ...
    def test_validate_row_with_nan_values(self):
        ...
        error = UserImportService.validate_row(row, 2, self.mock_db, set(), set())
        self.assertIsNone(error)

    def test_normalize_columns_no_mapping_needed(self):
        """测试不需要映射的列名"""
        df = pd.DataFrame({"custom1": ["value1"], "custom2": ["value2"]})

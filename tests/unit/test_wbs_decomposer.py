...
class TestAIWbsDecomposer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 创建分解器实例
        self.decomposer = AIWbsDecomposer(db=self.mock_db, glm_service=self.mock_glm)

    # ========== __init__() 初始化测试 ==========

    def test_init_with_glm_service(self):
        """测试使用自定义GLM服务初始化"""
        decomposer = AIWbsDecomposer(db=self.mock_db, glm_service=self.mock_glm)
    ...
    def test_find_reference_tasks(self):
        ...
        result = self.decomposer._find_reference_tasks(
            task_name="需求分析", task_type="ANALYSIS", project_type="SOFTWARE"
        )
    ...
    def test_find_reference_tasks_with_limit(self):
        ...
        self.decomposer._find_reference_tasks(
            task_name="测试", task_type="TEST", project_type="SOFTWARE", limit=3
        )
    ...
    def test_create_suggestion_from_data(self):
        ...
        result = self.decomposer._create_suggestion_from_data(
            data=data, project=mock_project, parent_task=mock_parent, level=3, sequence=2
        )
    ...
    def test_create_suggestion_from_data_with_defaults(self):
        ...
        result = self.decomposer._create_suggestion_from_data(
            data=data, project=mock_project, parent_task=mock_parent, level=2, sequence=1
        )
    ...
    def test_decompose_project_basic(self):
        ...
        async def run_test():
            result = await self.decomposer.decompose_project(
                project_id=100, max_level=1  # 只生成1级避免复杂度
            )
    ...
    def test_decompose_project_with_template(self):
        ...
        async def run_test():
            result = await self.decomposer.decompose_project(
                project_id=200, template_id=10, max_level=1
            )
    ...
    def test_decompose_task_exceeds_max_level(self):
        ...
        async def run_test():
            result = await self.decomposer._decompose_task(
                parent_task=mock_parent, project=mock_project, level=4, max_level=3
            )
    ...
    def test_decompose_task_with_ai(self):
        ...
        async def run_test():
            result = await self.decomposer._decompose_task(
                parent_task=mock_parent, project=mock_project, level=2, max_level=2
            )
    ...
    def test_decompose_task_recursive(self):
        ...
        async def run_test():
            result = await self.decomposer._decompose_task(
                parent_task=mock_parent, project=mock_project, level=2, max_level=3
            )

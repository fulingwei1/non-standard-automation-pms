...
class TestTaskToDict:
    ...
    def test_task_to_dict_with_hours(self):
        ...
        task = TaskUnified(id=100, title="测试任务", task_type="DEVELOP", actual_hours=40.5)

        result = decomposer._task_to_dict(task)

        assert result == {"id": 100, "name": "测试任务", "type": "DEVELOP", "effort_hours": 40.5}

    def test_task_to_dict_without_hours(self):
        """测试无工时的任务转换"""
        decomposer = AIWbsDecomposer(db=MagicMock())

        task = TaskUnified(id=101, title="无工时任务", task_type="REVIEW", actual_hours=None)
...
class TestGenerateFallbackSubtasks:
    ...
    def test_requirement_task_fallback(self):
        ...
        parent_task = AIWbsSuggestion(
            id=1, task_name="需求分析阶段", task_type="ANALYSIS", estimated_duration_days=15
        )
    ...
    def test_design_task_fallback(self):
        ...
        parent_task = AIWbsSuggestion(task_name="系统设计", task_type="DESIGN")
    ...
    def test_generic_task_fallback(self):
        ...
        parent_task = AIWbsSuggestion(
            task_name="测试任务", task_type="TEST", estimated_duration_days=10
        )
    ...
    def test_fallback_with_null_duration(self):
        ...
        parent_task = AIWbsSuggestion(task_name="未知工期任务", estimated_duration_days=None)
...
class TestCreateSuggestionFromData:
    ...
    def test_create_suggestion_basic(self):
        ...
        parent_task = AIWbsSuggestion(id=10, wbs_code="1.2", template_id=20)
...
class TestDecomposeTask:
    ...
    async def test_decompose_task_max_level_reached(self):
        ...
        result = await decomposer._decompose_task(parent_task, project, level=5, max_level=3)
    ...
    async def test_decompose_task_with_ai(self):
        ...
        result = await decomposer._decompose_task(parent_task, project, level=2, max_level=2)
    ...
    async def test_decompose_task_fallback(self):
        ...
        parent_task = AIWbsSuggestion(
            id=20, task_name="需求分析", task_type="ANALYSIS", wbs_code="1"
        )
        project = Project(id=3)
    ...
    async def test_decompose_task_recursive(self):
        ...
        parent_task = AIWbsSuggestion(
            id=30, task_name="顶级任务", wbs_code="1", complexity="COMPLEX"
        )
        ...
        result = await decomposer._decompose_task(parent_task, project, level=2, max_level=3)
...
class TestDecomposeProject:
    ...
    async def test_decompose_project_basic(self, mock_decompose):
        ...
        result = await decomposer.decompose_project(project_id=100, max_level=2)
    ...
    async def test_decompose_project_with_template(self, mock_decompose):
        ...
        result = await decomposer.decompose_project(project_id=101, template_id=50, max_level=1)
    ...
    async def test_decompose_project_max_level(self, mock_decompose):
        ...
        result = await decomposer.decompose_project(project_id=102, max_level=1)
...
class TestEdgeCases:
    ...
    def test_complex_wbs_code_generation(self):
        ...
        result = decomposer._create_suggestion_from_data(data, project, parent, level=4, sequence=7)
    ...
    def test_json_serialization(self):
        ...
        result = decomposer._create_suggestion_from_data(data, project, parent, level=2, sequence=1)

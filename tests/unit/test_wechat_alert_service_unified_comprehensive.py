...
class TestSendShortageAlert:
    ...
    def test_delegates_to_unified_service(self):
        ...
        with patch("app.services.wechat_alert_service_unified.NotificationService") as mock_service:
            ...
            result = WeChatAlertService.send_shortage_alert(
                mock_db, mock_shortage, alert_level="L3"
            )

            mock_unified.send_shortage_alert.assert_called_once_with(mock_db, mock_shortage, "L3")
    ...
    def test_uses_default_alert_level(self):
        ...
        with patch("app.services.wechat_alert_service_unified.NotificationService") as mock_service:
            ...
            result = WeChatAlertService.send_shortage_alert(mock_db, mock_shortage)

            mock_unified.send_shortage_alert.assert_called_once_with(mock_db, mock_shortage, "L4")
...
class TestBatchSendAlerts:
    ...
    def test_returns_result_dict(self):
        ...
        with patch("app.services.wechat_alert_service_unified.NotificationService") as mock_service:
            ...
            mock_db = MagicMock()
            mock_unified = MagicMock()
            expected_result = {"sent": 3, "failed": 1, "errors": ["error1"]}

...
class TestGetSalespersonHistoricalWinRate:
    ...
    def test_different_lookback_months(self):
        ...
        # 12个月
        rate2, count2 = get_salesperson_historical_win_rate(
            service, salesperson_id=1, lookback_months=12
        )

        # 6个月
        rate3, count3 = get_salesperson_historical_win_rate(
            service, salesperson_id=1, lookback_months=6
        )
...
class TestGetCustomerCooperationHistory:
    ...
    def test_prefer_id_over_name(self):
        ...
        total, won = get_customer_cooperation_history(
            service, customer_id=42, customer_name="Test Customer"
        )
    ...
    def test_different_score_tolerance(self):
        ...
        for tolerance in [5, 10, 15, 20]:
            count, rate = get_similar_leads_statistics(
                service, dimension_scores, score_tolerance=tolerance
            )
            # 无数据时都返回0
            assert count == 0
            assert rate == 0
...
class TestEdgeCases:
    ...
    def test_customer_with_special_name(self):
        ...
        total, won = get_customer_cooperation_history(
            service, customer_name="客户名称 & <special> 'chars'"
        )

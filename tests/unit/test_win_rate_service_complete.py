...
class TestPredictWinRate:
    ...
    def test_predict_basic(self):
        ...
        result = run_async(
            service.predict_win_rate(presale_ticket_id=1, ticket_data=ticket_data, created_by=5)
        )
    ...
    def test_predict_with_historical_data(self):
        ...
        result = run_async(
            service.predict_win_rate(
                presale_ticket_id=2, ticket_data={"test": "data"}, created_by=3
            )
        )
    ...
    def test_predict_error_handling(self):
        ...
        with pytest.raises(Exception, match="AI Error"):
            run_async(service.predict_win_rate(presale_ticket_id=1, ticket_data={}, created_by=1))
    ...
    def test_predict_creates_history_record(self):
        ...
        run_async(
            service.predict_win_rate(
                presale_ticket_id=3, ticket_data={"amount": 50000}, created_by=7
            )
        )
...
class TestGetInfluencingFactors:
    ...
    def test_get_factors_top_5(self):
        ...
        prediction = MagicMock()
        prediction.influencing_factors = [
            {"factor": f"factor_{i}", "score": 10 - i} for i in range(10)
        ]
...
class TestUpdateActualResult:
    ...
    def test_update_result_calculates_error(self):
        ...
        run_async(
            service.update_actual_result(
                ticket_id=1, actual_result=WinRateResultEnum.WON, updated_by=1
            )
        )
    ...
    def test_update_result_correct_prediction_low(self):
        ...
        run_async(
            service.update_actual_result(
                ticket_id=1, actual_result=WinRateResultEnum.LOST, updated_by=1
            )
        )
    ...
    def test_update_result_not_found(self):
        ...
        with pytest.raises(ValueError, match="未找到工单"):
            run_async(
                service.update_actual_result(
                    ticket_id=999, actual_result=WinRateResultEnum.WON, updated_by=1
                )
            )
    ...
    def test_update_result_pending_no_calculation(self):
        ...
        run_async(
            service.update_actual_result(
                ticket_id=1, actual_result=WinRateResultEnum.PENDING, updated_by=1
            )
        )

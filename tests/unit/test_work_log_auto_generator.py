...
class TestWorkLogAutoGeneratorCore(unittest.TestCase):
    ...
    def test_generate_work_log_success(self):
        ...
        result = self.generator.generate_work_log_from_timesheet(
            user_id=self.test_user_id, work_date=self.test_work_date, auto_submit=False
        )
    ...
    def test_generate_work_log_auto_submit(self):
        ...
        # 执行（启用自动提交）
        result = self.generator.generate_work_log_from_timesheet(
            user_id=self.test_user_id, work_date=self.test_work_date, auto_submit=True
        )
    ...
    def test_generate_work_log_existing_submitted(self):
        ...
        # 执行
        result = self.generator.generate_work_log_from_timesheet(
            user_id=self.test_user_id, work_date=self.test_work_date
        )
    ...
    def test_batch_generate_work_logs_success(self):
        ...
        self.db.query.return_value.filter.return_value.all.return_value = [mock_user1, mock_user2]
        ...
        self.generator.generate_work_log_from_timesheet = MagicMock(side_effect=mock_generate)

        # 执行
        stats = self.generator.batch_generate_work_logs(
            start_date=start_date, end_date=end_date, user_ids=user_ids, auto_submit=False
        )
    ...
    def test_batch_generate_work_logs_all_users(self):
        ...
        stats = self.generator.batch_generate_work_logs(
            start_date=start_date, end_date=end_date, user_ids=None, auto_submit=False
        )
    ...
    def test_batch_generate_work_logs_date_range(self):
        ...
        self.assertEqual(self.generator.generate_work_log_from_timesheet.call_count, 6)
    ...
    def test_generate_yesterday_work_logs(self):
        ...
        self.generator.batch_generate_work_logs = MagicMock(return_value=expected_stats)

        # 执行
        stats = self.generator.generate_yesterday_work_logs(
            user_ids=[1, 2, 3, 4, 5], auto_submit=True
        )

        # 验证调用参数
        yesterday = date.today() - timedelta(days=1)
        self.generator.batch_generate_work_logs.assert_called_once_with(
            start_date=yesterday, end_date=yesterday, user_ids=[1, 2, 3, 4, 5], auto_submit=True
        )
    ...
    def test_generate_yesterday_work_logs_all_users(self):
        ...
        # 执行（user_ids=None）
        stats = self.generator.generate_yesterday_work_logs(user_ids=None, auto_submit=False)

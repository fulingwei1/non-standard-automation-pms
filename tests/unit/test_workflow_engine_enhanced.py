...
测试覆盖：
- WorkflowEngine 核心方法（实例创建、节点获取、条件评估、审批提交等）
- ApprovalFlowResolver 流程解析
- ApprovalRouter 路由逻辑
- 边界条件和异常场景

**Mock策略**：只mock外部依赖，构造真实数据对象让业务逻辑真正执行
"""
...
class TestWorkflowEngineInit(TestWorkflowEngine):
    """测试初始化"""
...
class TestCreateInstance(TestWorkflowEngine):
    """测试创建审批实例"""
    ...
    def test_create_instance_success(self):
        ...
        flow.nodes = [
            ApprovalNode(
                id=1, flow_id=1, node_code="N1", node_name="技术审批", sequence=1, role_type="USER"
            ),
            ApprovalNode(
                id=2, flow_id=1, node_code="N2", node_name="财务审批", sequence=2, role_type="USER"
            ),
        ]
        ...
        self.assertEqual(instance.business_title, "ECN-001")
    ...
    def test_create_instance_flow_not_found(self):
        ...
        self.assertIn("不存在或未启用", str(context.exception))
    ...
    def test_create_instance_with_config(self):
        ...
        flow.nodes = [
            ApprovalNode(
                id=1, flow_id=1, node_code="N1", node_name="审批", sequence=1, role_type="USER"
            )
        ]
...
class TestGetCurrentNode(TestWorkflowEngine):
    """测试获取当前审批节点"""
    ...
    def test_get_current_node_with_current_node_id(self):
        ...
        node = ApprovalNode(
            id=10, flow_id=1, node_code="N1", node_name="审批", sequence=1, role_type="USER"
        )
...
class TestEvaluateNodeConditions(TestWorkflowEngine):
    """测试节点条件评估"""
    ...
    @patch("app.services.approval_engine.condition_parser.ConditionEvaluator")
    def test_evaluate_node_conditions_generic_error(self, mock_evaluator_class):
        """测试通用异常时返回 True（默认允许）"""
        mock_evaluator = MagicMock()
...
class TestBuildConditionContext(TestWorkflowEngine):
    """测试构建条件评估上下文"""
    ...
    def test_build_condition_context_with_form_data(self):
        ...
        instance.form_data = {"amount": 5000, "priority": "high"}
...
class TestGetBusinessEntityData(TestWorkflowEngine):
    """测试获取业务实体数据"""
...
class TestSubmitApproval(TestWorkflowEngine):
    """测试提交审批决策"""
    ...
    def test_submit_approval_success_approved(self):
        ...
        def mock_query_side_effect(model):
            ...
            if model == ApprovalNode:
                # 第一次调用：获取当前节点
                # 第二次调用：查找下一节点
                mock_q.filter.return_value.first.side_effect = [node, None]
    ...
    def test_submit_approval_no_node(self):
        ...
        self.assertIn("没有可审批的节点", str(context.exception))
    ...
    def test_submit_approval_condition_not_met(self, mock_evaluate):
        ...
        self.assertIn("不满足审批条件", str(context.exception))


class TestUpdateInstanceStatus(TestWorkflowEngine):
    """测试更新实例状态"""
...
class TestFindNextNode(TestWorkflowEngine):
    """测试查找下一个节点"""
    ...
    def test_find_next_node_exists(self):
        ...
        found_node = self.engine._find_next_node(current_node)

        self.assertIsNotNone(found_node)
        self.assertEqual(found_node.id, 2)
...
class TestFindPreviousNode(TestWorkflowEngine):
    """测试查找上一个节点"""
    ...
    def test_find_previous_node_exists(self):
        ...
        found_node = self.engine._find_previous_node(current_node)

        self.assertIsNotNone(found_node)
        self.assertEqual(found_node.id, 1)
...
class TestIsExpired(TestWorkflowEngine):
    """测试是否超时"""
    ...
    def test_is_expired_with_created_at_expired(self):
        """测试通过 created_at 判断已超时（无 due_date 时）"""
        instance = ApprovalInstance(
            id=1,
            flow_id=1,
            due_date=None,
...
class TestApprovalFlowResolver(unittest.TestCase):
    ...
    def test_determine_approval_flow(self):
        """测试根据业务类型确定流程编码"""
...
class TestApprovalRouter(unittest.TestCase):
    ...
    def test_determine_approval_flow_ecn(self):
        """测试 ECN 业务类型确定流程"""
        business_data = {"business_id": 100}
        result = self.router.determine_approval_flow("ECN", business_data)

        self.assertEqual(result, "ECN_STANDARD")

    def test_determine_approval_flow_sales_invoice_single(self):
        """测试销售发票金额 < 50000 使用单级流程"""
        business_data = {"amount": 30000}
        result = self.router.determine_approval_flow("SALES_INVOICE", business_data)
    ...
    def test_determine_approval_flow_sales_quote(self):
        """测试销售报价使用单级流程"""
        business_data = {}
        result = self.router.determine_approval_flow("SALES_QUOTE", business_data)
    ...
    def test_determine_approval_flow_unknown(self):
        """测试未知业务类型返回 None"""
        business_data = {}
        result = self.router.determine_approval_flow("UNKNOWN_TYPE", business_data)

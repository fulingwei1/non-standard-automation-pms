...
print("=" * 60)
print("ECN 状态机实现验证（简化版）")
print("=" * 60)

# Test 1: Import StateMachine
try:
...
try:
    # Reset model
    model.status = "DRAFT"
    model.change_reason = "测试变更"
    model.change_description = "测试描述"
    model.ecn_type = "DC"
    ...
    model.approved_at = datetime.now()
    model.execution_start = datetime.now()

    # DRAFT → PENDING_REVIEW → APPROVED → IMPLEMENTED
    sm.transition_to("PENDING_REVIEW")
    sm.transition_to("APPROVED")
    sm.transition_to("IMPLEMENTED")

    # Check final state
    assert sm.current_state == "IMPLEMENTED"

    # Check history
    history = sm.get_transition_history()
    assert len(history) == 3

    print("✓ Test 9: 完整流程测试成功")
    print(f"   最终状态: {sm.current_state}")
    print(f"   总转换次数: {len(history)}")
    workflow_success = True
except Exception as e:
    print(f"✗ Test 9: 完整流程测试失败 - {e}")
